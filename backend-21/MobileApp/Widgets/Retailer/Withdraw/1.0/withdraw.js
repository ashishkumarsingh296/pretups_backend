...
var WITHDRAW_TYPE = widget.widgetProperty ("TYPE") ;
...
function withdrawmenu()
{
	...
	else
	{
		...
		formPage += "<div id='withdraw' class='c3lMenuGroup'>";
	...
	divElement= document.getElementById("withdraw");
...
function sendWithdrawReq(mobNumber,Amount,Pin)
{
	...
	widget.logWrite(7,"url for sendWithdrawReq request::"+url);
	if(SEND_ENCRYPTREQ)
	{
		postData = "TYPE="+WITHDRAW_TYPE+"&MSISDN="+mobile+"&Message="+getEncrypt("IMEI="+imei+"&PIN="+Pin+"&MSISDN2="+mobNumber+"&AMOUNT="+RechargeAmount+"&SELECTOR="+SELECTOR);

	}else
	{		
		postData = "TYPE="+WITHDRAW_TYPE+"&MSISDN="+mobile+"&IMEI="+imei+"&PIN="+Pin+"&MSISDN2="+mobNumber+"&AMOUNT="+RechargeAmount+"&LANGUAGE1="+LANGUAGE1+"&LANGUAGE2="+LANGUAGE2;
	}

	widget.logWrite(7," sendWithdrawReq postdata ::"+postData);
	...
	if (xmlHttp)
	{
		...
		xmlHttp.onreadystatechange = function()
		{
			if (4 == xmlHttp.readyState &&  200 == xmlHttp.status)
			{
				...
				widget.logWrite(7,"response for sendWithdrawReq::"+xmlText);
		...
		cdrStr += changetimeformat()+"| Withdraw";
...
{
	var str="";
	str += "<span>Withdraw</span><br/>Enter Amount<br/>";
...
{
	var str="";
	str += "<span>Withdraw</span><br/>Enter PIN<br/>";
	str += "<div id='main' class=''>";
...
{
	var formPage="";
	formPage += "<span>Withdraw</span><br/>Confirm Mobile No<br/>";
	formPage += "<div id='main' class=''>";
