...
var EVOUCHER_TYPE = widget.widgetProperty ("TYPE") ;
...
function sendElevoucherReq(mobNumber,RechargeAmount,Pin)
{
	...
	widget.logWrite(7,"url for electronic voucher request::"+url);
	if(SEND_ENCRYPTREQ)
	{
		postData = "TYPE="+EVOUCHER_TYPE+"&MSISDN="+mobile+"&Message="+getEncrypt("IMEI="+imei+"&PIN="+Pin+"&MSISDN2="+mobNumber+"&AMOUNT="+RechargeAmount+"&SELECTOR="+SELECTOR);

	}else
	{		
		postData = "TYPE="+EVOUCHER_TYPE+"&MSISDN="+mobile+"&IMEI="+imei+"&PIN="+Pin+"&MSISDN2="+mobNumber+"&AMOUNT="+RechargeAmount+"&LANGUAGE1="+LANGUAGE1+"&LANGUAGE2="+LANGUAGE2+"&SELECTOR="+SELECTOR;
	}

	widget.logWrite(7," Request for electronic voucher ::"+postData);
	...
	if (xmlHttp)
	{
		...
		xmlHttp.onreadystatechange = function()
		{
			if (4 == xmlHttp.readyState &&  200 == xmlHttp.status)
			{
				...
				widget.logWrite(7,"response for electronic voucher::"+xmlText);
