...
function sendchangepinreq(pin, newpin, confirmpin) {
	...
	}else
	{
		...
		if (newpin == confirmpin) {
			...
			if(SEND_ENCRYPTREQ)
			{
				postData = "TYPE="+CHANGEPIN_TYPE+"&MSISDN="+mobile+"&Message="+getEncrypt("IMEI="+imei+"&NEWPIN="+newpin+"&CONFIRMPIN="+confirmpin+"&PIN="+pin+"&LANGUAGE1="+langCode);
			}else
			{
				postData = "TYPE="+CHANGEPIN_TYPE+"&MSISDN="+mobile+"&IMEI="+imei+"&OLDPIN="+pin+"&NEWPIN="+newpin+"&CONFIRMPIN="+confirmpin+"&LANGUAGE1="+langCode;
			}
