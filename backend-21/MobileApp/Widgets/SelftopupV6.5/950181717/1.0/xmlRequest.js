...
function SubscriberRegReq(mobile,pin,imei,email)
{
       widget.logWrite(7,"IMEI::"+imei);
	...
	{      //TYPE,IMEI,MSISDN,EMAILID,LANGUAGE1

		//url = url+"&TYPE="+SUBS_REG_TYPE+"&MSISDN="+DEF_MSISDN+"&UCODE="+ucode+"&IMEI="+imei;
              //postData= "TYPE="+SUB_TYPE+"&MSISDN="+mobile+"&IMEI="+imei+"&EMAIL="+email+"&PIN="+pin;
              postData="TYPE="+SUB_TYPE+"&IMEI="+imei+"&MSISDN="+mobile+"&EMAILID="+email+"&LANGUAGE1="+LANGUAGE1;
	}
	...
	if (xmlHttp)
	{
		...
		{
                  widget.logWrite(7,"response for ajax::"+xmlHttp.status);
			...
			if (4 == xmlHttp.readyState &&  200 == xmlHttp.status)
			{
				...
				if (xmlText != null && !xmlText.indexOf("null") > -1 )
				{
					...
					widget.logWrite(7,"store msisdn=================.."+mobile);
...
widget.logWrite(7,"google plus tokenID**************************" +tokenID);
...
widget.logWrite(7,"google plus login**************************");
    ...
    widget.logWrite(7,"google plus responseText**************************" +xmlHttpReg.responseText);

    obj = eval('(' + xmlHttpReg.responseText + ')');

    var tokenExpiry = obj.expires_in;
    var email = obj.email;
     widget.logWrite(7,"google plus email**************************" +email);
    ...
    if (tokenExpiry != null && tokenExpiry != undefined && tokenExpiry != "undefined") {
      ...
      widget.logWrite(7,"google plus googlepluspeopleInfoUrl **************************" +googlepluspeopleInfoUrl);
        ...
        widget.logWrite(7,"google plus postdata**************************" +postData);
	    ...
	    cdrcommon(imei,subscriberId,"GOOGLEPLUS LOGIN","","","GOOGLEPLUS LOGIN SUCCESSFULL","","200");
            window.location = "wgt:264711061/1.0";
