...
import angular.feature.LowThresholdTransactionReportPageRevamp;
...
@ModuleManager(name = Module.PREREQUISITE_LOWTHRESHOLDTRANSACTIONREPORTPAGE_REVAMP)
public class PreRequisite_LowThresholdTransactionReportPage_Revamp extends BaseTest {
	
    public PreRequisite_LowThresholdTransactionReportPage_Revamp() {
...
//  @TestManager(TestKey = "PRETUPS-001") // TO BE UNCOMMENTED WITH JIRA TEST CASE
    public void TC_01_Test_LoginLogout(String ParentCategory, String FromCategory, String PIN, String service) throws InterruptedException {
    	...
    	CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PREVAMPLOWTTREPORT1");
        ...
        if (CommonUtils.roleCodeExistInLinkSheetRevamp(RolesI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS, FromCategory, EventsI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS_REVAMP)) {
            String webAccessAllowed = DBHandler.AccessHandler.webInterface(FromCategory);
            LowThresholdTransactionReportPageRevamp LowThresholdTransactionReportPage = new LowThresholdTransactionReportPageRevamp(driver);

            if (webAccessAllowed.equals("Y")) {
            	LowThresholdTransactionReportPage.performLoginLogout(ParentCategory, FromCategory, PIN, service);
            } else {
                Assertion.assertSkip("As webaccess is not allowed for " + FromCategory + ", case is skipped.");
            }
        } else {
            Assertion.assertSkip("LowThresholdTransactionReportPage is not allowed to category[" + FromCategory + "].");
          }
...
//  @TestManager(TestKey = "PRETUPS-002") // TO BE UNCOMMENTED WITH JIRA TEST CASE
    public void TC_02_Test_VerifyHidelink(String ParentCategory, String FromCategory, String PIN, String service) throws InterruptedException {
    	...
    	CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PREVAMPLOWTTREPORT2");
    	currentNode = test.createNode(MessageFormat.format(CaseMaster.getDescription(), FromCategory)).assignCategory(TestCategory.PREREQUISITE);
    	...
    	if (CommonUtils.roleCodeExistInLinkSheetRevamp(RolesI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS, FromCategory, EventsI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS_REVAMP)) {
            ...
            if (webAccessAllowed.equals("Y")) {
            	LowThresholdTransactionReportPage.performHidelink(ParentCategory, FromCategory, PIN, service);
            } else {
                Assertion.assertSkip("As webaccess is not allowed for " + FromCategory + ", case is skipped.");
            }
        } else {
...
//  @TestManager(TestKey = "PRETUPS-003") // TO BE UNCOMMENTED WITH JIRA TEST CASE
    public void TC_03_Test_VerifyProceedButton(String ParentCategory, String FromCategory, String PIN, String service) throws InterruptedException {
    	...
    	CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PREVAMPLOWTTREPORT3");
    	currentNode = test.createNode(MessageFormat.format(CaseMaster.getDescription(), FromCategory)).assignCategory(TestCategory.PREREQUISITE);
        ...
        if (CommonUtils.roleCodeExistInLinkSheetRevamp(RolesI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS, FromCategory, EventsI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS_REVAMP)) {
            ...
            if (webAccessAllowed.equals("Y")) {
            	LowThresholdTransactionReportPage.performProceedButton(ParentCategory, FromCategory, PIN, service);
            } else {
                Assertion.assertSkip("As webaccess is not allowed for " + FromCategory + ", case is skipped.");
            }
        } else {
...
//  @TestManager(TestKey = "PRETUPS-004") // TO BE UNCOMMENTED WITH JIRA TEST CASE
    public void TC_04_Test_SelectFromdateTodate(String ParentCategory, String FromCategory, String PIN, String service) throws InterruptedException {
    	...
    	CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PREVAMPLOWTTREPORT4");
    	currentNode = test.createNode(MessageFormat.format(CaseMaster.getDescription(), FromCategory)).assignCategory(TestCategory.PREREQUISITE);
        ...
        if (CommonUtils.roleCodeExistInLinkSheetRevamp(RolesI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS, FromCategory, EventsI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS_REVAMP)) {
            ...
            if (webAccessAllowed.equals("Y")) {
            	LowThresholdTransactionReportPage.performSelectFromdateTodate(ParentCategory, FromCategory, PIN, service);
            } else {
                Assertion.assertSkip("As webaccess is not allowed for " + FromCategory + ", case is skipped.");
            }
        } else {
...
//  @TestManager(TestKey = "PRETUPS-005") // TO BE UNCOMMENTED WITH JIRA TEST CASE
    public void TC_05_Test_VerifyProductcode(String ParentCategory, String FromCategory, String PIN, String service) throws InterruptedException {
    	...
    	CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PREVAMPLOWTTREPORT5");
    	currentNode = test.createNode(MessageFormat.format(CaseMaster.getDescription(), FromCategory)).assignCategory(TestCategory.PREREQUISITE);
        ...
        if (CommonUtils.roleCodeExistInLinkSheetRevamp(RolesI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS, FromCategory, EventsI.LOWTHRESHOLDTRANSACTIONREPORT_DETAILS_REVAMP)) {
            ...
            if (webAccessAllowed.equals("Y")) {
            	LowThresholdTransactionReportPage.performVerifyProductcode(ParentCategory, FromCategory, PIN, service);
            } else {
                Assertion.assertSkip("As webaccess is not allowed for " + FromCategory + ", case is skipped.");
            }
        } else {
