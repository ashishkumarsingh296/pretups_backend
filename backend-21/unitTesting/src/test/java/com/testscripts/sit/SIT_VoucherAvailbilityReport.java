...
public class SIT_VoucherAvailbilityReport extends BaseTest {


	 @Test
	    @TestManager(TestKey = "PRETUPS-404") // TO BE UNCOMMENTED BY WITH JIRA TEST CASE ID
	    public void Test_CreateVoucherAvailbilityReport() throws InterruptedException {
	       ...
	       for(int i=0;i<Array.size();i++) {
	    	   currentNode = test.createNode(MessageFormat.format(_masterVO.getCaseMasterByID("VAR1").getExtentCase(), Array.get(i)))
		                .assignCategory(TestCategory.SIT);
	        o2cTrans.voucherAvailbilityReport(Array.get(i));
	       }
	    ...
	    public void ScriptExecution()
		{
			...
			SSHService.executeScript("VoucherAvailabilityReport.sh");
