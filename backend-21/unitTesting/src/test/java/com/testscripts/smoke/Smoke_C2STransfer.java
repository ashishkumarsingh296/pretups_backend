...
@ModuleManager(name = Module.SMOKE_C2S_TRANSFER)
public class Smoke_C2STransfer extends BaseTest {

    @Test(dataProvider = "categoryData")
     @TestManager(TestKey = "PRETUPS-503") // TO BE UNCOMMENTED WITH JIRA TEST CASE ID
    public void Test_C2SRecharge(String FromParent, String FromCategory, String PIN, String service)
            throws IOException, InterruptedException {
        final String methodName = "Test_C2SRecharge";
        Log.startTestCase(methodName, FromParent, FromCategory, PIN, service);

        CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PC2STRF1");
        currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(), FromCategory)).assignCategory(TestCategory.SMOKE);
