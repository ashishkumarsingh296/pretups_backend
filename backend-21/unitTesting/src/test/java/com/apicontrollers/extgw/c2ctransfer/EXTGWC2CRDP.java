...
public class EXTGWC2CRDP {
	...
	public static String FROM_TCP_ID = null;
	...
	public static HashMap<String, String> getAPIdata() {
		...
		for (userCounter = 1; userCounter <= sheetRowCounter; userCounter++) {
			...
			String toCategory = ExcelUtility.getCellData(0, ExcelI.FROM_CATEGORY, userCounter);
			String fromCategory = ExcelUtility.getCellData(0, ExcelI.TO_CATEGORY, userCounter);
		...
		for (int userDetailsCounter = 1; userDetailsCounter <= sheetRowCounter; userDetailsCounter++) {
			...
			if (sheetCategory.equals(fromCategory)) {
				...
				if (FROMUser_WEBAccessStatus.equalsIgnoreCase("Y")) {
					...
					//apiData.put(C2CTransferAPI.PASSWORD, Decrypt.APIEncryption(ExcelUtility.getCellData(0, ExcelI.PASSWORD, userDetailsCounter)));
				}
				...
				///changed MSIDN2 to MSIDN1 in above line
				break;
			}
		}
		...
		for (int userDetailsCounter = 1; userDetailsCounter <= sheetRowCounter; userDetailsCounter++) {
			...
			if (sheetCategory.equals(toCategory)) {
				...
				FROM_TCP_ID = ExcelUtility.getCellData(0, ExcelI.NA_TCP_PROFILE_ID, userDetailsCounter);
