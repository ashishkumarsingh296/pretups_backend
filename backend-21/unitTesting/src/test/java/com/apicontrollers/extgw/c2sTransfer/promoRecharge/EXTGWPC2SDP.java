...
public class EXTGWPC2SDP {
	...
	public static HashMap<String, String> getAPIdata() throws Exception {
		...
		EXTGWPC2SAPI PromoC2STransferAPI = new EXTGWPC2SAPI();
		 ...
		 */
		apiData.put(PromoC2STransferAPI.EXTNWCODE, _masterVO.getMasterValue(MasterI.NETWORK_CODE));
		...
		for (int i = 0; i<=dataRowCounter;i++) {
			...
			if (excelCategory.equals(channelUserCategory)) {
				apiData.put(PromoC2STransferAPI.MSISDN, ExcelUtility.getCellData(0, ExcelI.MSISDN, i));
				...
				apiData.put(PromoC2STransferAPI.LOGINID, ExcelUtility.getCellData(0, ExcelI.LOGIN_ID, i));
				apiData.put(PromoC2STransferAPI.PASSWORD, _APIUtil.implementEncryption(ExcelUtility.getCellData(0, ExcelI.PASSWORD, i)));
				apiData.put(PromoC2STransferAPI.EXTCODE, DBHandler.AccessHandler.getUserDetails(LoginID, "EXTERNAL_CODE")[0]);
				break;
			}
		}

		apiData.put(PromoC2STransferAPI.EXTREFNUM, RandomGeneration.randomNumeric(10));
		apiData.put(PromoC2STransferAPI.DATE, _APIUtil.getCurrentTimeStamp());
		...
		for (int k = 1; k<=rowCount1;k++) {
			...
			if(ExcelProductType.equals(Product_Type)){		
				...
				apiData.put(PromoC2STransferAPI.PRODUCTCODE, ExcelUtility.getCellData(0, ExcelI.PRODUCT_CODE, k));
	...
	public static Object[] getAPIdataWithAllUsers() {
		...
		for (int counter = 0; counter <= dataRowCounter; counter++) {
			...
			if (categoryServices.contains(CustomerRechargeCode) && gatewayType.contains("EXTGW")) {
				...
				apiData.put(PromoC2STransferAPI.EXTNWCODE, _masterVO.getMasterValue(MasterI.NETWORK_CODE));
				apiData.put(PromoC2STransferAPI.AMOUNT, "100");
				apiData.put(PromoC2STransferAPI.EXTREFNUM, RandomGeneration.randomNumeric(7));
