...
public class EXTGW_LiteRCDP extends CaseMaster{
	...
	public static HashMap<String, String> getAPIdata() throws Exception {
		...
		EXTGW_LiteRCAPI EXTGW_LiteRCAPI = new EXTGW_LiteRCAPI();
		 ...
		 */
		apiData.put(EXTGW_LiteRCAPI.EXTNWCODE, _masterVO.getMasterValue(MasterI.NETWORK_CODE));
		...
		for (int i = 0; i<=dataRowCounter;i++) {
			...
			if (excelCategory.equals(channelUserCategory)) {
				apiData.put(EXTGW_LiteRCAPI.MSISDN1, ExcelUtility.getCellData(0, ExcelI.MSISDN, i));
				apiData.put(EXTGW_LiteRCAPI.PIN, _APIUtil.implementEncryption(ExcelUtility.getCellData(0, ExcelI.PIN, i)));
				...
				apiData.put(EXTGW_LiteRCAPI.LOGINID, ExcelUtility.getCellData(0, ExcelI.LOGIN_ID, i));
				apiData.put(EXTGW_LiteRCAPI.PASSWORD, _APIUtil.implementEncryption(ExcelUtility.getCellData(0, ExcelI.PASSWORD, i)));
				apiData.put(EXTGW_LiteRCAPI.EXTCODE, DBHandler.AccessHandler.getUserDetails(LoginID, "EXTERNAL_CODE")[0]);
				break;
			}
		}
		
		apiData.put(EXTGW_LiteRCAPI.EXTREFNUM, RandomGeneration.randomNumeric(10));
		apiData.put(EXTGW_LiteRCAPI.DATE, _APIUtil.getCurrentTimeStamp());

	
		apiData.put(EXTGW_LiteRCAPI.AMOUNT, "100");
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			if (service.equals(CustomerRechargeCode)&& !cardGroupName.isEmpty()) {
				...
				//apiData.put(EXTGW_LiteRCAPI.SELECTOR,ExcelUtility.getCellData(0, ExcelI.SELECTOR_NAME, i));

				apiData.put(EXTGW_LiteRCAPI.SELECTOR,DBHandler.AccessHandler.getSelectorCode(ExcelUtility.getCellData(0, ExcelI.SELECTOR_NAME, i),service));
				break;
			}
		}
		...
		apiData.put(EXTGW_LiteRCAPI.PRODUCTCODE, ExcelUtility.getCellData(0, ExcelI.PRODUCT_CODE, k));
		...
		apiData.put(EXTGW_LiteRCAPI.LANGUAGE1, DBHandler.AccessHandler.checkForLangCode(_masterVO.getMasterValue(MasterI.LANGUAGE)));
		apiData.put(EXTGW_LiteRCAPI.LANGUAGE2, DBHandler.AccessHandler.checkForLangCode(_masterVO.getMasterValue(MasterI.LANGUAGE)));
		

		String prefix = _masterVO.getMasterValue(MasterI.SUBSCRIBER_PREPAID_PREFIX);

		apiData.put(EXTGW_LiteRCAPI.MSISDN2, prefix + RandomGeneration.randomNumeric(gnMsisdn.generateMSISDN()));
		
		
		
		apiData.put(EXTGW_LiteRCAPI.SERVICECLASS,"" );
	...
	public static Object[] getAPIdataWithAllUsers() {
		...
		RandomGeneration RandomGeneration = new RandomGeneration();
		GenerateMSISDN gnMsisdn = new GenerateMSISDN();
		EXTGW_LiteRCAPI C2STransferAPI = new EXTGW_LiteRCAPI();
		/*
		 * Variable initializations
		 */	
		...
		for (int counter = 0; counter <= dataRowCounter; counter++) {
			...
			if (categoryServices.contains(CustomerRechargeCode) && gatewayType.contains("EXTGW")) {
				...
				EXTGW_LITERCDAO APIDataDAO = new EXTGW_LITERCDAO();
				...
				apiData.put(C2STransferAPI.SERVICECLASS,"" );
				...
				apiDataObj[objCounter] = (EXTGW_LITERCDAO) APIDataDAO;
				objCounter++;
			}
		}
