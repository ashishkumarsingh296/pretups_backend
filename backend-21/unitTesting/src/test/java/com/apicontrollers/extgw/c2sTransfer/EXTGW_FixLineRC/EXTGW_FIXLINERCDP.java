...
public class EXTGW_FIXLINERCDP {
	...
	public static HashMap<String, String> getAPIdata() throws Exception {
		...
		EXTGW_FIXLINERCAPI EXTGW_IntRCAPI = new EXTGW_FIXLINERCAPI();
		c2stransferStatusAPI c2stransferStatusAPI = new c2stransferStatusAPI();
		GenerateMSISDN gnMsisdn = new GenerateMSISDN();
		String FixLineRechargeCode = _masterVO.getProperty("CustomerRechargeCode");
		...
		for (int i=0; i <= dataRowCounter; i++) {
			...
			if (alist.contains(FixLineRechargeCode)&& alist1.contains("EXTGW")) {
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			if (service.equals(FixLineRechargeCode)&& !cardGroupName.isEmpty()) {
	...
	public static Object[] getAPIdataWithAllUsers() {
		...
		RandomGeneration RandomGeneration = new RandomGeneration();
		GenerateMSISDN gnMsisdn = new GenerateMSISDN();
		EXTGW_FIXLINERCAPI C2STransferAPI = new EXTGW_FIXLINERCAPI();
		/*
		 * Variable initializations
		 */	
		...
		for (int i=0; i <= dataRowCounter; i++) {
			String categoryServices = ExcelUtility.getCellData(0, ExcelI.SERVICES, i);
			String gatewayType = ExcelUtility.getCellData(0, ExcelI.ACCESS_BEARER, i);
			if (categoryServices.contains(FixLineRechargeCode) && gatewayType.contains("EXTGW")) {
				objSize++;
			}
		}
		...
		for (int counter = 0; counter <= dataRowCounter; counter++) {
			...
			if (categoryServices.contains(FixLineRechargeCode) && gatewayType.contains("EXTGW")) {
				...
				EXTGW_FIXLINERCDAO APIDataDAO = new EXTGW_FIXLINERCDAO();
				...
				apiDataObj[objCounter] = (EXTGW_FIXLINERCDAO) APIDataDAO;
				objCounter++;
			}
		}
