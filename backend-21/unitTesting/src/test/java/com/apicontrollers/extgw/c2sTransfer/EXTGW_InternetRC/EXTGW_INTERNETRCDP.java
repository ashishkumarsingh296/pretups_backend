...
public class EXTGW_INTERNETRCDP {
	...
	public static HashMap<String, String> getAPIdata() throws Exception {
		...
		EXTGW_INTERNETRCAPI EXTGW_IntRCAPI = new EXTGW_INTERNETRCAPI();
		c2stransferStatusAPI c2stransferStatusAPI = new c2stransferStatusAPI();
		GenerateMSISDN gnMsisdn = new GenerateMSISDN();
		String InternetRechargeCode = _masterVO.getProperty("CustomerRechargeCode");
		 ...
		 */
		apiData.put(EXTGW_IntRCAPI.EXTNWCODE, _masterVO.getMasterValue(MasterI.NETWORK_CODE));
		...
		for (int i=0; i <= dataRowCounter; i++) {
			...
			if (alist.contains(InternetRechargeCode)&& alist1.contains("EXTGW")) {
		...
		for (int i = 0; i<=dataRowCounter;i++) {
			...
			if (excelCategory.equals(channelUserCategory)) {
				apiData.put(EXTGW_IntRCAPI.MSISDN, ExcelUtility.getCellData(0, ExcelI.MSISDN, i));
				apiData.put(EXTGW_IntRCAPI.PIN, _APIUtil.implementEncryption(ExcelUtility.getCellData(0, ExcelI.PIN, i)));
				...
				apiData.put(EXTGW_IntRCAPI.LOGINID, ExcelUtility.getCellData(0, ExcelI.LOGIN_ID, i));
				apiData.put(EXTGW_IntRCAPI.PASSWORD, _APIUtil.implementEncryption(ExcelUtility.getCellData(0, ExcelI.PASSWORD, i)));
				apiData.put(EXTGW_IntRCAPI.EXTCODE, DBHandler.AccessHandler.getUserDetails(LoginID, "EXTERNAL_CODE")[0]);
				apiData.put(EXTGW_IntRCAPI.NOTIFICATION_MSISDN,ExcelUtility.getCellData(0, ExcelI.MSISDN, i));
				break;
			}
		}
		
		apiData.put(EXTGW_IntRCAPI.EXTREFNUM, RandomGeneration.randomNumeric(10));
		apiData.put(EXTGW_IntRCAPI.DATE, _APIUtil.getCurrentTimeStamp());

	
		apiData.put(EXTGW_IntRCAPI.AMOUNT, "100");
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			if (service.equals(InternetRechargeCode)&& !cardGroupName.isEmpty()) {

				//apiData.put(EXTGW_IntRCAPI.SELECTOR,ExcelUtility.getCellData(0, ExcelI.SELECTOR_NAME, i));

				apiData.put(EXTGW_IntRCAPI.SELECTOR,DBHandler.AccessHandler.getSelectorCode(ExcelUtility.getCellData(0, ExcelI.SELECTOR_NAME, i),service));
				break;
			}
		}
		...
		//apiData.put(EXTGW_IntRCAPI.PRODUCTCODE, ExcelUtility.getCellData(0, ExcelI.PRODUCT_CODE, k));
		...
		apiData.put(EXTGW_IntRCAPI.LANGUAGE1, DBHandler.AccessHandler.checkForLangCode(_masterVO.getMasterValue(MasterI.LANGUAGE)));
		apiData.put(EXTGW_IntRCAPI.LANGUAGE2, DBHandler.AccessHandler.checkForLangCode(_masterVO.getMasterValue(MasterI.LANGUAGE)));
		

		String prefix = _masterVO.getMasterValue(MasterI.SUBSCRIBER_PREPAID_PREFIX);

		apiData.put(EXTGW_IntRCAPI.MSISDN2, prefix + RandomGeneration.randomNumeric(gnMsisdn.generateMSISDN()));
	...
	public static Object[] getAPIdataWithAllUsers() {
		...
		RandomGeneration RandomGeneration = new RandomGeneration();
		GenerateMSISDN gnMsisdn = new GenerateMSISDN();
		EXTGW_INTERNETRCAPI C2STransferAPI = new EXTGW_INTERNETRCAPI();
		...
		for (int i=0; i <= dataRowCounter; i++) {
			String categoryServices = ExcelUtility.getCellData(0, ExcelI.SERVICES, i);
			String gatewayType = ExcelUtility.getCellData(0, ExcelI.ACCESS_BEARER, i);
			if (categoryServices.contains(InternetRechargeCode) && gatewayType.contains("EXTGW")) {
				objSize++;
			}
		}
		...
		for (int counter = 0; counter <= dataRowCounter; counter++) {
			...
			if (categoryServices.contains(InternetRechargeCode) && gatewayType.contains("EXTGW")) {
				...
				EXTGW_INTERNETRCDAO APIDataDAO = new EXTGW_INTERNETRCDAO();
				...
				apiDataObj[objCounter] = (EXTGW_INTERNETRCDAO) APIDataDAO;
				objCounter++;
			}
		}
