...
public class USSDGRCDP extends CaseMaster{
	...
	for (int i=0; i <= dataRowCounter; i++) {
		...
		if (categoryServices.contains(GiftRechargeCode) && gatewayType.contains("USSD")) {
	...
	apiData.put(C2STransferAPI.PRODUCT_CODE, ExcelUtility.getCellData(0, ExcelI.PRODUCT_CODE, k));
	...
	apiData.put(C2STransferAPI.GIFTER_LANGUAGE, DBHandler.AccessHandler.checkForLangCode(_masterVO.getMasterValue(MasterI.LANGUAGE)));
	...
	public static Object[] getAPIdataWithAllUsers() {
		...
		for (int counter = 0; counter <= dataRowCounter; counter++) {
			...
			if (categoryServices.contains(CustomerRechargeCode) && gatewayType.contains("EXTGW")) {
				...
				USSD_GRCDAO APIDataDAO = new USSD_GRCDAO();
				...
				apiDataObj[objCounter] = (USSD_GRCDAO) APIDataDAO;
				objCounter++;
			}
		}
