...
public class USSD_CRInternet_DP extends CaseMaster {
    ...
    public static HashMap<String, String> getAPIdata() {
		...
		USSD_CRInternet_API internetRechargeAPI = new USSD_CRInternet_API();
		...
		for (int i = 0; i<=dataRowCounter;i++) {
			...
			if (excelCategory.equals(channelUserCategory)) {
				apiData.put(internetRechargeAPI.MSISDN1, ExcelUtility.getCellData(0, ExcelI.MSISDN, i));
				apiData.put(internetRechargeAPI.PIN, _APIUtil.implementEncryption(ExcelUtility.getCellData(0, ExcelI.PIN, i)));
				apiData.put(internetRechargeAPI.NOTIFICATION_MSISDN, ExcelUtility.getCellData(0, ExcelI.MSISDN, i));
				break;
			}
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			if (service.equals(CustomerRechargeCode)&& !cardGroupName.isEmpty()) {
                apiData.put(internetRechargeAPI.SELECTOR,DBHandler.AccessHandler.getSelectorCode(ExcelUtility.getCellData(0, ExcelI.SELECTOR_NAME, i),service));
				break;
			}
		}
		
		
		apiData.put(internetRechargeAPI.MSISDN2, _masterVO.getMasterValue(MasterI.SUBSCRIBER_PREPAID_PREFIX) + gnMsisdn.generateMSISDN());
		apiData.put(internetRechargeAPI.LANGUAGE1, DBHandler.AccessHandler.checkForLangCode(LangCode));
		apiData.put(internetRechargeAPI.LANGUAGE2, DBHandler.AccessHandler.checkForLangCode(LangCode));
		apiData.put(internetRechargeAPI.AMOUNT, "200");
		return apiData;
		}
