...
public class USSDPlain_Withdraw_DP extends CaseMaster {
	...
	public static HashMap<String, String> getAPIdata() {
		...
		String transactionType = _masterVO.getProperty("C2CWithdrawCode");
		...
		for (int userDetailsCounter = 1; userDetailsCounter <= sheetRowCounter; userDetailsCounter++) {
			...
			if (sheetCategory.equals(fromCategory)) {
				...
				apiData.put(withdrawAPI.MSISDN1, ExcelUtility.getCellData(0, ExcelI.MSISDN, userDetailsCounter));
				apiData.put(withdrawAPI.PIN, ExcelUtility.getCellData(0, ExcelI.PIN, userDetailsCounter));
				break;
			}
		}
		...
		for (int userDetailsCounter = 1; userDetailsCounter <= sheetRowCounter; userDetailsCounter++) {
			...
			apiData.put(withdrawAPI.MSISDN2, ExcelUtility.getCellData(0, ExcelI.MSISDN, userDetailsCounter));
			break;
		}
		}
		...
		apiData.put(withdrawAPI.PRODUCTCODE, ExcelUtility.getCellData(0, ExcelI.PRODUCT_SHORT_CODE, 1));
		
		apiData.put(withdrawAPI.TOPUPVALUE, "200");
		apiData.put(withdrawAPI.LANGUAGE1, DBHandler.AccessHandler.checkForLangCode(LangCode));
