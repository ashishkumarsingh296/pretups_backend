...
import restassuredapi.api.CommissionProductListAPI;
import restassuredapi.api.oauthentication.OAuthenticationAPI;
import restassuredapi.pojo.commissionProfile.ProductListPOJO;
import restassuredapi.pojo.oauthenticationrequestpojo.OAuthenticationRequestPojo;
import restassuredapi.pojo.oauthenticationresponsepojo.OAuthenticationResponsePojo;

@ModuleManager(name = Module.COM2)
public class ProductListTest extends BaseTest {

	static String moduleCode;

	ProductListPOJO viewListResponsePOJO = new ProductListPOJO();
	...
	public void A_01_Test_ValidList(String loginID, String password, String msisdn, String PIN, String parentName,
			String categoryName, String categorCode, String externalCode) throws Exception {
		final String methodName = "A_01_Test_Test_ValidList";
		Log.startTestCase(methodName);
		if (_masterVO.getProperty("identifierType").equals("loginid"))
			BeforeMethod(loginID, password, categoryName);
		else if (_masterVO.getProperty("identifierType").equals("msisdn"))
			BeforeMethod(msisdn, PIN, categoryName);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("COM2");
		moduleCode = CaseMaster.getModuleCode();

		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(), categoryName));
		currentNode.assignCategory("REST");

		CommissionProductListAPI commissionProfileAPI = new CommissionProductListAPI(
		...
		viewListResponsePOJO = commissionProfileAPI.getAPIResponseAsPOJO(ProductListPOJO.class);
