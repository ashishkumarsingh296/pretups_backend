...
@ModuleManager(name = Module.CUSTOMER_RC_BATCH_TEMPLATE)
public class CustomerRechargeDownloadTemplate extends BaseTest {
	...
	@TestManager(TestKey="PRETUPS-7966")
	public void A_01_Test_success(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DTCRC1");
		moduleCode = CaseMaster.getModuleCode();
		...
		long statusCode = customerRechargeTemplateResponsePojo.getStatus();
	...
	@Test(dataProvider = "userData")
	@TestManager(TestKey="PRETUPS-7967")
	public void A_02_Test_invalid_token(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		if(_masterVO.getProperty("identifierType").equals("loginid"))
			...
			BeforeMethod(msisdn, PIN,categoryName);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DTCRC2");
		moduleCode = CaseMaster.getModuleCode();

		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(),categoryName));
		currentNode.assignCategory("REST");
		CustomerRechargeTemplateApi customerRechargeTemplateApi = new CustomerRechargeTemplateApi(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken+new RandomGeneration().randomAlphabets(4));
		customerRechargeTemplateApi.setContentType(_masterVO.getProperty("contentType"));
		customerRechargeTemplateApi.setExpectedStatusCode(401);
		customerRechargeTemplateApi.perform();
		customerRechargeTemplateResponsePojo = customerRechargeTemplateApi
				.getAPIResponseAsPOJO(CustomerRechargeTemplateResponsePojo.class);
		String status = customerRechargeTemplateResponsePojo.getMessageCode();
