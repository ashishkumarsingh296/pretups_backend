...
import restassuredapi.api.getdomaincategoryparentcat.GetDomainCategoryParentCatAPI;
...
@ModuleManager(name = Module.GET_DOMAIN_CATEGORY_PARENT_CAT)
public class GetDomainCategoryParentCat extends BaseTest {
	...
	{
		final String methodName = "Test_GetDomainCategoryParentCatAPI";
		Log.startTestCase(methodName);
		if(_masterVO.getProperty("identifierType").equals("loginid"))
			BeforeMethod(loginID, password,categoryName);
		else if(_masterVO.getProperty("identifierType").equals("msisdn"))
			BeforeMethod(msisdn, PIN,categoryName);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("GDCPC1");
		moduleCode = CaseMaster.getModuleCode();
		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(),categoryName));
		currentNode.assignCategory("REST");
		GetDomainCategoryParentCatAPI getDomainCategoryParentCatAPI=new GetDomainCategoryParentCatAPI(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken);
		getDomainCategoryParentCatAPI.setContentType(_masterVO.getProperty("contentType"));
		getDomainCategoryParentCatAPI.setExpectedStatusCode(200);
		getDomainCategoryParentCatAPI.perform();
		getDomainCategoryParentCategoryResponsePojo = getDomainCategoryParentCatAPI
				.getAPIResponseAsPOJO(GetDomainCategoryParentCategoryResponsePojo.class);
		int statusCode ;
		if (getDomainCategoryParentCategoryResponsePojo != null && getDomainCategoryParentCategoryResponsePojo.getStatus()==200) {
			statusCode = 200;
		} else {
			statusCode = 400;
			}
