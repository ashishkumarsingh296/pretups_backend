...
@ModuleManager(name = Module.C2CTNXDETAILSVIEW)
public class C2cViewTransferDetailsChannelAdmin extends BaseTest {
    ...
    public String transferAndApproval(String fromCategory, String toCategory,
	 ...
	 public C2CViewTransferDetailsResponsePojo c2cTnxView(C2CViewTransferDetailsRequestPojo c2CViewTransferDetailsRequestPojo,
		   ...
		   C2CViewTransferDetailsAPI c2CViewTransferDetailsAPI = new C2CViewTransferDetailsAPI(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER),accessToken);
			...
			c2CViewTransferDetailsAPI.setExpectedStatusCode(expectedCode);
		 ...
		 return c2CViewTransferDetailsResponsePojo;
		}
    
    
	// Successful data with valid data.
	@Test(dataProvider = "userData")
	@TestManager(TestKey="PRETUPS-15682")
	public void A_01_Test_success(String fromCategory, String toCategory,
	    ...
	    int expected=200;
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("C2CTNXDETAILSVIEW01");
		moduleCode = CaseMaster.getModuleCode();
		currentNode = test.createNode(CaseMaster.getExtentCase());
		currentNode.assignCategory("REST");
		Data data=new Data();
		data.setTransferId(tnxid);
		...
		data.setPin("");
		data.setLoginid(cAdminLoginId);
		data.setPassword(cAdminPassword);
		data.setExtcode("");
		c2CViewTransferDetailsRequestPojo.setData(data);
		c2CViewTransferDetailsResponsePojo=this.c2cTnxView(c2CViewTransferDetailsRequestPojo, expected);
	...
	@Test(dataProvider = "userData")
	@TestManager(TestKey="PRETUPS-15683")
	public void A_02_InvalidTnxID(String fromCategory, String toCategory,
			String fromMSISDN, String toMSISDN, String fromLoginID, String fromPIN,
			String fromPassword,String toCatcode,String cAdminLoginId,String cAdminPassword,String cAdminCat) throws Exception {
		final String methodName = "A_02_InvalidTnxID";
		Log.startTestCase(methodName);
	    String tnxid=transferAndApproval(fromCategory,toCategory,fromMSISDN,toMSISDN,fromLoginID,fromPIN,fromPassword,toCatcode);
	    BeforeMethod(cAdminLoginId, cAdminPassword, cAdminCat);
	    int expected=200;
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("C2CTNXDETAILSVIEW02");
		moduleCode = CaseMaster.getModuleCode();
		currentNode = test.createNode(CaseMaster.getExtentCase());
		currentNode.assignCategory("REST");
		Data data=new Data();
		data.setTransferId(tnxid+"dfjkdas");
		data.setTransferType(_masterVO.getProperty("transferTypeStock"));
		data.setNetworkCode(_masterVO.getMasterValue(MasterI.NETWORK_CODE));
		data.setNetworkCodeFor(_masterVO.getMasterValue(MasterI.NETWORK_CODE));
		data.setExtnwcode(_masterVO.getMasterValue(MasterI.NETWORK_CODE));
		...
		c2CViewTransferDetailsResponsePojo=this.c2cTnxView(c2CViewTransferDetailsRequestPojo, expected);
		String status = c2CViewTransferDetailsResponsePojo.getStatus();
		String msgcode=c2CViewTransferDetailsResponsePojo.getMessageCode();
		Assert.assertEquals("400", status);
		Assertion.assertEquals(msgcode, "8140");
		Assertion.completeAssertions();
		Log.endTestCase(methodName);
	}
	@Test(dataProvider = "userData")
	@TestManager(TestKey="PRETUPS-15684")
	public void A_03_InvalidNetworkCode(String fromCategory, String toCategory,
			String fromMSISDN, String toMSISDN, String fromLoginID, String fromPIN,
			String fromPassword,String toCatcode,String cAdminLoginId,String cAdminPassword,String cAdminCat) throws Exception {
		final String methodName = "A_03_InvalidNetworkCode";
		Log.startTestCase(methodName);
	    String tnxid=transferAndApproval(fromCategory,toCategory,fromMSISDN,toMSISDN,fromLoginID,fromPIN,fromPassword,toCatcode);
	    BeforeMethod(cAdminLoginId, cAdminPassword, cAdminCat);
	    int expected=200;
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("C2CTNXDETAILSVIEW03");
		moduleCode = CaseMaster.getModuleCode();
		currentNode = test.createNode(CaseMaster.getExtentCase());
		currentNode.assignCategory("REST");
		Data data=new Data();
		data.setTransferId(tnxid);
		data.setTransferType(_masterVO.getProperty("transferTypeStock"));
		data.setNetworkCode(_masterVO.getMasterValue(MasterI.NETWORK_CODE)+"sdfds");
		data.setNetworkCodeFor(_masterVO.getMasterValue(MasterI.NETWORK_CODE));
		data.setExtnwcode(_masterVO.getMasterValue(MasterI.NETWORK_CODE));
		data.setMsisdn(fromMSISDN);
		data.setPin("");
