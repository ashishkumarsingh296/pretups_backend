...
@ModuleManager(name = Module.GIFT_BATCH_RC_TEMPLATE)
public class GiftRechargeDownloadTemplate extends BaseTest {
	...
	@TestManager(TestKey="PRETUPS-7964")
	public void A_01_Test_success(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DTGRC1");
		moduleCode = CaseMaster.getModuleCode();
		...
		long statusCode = giftRechargeTemplateResponsePojo.getStatus();
	...
	@Test(dataProvider= "userData")
	@TestManager(TestKey="PRETUPS-7965")
	public void A_02_Test_invalid_token(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		if(_masterVO.getProperty("identifierType").equals("loginid"))
			...
			BeforeMethod(msisdn, PIN,categoryName);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DTGRC2");
		moduleCode = CaseMaster.getModuleCode();

		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(),categoryName));
		currentNode.assignCategory("REST");
		GiftRechargeTemplateApi giftRechargeTemplateApi = new GiftRechargeTemplateApi(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken+new RandomGeneration().randomAlphabets(4));
		giftRechargeTemplateApi.setContentType(_masterVO.getProperty("contentType"));
		giftRechargeTemplateApi.setExpectedStatusCode(401);
		giftRechargeTemplateApi.perform();
		giftRechargeTemplateResponsePojo = giftRechargeTemplateApi
				.getAPIResponseAsPOJO(GiftRechargeTemplateResponsePojo.class);
		String status = giftRechargeTemplateResponsePojo.getMessageCode();
