...
import restassuredapi.api.FocBatchStockTransferAppRejApi;
...
@ModuleManager(name = Module.BULK_FOC_APP_REJ)
public class FocBatchStockTransferAppRejTest  extends BaseTest {
	...
	ArrayList<String> listapp3 = new ArrayList<String>();
	Data data = new Data();
	...
	// Initatie bulk commision payout and approve at level 1
	...
	public void A_01_Test_success(String loginID, String password, String msisdn, String PIN, String parentName,
		...
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("FOCBTRFAPPREJ1");
		moduleCode = CaseMaster.getModuleCode();
		...
		batchId = focBatchStockTransferResponsepojo.getBatchID();
		list.add(batchId);
		BulkFocAppRej bulkFocAppRej = new BulkFocAppRej();
		restassuredapi.pojo.focbulkapprej.Data data = new restassuredapi.pojo.focbulkapprej.Data();
		data.setRemarks("remarks");
		data.setRequestType("approval1");
		data.setRequest("approve");
		data.setBatchId(batchId);
		data.setBatchName("batch12");
		bulkFocAppRej.setData(data);
		FocBatchStockTransferAppRejApi batchO2cTransferApi = new FocBatchStockTransferAppRejApi(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken);
		batchO2cTransferApi.setContentType(_masterVO.getProperty("contentType"));
		batchO2cTransferApi.addBodyParam(bulkFocAppRej);
	...
	// approve at level 2
		...
		public void A_02_Test_success(String loginID, String password, String msisdn, String PIN, String parentName,
			...
			if (_masterVO.getProperty("identifierType").equals("loginid"))
				...
				BeforeMethod(msisdn, PIN, categoryName);
			CaseMaster CaseMaster = _masterVO.getCaseMasterByID("FOCBTRFAPPREJ2");
			moduleCode = CaseMaster.getModuleCode();
		...
		// approve at level 3
				...
				public void A_03_Test_success(String loginID, String password, String msisdn, String PIN, String parentName,
					...
					if (_masterVO.getProperty("identifierType").equals("loginid"))
						...
						BeforeMethod(msisdn, PIN, categoryName);
					CaseMaster CaseMaster = _masterVO.getCaseMasterByID("FOCBTRFAPPREJ3");
					moduleCode = CaseMaster.getModuleCode();
					...
					restassuredapi.pojo.focbulkapprej.Data data = new restassuredapi.pojo.focbulkapprej.Data();
					data.setRemarks("remarks");
					data.setRequestType("approval3");
					data.setRequest("approve");
					if(listapp3.size()>0) {
						data.setBatchId(listapp3.get(0));
						listapp3.remove(0);
						}
					data.setBatchName("batch12");
					bulkFocAppRej.setData(data);
