...
@ModuleManager(name = Module.INTERNET_RC_BATCH_TEMPLATE)
public class InternetRechargeDownloadTemplate extends BaseTest {
	...
	@TestManager(TestKey="PRETUPS-7968")
	public void A_01_Test_success(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DTINTRC1");
		moduleCode = CaseMaster.getModuleCode();
		...
		long statusCode = internetRechargeTemplateResponsePojo.getStatus();
	...
	public void A_02_Test_invalid_token(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		if(_masterVO.getProperty("identifierType").equals("loginid"))
			...
			BeforeMethod(msisdn, PIN,categoryName);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DTINTRC2");
		moduleCode = CaseMaster.getModuleCode();

		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(),categoryName));
		currentNode.assignCategory("REST");
		InternetRechargeTemplateApi internetRechargeTemplateApi = new InternetRechargeTemplateApi(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken+new RandomGeneration().randomAlphabets(4));
		internetRechargeTemplateApi.setContentType(_masterVO.getProperty("contentType"));
		internetRechargeTemplateApi.setExpectedStatusCode(401);
		internetRechargeTemplateApi.perform();
		internetRechargeTemplateResponsePojo = internetRechargeTemplateApi
				.getAPIResponseAsPOJO(InternetRechargeTemplateResponsePojo.class);
		String status = internetRechargeTemplateResponsePojo.getMessageCode();
