...
@ModuleManager(name = Module.CUSTOMER_RC_RESTRICTED_LIST_TEMPLATE)
public class CustomerRechargeRestrictedList extends BaseTest {
	...
	@TestManager(TestKey="PRETUPS-7971")
	public void A_01_Test_success(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DRLCRC1");
		moduleCode = CaseMaster.getModuleCode();

		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(),categoryName));
		currentNode.assignCategory("REST");
		CustomerRechargeRestrictedListApi customerRechargeRestrictedListApi = new CustomerRechargeRestrictedListApi(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken);
		...
		long statusCode = customerRechargeRestrictedListResponsePojo.getStatus();
	...
	@Test
	@TestManager(TestKey="PRETUPS-7972")
	public void A_02_Test_invalid_token(String loginID, String password, String msisdn, String PIN, String parentName, String categoryName, String categorCode) throws Exception {
		...
		if(_masterVO.getProperty("identifierType").equals("loginid"))
			...
			BeforeMethod(msisdn, PIN,categoryName);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("DRLCRC2");
		moduleCode = CaseMaster.getModuleCode();

		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(),categoryName));
		currentNode.assignCategory("REST");
		CustomerRechargeRestrictedListApi customerRechargeRestrictedListApi = new CustomerRechargeRestrictedListApi(_masterVO.getMasterValue(MasterI.WEB_URL_REST_SWAGGER), accessToken+new RandomGeneration().randomAlphabets(4));
		customerRechargeRestrictedListApi.setContentType(_masterVO.getProperty("contentType"));
		customerRechargeRestrictedListApi.setExpectedStatusCode(401);
		customerRechargeRestrictedListApi.perform();
		customerRechargeRestrictedListResponsePojo = customerRechargeRestrictedListApi
				.getAPIResponseAsPOJO(CustomerRechargeRestrictedListResponsePojo.class);
		String status = customerRechargeRestrictedListResponsePojo.getMessageCode();
