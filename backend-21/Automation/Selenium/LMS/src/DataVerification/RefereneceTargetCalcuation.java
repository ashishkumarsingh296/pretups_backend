...
public class RefereneceTargetCalcuation extends ExtentReportMultipleClasses {
						...
						while (eight.next()) {
							...
							if(type.equalsIgnoreCase("C2S")&&module.equalsIgnoreCase("ALL")){
							 ...
							 if(type.equalsIgnoreCase("O2C")){
								q = "select sum(TRANSFER_MRP) from CHANNEL_TRANSFERS where TO_USER_ID = '"+ USER_id+ "' and TYPE = 'O2C' and STATUS = 'CLOSE' and TRANSFER_DATE "+ datequer;
								value(q, "sum(TRANSFER_MRP)", detailsubtype, target);
								Assert.assertTrue(verify(USER_id, detailid, LMS, ver));	
							}
							 if(type.equalsIgnoreCase("C2C")){
								q = "select sum(TRANSFER_MRP) from CHANNEL_TRANSFERS where (FROM_USER_ID =  '"+ USER_id+ "'  or TO_USER_ID = '"+ USER_id+ "') and TYPE = 'C2C' and STATUS = 'CLOSE' and TRANSFER_DATE "+ datequer;
								value(q, "sum(TRANSFER_MRP)", detailsubtype, target);
								Assert.assertTrue(verify(USER_id, detailid, LMS, ver));
	...
	@DataProvider(name = "DP")
	 public static String[][] excelRead() throws Exception {
		...
		return Read_file.excelRead("demo_data2.xlsx","targetref");
