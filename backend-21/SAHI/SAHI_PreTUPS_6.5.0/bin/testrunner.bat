...
java -cp %SAHI_HOME%\lib\ant-sahi.jar net.sf.sahi.test.TestRunner %CUSTOM_FIELDS% -test %SCRIPTS_PATH%/%1 -browserType %3 -logsInfo %LOGS_INFO% -baseURL %START_URL% -host localhost -port 9090 -threads %THREADS% -useSingleSession %SINGLE_SESSION% -sendEmail %SEND_EMAIL_REPORT% -emailTrigger "%EMAIL_TRIGGER%" -emailProperties "%EMAIL_PROPERTIES%" -emailPasswordHidden "%EMAIL_PASSWORD_HIDDEN%" -tags %4
goto :EOF
