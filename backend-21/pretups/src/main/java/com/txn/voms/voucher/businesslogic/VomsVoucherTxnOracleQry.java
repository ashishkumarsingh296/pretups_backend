...
public class VomsVoucherTxnOracleQry implements VomsVoucherTxnQry{
	...
	public StringBuilder loadDatafromVoms(String tablename, String sno) {
		...
		sqlSelectBuf.append("  FOR UPDATE OF current_status NOWAIT ");
	...
	public StringBuilder getVoucherForMrp() {
		...
		StringBuilder selectBuff= new StringBuilder(" select  serial_no,pin_no,EXPIRY_DATE,TALKTIME,VALIDITY from VOMS_VOUCHERS_SNIFFER where product_id=? and mrp=? and EXPIRY_DATE>=trunc(SYSDATE)  ");
        selectBuff.append(" and current_status=? and subscriber_id is null and rownum< ?  FOR UPDATE NOWAIT ");
	...
	public StringBuilder loadDatafromVoms1(String tablename) {
        ...
        sqlSelectBuf.append(" WHERE pin_no=?  and serial_no =? FOR UPDATE OF current_status NOWAIT ");
