...
public class C2STransferTxnOracleQry implements C2STransferTxnQry {
	private Log _log = LogFactory.getLog(this.getClass().getName());
	private String className = "C2STransferTxnOracleQry";
	...
	public String loadLastTransfersStatusVOForC2SWithExtRefNumQry() {
		...
		//Changes for Performance issue
		strBuff.append(" FROM c2s_transfers C2S,products P, key_values KV,service_type ST WHERE ");
		strBuff.append(" TRANSFER_DATE >= trunc(?) AND TRANSFER_DATE <= trunc(?) AND ");
	...
	public PreparedStatement loadLastXCustTransfersQry(Connection p_con,
		...
		strBuff.append(" CS.transfer_date_time created_on, CS.service_type service, ST.name, 'C2S' type, KV.value statusname FROM ");
		strBuff.append(" key_values KV,SERVICE_TYPE ST,C2S_TRANSFERS CS ");
		strBuff.append(" WHERE CS.transfer_date >= ? and CS.ACTIVE_USER_ID=? AND CS.service_type=ST.service_type ");
		strBuff.append(" AND CS.transfer_status=KV.key(+) AND KV.type(+)=? ");
	...
	public PreparedStatement getChanneltransAmtDatewiseQry(Connection p_con,
		...
		selectQueryBuff.append("FROM " + tbl_name
				+ " CTRF,SERVICE_TYPE ST,KEY_VALUES KV1 ");
		...
		selectQueryBuff
				.append("AND  KV1.KEY(+)=CTRF.transfer_status AND KV1.TYPE(+)=? ");
	...
	public PreparedStatement loadLastC2STransfersBySubscriberMSISDNQry(Connection con,String senderMsisdn,String receiverMsisdn,Date fromDate)throws SQLException {
        ...
        strBuff.append("created_on, short_name, product_short_code, NAME FROM (SELECT   c2s.transfer_id, c2s.sender_id, c2s.service_type, ");
        ...
        strBuff.append(" ORDER BY created_on DESC) WHERE ROWNUM <= 1 ");
        String sqlSelect = strBuff.toString();
	...
	public PreparedStatement getC2SnProdTxnDetails(Connection con,
			String userId, Date fromDate, Date toDate, String ServiceType,String value)
			throws SQLException {
    	final String methodName="getC2STxnDetailsAll";
		 if (_log.isDebugEnabled())
			 _log.debug(methodName, "Entered  with userId" + userId);
    	 PreparedStatement pstmt = null;
         StringBuilder strBuff = new StringBuilder("SELECT * FROM(SELECT COUNT( TRANSFER_VALUE )transferCount,SUM( TRANSFER_VALUE ) transferValue,TRANSFER_VALUE amount");
         ...
         strBuff.append(" GROUP BY TRANSFER_VALUE ORDER BY transferValue DESC ) WHERE rownum < ?");
         if (_log.isDebugEnabled())
...
//	c2sTranscommqry.append( " AND L5.lookup_code= ADJ.MARGIN_TYPE ");
    ...
    c2sTranscommqry.append( "  AND    ug1.user_id=?)) AND ( adj.USER_ID IS  NULL OR (adj.USER_ID IS NOT NULL AND  adj.USER_ID <> 'OPERATOR')) " );
...
// 	    }
	...
	public String getAddtnlCommSummaryDets(AddtnlCommSummryReqDTO addtnlCommSummryReqDTO) {
		...
		if(addtnlCommSummryReqDTO.getDailyOrmonthlyOption()!=null && addtnlCommSummryReqDTO.getDailyOrmonthlyOption().trim().toUpperCase().equals(PretupsI.PERIOD_DAILY)) {
			...
			addtnlCommQuery.append("   AND UG.grph_domain_code IN (SELECT GD1.grph_domain_code "); 
			addtnlCommQuery.append("     FROM   GEOGRAPHICAL_DOMAINS GD1 "); 
			addtnlCommQuery.append("   WHERE GD1.status IN('Y', 'S') ");
			addtnlCommQuery.append("    CONNECT BY PRIOR GD1.grph_domain_code = GD1.parent_grph_domain_code ");
			addtnlCommQuery.append("     START WITH GD1.grph_domain_code IN ");
			addtnlCommQuery.append("    (SELECT UG1.grph_domain_code ");
			...
			addtnlCommQuery.append("    AND differential_amount <>0 ");
			addtnlCommQuery.append("    AND UG.grph_domain_code IN (SELECT GD1.grph_domain_code "); 
			addtnlCommQuery.append("  FROM ");
			addtnlCommQuery.append("   GEOGRAPHICAL_DOMAINS GD1 "); 
			addtnlCommQuery.append("  WHERE GD1.status IN('Y', 'S') ");
			addtnlCommQuery.append("    CONNECT BY PRIOR GD1.grph_domain_code = GD1.parent_grph_domain_code  ");
			addtnlCommQuery.append("      START WITH GD1.grph_domain_code IN ");
			addtnlCommQuery.append("   (SELECT UG1.grph_domain_code  ");
	...
	pbquery.append("   GD.grph_domain_name as USEGEOGRPHY ,GP.user_name grand_name,GP.msisdn grand_msisdn,GD1.GRPH_DOMAIN_NAME ParentGeography,GD2.GRPH_DOMAIN_NAME owner_geo,CT.opening_balance,CT.closing_balance,OC.category_name ownercategoryName,ou.msisdn ownermsisdn, ");
	...
	pbquery.append("   FROM ( SELECT user_id,parent_id,owner_id ");
	pbquery.append("   FROM USERS CONNECT BY PRIOR user_id = parent_id START WITH user_id=? ) X, ");
	...
	pbquery.append("   SELECT grph_domain_code "); 
	pbquery.append("   FROM  ");
	pbquery.append("   GEOGRAPHICAL_DOMAINS GD1  "); 
	pbquery.append("   WHERE status IN('Y', 'S') ");
	pbquery.append("   CONNECT BY PRIOR grph_domain_code = parent_grph_domain_code  ");
	pbquery.append("    START WITH grph_domain_code IN ");
	pbquery.append("   (SELECT grph_domain_code ");
