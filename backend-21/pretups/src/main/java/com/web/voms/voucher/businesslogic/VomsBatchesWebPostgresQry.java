...
public class VomsBatchesWebPostgresQry implements VomsBatchesWebQry {

	@Override
	public String loadBatchListWithBatchNoNewQry() {
		StringBuilder strBuff = new StringBuilder();
		strBuff.append(" SELECT B.batch_no BATCHNO,B.batch_type BATCHTYPE,B.reference_no REFERENCENO,B.product_id PRODUCTID, B.SIGNED_DOC SIGNEDDOC, P.product_name PRODUCTNAME, P.mrp MRP, B.created_by CREATEDBY, B.total_no_of_vouchers TOTALVOUCHER, B.created_on CREATEDON, B.status STATUS, coalesce(B.message,'') message, ");
		...
		strBuff.append(" FROM  voms_products P, voms_batches B LEFT JOIN USERS U ON B.USER_ID=U.USER_ID ");
		...
		strBuff.append(" B.network_code=case ? when ? then B.network_code else ? end AND ");
	...
	public String loadBatchListNewQry() {
		StringBuilder strBuff = new StringBuilder();
		strBuff.append(" SELECT B.batch_no BATCHNO,B.batch_type BATCHTYPE,B.product_id PRODUCTID, B.SIGNED_DOC SIGNEDDOC, B.reference_no REFERENCENO, P.product_name PRODUCTNAME, P.mrp MRP, B.created_by CREATEDBY, B.total_no_of_vouchers TOTALVOUCHER, B.created_on CREATEDON, B.status STATUS, coalesce(B.message,'') message,");
		...
		//strBuff.append(" FROM voms_batches B, voms_products P ,users U WHERE B.product_id=P.product_id AND ");
		strBuff.append(" FROM  voms_products P, voms_batches B LEFT JOIN USERS U ON B.USER_ID=U.USER_ID " );
		if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.USER_VOUCHERTYPE_ALLOWED))).booleanValue() || ((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.USER_VOUCHERSEGMENT_ALLOWED))).booleanValue())
			strBuff.append(" , voms_categories vc ");
		...
		strBuff.append(" date_trunc('day',B.created_date::TIMESTAMP)>= date_trunc('day',?::TIMESTAMP) AND date_trunc('day',B.created_date::TIMESTAMP)<=date_trunc('day',?::TIMESTAMP) " );
	...
	public String loadBatchListOnDaysNewQry() {
		...
		strBuff.append("AND date_trunc('day',B.created_date::TIMESTAMP)>=case ? when 'ALL' then B.created_date else ? end");
	...
	public String loadBatchListOnStatusQry(String p_productid) {
		StringBuilder strBuff = new StringBuilder();
		strBuff.append(" SELECT B.batch_no BATCHNO,B.batch_type BATCHTYPE,B.reference_no REFERENCENO,B.product_id PRODUCTID, B.SIGNED_DOC AS SIGNEDDOC, P.product_name PRODUCTNAME, P.mrp MRP, B.created_by CREATEDBY, B.total_no_of_vouchers TOTALVOUCHER, B.created_on CREATEDON, l.LOOKUP_NAME STATUS,coalesce(B.message,'') message,");
		...
		strBuff.append("B.network_code=case ? when ? then B.network_code else ? end  AND B.status=? and l.LOOKUP_TYPE=? and P.voucher_segment = ? ");
	...
	public String loadBatchListOnStatusNewQry(String p_productid) {
		...
		strBuff.append("B.network_code=case ? when ? then B.network_code else ? end AND B.status=? ");
		if (!BTSLUtil.isNullString(p_productid)) {
			strBuff.append("AND B.product_id=? ");
	...
	public String getVomsPrinterBatchQry(String p_batchType,UserVO userVO) {
		...
		sqlSelectBuf
				.append(" SELECT P.printer_batch_id,P.start_serial_no,P.end_serial_no,P.user_id,P.downloaded,");
		sqlSelectBuf
				.append("P.product_id,P.voms_decryp_key,P.total_no_of_vouchers,P.created_on,P.created_by,");
		sqlSelectBuf
				.append("P.modified_on,P.modified_by,PR.product_name,PR.mrp,VC.voucher_type,VC.voucher_segment FROM VOMS_PRINT_BATCHES P, ");
		...
		sqlSelectBuf
				.append("PR.category_id=VC.category_id and date_trunc('day',P.created_on::TIMESTAMP)>=? and date_trunc('day',P.created_on::TIMESTAMP)<=? and VC.network_code=? order by P.created_on");
	...
	public String updateVomsPrintBatchstatusQry() {
		...
		sqlSelectBuf.append(" WHERE(to_number(from_serial_no) <=to_number(?) AND to_number(to_serial_no)>=to_number(?)) ");
		return sqlSelectBuf.toString();
	}
	...
	public String getBatchInfoForUserInputsQry() {
		...
		sqlLoadBuf
				.append(" SELECT b.batch_no batchno,b.from_serial_no,b.to_serial_no,b.batch_type bType,b.total_no_of_vouchers,coalesce(b.download_count,0) DOWNCOUNT,b.sequence_id,p.voucher_segment");
		sqlLoadBuf
	...
	public String loadBatchListForMsisdnQry() {
		...
		strBuff.append(" WHERE B.product_id=P.product_id AND ");
		strBuff.append("B.network_code=case ? when 'ALL' then B.network_code else ? end AND ");
		if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.USER_VOUCHERTYPE_ALLOWED))).booleanValue())
		strBuff.append("  p.category_id = vc.category_id AND " );
	...
	public String getBatchInfoForSelectUserInputsQry() {
		StringBuilder sqlLoadBuf = new StringBuilder();
		sqlLoadBuf
				.append("SELECT b.batch_no batchno,b.from_serial_no,b.to_serial_no,b.batch_type bType,b.total_no_of_vouchers,coalesce(b.download_count,0) DOWNCOUNT,b.sequence_id ");
		sqlLoadBuf
				.append(" from voms_batches b,voms_products p WHERE p.product_id=? AND p.product_id=b.product_id ");
		sqlLoadBuf.append(" AND p.mrp=? AND b.from_serial_no <=? ");
		sqlLoadBuf.append(" AND b.to_serial_no >=? AND b.status=? ");
		// sqlLoadBuf.append(" AND b.location_code=? " );
	...
	public String getVomsPrinterBatchForUserQry(String p_batchType,UserVO userVO) {
		...
		sqlSelectBuf
				.append("PR.category_id=VC.category_id and date_trunc('day',P.created_on)>=? and date_trunc('day',P.created_on)<=?  AND u.USER_ID = ?  and u.USER_ID =uv.USER_ID and vc.VOUCHER_TYPE = uv.VOUCHER_TYPE and VC.NETWORK_CODE = ? order by P.created_on ");
		return sqlSelectBuf.toString();
	}
