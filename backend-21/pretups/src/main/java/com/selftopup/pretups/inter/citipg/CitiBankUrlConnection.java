...
import com.selftopup.pretups.inter.connection.BTSLConnection;
 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CitiBankUrlConnection extends BTSLConnection {
    private static Log _log = LogFactory.getLog(CitiBankUrlConnection.class.getName());
    private HttpURLConnection _urlConnection = null;

    public CitiBankUrlConnection(String p_url, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength, String p_soapAction) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CitiBankUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive + " p_soapAction=" + p_soapAction);
        try {
            ...
            e.printStackTrace();
            _log.error("CitiBankUrlConnection", "Exception " + e.getMessage());
            if (_log.isDebugEnabled())
                _log.debug("CitiBankUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("CitiBankUrlConnection", "Exiting");
    }
