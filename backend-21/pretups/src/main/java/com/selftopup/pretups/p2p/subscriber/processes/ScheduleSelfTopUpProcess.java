package com.selftopup.pretups.p2p.subscriber.processes;
 ...
 *         --------------------------------------------------------------------
 *         -----------------------------
 *         GAURAV PANDEY 29/05/2014 Initial Creation
 *         --------------------------------------------------------------------
 *         -----------------------------
 * 
 */
...
public class ScheduleSelfTopUpProcess {

    String responseCode = null;
    String transId = null;
    private static ProcessStatusVO _processStatusVO;
    private static ProcessBL _processBL = null;
    private static Log _logger = LogFactory.getLog(ScheduleSelfTopUpProcess.class.getName());
    ...
    public static void main(String arg[]) {
...

        {
...

            {
                ...
                {
                    System.out.println("Usage : ScheduleSelfTopUpProcess [Constants file] [LogConfig file] [Message Flag y/n]");
                    return;
            ...
            {
                System.out.println("ScheduleSelfTopUpProcess" + " Constants File Not Found .............");
                return;
            ...
            {
                System.out.println("ScheduleSelfTopUpProcess" + " Logconfig File Not Found .............");
                return;
    ...
    private static void process() throws BTSLBaseException {
        ...
        try {
...

            {
                ...
                EventHandler.handle(EventIDI.DATABASE_CONECTION_PROBLEM, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ScheduledMultipleCreditTransferProcess[process]", "", "", "", "DATABASE Connection is NULL");
                return;
            ...
            if (statusOk) {
                ...
                if (subscriberList.size() > 0) {
                    for (int i = 0; i < subscriberList.size(); i++) {
                        ...
                        if (subscriberVO.getFailRetryCount() <= Integer.parseInt(Constants.getProperty("Fail_RETRY_COUNT")))
                            topUpToSubscriber(con, subscriberVO);
                        else {
                            updateSubscriber(con, subscriberVO);
                            BTSLMessages sendbtslMessage = null;
                            sendbtslMessage = new BTSLMessages(SelfTopUpErrorCodesI.USER_MAX_RETRY_COUNT_REACHED);
                            ...
                            try {
                                PushMessage pushMessage = new PushMessage(subscriberVO.getMsisdn(), sendbtslMessage, "", "", locale, subscriberVO.getNetworkCode(), "SMS will be delivered shortly thankyou");
                                pushMessage.push();
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                    ...
                    throw new BTSLBaseException("ScheduledMultipleCreditTransferProcess", "process", SelfTopUpErrorCodesI.NO_USER_FOUND_FOR_SCHEDULE_TOPUP);
                }

            } else {
                throw new BTSLBaseException("ScheduledMultipleCreditTransferProcess", "process", SelfTopUpErrorCodesI.PROCESS_ALREADY_RUNNING);
...

        {
            ...
            _logger.error("ScheduledMultipleCreditTransferProcess", "Exception : " + e.getMessage());
            e.printStackTrace();
            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "ScheduledMultipleCreditTransferProcess[process]", "", "", "", " ScheduledMultipleCreditTransferProcess could not be executed successfully.");
            throw new BTSLBaseException("ScheduledMultipleCreditTransferProcess", "process", SelfTopUpErrorCodesI.SCHEDULE_TOPUP_PROCESS);
            ...
            try {
                if (_processStatusVO.isStatusOkBool()) {
                    ...
                    updateCount = (new ProcessStatusDAO()).updateProcessDetail(con, _processStatusVO);
                    if (updateCount > 0) {
                        con.commit();
                    }
                }
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("ScheduledMultipleCreditTransferProcess", "Exception in closing connection ");
            }
            if (con != null)
                try {
                    con.close();
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("ScheduledMultipleCreditTransferProcess", "Exiting..... ");
        }
    ...
    private static ArrayList<SubscriberVO> loadSubscriberList(Connection connnection) throws BTSLBaseException, java.sql.SQLException {
        if (_logger.isDebugEnabled())
            _logger.info("loadSubscriberList", "Entered");
        ArrayList<SubscriberVO> SubscriberList = new ArrayList<SubscriberVO>();
        ...
        try {
            ...
            queryBuf.append(" where SC.USER_ID=PS.USER_ID and SC.schedule_date<=? and SC.status='Y' and SC.schedule_date<=SC.DE_ACTIVATION_DATE ");
            String query = queryBuf.toString();
            if (_logger.isDebugEnabled())
                _logger.debug("loadSubscriberList", "Query:" + query);
            pstmt = connnection.prepareStatement(query.toString());
            pstmt.setDate(1, BTSLUtil.getSQLDateFromUtilDate(currentDate));
            ...
            _logger.error("loadSubscriberList", "SQLException " + sqle.getMessage());
            sqle.printStackTrace();
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ScheduleSelfTopUpProcess[loadSubscriberList]", "", "", "", "SQL Exception:" + sqle.getMessage());
            throw new BTSLBaseException("ScheduleSelfTopUpProcess", "loadSubscriberList", "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _logger.error("balanceAlertUsers", "Exception " + e.getMessage());
            e.printStackTrace();
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ScheduleSelfTopUpProcess[loadSubscriberList]", "", "", "", "Exception:" + e.getMessage());
            throw new BTSLBaseException("ScheduleSelfTopUpProcess", "loadSubscriberList", "error.general.processing");
        }// end of catch
        finally {
            ...
            if (_logger.isDebugEnabled())
                _logger.info("loadSubscriberList", " Exiting list size " + SubscriberList.size());
        }// end finally
    ...
    private static void topUpToSubscriber(Connection connnection, SubscriberVO subscriberVO) throws BTSLBaseException, java.sql.SQLException {
        if (_logger.isDebugEnabled())
            _logger.info("topUpToSubscriber", "Entered");
        ...
        if (!BTSLUtil.isNullString(FinalResponce)) {
            ...
            if (subscriberVO.getScheduleType().equals("W")) {
                ...
                now.setTime(scheduleDate);
                now.add(Calendar.DATE, now.getActualMaximum(Calendar.DAY_OF_MONTH));
            }
    ...
    private static String generateRequestTopUP(SubscriberVO subscriberVO) {
        if (_logger.isDebugEnabled())
            _logger.debug("generateRequestTopUP", "Entered");
        String requesStr = null;
        StringBuffer sbf = null;
        try {
            sbf = new StringBuffer();
            sbf.append("TYPE=RGRCREQ");
            ...
            sbf.append("&LANGUAGE1=0&LANGUAGE2=0");
            requesStr = sbf.toString();
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
            if (_logger.isDebugEnabled())
                _logger.debug("generateRequestTopUP", "Exiting requesStr=" + requesStr);
        }
        return requesStr;
    }
    ...
    private static String initiateTopUpRequest(SubscriberVO subscriberVO) {
        if (_logger.isDebugEnabled())
            _logger.debug("initiateTopUpRequest", "Entered");
        ...
        try {
            ...
            MessageGatewayVO messageGatewayVO = MessageGatewayCache.getObject(PretupsI.GATEWAY_TYPE_SELFTOPUP);
            if (_logger.isDebugEnabled())
                _logger.debug("initiateTopUpRequest", "messageGatewayVO: " + messageGatewayVO);
            if (messageGatewayVO == null) {

                throw new BTSLBaseException("initiateTopUpRequest", "initiateTopUpRequest", SelfTopUpErrorCodesI.ERROR_NOTFOUND_MESSAGEGATEWAY);
            }
            RequestGatewayVO requestGatewayVO = messageGatewayVO.getRequestGatewayVO();
            if (requestGatewayVO == null) {
                throw new BTSLBaseException("initiateTopUpRequest", "initiateTopUpRequest", SelfTopUpErrorCodesI.ERROR_NOTFOUND_REQMESSAGEGATEWAY);
            }
            if (!PretupsI.STATUS_ACTIVE.equals(messageGatewayVO.getStatus())) {
                ...
                throw new BTSLBaseException("initaiateFocRedemptionRequest", "initiateTopUpRequest", SelfTopUpErrorCodesI.ERROR_NOTFOUND_REQMESSAGEGATEWAY);
            }
            ...
            if (LoadControllerCache.getNetworkLoadHash() != null && LoadControllerCache.getNetworkLoadHash().containsKey(LoadControllerCache.getInstanceID() + "_" + networkCode))
                smsInstanceID = ((NetworkLoadVO) (LoadControllerCache.getNetworkLoadHash().get(LoadControllerCache.getInstanceID() + "_" + networkCode))).getP2pInstanceID();

            else {
                throw new BTSLBaseException("initaiateFocRedemptionRequest", "initiateTopUpRequest", SelfTopUpErrorCodesI.NO_INSTANCE_FOR_REQUESTED_NETWORK);
            }
            ...
            if (SystemPreferences.HTTPS_ENABLE)
                httpURLPrefix = "https://";
            urlToSend = httpURLPrefix + instanceLoadVO.getHostAddress() + ":" + instanceLoadVO.getHostPort() + Constants.getProperty("CHANNEL_WEB_CP2PSUBSCRIBER_REGISTRATION_SERVLET") + "?" + "REQUEST_GATEWAY_CODE=" + messageGatewayVO.getGatewayCode();
            urlToSend = urlToSend + "&REQUEST_GATEWAY_TYPE=" + messageGatewayVO.getGatewayType() + "&LOGIN=" + messageGatewayVO.getRequestGatewayVO().getLoginID() + "&PASSWORD=" + BTSLUtil.decryptText(messageGatewayVO.getRequestGatewayVO().getPassword());
            urlToSend = urlToSend + "&SERVICE_PORT=" + messageGatewayVO.getRequestGatewayVO().getServicePort() + "&SOURCE_TYPE=" + PretupsI.REQUEST_SOURCE_TYPE_STUGW;
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("initiateTopUpRequest", "Exiting response=" + response);
        }
     ...
     * @throws java.sql.SQLException
     */
    private static void updateSubscriberScheduleDate(Connection connnection, SubscriberVO subscriberVO) throws BTSLBaseException, java.sql.SQLException {
        if (_logger.isDebugEnabled())
            _logger.info("updateSubscriberScheduleDate", "Entered");
        ...
        Date currentDate = new Date();
        try {
            StringBuffer queryBuf = new StringBuffer(" update schedule_topup_details set schedule_date=?,modified_on=?,fail_retry_count=? where user_id=? AND nick_name=?");
            String query = queryBuf.toString();
            if (_logger.isDebugEnabled())
                _logger.debug("updateSubscriberScheduleDate", "Query:" + query);
            pstmt = connnection.prepareStatement(query.toString());
            pstmt.setDate(1, BTSLUtil.getSQLDateFromUtilDate(subscriberVO.getScheduleDate()));
            ...
            _logger.error("updateSubscriberScheduleDate", "SQLException " + sqle.getMessage());
            sqle.printStackTrace();
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ScheduleSelfTopUpProcess[updateSubscriberScheduleDate]", "", "", "", "SQL Exception:" + sqle.getMessage());
            throw new BTSLBaseException("ScheduleSelfTopUpProcess", "loadSubscriberList", "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _logger.error("updateSubscriberScheduleDate", "Exception " + e.getMessage());
            e.printStackTrace();
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ScheduleSelfTopUpProcess[updateSubscriberScheduleDate]", "", "", "", "Exception:" + e.getMessage());
            throw new BTSLBaseException("ScheduleSelfTopUpProcess", "loadSubscriberList", "error.general.processing");
        }// end of catch
        finally {
            if (rst != null)
            ...
            if (_logger.isDebugEnabled())
                _logger.info("updateSubscriberScheduleDate", " Exiting updateCount " + updatecount);
        }// end finally
     ...
     * @throws java.sql.SQLException
     */
    private static void updateSubscriber(Connection connnection, SubscriberVO subscriberVO) throws BTSLBaseException, java.sql.SQLException {
        if (_logger.isDebugEnabled())
            _logger.info("updateSubscriber", "Entered");
        ...
        Date currentDate = new Date();
        try {
            StringBuffer queryBuf = new StringBuffer(" update schedule_topup_details set status=?,modified_on=? where user_id=? AND nick_name=?");
            String query = queryBuf.toString();
            if (_logger.isDebugEnabled())
                _logger.debug("updateSubscriber", "Query:" + query);
            pstmt = connnection.prepareStatement(query.toString());
