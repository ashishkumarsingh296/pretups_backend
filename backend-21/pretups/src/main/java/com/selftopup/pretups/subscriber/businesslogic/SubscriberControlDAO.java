...
public class SubscriberControlDAO {
    ...
    private static Log _log = LogFactory.getLog(SubscriberControlDAO.class.getName());
    ...
    public boolean loadSubscriberControlDetails(Connection p_con, ReceiverVO p_receiverVO) throws BTSLBaseException {
        ...
        try {
             ...
             * " SELECT prefix_id,failed_count, consecutive_failures, last_failure_on, success_count, last_success_on,total_transfer_amount, "
             * );
             * selectQueryBuff.append(
             * " last_transfer_id, last_transfer_status, last_transfer_stage, last_transfer_on "
             * );
             * selectQueryBuff.append(" FROM subscriber_control ");
             * selectQueryBuff.append(
             * " WHERE module=? AND msisdn=? AND created_date=? ");
             */

            StringBuffer selectQueryBuff = new StringBuffer(" SELECT	prefix_id, failed_count, consecutive_failures, last_failure_on, success_count, ");
            ...
            selectQueryBuff.append("prev_month_success_counts, prev_month_transfer_amount, last_transfer_amount, last_service_type");
            ...
            // pstmtSelect.setDate(3,BTSLUtil.getSQLDateFromUtilDate(p_receiverVO.getCreatedDate()));
    ...
    public int addSubscriberControlDetails(Connection p_con, ReceiverVO p_receiverVO) throws BTSLBaseException {
        ...
        try {
            int i = 1;
            StringBuffer insertQueryBuff = new StringBuffer(" INSERT INTO subscriber_control (module,msisdn,subscriber_type, ");
            insertQueryBuff.append(" prefix_id,total_transfer_amount,last_transfer_id, last_transfer_status, last_transfer_stage, ");
            insertQueryBuff.append(" last_transfer_on,created_date,last_transfer_amount,last_service_type) ");
            ...
            pstmtInsert.setDate(i++, BTSLUtil.getSQLDateFromUtilDate(p_receiverVO.getCreatedDate()));
    ...
    public int updateSubscriberControlDetails(Connection p_con, ReceiverVO p_receiverVO) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("updateSubscriberControlDetails", "Entered p_receiverVO:" + p_receiverVO.toString());
        ...
        try {
             ...
             * " UPDATE subscriber_control SET subscriber_type=?,prefix_id=?,total_transfer_amount=?,failed_count=?,consecutive_failures=?, "
             * );
             * updateQueryBuff.append(
             * " last_failure_on=? ,success_count=? ,last_success_on=?, last_transfer_status=?, last_transfer_id=?, last_transfer_stage=?,last_transfer_on=?  "
             * );
             * updateQueryBuff.append(
             * " WHERE module=? AND msisdn=? AND created_date=?  ");
             */
            StringBuffer updateQueryBuff = new StringBuffer(" UPDATE subscriber_control SET subscriber_type=?,prefix_id=?,total_transfer_amount=?,failed_count=?,consecutive_failures=?,");
            ...
            if (_log.isDebugEnabled())
                _log.debug("updateSubscriberControlDetails", "update query:" + updateQuery);
            ...
            // pstmtUpdate.setDate(i++,BTSLUtil.getSQLDateFromUtilDate(p_receiverVO.getCreatedDate()));
            ...
            if (_log.isDebugEnabled())
                _log.debug("updateSubscriberControlDetails", "Exiting updateCount=" + updateCount);
        }// end of finally
    }
    ...
    public void lockSubscriberControlTable(Connection p_con, ReceiverVO p_receiverVO) throws BTSLBaseException {
        ...
        try {
            StringBuffer selectQueryBuff = new StringBuffer("SELECT	last_transfer_status ");
            ...
            throw new BTSLBaseException("PretupsBL", "lockSubscriberControlTable", SelfTopUpErrorCodesI.RECEIVER_LAST_REQ_UNDERPROCESS_S, 0, strArr, null);
        }// end of catch
        catch (Exception e) {
    ...
    public void loadSubscriberLastDetails(Connection p_con, String p_module, String p_msisdn, TransferVO p_transferVO) throws BTSLBaseException {
        ...
        ResultSet rs = null;
        try {
            StringBuffer selectQueryBuff = new StringBuffer(" SELECT	last_transfer_amount, last_service_type");
            selectQueryBuff.append(" FROM subscriber_control WHERE module=? AND msisdn=? ");
