...
public class FermaConnectionManager {
    ...
    private static FermaRequestFormatter _formatter = new FermaRequestFormatter();
    private int _transactionID = 0;
    private Log _log = LogFactory.getLog(this.getClass().getName());
    private long _lastLoginTime = 0;
    private long _minLoginRetryTime = 0;

    public FermaConnectionManager(String p_inID) {
        _inID = p_inID;
    }
    ...
    public synchronized void login(String p_url) throws Exception {
        ...
        FermaUrlConnection fermaUrlConnection = null;
        ...
        try {
            ...
            fermaUrlConnection = new FermaUrlConnection(p_url, timeout, readtimeout, keepAlive);
            ...
            requestStr = _formatter.generateRequest(FermaRequestResponse.ACTION_LOGIN, map);
            if (_log.isDebugEnabled())
            ...
            if (responseStr != null) {
                HashMap responseMap = _formatter.parseResponse(FermaRequestResponse.ACTION_LOGIN, responseStr);
            ...
            e.printStackTrace();
            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "FermaConnectionManager[login]", "", "", "", "Problem in Login on Ferma IN with _interfaceID: " + _inID + " Exception: " + e.getMessage());
            // generate Alarm
        } finally {
    ...
    public synchronized void logout(String p_url) throws Exception {
        ...
        try {
            ...
            if (responseStr != null) {
                ...
                if (status.equalsIgnoreCase(FermaINHandler.XML_STATUS_SUCCESS))
                    if (_log.isDebugEnabled())
            ...
            e.printStackTrace();
            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "FermaConnectionManager[logout]", "", "", "", "Connection not established with _interfaceID: " + _inID + " Exception: " + e.getMessage());
            // generate Alarm
        } finally {
            try {
                if (fermaUrlConnection != null)
