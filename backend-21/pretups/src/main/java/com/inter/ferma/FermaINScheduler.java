...
public class FermaINScheduler {
    private static Log _log = LogFactory.getLog(FermaINScheduler.class.getName());
    ...
    public synchronized void replaceUrl() {
                                                        ...
                                                        // divert URL
        {
            ...
            if (_urlID == 1) {
                _urlID = 2;
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "FermaINScheduler[replaceUrl]", "", "", "", "Ferma Switchover from Node 1 to Node 2");
            } else {
                _urlID = 1;
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "FermaINScheduler[replaceUrl]", "", "", "", "Ferma Switchover from Node 2 to Node 1");
            }
        ...
        try {
            if (_urlID == 1)
                ...
                if (currentTime - _resetTime > _maxHoldDuration) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "FermaINScheduler[getUrlStr]", "", "", "", "Switching to IN 1");
                }
                return _urlStr;
            }
        } finally {
