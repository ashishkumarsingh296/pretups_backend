...
public class FermaConnectionServlet extends HttpServlet {
    private static Log _log = LogFactory.getLog(FermaConnectionServlet.class.getName());
    ...
    public static void initialize() {
        ...
        if (_log.isDebugEnabled())
            _log.debug("initialize", "Entered FERMA_CONN_POOL_IDS : " + ids);
        ...
        for (int i = 0; i < _ids.length; i++) {
            id = _ids[i];
            try {
                FermaINScheduler fermaINScheduler = new FermaINScheduler();
                ...
                FermaConnectionManager fermaConnectionManager = new FermaConnectionManager(id);
                ...
                e.printStackTrace();
                EventHandler.handle(EventIDI.SYSTEM_ERROR, id, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "FermaConnectionServlet[initialize]", "", "", "", "Exception while initialize FERMA Scheduler Parameters Exception:" + e.getMessage());
            }
        }
        if (_log.isDebugEnabled())
    ...
    public static void refresh() {
        String ids = Constants.getProperty("FERMA_CONN_POOL_IDS");
        if (_log.isDebugEnabled())
            _log.debug("refresh", "Entered FERMA_CONN_POOL_IDS : " + ids);
        String[] _ids = ids.split(",");
        String id;
        String urlStr = null;
        long minHoldDuration = 0;
        ...
        for (int i = 0; i < _ids.length; i++) {
            id = _ids[i];
            try {
                FermaINScheduler fermaINScheduler = (FermaINScheduler) _schedulerMap.get(id);
                minHoldDuration = Long.parseLong(FileCache.getValue(id, "MIN_HOLD_DURATION"));
                maxHoldDuration = Long.parseLong(FileCache.getValue(id, "MAX_HOLD_DURATION"));
                if (_log.isDebugEnabled())
                ...
                e.printStackTrace();
                EventHandler.handle(EventIDI.SYSTEM_ERROR, id, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "FermaConnectionServlet[refresh]", "", "", "", "Exception while referesh FERMA Scheduler Parameters Exception:" + e.getMessage());
            }
        }
        if (_log.isDebugEnabled())
    ...
    public void destroy() {
        String ids = Constants.getProperty("FERMA_CONN_POOL_IDS");
        if (_log.isDebugEnabled())
            _log.debug("destroy", "Entered FERMA_CONN_POOL_IDS : " + ids);
        String[] _ids = ids.split(",");
        String id;
        String urlStr = null;
        for (int i = 0; i < _ids.length; i++) {
            ...
            try {
                ...
                FermaConnectionManager fermaConnectionManager = (FermaConnectionManager) _managerMap.get(id);
                fermaConnectionManager.logout(urlStr);
            } catch (Exception e) {
                _log.error("destroy", "Exception e:" + e.getMessage() + " id=" + id);
                e.printStackTrace();
                EventHandler.handle(EventIDI.SYSTEM_INFO, id, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "FermaConnectionServlet[destroy]", "", "", "", "Exception while initialize FERMA Scheduler Parameters Exception:" + e.getMessage());
            }
        }
        if (_log.isDebugEnabled())
