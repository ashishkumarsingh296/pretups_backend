package com.inter.scheduler.cs3vfe;
 ...
 *                   Chetan Kothari July 30,2008 Updated constructor for
...
public class NodeScheduler {
     ...
     */
    public synchronized NodeVO getNodeVO(String p_transId, HashMap p_requestMap, int p_action, int p_ambRtryCnt, int p_conRtryCnt) throws BTSLBaseException {
        if (_log.isDebugEnabled())
        ...
        try {
            nodeNumStr = getNode(p_transId, p_requestMap, p_action, p_ambRtryCnt, p_conRtryCnt);
            if (nodeNumStr == null) {
     ...
     * @throws BTSLBaseException
     */
    private synchronized String getNode(String p_transId, HashMap p_requestMap, int p_action, int p_ambRtryCnt, int p_conRtryCnt) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            ...
            while (isNext) {
                try {
                    if (tempCount > _maxAllowedNum) {
                        ...
                        INNodeLog.log((String) p_requestMap.get("TRANSACTION_ID"), p_transId, cs3NodeVO.getUrl(), cs3NodeVO.isBlocked(), cs3NodeVO.isSuspended(), (String) p_requestMap.get("MSISDN"), p_action, (p_ambRtryCnt - 1), p_conRtryCnt, (String) p_requestMap.get("NETWORK_CODE"), cs3NodeVO.getConNumber(), "Number of nodes exceeded, allowed are : " + _maxAllowedNum);
                        break;
                    }
                    ...
                    if (!cs3NodeVO.isBlocked()) {
                        if (cs3NodeVO.getConNumber() > cs3NodeVO.getMaxConPerNode()) {
                            ...
                            incrementNodeNumber();
                            INNodeLog.log((String) p_requestMap.get("TRANSACTION_ID"), p_transId, cs3NodeVO.getUrl(), cs3NodeVO.isBlocked(), cs3NodeVO.isSuspended(), (String) p_requestMap.get("MSISDN"), p_action, (p_ambRtryCnt - 1), p_conRtryCnt, (String) p_requestMap.get("NETWORK_CODE"), cs3NodeVO.getConNumber(), "Number of concurrent connection exceeded, allowed are : " + cs3NodeVO.getMaxConPerNode());
                            continue;
                        }
                        ...
                        INNodeLog.log((String) p_requestMap.get("TRANSACTION_ID"), p_transId, cs3NodeVO.getUrl(), cs3NodeVO.isBlocked(), cs3NodeVO.isSuspended(), (String) p_requestMap.get("MSISDN"), p_action, (p_ambRtryCnt - 1), p_conRtryCnt, (String) p_requestMap.get("NETWORK_CODE"), cs3NodeVO.getConNumber(), "Node is not blocked (due to connection error). Not sure whether node is suspended (due to ambiguous case) or not");
                        break;
                    }
                    if (_log.isDebugEnabled())
                    ...
                    if (isExpired(cs3NodeVO.getBlokedAt(), cs3NodeVO.getExpiryDuration())) {
                        ...
                        INNodeLog.log((String) p_requestMap.get("TRANSACTION_ID"), p_transId, cs3NodeVO.getUrl(), cs3NodeVO.isBlocked(), cs3NodeVO.isSuspended(), (String) p_requestMap.get("MSISDN"), p_action, (p_ambRtryCnt - 1), p_conRtryCnt, (String) p_requestMap.get("NETWORK_CODE"), cs3NodeVO.getConNumber(), "Node was blocked (due to connection error). Now resumed");
                        break;
                    }
