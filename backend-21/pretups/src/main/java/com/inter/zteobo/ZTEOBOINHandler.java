 ...
 *                          Babu Kunwar August 17, 2011 Modified
 *                          Shashank Shukla June 16,2012 Modified
 *                          ----------------------------------------------------
 *                          --------------------------------------------
 */
...
public class ZTEOBOINHandler implements InterfaceHandler {
    private Log _log = LogFactory.getLog(ZTEOBOINHandler.class.getName());
    private ZTEOBOFormatter _formatter = null;
     ...
     * Constructor
     */
    public ZTEOBOINHandler() {
        _formatter = new ZTEOBOFormatter();
    }
    ...
    public void validate(HashMap<String, String> p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            setInterfaceParameters(ZTEOBOI.ACTION_ACCOUNT_INFO);
            // generate Request Object
            String requestStr = _formatter.generateRequest(ZTEOBOI.ACTION_ACCOUNT_INFO, _requestMap);
            // sending the AccountInfo request to IN along with validate action
            // defined in Huawei84I interface
            sendRequestToIN(ZTEOBOI.ACTION_ACCOUNT_INFO, requestStr);
    ...
    public void credit(HashMap<String, String> p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            // Set the interface parameters into requestMap
            setInterfaceParameters(ZTEOBOI.ACTION_RECHARGE_CREDIT);
            ...
            String requestStr = _formatter.generateRequest(ZTEOBOI.ACTION_RECHARGE_CREDIT, _requestMap);
            // sending the Re-charge request to IN along with re-charge action
            // defined in SafcomI interface
            sendRequestToIN(ZTEOBOI.ACTION_RECHARGE_CREDIT, requestStr);
            // set TRANSACTION_STATUS as Success in request map
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
    ...
    public void debitAdjust(HashMap<String, String> p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            // Set the interface parameters into requestMap
            setInterfaceParameters(ZTEOBOI.ACTION_IMMEDIATE_DEBIT);
            ...
            // value of INTERFACE_AMOUNT)
            _requestMap.put("transfer_amount", amountStr);
            String requestSre = _formatter.generateRequest(ZTEOBOI.ACTION_IMMEDIATE_DEBIT, _requestMap);
            // sending the Re-charge request to IN along with re-charge action
            // defined in SafcomI interface
            sendRequestToIN(ZTEOBOI.ACTION_IMMEDIATE_DEBIT, requestSre);
            // set TRANSACTION_STATUS as Success in request map
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
            // Set the post balance after multiplied by multiplication factor.
            try {
    ...
    private void sendRequestToIN(int p_action, String p_requestStr) throws BTSLBaseException {
        ...
        try {
            ...
            try {
                ...
                try {
                    ZTEOBOWebServiceConnector serviceConnection = new ZTEOBOWebServiceConnector();
                    ...
                    throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_WEBSERVICE_EXCEPTION);
                }
            ...
            if (p_action == ZTEOBOI.ACTION_ACCOUNT_INFO && ZTEOBOI.RESULT_OK.equals(status)) {
            ...
            if (!ZTEOBOI.RESULT_OK.equals(status)) {
                int len = status.length();
                if (len > 10) {
                    status = status.substring(len - 10);
                }
                _requestMap.put("INTERFACE_STATUS", status);
                if (ZTEOBOI.MSISDN_NOT_FOUND.equals(status)) {
    ...
    public void setInterfaceParameters(int p_action) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            // HashMap<String,String> bankChannelIDMap=new
            ...
            if (!InterfaceUtil.isNullString(reqService) && PretupsI.SERVICE_TYPE_CHNL_RECHARGE.equals(reqService)) {
                ...
                String allowedDomainUsers = FileCache.getValue(_interfaceID, domainCode);
                if (InterfaceUtil.isNullString(allowedDomainUsers)) {
                    _log.error("setInterfaceParameters", "Value of Domain Users is not defined in the INFile");
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOBOINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Domain Users is not defined in the INFile.");
                    throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CHANNEL_ID_CONFIG_PROBLEM);
                }
                String[] domainUsers = allowedDomainUsers.split(":");
                if (domainUsers.length > 1) {
                    for (int i = 0; i < domainUsers.length; i++) {
                        String domainUserName = domainUsers[i];
                        String senderMsisdn = _requestMap.get("SENDER_MSISDN");
                        String allowedMsisdn = FileCache.getValue(_interfaceID, domainUserName);
                        if (!InterfaceUtil.isNullString(allowedMsisdn)) {
                            if (Arrays.asList(allowedMsisdn.split(",")).contains(senderMsisdn)) {
                                channelId = FileCache.getValue(_interfaceID, domainUserName + "_CHANNEL_ID");
                                _requestMap.put(domainUserName + "_CHANNEL_ID", channelId);
                                break;
                            }
                        } else {
                            _log.error("setInterfaceParameters", "Value of Domain User MSISDN is not defined in the INFile");
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOBOINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Domain User MSISDN is not defined in the INFile.");
                            throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CHANNEL_ID_CONFIG_PROBLEM);
                        }
                    }
                } else {
                    allowedDomainUsers = allowedDomainUsers.trim();
                    channelId = FileCache.getValue(_interfaceID, allowedDomainUsers + "_CHANNEL_ID");
                }

                if (InterfaceUtil.isNullString(channelId))
                    channelId = FileCache.getValue(_interfaceID, reqService + "_DEFAULT" + "_CHANNEL_ID");
            }
            ...
            if (!InterfaceUtil.isNullString(reqService) && PretupsI.SERVICE_TYPE_C2S_ENQUIRY.equals(reqService))
                channelId = FileCache.getValue(_interfaceID, reqService + "_CHANNEL_ID");
            if (InterfaceUtil.isNullString(channelId))
                channelId = FileCache.getValue(_interfaceID, "CHANNEL_ID_C2S");
