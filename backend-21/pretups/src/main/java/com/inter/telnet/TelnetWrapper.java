 ...
 * send() and wait() or receiveUntil() calls. send() sends a string across the
 * telnet connection. Add a '\r' to the end if you want to complete a command.
 * wait() waits for an exact string from the other side of the telnet
 * connection, and returns nothing, receiveUntil() also waits for a string, but
 * returns all the data that it received while waiting, including the string
 * itself. Use this if you want the output from a command. Please note that the
 * telnet connection will usually echo the sent command.
 * <P>
 * sendLine() is generally better, since it adds the '\r' automatically, waits
 * for the prompt before returning, and returns all data received before the
 * prompt, with the prompt itself cut off the end, and the sent command cut off
 * the beginning. login() and sendLine() are implemented using send(), wait()
 * and receiveUntil(). They can be freely mixed and matched.
 * <P>
...
public class TelnetWrapper {
     ...
     * token or timeout.
     * <P>
     * More efficient than receiveUntil, but liable to fail on large tokens that
     * can be spread over several "send"s. In that case, consider using
     * receiveUntil and ignoring the return value.
     * 
     * @param token
     *            String to wait for
     * @param timeout
    ...
    public String sendLine(String command) throws IOException {
        ...
        System.out.println("TelnetWrapper sendLine() command=" + command + "     prompt=" + prompt);
    ...
    public static void main(String args[]) throws IOException {
        ...
        Date now = new Date();
        String timestamp = now.getYear() + "-" + (now.getMonth() + 1) + "-" + now.getDate() + "-" + now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds();
    ...
    public BufferedInputStream getTelnetInputStream() {
        return tio.getTelnetInputStream();
    }
