 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class CS3VfeUrlConnection extends BTSLConnection {
    private HttpURLConnection _urlConnection = null;

    // public CS3VfeUrlConnection(String p_url,String p_username,String
    // p_password,int p_connectTimeout,int p_readTimeout,String p_keepAlive,long
    // p_contentLength, String p_hostName,String p_userAgent) throws Exception
    public CS3VfeUrlConnection(String p_url, String p_username, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength, String p_hostName, String p_userAgent) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CS3VfeUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive + " p_hostName=" + p_hostName + " p_userAgent=" + p_userAgent);
        try {
            ...
            setRequestHeader(p_username, p_contentLength, p_hostName, p_userAgent, p_keepAlive);
            setPrintWriter();
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @param p_keepAlive
     */
    private void setRequestHeader(String p_username, long p_contenetLength, String p_host, String p_userAgent, String p_keepAlive) throws Exception {
        if (_log.isDebugEnabled())
        ...
        try {
            ...
            _urlConnection.setRequestProperty("Authorization", "Basic " + p_username);
