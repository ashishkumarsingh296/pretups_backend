...
/**
 * @(#)cs5omgUrlConnection.java
 * Copyright(c) 2011, Comviva Technologies Ltd.
 * All Rights Reserved
 *-------------------------------------------------------------------------------------------------
 * 	  Author				Date				 History
 *-------------------------------------------------------------------------------------------------
 * Diwakar        Mar 29, 2012		    Initial Creation
 * ------------------------------------------------------------------------------------------------
 */
...
public class CS5OmgUrlConnection extends BTSLConnection
{
	private HttpURLConnection _urlConnection=null;

	public CS5OmgUrlConnection(String p_url,String p_username,String p_password,int p_connectTimeout,int p_readTimeout,String p_keepAlive,long p_contentLength, String p_hostName,String p_userAgent) throws Exception
	{
		String METHOD_NAME="cs5omgUrlConnection";
		if(_log.isDebugEnabled())_log.debug(METHOD_NAME,"Entered p_url: "+p_url+"p_connecttimeout="+p_connectTimeout+" p_readtimeout="+p_readTimeout+" p_keepAlive="+p_keepAlive+" p_hostName="+p_hostName+" p_userAgent="+p_userAgent);
		try
		{
		...
		{
		    if(_log.isDebugEnabled()) _log.debug(METHOD_NAME+", encodedPass:"+_urlConnection.getRequestProperty("Authorization")+", Host:"+_urlConnection.getRequestProperty("Host")+", User-Agent:"+_urlConnection.getRequestProperty("User-Agent")+", URI:"+_urlConnection.getURL(),"Exited");
		}
	}
	...
	{
		String METHOD_NAME="setPrintWriter";
		try 
		{
	...
	{
		String METHOD_NAME="setBufferedReader";
		try 
		{
...
//		String encodedPass=encode.encode(userPass.getBytes());
		...
		{
			if(_log.isDebugEnabled()) _log.debug("setRequestHeader"+", encodedPass:"+encodedPass+", _urlConnection.toString()"+_urlConnection.toString()+", Host:"+_urlConnection.getRequestProperty("Host"),"Exited");
