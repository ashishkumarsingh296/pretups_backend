...
public class CS5OmgRequestFormatter 
{
	public static Log _log = LogFactory.getLog(CS5OmgRequestFormatter.class);
	private SimpleDateFormat _sdf = null;
	private TimeZone _timeZone = null;
	private String _transDateFormat="yyyyMMdd'T'HH:mm:ss";//Defines the Date and time format of CS5OmgIN.
	...
	public CS5OmgRequestFormatter() throws Exception
	{
		String METHOD_NAME="CS5OmgRequestFormatter[constructor]";
	...
	protected String generateRequest(int p_action, HashMap<String,String> p_map) throws Exception 
	{
		...
		try
		{
			switch(p_action)
			{
			case CS5OmgI.ACTION_ACCOUNT_INFO: 
			{
				str=generateGetAccountInfoRequest(p_map);
				break;	
			}
			case CS5OmgI.ACTION_ACCOUNT_DETAILS: 
			{
				str=generateGetAccountDetailRequest(p_map);
				break;	
			}
			case CS5OmgI.ACTION_RECHARGE_CREDIT: 
			{
				str=generateRechargeCreditRequest(p_map);
				break;	
			}
			case CS5OmgI.ACTION_IMMEDIATE_DEBIT: 
			{
				str=generateImmediateDebitRequest(p_map);
				break;	
			}
			case CS5OmgI.ACTION_DEDICATED_ACCOUNT_CD: 
			{
	...
	private HashMap<String,String> parseImmediateDebitResponse(String p_responseStr) throws Exception
	{
		...
		try
		{
			...
			if(tempIndex>0)
			{
				...
				responseMap.put("responseCode",responseCode.trim());
				Object[] successList=CS5OmgI.RESULT_OK.split(",");
				//if(!CS5OmgI.RESULT_OK.equals(responseCode))
				if(!Arrays.asList(successList).contains(responseCode))
					return responseMap;
			}
			indexStart= p_responseStr.indexOf("<member><name>originTransactionID",indexEnd);
