...
public class NodeVO 
{
	...
	private ArrayList<String> _transIdList=new ArrayList<String>();//This list is used to contains the transaction id for connections of the NODE.
	...
	private boolean _processSingleReqFlag = false;
	...
	{
		String METHOD_NAME="incrementConNumber";
		if(_transIdList.add(p_inTxnID))
			_log.info(METHOD_NAME,"p_inTxnID::"+p_inTxnID+" is added to the List");
		_conNumber++;
	}
	...
	{
		String METHOD_NAME="decrementConNumber";
		//Removing the transaction id from the Node List.
		if(_transIdList.remove(p_inTxnID))
			_log.info(METHOD_NAME,"p_inTxnID::"+p_inTxnID+" is removed from the List");
	 ...
	 */
	public void setTransIdList(ArrayList<String> transIdList) 
	{
	...
	{
		this._interfacePrevStatus = prevStatus;
	}
