...
public class CS3BangladeshRequestFormatter {

    public static Log _log = LogFactory.getLog(CS3BangladeshRequestFormatter.class);
    ...
    public CS3BangladeshRequestFormatter() throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CS3BangladeshRequestFormatter[constructor]", "Entered");
        try {
            ...
            e.printStackTrace();
            _log.error("CS3BangladeshRequestFormatter[constructor]", "Exception e::" + e.getMessage());
            throw e;
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
                _log.debug("CS3BangladeshRequestFormatter[constructor]", "Exited");
        }// end of finally
    }// end of CS3BangladeshRequestFormatter[constructor]
    ...
    protected String generateRequest(int p_action, HashMap p_map) throws Exception {
        ...
        try {
            switch (p_action) {
            case CS3BangladeshI.ACTION_ACCOUNT_INFO: {
                str = generateGetAccountInfoRequest(p_map);
                break;
            }
            case CS3BangladeshI.ACTION_ACCOUNT_DETAILS: {
                str = generateGetAccountDetailRequest(p_map);
                break;
            }
            case CS3BangladeshI.ACTION_RECHARGE_CREDIT: {
                str = generateRechargeCreditRequest(p_map);
                break;
            }
            case CS3BangladeshI.ACTION_IMMEDIATE_DEBIT: {
