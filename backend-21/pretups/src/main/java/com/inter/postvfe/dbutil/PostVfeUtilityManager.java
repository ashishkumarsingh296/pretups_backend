 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class PostVfeUtilityManager {
    private Log _log = LogFactory.getLog(PostVfeUtilityManager.class.getName());
    public static HashMap _utilityObjectMap = new HashMap();

    public void initialize(String p_ids) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("initialize", "Entered p_ids::" + p_ids);
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "PostVfeUtilityManager[initialize]", "utilityId:" + utilityId, "", "", "While instantiation of HandlerCommonUTility object got the Exception " + e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
        } finally {
            if (_log.isDebugEnabled())
    ...
    public Object getNewUtilityObj(String p_id) throws BTSLBaseException {
        ...
        try {
            ...
            utilityClass = FileCache.getValue(p_id, "DBUTILITY_HANDLER_CLASS");
            // utilityClass =
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "PostVfeUtilityManager[getNewDBUtilityObj]", "p_id:" + p_id, "", "", "While getting the instance of Client objects got the Exception " + e.getMessage());
    ...
    public void destroy(String p_interfaceId) {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "PostVfeUtilityManager[destroy]", "", "", "", "While destorying the Client objects got the Exception " + e.getMessage());
