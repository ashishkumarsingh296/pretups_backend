...
public class HuaweiPoolLoaderServlet extends HttpServlet {
    private static final long serialVersionUID = 551L;
    private Log _log = LogFactory.getLog(HuaweiPoolLoaderServlet.class.getName());
    private HuaweiPoolManager _poolManager = new HuaweiPoolManager();
    private HuaweiHeartBeatController _huaweiHeartController = null;

    /**
     * Constructor of the object.
     */
    public HuaweiPoolLoaderServlet() {
        super();
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String interfaceID = getInitParameter("INTERFACE_ID");
        if (_log.isDebugEnabled())
    ...
    public void destroy() {
        ...
        String poolIDs = Constants.getProperty("HUAWEI_POOL_IN_IDS");
        if (_log.isDebugEnabled())
    ...
    public void initialize() {
        ...
        try {
            // _poolManager = new HuaweiPoolManager();
            ...
            if (!InterfaceUtil.isNullString(poolINIDs)) {
                HuaweiPoolManager.initialize(poolINIDs);
                _huaweiHeartController = new HuaweiHeartBeatController(poolINIDs);
            }
    ...
    public void initialize(String p_interfaceId) {
        ...
        try {
            ...
            if (!InterfaceUtil.isNullString(p_interfaceId)) {
                HuaweiPoolManager.initialize(p_interfaceId);
                _huaweiHeartController = new HuaweiHeartBeatController(p_interfaceId);
            }
        } catch (BTSLBaseException be) {
            _log.error("initialize", "BTSLBaseException be::" + be.getMessage());
        } catch (Exception e) {
