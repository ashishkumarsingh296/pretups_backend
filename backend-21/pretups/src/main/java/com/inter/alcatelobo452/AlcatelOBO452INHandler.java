...
public class AlcatelOBO452INHandler implements InterfaceHandler {
    private static Log _log = LogFactory.getLog("AlcatelOBO452INHandler".getClass().getName());
    private HashMap<String, String> _requestMap = null;
    private HashMap<String, String> _responseMap = null;
    private AlcatelOBO452RequestFormatter _formatter = null;
    ...
    public void validate(HashMap<String, String> p_map) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            _formatter = new AlcatelOBO452RequestFormatter();
            String inStr = _formatter.generateRequest(AlcatelOBO452I.ACTION_GET_ACCOUNT_INFO, _requestMap);

            // Sending the request to IN
            sendRequestToIN(inStr, AlcatelOBO452I.ACTION_GET_ACCOUNT_INFO);
            String serviceType = (String) p_map.get("REQ_SERVICE");
            _userType = (String) p_map.get("USER_TYPE");
            // System.out.println("$$$$$$ SELECTOR_BUNDLE_ID :"+
            // ((String)_requestMap.get("SELECTOR_BUNDLE_ID")).trim());
            if (!(("PRC".equals(serviceType)) || ("CPN".equals(serviceType)))) {
                System.out.println("$$$$$$ SELECTOR_BUNDLE_ID :" + ((String) _requestMap.get("SELECTOR_BUNDLE_ID")).trim());
                // Check whether the bonus bundle feature is allowed or not.
                if ("Y".equals(FileCache.getValue(_interfaceID, "BUNDLE_FEATURE_ALLOWED"))) {
                    ...
                    if (InterfaceUtil.isNullString(selectorBundleCode) || ((!"1".equals(selectorBundleId) && InterfaceUtil.isNullString(receivedBundles)) || (!"1".equals(selectorBundleId) && !Arrays.asList(bundleList).contains(selectorBundleCode)))) {
                        ...
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[credit]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "Subscriber " + (String) p_map.get("MSISDN") + "does not have " + selectorBundleCode + "bundle assigned.");
                ...
                _requestMap.put("IN_RESP_BUNDLE_CODES", _responseMap.get("received_bundles"));
            }
            ...
            try {
                ...
                e.printStackTrace();
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[credit]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "credit balance from response is not numeric or multiplication factor defined in the INFile is not Numeric, Exception e:" + e.getMessage());
            ...
            e.printStackTrace();
            _log.error("validate: ", "Exception e:" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[validate]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while validate");
    ...
    public void credit(HashMap<String, String> p_map) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            selectorBundleId = FileCache.getValue(_interfaceID, "SELECTOR_" + p_map.get("SELECTOR_BUNDLE_ID"));
            ...
            String seperateBonusAllowed = FileCache.getValue(_interfaceID, "SEPERATE_BONUS_ALLOWED").trim();
            ...
            // If bonus is allowed then deduct the main bonus amount from the
            // interface amount.
            if ("N".equals(seperateBonusAllowed))
            ...
            // Convert the amount in amount
            inAmount = InterfaceUtil.getINAmountFromSystemAmountToIN(Double.parseDouble(amountStr), Double.parseDouble(mulFactor.trim()));
            ...
            if (amountStr.contains(".") && ('.' == amountStr.charAt(amountStr.length() - 2)))
            ...
            // AlcatelOBO452RequestFormatter class with appropriate
            ...
            _formatter = new AlcatelOBO452RequestFormatter();
            String inStr = _formatter.generateRequest(AlcatelOBO452I.ACTION_IMMEDIATE_CREDIT, _requestMap);

            // send the credit request to IN
            sendRequestToIN(inStr, AlcatelOBO452I.ACTION_IMMEDIATE_CREDIT);
            ...
            try {
                ...
                e.printStackTrace();
                _log.error("credit", "Exception e:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[credit]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in credit");
                throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[credit]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "While credit get Exception e:" + e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
    ...
    public void debitAdjust(HashMap<String, String> p_map) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(mulFactor)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[debitAdjust]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "Multiplication factor is not defined in IN File");
            ...
            inAmount = InterfaceUtil.getINAmountFromSystemAmountToIN(requestedAmtDouble, Double.parseDouble(mulFactor.trim()));
            amountStr = String.valueOf(inAmount);
            /*
             * }
             * else
             ...
             * //Convert the amount in amount according to the selector.
             * inAmount=InterfaceUtil.getINAmountFromSystemAmountToIN(
             * requestedAmtDouble,Double.parseDouble(mulFactor.trim()));
             * }
             * }
             */
            ...
            // debitAdjust request.
            _formatter = new AlcatelOBO452RequestFormatter();
            String inStr = _formatter.generateRequest(AlcatelOBO452I.ACTION_IMMEDIATE_DEBIT, _requestMap);
            // sending the debit request to the IN
            sendRequestToIN(inStr, AlcatelOBO452I.ACTION_IMMEDIATE_DEBIT);
            // On successful response, set TRANSACTION_STATUS as SUCCES into
            // request map.
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[debitAdjust]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in debitAdjust");
            ...
            e.printStackTrace();
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[debitAdjust]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "While debitAdjust get Exception e:" + e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
    ...
    public void creditAdjust(HashMap<String, String> p_map) throws BTSLBaseException, Exception {
        ...
        try {
             ...
             * mulFactor=FileCache.getValue(_interfaceID,"UNIT_MULT_FACTOR");
             * //Convert the amount in amount according to the selector.
             * inAmount=InterfaceUtil.getINAmountFromSystemAmountToIN(Double.
             * parseDouble(amountStr),Double.parseDouble(mulFactor.trim()));
             * amountStr=String.valueOf(inAmount);
             * }
             * }
             */
    ...
    public void sendRequestToIN(String p_inRequestStr, int p_action) throws BTSLBaseException {
        ...
        String responseStr = "";
        AlcatelOBO452UrlConnection urlConnection = null;
        long startTime = 0;
        long endTime = 0;
        long warnTime = 0;
        int readTimeOut = 0;
        ...
        try {
            ...
            try {
                ...
                if (InterfaceUtil.isNullString(urlFromFileCache)) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "URL is not defined in IN File");
                ...
                if (InterfaceUtil.isNullString(connectTimeOutStr) || InterfaceUtil.isNullString(keepAlive)) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "CONNECT_TIMEOUT or KEEP_ALIVE is not defined in IN File");
                    throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                int connectTimeOut = Integer.parseInt(connectTimeOutStr);
                // Get read time out from INFile

                if (AlcatelOBO452I.ACTION_GET_ACCOUNT_INFO == p_action) {
                    ...
                    if (InterfaceUtil.isNullString(readTimeOutValStr)) {
                        ...
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "READ_TIMEOUT_VAL is not defined in IN File");
                        throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    if (InterfaceUtil.isNullString(readTimeOutTopStr)) {
                        ...
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "READ_TIMEOUT_TOP is not defined in IN File");
                        throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                ...
                // ServletName.
                urlConnection = new AlcatelOBO452UrlConnection(urlFromFileCache, connectTimeOut, readTimeOut, keepAlive);
            } catch (BTSLBaseException be) {
                throw be;
            }// end of catch-BTSLBaseException
            catch (Exception e) {
                e.printStackTrace();
                EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Error while getting the http connection for INTERFACE_ID=[" + _interfaceID + "]" + " Exception : " + e.getMessage() + " action: " + p_action);
            ...
            try {
                ...
                // object from
                // the AlcatelOBO452UrlConnection class.
            ...
            // error occurs does the following.
                ...
                e.printStackTrace();
                _log.error("sendRequestToIN: ", "Exception e:" + e.getMessage());
                EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE") + " INTERFACE ID = " + _interfaceID + " Action = " + p_action, "Exception while sending request to AlcatelOCI IN" + " action :" + p_action);
            ...
            try {
                ...
                // AlcatelOBO452UrlConnection class.
                ...
                if (endTime - startTime > warnTime) {
                    _log.info("sendRequestToIN: ", "WARN time reaches for the AlcatelOBO452IN, startTime: " + startTime + " endTime: " + endTime + " warnTime: " + warnTime + " time taken: " + (endTime - startTime));
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "AlcatelOBO452INHandler[sendRequestToIN]", _inTXNID, _msisdn, (String) _requestMap.get("NETWORK_CODE") + " INTERFACE ID = " + _interfaceID + " Stage = " + p_action, "AlcatelOCI452 IN is taking more time than the warning threshold. Time: " + (endTime - startTime) + " action: " + p_action);
                }
            } // try
            catch (Exception e) {
                e.printStackTrace();
                _log.error("sendRequestToIN: ", " Response form interface is null, exception is: " + e.getMessage());
                EventHandler.handle(EventIDI.INTERFACE_RESPONSE_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "AlcatelOBO452INHandler[sendRequestToIN]", _inTXNID, _msisdn, (String) _requestMap.get("NETWORK_CODE") + " INTERFACE ID = " + _interfaceID + " Stage = " + p_action, "Exception while getting response from AlcatelOBO452IN e: " + e.getMessage());
            ...
            if (!AlcatelOBO452I.HTTP_STATUS_OK.equals(httpStatus))
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            if (InterfaceUtil.isNullString(responseStr)) {
                // raise the alarm
                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE, _interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE") + " INTERFACE ID = " + _interfaceID + " Stage = " + p_action, "Blank response from AlcatelOCI IN");
                _log.error("sendRequestToIN: ", "NULL response from interface. ");
                _requestMap.put("INTERFACE_STATUS", InterfaceErrorCodesI.AMBIGOUS);
                throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
            }
            // Parse the response using the parseResponse method of
            // AlcatelONIRequestFormatter class.
            ...
            if (!InterfaceUtil.isNullString(result) && !result.equals(AlcatelOBO452I.RESULT_OK)) {
                _log.info("sendRequestToIN: ", "cp transID:" + cpTransID + " CP ID=" + (String) _responseMap.get("cp_id") + " result=" + result);
                if (result.equals(AlcatelOBO452I.RESULT_ERROR_ACC_NOT_FOUND))
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE") + " INTERFACE ID = " + _interfaceID + " Stage = " + p_action, "System Exception:" + e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
        } finally {
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[sendRequestToIN]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE") + " INTERFACE ID = " + _interfaceID + " Stage = " + p_action, "Not able to close connection:" + e.getMessage());
            }
            if (_log.isDebugEnabled())
    ...
    private void setInterfaceParameters() throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(cpID)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, (String) _requestMap.get("NETWORK_CODE"), "CP_ID_" + (String) _requestMap.get("MODULE") + " is not defined in the INFile.");
            ...
            if (InterfaceUtil.isNullString(application)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, (String) _requestMap.get("NETWORK_CODE"), "APPLICATION is not defined in the INFile.");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(cancelTxnAllowed)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, (String) _requestMap.get("NETWORK_CODE"), "CANCEL_TXN_ALLOWED is not defined in the INFile.");
            ...
            if (InterfaceUtil.isNullString(systemStatusMappingCr)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters ", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
    ...
    private void setLanguageFromMapping() throws Exception {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(mappingString)) {
                mappingString = "";
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "AlcatelOBO452INHandler[setLanguageFromMapping]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "LANGUAGE_MAPPING is not defined in IN file,Hence setting the Default language");
            }
            ...
            if (mappingNotFound)
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "AlcatelOBO452INHandler[setLanguageFromMapping]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "Mapping for language received from IN =  " + langFromIN + " is not defined in IN file Hence setting the Default language");
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "AlcatelOBO452INHandler[setLanguageFromMapping]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) _requestMap.get("NETWORK_CODE"), "Mapping KEY for language is not defined in IN file and Getting Excption=" + e.getMessage());
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
