...
public class CRMWebServiceConnectionManager {
	 ...
	 * @date 17 Oct 2013
	 */
	public CRMWebServiceConnectionManager(NodeVO p_nodevo, String p_interfaceID, String p_txnid)throws Exception
	{
		...
		try
		{
			...
			soapHeader.addChildElement("Username").addTextNode(p_nodevo.getUsername());
			soapHeader.addChildElement("Password").addTextNode(p_nodevo.getPassword());
			soapHeader.addChildElement("REQUEST_ID").addTextNode(p_txnid);

			ServicePortalLocator topupServiceLocator = new ServicePortalLocator();
			_stub=topupServiceLocator.getwebservice(new java.net.URL(p_nodevo.getUrl()));		
	...
	protected ServicePortalPortType getService()
	{
		return (ServicePortalPortType) _stubSuper;
	}
