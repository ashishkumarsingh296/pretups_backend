		...
		try
		{
			...
			for(int l=1;l<=_totalNodes;l++)
			{
			   try
			   {
					...
					name = FileCache.getValue(p_cs5INId,"USERNAME_"+l);
				    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","USERNAME_"+l+"::"+name);
					if(InterfaceUtil.isNullString(name))
					{
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of USERNAME_"+l +" in INFile "+p_cs5INId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_INITIALIZATION);
					}   
				    cs5NodeVO.setUsername(name.trim());
			        
					password = FileCache.getValue(p_cs5INId,"PASSWORD_"+l);
				    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","PASSWORD_"+l+"::"+password);
					if(InterfaceUtil.isNullString(password))
					{
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of PASSWORD_"+l +" in INFile "+p_cs5INId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_INITIALIZATION);
					}   
				    cs5NodeVO.setPassword(password.trim());
