...
public class ZTEINRequestFormatter{
	...
	public String generateRequest(String fileCacheId,int p_action, HashMap p_map) throws BTSLBaseException,Exception 
	{
		...
		try
		{
			...
			{
				str=generateWrongPINRequest(fileCacheId,p_map);
				break;	
			}
			case ZTEINI.ACTION_ACCOUNT_ADJUST: 
			{
	...
	public HashMap<String,String> parseResponse(int p_action, String p_responseStr) throws BTSLBaseException,Exception 
	{
		...
		try
		{
			...
			case ZTEINI.ACTION_ACCOUNT_INFO: 
			{
				map=parseWringPINInfoResponse(p_responseStr);
				break;	
			}
			case ZTEINI.ACTION_ACCOUNT_ADJUST: 
			{
	 ...
	 * @throws	Exception
	 */
	private String generateWrongPINRequest(String fileCacheId,HashMap p_map) throws Exception
	{
		if(_logger.isDebugEnabled())_logger.debug("generateWrongPINRequest"," Entered p_map= "+p_map);
		...
		try
		{
			...
			{
				_logger.error("generateWrongPINRequest "," Value of START_FLAG is not defined in the INFile");
			...
			{
				_logger.error("generateWrongPINRequest"," Value of VERSION_NUMBER is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest"," Value of TERM is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest ","Value of ACCOUNT_INFO_SERIVICE is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest"," Value of dlgCon is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest"," Value of RSV is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest"," Value of DLGCTRL is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest"," Value of TSRV is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			{
				_logger.error("generateWrongPINRequest","Value of DLGLGN is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}

			String acntInfoCommand = FileCache.getValue(fileCacheId,"WRONGPIN_COMMAND");
			if(InterfaceUtil.isNullString(acntInfoCommand))
			{
				_logger.error("generateWrongPINRequest"," Value of WRONGPIN_COMMAND is not defined in the INFile");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_BAD_REQUEST);
			}
			...
			requestDataBuffer.append("ATTR=ADDERRTIMES="+(String)p_map.get("RECEIVER_INVALID_PIN_CNT"));
		...
		{
			_logger.error("generateWrongPINRequest"," Exception e:" + e.getMessage());
			throw e;
		}
		finally
		{
			if(_logger.isDebugEnabled()) _logger.debug("generateWrongPINRequest"," Exiting  requestStr:"+requestStr);
		}
		return requestStr;
	}
	/**
	 ...
	 * @throws  Exception
	 */
	private HashMap<String,String> parseWringPINInfoResponse(String p_responseStr) throws Exception
	{		

		if(_logger.isDebugEnabled())_logger.debug("parseWringPINInfoResponse"," Entered p_responseStr: "+p_responseStr);
		HashMap<String,String> map=null;
		int index=0;
		String status=null;
		String attr=null;
		...
		{
			_logger.error("parseWringPINInfoResponse"," Exception e:" + e.getMessage());
			throw e;
		}
		finally
		{	    	
			if(_logger.isDebugEnabled())_logger.debug("parseWringPINInfoResponse"," Exit  map:"+map);
		}
		return map;
	}
	...
	private String generateRechargeCreditRequest(String fileCacheId,HashMap<String,String> p_map) throws Exception
	{
		...
		try
		{
			...
			rechargeDataBuffer.append("CREDIT="+trannsferAmt+",");
			rechargeDataBuffer.append("CARDTYPE="+(String)p_map.get("VOUCHER_TYPE")+",");
			rechargeDataBuffer.append("TYPE="+(String)p_map.get("RECHARGE_TYPE")+",");
			rechargeDataBuffer.append("VCNUMBER="+(String)p_map.get("VOUCHER_CODE")+",");
	...
	private String generateRechargeDebitRequest(String fileCacheId,HashMap<String,String> p_map) throws Exception
	{
		...
		try
		{
			...
			StringBuffer rechargeDataBuffer=new StringBuffer(rechargeCommand);
			rechargeDataBuffer.append("ID="+rechargeID+",");
			rechargeDataBuffer.append("SACCNT="+(String)p_map.get("SENDER_MSISDN")+",");
			rechargeDataBuffer.append("DACCNT="+(String)p_map.get("RECEIVER_MSISDN")+",");
			rechargeDataBuffer.append("FEE="+trannsferAmt+",");
			rechargeDataBuffer.append("OPERFEE="+(String)p_map.get("ACCESS_FEE")+",");
	...
	private HashMap<String,String> parseRechargeDebitResponse(String p_responseStr) throws Exception
	{
		...
		try
		{
			...
			//map.put("DESC",(p_responseStr.substring(index+5,p_responseStr.lastIndexOf("ATTR")-1)).trim()); 
			...
			if(map.get("RETN").equals("0000"))
			{
				index=p_responseStr.indexOf("AAFTEROPER=");
				if(index<0)
					throw new BTSLBaseException(this,"parseRechargeCreditResponse",InterfaceErrorCodesI.ERROR_RESPONSE);

				map.put("accountPostBalance",(p_responseStr.substring(index+11,p_responseStr.indexOf(",",index))).trim());
	...
	private HashMap<String,String> parseRechargeCreditResponse(String p_responseStr) throws Exception
	{
		...
		try
		{
			...
			if(map.get("RETN").equals("0000"))
			{
				...
				index=p_responseStr.indexOf("BALANCE=");
				
				if(index<0)
					throw new BTSLBaseException(this,"parseRechargeCreditResponse",InterfaceErrorCodesI.ERROR_RESPONSE);

				map.put("accountPostBalance",(p_responseStr.substring(index+8,p_responseStr.indexOf(",",index))).trim());
	...
	private String getResponseTransactionId(String p_responseStr) throws Exception
	{
		...
		try
		{
			...
			transIdStr=p_responseStr.substring(index+10,p_responseStr.indexOf("TXBEG"));
		} 
		catch (Exception e)
		{
	...
	public static void main(String[] args)
	{
		try
		{
			...
			ZTEINRequestFormatter formatter=new ZTEINRequestFormatter();
			String s="`SC`01d61.00    1.00PPS     00000002DLGCONFFFF10264101TXBEG FFFFACK:DISP PHS ACNTINFO:RETN=0000,DESC=SUCCESS ,ATTR=DN|MAXCHR|MINAMT|ACNTSTOP|SERVICESTART|CVSTOP|ACNTSTAT|ACNTLEFT|BRANDNAME|LOCKED|BLACKLIST|ERRORTIMES|MAXERRTIMES|PASS|FIRSTCALL|DEFLANG|BRANDINDEX|PAIDFLAG|CALLOUTPWD|CALLBLACKLIST|HASCALLOUTPWD|ACTIVEDATETIME,RESULT=989200403499|100000||2020-06-25|2018-06-27|2020-06-26|1|4197707|JAVAN -Prepaid Voice SIM (commercial)|0|N||2|000000|1|1|5000010|0||N|N|2018-06-27E3889A84";
			
			formatter.parseGetAccountInfoResponse(s);
	...
	{		
		...
		try
		{
			...
			map.put("DESC",(p_responseStr.substring(index+5,p_responseStr.lastIndexOf(",ATTR"))).trim());            
