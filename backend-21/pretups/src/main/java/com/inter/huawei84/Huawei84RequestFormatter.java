 ...
 *                                interface.
 */
public class Huawei84RequestFormatter {
    public Log _log = LogFactory.getLog("Huawei84RequestFormatter".getClass().getName());
    ...
    public String generateRequest(int p_action, HashMap p_map) throws BTSLBaseException, Exception {
        ...
        try {
            switch (p_action) {
            case Huawei84I.ACTION_LOGIN: {
                str = generateLoginRequest(p_map);
                break;
            }
            case Huawei84I.ACTION_LOGOUT: {
                str = generateLogoutRequest(p_map);
                break;
            }
            case Huawei84I.ACTION_ACCOUNT_INFO: {
                str = generateGetAccountInfoRequest(p_map);
                break;
            }
            case Huawei84I.ACTION_RECHARGE_CREDIT: {
                str = generateRechargeCreditRequest(p_map);
                break;
            }
            case Huawei84I.ACTION_IMMEDIATE_DEBIT: {
                str = generateImmediateDebitRequest(p_map);
                break;
            }
            case Huawei84I.ACTION_HEART_BEAT: {
                str = generateHeartBeatRequest(p_map);
                break;
            }
            case Huawei84I.ACTION_VALIDITY_ADJUST: {
    ...
    private HashMap parseGetAccountInfoResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (!Huawei84I.RESULT_OK.equals(status))// if status is not OK then
    ...
    public static void main(String[] args) {
        try {
            /*
             * inReconID="134.456.567";
             * inReconID1=inReconID.replaceAll("\\.","");
             * System.out.println("inReconID "+inReconID +" "+inReconID1);
             */
            /*
             * String str=
             * "BAB6F9F9`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`0004HBHBB7BDB7BD`SC`00F41.01internal     ACK English10000001DLGCON0000MMMMM TXEND0000ACK:MODI PPS BALANCE: RETN=0, DESC=\"Modifying subscriber's balance succeeded.\", ORGBALANCE=602896290, LASTBALANCE=602846212, SUCCNUM=1, SUCCLIST=1815050020, FAILNUM=0, FAILLIST="
             * ;
             * int index=str.indexOf("LASTBALANCE=");
             * String
             * lastBalance=str.substring(index+12,str.indexOf(",",index+12));
             * System.out.println("lastBalance "+lastBalance );
             */

            // String str="290000|0|30|0|290000|20080418|0|0|";
            /*
             * StringTokenizer attrToken=new StringTokenizer(str,"\\|");
             * System.out.println("tokens1 "+attrToken.countTokens());
             */
            /*
             * String[] str1 =str.split("\\|");
             * 
             * System.out.println("tokens2 "+str1.length);
             */
            Huawei84RequestFormatter obj = new Huawei84RequestFormatter();
