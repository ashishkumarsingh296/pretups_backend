...
public class ClaroDataWSResponseParser {

	private static Log _log = LogFactory.getLog(ClaroDataWSResponseParser.class.getName());
	...
	public HashMap parseRechargeCreditResponse(HashMap _requestMap) throws Exception
	{
		...
		try
		{
			...
			if(object!=null)
				recargasResponse=(EjecutarRecargaDatosResponse)object;
			else
			...
			if(recargasResponse!=null && !BTSLUtil.isNullString(recargasResponse.getCodigoRespuesta())){
				...
				if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_SUCCESS)){

					double preBalance=0;
					try{
						preBalance=Double.parseDouble(recargasResponse.getMonto())-Double.parseDouble((String)_requestMap.get("transfer_amount"));
					}catch (Exception e) {
						preBalance=0;
					}
					responseMap.put("INTERFACE_PRE_BALANCE",new DecimalFormat("#############").format(preBalance));
					responseMap.put("INTERFACE_POST_BALANCE",recargasResponse.getMonto());

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_TIMEOUT_INVALID)){

					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_MONTO_INVALID)){
					throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_INVALID_AMOUNT);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_CLIENT_INVALID_RANGE)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_INVALID_CLIENT);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_CUSTOMER_INVALID)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_INVALID_CLIENT);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_MSISDN_INVALID)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_INVALID_NUMBER);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_MONTO_AMOUNT_INVALID)){
					throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_INVALID_AMOUNT);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_ERRRO)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_ERROR1)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_ERROR2)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_ERROR3)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);

				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_ERROR6)){
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
				}else{
					throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
				}
	...
	{
		if(_log.isDebugEnabled()) _log.debug("parseImmediateDebitResponse","Entered ");
		HashMap responseMap = null;
		EjecutarRecargaDatosResponse recargasResponse=null;
		try
		{
			...
			if(recargasResponse!=null && !BTSLUtil.isNullString(recargasResponse.getCodigoRespuesta())){
				...
				if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_SUCCESS)){
					...
					responseMap.put("INTERFACE_PRE_BALANCE","0");
					responseMap.put("INTERFACE_POST_BALANCE","0");
				}else if(recargasResponse.getCodigoRespuesta().equalsIgnoreCase(ClaroDataWSI.RESPONSE_TIMEOUT_INVALID)){
