...
public class VASUrlConnection extends BTSLConnection {
    private HttpURLConnection _urlConnection = null;

    public VASUrlConnection(String p_url, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("VASUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive);
        try {
            ...
            setRequestHeader(p_contentLength, p_keepAlive);
            setPrintWriter();
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("VASUrlConnection", "Exception " + e.getMessage());
            if (_log.isDebugEnabled())
                _log.debug("VASUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("VASUrlConnection", "Exiting");
    }
     ...
     * @param p_keepAlive
     */
    private void setRequestHeader(long p_contenetLength, String p_keepAlive) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("setRequestHeader", "Entered... p_contenetLength:" + p_contenetLength + ", p_keepAlive:" + p_keepAlive);
        try {
