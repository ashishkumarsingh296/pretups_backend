...
public class IATINHandler implements IATInterfaceHandlerI {
	...
	private String _senderCurrency = null;
	...
	public void credit(IATInterfaceVO p_iatReqResVO){
	...
	/*		try {
				System.out.println("########"+p_iatReqResVO.getIatResponseCodeCredit());				
				System.out.println("########"+p_iatReqResVO.getIatINTransactionStatus());
	...
	public void sendRequestToIN(IATInterfaceVO p_iatReqResVO, String p_action, String p_reqSource) throws Exception {
		...
		final String methodName="sendRequestToIN"; 
                 if (_log.isDebugEnabled())
                                 _log.debug(methodName,"Entered"+p_iatReqResVO+" :: Action ::"+p_action);
		...
		CurrencyConversionDAO currencyDAO=new CurrencyConversionDAO();
		Connection p_con=null;
		MComConnectionI p_mcomCon = null;
		...
		try{
			...
			long requestedAmtDouble = p_iatReqResVO.getIatInterfaceAmt();
                        double convertedTransferAmount=0.0;
			//double convertedTransferAmount = c2sDAO.getCovertedAmount(p_con, p_iatReqResVO);
                        double fromCurrency = currencyDAO.getCovertedAmount(p_con,p_iatReqResVO.getIatSenderNWTYPE(),p_iatReqResVO.getIatSenderNWID());
                        double toCurrency = currencyDAO.getCovertedAmount(p_con,p_iatReqResVO.getReceiverCurrency(),p_iatReqResVO.getIatRcvrNWID());
                        try{
                                convertedTransferAmount = (p_iatReqResVO.getIatInterfaceAmt() / fromCurrency) * toCurrency;
                    String amountStr = new DecimalFormat("##############").format(convertedTransferAmount);
                    convertedTransferAmount = Double.parseDouble(amountStr);
			p_iatReqResVO.setIatAmountSentToIAT(IATUtil.getINAmountFromSystemAmountToIN(convertedTransferAmount, Double.parseDouble(_multiFact)));
		}
                        catch(Exception ex){
		...
		catch(Exception e)
		{
			...
			if (p_mcomCon != null) {
				p_mcomCon.close("IATINHandler#sendRequestToIN");
				p_mcomCon = null;
			}
		}
		requestMessage="<?xml version=\"1.0\"?><!DOCTYPE COMMAND PUBLIC \"-//Ocam//DTD XML Command 1.0//EN\" \"xml/command.dtd\"><COMMAND><TYPE>EXRCTRFREQ</TYPE><DATE></DATE><EXTNWCODE>"+p_iatReqResVO.getIatRcvrNWID()+"</EXTNWCODE><MSISDN>"+FileCache.getValue(_interfaceID,p_iatReqResVO.getIatRcvrNWID()+"_SENDER_MSISDN")+"</MSISDN><PIN>"+FileCache.getValue(_interfaceID,p_iatReqResVO.getIatRcvrNWID()+"_SENDER_PIN")+"</PIN><LOGINID></LOGINID><PASSWORD></PASSWORD><EXTCODE></EXTCODE><EXTREFNUM></EXTREFNUM><MSISDN2>"+p_iatReqResVO.getIatReceiverMSISDN()+"</MSISDN2><AMOUNT>"+p_iatReqResVO.getIatAmountSentToIAT()+"</AMOUNT><LANGUAGE1>0</LANGUAGE1><LANGUAGE2>1</LANGUAGE2><SELECTOR>1</SELECTOR></COMMAND>";
		//requestMessage="<?xml version=\"1.0\"?><!DOCTYPE COMMAND PUBLIC \"-//Ocam//DTD XML Command 1.0//EN\" \"xml/command.dtd\"><COMMAND><TYPE>EXRCTRFREQ</TYPE><DATE></DATE><EXTNWCODE>ML</EXTNWCODE><MSISDN>74200001896</MSISDN><PIN>2468</PIN><LOGINID></LOGINID><PASSWORD></PASSWORD><EXTCODE></EXTCODE><EXTREFNUM></EXTREFNUM><MSISDN2>74200121096</MSISDN2><AMOUNT>10000</AMOUNT><LANGUAGE1>0</LANGUAGE1><LANGUAGE2>1</LANGUAGE2><SELECTOR>S1</SELECTOR></COMMAND>";
		...
		if(_log.isDebugEnabled())_log.debug("CommonReceiver : processRequest","Instance Load VO="+instanceLoadVO);
...
//		urlToSend="http://"+"127.0.0.1"+":"+"9898"+Constants.getProperty("CHANNEL_SMS_RECHARGE_SERVLET");
		urlToSend="http://"+FileCache.getValue(_interfaceID,"IAT_IP_PORT")+Constants.getProperty("CHANNEL_SMS_RECHARGE_SERVLET");
		...
		try
		{
			...
			if(_log.isDebugEnabled())_log.debug(methodName,"Request sent   ="+requestMessage);
		...
		{
			_log.errorTrace(methodName,e);
			_log.error(methodName,"Exception in reading or writing  e:"+e.getMessage());
		}//end of catch-Exception
		finally
		{
			try{if(out!=null)out.close();}catch(Exception e){_log.errorTrace(methodName,e);}
			try{if(in!=null)in.close();}catch(Exception e){_log.errorTrace(methodName,e);}
		}//end of finally
		responseStr = buffer.toString();
		if(_log.isDebugEnabled())_log.debug(methodName,"Response Received   ="+responseStr);
		Connection con=null;
		try
		{		
			_senderZebraTXNID = (String) p_iatReqResVO.getIatSenderNWTRXID();// Transaction ID
			String c2sStatus=null;
			if(!BTSLUtil.isNullString(responseStr))
			{
				if(responseStr.contains("<TXNSTATUS>")){
				 int index = responseStr.indexOf("<TXNSTATUS>");
				 c2sStatus = responseStr.substring(index + "<TXNSTATUS>".length(), responseStr.indexOf("</TXNSTATUS>", index));				
				}
			}			
			if(!BTSLUtil.isNullString(c2sStatus) && IATI.CREDIT_RESPONSE_CODE.equalsIgnoreCase(c2sStatus)){			
			...
			p_iatReqResVO.setIatResponseCodeCredit(IATI.CREDIT_RESPONSE_CODE);
			}
			else{
				if(!BTSLUtil.isNullString(c2sStatus))
					p_iatReqResVO.setIatResponseMsgCredit("C2S failed with error code"+c2sStatus);
				else
					p_iatReqResVO.setIatResponseMsgCredit("error.general.processing");
                 p_iatReqResVO.setIatResponseCodeCredit(IATI.RECHARGE_CREDIT_RESPONSE_FAIL);
			}
		}
		/*		catch (BTSLBaseException be) 
		{
			if (con != null){try{con.rollback();}catch (Exception e){_log.errorTrace(methodName,e);}}
			throw be; 
		}*/
		catch (Exception e)
		{
			_log.errorTrace(methodName,e);
			if (con != null){try{con.rollback();}catch (Exception e1){_log.errorTrace(methodName,e);}}
			_log.error(methodName, "Exception e:" + e.getMessage() + InterfaceUtil.getPrintMap(_requestMap));
		...
		finally
		{
			...
			if (_log.isDebugEnabled())  _log.debug(methodName, "Exited p_iatReqResVO=" + p_iatReqResVO.toString());
		}
	...
	private void setInterfaceParameters(IATInterfaceVO p_iatReqResVO) throws BTSLBaseException, Exception {
		...
		try {
			...
			_senderCurrency = FileCache.getValue(_interfaceID, "SENDER_CURNCY");
			if (IATUtil.isNullString(_senderCurrency)) {
				_log.error("setInterfaceParameters", "_senderCurrency:" + _senderCurrency);
				EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "IATINHandler[setInterfaceParameters]", _senderZebraTXNID, _msisdn + " INTERFACE ID = " + _interfaceID, (String) p_iatReqResVO.getIatSenderNWTRXID(), "Sender Currency is not defined in IN File");
				throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
			_requestMap.put("SENDER_CURNCY", _senderCurrency);
			p_iatReqResVO.setIatSenderNWTYPE(_senderCurrency);
