...
import com.inter.claroca.cs5ca.cs5scheduler.NodeCloser;
import com.inter.claroca.cs5ca.cs5scheduler.NodeManager;
import com.inter.claroca.cs5ca.cs5scheduler.NodeScheduler;
import com.inter.claroca.cs5ca.cs5scheduler.NodeVO;

public class CS5CaINHandler implements InterfaceHandler
{
        private static Log _log = LogFactory.getLog(CS5CaINHandler.class.getName());

        private HashMap<String,String> _requestMap = null;//Contains the response of the request as key and value pair.
        private HashMap<String,String> _responseMap = null;//Contains the request parameter as key and value pair.
        ...
        private static CS5CaRequestFormatter _cs5CaRequestFormatter=null;
        ...
        static
        {
                if(_log.isDebugEnabled()) _log.debug("CS5CaINHandler[static]","Entered");
                try
                {
                        _cs5CaRequestFormatter = new CS5CaRequestFormatter();
                }
                catch(Exception e)
                {
                        _log.error("CS5CaINHandler[static]","While instantiation of CS5CaRequestFormatter get Exception e::"+e.getMessage());
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[static]","","", "","While instantiation of CS5CaRequestFormatter get Exception e::"+e.getMessage());
                }
                finally
                {
                        if(_log.isDebugEnabled()) _log.debug("CS5MobililINHandler[static]","Exited");
                }
        }
        ...
        public void validate(HashMap<String,String> p_requestMap) throws BTSLBaseException, Exception
        {
            ...
            try
            {
                    ...
                    if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"multFactor: "+multFactor);
                    if(InterfaceUtil.isNullString(multFactor))
                    {
                            _log.error(METHOD_NAME,"MULT_FACTOR  is not defined in the INFile");
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[validate]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
                    ...
                    String inStr4CustomerValidation = _cs5CaRequestFormatter.generateRequest(CS5CaI.ACTION_ACCOUNT_INFO,_requestMap);
                    //sending the AccountInfo request to IN along with validate action defined in CS5CaI interface
                    sendRequestToIN(inStr4CustomerValidation,CS5CaI.ACTION_ACCOUNT_INFO);
                    ...
                    if (CS5CaI.INSTALL_STATUS.equals((String)_requestMap.get("INTERFACE_STATUS")))
                    {
                            String serviceClassInstall = FileCache.getValue(_interfaceID,"SERVICE_CLASS_INSTALL");
                            _requestMap.put("SERVICE_CLASS", serviceClassInstall);
                            _requestMap.put("ACCOUNT_STATUS","INSTALL");
                            String amountStr="0";
                            _requestMap.put("accountValue1","0");
                    ...
                    else
                    {
						...
						Date serviceRemovalDate = InterfaceUtil.getDateFromDateString((String)_responseMap.get("serviceRemovalDate"), "yyyyMMdd");
						Date currentDate =InterfaceUtil.getDateFromDateString(InterfaceUtil.getCurrentDateString("yyyyMMdd"), "yyyyMMdd");
						if((supervisionExpiryDate.after(currentDate) || supervisionExpiryDate.equals(currentDate)) && (serviceFeeExpiryDate.after(currentDate) || serviceFeeExpiryDate.equals(currentDate)))
						_requestMap.put("ACCOUNT_STATUS","ACTIVE");
						else if(currentDate.after(supervisionExpiryDate) && (serviceFeeExpiryDate.after(currentDate) || serviceFeeExpiryDate.equals(currentDate)))
						...
						if(currentDate.after(serviceRemovalDate))
						_requestMap.put("ACCOUNT_STATUS","EXPIRED");
						String interfaceServiceClass=_responseMap.get("serviceClassCurrent");
						String service_class=interfaceServiceClass;
						try
						{
    						String serviceClassRangeType=FileCache.getValue(_interfaceID,"SERVICE_CLASS_RANGE_TYPE");
    						if(!InterfaceUtil.isNullString(serviceClassRangeType))
    			            {
    			                String[] serviceClassRange=serviceClassRangeType.split(",");
    			                int startRange=0,endRange=0;
    			                String[] serviceRange=null;
    			                if(serviceClassRange.length>0)
    			                {
        			                int intServiceClass=Integer.parseInt(interfaceServiceClass);
        			                for(int i=0;i<serviceClassRange.length;i++)
        			                {
        			                    serviceRange=null;
        			                    serviceRange=serviceClassRange[i].split(":");
        			                    startRange=Integer.parseInt(serviceRange[0].trim());
        			                    endRange=Integer.parseInt(serviceRange[1].trim());
        			                    if(intServiceClass>=startRange && intServiceClass<=endRange)
        			                    {
        			                        service_class=serviceRange[2].trim();
        			                        String dedicatedAccountID=FileCache.getValue(_interfaceID,service_class);
        			                        String dedicatedAcBalance=getDedicatedAccountValue(_responseMap,dedicatedAccountID);
                                                amountStr = InterfaceUtil.getSystemAmountFromINAmount(dedicatedAcBalance,Double.parseDouble(multFactor));
        			                        if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"service_class:"+service_class+", dedicatedAccountID:"+dedicatedAccountID+", previousBalance:"+_responseMap.get("accountValue1")+", replaced with dedicatedAcBalance:"+dedicatedAcBalance+", amountStr:"+amountStr);
        			                       // _responseMap.put("accountValue1",dedicatedAcBalance);
        	                                _requestMap.put("INTERFACE_PREV_BALANCE",amountStr);
        			                    }
        			                }
    			                }
    			            }
						}catch(Exception be){ _log.error(METHOD_NAME,"Exception be="+be.getMessage());}
						_requestMap.put("SERVICE_CLASS",service_class);
            ...
            catch(Exception e)
            {
                    _log.error(METHOD_NAME,e);
                    _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[validate]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String)_requestMap.get("NETWORK_CODE"), "While validation of the subscriber get the Exception e:"+e.getMessage());
                    throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            finally
            {
        ...
        public void credit(HashMap<String,String> p_requestMap) throws BTSLBaseException, Exception
        {
            ...
            try
            {
                    ...
                    {
                            _log.error(METHOD_NAME,"MULT_FACTOR  is not defined in the INFile");
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[credit]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    multFactor = multFactor.trim();
                    ...
                    _requestMap.put("OTT_CODE",_requestMap.get("ExternalData1"));
                    ...
                    try
                    {
                            ...
                            if(!BTSLUtil.isNullString(_requestMap.get("USD_AMNT_APPL")) && PretupsI.YES.equalsIgnoreCase(_requestMap.get("USD_AMNT_APPL"))){
                                        ...
                                        amountStr = String.valueOf(InterfaceUtil.getINAmountFromSystemAmountToIN(interfaceAmtDouble, currncyMultFactor));
                    ...
                    catch(Exception e)
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[credit]","REFERENCE ID = "+_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    if(_log.isDebugEnabled())
                    ...
                    //key value of requestMap is formatted into XML string for the validate request.
                    String serviceClass=_requestMap.get("SERVICE_CLASS");
                    int action=CS5CaI.ACTION_RECHARGE_CREDIT;
                    String dedicatedAccountID=null;
                    try{
                        dedicatedAccountID=FileCache.getValue(_interfaceID,serviceClass);
                        if(!InterfaceUtil.isNullString(dedicatedAccountID))
                        {
                            action=CS5CaI.ACTION_DEDICATED_ACCOUNT_CD;
                            if(_log.isDebugEnabled())
                                _log.debug(METHOD_NAME,"action:"+action+",dedicatedAccountID="+dedicatedAccountID);
                            _requestMap.put("DEDICATED_ACCOUNT_ID",dedicatedAccountID.trim());
                            _requestMap.put("DEDI_EXPIRY_DATE_S",addDaysInUtilDate(Integer.parseInt(_requestMap.get("VALIDITY_DAYS")),"yyyyMMdd"));
                        }
                    }catch(Exception be){_log.error(METHOD_NAME,"Exception be:"+be.getMessage());}
                    
                    String inStr = _cs5CaRequestFormatter.generateRequest(action,_requestMap);
                    try
                    {
                    ...
                    if(!_isRetryRequest)
                    {
                            ...
                            try
                            {
                                String postBalanceStr="0";
                                //if(CS5CaI.ACTION_DEDICATED_ACCOUNT_CD==action)
                                if(!InterfaceUtil.isNullString(dedicatedAccountID))
                                {
                                    String dedicatedAccID=_responseMap.get("DEDICATED_ACC_ID");
                                    String dedicatedAccValue=_responseMap.get("DEDICATED_ACC_VALUE");
                                    if(dedicatedAccountID.equalsIgnoreCase(dedicatedAccID))
                                    {
                                        postBalanceStr = InterfaceUtil.getSystemAmountFromINAmount(dedicatedAccValue,multFactorDouble);
                                        _requestMap.put("INTERFACE_POST_BALANCE",postBalanceStr);
                                        _requestMap.put("NEW_EXPIRY_DATE", InterfaceUtil.getInterfaceDateFromDateString(getDedicatedAccountExpiryDate(_responseMap,dedicatedAccountID), "yyyyMMdd"));
                                    }
                                }else
                                {
                            ...
                            catch(Exception e)
                            {
                                    _log.error(METHOD_NAME,e);
                                    _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[credit]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "NEWBALANCE  is not Numeric");
                                    throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_RESPONSE);
                            }
            ...
            catch (BTSLBaseException be)
            {
                    ...
                    catch(Exception e)
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[credit]", _referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in credit");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
            }
            catch (Exception e)
            {
                    _log.error(METHOD_NAME,e);
                    _log.error(METHOD_NAME, "Exception e:" + e.getMessage());
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[credit]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "While credit get the Exception e:"+e.getMessage());
                    throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            finally
            {
			if (mcomCon != null) {
				mcomCon.close("CS5CaINHandler#credit");
				mcomCon = null;
			}
                    if (_log.isDebugEnabled())
        ...
        public void creditAdjust(HashMap<String,String> p_requestMap) throws BTSLBaseException, Exception
        {
            ...
            try
            {
                    ...
                    {
                            _log.error(METHOD_NAME,"MULT_FACTOR  is not defined in the INFile");
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[creditAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is  not defined in the INFile");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    multFactor = multFactor.trim();
                    //Set the interface parameters into requestMap
                    ...
                    try
                    {
                            double cs5MultFactorDouble=Double.parseDouble(multFactor);
                            double interfaceAmtDouble = Double.parseDouble((String)_requestMap.get("INTERFACE_AMOUNT"));
                            systemAmtDouble = InterfaceUtil.getINAmountFromSystemAmountToIN(interfaceAmtDouble,cs5MultFactorDouble);
                            amountStr=String.valueOf(systemAmtDouble);
                            //Based on the INFiles ROUND_FLAG flag, we have to decide to round the transfer amount or not.
                            String roundFlag = FileCache.getValue(_interfaceID,"ROUND_FLAG");
                            if(_log.isDebugEnabled())
                            ...
                            {
                                    roundFlag="Y";
                                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MINOR, "CS5MobililINHandler[creditAdjust]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn ," INTERFACE ID = "+_interfaceID, "Network code = "+(String) _requestMap.get("NETWORK_CODE"), "ROUND_FLAG is not defined in IN File hence system taken the default value=N.");
                            }
                            //If rounding of amount is allowed, round the amount value and put this value in request map.
                            if("Y".equals(roundFlag.trim()))
                            {
                    ...
                    catch(Exception e)
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[creditAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    ...
                    String inStr =null;
                   /* if(selectorCode<=3)
                            action=CS5CaI.ACTION_IMMEDIATE_DEBIT;
                    else
                            action=CS5CaI.ACTION_DEDICATED_ACCOUNT_CD;*/
                    String serviceClass=_requestMap.get("SERVICE_CLASS");
                    action=CS5CaI.ACTION_IMMEDIATE_DEBIT;
                    ...
                    try{
                        ...
                        if(!InterfaceUtil.isNullString(dedicatedAccountID))
                        {
                            action=CS5CaI.ACTION_DEDICATED_ACCOUNT_CD;
                            _requestMap.put("DEDICATED_ACCOUNT_ID",dedicatedAccountID.trim());
                            _requestMap.put("DEDI_EXPIRY_DATE_S",addDaysInUtilDate(Integer.parseInt(_responseMap.get("VALIDITY_DAYS")),"yyyyMMdd"));
                        }
                    }catch(Exception be){_log.error(METHOD_NAME,"Exception be:"+be.getMessage());}
                    
                    
                    inStr = _cs5CaRequestFormatter.generateRequest(action,_requestMap);
            ...
            catch (BTSLBaseException be)
            {
                    ...
                    catch(Exception e)
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[creditAdjust]", _referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in creditAdjust");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
            }
            catch (Exception e)
            {
                    _log.error(METHOD_NAME,e);
                    _log.error(METHOD_NAME, "Exception e:"+e.getMessage());
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[creditAdjust]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while creditAdjust e:"+e.getMessage());
                    throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            finally
            {
        ...
        public void debitAdjust(HashMap<String,String> p_requestMap) throws BTSLBaseException, Exception
        {
            ...
            try
            {
                    ...
                    {
                            _log.error(METHOD_NAME,"MULT_FACTOR  is not defined in the INFile");
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[debitAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is  not defined in the INFile");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    multFactor = multFactor.trim();
                    //Set the interface parameters into requestMap
                    ...
                    {
                            double cs5MultFactorDouble=Double.parseDouble(multFactor);
                            double interfaceAmtDouble = Double.parseDouble((String)_requestMap.get("INTERFACE_AMOUNT")) - (Integer.parseInt(_requestMap.get("CG_MULT_FACTOR"))*(Double.parseDouble((String)_requestMap.get("TAX_AMOUNT"))));
                            systemAmtDouble = InterfaceUtil.getINAmountFromSystemAmountToIN(interfaceAmtDouble,cs5MultFactorDouble);
                            amountStr=String.valueOf(systemAmtDouble);
                            //Based on the INFiles ROUND_FLAG flag, we have to decide to round the transfer amount or not.
                            String roundFlag = FileCache.getValue(_interfaceID,"ROUND_FLAG");
                    ...
                    catch(Exception e)
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[debitAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"transfer_amount:"+amountStr+" multFactor:"+multFactor);
                    //set transfer_amount in request map as amountStr (which is round value of INTERFACE_AMOUNT)
                    ...
                    if(!InterfaceUtil.isNullString((String)_requestMap.get("VALIDITY_DAYS")))
                    {
                            try
                            {
                                    ...
                                    validityDays =Integer.parseInt(((String)_requestMap.get("VALIDITY_DAYS")).trim());
                                    if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"validityDays::"+validityDays);
                                    //Set the validity days into request map with key as 'relative_date_adjustment_service_fee'
                                    _requestMap.put("supervisionExpiryDateRelative","-"+String.valueOf(validityDays));
                            }
                            catch(Exception e)
                    ...
                    if(!InterfaceUtil.isNullString((String)_requestMap.get("GRACE_DAYS")))
                    {
                            try
                            {
                                    //For the adjustment make grace days as negative.
                                    graceDays =Integer.parseInt(((String)_requestMap.get("GRACE_DAYS")).trim());
                                    if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"graceDays::"+graceDays);
                                    //Set the grace days into request map with key as 'relative_date_adjustment_supervision'
                                    _requestMap.put("serviceFeeExpiryDateRelative","-"+String.valueOf(graceDays));
                            }
                            catch(Exception e)
                    ...
                    //sending the CreditAdjust request to IN along with ACTION_IMMEDIATE_DEBIT action defined in CS5CaI interface
                    sendRequestToIN(inStr,action);
                    //set TRANSACTION_STATUS as Success in request map
                    _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
                    //set INTERFACE_POST_BALANCE into request map as obtained thru response map.
                    _requestMap.put("POST_BALANCE_ENQ_SUCCESS", "N");
                    if(!BTSLUtil.isNullString(_requestMap.get("TRANSACTION_STATUS")) && InterfaceErrorCodesI.SUCCESS.equalsIgnoreCase(_requestMap.get("TRANSACTION_STATUS"))){
                          _requestMap.put("OTT_CODE",_requestMap.get("OTT_DEBIT_COMM"));
                          try
                          {
                                  double cs5MultFactorDouble=Double.parseDouble(multFactor);
                                  double interfaceAmtDouble = Double.parseDouble((String)_requestMap.get("TAX_AMOUNT"));
                                  systemAmtDouble = InterfaceUtil.getINAmountFromSystemAmountToIN(interfaceAmtDouble,cs5MultFactorDouble);
                                  amountStr=String.valueOf(systemAmtDouble);
                                  String roundFlag = FileCache.getValue(_interfaceID,"ROUND_FLAG");
                                  if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"From file cache roundFlag = "+roundFlag);
                          ...
                          selectorCode=Integer.valueOf((String)_requestMap.get("CARD_GROUP_SELECTOR"));
                          serviceClass=_requestMap.get("SERVICE_CLASS");
                          action=CS5CaI.ACTION_IMMEDIATE_DEBIT;
            ...
            catch (BTSLBaseException be)
            {
                    ...
                    catch(Exception e)
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME,"Exception e:"+e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[debitAdjust]", _referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in creditAdjust");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
            }
            catch (Exception e)
            {
                    _log.error(METHOD_NAME,e);
                    _log.error(METHOD_NAME, "Exception e:"+e.getMessage());
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5MobililINHandler[debitAdjust]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while creditAdjust e:"+e.getMessage());
                    throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            finally
            {
        ...
        private void sendRequestToIN(String p_inRequestStr, int p_action) throws BTSLBaseException
        {
            ...
            if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"Entered _msisdn="+_msisdn+" p_action="+p_action+" , p_inRequestStr="+p_inRequestStr);
            ...
            CS5CaUrlConnection cs5URLConnection = null;
            long startTime=0;
            long endTime=0;
            ...
            try
            {
                if(p_action!=CS5CaI.ACTION_ACCOUNT_DETAILS)
                ...
                while(_ambgCurrentRetryCount++ <=_ambgMaxRetryCount)
                {
                        try
                        {
                                if(p_action!=CS5CaI.ACTION_ACCOUNT_INFO && p_action!=CS5CaI.ACTION_ACCOUNT_DETAILS)
                                {
                                        if(_log.isDebugEnabled())_log.error(METHOD_NAME,"SENDING RETRY........"+ (_ambgCurrentRetryCount-1)+ "_isRetryRequest "+_isRetryRequest + "IN Transaction Id" +(String)_requestMap.get("IN_RECON_ID"));
                                }
                                long startTimeNode = System.currentTimeMillis();
                                if(_log.isDebugEnabled())_log.debug(METHOD_NAME,"Start time to find the scheduled Node startTimeNode::"+startTimeNode+"miliseconds");
                                ...
                                for(int loop=1;loop<=conRetryNumber;loop++)
                                {
                                    try
                                    {
                                            ...
                                            if(_log.isDebugEnabled())_log.error(METHOD_NAME,"URL PICKED IS ......."+ cs5NodeVO.getUrl());
                                            ...
                                            catch(BTSLBaseException be)
                                            {
                                                    if(loop==conRetryNumber)
                                                    {
                                                            if("Y".equals(_requestMap.get("ADJUST")) && "C".equals(_requestMap.get("INTERFACE_ACTION")) && "S".equals(_userType))
                                                            {
                                                                    ...
                                                                    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"Sender Credit Back case. But all nodes are suspended. So marking the response as AMBIGUOUS");
                                                                    throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
                                                            }
                                                            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"MAXIMUM SHIFTING OF NODE IS REACHED");
                                            ...
                                            if(CS5CaI.ACTION_ACCOUNT_INFO==p_action || CS5CaI.ACTION_ACCOUNT_DETAILS==p_action)
                                            ...
                                            cs5URLConnection = new CS5CaUrlConnection(cs5NodeVO.getUrl(),cs5NodeVO.getUsername(),cs5NodeVO.getPassword(),cs5NodeVO.getConnectionTimeOut(),readTimeOut,cs5NodeVO.getKeepAlive(),p_inRequestStr.length(),hostName,userAgent);
                                            //break the loop on getting the successful connection for the node;
                                            if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"Connection of _interfaceID ["+_interfaceID+"] for the Node Number ["+cs5NodeVO.getNodeNumber()+"] created after the attempt number(loop)::"+loop);
                                            break;
                                    }
                                    catch(BTSLBaseException be)
                                    {
                                    ...
                                    catch(Exception e)
                                    {
                                            ...
                                            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"Exception while getting the connection for CS5CaIN with INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs5NodeVO.getNodeNumber()+"]");
                                            ...
                                            if(loop==conRetryNumber)
                                            {
                                                    ...
                                                    {
                                                            _requestMap.put("INTERFACE_STATUS",InterfaceErrorCodesI.AMBIGOUS);
                                                            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"Sender Credit Back case. But all nodes are Blocked. So marking the response as AMBIGUOUS");
                                                            throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
                                                    }
                                                    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"MAXIMUM SHIFTING OF NODE IS REACHED");
                                                    throw new BTSLBaseException(InterfaceErrorCodesI.RETRY_ATTEMPT_FAILED);
                        ...
                        catch(Exception e)
                        {
                                ...
                                EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"),"While sending request to CS5Ca IN INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs5NodeVO.getNodeNumber()+"] Exception::"+e.getMessage());
                        ...
                        try
                        {
                                ...
                                try
                                {
                                        ...
                                        if(warnTime<=(endTime-startTime))
                                        {
                                                ...
                                                EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.INTERFACES,EventStatusI.RAISED,EventLevelI.MAJOR,"CS5CaINHandler[sendRequestToIN]",_inTXNID,_msisdn,(String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"CS5CaIN is taking more time than the warning threshold. Time: "+(endTime-startTime)+"INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs5NodeVO.getNodeNumber()+"]");
                                        }
                                }
                                catch(Exception e)
                                {
                                        _log.error(METHOD_NAME,e);
                                        _log.error(METHOD_NAME,"Exception e::"+e.getMessage());
                                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"),"While getting the response from the CS5CaIN for INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs5NodeVO.getNodeNumber()+"]"+"Exception="+e.getMessage());
                                ...
                                if(!CS5CaI.HTTP_STATUS_200.equals(httpStatus))
                                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_BAD_REQUEST);
                                if (InterfaceUtil.isNullString(responseStr))
                                {
                                        EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Blank response from CS5CaIN");
                                        _log.error(METHOD_NAME, "NULL response from interface");
                                        _nodeCloser.updateCountersOnAmbiguousResp(cs5NodeVO);
                                        if(_ambgCurrentRetryCount > _ambgMaxRetryCount)
                                        {
                                                _log.error(METHOD_NAME,"Read time out occured. Retry attempts exceeded so throwing AMBIGOUS exception");
                                                EventHandler.handle(EventIDI.INTERFACE_RESPONSE_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "CS5CaINHandler[sendRequestToIN]",_inTXNID,_msisdn,(String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,  "NULL response from IN. Retry attempts exceeded so throwing AMBIGOUS exception");
                                                _requestMap.put("INTERFACE_STATUS",InterfaceErrorCodesI.AMBIGOUS);
                                                throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
                                        }
                                        Thread.sleep(retrySleepTime);
                                ...
                                if(cs5NodeVO.isSuspended())
                                        _nodeCloser.resetCounters(cs5NodeVO);
                                _responseMap=_cs5CaRequestFormatter.parseResponse(p_action,responseStr,_responseMap);
                                ...
                                if(!InterfaceUtil.isNullString(faultCode))
                                {
                                        ...
                                        _log.info(METHOD_NAME, "faultCode::"+faultCode +"_inTXNID::"+_inTXNID+" _msisdn::"+_msisdn);
                                        ...
                                        EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,(String)_responseMap.get("faultString"));
                                ...
                                {
                                        if(CS5CaI.SUBSCRIBER_NOT_FOUND.equals(responseCode))
                                        {
                                                _log.error(METHOD_NAME,"Subscriber not found with MSISDN::"+_msisdn);
                                                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Subscriber is not found at IN");
                                                throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND);
                                        }
                                        else if(CS5CaI.OLD_TRANSACTION_ID.equals(responseCode))
                                        {
                                                if(!_isRetryRequest)
                                                {
                                                        if(_log.isDebugEnabled())_log.debug(METHOD_NAME,"_isRetryRequest:"+_isRetryRequest);
                                                        EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID,EventComponentI.INTERFACES,EventStatusI.RAISED,EventLevelI.FATAL,"CS5CaINHandler[sendRequestToIN]",_inTXNID,_msisdn,(String)_requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Transaction ID mismatch");
                                        ...
                                        else
                                        {
                                                ...
                                                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS5CaINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Error code received from IN "+responseCode);
                                                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
                                        }
                                }
                        }
            ...
            finally
            {
                    ...
                    {
                            _log.error(METHOD_NAME,e);
                            _log.error(METHOD_NAME, "While closing CS5CaIN Connection Exception e::" + e.getMessage());
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[sendRequestToIN]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action, "Not able to close connection:" + e.getMessage()+"INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs5NodeVO.getNodeNumber()+"]");
                    }
                    if (_log.isDebugEnabled())
        ...
        {
        	String METHOD_NAME="setInterfaceParameters";
            ...
            try
            {
                    ...
                    if(InterfaceUtil.isNullString(systemStatusMappingCrAdj))
                    {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT_ADJ is not defined in the INFile.");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    if(InterfaceUtil.isNullString(systemStatusMappingDbtAdj))
                    {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_DEBIT_ADJ is not defined in the INFile.");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    if(InterfaceUtil.isNullString(systemStatusMappingCrBck))
                    {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT_BCK is not defined in the INFile.");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    if(InterfaceUtil.isNullString(cancelCommandStatusMapping))
                    {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "CANCEL_COMMAND_STATUS_MAPPING is not defined in the INFile.");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    if(InterfaceUtil.isNullString(cancelNA))
                    {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "CANCEL_NA is not defined in the INFile.");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    {
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "NODE_TYPE is not defined in IN File ");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    {
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "HOST_NAME is not defined in IN File ");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    {
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "CURRENCY is not defined in IN File ");
                            throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                    }
                    ...
                    if(InterfaceUtil.isNullString(extData1))
                            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "EXTERNAL_DATA1 is not defined in IN File ");
                    ...
                    if(InterfaceUtil.isNullString(extData2))
                            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "EXTERNAL_DATA2 is not defined in IN File ");
                ...
                String refillProfileID=FileCache.getValue(_interfaceID, "REFILL_PROFILE_ID");
                if(InterfaceUtil.isNullString(refillProfileID))
                {
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "Refill Profile Id is not defined in IN File ");
                        throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("REFILL_PROFILE_ID",refillProfileID.trim());
                String ottDebit=FileCache.getValue(_interfaceID, "OTT_DEBIT");
                if(InterfaceUtil.isNullString(ottDebit))
                {
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "OTT_DEBIT is not defined in IN File ");
                        throw new BTSLBaseException(this,METHOD_NAME,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
        ...
        {
        	String METHOD_NAME="setLanguageFromMapping";
            if(_log.isDebugEnabled())
            ...
            try
            {
                    ...
                    if(_log.isDebugEnabled()) _log.debug(METHOD_NAME,"mappingString::"+mappingString +" langFromIN::"+langFromIN);
                    ...
                    if(mappingNotFound)
                            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "CS5CaINHandler[setLanguageFromMapping]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Mapping for language received from IN =  "+langFromIN +" is not defined in IN file Hence setting the Default language");
            ...
            catch(Exception e)
            {
                    _log.error(METHOD_NAME,e);
                    _log.error(METHOD_NAME,"Exception e::"+e);
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[setLanguageFromMapping]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "While mapping the IN Language with the system Language getting the Exception =" + e.getMessage());
            }
            finally
            {
                    if(_log.isDebugEnabled())
        ...
        protected static synchronized String getINTransactionID(HashMap p_requestMap) throws BTSLBaseException
        {
            ...
            try
            {
                    ...
                    if(InterfaceUtil.isNullString(instanceID))
                    {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS5CaINHandler[validate]","","" , (String) p_requestMap.get("NETWORK_CODE"), "Instance id[INSTANCE_ID] is not defined in IN File");
                            throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
                    }
                    mydate = new Date();
            ...
            {
            	if (_log.isDebugEnabled())  _log.debug(METHOD_NAME, "Exited transactionId="+transactionId);
            }
        ...
        {
        	String METHOD_NAME="getDedicatedAccountValue";
            if (_log.isDebugEnabled()) _log.debug(METHOD_NAME, "Entered with p_dedicatedAccID="+p_dedicatedAccID);
            ...
            {
                    if (_log.isDebugEnabled())   _log.debug(METHOD_NAME, "Exited with dedicatedAccValue="+dedicatedAccValue);
            }
        ...
        public String  getDedicatedAccountExpiryDate(HashMap p_responseMap, String p_dedicatedAccID) throws Exception
        {
            String METHOD_NAME="getDedicatedAccountExpiryDate";
            if (_log.isDebugEnabled()) _log.debug(METHOD_NAME, "Entered with p_dedicatedAccID="+p_dedicatedAccID);
            String dedicatedExpDate="";
            String split="\\|";
            try
            {
                if(!InterfaceUtil.isNullString((String)p_responseMap.get("DEDICATED_ACC_ID")))
                {
                    ...
                    {
                            String[] dedAccIDArray=((String)p_responseMap.get("DEDICATED_ACC_ID")).split(split);
                            String[] dedAccValueArray=((String)p_responseMap.get("DEDICATED_ACC_EXPIRY")).split(split);
                            for(int i=0; i<dedAccIDArray.length; i++)
                            {
                                    if(p_dedicatedAccID.equals(dedAccIDArray[i]))
                                    {
                                            dedicatedExpDate=dedAccValueArray[i];
                                            break;
                                    }
                            }
                    }
            ...
            {
                    if (_log.isDebugEnabled())   _log.debug(METHOD_NAME, "Exited with dedicatedExpDate="+dedicatedExpDate);
            }
            return dedicatedExpDate;
        }
        public static String addDaysInUtilDate(int p_no_ofDays,String p_format)
        {
        	final String METHOD_NAME="addDaysInUtilDate";
            ...
            newDateString= sdf.format(cal.getTime());
            if (_log.isDebugEnabled())   _log.debug(METHOD_NAME, "Exited with newDateString="+newDateString);
            return newDateString;
        }
        
        public static void main(String[] args) {
        	System.out.println(addDaysInUtilDate(Integer.parseInt("365"), "yyyyMMdd"));
		}
}
