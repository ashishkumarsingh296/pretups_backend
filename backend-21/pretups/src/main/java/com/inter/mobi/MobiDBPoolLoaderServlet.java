 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class MobiDBPoolLoaderServlet extends HttpServlet {
    private Log _log = LogFactory.getLog(MobiDBPoolLoaderServlet.class.getName());
    private MobiDBPoolManager _dbPoolManager = null;

    /**
     * Constructor of the object.
     */
    public MobiDBPoolLoaderServlet() {
        super();
    }
    ...
    public void destroy() {
        ...
        String dbPoolIDs = Constants.getProperty("MOBI_DB_POOL_IN_IDS");
        if (_log.isDebugEnabled())
    ...
    public void initialize() {
        ...
        String dbINIds = null;
        try {
            _dbPoolManager = new MobiDBPoolManager();
            dbINIds = Constants.getProperty("MOBI_DB_POOL_IN_IDS");
            if (_log.isDebugEnabled())
                _log.debug("initialize", "dbINIds = " + dbINIds);
            // Initializes the MobiDBPoolManager if there are IN_IDS defined
            // under key MOBI_DB_POOL_IN_IDS.
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, dbINIds, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "MobiDBPoolLoaderServlet[initialize]", "", "", "", "Exception while initialize MobiDBPoolManager Exception:" + e.getMessage());
    ...
    public void initialize(String p_interfaceId) {
        ...
        try {
            ...
            e.printStackTrace();
            _log.error("initialize", "Exception e::" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, p_interfaceId, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MINOR, "MobiDBPoolLoaderServlet[initialize]", "", "", "", "Exception while initialize MobiDBPoolManager Parameters Exception:" + e.getMessage());
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
                _log.debug("initialize", "Exited");
