 ...
 *                              --
 *                              -----------------------------------------------
 *                              vikas c yadav Nov 05,2007 Initial Creation
 * 
 *                              ------------------------------------------------
 *                              ------------------------------------------------
 * 
...
public class BankINUrlConnection extends BTSLConnection {
     ...
     * @throws Exception
     */
    public BankINUrlConnection(String p_url, int p_connecttimeout, int p_readtimeout, String p_keepAlive, String p_baseAuth) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("BankINUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connecttimeout + " p_readtimeout=" + p_readtimeout + " p_keepAlive=" + p_keepAlive);
        try {
            ...
            _urlConnection.setRequestProperty("Authorization", "Basic " + p_baseAuth);
            setRequestHeader(p_keepAlive);
            setPrintWriter();
            // setBufferedReader();
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("BankINUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("BankINUrlConnection", "Exiting");
    }
