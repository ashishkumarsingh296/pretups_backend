package com.inter.meditel.waradvcaut;
 ...
 *                              ------------------------------------------------
 *                              ------------------------------------------------
 *                              This File is used for Warranty,Advance , Caution
 *                              recharge Flat file in Meditel .
 */
import java.io.File;
...
public class WarrAdvCautFlatFile {
    private static Log _log = LogFactory.getLog(WarrAdvCautFlatFile.class.getName());
    ...
    public boolean createFlatFile(HashMap p_data) throws BTSLBaseException {
        ...
        try {
            flatfile = false;
            Date date = new Date();
            String service = (String) p_data.get("REQ_SERVICE");
            service = service.trim();

            if (service.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_WARRANTY_RECHARGE)) {
                System.out.println("FLATFILE_PATH_WARRANTY" + filePath);
                filePath = FileCache.getValue(intefaceid, "FLATFILE_PATH_WARRANTY");
                String fileNameForWarranty = FileCache.getValue(intefaceid, "FLATFILENAME_WRC");
                fileName = fileNameForWarranty + BTSLUtil.getDateStringFromDate(date, "yyyyMMddHHmmss") + ".txt";
            } else if (service.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_ADVANCE_RECHARGE)) {
                filePath = FileCache.getValue(intefaceid, "FLATFILE_PATH_ADVANCE");
                String fileNameForAdvance = FileCache.getValue(intefaceid, "FLATFILENAME_ADV");
                fileName = fileNameForAdvance + BTSLUtil.getDateStringFromDate(date, "yyyyMMddHHmmss") + ".txt";
            } else if (service.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_CAUTION_RECHARGE)) {
                filePath = FileCache.getValue(intefaceid, "FLATFILE_PATH_CAUTION");
                String fileNameForCaution = FileCache.getValue(intefaceid, "FLATFILENAME_CAUT");
                fileName = fileNameForCaution + BTSLUtil.getDateStringFromDate(date, "yyyyMMddHHmmss") + ".txt";
            } else if (service.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_BILLPAY)) {
