...
public class NodeScheduler {
    ...
    public NodeScheduler(String p_cs3INId) throws BTSLBaseException {
    	final String METHOD_NAME="NodeScheduler[constructor]";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, "Entered p_cs3INId::" + p_cs3INId);
        ...
        try {
            ...
            try {
                ...
                if (_log.isDebugEnabled())
                    _log.debug(METHOD_NAME, "MAX_NODE::" + _totalNodes);
            } catch (Exception e) {
            ...
            try {
                ...
                if (_log.isDebugEnabled())
                    _log.debug(METHOD_NAME, "MAX_ALWD_NO::" + _maxAllowedNum);
            } catch (Exception e) {
            	_log.errorTrace(METHOD_NAME, e);
            ...
            try {
                ...
                if (_log.isDebugEnabled())
                    _log.debug(METHOD_NAME, "RETRY_NUM::" + _retryNum);
            } catch (Exception e) {
            	_log.errorTrace(METHOD_NAME, e);
            ...
            try {
                ...
                if (_log.isDebugEnabled())
                    _log.debug(METHOD_NAME, "HEADER_HOST_NAME::" + _headerHostName);
            } catch (Exception e) {
            	_log.errorTrace(METHOD_NAME, e);
            ...
            try {
                ...
                if (_log.isDebugEnabled())
                    _log.debug(METHOD_NAME, "HOST_NAME::" + _hostName);
            } catch (Exception e) {
            	_log.errorTrace(METHOD_NAME, e);
            ...
            try {
                ...
                if (_log.isDebugEnabled())
                    _log.debug(METHOD_NAME, "USER_AGENT::" + _userAgent);
            } catch (Exception e) {
            	_log.errorTrace(METHOD_NAME, e);
            ...
            for (int l = 1; l <= _totalNodes; l++) {
                try {
                    ...
                    if (_log.isDebugEnabled())
                        _log.debug(METHOD_NAME, "url_" + l + "::" + url);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "CONN_TIMEOUT_" + l + "::" + connectionTimeOut);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "MAX_CON_NODE_" + l + "::" + maxConPerNode);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "VAL_READ_TIMEOUT_" + l + "::" + readTimeOutVal);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "TOP_READ_TIMEOUT_" + l + "::" + readTimeOutTop);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "WARN_TIME_" + l + "::" + readTimeOutVal);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "EXPIRY_TIME_" + l + "::" + expiryTimeStr);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "KEEP_ALIVE_" + l + "::" + keepAlive);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "USERNAME_" + l + "::" + userName);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled())
                            _log.debug(METHOD_NAME, "PASSWORD_" + l + "::" + password);
                    ...
                    if (InterfaceUtil.isNullString(numberOfAmbguousTxnAllowedStr) || !InterfaceUtil.isNumeric(numberOfAmbguousTxnAllowedStr.trim())) {
                        _log.error(METHOD_NAME, "NO_ALLWD_AMB_TXN is either not defined in the INFile or its value is not numeric");
                    ...
                    if (InterfaceUtil.isNullString(thresholdTimeStr) || !InterfaceUtil.isNumeric(thresholdTimeStr.trim())) {
                        _log.error(METHOD_NAME, "THRESHOLD_TIME is either not defined in the INFile or its value is not numeric");
            ...
            _log.error(this, METHOD_NAME, e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS3_NODE_INITIALIZATION);
        } finally {
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, "Exiting _nodeTable::" + _nodeTable);
        }
    }
    ...
    public synchronized NodeVO getNodeVO(String p_transId) throws BTSLBaseException {
    	final String METHOD_NAME="getNodeVO";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, p_transId, "Entered p_transId" + p_transId);
        ...
        try {
            ...
            if (nodeNumStr == null) {
                _log.error(METHOD_NAME, p_transId, "There is no scheduled node to process the request for p_transId::" + p_transId);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS3_NODE_SCHEDULING);
            }
            cs3NodeVO = (NodeVO) _nodeTable.get(nodeNumStr);
            if (cs3NodeVO == null) {
                _log.error(METHOD_NAME, p_transId, "No detail found for the Node number::" + nodeNumStr);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS3_NODE_SCHEDULING);
            }
        } catch (BTSLBaseException be) {
            _log.error(METHOD_NAME, p_transId, "while processing the request for p_transId::" + p_transId + " get BTSLBaseException  be::" + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.errorTrace(METHOD_NAME, e);
            ...
            _log.error(METHOD_NAME, p_transId, "Exception  e::" + e.getMessage() + " p_transId::" + p_transId);
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS3_NODE_SCHEDULING);
        } finally {
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, p_transId, "Exited cs3NodeVO::" + cs3NodeVO);
        }
        return cs3NodeVO;
    }
    ...
    private synchronized String getNode(String p_transId) throws BTSLBaseException {
    	final String METHOD_NAME="getNode";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, p_transId, "Entered p_transId::" + p_transId);
        boolean isNext = true;
        String nodeNumStr = null;
        ...
        try {
            ...
            while (isNext) {
                try {
                    ...
                    if (_log.isDebugEnabled())
                        _log.debug(METHOD_NAME, p_transId, "tempCount::" + tempCount);
                    ...
                    if (_log.isDebugEnabled())
                        _log.debug(METHOD_NAME, p_transId, "NodeNumber [" + _nodeNumber + "] is blocked" + "p_transId::" + p_transId);
            ...
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, p_transId, "Exited nodeNumStr::" + nodeNumStr);
        }
        return nodeNumStr;
    }
    ...
    public boolean isExpired(long p_blockedAt, long p_expiryDuration) throws BTSLBaseException {
    	final String METHOD_NAME="isExpired";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, "Entered p_blockedAt::" + p_blockedAt + " p_expiryDuration::" + p_expiryDuration);
        boolean isExp = false;
        try {
            long currentTime = System.currentTimeMillis();
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, "currentTime::" + currentTime);
            ...
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, "isExp::" + isExp);
        }
        return isExp;
    }
