...
public class NodeCloser {
    ...
    public synchronized void checkExpiry(NodeVO p_nodeVO) throws BTSLBaseException {
    	final String METHOD_NAME="checkExpiry";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, "Entered p_nodeVO:" + p_nodeVO);
        ...
        try {
            ...
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, "Exited isSuspensionExpired:" + isSuspensionExpired);
        }
    }
    ...
    public synchronized void updateCountersOnAmbiguousResp(NodeVO p_nodeVO) {
    	final String METHOD_NAME="updateCountersOnAmbiguousResp";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, "Entered printNodeVO:" + printNodeVO(p_nodeVO));
        try {
            ...
            // then only put the changed status in to map.previous status 'null'
            // is checked if number of max allowed amb txn=1.
            ...
            if (_log.isDebugEnabled())
                _log.debug(METHOD_NAME, "Exited printNodeVO:" + printNodeVO(p_nodeVO));
        }
    }
    ...
    public synchronized void resetCounters(NodeVO p_nodeVO) {
    	final String METHOD_NAME="restCounters";
        if (_log.isDebugEnabled())
            _log.debug(METHOD_NAME, "Entered printNodeVO:" + printNodeVO(p_nodeVO));
        try {
            if (p_nodeVO.isSuspended())
