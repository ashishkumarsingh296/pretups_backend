...
public class ZTEOSNReqResFormatter 
{
	private static Log _log = LogFactory.getLog(ZTEOSNReqResFormatter.class.getName());
 ...
 */
	...
	protected String generateRequest(int action, HashMap<String,String> p_requestMap) throws Exception 
	{
       if(_log.isDebugEnabled())
		...
		switch(action)
		{
			case ZTEOSNI.ACTION_ACCOUNT_INFO: 
			{
				requestStr=generateGetAccountInfoRequest(p_requestMap);
				break;	
			}
			case ZTEOSNI.ACTION_RECHARGE_CREDIT: 
			{
			...
			case ZTEOSNI.ACTION_IMMEDIATE_DEBIT: 
			{
	...
	public HashMap<String,String> parseGetAccountInfoResponse(String p_responseStr) throws Exception
	{
		...
		{  
		    ...
		    if(_str.equals(returnCode))
	        {
				...
				//if selector value is not 1, set selector balance in the map in credit_balance
	...
	private String generateRechargeCreditRequest(HashMap<String,String> p_requestMap) throws Exception
    {
		...
		try
		{
			...
			sbf.append("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
		 	...
		 	sbf.append("<Data>");
		 	sbf.append("<header>");
                        //sbf.append("<ACTION_ID>BuyBalancePackage</ACTION_ID>");
                        sbf.append("<ACTION_ID>ModifyAllBalReturnAllBal</ACTION_ID>");
    	 	sbf.append("<REQUEST_ID>"+p_requestMap.get("IN_REQ_ID")+"</REQUEST_ID>");
    	 	sbf.append("</header>");
    	 	sbf.append("<body>");
                //sbf.append("<DebitMSISDN>"+InterfaceUtil.getFilterMSISDN(p_requestMap.get("INTERFACE_ID"),p_requestMap.get("SENDER_MSISDN"))+"</DebitMSISDN>");
                //sbf.append("<BenefitMSISDN>"+InterfaceUtil.getFilterMSISDN(p_requestMap.get("INTERFACE_ID"),p_requestMap.get("MSISDN"))+"</BenefitMSISDN>");
                //sbf.append("<DeductBalance>"+transAmtInt+"</DeductBalance>");
                sbf.append("<MSISDN>"+InterfaceUtil.getFilterMSISDN(p_requestMap.get("INTERFACE_ID"),p_requestMap.get("MSISDN"))+"</MSISDN>");
            ...
            sbf.append("<TransactionSN>"+p_requestMap.get("IN_RECON_ID")+"</TransactionSN>");
                ...
                //sbf.append("<BenefitBalDtoList>");
                sbf.append("<BalInputDtoList>");
                sbf.append("<AcctResCode>"+p_requestMap.get("CARD_GROUP")+"</AcctResCode>");
                sbf.append("<Balance></Balance>");
                sbf.append("<AddBalance>"+p_requestMap.get("PACKAGE_VALUE")+"</AddBalance>");                                                                           
                sbf.append("<AddDays>"+p_requestMap.get("CREDIT_BONUS_VAL")+"</AddDays>");
 ...
 */
	 private HashMap<String,String> parseRechargeCreditResponse(String responseStr) throws Exception
	 {
		...
		catch(Exception e)
		{
			...
			_log.errorTrace("parseRechargeCreditResponse",e);
			throw e;
		}
		finally
		{
  ...
  */	
 	private String generateCreditAdjustRequest(HashMap<String,String> p_requestMap) throws Exception
    {
		...
		try
		{
            ...
            String txnDesc = FileCache.getValue( p_requestMap.get("INTERFACE_ID"),module+"_TXN_DESC");
    	 	if(!InterfaceUtil.isNullString(txnDesc))
    	 	    sbf.append("<TransactionDescription>"+txnDesc+"</TransactionDescription>");
            sbf.append("<AccountCode></AccountCode>");
            if("S".equals(p_requestMap.get("USER_TYPE"))|| module.equalsIgnoreCase("P2P"))
            	sbf.append("<SuspendAddDays></SuspendAddDays>");
            else
            	sbf.append("<SuspendAddDays>"+graceDays+"</SuspendAddDays>");
    	 	
    	 	if("S".equals(p_requestMap.get("USER_TYPE")))
    	 	{
    	 		...
    	 		String senderBundleId=p_requestMap.get("SENDER_BUNDLE_ID");
    	 		sbf.append("<BalInputDtoList>");
    	 		//sbf.append("<AcctResCode>"+FileCache.getValue(_interfaceId,senderBundleId)+"</AcctResCode>");
    	 		sbf.append("<AcctResCode>"+FileCache.getValue(p_requestMap.get("INTERFACE_ID"),"1")+"</AcctResCode>");
    	 	...
    	 	else
    	 	{
        	 	...
        	 	{  
        			...
        			validity=p_requestMap.get("VALIDITY_DAYS");
        			//If bonus validity is separate from the main validity, then subtract it from the main validity.
        			String addMainBnsVal=FileCache.getValue(_interfaceId,"ADD_MAIN_AND_BUNUS_VALIDITY").trim();
        			String bonusValidity=p_requestMap.get("BONUS_VALIDITY_DAYS");
        			if("N".equals(addMainBnsVal) && !InterfaceUtil.isNullString(bonusValidity.trim()) && !InterfaceUtil.isNullString(validity))
		...
		catch(Exception e)
		{
			...
			_log.errorTrace("generateCreditAdjustRequest",e);
			throw e;
		}
		finally{
			if(_log.isDebugEnabled())
 ...
 */
 	 private HashMap<String,String> parseCreditAdjustResponse(String responseStr) throws Exception
	 {
		...
		catch(Exception e)
		{
			...
			_log.errorTrace("parseCreditAdjustResponse",e);
			throw e;
		}
		finally
		{
 ...
 */
	 private String generateImmediateDebitRequest(HashMap<String,String> p_requestMap) throws Exception
	 {
		...
		if(_log.isDebugEnabled())
		    _log.debug("generateImmediateDebitRequest","Entered with p_requestMap=" + p_requestMap);
		String requestStr= null;
		StringBuffer sbf=null;
		...
		catch(Exception e)
		{
			...
			_log.errorTrace("generateImmediateDebitRequest",e);
			throw e;
		}
		finally{
			if(_log.isDebugEnabled())
	...
	public HashMap<String,String> parseImmediateDebitResponse(String p_responseStr) throws Exception
	{
		...
		{  
		    ...
		    if(_str.equals((returnCode)))
	        {
		    	...
		    	int end = p_responseStr.indexOf("<AcctResCode>1</AcctResCode>");//1 is for main account on live environment.
		    //	end = p_responseStr.indexOf("</BalDtoList>", end);
			    ...
			    if(index!=-1)
		    	{
	           	...
	           	index=p_responseStr.indexOf("<Balance>", end);
	           	if(index!=-1)
		    	{
	           		String grossBalance=p_responseStr.substring(index+"<Balance>".length(),p_responseStr.indexOf("</Balance>",index));
	           		//If the main balance amount is less than zero, that means the subscriber has that much credit amount in his/her account,
	           		// else he/she is in negative by the amount in his/her account.
	            	...
	            	responseMap.put("resp_Balance",grossBalance);
	            }
		        index=p_responseStr.indexOf("<EffDate>", end);
		        if(index!=-1)
		    	{
		        ...
		        index=p_responseStr.indexOf("<ExpDate>", end);
		        if(index!=-1)
		    	{
		        	String expDate=p_responseStr.substring(index+"<ExpDate>".length(),p_responseStr.indexOf("</ExpDate>",index));
		        	responseMap.put("resp_expDate",expDate);
		    	}
	           	index=p_responseStr.indexOf("<BalanceID>", end);
	           	if(index!=-1)
		    	{
	          	...
	          	index=p_responseStr.indexOf("<AcctResCode>", end);
	          	if(index!=-1)
		    	{
	          		String accountResCode=p_responseStr.substring(index+"<AcctResCode>".length(),p_responseStr.indexOf("</AcctResCode>",index));
	          		responseMap.put("resp_accountResCode",accountResCode);
		    	}
		        index=p_responseStr.indexOf("<AcctResName>", end);
		        if(index!=-1)
		    	{
		        	String accountResName=p_responseStr.substring(index+"<AcctResName>".length(),p_responseStr.indexOf("</AcctResName>",index));
	          		responseMap.put("resp_accountResName",accountResName);
		    	}
	          	index=p_responseStr.indexOf("<TransactionSN>");
	          	if(index!=-1)
		    	{
	          		String transactionSN=p_responseStr.substring(index+"<TransactionSN>".length(),p_responseStr.indexOf("</TransactionSN>",index));
	          		responseMap.put("resp_transactionSN",transactionSN);
		    	}
	        }
			return responseMap;
		...
		catch(Exception e)
		{
			...
			_log.errorTrace("parseImmediateDebitResponse",e);
			throw e;
		}
		finally
		{
	...
	private String getBundleRequestString(HashMap<String,String> p_requestMap) throws Exception
	{
		...
		try
		{
			if(!InterfaceUtil.isNullString(p_requestMap.get("BONUS_BUNDLE_IDS")))
			{
			    ...
			    {
	                _log.error("getBundleRequestString","Value of AMT_MULT_FACTOR or UNIT_MULT_FACTOR is not defined in the INFile");
		...
		{
            _log.errorTrace("ZTEOGBReqResFormatter",e);
            _log.error("ZTEOGBReqResFormatter[getBundleRequestString]","Exception e::"+e.getMessage());
             throw e;
		}
		finally
		{
 ...
 */	
	private String getINRequestID(HashMap<String,String> p_map) throws Exception
    {
       ...
       {
			 _log.errorTrace("ZTEOGBReqResFormatter",e);
             _log.error("ZTEOGBReqResFormatter[getINRequestID]","Exception e="+e.getMessage());
              throw e;
 ...
 */
	public static synchronized String getIncrCounter() throws BTSLBaseException
	{
	    ...
	    {
	        _log.errorTrace("getIncrCounter",e);
