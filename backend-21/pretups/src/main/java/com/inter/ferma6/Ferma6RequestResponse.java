 ...
 *                                ----------------------------------------------
 *                                ----
 *                                A Ferma6RequestResponse object is a wrapper
 *                                around a DOM tree.
...
public class Ferma6RequestResponse extends XMLRequestResponse {
    ...
    private String generateGetAccountInfoRequest2(HashMap map) throws Exception {
        ...
        String requestStr = null;
        try {
            Document document = _parser.parse(new ByteArrayInputStream(("<?xml version=\"1.0\" encoding=\"UTF-8\"?><envelope  action=\"getAccountInfo2\"><GettingAccountInfoRequest2 xmlns=\"http://www.ferma.fr/ppg/logic/service/accountinfo2\"><ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">" + (String) map.get("IN_TXN_ID") + "</ns1:TransactionId></GettingAccountInfoRequest2></envelope>").getBytes()));
            NodeList rootNodes = document.getElementsByTagName("GettingAccountInfoRequest2");
            Element accountInfoGettingRequestTag = (Element) rootNodes.item(0);
            Element interfaceIdNode = document.createElement("InterfaceId");
            Element accessIdentifierNode = document.createElement("AccessIdentifier");
            Element accessTypeNode = document.createElement("AccessType");
        ...
        try {
            ...
            if (status.equals("0")) {
                ...
                if (currentStateNode == null)
                    ...
                    currentState = currentStateNode.getData().trim();
                String iccState = null;
                Element iccTag = (Element) balanceTag.getElementsByTagName("ser:Icc").item(0);
                Text iccStateNode = (Text) iccTag.getFirstChild();
                if (iccTag == null)
                    iccState = "";
                else
                    iccState = iccStateNode.getData().trim();
                String firstCallDate = null;
                Element firstCallDateTag = (Element) balanceTag.getElementsByTagName("ser:FirstCallDate").item(0);
                Text firstCallDateNode = (Text) firstCallDateTag.getFirstChild();
                if (firstCallDate == null)
                    firstCallDate = "";
                else
                    firstCallDate = firstCallDateNode.getData().trim();
