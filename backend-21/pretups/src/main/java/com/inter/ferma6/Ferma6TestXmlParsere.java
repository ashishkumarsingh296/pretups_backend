...
public class Ferma6TestXmlParsere extends XMLRequestResponse {
    ...
    private HashMap parseGetAccountInfoRequest(String responseStr) throws Exception {
        ...
        try {
             ...
             * ByteArrayInputStream(responseStr.getBytes()));
             * NodeList rootNodes =
             * document.getElementsByTagName("AccountInfoGettingRequest");
             * Element accountInfoGettingRequestTag =
             * (Element)rootNodes.item(0);
             * Element transactionIdTag =
             * (Element)accountInfoGettingRequestTag.getElementsByTagName
             * ("ser:TransactionId").item(0);
             * String
             * transactionId=((Text)transactionIdTag.getFirstChild()).getData
             * ().trim();
             * System.out.println("TransactionId: "+transactionId);
             * 
             * String interfaceId=null;
             * Element interfaceIdTag =
             * (Element)accountInfoGettingRequestTag.getElementsByTagName
             * ("ser:InterfaceId").item(0);
             ...
             * Text
             * accessIdentifierNode=(Text)accessIdentifierTag.getFirstChild();
             ...
             */
            int index = 0;
            index = responseStr.indexOf("<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">");
            String transactionId = null, interfaceId = null, accessIdentifier = null, accessType = null, balanceId = null;
            if (index != -1) {
                transactionId = responseStr.substring(index + "<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">".length(), responseStr.indexOf("</ns1:TransactionId>", index));

            }
            index = responseStr.indexOf("<InterfaceId>");
            if (index != -1) {
                interfaceId = responseStr.substring(index + "<InterfaceId>".length(), responseStr.indexOf("</InterfaceId>", index));

            }
            index = responseStr.indexOf("<AccessIdentifier>");
            if (index != -1) {
                accessIdentifier = responseStr.substring(index + "<AccessIdentifier>".length(), responseStr.indexOf("</AccessIdentifier>", index));

            }
            index = responseStr.indexOf("<AccessType>");
            if (index != -1) {
                accessType = responseStr.substring(index + "<AccessType>".length(), responseStr.indexOf("</AccessType>", index));

            }
            index = responseStr.indexOf("<BalanceId>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<BalanceId>".length(), responseStr.indexOf("</BalanceId>", index));
    ...
    private HashMap parseRechargeBalanceRequest(String responseStr) throws Exception {
        ...
        try {
            ...
            int index = 0;
            index = responseStr.indexOf("<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">");
            String transactionId = null, interfaceId = null, accessIdentifier = null, accessType = null, balanceId = null, accountId = null, rechargeValue = null, unitType = null;
            if (index != -1) {
                transactionId = responseStr.substring(index + "<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">".length(), responseStr.indexOf("</ns1:TransactionId>", index));
            ...
            index = responseStr.indexOf("<AccountId>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<AccountId>".length(), responseStr.indexOf("</AccountId>", index));

            }
            index = responseStr.indexOf("<RechargeValue>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<RechargeValue>".length(), responseStr.indexOf("</RechargeValue>", index));

            }
            index = responseStr.indexOf("<UnitType>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<UnitType>".length(), responseStr.indexOf("</UnitType>", index));
            ...
            if (_log.isDebugEnabled())
                _log.debug("parseRechargeBalanceRequest", "Exiting map: " + map);
        }
    }
    ...
    private HashMap parseBalanceAdjustmentRequest(String responseStr) throws Exception {
        ...
        try {
             ...
             * ByteArrayInputStream(responseStr.getBytes()));
             * NodeList rootNodes =
             * document.getElementsByTagName("BalanceAdjustmentRequest");
             * Element balanceAdjustmentRequestTag = (Element)rootNodes.item(0);
             * Element transactionIdTag =
             * (Element)balanceAdjustmentRequestTag.getElementsByTagName
             * ("ns1:TransactionId").item(0);
             * String
             * transactionId=((Text)transactionIdTag.getFirstChild()).getData
             * ().trim();
            ...
            /*
             * NodeList
             * balancesNode=balanceAdjustmentRequestTag.getElementsByTagName
             * ("Balances");
             ...
             * String balanceId=null;
             * Element balanceIdTag =
             * (Element)balanceTag.getElementsByTagName("BalanceId").item(0);
             ...
             * Element lifeCycleTag =
             * (Element)balanceTag.getElementsByTagName("LifeCycle").item(0);
             ...
             * String option=null;
             * Element optionTag =
             * (Element)balanceTag.getElementsByTagName("Option").item(0);
             ...
             * String amount=null;
             * Element amountTag =
             * (Element)balanceTag.getElementsByTagName("Amount").item(0);
            ...
            String transactionId = null, interfaceId = null, accessIdentifier = null, accessType = null, balanceId = null, accountId = null, unitType = null;
            String currentState = null, newGraceDate = null, accountStatus = null, lockStatus = null, rechInstallment = null, lifeCycle = null, option = null, amount = null, currentValidityDate = null, currentGraceDate = null, newValidityDate = null;
            index = responseStr.indexOf("<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">");
            if (index != -1) {
                transactionId = responseStr.substring(index + "<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">".length(), responseStr.indexOf("</ns1:TransactionId>", index));
            ...
            index = responseStr.indexOf("<Amount>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<Amount>".length(), responseStr.indexOf("</Amount>", index));

            }
            index = responseStr.indexOf("<CurrentValidityDate>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<CurrentValidityDate>".length(), responseStr.indexOf("</CurrentValidityDate>", index));

            }
            index = responseStr.indexOf("<NewValidityDate>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<NewValidityDate>".length(), responseStr.indexOf("</NewValidityDate>", index));

            }
            index = responseStr.indexOf("<NewGraceDate>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<NewGraceDate>".length(), responseStr.indexOf("</NewGraceDate>", index));

            }
            index = responseStr.indexOf("<CurrentState>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<CurrentState>".length(), responseStr.indexOf("</CurrentState>", index));

            }
            index = responseStr.indexOf("<AccountStatus>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<AccountStatus>".length(), responseStr.indexOf("</AccountStatus>", index));

            }
            index = responseStr.indexOf("<LockStatus>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<LockStatus>".length(), responseStr.indexOf("</LockStatus>", index));

            }
            index = responseStr.indexOf("<RechInstallment>");
            if (index != -1) {
                balanceId = responseStr.substring(index + "<RechInstallment>".length(), responseStr.indexOf("</RechInstallment>", index));
    ...
    private HashMap parseLoginRequest(String responseStr) throws Exception {
        ...
        try {
             ...
             * ByteArrayInputStream(responseStr.getBytes()));
             * NodeList rootNodes =
             * document.getElementsByTagName("LoginRequest");
             * Element loginRequestTag = (Element)rootNodes.item(0);
             * Element transactionIdTag =
             * (Element)loginRequestTag.getElementsByTagName
             * ("ser:TransactionId").item(0);
             ...
             * ("UserName").item(0);
             * String userName
             * =((Text)userNameTag.getFirstChild()).getData().trim();
             * System.out.println("UserName: "+userName);
             * Element passwordTag =
             * (Element)loginRequestTag.getElementsByTagName
             * ("Password").item(0);
             * String password
             * =((Text)passwordTag.getFirstChild()).getData().trim();
             * System.out.println("Password: "+password);
             * Element protocolVersionTag =
             * (Element)loginRequestTag.getElementsByTagName
             * ("ProtocolVersion").item(0);
             * String
             * protocolVersion=((Text)protocolVersionTag.getFirstChild()).
             * getData().trim();
             * System.out.println("ProtocolVersion: "+protocolVersion);
             */
            String transactionId = null, interfaceId = null, userName = null, password = null, protocolVersion = null;

            int index = responseStr.indexOf("<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">");
            if (index != -1) {
                transactionId = responseStr.substring(index + "<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">".length(), responseStr.indexOf("</ns1:TransactionId>", index));
            ...
            index = responseStr.indexOf("<UserName>");
            if (index != -1) {
                userName = responseStr.substring(index + "<UserName>".length(), responseStr.indexOf("</UserName>", index));

            }
            index = responseStr.indexOf("<Password>");
            if (index != -1) {
                password = responseStr.substring(index + "<Password>".length(), responseStr.indexOf("</Password>", index));

            }
            index = responseStr.indexOf("<ProtocolVersion>");
            if (index != -1) {
                protocolVersion = responseStr.substring(index + "<ProtocolVersion>".length(), responseStr.indexOf("</ProtocolVersion>", index));
    ...
    private HashMap parseLogoutResponse(String responseStr) throws Exception {
        ...
        try {
            ...
            String transactionId = null, interfaceId = null;
            int index = responseStr.indexOf("<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">");
            if (index != -1) {
                transactionId = responseStr.substring(index + "<ns1:TransactionId xmlns:ns1=\"http://www.ferma.fr/ppg/logic/service\">".length(), responseStr.indexOf("</ns1:TransactionId>", index));
    ...
    private String generateLoginReponse(HashMap map) throws Exception {
        ...
        try {
            // Document document = _parser.parse(new
            // ByteArrayInputStream(("<?xml version=\"1.0\" encoding=\"UTF-8\"?><log:LoginResponse xmlns:log=\"http://www.ferma.fr/ppg/logic/service/login\"><ser:Status>"+map.get("Status")+"</ser:Status><ser:TransactionId>"+map.get("TransactionId")+"</ser:TransactionId><ser:InterfaceId>"+map.get("InterfaceId")+"</ser:InterfaceId></log:LoginResponse>").getBytes()));
            /*
             * NodeList rootNodes =
            ...
            requestStr = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><LoginResponse xmlns=\"http://www.ferma.fr/ppg/logic/service/login\" xmlns:ser=\"http://www.ferma.fr/ppg/logic/service\"><ser:Status>" + map.get("Status") + "</ser:Status><ser:TransactionId>" + map.get("TransactionId") + "</ser:TransactionId><ser:InterfaceId>" + map.get("InterfaceId") + "</ser:InterfaceId></LoginResponse>";
    ...
    private String generateLogoutRequest(HashMap map) throws Exception {
        ...
        try {
            // Document document = _parser.parse(new
            // ByteArrayInputStream(("<?xml version=\"1.0\" encoding=\"UTF-8\"?><log:LogoutResponse xmlns:log=\"http://www.ferma.fr/ppg/logic/service/logout\"><ser:Status>"+map.get("Status")+"</ser:Status><ser:TransactionId>"+map.get("TransactionId")+"</ser:TransactionId><ser:InterfaceId>"+map.get("InterfaceId")+"</ser:InterfaceId></log:LogoutResponse>").getBytes()));
            /*
             * NodeList rootNodes =
             * document.getElementsByTagName("LogoutRequest");
             ...
             * requestStr= write(document,strBuff,"");
             */
            requestStr = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><LogoutResponse xmlns=\"http://www.ferma.fr/ppg/logic/service/logout\"  xmlns:ser=\"http://www.ferma.fr/ppg/logic/service\"><ser:Status>" + map.get("Status") + "</ser:Status><ser:TransactionId>" + map.get("TransactionId") + "</ser:TransactionId><ser:InterfaceId>" + map.get("InterfaceId") + "</ser:InterfaceId></LogoutResponse>";
