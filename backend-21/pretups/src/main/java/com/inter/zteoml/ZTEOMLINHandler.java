 ...
 */
public class ZTEOMLINHandler implements InterfaceHandler
{
	private Log _log = LogFactory.getLog(ZTEOMLINHandler.class.getName());
	private ZTEOMLRequestResponseFormatter _formatter=null;
	 ...
	 *  Constructor
	 */
	public ZTEOMLINHandler()
	{
		_formatter=new ZTEOMLRequestResponseFormatter();
	...
	{
		String methodName="ZTEOMLINHandler[validate()]";
		...
		try
		{
			...
			sendRequestToIN(ZTEOMLI.ACTION_ACCOUNT_INFO);
	...
	{
		String methodName="ZTEOMLINHandler[credit()]";
		if (_log.isDebugEnabled())_log.debug(methodName,"Entered p_requestMap: " + p_requestMap);
		...
		try
		{
			...
			sendRequestToIN(ZTEOMLI.ACTION_RECHARGE_CREDIT);
			//set TRANSACTION_STATUS as Success in request map
			_requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
	...
	{
		String methodName="ZTEOMLINHandler[creditAdjust()]";
		if (_log.isDebugEnabled())_log.debug(methodName,"Entered p_requestMap: " + p_requestMap);
		double interfaceAmtDouble=0;
		double multFactorDouble=0;
		String amountStr=null;
		...
		try
		{
			...
			sendRequestToIN(ZTEOMLI.ACTION_TXN_CANCEL);
			//set TRANSACTION_STATUS as Success in request map
			_requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
			//Set the post balance after multiplied by multiplication factor.
			try
	...
	{
		String methodName="ZTEOMLINHandler[debitAdjust()]";
		if (_log.isDebugEnabled())_log.debug(methodName,"Entered p_requestMap: " + p_requestMap);
		double interfaceAmtDouble=0;
		double multFactorDouble=0;
		String amountStr=null;
		...
		try
		{
			...
			//sending the Re-charge request to IN along with re-charge action defined in SafcomI interface
			sendRequestToIN(ZTEOMLI.ACTION_IMMEDIATE_DEBIT);
			//set TRANSACTION_STATUS as Success in request map
			_requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
			//Set the post balance after multiplied by multiplication factor.
			try
	...
	{
		String methodName="ZTEOMLINHandler[sendRequestToIN()]";
		if(_log.isDebugEnabled()) _log.debug(methodName," p_action="+p_action);

		long startTime=0,endTime=0,warnTime=0;
		int _valTimeOut=1000;
		int _refillTimeOut=1000;

		ZTEOMLWebServiceConnector connector =null;
		OrangeServiceSoap11BindingStub stub=null;

		TQueryProfileAndBalRequest queryProfileAndBalReq=null;
		TRechargingRequest rechargingReq=null;
		TDeductFeeRequest transferBalReq=null;
		TRefundRequest refundBalRequest=null;
		//TAdjustBalanceRequest transferBalReq=null;

		TQueryProfileAndBalResponse queryProfileAndBalResp=null;
		TRechargingResponse rechargingResponse=null;
		TDeductFeeResponse transferBalResp=null;
		TRefundResponse refundBalResponse=null;
		...
		try
		{
			connector=new ZTEOMLWebServiceConnector(_interfaceID);
			...
			try
			{
				try
				{
					...
					case ZTEOMLI.ACTION_ACCOUNT_INFO:
					{
						...
						try{
							_valTimeOut=Integer.valueOf((String)_requestMap.get("VAL_TIME_OUT"));
						}
						catch(Exception e){
							_valTimeOut=1000;
						}

						queryProfileAndBalReq=_formatter.generateQueryProfileAndBalRequest(_requestMap);
						stub.setTimeout(_valTimeOut);
					...
					{
						{
							try{
								...
								_refillTimeOut=Integer.valueOf((String)_requestMap.get("REFILL_TIME_OUT"));
							...
							rechargingReq=_formatter.generateRechargingRequest(_requestMap);
							stub.setTimeout(_refillTimeOut);

							if(rechargingReq==null)
								throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
							try
							{
								rechargingResponse=stub.recharging(rechargingReq, generateTAuthHeaderObject());
					...
					case ZTEOMLI.ACTION_IMMEDIATE_DEBIT:
					{
						...
						transferBalReq=_formatter.generateDeductFeeRequest(_requestMap);
						stub.setTimeout(_refillTimeOut);
						//transferBalReq=_formatter.generateDebitAdjustBalanceRequest(_requestMap);
						if(transferBalReq==null)
							throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);

						TransactionLog.log(_interfaceID,_referenceID,_msisdn,_requestMap.get("NETWORK_CODE"),String.valueOf(p_action),PretupsI.TXN_LOG_REQTYPE_REQ,"_requestMap="+_requestMap,PretupsI.TXN_LOG_STATUS_UNDERPROCESS,", action="+p_action );
						try
						{
							transferBalResp=stub.deductFee(transferBalReq, generateTAuthHeaderObject());
					...
					{
						refundBalRequest=_formatter.generateRefundRequest(_requestMap);
						//transferBalReq=_formatter.generateDebitAdjustBalanceRequest(_requestMap);
						if(refundBalRequest==null)
							throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);

						TransactionLog.log(_interfaceID,_referenceID,_msisdn,_requestMap.get("NETWORK_CODE"),String.valueOf(p_action),PretupsI.TXN_LOG_REQTYPE_REQ,"_requestMap="+_requestMap,PretupsI.TXN_LOG_STATUS_UNDERPROCESS,", action="+p_action );
						try
						{
							refundBalResponse=stub.refund(refundBalRequest, generateTAuthHeaderObject());
							//transferBalResp=stub.adjustBalance(transferBalReq, generateTAuthHeaderObject());
						}catch(AxisFault axisFaultException)
						{
							_log.error(methodName,"REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn+"INTERFACE ID = "+_interfaceID,"Network code = "+_requestMap.get("NETWORK_CODE")+" Action="+p_action+"Exception Error Message :"+axisFaultException.getFaultReason()+" Error Code:"+axisFaultException.getFaultCode());
				...
				catch(java.rmi.RemoteException re)
				{
					...
					_log.error("sendRequestToIN","Remote Exception occured. So marking the response as FAIL or Ambiguous");
					if(p_action==ZTEOMLI.ACTION_RECHARGE_CREDIT)
					{

						int retryNumber=0;
						try{
							retryNumber=(Integer.valueOf((String)_requestMap.get("RETRY_ATTEMPT")));
						}
						catch( Exception e1)
						{
							retryNumber=0;
					...
					else   if(p_action==ZTEOMLI.ACTION_IMMEDIATE_DEBIT)  {
						_requestMap.put("INTERFACE_STATUS",InterfaceErrorCodesI.AMBIGOUS);
						throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
					}
					else
			...
			_responseMap=_formatter.parseResponseObject(p_action,queryProfileAndBalResp,rechargingResponse,transferBalResp,refundBalResponse);
			...
			String respTransactionID=_responseMap.get("resp_transactionSN");
			_requestMap.put("RESP_TRANSACTION_ID",respTransactionID);
	...
	{
		String methodName="ZTEOMLINHandler[checkInterfaceB4SendingRequest()]";
	...
	{
		String methodName="ZTEOMLINHandler[setInterfaceParameters()]";
		_log.debug(methodName,"Entered");
		try
		{
			...
			// New Parameter for Time Out
			String valTimeOut=FileCache.getValue(_interfaceID, "VAL_TIME_OUT");
			if(InterfaceUtil.isNullString(valTimeOut))
			{
				EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEMeditelINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+ _requestMap.get("NETWORK_CODE") , "VAL_TIME_OUT is not defined in IN File");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
			}
			_requestMap.put("VAL_TIME_OUT",valTimeOut.trim());

			String refillTimeOut=FileCache.getValue(_interfaceID, "REFILL_TIME_OUT");
			if(InterfaceUtil.isNullString(refillTimeOut))
			{

				EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEMeditelINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+ _requestMap.get("NETWORK_CODE") , "REFILL_TIME_OUT is not defined in IN File");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
			}
			_requestMap.put("REFILL_TIME_OUT",refillTimeOut.trim());
			String retryAttempt=FileCache.getValue(_interfaceID, "RETRY_ATTEMPT");
			if(InterfaceUtil.isNullString(retryAttempt))
			{
				EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEMeditelINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+ _requestMap.get("NETWORK_CODE") , "RETRY_ATTEMPT is not defined in IN File");
				throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
			}
			_requestMap.put("RETRY_ATTEMPT",retryAttempt.trim());
	...
	{
		String methodName="ZTEOMLINHandler[setLanguageFromMapping()]";
	...
	{
		String methodName="ZTEOMLINHandler[handleCancelTransaction()]";
		if (_log.isDebugEnabled())
	...
	{
		String methodName="ZTEOMLINHandler[getINTransactionID()]";
