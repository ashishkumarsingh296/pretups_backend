...
public class SafComURLConnection extends BTSLConnection
{
	 ...
	 * @throws Exception
	 */
	public SafComURLConnection(String p_url,int p_connecttimeout,int p_readtimeout,String p_keepAlive) throws Exception 
	{
		if(_log.isDebugEnabled())_log.debug("SafComURLConnection","Entered p_url: "+p_url+"p_connecttimeout="+p_connecttimeout+" p_readtimeout="+p_readtimeout+" p_keepAlive="+p_keepAlive);
		try
		{
		...
		{
			e.printStackTrace();
	       _log.error("SafComURLConnection", "Exception " + e.getMessage());
	        throw e;
  		}
		if(_log.isDebugEnabled())_log.debug("SafComURLConnection","Exiting");
	}
	/**
