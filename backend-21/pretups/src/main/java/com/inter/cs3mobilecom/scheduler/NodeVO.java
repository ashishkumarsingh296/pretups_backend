...
/**
 * @(#)NodeVO
 *            Copyright(c) 2011, COMVIVA TECHNOLOGIES LIMITED. All rights
 *            reserved.
 *            COMVIVA PROPRIETARY/CONFIDENTIAL. Use is subject to license terms.
 *            All Rights Reserved
 *            ------------------------------------------------------------------
 *            -------------------------------
 *            Author Date History
...
public class NodeVO {
    ...
    private boolean _isMaxConReached = false;// set as true when maximum allowed
                                             ...
                                             // reached.
    private ArrayList<String> _transIdList = new ArrayList<String>();// This
                                                                     // list is
                                                                     // used to
                                                                     // contains
                                                                     // the
    ...
    private boolean _processSingleReqFlag = false;// flag is used to ensure that
                                                  // just after expiry only one
                                                  // request

    // would be sent to IN to check whether IN is resumed or not. Otherwise if
    // IN is not Resumed many ambiguous case may occur.
     ...
     */
    public ArrayList<String> getTransIdList() {
        return _transIdList;
    }
