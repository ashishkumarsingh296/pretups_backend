...
public class NodeScheduler {
    ...
    private Hashtable<String, NodeVO> _nodeTable = null;// Contains the node
    ...
    public NodeScheduler(String p_cs3INId) throws BTSLBaseException {
        ...
        String url = null;
        long expiryTime = 0;// Defines the expire duration of blocked node.
        int readTimeOutVal = 0;// Defines the read time out for the validation
                               // request
        int readTimeOutTop = 0;// Defines the read time out for the top-up
        ...
        try {
            ...
            // 5.Max Number up to which the node is shifted in case the failure
            // of one node.
            // 6.Warn time
            // 7.Retry number
        ...
        try {
            ...
            while (isNext) {
                try {
                    ...
                    // 3-b.If the max node reaches throw a separate error code
                    // showing channel is full and decrement the max when a
                    // request is successful
                    // processed for that node.
                    // 4.If node is blocked do the following.
                    // 4-a.Check the expire time of the blocked node
                    // 4-b.if difference of current time and blocked time is
                    // greater than expire time, blocked node is expired current
                    // node is returned and increment
    ...
    /**
     * This method is used to check the expire of node,if it is blocked.
     * 
     * @param p_blockedAt
    ...
    public Hashtable<String, NodeVO> getNodeTable() {
        return _nodeTable;
    }
}
