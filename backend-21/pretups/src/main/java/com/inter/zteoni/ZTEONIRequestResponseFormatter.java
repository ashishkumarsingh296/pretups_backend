...
/**
 * @(#)ZTEONIRequestResponseFormatter.java
 *                                         Copyright(c) 2013, MComviva
 *                                         Technologies Ltd.
 *                                         All Rights Reserved
 *                                         ------------------------------------
 *                                         --
...
public class ZTEONIRequestResponseFormatter {
    private static Log _log = LogFactory.getLog(ZTEONIRequestResponseFormatter.class.getName());
     ...
     * Constructor
     */
    public ZTEONIRequestResponseFormatter() {
        super();
    ...
    protected String generateRequest(int action, HashMap p_map) throws Exception {
        ...
        switch (action) {
        case ZTEONII.ACTION_ACCOUNT_INFO: {
            str = generateGetAccountInfoRequest(p_map);
            break;
        }
        case ZTEONII.ACTION_RECHARGE_CREDIT: {
        ...
        case ZTEONII.ACTION_IMMEDIATE_DEBIT: {
    ...
    public HashMap parseGetAccountInfoResponse(String responseStr) throws Exception {
        ...
        try {
            ...
            if (_str.equals(returnCode)) {
                ...
                index = responseStr.indexOf("<DefLang>");
                if (index != -1) {
                    String resp_defLang = responseStr.substring(index + "<DefLang>".length(), responseStr.indexOf("</DefLang>", index));
                    map.put("LANGUAGETYPE", resp_defLang);
                }
    ...
    private String generateRechargeCreditRequest(HashMap p_map) throws Exception {
        ...
        try {
            ...
            sbf.append("<Data>");
            sbf.append("<header>");
            sbf.append("<ACTION_ID>Recharge</ACTION_ID>");
            sbf.append("<REQUEST_ID>" + (String) p_map.get("IN_REQ_ID") + "</REQUEST_ID>");
            sbf.append("</header>");
            sbf.append("<body>");
            sbf.append("<MSISDN>" + InterfaceUtil.getFilterMSISDN((String) p_map.get("INTERFACE_ID"), (String) p_map.get("MSISDN")) + "</MSISDN>");
            ...
            // If transfer amount is not zero, then set it in to the request
            // string.
            if (transAmtDbl != 0)
            ...
            // If validity period is not null, then set it in to the request
            // string.
            ...
            sbf.append("<OperationStaff></OperationStaff>");
            sbf.append("<InactiveDuration></InactiveDuration>");
            sbf.append("<TransactionSN>" + (String) p_map.get("IN_RECON_ID") + "</TransactionSN>");
    ...
    private String generateCreditAdjustRequest(HashMap p_map) throws Exception {
        ...
        try {
            ...
            sbf.append("<Data>");
            sbf.append("<header>");
            sbf.append("<ACTION_ID>TransferBalance</ACTION_ID>");
            sbf.append("<REQUEST_ID>" + (String) p_map.get("IN_REQ_ID") + "</REQUEST_ID>");
            sbf.append("</header>");
            sbf.append("<body>");
            sbf.append("<MSISDN>" + InterfaceUtil.getFilterMSISDN((String) p_map.get("INTERFACE_ID"), (String) p_map.get("MSISDN")) + "</MSISDN>");
            ...
            if ("S".equals((String) p_map.get("USER_TYPE"))) {
                ...
                // String senderBundleId=(String)p_map.get("SENDER_BUNDLE_ID");
                String senderBundleId = (String) p_map.get("CARD_GROUP_SELECTOR");
    ...
    private String generateImmediateDebitRequest(HashMap p_map) throws Exception {
        ...
        try {
            ...
            if (!InterfaceUtil.isNullString((String) p_map.get("transfer_amount")))
                transAmtDbl = Double.parseDouble((String) p_map.get("transfer_amount"));
            if (!InterfaceUtil.isNullString((String) p_map.get("access_fees")))
                acessFees = Double.parseDouble((String) p_map.get("access_fees"));

            // senderBundleId=(String)p_map.get("SENDER_BUNDLE_ID");
            senderBundleId = (String) p_map.get("CARD_GROUP_SELECTOR");
            getINRequestID(p_map);
            sbf = new StringBuffer(1024);
            sbf.append("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
            sbf.append("<zsmart>");
            ...
            sbf.append("<body>");
            sbf.append("<TransactionSN>" + (String) p_map.get("IN_RECON_ID") + "</TransactionSN>");
            sbf.append("<FromMSISDN>" + InterfaceUtil.getFilterMSISDN((String) p_map.get("INTERFACE_ID"), (String) p_map.get("MSISDN")) + "</FromMSISDN>");
            sbf.append("<ToMSISDN>" + InterfaceUtil.getFilterMSISDN((String) p_map.get("INTERFACE_ID"), (String) p_map.get("RECEIVER_MSISDN")) + "</ToMSISDN>");

            if (transAmtDbl != 0)
                sbf.append("<Amount>" + String.valueOf(Math.round(transAmtDbl)) + "</Amount>");
            else
            ...
            if (acessFees != 0)
                sbf.append("<AddFee>" + String.valueOf(Math.round(acessFees)) + "</AddFee>");
            else
                sbf.append("<AddFee></AddFee>");
            // If validity period is not null, then set it in to the request
            // string.
            if (!InterfaceUtil.isNullString(validity))
                sbf.append("<AddDays>" + validity + "</AddDays>");
    ...
    public HashMap parseImmediateDebitResponse(String responseStr) throws Exception {
        ...
        try {
...

            {
                ...
                // int end =
                // responseStr.indexOf("<AccountResCode>1</AccountResCode>");//1
                // is for main account on live env.
                // end = responseStr.indexOf("</BenefitDtoList>", end);
                ...
                /*
                 * int start =
                 * responseStr.indexOf("<AcctResCode>1</AcctResCode>");
                 * int end1 =responseStr.indexOf("</BalDto>",start);
                 * String mainBalanceBundle= responseStr.substring(start, end1);
                 * 
                 * index=mainBalanceBundle.lastIndexOf("<Balance>", start);
                 * if(index!=-1)
                 * {
                 * String
                 * grossBalance=mainBalanceBundle.substring(index+"<Balance>"
                 * .length(),mainBalanceBundle.indexOf("</Balance>",index));
                 ...
                 * }
                 * 
                 * index=mainBalanceBundle.lastIndexOf("<EffDate>", start);
                 * if(index!=-1)
                 * {
                 * String
                 * effDate=mainBalanceBundle.substring(index+"<EffDate>".length
                 * (),mainBalanceBundle.indexOf("</EffDate>",index));
                 * map.put("resp_effDate",effDate);
                 * }
                 * index=responseStr.lastIndexOf("<ExpDate>", end1);
                 * if(index!=-1)
                 * {
                 * String
                 * expDate=responseStr.substring(index+"<ExpDate>".length(
                 * ),responseStr.indexOf("</ExpDate>",index));
                 ...
                 * {
                 * String
                 * balanceID=responseStr.substring(index+"<BalanceID>".length
                 * (),responseStr.indexOf("</BalanceID>",index));
                 * map.put("resp_balanceID",balanceID);
                 * 
                 * }
                 */
                /*
                 * index=responseStr.lastIndexOf("<AcctResCode>", end1);
                 * if(index!=-1)
                 * {
                 * String
                 * accountResCode=responseStr.substring(index+"<AcctResCode>"
                 * .length(),responseStr.indexOf("</AcctResCode>",index));
                 * map.put("resp_accountResCode",accountResCode);
                 * }
                 * index=responseStr.lastIndexOf("<AcctResName>", end1);
                 * if(index!=-1)
                 * {
                 * String
                 * accountResName=responseStr.substring(index+"<AcctResName>"
                 * .length(),responseStr.indexOf("</AcctResName>",index));
    ...
    private String getBundleRequestString(HashMap p_map) throws Exception {
        ...
        try {
            if (!InterfaceUtil.isNullString((String) p_map.get("BONUS_BUNDLE_IDS"))) {
                ...
                bonusValidity = ((String) p_map.get("BONUS_VALIDITY_DAYS")).split("\\|");
                ...
                // If bunus is associated with recharge, then add the request.
                bonusBundleStr = sbfBundle.toString();
                if (!InterfaceUtil.isNullString(bonusBundleStr))
                    bonusBundleStr = "<BenefitDtoList>" + sbfBundle.toString() + "</BenefitDtoList>";

                _log.debug("getBundleRequestString", "bonusBundleStr=" + bonusBundleStr);

                if (_combinedRecharge && !"1".equals((String) p_map.get("CARD_GROUP_SELECTOR")))
                    bundleReqStr = sbfMainBundle.toString();
                else
                    bundleReqStr = sbfMainBundle.toString() + bonusBundleStr;
            }
            return bundleReqStr;
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("ZTEONIRequestResponseFormatter[getBundleRequestString]", "Exception e::" + e.getMessage());
            throw e;
        } finally {
            if (_log.isDebugEnabled())
            ...
            if (index != -1) {
                ...
                for (int i = 0; i < noOfBundles; i++) {
                    ...
                    if (index != -1) {
                        ...
                        System.out.println("bdl_code Before" + bdl_code);
                        if ("1".equalsIgnoreCase(bdl_code))
                        // if(bdl_code.equalsIgnoreCase("10"))
                        {
                            System.out.println("bdl_code After" + bdl_code);
                        ...
                        if ("32".equalsIgnoreCase(bdl_code)) {
                            int index1 = bundleArray[i].indexOf("<Balance>");
                            String wadata_balance = bundleArray[i].substring(index1 + "<Balance>".length(), bundleArray[i].indexOf("</Balance>", index1));
                            long wadata_balancelng = Long.parseLong(wadata_balance);
                            wadata_balancelng = 0 - (wadata_balancelng);
                            wadata_balance = String.valueOf(wadata_balancelng);
                            System.out.println("resp_Balance_wadata" + p_requestMap.get("resp_Balance_wadata"));
                            if (InterfaceUtil.isNullString((String) (p_requestMap.get("resp_Balance_wadata")))) {
                                p_requestMap.put("resp_Balance_wadata", wadata_balance);

                            } else {
                                String oldWadataBal = (String) (p_requestMap.get("resp_Balance_wadata"));
                                long oldWadataBalLng = Long.parseLong(oldWadataBal);
                                p_requestMap.put("resp_Balance_wadata", String.valueOf(oldWadataBalLng + wadata_balancelng));
        ...
        try {
            ...
            if (index != -1) {
                ...
                for (int i = 0; i < noOfBundles; i++) {
                    ...
                    if (index != -1) {
                        bdl_code = bundleArray[i].substring(index + "<AcctResCode>".length(), bundleArray[i].indexOf("</AcctResCode>", index));
                        _log.debug("getBundlesInfoRcvdFromIN4Credit: ", "bdl_code=" + bdl_code);
                        if ("1".equalsIgnoreCase(bdl_code)) {
                            int index1 = bundleArray[i].indexOf("<Balance>");
                            String bdl_Balance = bundleArray[i].substring(index1 + "<Balance>".length(), bundleArray[i].indexOf("</Balance>", index1));
                            long bdl_BalanceLng = Long.parseLong(bdl_Balance);
    ...
    private String getINRequestID(HashMap p_map) throws Exception {
        ...
        try {
            ...
            e.printStackTrace();
            _log.error("ZTEONIRequestResponseFormatter[getINRequestID]", "Exception e::" + e.getMessage());
            throw e;
