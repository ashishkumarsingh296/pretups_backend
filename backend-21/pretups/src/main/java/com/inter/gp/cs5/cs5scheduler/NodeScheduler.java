...
public class NodeScheduler
{
    ...
    private int _maxAllowedNum=0;//This number describes that the number of nodes to be shifted incase of failure.
    public NodeScheduler()
    {
    	LogFactory.printLog("NodeScheduler", "Entered", log);
    ...
    {
    	String METHOD_NAME= "NodeScheduler";
    	LogFactory.printLog("Static", "Entered p_cs5INId::"+p_cs5INId, log);
		...
		String password=null;//Defines password for Node.
		...
		try
		{
	        ...
	        try
			{
	            ...
	            LogFactory.printLog(METHOD_NAME,"MAX_NODE::"+_totalNodes,log);
			...
			try
			{
			    ...
			    LogFactory.printLog(METHOD_NAME,"MAX_ALWD_NO::"+_maxAllowedNum,log);
			}
			catch(Exception e)
			{
			    log.errorTrace("Exception",e);
			...
			try
			{
			    ...
			    LogFactory.printLog(METHOD_NAME,"RETRY_NUM::"+_retryNum,log);
			...
			try
			{
				...
				LogFactory.printLog(METHOD_NAME,"HEADER_HOST_NAME::"+_headerHostName,log);
			}
			catch(Exception e)
			{
			    log.errorTrace("Exception",e);
			...
			try
			{
				...
				LogFactory.printLog(METHOD_NAME,"HOST_NAME::"+_hostName,log);
			}
			catch(Exception e)
			{
			    log.errorTrace("Exception",e);
			...
			try
			{
			    ...
			    LogFactory.printLog(METHOD_NAME,"USER_AGENT::"+_userAgent,log);
			}
			catch(Exception e)
			{
			    log.errorTrace("Exception",e);
			...
			for(int l=1;l<=_totalNodes;l++)
			{
			   try
			   {
				    ...
				    LogFactory.printLog(METHOD_NAME,"url_"+l+"::"+url,log);
					...
					try
					{
					    ...
					    LogFactory.printLog(METHOD_NAME,"CONN_TIMEOUT::"+connectionTimeOut,log);
					...
					try
					{
						...
						LogFactory.printLog(METHOD_NAME,"MAX_CON_NODE::"+maxConPerNode,log);
					...
					try
					{
					    ...
					    LogFactory.printLog(METHOD_NAME,"VAL_READ_TIMEOUT::"+readTimeOutVal,log);
					...
					try
					{
					    ...
					    LogFactory.printLog(METHOD_NAME,"TOP_READ_TIMEOUT::"+readTimeOutTop,log);
					...
					try
					{
					    ...
					    LogFactory.printLog(METHOD_NAME,"WARN_TIME::"+readTimeOutVal,log);
					...
					try
					{
					    ...
					    LogFactory.printLog(METHOD_NAME,"EXPIRY_TIME::"+expiryTimeStr,log);
					...
					{ 
						...
						LogFactory.printLog(METHOD_NAME,"KEEP_ALIVE::"+keepAlive,log);
					...
					{ 
						...
						LogFactory.printLog(METHOD_NAME,"USERNAME::"+userName,log);
					...
					{ 
						...
						LogFactory.printLog(METHOD_NAME,"PASSWORD::"+password,log);
		            ...
		            {
		            	LogFactory.printLog(METHOD_NAME,"THRESHOLD_TIME is either not defined in the INFile or its value is not numeric",log);
			   ...
			   {
			       log.error("NodeScheduler[constructor]","BTSLBaseException be::"+be.getMessage());
			       throw be;
			   }
			   catch(Exception e)
			   {
		...
		{
			LogFactory.printLog(METHOD_NAME,"Exiting _nodeTable::"+_nodeTable,log);
		}//end of finally
    }//end of constructor
    ...
    public synchronized NodeVO getNodeVO(String p_transId) throws BTSLBaseException
    {
    	...
    	LogFactory.printLog(METHOD_NAME,"Entered p_transId"+p_transId,log);
        ...
        try
        {
            ...
            if(nodeNumStr==null)
            {
                log.error("getNodeVO",p_transId,"There is no scheduled node to process the request for p_transId::"+p_transId);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_SCHEDULING);
            }
            cs5NodeVO = (NodeVO)_nodeTable.get(nodeNumStr);
            if(cs5NodeVO==null)
            {
                log.error("getNodeVO",p_transId,"No detail found for the Node number::"+nodeNumStr);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_SCHEDULING);
            }
        }
        catch(BTSLBaseException be)
        {
            log.error("getNodeVO",p_transId,"while processing the request for p_transId::"+p_transId+ " get BTSLBaseException  be::"+be.getMessage());
            throw be;
        }
        catch(Exception e)
        {
            ...
            log.error("getNodeVO",p_transId,"Exception  e::"+e.getMessage()+" p_transId::"+p_transId);
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_SCHEDULING);
        }//end of catch-Exception
        finally
        {
        	LogFactory.printLog(METHOD_NAME,"Exited cs5NodeVO::"+cs5NodeVO,log);
        }//end of finally
        return cs5NodeVO;
    }//end of getNodeVO
    /**
    ...
    private synchronized String getNode(String p_transId) throws BTSLBaseException
    {
        ...
        try
        {
	        ...
	        while(isNext)
	        {
	            try
	            {
		            ...
		            if(log.isDebugEnabled()) log.debug("getNode",p_transId,"tempCount::"+tempCount);
		            ...
		            LogFactory.printLog(METHOD_NAME,"NodeNumber ["+_nodeNumber+"] is blocked"+ "p_transId::"+p_transId,log);
	                ...
	                log.info("getNode",p_transId,"Node number ::"+_nodeNumber+" of interfaceID::"+_interfaceID+"is blocked and its expiry is not reached");
	                incrementNodeNumber();
	            }
	            catch(BTSLBaseException be)
	            {
	            ...
	            catch(Exception e)
	            {
	                ...
	                log.errorTrace("Exception",e);
	                log.error("getNode",p_transId,"_interfaceID"+_interfaceID+" p_transId ::"+p_transId+" Exception e::"+e.getMessage());
	                if(connIncremented)
        ...
        {
            log.error("getNode",p_transId,"p_transId::"+p_transId+" BTSLBaseException  be::"+be.getMessage());
            throw be;
        }
        catch(Exception e)
        {
            ...
            log.error("getNode",p_transId,"p_transId::"+p_transId+" Exception e::"+e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_SCHEDULING);
        }//end of catch-Exception
        finally
        {
        	LogFactory.printLog(METHOD_NAME,"Exited nodeNumStr::"+nodeNumStr,log);
        }//end of finally
        return nodeNumStr;
    }//end of 
    /**
    ...
    public boolean isExpired(long p_blockedAt,long p_expiryDuration)throws BTSLBaseException
    {
    	...
    	LogFactory.printLog(METHOD_NAME,"Entered p_blockedAt::"+p_blockedAt+" p_expiryDuration::"+p_expiryDuration,log);
      
        boolean isExp=false;
        try
        {
            long currentTime = System.currentTimeMillis();
            if(log.isDebugEnabled())log.debug("isExpired","currentTime::"+currentTime);
        ...
        {
            log.errorTrace("Exception",e);
            log.error("isExpired","While checking the expiry of blocked node get the Exception e::"+e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_SCHEDULING);
        }//end of catch-Exception
        finally
        {
        	LogFactory.printLog(METHOD_NAME,"isExp::"+isExp,log);
        }//end of finally
        return isExp;
    }//end of isExpired
    ...
    {
    	return (Hashtable) _nodeTable.clone();
    }
}
