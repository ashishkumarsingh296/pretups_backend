package com.inter.claroCollectionEnqWS.scheduler;
...
import com.inter.claroCollectionEnqWS.ClaroCollEnqWSConnectionManager;
 ...
 * 
 * Blocked Node-A node is set as blocked node from the ComverseTGINHandler while creation of connection for the node results into error.
 */
public class NodeScheduler
{
     ...
     * @throws	Exception
     */
    public NodeScheduler(String p_iNId) throws BTSLBaseException
    {
        if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","Entered p_iNId::"+p_iNId);
		String url=null;
		long expiryTime=0;//Defines the expiry duration of blocked node.
		int readTimeOut=0;//Defines the read time out for the validation request
		...
		try
		{
	        ...
	        try
			{
	            _totalNodes = Integer.parseInt(FileCache.getValue(p_iNId,"MAX_NODE").trim());
			...
			try
			{
			    _maxAllowedNum = Integer.parseInt(FileCache.getValue(p_iNId,"MAX_ALWD_NO").trim());
			...
			try
			{
			    _retryNum = Integer.parseInt(FileCache.getValue(p_iNId,"RETRY_NUM").trim());
			...
			for(int l=1;l<=_totalNodes;l++)
			{
			   try
			   {
				   ...
				   nodeVO.setNodeNumber(l);
				    url = FileCache.getValue(p_iNId,"URL_"+l);
				    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","url_"+l+"::"+url);
					if(InterfaceUtil.isNullString(url))
					{
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of URL_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}   
					...
					try
					{
					    String ReadTimeOutStr = FileCache.getValue(p_iNId,"READ_TIMEOUT_"+l).trim();
					    readTimeOut = Integer.parseInt(ReadTimeOutStr);
					    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","READ_TIMEOUT_"+l+"::"+readTimeOut);
					    nodeVO.setReadTimeOut(readTimeOut);
					}
					catch(Exception e)
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of READ_TIMEOUT_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}
					
					try
					{
					    String warnTimeStr = FileCache.getValue(p_iNId,"WARN_TIME_"+l).trim();
					    warnTime = Long.parseLong(warnTimeStr);
					    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","WARN_TIME_"+l+"::"+warnTimeStr);
					    nodeVO.setWarnTime(warnTime);
					}
					catch(Exception e)
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of WARN_TIME_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}
					try
					{
						userName = FileCache.getValue(p_iNId,"USER_NAME_"+l).trim();
						if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","USER_NAME_"+l+"::"+userName);
						nodeVO.setUserName(userName);
					}
					catch(Exception e)
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of USER_NAME_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}

					try
					{
						password = FileCache.getValue(p_iNId,"PASSWORD_"+l).trim();
						if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","PASSWORD_"+l+"::"+password);
						nodeVO.setPassword(password);
					}
					catch(Exception e)
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of PASSWORD_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}
					
					try
					{
					    String expiryTimeStr = FileCache.getValue(p_iNId,"EXPIRY_TIME_"+l).trim();
					    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","EXPIRY_TIME_"+l+"::"+expiryTimeStr);
					...
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of EXPIRY_TIME_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}
					
					try
					{
					    int maxBarredCount = Integer.parseInt(FileCache.getValue(p_iNId,"MAX_BARRED_COUNT_"+l).trim());					    
					    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","MAX_BARRED_COUNT_"+l+"::"+maxBarredCount);
					    nodeVO.setMaxBarredCount(maxBarredCount);
					}
					catch(Exception e)
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of MAX_BARRED_COUNT_"+l +" in INFile "+p_iNId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
					}
				...
				/*	try
					{
						...
						if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","MAX_CON_NODE_"+l+"::"+maxConPerNode);
					...
					_nodeTable.put(String.valueOf(l),nodeVO);
			   }
			   ...
			   {
			       e.printStackTrace();
			       EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "","Exception while initializing the Node details of interfaceID = "+p_iNId +" Exception e="+e.getMessage());
			       throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_NODE_INITIALIZATION);
			   }
			}
		}
    ...
    public synchronized NodeVO getNodeVO(String p_transId) throws BTSLBaseException
    {
        ...
        try
        {
            ...
            nodeVO = (NodeVO)_nodeTable.get(nodeNumStr);
            if(nodeVO==null)
            {
        ...
        {
            if(_log.isDebugEnabled()) _log.debug("getNodeVO",p_transId,"Exited nodeVO::"+nodeVO);
        }//end of finally
        return nodeVO;
    }//end of getNodeVO
    /**
    ...
    private synchronized String getNode(String p_transId) throws BTSLBaseException
    {
        ...
        try
        {
        	...
        	nodeVO = (NodeVO)_nodeTable.get(String.valueOf(_nodeNumber));
	        ...
	        while(isNext)
	        {
	            try
	            {
		            if(tempCount>_maxAllowedNum)
	                {
	                    ...
	                    //throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_MAX_NODE_CHECK_REACH);
		            ...
		            // check for the Max count reached for barred the transaction if reached then barred the node
		            ...
		            if(!nodeVO.isBlocked())
		            {
		            	...
		            	if(nodeVO.getBarredCount()>=nodeVO.getMaxBarredCount())
			            {
			            	...
			            	nodeNumStr = String.valueOf(_nodeNumber);
			            	incrementNodeNumber();
                                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "NodeScheduler[getNode]", "INTERFACE_ID::"+_interfaceID,"", "p_transId::"+p_transId, "CCWS IP="+nodeVO.getUrl()+" is not responding and blocked");
			            	continue;
			            }
	                ...
	                if(isExpired(nodeVO.getBlokedAt(),nodeVO.getExpiryDuration()))
	                {
                        ...
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "NodeScheduler[getNode]", "INTERFACE_ID::"+_interfaceID,"", "p_transId::"+p_transId, "CCWS IP="+nodeVO.getUrl()+" is unblocked");
	                    break;
	                }
