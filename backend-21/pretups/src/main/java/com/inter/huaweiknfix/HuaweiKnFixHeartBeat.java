...
public class HuaweiKnFixHeartBeat implements Runnable {
    private Log _log = LogFactory.getLog(HuaweiKnFixHeartBeat.class.getName());
    HuaweiKnFixSocketWrapper _socketConnection = null;
    ...
    public HuaweiKnFixHeartBeat(String p_interfaceID) throws Exception {
        ...
        if (InterfaceUtil.isNullString(heartBeatCommand)) {
            _log.error("HuaweiKnFixHeartBeat[Constructor]", "HEART_BEAT_COMMAND is not defined in the INFile");
            throw new BTSLBaseException(this, "HuaweiKnFixHeartBeat[Constructor]", InterfaceErrorCodesI.ERROR_BAD_REQUEST);
        }
        ...
        if (InterfaceUtil.isNullString(_startFlag)) {
            _log.error("HuaweiKnFixHeartBeat[Constructor]", "START_FLAG is not defined in the INFile");
            throw new BTSLBaseException(this, "HuaweiKnFixHeartBeat[Constructor]", InterfaceErrorCodesI.ERROR_BAD_REQUEST);
        }
         ...
         * {
         * _log.error("HuaweiKnFixHeartBeat[Constructor]",
        ...
        if (InterfaceUtil.isNullString(heartBeatCountStr)) {
            _log.error("HuaweiKnFixHeartBeat[Constructor]", "OPTIMUM_HEART_BEAT_COUNT is not defined in the INFile");
            heartBeatCountStr = "3";
        }
        ...
        if (InterfaceUtil.isNullString(heartBeatTimeStr)) {
            _log.error("HuaweiKnFixHeartBeat[Constructor]", "OPTIMUM_HEART_BEAT_TIME is not defined in the INFile");
        ...
        try {
            _requestStr = _formatter.generateRequest(HuaweiKnFixI.ACTION_HEART_BEAT, _requestMap);
        } catch (Exception e) {
            e.printStackTrace();// for testing
        }
        _freeList = (Vector) HuaweiKnFixPoolManager._freeBucket.get(_interfaceID);
        _busyList = (Vector) HuaweiKnFixPoolManager._busyBucket.get(_interfaceID);
    ...
    public void run() {
        ...
        while (_startHeartBeat) {
            try {
                ...
                _socketConnection = HuaweiKnFixPoolManager.getClientObject(_interfaceID, true);
