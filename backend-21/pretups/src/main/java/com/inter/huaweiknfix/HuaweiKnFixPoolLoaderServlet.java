...
/**
 * @(#)HuaweiiKnFixPoolLoaderServlet.java
 ...
 *                                        --------------------------------------
 *                                        ---------------------
 *                                        Abhay January 27, 2009 Initial
 *                                        Creation
 *                                        --------------------------------------
 *                                        --------------------------------------
 *                                        -------------------
 *                                        This class is responsible to
 *                                        instantiate the PoolManager either at
 *                                        the server start-up or
...
public class HuaweiKnFixPoolLoaderServlet extends HttpServlet {
    private Log _log = LogFactory.getLog(HuaweiKnFixPoolLoaderServlet.class.getName());
    private HuaweiKnFixPoolManager _poolManager = null;
    private HuaweiKnFixHeartBeatController _huaweiHeartController = null;

    /**
     * Constructor of the object.
     */
    public HuaweiKnFixPoolLoaderServlet() {
        super();
    }
    ...
    public void destroy() {
        ...
        String poolIDs = Constants.getProperty("HUAWEIiKnFix_POOL_IN_IDS");
        if (_log.isDebugEnabled())
    ...
    public void initialize() {
        ...
        try {
            _poolManager = new HuaweiKnFixPoolManager();
            poolINIDs = Constants.getProperty("HUAWEI_POOL_IN_IDS");
            if (_log.isDebugEnabled())
                _log.debug("initialize", "poolINIDs = " + poolINIDs);
            if (!InterfaceUtil.isNullString(poolINIDs)) {
                _poolManager.initialize(poolINIDs);
                _huaweiHeartController = new HuaweiKnFixHeartBeatController(poolINIDs);
            }
    ...
    public void initialize(String p_interfaceId) {
        ...
        try {
            ...
            if (!InterfaceUtil.isNullString(p_interfaceId)) {
                ...
                _huaweiHeartController = new HuaweiKnFixHeartBeatController(p_interfaceId);
            }
        } catch (BTSLBaseException be) {
            _log.error("initialize", "BTSLBaseException be::" + be.getMessage());
        } catch (Exception e) {
