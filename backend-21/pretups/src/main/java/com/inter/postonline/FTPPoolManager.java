...
public class FTPPoolManager {
    private static Log _log = LogFactory.getLog(FTPPoolManager.class.getName());
        ...
        try {
            ...
            for (int i = 0, size = inStrArray.length; i < size; i++) {
                ...
                try {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[initialize]", "", "", "", "While instantiation of the Client objects MAX_POOL_SIZE/POOL_SLEEP is either not defined or not numeric, in the INFile");
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
                }
                ...
                for (int loop = 0; loop < poolSize; loop++) {
                    try {
                        ...
                        e.printStackTrace();
                        _log.error("initialize", "Exception e:" + e.getMessage());
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[initialize]", "", "", "", "While instantiation of the Client objects got the Exception " + e.getMessage());
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
                    }
                }// end of for loop
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[initialize]", "String of interface ids=" + p_interfaceIDs, "", "", "While initializing the instance of Client Object for the INTERFACE_ID =" + interfaceId + " get Exception=" + e.getMessage());
     ...
     * @throws Exception
     */
    public static synchronized FtpClient getClientObject(String p_interfaceID, boolean p_module) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("getClientObject", "Entered p_interfaceID::" + p_interfaceID + " p_module:" + p_module);
        FtpClient clientObject = null;
        ...
        try {
            if (_freeBucket.isEmpty() || !_freeBucket.containsKey(p_interfaceID)) {
                ...
                // EventLevelI.FATAL,
                // "FTPPoolManager[getClientObject]","INTERFACE_ID="+p_interfaceID,
                // "","",
                ...
                // throw new
                // BTSLBaseException("FTPPoolManager[getClientObject]",InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
            }
            ...
            if (freeList == null) {
                ...
                if (!_isPoolInitializing && ((System.currentTimeMillis() / 1000) - _lastPoolInitializationTime) > poolInitializationTimeLong) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "FTPPoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "Pool Container is Null for interface Id =" + p_interfaceID + " Hence reinitialized the Pool: last Initiazation time:" + _lastPoolInitializationTime);
                }
                _log.error("getClientObject", "POOL Container(Vector list) is NULL for the interface id:" + p_interfaceID);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "While getting the the instance of ClintObject for the interfaceID =" + p_interfaceID + "POOL contains no object for the p_interfaceID:" + p_interfaceID);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
            }
            ...
            clientObject = (FtpClient) freeList.remove(0);
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "While getting the the instance of ClintObject for the interfaceID =" + p_interfaceID + " get Exception=" + e.getMessage());
     ...
     * @throws BTSLBaseException
     */
    public static synchronized FtpClient getNewClientObject(String p_interfaceID) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("getNewClientObject", "p_interfaceID::" + p_interfaceID);
        FtpClient ftpClient = null;
        try {
            ftpClient = new FtpClient(p_interfaceID);
        } catch (BTSLBaseException be) {
            throw be;
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("getNewClientObject", "Exception e::" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[getNewClientObject]", "INTERFACEID:" + p_interfaceID, "", "", "While getting the instance of Client objects got the Exception " + e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_GETTIN_NEW_CLIENT_OBJECT);
        } finally {
            if (_log.isDebugEnabled())
                _log.debug("getNewClientObject", "Exiting ftpClient:" + ftpClient);
        }
        return ftpClient;
    }
    ...
    public void destroy(String p_interfaceId) {
        ...
        try {
                            ...
                            try {
                                ...
                                if (objectList != null) {
                                    ...
                                    for (int i = 0; i < size; i++) {
                                        FtpClient ftpClient = (FtpClient) objectList.get(i);
                                        ftpClient.destroy();
                                    }
                                }
                            } catch (Exception e) {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "FTPPoolManager[destroy]", "", "", "", "While destorying the Client objects got the Exception " + e.getMessage());
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
