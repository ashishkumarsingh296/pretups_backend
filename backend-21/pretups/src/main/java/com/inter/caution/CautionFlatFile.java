...
public class CautionFlatFile {
    private static Log _log = LogFactory.getLog(CautionFlatFile.class.getName());
    ...
    public boolean createFlatFile(HashMap data) throws BTSLBaseException {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(filePath)) {
                _log.error("createFlatFile", "File Path for caution is not defined in the Constant.props");
            ...
            if (InterfaceUtil.isNullString(fileName)) {
                _log.error("createFlatFile", "File name for caution is not defined in the Constant.props");
                throw new BTSLBaseException(this, "createFlatFile", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            try {
