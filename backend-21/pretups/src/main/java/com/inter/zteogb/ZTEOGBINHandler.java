...
public class ZTEOGBINHandler implements InterfaceHandler {
    private Log _log = LogFactory.getLog(ZTEOGBINHandler.class.getName());
    private ZTEOGBReqResFormatter _formatter = null;
     ...
     * Constructor
     */
    public ZTEOGBINHandler() {
        _formatter = new ZTEOGBReqResFormatter();
    ...
    public void validate(HashMap<String, String> p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(multFactor)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[validate]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
            ...
            setInterfaceParameters(ZTEOGBI.ACTION_ACCOUNT_INFO);
            // generate Request Object
            String requestStr = _formatter.generateRequest(ZTEOGBI.ACTION_ACCOUNT_INFO, _requestMap);
            // sending the AccountInfo request to IN along with validate action
            // defined in Huawei84I interface
            sendRequestToIN(ZTEOGBI.ACTION_ACCOUNT_INFO, requestStr);

            // Check whether the subscriber is black listed on the IN or not.
            /**
             * String stateSetBits =_responseMap.get("resp_stateSet");
             * String checkBlackList=FileCache.getValue(_interfaceID,
             * "BLACKLIST_CHECK");
             ...
             * if(_log.isDebugEnabled())
             * _log.debug("validate",
             * "Subscriber is blacklisted on IN. Either State Set for eighth bit or tenth bit is 1. "
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[validate]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "Balance obtained from the IN is not numeric, while parsing the Balance get Exception e:" + e.getMessage());
            ...
            e.printStackTrace();
            _log.error("validate", "Exception e:" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[validate]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "While validating the subscriber get the Exception e:" + e.getMessage());
            throw new BTSLBaseException(this, "validate", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled())
    ...
    public void credit(HashMap<String, String> p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            // factor, and received balance would be devided by this factor.
            String multFactor = FileCache.getValue(_interfaceID, "MULT_FACTOR");
            if (_log.isDebugEnabled())
                _log.debug("credit", "multFactor:" + multFactor);
            if (InterfaceUtil.isNullString(multFactor)) {
                _log.error("credit", "MULT_FACTOR  is not defined in the INFile");
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[credit]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
            ...
            // Set the interface parameters into requestMap
            setInterfaceParameters(ZTEOGBI.ACTION_RECHARGE_CREDIT);
            ...
            try {
                ...
                if (InterfaceUtil.isNullString(roundFlag)) {
                    roundFlag = "Y";
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MINOR, "ZTEOGBINHandler[credit]", _referenceID + "MSISDN = " + _msisdn, " INTERFACE ID = " + _interfaceID, "Network code = " + _requestMap.get("NETWORK_CODE"), "ROUND_FLAG is not defined in IN File hence system taken the default value=N.");
                }
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[credit]", "REFERENCE ID = " + _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
                throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            if (_log.isDebugEnabled())
            ...
            String requestStr = _formatter.generateRequest(ZTEOGBI.ACTION_RECHARGE_CREDIT, _requestMap);
            // sending the Re-charge request to IN along with re-charge action
            // defined in SafcomI interface
            sendRequestToIN(ZTEOGBI.ACTION_RECHARGE_CREDIT, requestStr);
            // set TRANSACTION_STATUS as Success in request map
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
            ...
            try {
                ...
                _log.error("credit", "Exception e:" + e.getMessage());
                e.printStackTrace();
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[credit]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "credit balance from response is not numeric or multiplication factor defined in the INFile is not Numeric, Exception e:" + e.getMessage());
                throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            ...
            try {
                ...
                e.printStackTrace();
                _log.error("credit", "Exception e:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[credit]", _referenceID, _msisdn, _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in credit");
                throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("credit", "Exception e:" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[credit]", _referenceID, _msisdn, _requestMap.get("NETWORK_CODE"), "While credit get the Exception e:" + e.getMessage());
            throw new BTSLBaseException(this, "credit", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled())
    ...
    public void debitAdjust(HashMap<String, String> p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(multFactor)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[debitAdjust]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
            ...
            // Set the interface parameters into requestMap
            setInterfaceParameters(ZTEOGBI.ACTION_IMMEDIATE_DEBIT);
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[debitAdjust]", "REFERENCE ID = " + _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
                throw new BTSLBaseException(this, "debitAdjust", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            if (_log.isDebugEnabled())
            ...
            // value of INTERFACE_AMOUNT)
            _requestMap.put("transfer_amount", amountStr);
            String requestSre = _formatter.generateRequest(ZTEOGBI.ACTION_IMMEDIATE_DEBIT, _requestMap);
            // sending the Re-charge request to IN along with re-charge action
            // defined in SafcomI interface
            sendRequestToIN(ZTEOGBI.ACTION_IMMEDIATE_DEBIT, requestSre);
            // set TRANSACTION_STATUS as Success in request map
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
            // Set the post balance after multiplied by multiplication factor.
            try {
                ...
                _log.error("debitAdjust", "Exception e:" + e.getMessage());
                e.printStackTrace();
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[debitAdjust]", _referenceID, _msisdn + " INTERFACE ID = " + _interfaceID, _requestMap.get("NETWORK_CODE"), "balance from response is not numeric or multiplication factor defined in the INFile is not Numeric, Exception e:" + e.getMessage());
                throw new BTSLBaseException(this, "debitAdjust", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            // Set the value of NEW_EXPIRY_DATE as the value of end_val_date
            ...
            try {
                ...
                e.printStackTrace();
                _log.error("debitAdjust", "Exception e:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[debitAdjust]", _referenceID, _msisdn, _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in debitAdjust");
                throw new BTSLBaseException(this, "debitAdjust", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("debitAdjust", "Exception e:" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[debitAdjust]", _referenceID, _msisdn, _requestMap.get("NETWORK_CODE"), "While debitAdjust get the Exception e:" + e.getMessage());
            throw new BTSLBaseException(this, "debitAdjust", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled())
    ...
    private void sendRequestToIN(int p_action, String p_requestStr) throws BTSLBaseException {
        ...
        try {
            ...
            try {
                ZTEOGBWebServiceConnector serviceConnection = new ZTEOGBWebServiceConnector();
                call = serviceConnection.callService(_requestMap);

                if (call == null) {
                    EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[sendRequestToIN]", "REFERENCE ID = " + _referenceID + "MSISDN = " + _msisdn, "INTERFACE ID = " + _interfaceID, "Network code = " + _requestMap.get("NETWORK_CODE") + " Action = " + p_action, "Unable to get Client Object");
            ...
            /**
             * if(p_action==ZTEOGBI.ACTION_ACCOUNT_INFO &&
             * ZTEOGBI.RESULT_OK.equals(status))
             * {
             ...
             * if(!InterfaceUtil.isNullString(stateSet))
             * {
             * 
             ...
             * "ZTEOGBINHandler[sendRequestToIN]"
             * ,"REFERENCE ID = "+_referenceID+
             * "MSISDN = "+_msisdn,"INTERFACE ID = "
             * +_interfaceID,"Network code = "+
             * _requestMap.get("NETWORK_CODE")+" Action = "
             * +p_action," Subscriber has applied for the block.");
             * _log.error("sendRequestToIN",
             * "Subscriber has applied for the block. StateSet Received from IN="
             * +stateSet);
             * throw new BTSLBaseException(InterfaceErrorCodesI.
             * INTERFACE_USER_APPLIED_BLOCK);
             * }
             ...
             * +_interfaceID,"Network code = "+
             * _requestMap.get("NETWORK_CODE")+" Action = "
             * +p_action," Operator forced block.");
             * _log.error("sendRequestToIN",
             * "Subscriber has applied for the block. StateSet Received from IN="
             * +stateSet);
             * throw new BTSLBaseException(InterfaceErrorCodesI.
             * INTERFACE_OPERATOR_FORCED_BLOCK);
             * }
             * //If 2nd digit of StateSet is not 0(zero) i.e. operator forced
             * for the SIM block, then exception with error code as
             ...
             * +_interfaceID,"Network code = "+
             * _requestMap.get("NETWORK_CODE")+" Action = "
             * +p_action," Recharge not allowed due to SIM loss.");
             * _log.error("sendRequestToIN",
             * "Recharge not allowed due to SIM loss. StateSet Received from IN"
             * +stateSet);
             * throw new
             * BTSLBaseException(InterfaceErrorCodesI.INTERFACE_SIM_LOSS_BLOCk);
             * }
             * }
             * }
             **/
            ...
            if (!ZTEOGBI.RESULT_OK.equals(status)) {
                int len = status.length();
                if (len > 10)
                    status = status.substring(len - 10);

                _requestMap.put("INTERFACE_STATUS", status);
                if (ZTEOGBI.MSISDN_NOT_FOUND.equals(status)) {
    ...
    public void setInterfaceParameters(int p_action) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(cancelTxnAllowed)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "CANCEL_TXN_ALLOWED is not defined in the INFile.");
            ...
            if (InterfaceUtil.isNullString(systemStatusMappingCr)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(systemStatusMappingCrAdj)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT_ADJ is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(systemStatusMappingDbtAdj)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_DEBIT_ADJ is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(systemStatusMappingCrBck)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT_BCK is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(cancelCommandStatusMapping)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "CANCEL_COMMAND_STATUS_MAPPING is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(cancelNA)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "CANCEL_NA is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(userId)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "USER_NAME is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(passwd)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "PASSWORD is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(authHeader)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "AUTH_HEADER is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(zteNameSpace)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "ZTE_NAMESPACE is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(uri)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "SOAP_ACTION_URI is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(endPt)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "END_POINT is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(operationName)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "OPERATION_NAME is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(channelId)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", _referenceID, "INTERFACE ID" + _interfaceID + " MSISDN " + _msisdn, _requestMap.get("NETWORK_CODE"), "CHANNEL_ID is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
            ...
            if (InterfaceUtil.isNullString(warnTimeStr) || !InterfaceUtil.isNumeric(warnTimeStr)) {
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ZTEOGBINHandler[setInterfaceParameters]", "REFERENCE ID = " + _referenceID + "MSISDN = " + _msisdn, " INTERFACE ID = " + _interfaceID, "Network code " + _requestMap.get("NETWORK_CODE"), "WARN_TIMEOUT is not defined in IN File or not numeric");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
            }
    ...
    private void setLanguageFromMapping() throws Exception {
        ...
        try {
            ...
            if (mappingNotFound)
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "ZTEOGBINHandler[setLanguageFromMapping]", _referenceID, _msisdn, _requestMap.get("NETWORK_CODE"), "Mapping for language received from IN =  " + langFromIN + " is not defined in IN file Hence setting the Default language");
            ...
            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "ZTEOGBINHandler[setLanguageFromMapping]", _referenceID, _msisdn, _requestMap.get("NETWORK_CODE"), "while setting the language mapping get Exception=" + e.getMessage());
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
