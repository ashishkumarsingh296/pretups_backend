...
public class PoolManager {
    private static Log _log = LogFactory.getLog(PoolManager.class.getName());
    ...
    public static void initialize(String p_interfaceIDs) throws BTSLBaseException {
        ...
        Vector objectList = null;// To schyncronizes the pooled objects
        try {
            ...
            for (int i = 0, size = inStrArray.length; i < size; i++) {
                ...
                try {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[initialize]", "", "", "", "While instantiation of the Client objects MAX_POOL_SIZE/POOL_SLEEP is either not defined or not numeric, in the INFile");
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
                }
                ...
                for (int loop = 0; loop < poolSize; loop++) {
                    try {
                        ...
                        e.printStackTrace();
                        _log.error("initialize", "Exception e:" + e.getMessage());
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[initialize]", "", "", "", "While instantiation of the Client objects got the Exception " + e.getMessage());
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
                    }
                }// end of for loop
            ...
            e.printStackTrace();
            _log.error("initialize", "Exception e::" + e.getMessage());
            // Destroying the Client Objects from Hashtable _freeBucket
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[initialize]", "String of interface ids=" + p_interfaceIDs, "", "", "While initializing the instance of Client Object for the INTERFACE_ID =" + interfaceId + " get Exception=" + e.getMessage());
     ...
     * @throws Exception
     */
    public static synchronized Object getClientObject(String p_interfaceID, boolean p_module) throws BTSLBaseException {
        if (_log.isDebugEnabled())
        ...
        try {
            if (_freeBucket.isEmpty() || !_freeBucket.containsKey(p_interfaceID)) {
                _log.info("getClientObject", "POOL is not initialized and contains no objects in the pool,Hence reinitialize the pool");
                /*
                 * EventHandler.handle(EventIDI.SYSTEM_ERROR,
                 * EventComponentI.SYSTEM, EventStatusI.RAISED,
                 * EventLevelI.FATAL,
                 * "PoolManager[getClientObject]","INTERFACE_ID="+p_interfaceID,
                 * "","",
                 * "While getting the the instance of ClintObject for the interfaceID ="
                 * +p_interfaceID+
                 * "POOL is not initialized and contains no objects in the pool"
                 * );
                 * throw new BTSLBaseException("PoolManager[getClientObject]",
                 * InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
                 */
            ...
            if (freeList == null) {
                _log.error("getClientObject", "freeList for the interface id :" + p_interfaceID + " Null hence reinitialize the pool");
                ...
                if (!_isPoolInitializing && ((System.currentTimeMillis() / 1000) - _lastPoolInitializationTime) > poolInitializationTimeLong) {
                    initialize(p_interfaceID);
                    _log.error("getClientObject", "Pool for interface id :" + p_interfaceID + " is initialized successfully");
                    _lastPoolInitializationTime = (System.currentTimeMillis() / 1000);
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "Pool Container is Null for interface Id =" + p_interfaceID + " Hence reinitialized the Pool: last Initiazation time:" + _lastPoolInitializationTime);
                }
                _log.error("getClientObject", "POOL Container(Vector list) is NULL for the interface id:" + p_interfaceID);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "While getting the the instance of ClintObject for the interfaceID =" + p_interfaceID + "POOL contains no object for the p_interfaceID:" + p_interfaceID);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
            }
            ...
            // 3.Or we have to creat a new refrence of client object.
            ...
            if (listSize == 0) {
                try {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "While getting the retry attempt[SLEEP_TIME, C2S_RETRY_ATTEMPT and P2P_RETRY_ATTEMPT] for interface id=" + p_interfaceID + " get Exception=" + e.getMessage());
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_FETCH_CLIENT_OBJECT);
                }
            ...
            clientObject = freeList.remove(0);
            ...
            // It should be removed after the request has been completed(suc or
            // fail) In the Hander Class.
            // And should be added to the freeBucketList.
        } catch (BTSLBaseException be) {
            _log.error("getClientObject", "BTSLBaseException be:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("getClientObject", "Exception e::" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[getClientObject]", "INTERFACE_ID=" + p_interfaceID, "", "", "While getting the the instance of ClintObject for the interfaceID =" + p_interfaceID + " get Exception=" + e.getMessage());
            ...
            if (_log.isDebugEnabled())
                _log.debug("getClientObject", "Exited clientObject::" + clientObject);
        }// end of finally
        return clientObject;
    }// end of getClientObject
     ...
     * @throws BTSLBaseException
     */
    public static synchronized Object getNewClientObject(String p_interfaceID) throws BTSLBaseException {
        if (_log.isDebugEnabled())
        ...
        try {
            ...
            // Get the full name of client class from the INFile
            clientHandlerClass = FileCache.getValue(p_interfaceID, "CLIENT_HANDLER_CLASS");
            if (_log.isDebugEnabled())
                _log.debug("getNewClientObject", "clientHandlerClass::" + clientHandlerClass);
            // Check for the Null value of clientHandlerClass and handle the
            // event
            if (InterfaceUtil.isNullString(clientHandlerClass))
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_GETTIN_NEW_CLIENT_OBJECT);
            try {
                client = Class.forName(clientHandlerClass.trim());
                argTypeClass = new Class[] { String.class };
                stringArgConstructor = client.getConstructor(argTypeClass);
                clientObject = stringArgConstructor.newInstance(new String[] { p_interfaceID });
            } catch (Exception e) {
                e.printStackTrace();
                _log.error("getNewClientObject", "Exception e:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[getNewClientObject]", "INTERFACEID:" + p_interfaceID, "", "", "While getting the instance of Client objects got the Exception " + e.getMessage());
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_GETTIN_NEW_CLIENT_OBJECT);
            }
        } catch (BTSLBaseException be) {
            _log.error("getNewClientObject", "BTSLBaseException be:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("getNewClientObject", "Exception e::" + e.getMessage());
            ...
            if (_log.isDebugEnabled())
                _log.debug("getNewClientObject", "Exiting clientObject:" + clientObject);
        }
        return clientObject;
    }

    /**
     * This method is used to destoy the ClientObject references from freeBucket
     * and busyBucket.
     * 
     * @param String
     *            p_interfaceId
    ...
    public void destroy(String p_interfaceId) {
        ...
        try {
                            ...
                            try {
                                ...
                                if (objectList != null) {
                                    ...
                                    for (int i = 0; i < size; i++) {
                                        ClientMarkerI markerI = (ClientMarkerI) objectList.get(i);
                                        markerI.destroy();
                                    }
                                }
                            } catch (Exception e) {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "PoolManager[destroy]", "", "", "", "While destorying the Client objects got the Exception " + e.getMessage());
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
