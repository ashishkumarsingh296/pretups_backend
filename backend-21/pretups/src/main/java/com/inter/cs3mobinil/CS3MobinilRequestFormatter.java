...
public class CS3MobinilRequestFormatter {
    public static Log _log = LogFactory.getLog(CS3MobinilRequestFormatter.class);
    ...
    public CS3MobinilRequestFormatter() throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CS3MobinilRequestFormatter[constructor]", "Entered");
        try {
            ...
            e.printStackTrace();
            _log.error("CS3MobinilRequestFormatter[constructor]", "Exception e::" + e.getMessage());
            throw e;
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
                _log.debug("CS3MobinilRequestFormatter[constructor]", "Exited");
        }// end of finally
    }// end of CS3MobinilRequestFormatter[constructor]
    ...
    protected String generateRequest(int p_action, HashMap p_map) throws Exception {
        ...
        try {
            switch (p_action) {
            case CS3MobinilI.ACTION_ACCOUNT_INFO: {
                str = generateGetAccountInfoRequest(p_map);
                break;
            }
            case CS3MobinilI.ACTION_ACCOUNT_DETAILS: {
                str = generateGetAccountDetailRequest(p_map);
                break;
            }
            case CS3MobinilI.ACTION_RECHARGE_CREDIT: {
                str = generateRechargeCreditRequest(p_map);
                break;
            }
            case CS3MobinilI.ACTION_IMMEDIATE_DEBIT: {
                str = generateImmediateDebitRequest(p_map);
                break;
            }
            case CS3MobinilI.ACTION_DEDICATED_ACCOUNT_CD: {
     ...
     * @throws Exception
     */
    private String generateDedicatedAccountCDRequest(HashMap p_requestMap) throws Exception {
        if (_log.isDebugEnabled())
        ...
        return requestStr;
    }// end of generateDedecatedAccountCDRequest
     ...
     * @throws Exception
     */
    private HashMap parseDedicatedAccountCDRequest(String p_responseStr) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("parseDedicatedAccountCDRequest", "Entered p_responseStr::" + p_responseStr);
        HashMap responseMap = null;
        int indexStart = 0;
        int indexEnd = 0;
        int tempIndex = 0;
        ...
        try {
            ...
            _log.error("parseDedicatedAccountCDRequest", "Exception e::" + e.getMessage());
            throw e;
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
                _log.debug("parseDedicatedAccountCDRequest", "Exited responseMap::" + responseMap);
        }
        return responseMap;
    }// end of parseDedicatedAccountCDRequest
     ...
     * @throws Exception
     */
    private void parseDedicatedAccountInfo(HashMap p_responseMap, String p_responseString) throws Exception {
        if (_log.isDebugEnabled())
     ...
     * @throws Exception
     */
    private String getDedicatedAccExpiryDate(HashMap p_requestMap) throws Exception {
        if (_log.isDebugEnabled())
