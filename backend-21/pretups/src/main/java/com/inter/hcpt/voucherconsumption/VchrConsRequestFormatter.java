...
public class VchrConsRequestFormatter {
    public static Log _log = LogFactory.getLog(VchrConsRequestFormatter.class);
    ...
    public VchrConsRequestFormatter() throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("VchrConsRequestFormatter[constructor]", "Entered");
        try {
            ...
            e.printStackTrace();
            _log.error("VchrConsRequestFormatter[constructor]", "Exception e::" + e.getMessage());
            throw e;
        } finally {
            if (_log.isDebugEnabled())
                _log.debug("VchrConsRequestFormatter[constructor]", "Exited");
        }
    }
    ...
    protected String generateRequest(int p_action, HashMap<String, String> p_map) throws Exception {
        ...
        try {
            switch (p_action) {
            case VchrConsI.ACTION_ACCOUNT_INFO: {
                str = generateGetAccountInfoRequest(p_map);
                break;
            }
            case VchrConsI.ACTION_ACCOUNT_DETAILS: {
                str = generateGetAccountDetailRequest(p_map);
                break;
            }
            case VchrConsI.ACTION_RECHARGE_CREDIT: {
                str = generateRechargeCreditRequest(p_map);
                break;
            }
            case VchrConsI.ACTION_IMMEDIATE_DEBIT: {
                str = generateImmediateDebitRequest(p_map);
                break;
            }
            case VchrConsI.ACTION_DEDICATED_ACCOUNT_CD: {
    ...
    private HashMap<String, String> parseImmediateDebitResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (tempIndex > 0) {
                ...
                responseMap.put("responseCode", responseCode.trim());
                Object[] successList = VchrConsI.RESULT_OK.split(",");
                // if(!VchrConsI.RESULT_OK.equals(responseCode))
                if (!Arrays.asList(successList).contains(responseCode))
                    return responseMap;
            }
            indexStart = p_responseStr.indexOf("<member><name>originTransactionID", indexEnd);
