 ...
 *                 CORBA.
 */
public class NokiaClient implements ClientMarkerI {
    private static Log _log = LogFactory.getLog("NokiaClient".getClass().getName());
     ...
     * @param p_interfaceID
     */
    public NokiaClient(String p_interfaceID) throws BTSLBaseException, Exception {
    ...
    /**
     * Method to get the Account Information for a MSISDN
     * 
     * @param p_requestMap
     * @return
     * @throws Exception
     */
    private String accountInfoRequest(HashMap p_requestMap) throws Exception {
        if (_log.isDebugEnabled())
        ...
        Property[] commonProperties = new Property[1];
        AccountInfo[] retAccountInfo = null;
        ...
        try {
            ...
            commonProperties[0] = new Property();
            commonProperties[0].name = "returnFields";
            commonProperties[0].value = "balance lastCallDate lastCallCost provisionId scpId expiryInd nearCreditExpiry creditExpiry nearSubscriptionExpiry subscriptionExpiry subscriberLanguage rechargeAllowed secondsBeforeClearing firstCallIndicator providerId latestStateChange expiryServiceId profileId firstCallDate maxBalanceLimit expiryState";
            retAccountInfo = _accountManager.getAccountInfo(msisdn, accountInfoSeqHolder, commonProperties);
            ...
            for (int i = 0; i < accountInfo.length; i++) {
                System.out.println("For Account Info Pos=" + i + " For ID=" + accountInfo[i].accountId);
                returnProp = accountInfo[i].properties;
                for (int j = 0; j < returnProp.length; j++) {
                    if (_log.isDebugEnabled())
                        _log.debug("accountInfoRequest", "At Pos=" + j + " Name=" + returnProp[j].name + " Value=" + returnProp[j].value);
                    /*
            ...
            if (_log.isDebugEnabled())
                _log.debug("accountInfoRequest", "Exited responseStr:" + responseStr);
        }
        return responseStr;
    }
            ...
            try {
                ...
                if (NokiaI.RESULT_OK.equals((String) responseMap.get("statusInd"))) {
                    try {
                        ...
                        EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "NokiaClient[credit]", inReconID, msisdn, (String) p_requestMap.get("NETWORK_CODE"), "Not able to fire the Account Info Request after Credit for balance:" + e.getMessage());
                        respBuff.append("&PBLENQ=N");
                        responseStr = respBuff.toString();
                    }
                }
            ...
            try {
                ...
                if (NokiaI.RESULT_OK.equals((String) responseMap.get("statusInd"))) {
                    try {
                        ...
                        // Handle the exception
                        // Confirm to handle the event
                        EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "NokiaClient[credit]", inReconID, msisdn, (String) p_requestMap.get("NETWORK_CODE"), "Not able to fire the Account Info Request after Debit for balance:" + e.getMessage());
                        respBuff.append("&PBLENQ=N");
                        responseStr = respBuff.toString();
                    }
                }
