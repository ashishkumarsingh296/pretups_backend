...
public class CS3CongoRequestFormatter {
    public static Log _log = LogFactory.getLog(CS3CongoRequestFormatter.class);
    ...
    public CS3CongoRequestFormatter() throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CS3CongoRequestFormatter[constructor]", "Entered");
        try {
            ...
            e.printStackTrace();
            _log.error("CS3CongoRequestFormatter[constructor]", "Exception e::" + e.getMessage());
            throw e;
        }// end of catch-Exception
        finally {
            if (_log.isDebugEnabled())
                _log.debug("CS3CongoRequestFormatter[constructor]", "Exited");
        }// end of finally
    }// end of CS3CongoRequestFormatter[constructor]
    ...
    protected String generateRequest(int p_action, HashMap p_map) throws Exception {
        ...
        try {
            switch (p_action) {
            case CS3CongoI.ACTION_ACCOUNT_INFO: {
                str = generateGetAccountInfoRequest(p_map);
                break;
            }
            case CS3CongoI.ACTION_ACCOUNT_DETAILS: {
                str = generateGetAccountDetailRequest(p_map);
                break;
            }
            case CS3CongoI.ACTION_RECHARGE_CREDIT: {
                str = generateRechargeCreditRequest(p_map);
                break;
            }
            case CS3CongoI.ACTION_IMMEDIATE_DEBIT: {
    ...
    private HashMap parseGetAccountInfoResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (tempIndex > 0) {
                ...
                System.out.println("shishupal IN req 1" + responseCode + ":" + Arrays.asList(successList));
                // if(!CS3CongoI.RESULT_OK.equals(responseCode))
                if (!Arrays.asList(successList).contains(responseCode)) {
                    System.out.println("shishupal IN req 2" + successList + responseCode);
                    return responseMap;
                }
            }
    ...
    public HashMap parseRechargeCreditResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (tempIndex > 0) {
                ...
                responseMap.put("responseCode", responseCode.trim());
                Object[] successList = CS3CongoI.RESULT_OK.split(",");
                System.out.println("shishupal INReq 2" + successList);
                // if(!CS3CongoI.RESULT_OK.equals(responseCode))
                if (!Arrays.asList(successList).contains(responseCode))
                    return responseMap;
            }
    ...
    private HashMap parseImmediateDebitResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (tempIndex > 0) {
                ...
                responseMap.put("responseCode", responseCode.trim());
                Object[] successList = CS3CongoI.RESULT_OK.split(",");
                System.out.println("shishupal INReq 3" + successList);
                // if(!CS3CongoI.RESULT_OK.equals(responseCode))
                if (!Arrays.asList(successList).contains(responseCode))
                    return responseMap;
            }
