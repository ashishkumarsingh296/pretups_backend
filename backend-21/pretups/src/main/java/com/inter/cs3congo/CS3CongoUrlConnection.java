...
public class CS3CongoUrlConnection extends BTSLConnection {
    private HttpURLConnection _urlConnection = null;

    public CS3CongoUrlConnection(String p_url, String p_username, String p_password, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength, String p_hostName, String p_userAgent) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CS3CongoUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive + " p_hostName=" + p_hostName + " p_userAgent=" + p_userAgent);
        try {
            ...
            e.printStackTrace();
            _log.error("CS3CongoUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("CS3CongoUrlConnection", "Exiting");
    }
