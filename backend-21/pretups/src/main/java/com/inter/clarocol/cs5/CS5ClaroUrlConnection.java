...
public class CS5ClaroUrlConnection extends BTSLConnection{
    ...
    public CS5ClaroUrlConnection(String p_url,String p_username,String p_password,int p_connectTimeout,int p_readTimeout,String p_keepAlive,long p_contentLength, String p_hostName,String p_userAgent) throws Exception
    {
		if(_log.isDebugEnabled())_log.debug("CS5ClaroUrlConnection","Entered p_url: "+p_url+"p_connecttimeout="+p_connectTimeout+" p_readtimeout="+p_readTimeout+" p_keepAlive="+p_keepAlive+" p_hostName="+p_hostName+" p_userAgent="+p_userAgent);
		try
		{
		...
		{
			ste.printStackTrace();
	       _log.error("CS5ClaroUrlConnection", "Exception " + ste.getMessage());
	        throw ste;
  		}
		catch(Exception e)
		{
			e.printStackTrace();
	       _log.error("CS5ClaroUrlConnection", "Exception " + e.getMessage());
	        throw e;
  		}
		if(_log.isDebugEnabled())_log.debug("CS5ClaroUrlConnection","Exiting");
	}
    /**
