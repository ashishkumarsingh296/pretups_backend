        ...
        private String generateRechargeCreditRequest(HashMap<String,String> p_requestMap) throws Exception
    {
                ...
                try
                {
                    String module = p_requestMap.get("MODULE");
                        ...
                        double prevAmtDbl = 0;
                        if(!InterfaceUtil.isNullString(p_requestMap.get("prev_balance")))
                        {
                                prevAmtDbl=Double.parseDouble(p_requestMap.get("prev_balance"));

                                if(prevAmtDbl>0)
                                        prevAmtDbl = 0-prevAmtDbl;
                        ...
                        long validityLong = Long.parseLong(validity);
                        int extraDaysNeeded =BTSLUtil.getDifferenceInUtilDates(BTSLUtil.getDateFromDateString(p_requestMap.get("OLD_EXPIRY_DATE"),PretupsI.DATE_FORMAT),currentDate);

                        if(_log.isDebugEnabled())
                                _log.debug("generateRechargeCreditRequest","Validity : "+extraDaysNeeded + " : validity before : "+validity + " : currentDate - "+currentDate+"   Prev Balance : "+prevAmtDbl );

                        if( extraDaysNeeded > 0){
                                validityLong = extraDaysNeeded +validityLong;
                                transAmtDbl =  transAmtDbl- prevAmtDbl;
                        }
                        validity = String.valueOf(validityLong);

                        if(_log.isDebugEnabled())
                    _log.debug("generateRechargeCreditRequest"," : validity After : "+validity);
                        ...
                        sbf.append("<Data>");
                        sbf.append("<header>");
                        sbf.append("<ACTION_ID>AdjustBalance</ACTION_ID>");
                sbf.append("<REQUEST_ID>"+p_requestMap.get("IN_REQ_ID")+"</REQUEST_ID>");
                sbf.append("</header>");
                sbf.append("<body>");
            sbf.append("<MSISDN>"+InterfaceUtil.getFilterMSISDN(p_requestMap.get("INTERFACE_ID"),p_requestMap.get("MSISDN"))+"</MSISDN>");
         ...
         private HashMap<String,String> parseRechargeCreditResponse(String responseStr) throws Exception
         {
            if(_log.isDebugEnabled())
                    ...
                    if(_str.equals((returnCode)))
                {
                        ...
                        int index1=0;
 long    bdl_BalanceLng = 0;
                                        String bdl_Balance="";
                                        index=responseStr.indexOf("<AcctResCode>");
                                        if(index!=-1)
                                        {
                                                bdl_code=responseStr.substring(index+"<AcctResCode>".length(),responseStr.indexOf("</AcctResCode>",index));
                                                index1 = responseStr.indexOf("<Balance>");
                                                bdl_Balance=responseStr.substring(index1+"<Balance>".length(),responseStr.indexOf("</Balance>",index1));
                                                bdl_BalanceLng = Long.parseLong(bdl_Balance);
                                                bdl_BalanceLng=0-(bdl_BalanceLng);
                                                 bdl_Balance=String.valueOf(bdl_BalanceLng);
                                                index1 = responseStr.indexOf("<AcctResName>");
                                                String accountResName=responseStr.substring(index1+"<AcctResName>".length(),responseStr.indexOf("</AcctResName>",index1));

                                                        index1 = responseStr.indexOf("<ExpDate>");
                                                        String expDate=responseStr.substring(index1+"<ExpDate>".length(),responseStr.indexOf("</ExpDate>",index1));
						...
						long    bdl_BalanceLng = 0;
                                        ...
                                        if(index!=-1)
                                        {
                                                ...
                                                String accountResName=responseStr.substring(index1+"<AcctResName>".length(),responseStr.indexOf("</AcctResName>",index1));
                                                        ...
                                                        responseMap.put("resp_accountResName",accountResName);
                                                        responseMap.put("resp_expDate",expDate);
														responseMap.put("resp_Balance",bdl_Balance);		
         ...
         private String generateImmediateDebitOrCreditAdjustRequest(HashMap<String,String> p_requestMap) throws Exception
         {
                ...
                try
                {
                    ...
                    double transAmtDbl=0;
                        ...
                        sbf.append("<Data>");
                        sbf.append("<header>");
                        sbf.append("<ACTION_ID>DeductFee</ACTION_ID>");
                sbf.append("<REQUEST_ID>"+p_requestMap.get("IN_REQ_ID")+"</REQUEST_ID>");
                sbf.append("</header>");
                sbf.append("<body>");
                sbf.append("<TransactionSN>"+p_requestMap.get("IN_RECON_ID")+"</TransactionSN>");
                ...
                /*sbf.append("<AcctResCode>"+FileCache.getValue(_interfaceId,senderDebitID)+"</AcctResCode>");
                    ...
                    sbf.append("<AddDays>0</AddDays>");*/
                        for(String bdl_data : p_requestMap.get("budle_balance_deduction").split("\\|")){
                                transAmtDbl=Double.parseDouble(bdl_data.split(":")[0]);
                                /*if(action==CP2PDataI.ACTION_IMMEDIATE_DEBIT)
                                        transAmtDbl=0-transAmtDbl;*/
        ...
        public HashMap<String,String> parseImmediateDebitResponse(String p_responseStr) throws Exception
        {
            ...
            if(_log.isDebugEnabled())
                    ...
                    if(_str.equals((returnCode)))
                {
                        ...
                        index=p_responseStr.lastIndexOf("<AfterBalance>");
                        if(index!=-1)
                        {
                                String grossBalance=p_responseStr.substring(index+"<AfterBalance>".length(),p_responseStr.indexOf("</AfterBalance>",index));
                     ...
                     /*   index=p_responseStr.lastIndexOf("<EffDate>", end);
                        ...
                        index=p_responseStr.lastIndexOf("<ExpDate>");
                        if(index!=-1)
                        {
                        ...
                        index=p_responseStr.lastIndexOf("<AccountResName>");
                        if(index!=-1)
                        {
                                String accountResName=p_responseStr.substring(index+"<AccountResName>".length(),p_responseStr.indexOf("</AccountResName>",index));
                                responseMap.put("resp_accountResName",accountResName);
                        }
                        index=p_responseStr.indexOf("<BeforeBalance>");
                        if(index!=-1)
                        {
                                String beforeBal=p_responseStr.substring(index+"<BeforeBalance>".length(),p_responseStr.indexOf("</BeforeBalance>",index));
                                long beforeBalLong=Long.parseLong(beforeBal);
                                beforeBalLong=0-(beforeBalLong);
                                beforeBal=String.valueOf(beforeBalLong);
                                responseMap.put("resp_afterBalance",beforeBal);
                        }
...
long balance = Long.parseLong(bdl_Balance);
                                                        if(balance<0)
                                                                balance = 0- balance;

                                                        bdl_Balance = String.valueOf(balance);
