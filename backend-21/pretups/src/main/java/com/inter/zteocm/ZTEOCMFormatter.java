 ...
 *                          ----------------------------------------------------
 *                          ---------------------------------------------
 *                          Shashank Shukla June 20, 2012 Initial Creation
 *                          ----------------------------------------------------
 *                          --------------------------------------------
 */
...
public class ZTEOCMFormatter {
    private static Log _log = LogFactory.getLog(ZTEOCMFormatter.class.getName());
     ...
     * Constructor
     */
    public ZTEOCMFormatter() {
        super();
        // TODO Auto-generated constructor stub
    }

    protected String generateRequest(int action, HashMap<String, String> p_map) throws Exception {
    ...
    public HashMap<String, String> parseGetAccountInfoResponse(String responseStr) throws Exception {
        ...
        try {
            ...
            if (_str.equals(returnCode)) {
                ...
                index = responseStr.indexOf("<BrandIndex>");
                if (index != -1) {
                    String resp_brandIndex = responseStr.substring(index + "<BrandIndex>".length(), responseStr.indexOf("</BrandIndex>", index));
                    map.put("resp_brandIndex", resp_brandIndex);
                }
    ...
    private String generateRechargeCreditRequest(HashMap<String, String> p_map) throws Exception {
        ...
        try {
            ...
            sbf.append("<Data>");
            sbf.append("<header>");
            sbf.append("<ACTION_ID>RechargeByCash</ACTION_ID>");
            sbf.append("<REQUEST_ID>" + (String) p_map.get("IN_REQ_ID") + "</REQUEST_ID>");
            sbf.append("</header>");
            sbf.append("<body>");
            sbf.append("<TransactionSN>" + (String) p_map.get("IN_RECON_ID") + "</TransactionSN>");
            ...
            // String
            // addMainBnsVal=(String)FileCache.getValue(_interfaceId,"ADD_MAIN_AND_BUNUS_VALIDITY").trim();
            ...
            if (transAmtDbl != 0)
                ...
                sbf.append("<AddBalance>0</AddBalance>");
            ...
            // sbf.append("<BenefitDtoList>");
            sbf.append(getBundleRequestString(p_map));
            // sbf.append("</BenefitDtoList>");
            // end of bussinesslogic
             ...
             * String addMainBnsVal=(String)FileCache.getValue(_interfaceId,
             * "ADD_MAIN_AND_BUNUS_VALIDITY").trim();
             * String bonusValidity=(String)p_map.get("BONUS_VALIDITY_DAYS");
             ...
             * sbf.append("<header>");
             * sbf.append("<ACTION_ID>RechargeByCash</ACTION_ID>");
             * sbf.append("<REQUEST_ID>"+(String)p_map.get("IN_REQ_ID")+
             * "</REQUEST_ID>");
             * sbf.append("</header>");
             * sbf.append("<body>");
             * sbf.append("<MSISDN>"+InterfaceUtil.getFilterMSISDN((String)
             * p_map
             * .get("INTERFACE_ID"),(String)p_map.get("MSISDN"))+"</MSISDN>");
             * sbf.append("<TransactionSN>"+(String)p_map.get("IN_RECON_ID")+
             * "</TransactionSN>");
             * sbf.append("<AccountCode></AccountCode>");
             ...
             * Object[]
             * ambList=((String)p_map.get("BONUS_BUNDLE_IDS")).split("\\|");
             * //If combined re-charge, and selector value is other than 1
             * if(!"1".equals(cardGrpSelector) &&
             * !InterfaceUtil.isNullString((String
             * )p_map.get("BONUS_BUNDLE_IDS")))
             * {
             * if(Arrays.asList(ambList).contains("1"))
             * sbf.append(getBundleRequestString(p_map));
             * }
             * if(_combinedRecharge && "1".equals(cardGrpSelector))
             * {
             * sbf.append("<AcctResCode>"+FileCache.getValue(_interfaceId,
             * _selectorBundleId)+"</AcctResCode>");
             * //If transfer amount is not zero, then set it in to the request
             * string.
             * if(transAmtDbl!=0)
             * sbf.append("<AddBalance>"+String.valueOf(Math.round(transAmtDbl))+
             * "</AddBalance>");
             * else
             ...
             * else
             * {
             * sbf.append("<AcctResCode>1</AcctResCode>");
             * sbf.append("<AddBalance>0</AddBalance>");
             * sbf.append("<AddDays>0</AddDays>");
             * sbf.append("<OperationStaff></OperationStaff>");
             * }
    ...
    private String generateCreditAdjustRequest(HashMap<String, String> p_map) throws Exception {
        ...
        try {
            ...
            _cardGrp = (String) p_map.get("CARD_GROUP");
            String combined = (String) p_map.get("COMBINED_RECHARGE");
            String implicit = (String) p_map.get("EXPLICIT_RECHARGE");
            if (_log.isDebugEnabled())
                _log.debug("generateCreditAdjustRequest", "Checking" + "EX " + _explicitRecharge + "COM" + _combinedRecharge + "IMP" + _implicitRecharge);
            if (_log.isDebugEnabled())
                _log.debug("generateCreditAdjustRequest", "Checking  " + implicit + " " + combined);
            if ("N".equals(implicit) && "N".equals(combined))
                _explicitRecharge = true;
            else if ("Y".equals(implicit) && "Y".equals(combined))
                _combinedRecharge = true;
            ...
            sbf.append("<FromAccountCode></FromAccountCode>");
            sbf.append("<FromUserPwd></FromUserPwd>");
            ...
            sbf.append("<ToAccountCode></ToAccountCode>");
             ...
             * if(_explicitRecharge || _combinedRecharge)
             * {
             * if(_log.isDebugEnabled())_log.debug("generateCreditAdjustRequest",
             * "Entered IF Block  "+_explicitRecharge + _combinedRecharge);
             ...
             * if(_combinedRecharge && !"1".equals(cardGrpSelector) &&
             * !InterfaceUtil
             * .isNullString((String)p_map.get("BONUS_BUNDLE_IDS")))
             * {
             * Object[]
             * ambList=((String)p_map.get("BONUS_BUNDLE_IDS")).split("\\|");
             * if(Arrays.asList(ambList).contains("1"))
             ...
             * {
             * sbf.append("<BalInputDtoList>");
             * //sbf.append("<AcctResCode>0</AcctResCode>");
             * sbf.append("<AcctResCode>"+FileCache.getValue(_interfaceId,
             * _selectorBundleId)+"</AcctResCode>");
             * sbf.append("<AcctResName></AcctResName>");
             * sbf.append("<Balance></Balance>");
    ...
    public HashMap<String, String> parseImmediateDebitResponse(String responseStr) throws Exception {
        ...
        try {
            ...
            if (_str.equals((returnCode))) {
                ...
                // Change By Babu Kunwar as AccoutnResCode is 0 for Live Server
                int end = responseStr.indexOf("<AcctResCode>0</AcctResCode>");// 0
                                                                              // is
                                                                              // for
                                                                              // main
                                                                              // account
    ...
    private String getBundleRequestString(HashMap<String, String> p_map) throws Exception {
        ...
        try {
            if (!InterfaceUtil.isNullString((String) p_map.get("BONUS_BUNDLE_IDS"))) {
                ...
                sbfBundle.append("<BenefitDtoList>");
                for (int i = 0; i < bonusBundleCount; i++) {
                    ...
                    if (bundleValueDbl > 0 || Long.parseLong(bundleValidity) > 0) {
                        ...
                        bundleValueDbl = 0 - bundleValueDbl;

                        if (!bundleCode.equals("1") && _explicitRecharge) {

                            // set the bundle parameters in to the request
                            // string.
                            // sbfBundle.append("<BenefitDtoList>");
                            ...
                            if ("UNIT".equals(bundleTypes[i]))
                                sbfBundle.append("<AddBalance>" + String.valueOf(Math.round(inbundleValueDbl)) + "</AddBalance>");
                            else
                                sbfBundle.append("<AddBalance>" + String.valueOf(Math.round(bundleValueDbl)) + "</AddBalance>");
            ...
            e.printStackTrace();
            _log.error("ZTEOBOFormatter[getBundleRequestString]", "Exception e::" + e.getMessage());
            throw e;
        } finally {
            if (_log.isDebugEnabled())
    ...
    private String getINRequestID(HashMap<String, String> p_map) throws Exception {
        ...
        try {
             ...
             */
            getServiceType = (String) p_map.get("MODULE");
            if (!BTSLUtil.isNullString(getServiceType)) {
                if (getServiceType.equalsIgnoreCase("C2S"))
                    reqId = (String) p_map.get("CHANNEL_ID_C2S") + dateStrReqId + timeStrReqTime + Constants.getProperty("INSTANCE_ID") + counter;
                else
                    reqId = (String) p_map.get("CHANNEL_ID_P2P") + dateStrReqId + timeStrReqTime + Constants.getProperty("INSTANCE_ID") + counter;
            }
            /*
            ...
            e.printStackTrace();
            _log.error("ZTEOBOFormatter[getINRequestID]", "Exception e::" + e.getMessage());
            throw e;
     ...
     * @return void
     */
    private boolean isBundleAllowed(String p_bundleName, HashMap<String, String> p_requestMap) throws Exception {
        if (_log.isDebugEnabled())
     ...
     */
    private String getZTEINTransactionID(HashMap<String, String> p_map) {
        if (_log.isDebugEnabled())
