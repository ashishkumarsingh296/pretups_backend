 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class HuaweiEVRDBUtility implements ClientMarkerI {

    private static Log _log = LogFactory.getLog(HuaweiEVRDBUtility.class.getName());
     ...
     */
    public HuaweiEVRDBUtility(String p_interfaceId) throws BTSLBaseException {

        if (_log.isDebugEnabled())
            _log.debug("HuaweiEVRDBUtility[constructor]", "Entered p_interfaceId:" + p_interfaceId);
        try {
            _interfaceID = p_interfaceId;
            String db_url = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_CONN_URL");
            if (InterfaceUtil.isNullString(db_url)) {
                _log.error("HuaweiEVRDBUtility[constructor]", "HUAWEIEVR_DB_CONN_URL is not defined in the INFile with INTERFACE_ID:" + _interfaceID);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_CONN_URL is not defined in IN File");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
            }
            db_url = db_url.trim();

            String db_user = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_USERNAME");
            if (InterfaceUtil.isNullString(db_user)) {
                _log.error("HuaweiEVRDBUtility[constructor]", "HUAWEIEVR_DB_USERNAME is not defined in the INFile with INTERFACE_ID:" + _interfaceID);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_USERNAME is not defined in IN File");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
            }
            db_user = db_user.trim();
            // db_user = new CryptoUtil().decrypt(db_user,Constants.KEY);
            // String db_password = Constants.getProperty("passwd");
            String db_password = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_PASS");
            if (InterfaceUtil.isNullString(db_password)) {
                _log.error("HuaweiEVRDBUtility[constructor]", "HUAWEIEVR_DB_PASS is not defined in the INFile with INTERFACE_ID:" + _interfaceID);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_PASS is not defined in IN File");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
            }
            ...
            String strMinPoolSize = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_MINPOOLSIZE");
            if (InterfaceUtil.isNullString(strMinPoolSize)) {
                _log.error("HuaweiEVRDBUtility[constructor]", "HUAWEIEVR_DB_MINPOOLSIZE is not defined in the INFile with INTERFACE_ID:" + p_interfaceId);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_MINPOOLSIZE is not defined in IN File");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
            }
            strMinPoolSize = strMinPoolSize.trim();
            // String strPoolSize=Constants.getProperty("poolsize");
            String strPoolSize = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_MAXPOOLSIZE");
            if (InterfaceUtil.isNullString(strPoolSize)) {
                _log.error("HuaweiEVRDBUtility[constructor]", "HUAWEIEVR_DB_MAXPOOLSIZE is not defined in the INFile with INTERFACE_ID:" + p_interfaceId);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_MAXPOOLSIZE is not defined in IN File");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
            }
            strPoolSize = strPoolSize.trim();
            ...
            String oraclePoolLogFile = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_POOL_LOG_FILE");
            if (_log.isDebugEnabled())
            ...
            if (_log.isDebugEnabled())
                _log.debug("HuaweiEVRDBUtility[constructor]", "userid=" + db_user + ",  db_url=" + db_url + ", minPoolSize=" + minPoolSize + " max poolSize=" + poolSize + " oraclePoolLogFile=" + oraclePoolLogFile);
            try {
                if (db_url != null) {
                ...
                e.printStackTrace();
                _log.error("HuaweiEVRDBUtility[constructor]", "Exception e:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "While initialization of DB Connection Pool for interface id=" + _interfaceID + " get Exception e:" + e.getMessage());
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
            ...
            e.printStackTrace();
            _log.error("HuaweiEVRDBUtility[constructor]", "Exception e:" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[constructor]", "", "", " INTERFACE ID = " + _interfaceID, "While initialization of DB Connection Pool for interface id=" + p_interfaceId + " get Exception e:" + e.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
        } finally {
            if (_log.isDebugEnabled())
                _log.debug("HuaweiEVRDBUtility[constructor]", "Exited m_connectionPool:" + _connectionPool);
        }
    }
    ...
    public Connection getConnection() throws BTSLBaseException {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.DATABASE_CONECTION_PROBLEM, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[getConnection]", "", "", "", "Database Connection Problem, getting Exception ex:" + ex.getMessage());
    ...
    public Connection getSingleConnection() throws BTSLBaseException {
        ...
        try {
            if (_connectionSinglePool == null) {
                String db_url = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_CONN_URL");
                if (InterfaceUtil.isNullString(db_url)) {
                    _log.error("getSingleConnection", "HUAWEIEVR_DB_CONN_URL is not defined in the INFile with INTERFACE_ID:" + _interfaceID);
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[getSingleConnection]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_CONN_URL is not defined in IN File");
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
                }
                db_url = db_url.trim();
                String db_user = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_USERNAME");
                if (InterfaceUtil.isNullString(db_user)) {
                    _log.error("getSingleConnection", "HUAWEIEVR_DB_USERNAME is not defined in the INFile with INTERFACE_ID:" + _interfaceID);
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[getSingleConnection]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_USERNAME is not defined in IN File");
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
                }
                db_user = db_user.trim();
                ...
                String db_password = FileCache.getValue(_interfaceID, "HUAWEIEVR_DB_PASS");
                if (InterfaceUtil.isNullString(db_password)) {
                    _log.error("getSingleConnection", "HUAWEIEVR_DB_PASS is not defined in the INFile with INTERFACE_ID:" + _interfaceID);
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[getSingleConnection]", "", "", " INTERFACE ID = " + _interfaceID, "HUAWEIEVR_DB_PASS is not defined in IN File");
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
                }
                db_password = db_password.trim();
                // Confrim whether to user encrypted password or plain, if yest
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiEVRDBUtility[getSingleConnection]", "", "", " INTERFACE ID = " + _interfaceID, "Exception occurs while getting the single database connection, Exception ex:" + ex.getMessage());
            throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_DB_CON_INITILIZATION);
        }// end of catch
        finally {
            if (_log.isDebugEnabled())
