 ...
 *                                ----------------------------------------------
 *                                -----
 *                                Harpreet Kaur June 18, 2012 Initial Creation
 *                                ----------------------------------------------
 *                                ----------------------------------------------
 *                                ----
 *                                This class can be used as a parser class for
 *                                both request(before sending the request to IN)
 *                                and
 *                                response(after getting the response from the
 *                                IN).
 */
...
public class ZTEOGNReqResFormatter {
    private static Log _log = LogFactory.getLog(ZTEOGNReqResFormatter.class.getName());
    ...
    protected String generateRequest(int action, HashMap<String, String> p_requestMap) throws Exception {
        ...
        // else
        // _selectorBundleId=p_requestMap.get("CARD_GROUP_SELECTOR").trim();

        else if (!(InterfaceUtil.isNullString(p_requestMap.get("CARD_GROUP_SELECTOR"))))
            _selectorBundleId = p_requestMap.get("CARD_GROUP_SELECTOR").trim();
        else
            _selectorBundleId = "1";
        ...
        switch (action) {
        case ZTEOGNI.ACTION_ACCOUNT_INFO: {
            requestStr = generateGetAccountInfoRequest(p_requestMap);
            break;
        }
        case ZTEOGNI.ACTION_RECHARGE_CREDIT: {
        ...
        case ZTEOGNI.ACTION_IMMEDIATE_DEBIT: {
    ...
    private String generateCreditAdjustRequest(HashMap<String, String> p_requestMap) throws Exception {
        ...
        try {
            ...
            if ("S".equals(p_requestMap.get("USER_TYPE"))) {
                ...
                sbf.append("<AcctResCode>" + _selectorBundleId + "</AcctResCode>");
                sbf.append("<AcctResName></AcctResName>");
                sbf.append("<Balance></Balance>");
                sbf.append("<AddBalance>" + String.valueOf(Math.round(transAmtDbl)) + "</AddBalance>");
    ...
    private String generateImmediateDebitRequest(HashMap<String, String> p_requestMap) throws Exception {
        ...
        try {
            ...
            senderDebitID = "DEBIT_" + p_requestMap.get("CARD_GROUP_SELECTOR").trim();
            getINRequestID(p_requestMap);
            sbf = new StringBuffer(1024);
            sbf.append("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
            sbf.append("<zsmart>");
            ...
            sbf.append("<AcctResCode>" + p_requestMap.get("CARD_GROUP_SELECTOR").trim() + "</AcctResCode>");
            sbf.append("<AcctResName></AcctResName>");
            sbf.append("<Balance></Balance>");
            sbf.append("<AddBalance>" + String.valueOf(Math.round(transAmtDbl)) + "</AddBalance>");
            sbf.append("<ExpDate></ExpDate>");
    ...
    public HashMap<String, String> parseImmediateDebitResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (_str.equals((returnCode))) {
                ...
                int end = p_responseStr.indexOf("<AccountResCode>1</AccountResCode>");// 1
                                                                                      // is
                                                                                      // for
                                                                                      // main
                                                                                      // account
                ...
                index = p_responseStr.lastIndexOf("<Balance>", end);
                if (index != -1) {
                ...
                index = p_responseStr.lastIndexOf("<ExpDate>", end);
                if (index != -1) {
                ...
                index = p_responseStr.lastIndexOf("<AccountResCode>", end);
                if (index != -1) {
                ...
                index = p_responseStr.lastIndexOf("<AccountResName>", end);
                if (index != -1) {
                    String accountResName = p_responseStr.substring(index + "<AccountResName>".length(), p_responseStr.indexOf("</AccountResName>", index));
        ...
        try {
            if (!InterfaceUtil.isNullString(p_requestMap.get("BONUS_BUNDLE_IDS"))) {
                ...
                for (int i = 0; i < bonusBundleCount; i++) {
                    // String
                    // bundleCode=FileCache.getValue(_interfaceId,bundleCodes[i]);
            ...
            e.printStackTrace();
            _log.error("ZTEOGNReqResFormatter[getBundleRequestString]", "Exception e::" + e.getMessage());
            throw e;
        } finally {
            if (_log.isDebugEnabled())
            ...
            if (index != -1) {
                ...
                for (int i = 0; i < noOfBundles; i++) {
                    ...
                    if (index != -1) {
                        ...
                        System.out.println("bdl_code....." + bdl_code);
                        if (bdl_code.equalsIgnoreCase(_selectorBundleId)) {
    ...
    private void getBundlesInfoRcvdFromIN4Credit(String p_responseStr, HashMap<String, String> p_requestMap) throws Exception {
        ...
        try {
            ...
            index = p_responseStr.indexOf("</BalDto>");
            System.out.println("getBundlesInfoRcvdFromIN4Credit :index....." + index);
            if (index != -1) {
                String[] bundleArray = subStr.split("/BalDto>");
                noOfBundles = bundleArray.length;
                System.out.println("getBundlesInfoRcvdFromIN4Credit :noOfBundles....." + noOfBundles);
                for (int i = 0; i < noOfBundles; i++) {
                    // get the bdl_name if present in the bucket.
                    index = bundleArray[i].indexOf("<AcctResCode>");
                    if (index != -1) {
                        bdl_code = bundleArray[i].substring(index + "<AcctResCode>".length(), bundleArray[i].indexOf("</AcctResCode>", index));
                        System.out.println("getBundlesInfoRcvdFromIN4Credit :bdl_code....." + bdl_code);
    ...
    private String getINRequestID(HashMap<String, String> p_map) throws Exception {
        ...
        // 6 bit sequence number is required to generate the IN Request ID.
        int inTxnLength = 6;
        try {
            Date mydate = new Date();
            sdfReqId = new SimpleDateFormat("yyyyMMdd");
            ...
            reqId = p_map.get("CHANNEL_ID") + dateStrReqId + Constants.getProperty("INSTANCE_ID") + counter;
            ...
            e.printStackTrace();
            _log.error("ZTEOGNReqResFormatter[getINRequestID]", "Exception e=" + e.getMessage());
            throw e;
     ...
     */
    /*
     * private boolean isBundleAllowed(String p_bundleCode,
     * HashMap<String,String> p_requestMap) throws Exception
     * {
     * 
     * if(_log.isDebugEnabled())
     ...
     * return isAllowed;
     * //Check whether bonus bundle code is received from the IN in validate
     * request or not.
     * String bundleFromIN=p_requestMap.get("IN_RESP_BUNDLE_CODES");
     * if(!InterfaceUtil.isNullString(bundleFromIN) &&
     * bundleFromIN.contains(p_bundleCode))
     * isAllowed=true;
     * return isAllowed;
     * }
     * catch(Exception e)
     ...
     * {
     * if(_log.isDebugEnabled())
     * _log.debug("isBundleAllowed","Exiting: Is "+p_bundleCode+
     * " is allow on the IN="+ isAllowed);
     * }
     * }
     */
}
