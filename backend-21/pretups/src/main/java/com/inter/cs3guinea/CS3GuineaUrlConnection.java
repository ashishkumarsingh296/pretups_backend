 ...
 *                                 --------------------------------------------
 *                                 ------
 *                                 This class will give an instance of URL
 *                                 connection with all the header parameters
 *                                 customized for CS3CP5.
 */
public class CS3GuineaUrlConnection extends BTSLConnection {
    private HttpURLConnection _urlConnection = null;

    public CS3GuineaUrlConnection(String p_url, String p_username, String p_password, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength, String p_hostName, String p_userAgent) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("CS3GuineaUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive + " p_hostName=" + p_hostName + " p_userAgent=" + p_userAgent);
        try {
            ...
            e.printStackTrace();
            _log.error("CS3GuineaUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("CS3GuineaUrlConnection", "Exiting");
    }
