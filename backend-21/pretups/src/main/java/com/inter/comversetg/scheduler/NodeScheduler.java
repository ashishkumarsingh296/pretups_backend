package com.inter.comversetg.scheduler;
 ...
 *                   ----------------------------------------------------------
 *                   ---------------------------------------
 *                   Ashish Kumar Sep 11,2006 Initial Creation
 *                   ----------------------------------------------------------
 *                   --------------------------------------
...
public class NodeScheduler {
    ...
    private Hashtable _nodeTable = null;// Conatins the node number as Key and
                                        // VO as nod detail
     ...
     * @throws Exception
     */
    public NodeScheduler(String p_comverseINId) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("NodeScheduler[constructor]", "Entered p_comverseINId::" + p_comverseINId);
        String url = null;
        ...
        String password = "";
        NodeVO comverseNodeVO = null;// Contains all the detail of node
        ...
        try {
            ...
            _interfaceID = p_comverseINId;
            try {
                _totalNodes = Integer.parseInt(FileCache.getValue(p_comverseINId, "MAX_NODE").trim());
                if (_log.isDebugEnabled())
            ...
            try {
                _maxAllowedNum = Integer.parseInt(FileCache.getValue(p_comverseINId, "MAX_ALWD_NO").trim());
                if (_log.isDebugEnabled())
            ...
            try {
                _retryNum = Integer.parseInt(FileCache.getValue(p_comverseINId, "RETRY_NUM").trim());
                if (_log.isDebugEnabled())
            ...
            for (int l = 1; l <= _totalNodes; l++) {
                try {
                    ...
                    comverseNodeVO = new NodeVO();
                    // Set the Node number as key.
                    comverseNodeVO.setNodeNumber(l);
                    url = FileCache.getValue(p_comverseINId, "URL_" + l);
                    if (_log.isDebugEnabled())
                        _log.debug("NodeScheduler[constructor]", "url_" + l + "::" + url);
                    if (InterfaceUtil.isNullString(url)) {
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of URL_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }
                    comverseNodeVO.setUrl(url.trim());

                    try {
                        String ReadTimeOutStr = FileCache.getValue(p_comverseINId, "READ_TIMEOUT_" + l).trim();
                        readTimeOut = Integer.parseInt(ReadTimeOutStr);
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "READ_TIMEOUT_" + l + "::" + readTimeOut);
                        comverseNodeVO.setReadTimeOut(readTimeOut);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of READ_TIMEOUT_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        String WssdFile = FileCache.getValue(p_comverseINId, "WSDD_LOCATION_" + l).trim();
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "WSDD_LOCATION_" + l + "::" + readTimeOut);
                        comverseNodeVO.setWssdFileLoc(WssdFile);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of WSDD_LOCATION_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        String soapuri = FileCache.getValue(p_comverseINId, "SOAP_ACTION_URI_" + l).trim();
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "SOAP_ACTION_URI_" + l + "::" + readTimeOut);
                        comverseNodeVO.setSoapUri(soapuri);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of SOAP_ACTION_URI_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        String pwback = FileCache.getValue(p_comverseINId, "PWCALLBACK_" + l).trim();
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "PWCALLBACK_" + l + "::" + readTimeOut);
                        comverseNodeVO.setPwbackCall(pwback);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of PWCALLBACK_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        String warnTimeStr = FileCache.getValue(p_comverseINId, "WARN_TIME_" + l).trim();
                        warnTime = Long.parseLong(warnTimeStr);
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "WARN_TIME_" + l + "::" + warnTimeStr);
                        comverseNodeVO.setWarnTime(warnTime);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of WARN_TIME_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }
                    try {
                        userName = FileCache.getValue(p_comverseINId, "USER_NAME_" + l).trim();
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "USER_NAME_" + l + "::" + userName);
                        comverseNodeVO.setUserName(userName);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of USER_NAME_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        password = FileCache.getValue(p_comverseINId, "PASSWORD_" + l).trim();
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "PASSWORD_" + l + "::" + password);
                        comverseNodeVO.setPassword(password);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of PASSWORD_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        String expiryTimeStr = FileCache.getValue(p_comverseINId, "EXPIRY_TIME_" + l).trim();
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "EXPIRY_TIME_" + l + "::" + expiryTimeStr);
                        expiryTime = Long.parseLong(expiryTimeStr);
                        comverseNodeVO.setExpiryDuration(expiryTime);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of EXPIRY_TIME_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }

                    try {
                        int maxBarredCount = Integer.parseInt(FileCache.getValue(p_comverseINId, "MAX_BARRED_COUNT_" + l).trim());
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "MAX_BARRED_COUNT_" + l + "::" + maxBarredCount);
                        comverseNodeVO.setMaxBarredCount(maxBarredCount);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of MAX_BARRED_COUNT_" + l + " in INFile " + p_comverseINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                    }
                     ...
                     * {
                     * maxConPerNode =
                     * Integer.parseInt(FileCache.getValue(p_cs3INId
                     * ,"MAX_CON_NODE_"+l).trim());
                     * if(_log.isDebugEnabled())
                     * _log.debug("NodeScheduler[constructor]"
                     * ,"MAX_CON_NODE_"+l+"::"+maxConPerNode);
                     ...
                     * EventLevelI.FATAL, "NodeScheduler[constructor]",
                     * "INTERFACE_ID::"+_interfaceID,"", "",
                     * "check the value of MAX_CON_NODE_"
                     * +l+" for interface id p_cs3INId ::"+p_cs3INId);
                     * throw new BTSLBaseException(InterfaceErrorCodesI.
                     * ERROR_COMVERSE_NODE_INITIALIZATION);
                     * }
                     */

                    _nodeTable.put(String.valueOf(l), comverseNodeVO);
                } catch (BTSLBaseException be) {
                    _log.error("NodeScheduler[constructor]", "BTSLBaseException be::" + be.getMessage());
                    throw be;
                } catch (Exception e) {
                    e.printStackTrace();
                    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Exception while initializing the Node details of interfaceID = " + p_comverseINId + " Exception e=" + e.getMessage());
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_INITIALIZATION);
                }
            }
        } catch (BTSLBaseException be) {
    ...
    public synchronized NodeVO getNodeVO(String p_transId) throws BTSLBaseException {
        ...
        try {
            ...
            if (nodeNumStr == null) {
                ...
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_NODE_SCHEDULING);
            }
            comverseNodeVO = (NodeVO) _nodeTable.get(nodeNumStr);
            ...
            if (_log.isDebugEnabled())
                _log.debug("getNodeVO", p_transId, "Exited comverseNodeVO::" + comverseNodeVO);
        }// end of finally
        return comverseNodeVO;
    }// end of getNodeVO
    ...
    private synchronized String getNode(String p_transId) throws BTSLBaseException {
        ...
        try {
            ...
            // currentNode.
            comverseNodeVO = (NodeVO) _nodeTable.get(String.valueOf(_nodeNumber));
            ...
            while (isNext) {
                try {
                    if (tempCount > _maxAllowedNum) {
                        ...
                        // throw new
                        // BTSLBaseException(InterfaceErrorCodesI.ERROR_COMVERSE_MAX_NODE_CHECK_REACH);
                    ...
                    if (!comverseNodeVO.isBlocked()) {

                        if (comverseNodeVO.getBarredCount() >= comverseNodeVO.getMaxBarredCount()) {
                            comverseNodeVO.setBlocked(true);
                            comverseNodeVO.setBlokedAt(System.currentTimeMillis());
                            nodeNumStr = String.valueOf(_nodeNumber);
                            incrementNodeNumber();
                            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "NodeScheduler[getNode]", "INTERFACE_ID::" + _interfaceID, "", "p_transId::" + p_transId, "CCWS IP=" + comverseNodeVO.getUrl() + " is not responding and blocked");
                            continue;
                        }
                    ...
                    if (isExpired(comverseNodeVO.getBlokedAt(), comverseNodeVO.getExpiryDuration())) {
                        ...
                        incrementNodeNumber();
                        // connIncremented=true;
                        comverseNodeVO.setBlocked(false);
                        // Initialize the block time if it is expired.
                        comverseNodeVO.setBlokedAt(0);
                        comverseNodeVO.setBarredCount(0);
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "NodeScheduler[getNode]", "INTERFACE_ID::" + _interfaceID, "", "p_transId::" + p_transId, "CCWS IP=" + comverseNodeVO.getUrl() + " is unblocked");
                        break;
                    }
