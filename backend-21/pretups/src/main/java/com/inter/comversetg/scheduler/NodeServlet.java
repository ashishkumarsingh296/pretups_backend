 ...
 *                 ------------------------------------------------------------
 *                 -------------------------------------
 *                 Ashish Kumar Sep 12, 2006 Initial Creation
 *                 ------------------------------------------------------------
 *                 ------------------------------------
...
public class NodeServlet extends HttpServlet {
    ...
    public void initialize() {
        ...
        String comverseINIDs = null;
        try {
            // Fetch the Scheduler IN id's from the constant props
            comverseINIDs = Constants.getProperty("COMVERSE_IN_IDS");
            if (_log.isDebugEnabled())
                _log.debug("initialize", "comverseINIDs = " + comverseINIDs);
            // If ids are defined in constant props,invoke the initialize method
            // of NodeManager
            // if(schINIDs!=null)
            if (!BTSLUtil.isNullString(comverseINIDs))
                NodeManager.initialize(comverseINIDs);
        } catch (BTSLBaseException be) {
            _log.error("initialize", "BTSLBaseException be::" + be.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("initialize", "Exception e::" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, comverseINIDs, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "NodeServlet[initialize]", "", "", "", "Exception while initialize IN Scheduler Parameters Exception:" + e.getMessage());
