...
public class VOMSHandler implements InterfaceHandler {
    ...
    public void validate(HashMap p_map) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.PRIVATE_RECHARGE_ALLOWED))).booleanValue())
            	profileMap=vomsDAO.loadActiveProfilesForPrivateRecharge(con,networkCode,currDate,timeStmpChk,PretupsI.SERVICE_TYPE_CHNL_EVD);
            else
            	profileMap=vomsDAO.loadActiveProfiles(con,networkCode,currDate,timeStmpChk,PretupsI.SERVICE_TYPE_CHNL_EVD);
            ...
            ArrayList voucherList = VOMSVoucherDAO.loadPINAndSerialNumber(con, productVO, modifiedBy, currDate, statusChangeSource, currentStatus, transferIdList, (String) _requestMap.get("NETWORK_CODE"), 1);
    ...
    public void credit(HashMap p_map) throws BTSLBaseException {
        ...
        try {
            ...
            String modifiedBy = (String) _requestMap.get("SENDER_USER_ID");
            String statusChangeSource = (String) _requestMap.get("SOURCE");
            String recMSISDN = (String) _requestMap.get("MSISDN");
            ...
            vomsVO.setFirstConsumedBy(recMSISDN);
            VOMSVoucherDAO vomsDAO = new VOMSVoucherDAO();
    ...
    public void debit(HashMap p_map) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            vomsVO.setUserLocationCode((String) _requestMap.get("NETWORK_CODE"));
            VOMSVoucherDAO vomsDAO = new VOMSVoucherDAO();
            if (VOMSVoucherDAO.updateVoucherStatusRollback(con, vomsVO) > 0) {
                if (vomsDAO.insertDetailsInVoucherAudit(con, vomsVO) > 0)
                	mcomCon.finalCommit();
                else {
                	mcomCon.finalRollback();;
            ...
            _log.error("debit", "Exception e:" + e.getMessage() + InterfaceUtil.getPrintMap(_requestMap));
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "VOMSHandler[debit]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while updating voucher =" + _requestMap.get("SERIAL_NUMBER") + " status to " + _requestMap.get("UPDATE_STATUS") + " for Interface=" + _requestMap.get("INTERFACE_ID") + " ");
            throw e;
        } finally {
        	if(mcomCon != null)
        	{
        		mcomCon.close("VOMSHandler#debit");
        		mcomCon=null;
        		}
            if (_log.isDebugEnabled())
    ...
    public void debitAdjust(HashMap p_map) throws BTSLBaseException, Exception {
        if (_log.isDebugEnabled())
            _log.debug("debitAdjust Entered ", " _requestMap: " + _requestMap);
        Connection con = null;
        MComConnectionI mcomCon = null;
        try {
            _requestMap = p_map;
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "VOMSHandler[debitAdjust]", _referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while updating voucher =" + _requestMap.get("SERIAL_NUMBER") + " status to " + _requestMap.get("UPDATE_STATUS") + " for Interface=" + _requestMap.get("INTERFACE_ID") + " ");
            throw e;
        } finally {
        	if(mcomCon != null)
        	{
        		mcomCon.close("VOMSHandler#debitAdjust");
        		mcomCon=null;
        		}
            if (_log.isDebugEnabled())
