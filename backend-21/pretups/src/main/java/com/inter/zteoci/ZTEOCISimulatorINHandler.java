...
public class ZTEOCISimulatorINHandler implements InterfaceHandler {
    private Log _log = LogFactory.getLog(ZTEOCISimulatorINHandler.class.getName());
     ...
     * Constructor
     */
    public ZTEOCISimulatorINHandler() {
    ...
    public void debitAdjust(HashMap p_requestMap) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            // value of INTERFACE_AMOUNT)
            _requestMap.put("transfer_amount", amountStr);
            String requestSre = _formatter.generateRequest(ZTEOCII.ACTION_IMMEDIATE_DEBIT, _requestMap);
            // sending the Re-charge request to IN along with re-charge action
            // defined in SafcomI interface
            sendRequestToIN(ZTEOCII.ACTION_IMMEDIATE_DEBIT, requestSre);
            // set TRANSACTION_STATUS as Success in request map
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
            // Set the post balance after multiplied by multiplication factor.
            try {
        ...
        try {
            ...
            try {
                ...
                try(FileInputStream fileInputStream = new FileInputStream(file)) {
                    ...
                    if (p_action == ZTEOCII.ACTION_ACCOUNT_INFO) {
                        responseStr = properties.getProperty("ACCOUNT_INFO");
                        try {
                            sleepTime = Integer.parseInt(properties.getProperty("ACCOUNT_INFO_SLEEP"));
                        } catch (Exception e) {
                        	_log.errorTrace(methodName, e);
                        }
                    } else if (p_action == ZTEOCII.ACTION_RECHARGE_CREDIT) {
                        responseStr = properties.getProperty("ACCOUNT_RECHARGE");
                        try {
                            sleepTime = Integer.parseInt(properties.getProperty("ACCOUNT_RECHARGE_SLEEP"));
                        } catch (Exception e) {
                        	_log.errorTrace(methodName, e);
                        }
                    } else if (p_action == ZTEOCII.ACTION_IMMEDIATE_DEBIT) {
                        responseStr = properties.getProperty("ACCOUNT_CREDIT_XFER");
                        try {
                            sleepTime = Integer.parseInt(properties.getProperty("ACCOUNT_CREDIT_XFER_SLEEP"));
                        } catch (Exception e) {
                        	_log.errorTrace(methodName, e);
                        }
                    } else if (p_action == ZTEOCII.ACTION_IMMEDIATE_DEBIT) {
                        ...
                        try {
                            sleepTime = Integer.parseInt(properties.getProperty("ACCOUNT_DEBIT_SLEEP"));
                        } catch (Exception e) {
                        	_log.errorTrace(methodName, e);
                        }
                    }
