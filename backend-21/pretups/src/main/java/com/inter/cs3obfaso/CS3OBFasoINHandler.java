...
import com.inter.cs3obfaso.cs3scheduler.NodeCloser;
import com.inter.cs3obfaso.cs3scheduler.NodeManager;
import com.inter.cs3obfaso.cs3scheduler.NodeScheduler;
import com.inter.cs3obfaso.cs3scheduler.NodeVO;



public class CS3OBFasoINHandler implements InterfaceHandler{

    private static Log _log = LogFactory.getLog(CS3OBFasoINHandler.class.getName());
	...
	private static int  _prevMinut=0;
   	private static CS3OBFasoRequestFormatter _cs3obfasoRequestFormatter=null;
	...
	static
	{
	    if(_log.isDebugEnabled()) _log.debug("CS3OBFasoINHandler[static]","Entered");
	    try
	    {
	    	_cs3obfasoRequestFormatter = new CS3OBFasoRequestFormatter();
	    }
	    catch(Exception e)
	    {
	        _log.error("CS3OBFasoINHandler[static]","While instantiation of CS3OBFasoRequestFormatter get Exception e::"+e.getMessage());
	        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[static]","","", "","While instantiation of CS3OBFasoRequestFormatter get Exception e::"+e.getMessage());
	    }
	    finally
	    {
	        if(_log.isDebugEnabled()) _log.debug("CS3OBFasoINHandler[static]","Exited");
	    }
	}
    	...
    	final String methodName="validate";
         ...
         try
		 {
			...
			if(InterfaceUtil.isNullString(multFactor))
			{
			    ...
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[validate]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
            ...
            String inStr = _cs3obfasoRequestFormatter.generateRequest(CS3OBFaso.ACTION_ACCOUNT_INFO,_requestMap);
			//sending the AccountInfo request to IN along with validate action defined in CS3NigeriaI interface
            sendRequestToIN(inStr,CS3OBFaso.ACTION_ACCOUNT_INFO);
            ...
            try
			{
				...
				//in case if dedicated account is used this key can be used to get main account balance
				_requestMap.put("MAIN_ACCOUNT_BALANCE", amountStr);
				//temporary to run CS3 Nigeria as postpaid interface
            ...
            catch(Exception e)
			{
            	...
            	EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[validate]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "accountValue1 obtained from the IN is not numeric, while parsing the accountValue1 get Exception e:"+e.getMessage());
            ...
            /*Date supervisionExpiryDate = InterfaceUtil.getDateFromDateString((String)_responseMap.get("supervisionExpiryDate"), "yyyyMMddHHmmss");
            Date serviceFeeExpiryDate = InterfaceUtil.getDateFromDateString((String)_responseMap.get("serviceFeeExpiryDate"), "yyyyMMddHHmmss");
            */
         ...
         /*   if(supervisionExpiryDate > currentDate)
            	_requestMap.put("ACCOUNT_STATUS","ACTIVE");
            else if((supervisionExpiryDate < currentDate) && (currentDate< serviceFeeExpiryDate))
            	_requestMap.put("ACCOUNT_STATUS","INACTIVE");
            else if (serviceFeeExpiryDate < currentDate)
            	_requestMap.put("ACCOUNT_STATUS","DEACTIVE");*/
        	...
        	//get dedicated account and bal map as string
            _requestMap.put("DEDICATED_ACCOUNTS_STR", (String)_responseMap.get("DEDICATED_ACCOUNTS_STR"));
            //get dedicated account and unit type map as string
            _requestMap.put("DEDICATED_ACC_UNIT_TYPE_STR", (String)_responseMap.get("DEDICATED_ACC_UNIT_TYPE_STR"));
            
            //set dedicated accounts based on service class and dedicated account mapping
            if(_requestMap.get("REQ_AMOUNT")!=null)
            	setDedicatedAccountFromMapping();
            

			//check if mapping found for dedicated account and service class
			if(!BTSLUtil.isNullString((String)_requestMap.get("DEDICATED_ACC_MAPPING_FOUND")) && PretupsI.YES.equalsIgnoreCase((String)_requestMap.get("DEDICATED_ACC_MAPPING_FOUND"))) {
				//call for offer id method
				if(_log.isDebugEnabled()) {
					_log.debug(methodName, "Calling method to get offer id");
				}
				getOffers();	
			}
        	...
        	/* LDCC Handling not needed in nigeria
		if((_requestMap.get("ACCOUNT_STATUS").equals("INACTIVE") || requestAmount>interfacePrevAmount ) && ("S".equals(_requestMap.get("USER_TYPE"))))	
        	{
        		 inStr= _CS3OBFasoRequestFormatter.generateRequest(CS3NigeriaI.ACTION_ACCOUNT_DETAILS,_requestMap);
        		 _ambgCurrentRetryCount--;
        		 sendRequestToIN(inStr,CS3NigeriaI.ACTION_ACCOUNT_DETAILS);
        		 if(CS3NigeriaI.LDCC_SERVICE_OFFERING_ACT_FLAG.equals(_responseMap.get("serviceOfferingActiveFlag")))
        				 {
        			 // if ldcc then it should go to postpaid IN from controller.
        			 //setting MSISDN_NOT_FOUND .so that it will go to 
        			 		_requestMap.put("SERVICE_CLASS","LDCC");
        			 		_requestMap.put("INTERFACE_STATUS", CS3NigeriaI.SUBSCRIBER_NOT_FOUND);
         ...
         catch(Exception e)
         {
             _log.errorTrace(methodName,e);
            _log.error(methodName,"Exception e:"+e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[validate]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String)_requestMap.get("NETWORK_CODE"), "While validation of the subscriber get the Exception e:"+e.getMessage());
            throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
         }
         finally
         {
	...
	String transactionCode=null;
		...
		transactionCode=FileCache.getValue(_interfaceID,"TRANSACTION_CODE_RF");
			...
			{
			    _log.error(methodName,"MULT_FACTOR  is not defined in the INFile");
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[credit]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
			    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
			multFactor = multFactor.trim();
			...
			catch(Exception e)
			{
			     _log.errorTrace(methodName,e);
			    _log.error(methodName,"Exception e:"+e.getMessage());
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[credit]","REFERENCE ID = "+_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
			    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
...
//			key value of requestMap is formatted into XML string for the validate request.
            String inStr = _cs3obfasoRequestFormatter.generateRequest(CS3OBFaso.ACTION_RECHARGE_CREDIT,_requestMap);
            ...
            try{_ambgMaxRetryCount = Integer.parseInt(FileCache.getValue(_interfaceID,"CREDIT_RETRY_CNT"));}catch(Exception e){_ambgMaxRetryCount=1;}
             ...
             sendRequestToIN(inStr,CS3OBFaso.ACTION_RECHARGE_CREDIT);
            //set TRANSACTION_STATUS as Success in request map
            _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
            ...
            if(!_isRetryRequest)
            {
    			...
    			catch(Exception e)
    			{
    			     _log.errorTrace(methodName,e);
    			    _log.error(methodName,"Exception e:"+e.getMessage());
    			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[credit]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "NEWBALANCE  is not Numeric");
    				throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_RESPONSE);
    			}
        ...
        catch (BTSLBaseException be)
        {
			...
			catch(Exception e)
			{
				  _log.errorTrace(methodName,e);
				 _log.error(methodName,"Exception e:"+e.getMessage());
				 EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[credit]", _referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in credit");
				 throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}			
		}
         catch (Exception e)
         {
              _log.errorTrace(methodName,e);
             _log.error(methodName, "Exception e:" + e.getMessage());
             EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[credit]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "While credit get the Exception e:"+e.getMessage());
             throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
         }
         finally
         {
    ...
    public void creditAdjust(HashMap p_requestMap) throws BTSLBaseException, Exception
    {
		...
		transactionCode=FileCache.getValue(_interfaceID,"TRANSACTION_CODE_CR");
			...
			{
			    _log.error(methodName,"MULT_FACTOR  is not defined in the INFile");
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[creditAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is  not defined in the INFile");
			    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
			multFactor = multFactor.trim();
			//Set the interface parameters into requestMap
			...
			try
			{
				...
				{
				    roundFlag="Y";
				    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MINOR, "CS3OBFasoINHandler[creditAdjust]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn ," INTERFACE ID = "+_interfaceID, "Network code = "+(String) _requestMap.get("NETWORK_CODE"), "ROUND_FLAG is not defined in IN File hence system taken the default value=N.");
				}
				//If rounding of amount is allowed, round the amount value and put this value in request map.
				if("Y".equals(roundFlag.trim()))
				{
			...
			catch(Exception e)
			{
			     _log.errorTrace(methodName,e);
			    _log.error(methodName,"Exception e:"+e.getMessage());
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[creditAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
			    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
			...
			String inStr = _cs3obfasoRequestFormatter.generateRequest(CS3OBFaso.ACTION_IMMEDIATE_DEBIT,_requestMap);
			...
			try{_ambgMaxRetryCount = Integer.parseInt(FileCache.getValue(_interfaceID,"ADJ_RETRY_CNT"));}catch(Exception e){_ambgMaxRetryCount=1;}
             ...
             sendRequestToIN(inStr,CS3OBFaso.ACTION_IMMEDIATE_DEBIT);
             //set TRANSACTION_STATUS as Success in request map
             _requestMap.put("TRANSACTION_STATUS", InterfaceErrorCodesI.SUCCESS);
             //set INTERFACE_POST_BALANCE into request map as obtained thru response map.
 			_requestMap.put("POST_BALANCE_ENQ_SUCCESS", "N"); 
        ...
        catch (BTSLBaseException be)
        {
			...
			catch(Exception e)
			{
				  _log.errorTrace(methodName,e);
				 _log.error(methodName,"Exception e:"+e.getMessage());
				 EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[creditAdjust]", _referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in creditAdjust");
				 throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}			
		}
         catch (Exception e)
         {
         	  _log.errorTrace(methodName,e);
             _log.error(methodName, "Exception e:"+e.getMessage());
             EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[creditAdjust]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while creditAdjust e:"+e.getMessage());
             throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
         }
         finally
         {
	...
	String transactionCode=null;
		...
		transactionCode=FileCache.getValue(_interfaceID,"TRANSACTION_CODE_DR");
			...
			{
			    _log.error(methodName,"MULT_FACTOR  is not defined in the INFile");
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[debitAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is  not defined in the INFile");
			    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
			multFactor = multFactor.trim();
			//Set the interface parameters into requestMap
			...
			try
			{
				...
				{
				    roundFlag="Y";
				    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MINOR, "CS3OBFasoINHandler[debitAdjust]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn ," INTERFACE ID = "+_interfaceID, "Network code = "+(String) _requestMap.get("NETWORK_CODE"), "ROUND_FLAG is not defined in IN File hence system taken the default value=N.");
				}
				//If rounding of amount is allowed, round the amount value and put this value in request map.
				if("Y".equals(roundFlag.trim()))
				{
			...
			catch(Exception e)
			{
			     _log.errorTrace(methodName,e);
			    _log.error(methodName,"Exception e:"+e.getMessage());
			    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[debitAdjust]",_referenceID,_msisdn +" INTERFACE ID = "+_interfaceID, (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
			    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}
			if(_log.isDebugEnabled()) _log.debug(methodName,"transfer_amount:"+amountStr+" multFactor:"+multFactor);
			//set transfer_amount in request map as amountStr (which is round value of INTERFACE_AMOUNT)
			...
			//for dedicated account
			String taxAmountStr = "";
			String accessFeeStr = "";
			String processgFee = "";
			double reqAmountDouble = 0;
			double dedicatedAccMultifactor = 0;
			double processingFeeDouble = 0;
			if(PretupsI.YES.equalsIgnoreCase((String)_requestMap.get("USE_DEDICATED_ACCOUNT_FLAG"))) {
				try {
					// calculated debit amount for dedicated account
					// calculate processing fee for main account
					dedicatedAccMultifactor = Double.parseDouble(multFactor);
					reqAmountDouble = Double.parseDouble((String)_requestMap.get("REQUESTED_AMOUNT"));
					systemAmtDouble = InterfaceUtil.getINAmountFromSystemAmountToIN(reqAmountDouble,dedicatedAccMultifactor);
					amountStr=String.valueOf(systemAmtDouble);
					
					double taxAmountDouble = Double.parseDouble((String)_requestMap.get("TAX_AMOUNT"));
					double accessFeeDouble = Double.parseDouble((String)_requestMap.get("ACCESS_FEE"));
					double systemTaxAmount  = InterfaceUtil.getINAmountFromSystemAmountToIN(taxAmountDouble,dedicatedAccMultifactor);
					double systemAccessFee = InterfaceUtil.getINAmountFromSystemAmountToIN(accessFeeDouble,dedicatedAccMultifactor);
					
					processingFeeDouble = systemTaxAmount+systemAccessFee;
					
					taxAmountStr = String.valueOf(systemTaxAmount);
					accessFeeStr = String.valueOf(systemAccessFee);
					...
					{	
						...
						taxAmountStr = String.valueOf(Math.round(systemTaxAmount));
					    accessFeeStr = String.valueOf(Math.round(systemAccessFee));
					    processgFee = String.valueOf(Math.round(processingFeeDouble));
						_requestMap.put("DEDICATED_ACCOUNT_DEBIT_AMOUNT",amountStr);
				...
				if(_log.isDebugEnabled()) _log.debug(methodName,"dedicated_transfer_amount:"+amountStr+" multFactor:"+multFactor);
				//set transfer_amount in request map as amountStr (which is round value of INTERFACE_AMOUNT)
				
				
				
				
				//check if main account has enough balance for processing fee
				// if yes debit from main account and set flag to Y
				// else debit from dedicated account, add requested amount and processing fee and set trasnfer amount
				
				
				double mainAccountBal = Double.parseDouble((String)_requestMap.get("MAIN_ACCOUNT_BALANCE"));
				if(_log.isDebugEnabled()) {
					_log.debug(methodName, "Main Account Balance= " + mainAccountBal);
				}
				double systemMainAccountBal = InterfaceUtil.getINAmountFromSystemAmountToIN(mainAccountBal,dedicatedAccMultifactor);
				if(systemMainAccountBal >= processingFeeDouble) {
					_requestMap.put("processingFee_from_mainAccount", PretupsI.YES);
					_requestMap.put("dedicated_processing_fee", "-"+processgFee);
					
				}
				else {
					_requestMap.put("processingFee_from_mainAccount", PretupsI.NO);
					systemAmtDouble += processingFeeDouble;
				...
				{	
				    amountStr=String.valueOf(Math.round(systemAmtDouble));
				    _requestMap.put("dedicated_transfer_amount","-"+amountStr);
        ...
        catch (BTSLBaseException be)
        {
			...
			catch(Exception e)
			{
				  _log.errorTrace(methodName,e);
				 _log.error(methodName,"Exception e:"+e.getMessage());
				 EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[debitAdjust]", _referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while processing ambiguous case in creditAdjust");
				 throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
			}			
		}
         catch (Exception e)
         {
         	  _log.errorTrace(methodName,e);
             _log.error(methodName, "Exception e:"+e.getMessage());
             EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[debitAdjust]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"), "Exception while creditAdjust e:"+e.getMessage());
             throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
         }
         finally
         {
    ...
    private void sendRequestToIN(String p_inRequestStr, int p_action) throws BTSLBaseException
    {
        ...
        CS3OBFasoUrlConnection cs3URLConnection = null;
        long startTime=0;
        long endTime=0;
        ...
        try
        {
        	if(p_action!=CS3OBFaso.ACTION_ACCOUNT_DETAILS)
            ...
            //check if cs3NodeScheduler is null throw exception.Confirm for Error code(INTERFACE_CONNECTION_NULL)if required-It should be new code like ERROR_NODE_FOUND!
            if(cs3NodeScheduler==null)
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS3_WHILE_GETTING_SCHEDULER_OBJECT);
            
			if(_log.isDebugEnabled()) {
				_log.debug(methodName, "_ambgCurrentRetryCount=" + _ambgCurrentRetryCount + " _ambgMaxRetryCount="+_ambgMaxRetryCount);
			}
            while(_ambgCurrentRetryCount++ <=_ambgMaxRetryCount)
			{
				if(_log.isDebugEnabled()) {
					_log.debug(methodName, "Trying to connect");
				}
				
				try
		        {	
					if(p_action!=CS3OBFaso.ACTION_ACCOUNT_INFO && p_action!=CS3OBFaso.ACTION_ACCOUNT_DETAILS)
					{
						if(_log.isDebugEnabled())_log.error(methodName,"SENDING RETRY........"+ (_ambgCurrentRetryCount-1)+ "_isRetryRequest "+_isRetryRequest + "IN Transaction Id" +(String)_requestMap.get("IN_RECON_ID"));
					}
		            long startTimeNode = System.currentTimeMillis();
		            if(_log.isDebugEnabled())_log.debug(methodName,"Start time to find the scheduled Node startTimeNode::"+startTimeNode+"miliseconds");
		            ...
		            for(int loop=1;loop<=conRetryNumber;loop++)
		            {
			            try
			            {
				            ...
				            if(_log.isDebugEnabled())_log.error(methodName,"URL PICKED IS ......."+ cs3NodeVO.getUrl());
				            ...
				            catch(BTSLBaseException be)
							{
				            	...
				            	if(loop==conRetryNumber)
				                {
				            		if("Y".equals(_requestMap.get("ADJUST")) && "C".equals(_requestMap.get("INTERFACE_ACTION")) && "S".equals(_userType))
									{
				            			...
				            			EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"Sender Credit Back case. But all nodes are suspended. So marking the response as AMBIGUOUS");
				            			throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
									}
				                    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"MAXIMUM SHIFTING OF NODE IS REACHED");
				            ...
				            if(CS3OBFaso.ACTION_ACCOUNT_INFO==p_action || CS3OBFaso.ACTION_ACCOUNT_DETAILS==p_action)
				            ...
				            cs3URLConnection = new CS3OBFasoUrlConnection(cs3NodeVO.getUrl(),cs3NodeVO.getUsername(),cs3NodeVO.getPassword(),cs3NodeVO.getConnectionTimeOut(),readTimeOut,cs3NodeVO.getKeepAlive(),p_inRequestStr.length(),hostName,userAgent);
				            //break the loop on getting the successfull connection for the node;
				            if(_log.isDebugEnabled()) _log.debug(methodName,"Connection of _interfaceID ["+_interfaceID+"] for the Node Number ["+cs3NodeVO.getNodeNumber()+"] created after the attempt number(loop)::"+loop);
				            break;
			            }
			            catch(BTSLBaseException be)
			            {
			            ...
			            catch(Exception e)
			            {
			                ...
			                _log.error(methodName,"Exception while creating connection e::"+e.getMessage());
			                EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"Exception while getting the connection for CS3NigeriaIN with INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs3NodeVO.getNodeNumber()+"]");
			                
			                _log.info(methodName,"Setting the Node ["+cs3NodeVO.getNodeNumber()+"] as blocked for duration ::"+cs3NodeVO.getExpiryDuration() +" miliseconds");
			                ...
			                if(loop==conRetryNumber)
			                {
								...
								{
			                		_requestMap.put("INTERFACE_STATUS",InterfaceErrorCodesI.AMBIGOUS);	  
			            			EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"Sender Credit Back case. But all nodes are Blocked. So marking the response as AMBIGUOUS");
			            			throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
								}
			                    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn,(String) _requestMap.get("NETWORK_CODE"),"MAXIMUM SHIFTING OF NODE IS REACHED");
			                    throw new BTSLBaseException(InterfaceErrorCodesI.RETRY_ATTEMPT_FAILED);
		            ...
		            if(_log.isDebugEnabled())_log.debug(methodName,"Total time to find the scheduled Node totalTimeNode: "+totalTimeNode);
		        }
		        catch(BTSLBaseException be)
		        {
		            //try{cs3NodeVO.decrementConNumber();}catch(Exception e){}-to Check properly
		            throw be;
		        }//end of catch-BTSLBaseException
		        catch(Exception e)
		        {
		        ...
		        catch(Exception e)
		        {
	                ...
	                EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"),"While sending request to cs3nigeria IN INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs3NodeVO.getNodeNumber()+"] Exception::"+e.getMessage());
		        ...
		        try
		        {
			        ...
			        try
			        {
						...
						{
						    _log.info(methodName, "WARN time reaches, startTime::"+startTime+" endTime::"+endTime+" From file cache warnTime::"+warnTime+ " time taken (endTime-startTime)::"+(endTime-startTime));
						    EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.INTERFACES,EventStatusI.RAISED,EventLevelI.MAJOR,"CS3OBFasoINHandler[sendRequestToIN]",_inTXNID,_msisdn,(String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"CS3NigeriaIN is taking more time than the warning threshold. Time: "+(endTime-startTime)+"INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs3NodeVO.getNodeNumber()+"]");
						}
			        }
			        catch(Exception e)
			        {
			             _log.errorTrace(methodName,e);
			            _log.error(methodName,"Exception e::"+e.getMessage());
			            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE"),"While getting the response from the CS3NigeriaIN for INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs3NodeVO.getNodeNumber()+"]"+"Exception="+e.getMessage());
			            _nodeCloser.updateCountersOnAmbiguousResp(cs3NodeVO);
			            
			            if(_ambgCurrentRetryCount > _ambgMaxRetryCount)
			    		{
			            	_log.error(methodName,"Read time out occured. Retry attempts exceeded so throwing AMBIGOUS exception");
		            ...
		            if (_log.isDebugEnabled())_log.debug(methodName, "responseStr::" + responseStr);
		            //TransactionLog.log(_inTXNID,_referenceID,_msisdn,(String)_requestMap.get("NETWORK_CODE"),String.valueOf(p_action),PretupsI.TXN_LOG_REQTYPE_RES,"Response string:"+responseStr,PretupsI.TXN_LOG_STATUS_UNDERPROCESS,"action="+p_action);
		            String httpStatus = cs3URLConnection.getResponseCode();
		            _requestMap.put("PROTOCOL_STATUS", httpStatus);
		            if(!CS3OBFaso.HTTP_STATUS_200.equals(httpStatus))
		                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_BAD_REQUEST);
		            //Check if there is no response, handle the event showing Blank response from cs3nigeria and stop further processing. 
		            if (InterfaceUtil.isNullString(responseStr))
		            {
		                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Blank response from CS3NigeriaIN");
		                _log.error(methodName, "NULL response from interface");
		                _nodeCloser.updateCountersOnAmbiguousResp(cs3NodeVO);
		                
		                if(_ambgCurrentRetryCount > _ambgMaxRetryCount)
			    		{
		                	_log.error(methodName,"Read time out occured. Retry attempts exceeded so throwing AMBIGOUS exception");
							EventHandler.handle(EventIDI.INTERFACE_RESPONSE_EXCEPTION,EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "CS3OBFasoINHandler[sendRequestToIN]",_inTXNID,_msisdn,(String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,  "NULL response from IN. Retry attempts exceeded so throwing AMBIGOUS exception");
							_requestMap.put("INTERFACE_STATUS",InterfaceErrorCodesI.AMBIGOUS);	       
							throw new BTSLBaseException(InterfaceErrorCodesI.AMBIGOUS);
			    		} 
		                Thread.sleep(retrySleepTime);
		            ...
		            _responseMap=_cs3obfasoRequestFormatter.parseResponse(p_action,responseStr,_responseMap);
		            ...
		            if(!InterfaceUtil.isNullString(faultCode))
		            {
		                ...
		                _log.info(methodName, "faultCode::"+faultCode +"_inTXNID::"+_inTXNID+" _msisdn::"+_msisdn);
		                _requestMap.put("INTERFACE_STATUS", faultCode);//Put the interface_status in requestMap
		                _log.error(methodName,"faultCode="+_responseMap.get("faultCode")+"faultString = "+_responseMap.get("faultString"));
		                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,(String)_responseMap.get("faultString"));
		            ...
		            {
			            if(CS3OBFaso.SUBSCRIBER_NOT_FOUND.equals(responseCode))
			            {
			                _log.error(methodName,"Subscriber not found with MSISDN::"+_msisdn);
			                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Subscriber is not found at IN");
			                throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND);
			            }//end of checking the subscriber existance.
			            else if(CS3OBFaso.OLD_TRANSACTION_ID.equals(responseCode))
			            {
			            	if(!_isRetryRequest)
							{
								if(_log.isDebugEnabled())_log.debug(methodName,"_isRetryRequest:"+_isRetryRequest);
								EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID,EventComponentI.INTERFACES,EventStatusI.RAISED,EventLevelI.FATAL,"CS3OBFasoINHandler[sendRequestToIN]",_inTXNID,_msisdn,(String)_requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Transaction ID mismatch");
			            ...
			            else
			            {
			                ...
			                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"CS3OBFasoINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Error code received from IN "+responseCode);
			                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
			            }
        ...
        finally
        {
            try
            {
                ...
                if (cs3URLConnection != null) cs3URLConnection.close();
                if(cs3NodeVO!=null)
                {
                    _log.info(methodName,"Connection of Node ["+cs3NodeVO.getNodeNumber()+"] for INTERFACE_ID="+_interfaceID+" is closed");
                    //Decrement the connection number for the current Node.
                    cs3NodeVO.decrementConNumber(inReconID);
                    _log.info(methodName,"After closing the connection for Node ["+cs3NodeVO.getNodeNumber()+"] USED connections are ::["+cs3NodeVO.getConNumber()+"]");
                }
            }
            catch (Exception e)
            {
                 _log.errorTrace(methodName,e);
                _log.error(methodName, "While closing CS3NigeriaIN Connection Exception e::" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[sendRequestToIN]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action, "Not able to close connection:" + e.getMessage()+"INTERFACE_ID=["+_interfaceID +"]and Node Number=["+cs3NodeVO.getNodeNumber()+"]");
            }
            if (_log.isDebugEnabled())
    ...
    /**
    	...
    	private void setInterfaceParameters() throws Exception,BTSLBaseException
	    {
	        ...
	        {        	
		    	...
		    	if(InterfaceUtil.isNullString(cancelTxnAllowed))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "CANCEL_TXN_ALLOWED is not defined in the INFile.");
		    	...
		    	if(InterfaceUtil.isNullString(systemStatusMappingCr))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT is not defined in the INFile.");
		    		throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
		    	}
		    	...
		    	if(InterfaceUtil.isNullString(systemStatusMappingCrAdj))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT_ADJ is not defined in the INFile.");
		    		throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
		    	}
		    	...
		    	if(InterfaceUtil.isNullString(systemStatusMappingDbtAdj))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_DEBIT_ADJ is not defined in the INFile.");
		    		throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
		    	}
		    	...
		    	if(InterfaceUtil.isNullString(systemStatusMappingCrBck))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "SYSTEM_STATUS_MAPPING_CREDIT_BCK is not defined in the INFile.");
		    		throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
		    	}
		    	...
		    	if(InterfaceUtil.isNullString(cancelCommandStatusMapping))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "CANCEL_COMMAND_STATUS_MAPPING is not defined in the INFile.");
		    		throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
		    	}
		    	...
		    	if(InterfaceUtil.isNullString(cancelNA))
		    	{
		    		...
		    		EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn , (String) _requestMap.get("NETWORK_CODE"), "CANCEL_NA is not defined in the INFile.");
		    		throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
		    	}
				...
				{
					EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "WARN_TIMEOUT is not defined in IN File or not numeric");
				    throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
				}
				...
				{
					EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "NODE_TYPE is not defined in IN File ");
				    throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
				}
				...
				{
					EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "HOST_NAME is not defined in IN File ");
				    throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
				}
				...
				{
					EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "CURRENCY is not defined in IN File ");
				    throw new BTSLBaseException(this,"setInterfaceParameters",InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
				}
				...
				{
					EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "CS3OBFasoINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "EXTERNAL_DATA1 is not defined in IN File ");
				    
				}
				_requestMap.put("EXTERNAL_DATA1",extData1.trim());
				//Modification Done During LIVE for BurkinaFaso based on requirement basis
				//Put the optional value of ExternalData1.
				if(!InterfaceUtil.isNullString(extData1))
		    		_requestMap.put("ExternalData1",extData1.trim());
		    	...
		    	if(!InterfaceUtil.isNullString(externalData2Required)&& "Y".equals(externalData2Required))
		    	{
		    		...
		    		if(!InterfaceUtil.isNullString(extData2))
		    			_requestMap.put("ExternalData2",extData2.trim());
		    		else
		    		{
		    			extData2=(String)_requestMap.get("SENDER_MSISDN");
		    			_requestMap.put("ExternalData2",extData2);
		    		}
		    	}
        ...
        private void setLanguageFromMapping() throws Exception
        {
            ...
            try
            {
                //Get the mapping string from the FileCache and storing all the mappings into array which are separated by ','.
             	...
             	if(_log.isDebugEnabled()) _log.debug(methodName,"mappingString::"+mappingString +" langFromIN::"+langFromIN);
        	    ...
        	    if(mappingNotFound)
        	        EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "CS3OBFasoINHandler[setLanguageFromMapping]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Mapping for language received from IN =  "+langFromIN +" is not defined in IN file Hence setting the Default language");
            ...
            catch(Exception e)
            {
                 _log.errorTrace(methodName,e);
                _log.error(methodName,"Exception e::"+e);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setLanguageFromMapping]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "While mapping the IN Language with the system Language getting the Exception =" + e.getMessage());
            }//end of catch-Exception
            finally
            {
        ...
        protected static synchronized String getINTransactionID(HashMap p_requestMap) throws BTSLBaseException
		{
			...
			try
			{
		    	...
		    	if(InterfaceUtil.isNullString(instanceID))
		    	{
		    	    ...
		    	    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[validate]","","" , (String) p_requestMap.get("NETWORK_CODE"), "Instance id[INSTANCE_ID] is not defined in IN File");
		    	    throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
		    	}
    	...
    	/**
         * This method used to get the service class and dedicated accounts  mapping  in FileCache.Includes following
         * @throws Exception
         */
        private void setDedicatedAccountFromMapping() throws Exception {
        	final String methodName="setDedicatedAccountFromMapping";
            if(_log.isDebugEnabled()) _log.debug(methodName,"Entered");
            String mappedAccounts= "";//stores list of dedicated account applicable for service class
    	    String[] mappingArr;
    	    String[] tempArr;
    	    String[] accountsArr;
    	    boolean mappingNotFound = true;//Flag defines whether the mapping of service class and dedicated account is found or not.
    	    String serviceClassFromIN = null;
            try
            {
                //Get the mapping string from the FileCache and storing all the mappings into array which are separated by ','.
             	String mappingString = (String)FileCache.getValue(_interfaceID, "DEDICATED_ACCOUNT_MAPPING");
             	serviceClassFromIN = (String)_responseMap.get("serviceClassCurrent");
             	if(_log.isDebugEnabled()) 
             		_log.debug(methodName,"mappingString::"+mappingString +" serviceClassFromIN::"+serviceClassFromIN);
        	    mappingArr = mappingString.split(",");
        	    //Iterating the mapping array to map the service class from the dedicated account,if found break the loop.
        	    ...
        	    for(int in=0;in<mappingArr.length;in++)
        	    {
        	        ...
        	        if(tempArr.length>2) {
        	        	//2009:4:11
        	        	if(serviceClassFromIN.equals(tempArr[0].trim())) {
        	        		
        	        		for(int i=1; i<tempArr.length; i++) {
        	        			if(i!=tempArr.length-1) {
        	        				mappedAccounts = mappedAccounts + tempArr[i]+":";
        	        			}
        	        			else {
        	        				mappedAccounts = mappedAccounts + tempArr[i];
        	        			}
        	    ...
        	    //if the mapping of service class and dedicated account found
        	    if(mappingNotFound) {
        	    	// will use this flag to check if getOffers method need to be called or not
        	    	_requestMap.put("DEDICATED_ACC_MAPPING_FOUND", PretupsI.NO);
        	    	EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "CS3OBFasoINHandler[setDedicatedAccountFromMapping]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Mapping for service Class received from IN =  "+serviceClassFromIN +" is not defined in IN file Hence using main account");
        	    }
        	        
                //Set the mapped dedicated accounts to the requested map.
        	    _requestMap.put("DEDICATED_ACCOUNTS",mappedAccounts);
        	    ...
        	    if(!mappingNotFound) {
            	    ...
            	    if(_log.isDebugEnabled()) {
            	    	_log.debug(methodName, "service class and dedicated account mapping found");
            	    }
            	    

        	    	
            	    
            	    _requestMap.put("DEDICATED_ACC_MAPPING_FOUND", PretupsI.YES);
            	    
        	    	Double reqAmount = null;
        	    	try {
        	    		reqAmount = Double.parseDouble((String)_requestMap.get("REQ_AMOUNT"));
        	    	}
        	    	catch(Exception e){
                        _log.errorTrace(methodName,e);
                        _log.error(methodName,"Exception e::"+e);
        	    	...
        	    	if(reqAmount !=null) {
            	    	...
            	    	if(_log.isDebugEnabled()) {
            	    		_log.debug(methodName, "Select dedicated account that has enough balance");
            	    	}

        	    	
	        	    	String dedicatedAccounts = (String)_requestMap.get("DEDICATED_ACCOUNTS");//11:10000 format in case of multiple account and 11 in case of single account mapping
	        	    	String dedicatedAccountsBalanceMapping = (String)_requestMap.get("DEDICATED_ACCOUNTS_STR");
	        	    	String dedicatedAccUnitTypeMapping = (String)_requestMap.get("DEDICATED_ACC_UNIT_TYPE_STR");
	        	    	HashMap accountBalMap = BTSLUtil.getStringToHash(dedicatedAccountsBalanceMapping, "&","=");
	        	    	HashMap accountUnitTypeMap = BTSLUtil.getStringToHash(dedicatedAccUnitTypeMapping, "&","=");
	        	    	...
	        	    	//get balance for dedicated account that is mapped to service class.
	        	    	//if balance is enough set account id, balance  
	        	    	// set use dedicated account flag to Y
	        	    	// if balance not enough set use dedicated account flag to N
	        	    	String[] mappedArr = dedicatedAccounts.split(":");
	        	    	String balance_str = null;
	        	    	if(mappedArr.length>1) {
	        	    		for(int i=0; i<mappedArr.length; i++) {
	        	    			balance_str = (String)accountBalMap.get(mappedArr[i].trim());
	            	    		balance_str = InterfaceUtil.getSystemAmountFromINAmount(balance_str,Double.parseDouble(multFactor));
	            	    		String unitTypeStr = (String)accountUnitTypeMap.get(mappedArr[i].trim());
	            	    		if(balance_str!=null) {
	            	    			Double balance = Double.parseDouble(balance_str);
	            	    			if(balance >= reqAmount) {
	            	    				_requestMap.put("DEDICATED_ACCOUNT_ID",mappedArr[i].trim());
	            	    				_requestMap.put("DEDICATED_ACCOUNT_BALANCE", balance_str);
	            	    				_requestMap.put("USE_DEDICATED_ACCOUNT_FLAG", PretupsI.YES);
	            	    				_requestMap.put("DEDICATED_ACCOUNT_UNITTYPE", unitTypeStr);
	            	    				
	            	    				break;
	            	    			}
	            	    			else {
	            	    				_requestMap.put("USE_DEDICATED_ACCOUNT_FLAG", PretupsI.NO);
	            	    			}
	            	    		}
	        	    		}
	        	    	}
	        	    	else {
	        	    		balance_str = (String)accountBalMap.get(mappedArr[0].trim());
	        	    		balance_str = InterfaceUtil.getSystemAmountFromINAmount(balance_str,Double.parseDouble(multFactor));
	        	    		String unitTypeStr = (String)accountUnitTypeMap.get(mappedArr[0].trim());
	        	    		if(balance_str!=null) {
	        	    			Double balance = Double.parseDouble(balance_str);
	        	    			if(balance >= reqAmount) {
	        	    				_requestMap.put("DEDICATED_ACCOUNT_ID",mappedArr[0].trim());
	        	    				_requestMap.put("DEDICATED_ACCOUNT_BALANCE", balance_str);
	        	    				_requestMap.put("USE_DEDICATED_ACCOUNT_FLAG", PretupsI.YES);
	        	    				_requestMap.put("DEDICATED_ACCOUNT_UNITTYPE", unitTypeStr);
            ...
            catch(Exception e)
            {
                 _log.errorTrace(methodName,e);
                _log.error(methodName,"Exception e::"+e);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "CS3OBFasoINHandler[setDedicatedAccountFromMapping]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "While mapping the dedicated account with the system getting the Exception =" + e.getMessage());
            }//end of catch-Exception
            finally
            {
                if(_log.isDebugEnabled()) _log.debug(methodName,"Exited mappedAccounts= "+mappedAccounts.toString());
            }//end of finally setDedicatedAccountFromMapping
        }//end of setDedicatedAccountFromMapping
         ...
         * @throws	BTSLBaseException, Exception 
         */
        private void getOffers() throws BTSLBaseException, Exception
        {
        	final String methodName="getOffers";
        	 if(_log.isDebugEnabled()) _log.debug(methodName,"Entered");
             ...
             try
    		 {
            	 ...
            	 String inStr = _cs3obfasoRequestFormatter.generateRequest(CS3OBFaso.ACTION_GET_OFFERS,_requestMap);
                
            	 
            	 
    			//sending the getOffers request to IN along with get offers action defined in CS3OBFso interface
            	 _ambgCurrentRetryCount--;
            	sendRequestToIN(inStr,CS3OBFaso.ACTION_GET_OFFERS);

                
                
                // load valid offerid from INfile
                String offerIds = FileCache.getValue(_interfaceID,"OFFER_IDS");
                String[] offerIdsArr = offerIds.split(",");
                
                
                
                //extract offers map  and check if valid offerid is there
                
                String offerIdsResponse = (String)_responseMap.get("OFFER_IDS");
                HashMap<String,String> offerMap = BTSLUtil.getStringToHash(offerIdsResponse, "&", "=");
                boolean flag = false;
                for(String s: offerIdsArr) {
                	if(offerMap.containsKey(s)) flag = true;
                }
                
                if(!flag) {
                	//offer id not found in getOffers so fail the request
                	EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "CS3OBFasoINHandler[getOffers]",_referenceID, _msisdn, (String) _requestMap.get("NETWORK_CODE"), "Valid offer id not found");
                	throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.OFFER_ID_INVALID);
                }
    		 }
             catch (BTSLBaseException be)
             {
