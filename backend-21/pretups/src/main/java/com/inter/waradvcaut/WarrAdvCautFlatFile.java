...
public class WarrAdvCautFlatFile {
    ...
    public boolean createFlatFile(HashMap data) throws BTSLBaseException {
        ...
        File[] files = null;
        String wrc = PretupsI.SERVICE_TYPE_CHNL_WARRANTY_RECHARGE;
        String adv = PretupsI.SERVICE_TYPE_CHNL_ADVANCE_RECHARGE;
        String caut = PretupsI.SERVICE_TYPE_CHNL_CAUTION_RECHARGE;
        String ppb = PretupsI.SERVICE_TYPE_CHNL_BILLPAY;
        ...
        try {
            flatfile = false;
            Date date = new Date();
            String service = (String) data.get("REQ_SERVICE");
            String service1 = service.trim();
            String service2 = wrc.trim();
            String service3 = adv.trim();
            String service4 = caut.trim();
            String service5 = ppb.trim();

            if (service1.equalsIgnoreCase(service2)) {
                filePath = Constants.getProperty("FLATFILE_PATH_WARRANTY");
            } else if (service1.equalsIgnoreCase(service3)) {
                filePath = Constants.getProperty("FLATFILE_PATH_ADVANCE");
            } else if (service1.equalsIgnoreCase(service4)) {
                filePath = Constants.getProperty("FLATFILE_PATH_CAUTION");
            } else if (service1.equalsIgnoreCase(service5)) {
                filePath = Constants.getProperty("FLATFILE_PATH_INVOICE");
            }

            if (InterfaceUtil.isNullString(filePath)) {
                _log.error("createFlatFile", "File Path for  is not defined in the Constant.props");
            ...
            if (InterfaceUtil.isNullString(fileName)) {
                _log.error("createFlatFile", "File name for  is not defined in the Constant.props");
                throw new BTSLBaseException(this, "createFlatFile", InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            try {
