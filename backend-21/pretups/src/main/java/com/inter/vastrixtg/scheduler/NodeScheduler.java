package com.inter.vastrixtg.scheduler;
 ...
 *                   VASTGINHandler while creation of connection for the node
 *                   results into error.
 */
public class NodeScheduler {
     ...
     * @throws Exception
     */
    public NodeScheduler(String p_vasINId) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("NodeScheduler[constructor]", "Entered p_vasINId::" + p_vasINId);
        String url = null;
        ...
        try {
            ...
            _interfaceID = p_vasINId;
            try {
                _totalNodes = Integer.parseInt(FileCache.getValue(p_vasINId, "MAX_NODE").trim());
                if (_log.isDebugEnabled())
            ...
            try {
                _maxAllowedNum = Integer.parseInt(FileCache.getValue(p_vasINId, "MAX_ALWD_NO").trim());
                if (_log.isDebugEnabled())
            ...
            try {
                _retryNum = Integer.parseInt(FileCache.getValue(p_vasINId, "RETRY_NUM").trim());
                if (_log.isDebugEnabled())
            ...
            for (int l = 1; l <= _totalNodes; l++) {
                try {
                    ...
                    url = FileCache.getValue(p_vasINId, "URL_" + l);
                    if (_log.isDebugEnabled())
                        _log.debug("NodeScheduler[constructor]", "url_" + l + "::" + url);
                    if (InterfaceUtil.isNullString(url)) {
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of URL_" + l + " in INFile " + p_vasINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_NODE_INITIALIZATION);
                    }
                    VASNodeVO.setUrl(url.trim());

                    try {
                        String ReadTimeOutStr = FileCache.getValue(p_vasINId, "READ_TIMEOUT_" + l).trim();
                        ...
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of READ_TIMEOUT_" + l + " in INFile " + p_vasINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_NODE_INITIALIZATION);
                    }

                    try {
                        String warnTimeStr = FileCache.getValue(p_vasINId, "WARN_TIME_" + l).trim();
                        ...
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of WARN_TIME_" + l + " in INFile " + p_vasINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_NODE_INITIALIZATION);
                    }

                    try {
                        String expiryTimeStr = FileCache.getValue(p_vasINId, "EXPIRY_TIME_" + l).trim();
                        if (_log.isDebugEnabled())
                        ...
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of EXPIRY_TIME_" + l + " in INFile " + p_vasINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_NODE_INITIALIZATION);
                    }

                    try {
                        int maxBarredCount = Integer.parseInt(FileCache.getValue(p_vasINId, "MAX_BARRED_COUNT_" + l).trim());
                        if (_log.isDebugEnabled())
                            _log.debug("NodeScheduler[constructor]", "MAX_BARRED_COUNT_" + l + "::" + maxBarredCount);
                        VASNodeVO.setMaxBarredCount(maxBarredCount);
                    } catch (Exception e) {
                        e.printStackTrace();
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Check the value of MAX_BARRED_COUNT_" + l + " in INFile " + p_vasINId);
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_NODE_INITIALIZATION);
                    }

                    _nodeTable.put(String.valueOf(l), VASNodeVO);
                } catch (BTSLBaseException be) {
                    _log.error("NodeScheduler[constructor]", "BTSLBaseException be::" + be.getMessage());
                    throw be;
                } catch (Exception e) {
                    e.printStackTrace();
                    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::" + _interfaceID, "", "", "Exception while initializing the Node details of interfaceID = " + p_vasINId + " Exception e=" + e.getMessage());
                    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_NODE_INITIALIZATION);
                }
            }
        } catch (BTSLBaseException be) {
    ...
    public synchronized NodeVO getNodeVO(String p_transId) throws BTSLBaseException {
        ...
        try {
            ...
            VASNodeVO = (NodeVO) _nodeTable.get(nodeNumStr);
            if (VASNodeVO == null) {
            ...
            if (_log.isDebugEnabled())
                _log.debug("getNodeVO", p_transId, "Exited VASNodeVO::" + VASNodeVO);
        }// end of finally
        return VASNodeVO;
    }// end of getNodeVO
    ...
    private synchronized String getNode(String p_transId) throws BTSLBaseException {
        ...
        try {
            ...
            // currentNode.
            VASNodeVO = (NodeVO) _nodeTable.get(String.valueOf(_nodeNumber));
            ...
            while (isNext) {
                try {
                    if (tempCount > _maxAllowedNum) {
                        ...
                        // throw new
                        // BTSLBaseException(InterfaceErrorCodesI.ERROR_VAS_MAX_NODE_CHECK_REACH);
                    ...
                    if (!VASNodeVO.isBlocked()) {
                        ...
                        if (VASNodeVO.getBarredCount() >= VASNodeVO.getMaxBarredCount()) {
                            ...
                            nodeNumStr = String.valueOf(_nodeNumber);
                            incrementNodeNumber();
                            EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "NodeScheduler[getNode]", "INTERFACE_ID::" + _interfaceID, "", "p_transId::" + p_transId, "CCWS IP=" + VASNodeVO.getUrl() + " is not responding and blocked");
                            continue;
                        }
                    ...
                    if (isExpired(VASNodeVO.getBlokedAt(), VASNodeVO.getExpiryDuration())) {
                        ...
                        EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MAJOR, "NodeScheduler[getNode]", "INTERFACE_ID::" + _interfaceID, "", "p_transId::" + p_transId, "CCWS IP=" + VASNodeVO.getUrl() + " is unblocked");
                        break;
                    }
