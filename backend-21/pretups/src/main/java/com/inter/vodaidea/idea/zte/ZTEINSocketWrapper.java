 ...
 * ------------------------------------------------------------------------------------------------
 * This class would be responsible to provide the Wrapper of SocketConnection class.
 */
public class ZTEINSocketWrapper extends Socket{
	...
	public int IN_HEADER_SESSION_ID_PAD_LENGTH=6;
	...
	private String _nodeNumber=null;
	...
	public synchronized String getINHeaderTxnID() throws BTSLBaseException
	{
		...
		{
			if(_logger.isDebugEnabled()) _logger.debug("getINHeaderTxnID","Exit headerTransactionID:"+headerTransactionID);
			return headerTransactionID;
		}
	}
	/**
	 * This method is used to send the Login request to the IN.
	 * Counter is incremented to 1 each time and reinitializes the counter when its max value is reached.
	 * @param   p_requestStr
	 * @return	String 
	...
	{
		if(_logger.isDebugEnabled()) _logger.debug("sendLoginRequest "," Entered p_requestStr:"+p_requestStr);
		try
		{
	...
	{
		if(_logger.isDebugEnabled()) _logger.debug("getLoginResponse "," Entered");
		...
		{
			if(_logger.isDebugEnabled()) _logger.debug("getLoginResponse "," responseStr:"+responseStr);
		}
		return responseStr;
	}
	/**
	...
	{
		if(_logger.isDebugEnabled()) _logger.debug("sendLogoutRequest "," Entered p_requestStr:"+p_requestStr);
		try
		{
			_out.write(p_requestStr.getBytes());
			_out.flush();
		...
		catch(Exception e)
		{
			...
			//EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.INTERFACES,EventStatusI.RAISED,EventLevelI.FATAL,"getINHeaderTxnID","","_interfaceID:"+_interfaceID,"","While sending the login request get Exception:"+e.getMessage());
			////throw new BTSLBaseException("Error Code:ERROR_SEND_LOGOUT_REQ");
		}
	}
	/**
	...
	{
		if(_logger.isDebugEnabled()) _logger.debug("getLogoutResponse ","Entered");
		StringBuffer responseBuffer = null;
		String responseStr=null;
		try
		{
		...
		{
			if(_logger.isDebugEnabled()) _logger.debug("getLogoutResponse"," responseStr:"+responseStr);
		}
		return responseStr;
	}
	/**
	...
	{
		try{_out = _socketConnection.getOutputStream();} catch(Exception e) {}
	}
	/**
	 * This method is responsible to set OutputStream to Socket.
	 */
	...
	{
		try{_socketConnection.setSoTimeout(p_timeout);}catch(Exception e){}
	}
	/**
	 * This method is used to generate the session id, that would be present in the Session Header of request.
	 * @return String
	 */
	public synchronized String generateSessionID() throws BTSLBaseException
	{  
		String counter="";
		try
		{
			if(_sessionIdInt < 999999)
			{
				_sessionIdInt++;
			}else
			{
			...
			if(_logger.isDebugEnabled())_logger.debug("generateSessionID"," counter :"+ String.valueOf(_sessionIdInt));
			counter=BTSLUtil.padZeroesToLeft(String.valueOf(Constants.getProperty("INSTANCE_ID")),IN_HEADER_TRANSACTION_ID_PAD_LENGTH)+BTSLUtil.padZeroesToLeft( String.valueOf(_sessionIdInt),IN_HEADER_SESSION_ID_PAD_LENGTH);
		}
		catch(Exception e)
		{
			e.printStackTrace();
		...
		{
			if(_logger.isDebugEnabled()) _logger.debug("generateSessionID ","Exiting counter = "+counter);
		}//end of finally
		return counter;
	}
	/**
	 * This method is responsible to return the BufferedReader object of Opened socket.
	...
	/**
	 * This method is used to send logout request to IN and close the opened socket
	 * @throws BTSLBaseException
	 */
	public void destroy(String fileCacheId) throws BTSLBaseException
	{
		if(_logger.isDebugEnabled()) ZTEINHBLogger.logMessage("destroy Entered");
		try
		{
		...
		finally
		{
			...
			if(_logger.isDebugEnabled()) _logger.debug("destroy "," Exited _socketConnection"+_socketConnection);
		}
	}
	...
	public ZTEINSocketWrapper(String fileCacheId,String p_interfaceId, ZTEINSocket socket) throws BTSLBaseException
	{
		...
		if(_logger.isDebugEnabled()) _logger.debug("ZTESocketWrapper[constructor]"," Entered p_interfaceId::"+p_interfaceId);
		...
		try
		{
			...
			try{
			_nodeNumber=p_interfaceId.substring(11);
			}catch (Exception e) {
				_nodeNumber="1";
			}
			if(socket.getDestinationIP()==null)
			{
				ip=FileCache.getValue(fileCacheId,"IP_"+_nodeNumber);

				if(_logger.isDebugEnabled()) _logger.debug("ZTESocketWrapper[constructor]"," ip :: "+ip);
			...
			{
				portStr=FileCache.getValue(fileCacheId,"PORT_"+_nodeNumber);

				if(_logger.isDebugEnabled()) _logger.debug("ZTESocketWrapper[constructor]"," portStr :: "+portStr);
			...
			//If the SOCKET_TIMEOUT would not be numeric the exception will be catch in the catch block.

			int connectionTimeOut=Integer.parseInt(conTimeOutStr.trim());

			//Initializing the SocketConnection class to establish a socket connection.
			//Put the interfaceId and sessionId and transaction id to the _requestMap
			...
			{
				_requestMap.put("SESSIONID",socket.getSessionId());
			}else{
				_requestMap.put("SESSIONID", generateSessionID());
			...
			//Send the login request to the using PrintWriter object and Get the response of login from the ZTE IN.
			...
			//Setting Response Session Id if not null
			if(!BTSLUtil.isNullString(_responseMap.get("IN_SESSIONHEADERID")))
			{
			socket.setSessionId(_responseMap.get("IN_SESSIONHEADERID"));
			}else{
				socket.setSessionId(_requestMap.get("SESSIONID"));
			}
			//If the login is successful then the response map would contain the value of response_status
		...
		catch(BTSLBaseException be)
		{
			...
			if(_socketConnection!=null)
			{
				try{_socketConnection.close();}catch(Exception e){}
			}
			throw be;
		}
		catch(SocketException e)
		{
			...
			if(_socketConnection!=null)
			{
				try{_socketConnection.close();}catch(Exception ex){}
			}
