 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class UCIP4xUrlConnection extends BTSLConnection{
	private HttpURLConnection _urlConnection=null;
	public UCIP4xUrlConnection(String p_url,String p_username,String p_password,int p_connectTimeout,int p_readTimeout,String p_keepAlive,long p_contentLength, String p_hostName,String p_userAgent) throws Exception
	{
		if(_log.isDebugEnabled())_log.debug("CS3CP6UrlConnection","Entered p_url: "+p_url+"p_connecttimeout="+p_connectTimeout+" p_readtimeout="+p_readTimeout+" p_keepAlive="+p_keepAlive+" p_hostName="+p_hostName+" p_userAgent="+p_userAgent);
		try
		{
		...
		{
			e.printStackTrace();
			_log.error("CS3CP6UrlConnection", "Exception " + e.getMessage());
			if(_log.isDebugEnabled())_log.debug("CS3CP6UrlConnection","Exception " + e.getMessage());
			throw e;
		}
		if(_log.isDebugEnabled())_log.debug("CS3CP6UrlConnection","Exiting");
	}
	...
	{
	    if(_log.isDebugEnabled()) _log.debug("setRequestHeader","Entered p_keepAlive::"+p_keepAlive);
	    try
	    {
	...
	public UCIP4xUrlConnection(String p_url,int p_connectTimeout,int p_readTimeout,String p_keepAlive) throws Exception
	{
		if(_log.isDebugEnabled())_log.debug("UCIP4xUrlConnection","Entered p_url: "+p_url+"p_connecttimeout="+p_connectTimeout+" p_readtimeout="+p_readTimeout+" p_keepAlive="+p_keepAlive);
		try
		{
			URL url=new URL(p_url);
			_urlConnection=(HttpURLConnection)url.openConnection();
		...
		{
			e.printStackTrace();
			_log.error("UCIP4xUrlConnection", "Exception " + e.getMessage());
			if(_log.isDebugEnabled())_log.debug("UCIP4xUrlConnection","Exception " + e.getMessage());
			throw e;
		}
		if(_log.isDebugEnabled())_log.debug("UCIP4xUrlConnection","Exiting");
	}
}
