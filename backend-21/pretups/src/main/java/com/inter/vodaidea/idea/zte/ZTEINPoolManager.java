...
public class ZTEINPoolManager {
	...
	public static HashMap<String,Vector<Object>> _freeBucket = new HashMap<String,Vector<Object>>();//Contains the list of free client objects,associated with interface id.
	public static HashMap<String,Vector<Object>> _busyBucket=new HashMap<String,Vector<Object>>();//Contains the list of busy client objects,associated with interface id.

	public static HashMap<String,Vector<Object>> _inactiveNodes=new HashMap<String,Vector<Object>>();//Contains the list of busy client objects,associated with interface id.
	...
	public static long _lastPoolInitializationTime=System.currentTimeMillis();//Defines the last initialization time when pool was initialized.
	...
	/**
	 * This method is responsible to store the instance of Client objects into a HashMap with interface id as Key.
	 * @param	String p_interfaceIDs
	 * @throws	BTSLBaseException
	 */
	public static void initialize(String p_interfaceIDs) throws BTSLBaseException
	{
		if(_logger.isDebugEnabled()) _logger.debug("initialize Entered "," p_interfaceIDs::"+p_interfaceIDs);
		...
		try 
		{
			...
			//Confirm while creating instances,if any error occurs for an interface,
			//should we stop the process with handling the event and throw exception
			...
			{
				//Create a list in which store the client object equal to configured number.
				interfaceId="";
				interfaceId = inStrArray[i].trim();
				String fileCacheId=interfaceId;
				try
				{
					MAX_IP = Integer.parseInt(FileCache.getValue(fileCacheId,"MAX_ACTIVE_NODES"));
					LAST_USED=0;
				...
				for(int node=1;node<=nodes;node++)
				{
					...
					try
					{
						//poolSize = Integer.parseInt(FileCache.getValue(fileCacheId,"MAX_ZTE_POOL_SIZE_"+interfaceId));
						String maxSocket=FileCache.getValue(fileCacheId,"MAX_ZTE_POOL_SIZE_"+interfaceIdNode);
						poolSize = Integer.parseInt(maxSocket);
					...
					try
					{
						for(int loop=0;loop<poolSize;loop++)
						{
							object=null;
							_zteINSocket=null;
							ZTEINHBLogger.logMessage("Going to create the connection for circle ="+interfaceIdNode+" connection number" +loop);
							try
							{
								_zteINSocket=new ZTEINSocket();
							
								object = new ZTEINNewClientConnection().getNewClientObject(fileCacheId,interfaceIdNode,_zteINSocket);
								if(object!=null)
							...
							catch(Exception e)
							{
								e.printStackTrace();
								_logger.error("initialize "," Exception e:"+e.getMessage());
								ZTEINHBLogger.logMessage("Exception Going to create the connection for circle ="+interfaceIdNode+" connection number" +loop+" object"+object);
								EventHandler.handle(EventIDI.SYSTEM_ERROR,interfaceIdNode,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PoolLoader[initialize]","","","","While instantiation of the Client objects got the Exception:"+e.getMessage());
								if(object!=null)
									_zteINSocket.setZteINSocketWrapper((ZTEINSocketWrapper)object);
						...
						_freeBucket.put(interfaceIdNode,objectList);
						_busyBucket.put(interfaceIdNode,new Vector<Object>(poolSize));
					}
					catch(Exception e)
					{
						ZTEINHBLogger.logMessage("Going to barred the interfaceId="+interfaceIdNode);						
						ZTEINStatus.getInstance().barredAir(interfaceIdNode);
					}
				}	//next node 

				ZTEHashLastUsed.put(interfaceId,LAST_USED);
				ZTEHashMaxIp.put(interfaceId,MAX_IP);
			}//end of while.
		...
		catch(Exception e)
		{
			...
			throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);//New Error Code-Confirm
		}//end of catch-Exception
		finally
		{
			_isPoolInitializing=false;
			if(_logger.isDebugEnabled()) _logger.debug("initialize"," Exited _freeBucket::"+_freeBucket);
		}//end of finally
	}//end of initialize
	...
	/**
	 * This method is used to return the Client object based on the Interface Id,this includes the following
	 * 1.Based on the interface id fetch the ArrayList of Pooled client object from the free bucket.
	 * 2.Check whether any pooled client-object is present in the ArrayList of free-bucket.
	 * 3.If present then remove the client-object from the ArrayList of freeBucket.
	 * 4.Add the removed client-object to the ArrayList of busyBucket.(At the other point of control the client object would be moved again to
	...
	{
		if(_logger.isDebugEnabled()) _logger.debug("getClientObject"," Entered p_interfaceID::"+p_interfaceID);
		...
		try
		{
			...
			//Getting the list of client objects from pooledMap based on the interfaceID.
			...
			if(_logger.isDebugEnabled()) _logger.debug("getClientObject before getting the client object ","  size of freeList.size():"+listSize +" and busyList.size()"+busyList.size());

			//If the client object is present then remove the client object from the free list and add it to busyList
			...
			busyList.add(socket);
			if(_logger.isDebugEnabled()) _logger.debug("getClientObject After getting the client object from freeList ","  size of freeList.size():"+freeList.size() +" and busyList.size()"+busyList.size());
			//It should be removed after the request has been completed(success or fail) In the Hander Class.
		...
		catch(Exception e)
		{
			...
			throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_FETCH_CLIENT_OBJECT);//First we have to decide the Key then chnage the key ERROR_CS3_WHILE_GETTING_SCHEDULER_OBJECT
		}//end of catch
		finally
		{
			if(_logger.isDebugEnabled()) _logger.debug("getClinetObject ","  Exited socket::"+socket);
		}//end of finally
		return socket;
	...
	/**
	 *This method is used to destroy the ClientObject references from freeBucket and busyBucket. 
	 *@param String p_interfaceId
	 *@return void
	 */
	public void destroy(String p_interfaceId)
	{
		if(_logger.isDebugEnabled()) _logger.debug("destroy Entered ","  p_interfaceId:"+p_interfaceId);
		int nodes=0;
		try 
		{
			Set ketSetCode=null;
							...
							if(interfaceId.equals(key.trim()))
							{
								...
								try
								{
									...
									if(objectList!=null)
									{
										...
										{
											ZTEINSocket socketWrapper =(ZTEINSocket) objectList.get(i);
											socketWrapper.getZteINSocketWrapper().destroy(fileCacheId);
										}
									}
								}
								catch(Exception e)
		...
		{
			if(_logger.isDebugEnabled()) _logger.debug("destroy"," Exiting");
		}//end of finally
	}//end of destroy
	/**
	 *This method is used to destroy the ALL ClientObject references from freeBucket and busyBucket. 
	 *@param String p_interfaceId
	 *@param String p_all
	 *@return void
	 */
	...
	{
		if(_logger.isDebugEnabled()) _logger.debug("destroy ","Entered p_ids="+p_interfaceId+" p_all="+p_all);
		try 
		{
