...
public class ZTEINHeartBeat implements Runnable
{
	...
	private void sentHeartBeat(ZTEINSocketWrapper p_socketConnection) throws BTSLBaseException
	{
		...
		if(_logger.isDebugEnabled()) _logger.debug("run"," Heartbeat message : " +_requestStr + "sent to IN at "+getCurrentTime()+" ,_heartBeatSleepTime :"+_heartBeatSleepTime);
	...
	private String getHeartBeatResponse(ZTEINSocketWrapper p_socketConnection) throws BTSLBaseException
	{
		try
		{
			...
			if(_logger.isDebugEnabled()) _logger.debug("sendRequestToIN "," reading message");                
			int c = 0;
			int cnt=0;
			...
			if(_logger.isDebugEnabled()) _logger.debug("run "," response received for heartbeat in  "+ timeDiff +" ms");
		}
		catch(IOException e)
		{
			throw new BTSLBaseException("IOException"+e.getMessage()); 
		...
		{	
			if(_logger.isDebugEnabled()) _logger.debug("run "," Heartbeat message : "+_requestStr + "sent to IN at "+getCurrentTime()+" ,_heartBeatSleepTime :"+_heartBeatSleepTime+"RESPONSE of the heart beat from the IN"+_responseStr);
