 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class ComverseUrlConnection extends BTSLConnection
{
    private HttpURLConnection _urlConnection = null;
    
    public ComverseUrlConnection(String p_url,int p_connectTimeout,int p_readTimeout,String p_keepAlive,long p_contentLength, String p_soapAction) throws Exception
    {
		if(_log.isDebugEnabled())_log.debug("ComverseUrlConnection","Entered p_url: "+p_url+"p_connecttimeout="+p_connectTimeout+" p_readtimeout="+p_readTimeout+" p_keepAlive="+p_keepAlive+" p_soapAction="+p_soapAction);
		try
		{
		...
		catch(Exception e)
		{
			...
			if(_log.isDebugEnabled())_log.debug("ComverseUrlConnection","Exception " + e.getMessage());
			throw e;
  		}
		if(_log.isDebugEnabled())_log.debug("ComverseUrlConnection","Exiting");
	}
    /**
    ...
    {
		try{if(_out!=null)_out.close();}catch(Exception e){_log.error("close", "Exception " + e.getMessage());}
		try{if(_in!=null)_in.close();}catch(Exception e){_log.error("close", "Exception " + e.getMessage());}
		try{if(_urlConnection!=null)_urlConnection.disconnect();}catch(Exception e){_log.error("close", "Exception " + e.getMessage());}
	}
	/**
   ...
   public String getResponseCode() throws IOException {
	...
	{
	    if(_log.isDebugEnabled()) _log.debug("setRequestHeader","Entered... p_contenetLength:"+p_contenetLength+", p_soapAction:"+p_soapAction+", p_keepAlive:"+p_keepAlive);
	    try
	    {
