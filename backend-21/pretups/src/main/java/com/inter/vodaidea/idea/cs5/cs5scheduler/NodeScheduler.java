...
public class NodeScheduler
{
    ...
    public NodeScheduler(String p_cs5INId) throws BTSLBaseException
    {
		...
		try
		{
	        ...
	        try
			{
	            _totalNodes = Integer.parseInt(FileCache.getValue(p_cs5INId,"MAX_ACTIVE_NODES").trim());
				if(_log.isDebugEnabled())_log.debug("NodeScheduler[constructor]","MAX_ACTIVE_NODES::"+_totalNodes);
			...
			for(int l=1;l<=_totalNodes;l++)
			{
			   try
			   {
				   ...
				   String ip = FileCache.getValue(p_cs5INId,"IP_"+l);
				    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","IP_"+l+"::"+ip);
					if(InterfaceUtil.isNullString(ip))
					{
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of IP_"+l +" in INFile "+p_cs5INId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_INITIALIZATION);
					}   
				    cs5NodeVO.setIP(ip.trim());
				    
				    String port = FileCache.getValue(p_cs5INId,"PORT_"+l);
				    if(_log.isDebugEnabled()) _log.debug("NodeScheduler[constructor]","PORT_"+l+"::"+port);
					if(InterfaceUtil.isNullString(port))
					{
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of IP_"+l +" in INFile "+p_cs5INId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_INITIALIZATION);
					}   
				    cs5NodeVO.setPort(port.trim());
					...
					{
					    e.printStackTrace();
					    EventHandler.handle(EventIDI.INTERFACE_REQUEST_EXCEPTION, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "NodeScheduler[constructor]", "INTERFACE_ID::"+_interfaceID,"", "", "Check the value of KEEP_ALIVE in INFile "+p_cs5INId);
					    throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_CS5_NODE_INITIALIZATION);
					}
					try
					{ 
