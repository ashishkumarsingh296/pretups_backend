...
public class ComverseINHandler implements InterfaceHandler
{
    ...
    static
    {
        if(_log.isDebugEnabled()) _log.debug("ComverseINHandler[static]","Entered");
        try
        {
        ...
        {
            if(_log.isDebugEnabled()) _log.debug("ComverseINHandler[static]","Exited");
        }
    }
    ...
    public void validate(HashMap p_requestMap) throws BTSLBaseException, Exception
    {
        ...
        try
        {
           ...
           /** VFI: Birendra: START: To Set the interface id in RequestFormatter. */
           _comverseRequestFormatter.setInterfaceID(_interfaceID);
           /** VFI: Birendra: END */
           _msisdn=(String)_requestMap.get("MSISDN");

           String validateRequired = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("REQ_SERVICE")+"_"+_requestMap.get("USER_TYPE"));
           if(PretupsI.NO.equals(validateRequired)){
           ...
           String multFactor = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),"MULT_FACTOR");
           ...
           /** VFI: Birendra: START: Commented on the recommendation of Abhay.Singh */
           
/*           inStr = _comverseRequestFormatter.generateRequest(ComverseI.ACTION_LANGUAGE_CODE,_requestMap);
...
*/		
           ...
           _requestMap.put("SERVICE_CLASS", _responseMap.get("COS"));
...
//added by Vikas for prepaid reversal
          /* String balance = (String)_responseMap.get("AvailableBalance");
            if(!InterfaceUtil.isNullString(balance) || !"0".equals(_responseMap.get("ErrorCode")))
            {
                _requestMap.put("AvailableBalance", balance);//Put the balance in requestMap
            }
            else
            {
                _log.error("sendRequestToIN","balance received from IN ::"+balance);
                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID,"Balance received from IN "+balance);
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
            }*/
//added by Vikas for prepaid reversal ends              

           _requestMap.put("CORE_RESP_BALANCE",_responseMap.get("CORE_RESP_BALANCE"));
	...
	public void credit(HashMap p_requestMap) throws BTSLBaseException, Exception
    {
        ...
        try
        {
        	...
        	String creditRequired = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("REQ_SERVICE")+"_"+_requestMap.get("USER_TYPE")+"_C");
            ...
            {
                _log.error(methodName,"MULT_FACTOR is not defined in the INFile");
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ComverseINHandler[credit]",_referenceID,_msisdn +" INTERFACE ID = "+(String)_requestMap.get("INTERFACE_ID"), (String) _requestMap.get("NETWORK_CODE"), "MULT_FACTOR  is not defined in the INFile");
                throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
            multFactor = multFactor.trim();
            ...
            try
            {
                ...
                double interfaceMRPDouble = Double.parseDouble((String)_requestMap.get("MRP"));
                /*interfaceMRPDouble = InterfaceUtil.getINAmountFromSystemAmountToIN(interfaceMRPDouble,multFactorDouble);
                mrpAmountStr=String.valueOf(interfaceMRPDouble);*/
                ...
                if(_log.isDebugEnabled()) {
                	_log.debug(methodName,"From file cache roundFlag := "+roundFlag);
                }
                //If the ROUND_FLAG is not defined in the INFile
                if(InterfaceUtil.isNullString(roundFlag))
                {
                    roundFlag=PretupsI.YES;
             //       EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.MINOR, "ComverseINHandler[credit]",_referenceID+"MSISDN = "+_msisdn ," INTERFACE ID = "+(String)_requestMap.get("INTERFACE_ID"), "Network code = "+(String) _requestMap.get("NETWORK_CODE"), "ROUND_FLAG is not defined in IN File hence system taken the default value=N.");
                }
                //If rounding of amount is allowed, round the amount value and put this value in request map.
                if(PretupsI.YES.equals(roundFlag.trim()))
                {
                    amountStr=String.valueOf(Math.round(systemAmtDouble));
                    _requestMap.put("INTERFACE_ROUND_AMOUNT",amountStr);
                    mrpAmountStr=String.valueOf(Math.round(interfaceMRPDouble));
                    _requestMap.put("MRP", mrpAmountStr);
                }
            }
            catch(Exception e)
            {
                _log.error(methodName,"Exception e:= "+e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ComverseINHandler[credit]","REFERENCE ID = "+_referenceID,_msisdn +" INTERFACE ID = "+(String)_requestMap.get("INTERFACE_ID"), (String) _requestMap.get("NETWORK_CODE"), "INTERFACE_AMOUNT  is not Numeric");
                throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.INTERFACE_HANDLER_EXCEPTION);
            }
			...
			if(!InterfaceUtil.isNullString((String)_requestMap.get("VALIDITY_DAYS")))
			{
			    try
			    {
			        ...
			        if(_log.isDebugEnabled()) {
			        	_log.debug(methodName,"validityDays:= "+validityDays);
			        }
			    }
			    catch(Exception e)
			    {
            ...
            //sending the Re-charge request to IN along with re-charge action defined in Comverse interface
        ...
        catch (BTSLBaseException be)
        {
            ...
            //to fire VAL request in case no response in TOP and compare the balance before and after
            ...
            {
                _log.error(methodName,"AMBIGOUS Exception e:= "+exc.getMessage());*/
                try{
         ...
         try
         {
		     ...
		     {
		    	 _requestMap.put("INTERFACE_ACTION","B");
	...
	public void creditAdjust(HashMap p_requestMap) throws BTSLBaseException, Exception
    {
        ...
        try
		{
			...
			//sending the CreditAdjust request to IN along with ACTION_IMMEDIATE_DEBIT action defined in Comverse interface
	...
	public void debitAdjust(HashMap p_requestMap) throws BTSLBaseException, Exception
    {
        ...
        try
		{
			...
			if(_log.isDebugEnabled()) {
				_log.debug(methodName,"transfer_amount:= "+amountStr+" multFactor:= "+multFactor);
			}
			//set transfer_amount in request map as amountStr (which is round value of INTERFACE_AMOUNT)
			_requestMap.put("transfer_amount", amountStr);
     ...
     * 3.If the VO is not NULL then pass the Node detail to ComverseUrlConnection class and get connection.
    ...
    private void sendRequestToIN(String p_inRequestStr, int p_action) throws BTSLBaseException
    {
        ...
        if(_log.isDebugEnabled()) {
        	_log.debug(methodName,"Entered p_inRequestStr:= "+p_inRequestStr+", p_action:= "+p_action);
        }
        ...
        try
        {
            ...
            try
            {
	            ...
	            if(_log.isDebugEnabled()){
	            	_log.debug(methodName,"Start time to find the scheduled Node startTimeNode:= "+startTimeNode+" miliseconds");
	            }
	            try
	            {
...
//	            	    soapAction="RetrieveSubscriberLite";
	            	    //soapAction = "SubscriberRetrieveLite";
	            	    soapAction=_requestMap.get("SOAP_VAL_ACTION").toString();
	            	    url=url+_requestMap.get("COMV_ONE_SOAP_VAL_ENDPOINT_URL").toString();
	            	    readTimeOut = Integer.parseInt(FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+"READ_TIMEOUT_VAL"));
	            	    retryCount=Integer.parseInt(FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+"RETRY_COUNT_VAL"));
	            	}
	            	else if(p_action == ComverseI.ACTION_RECHARGE_CREDIT)
	            	{
//	            	    soapAction="NonVoucherRecharge";
	            	   // soapAction="NonVoucherRechargeSubscriber";
	            		soapAction=_requestMap.get("SOAP_TOP_ACTION").toString();
	            		url=url+_requestMap.get("COMV_ONE_SOAP_TOP_ENDPOINT_URL").toString();
	            	    readTimeOut = Integer.parseInt(FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+"READ_TIMEOUT_CREDIT"));
	            	    retryCount=Integer.parseInt(FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+"RETRY_COUNT_CREDIT"));
	            	}
	            	else if(p_action == ComverseI.ACTION_IMMEDIATE_DEBIT)
	            	{
//	            	    soapAction="NonVoucherRecharge";
	            	    //soapAction="NonVoucherRechargeSubscriber";
	            		soapAction=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),"SOAP_ADJ_ACTION");
	            		url=url+_requestMap.get("COMV_ONE_SOAP_ADJ_ENDPOINT_URL").toString();
	            	    readTimeOut = Integer.parseInt(FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), _requestMap.get("NETWORK_CODE")+"_"+"READ_TIMEOUT_DEBIT"));
	            	    retryCount=Integer.parseInt(FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+"RETRY_COUNT_DEBIT"));
	            	}
	            	/*else if(p_action == ComverseI.ACTION_IMMEDIATE_CREDIT)
	            	{
//	            	    soapAction="NonVoucherRecharge";
	            	    soapAction="NonVoucherRechargeSubscriber";
	            	    readTimeOut = Integer.parseInt(FileCache.getValue(_interfaceID, _requestMap.get("NETWORK_CODE")+"_"+"READ_TIMEOUT_CREDIT"));
	            	    retryCount=Integer.parseInt(FileCache.getValue(_interfaceID,_requestMap.get("NETWORK_CODE")+"_"+"RETRY_COUNT_CREDIT"));
	            	}
	            	else if(p_action== ComverseI.ACTION_LANGUAGE_CODE)
	            	{
	            	    soapAction="RetrieveSubscriberWithIdentityNoHistory";
	            	    readTimeOut = Integer.parseInt(FileCache.getValue(_interfaceID, _requestMap.get("NETWORK_CODE")+"_"+"READ_TIMEOUT_VAL"));
	            	    retryCount=Integer.parseInt(FileCache.getValue(_interfaceID,_requestMap.get("NETWORK_CODE")+"_"+"RETRY_COUNT_VAL"));
	            	}*/
                    ...
                    if(_log.isDebugEnabled()) {
                    	_log.debug(methodName,"Connection of _interfaceID ["+(String)_requestMap.get("INTERFACE_ID")+"] for the Node Number ["+url+"] created");
                    }
                }
                catch(BTSLBaseException be)
                {
                ...
                if(_log.isDebugEnabled()){
                	_log.debug(methodName,"Total time to find the scheduled Node totalTimeNode := "+totalTimeNode);
                }
            }
            catch(BTSLBaseException be)
            {
...
//                    e.printStackTrace();
                    ...
                    /*
                     * sayyed yasin
                    mark transaction ambiguous for credit request
                    if there is a read time out exception
                     */                    
                    if( p_action == ComverseI.ACTION_RECHARGE_CREDIT
                    		&& (System.currentTimeMillis() - startTime)  > readTimeOut)
                    {
                    	_log.error(methodName,"Exception on NonVoucherRechargeSubscriber[ACTION_RECHARGE_CREDIT] request : "+ e.getMessage() +" Response received from IN :"+ buffer.toString());
                ...
                if (_log.isDebugEnabled()){
                	_log.debug(methodName, "responseStr:= " + responseStr);
                }
                ...
                //Check if there is no response, handle the event showing Blank response from Comverse stop further processing.
                if (InterfaceUtil.isNullString(responseStr))
                {
                    EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,(String)_requestMap.get("INTERFACE_ID"), EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Blank response from Comverse IN");
                ...
                String tempResponseStr=new String();
			 	...
			 	tempResponseStr=tempResponseStr + " referenceId="+_requestMap.get("TRANSACTION_ID");
				...
				tempResponseStr=tempResponseStr + " senderMSISDN="+_requestMap.get("SENDER_MSISDN");
			 	tempResponseStr=tempResponseStr + " Balance="+_responseMap.get("Balance");
			 	tempResponseStr=tempResponseStr + " Expire_Date="+_responseMap.get("Expire_Date");
			 	tempResponseStr=tempResponseStr + " State="+_responseMap.get("State");
			 	tempResponseStr=tempResponseStr + " ServiceClass="+_responseMap.get("COS");
				tempResponseStr=tempResponseStr + " ErrorCode="+_responseMap.get("ErrorCode");
			 	tempResponseStr=tempResponseStr + " faultCode="+_responseMap.get("faultCode");
			 	tempResponseStr=tempResponseStr + " faultString="+_responseMap.get("faultString");
			 	tempResponseStr=tempResponseStr+" }";

                InterfaceRequestResponseLog.log(url,_requestMap,p_inRequestStr,tempResponseStr);
                ...
                {/*
                    _requestMap.put("INTERFACE_STATUS", errorCode);//Put the interface_status in requestMap
                  //  Object [] errorList=null;
                	if(ComverseI.SUBSCRIBER_NOT_FOUND.equalsIgnoreCase(errorCode))
                    {
                	...
                	else if(ComverseI.VOUCHER_RECHARGE_ERROR_75.equalsIgnoreCase(errorCode))
                	{
                		_log.error(methodName,"InValid Amount "+_requestMap.get("AMOUNT"));
                        EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Subscriber is not found at IN");
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
                	}
                	else if(errorCode.startsWith(ComverseI.VOUCHER_RECHARGE_ERROR_))
                	{
                		_log.error(methodName,"Voucher Recharge Error");
                        EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,_interfaceID, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Subscriber is not found at IN");
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
                    ...
                    {
                        _log.error(methodName,"Error code received from IN := "+errorCode);
                ...
                */
                ...
                if(p_action == ComverseI.ACTION_ACCOUNT_DETAILS)
                {
                	...
                	successList=_requestMap.get("Comv_Soap_One_Rchg_State").toString().split("\\^");
                	if(!Arrays.asList(successList).contains(responseCode))
                    {
                        _log.error(methodName,"Subscriber barred at IN:= "+_msisdn);
                        EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,(String)_requestMap.get("INTERFACE_ID"), EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Subscriber BARRED at IN");
                ...
                /*else if(p_action== ComverseI.ACTION_ACCOUNT_DETAILS)
            	{
                	responseCode = (String)_responseMap.get("State");
            		...
            		responseCode = _responseMap.get("errorCode").toString();
//            		successList=ComverseI.RESULT_OK.split(",");
            		successList=_requestMap.get("Comv_Soap_NonVoucher_Success_Res").toString().split(",");
...
//                e.printStackTrace();
        ...
        finally
        {
            try
            {
                ...
                if (comverseURLConnection != null) comverseURLConnection.close();
            }
            catch (Exception e)
            {
//                e.printStackTrace();
                _log.error(methodName, "While closing Comverse IN Connection Exception e:= " + e.getMessage());
            ...
            if (_log.isDebugEnabled())
                _log.debug(methodName, "Exiting  _interfaceID:= "+(String)_requestMap.get("INTERFACE_ID")+", Stage:= "+p_action + ", responseStr:= " + responseStr);
        }
    ...
    /**
        ...
        private void setInterfaceParameters() throws Exception,BTSLBaseException
        {
            ...
            try
            {
	            ...
	            String warnTimeStr=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "WARN_TIMEOUT");
                ...
                String currency=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "CURRENCY");
                ...
                String stateAllowed = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+"Comv_Soap_One_Rchg_State");
                ...
                _requestMap.put("Comv_Soap_One_Rchg_State",stateAllowed.trim());
                
                String valAction=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "SOAP_VAL_ACTION");
                ...
                String topAction=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "SOAP_TOP_ACTION");
                ...
                String adjAction=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "SOAP_ADJ_ACTION");
                ...
                /*String id=FileCache.getValue(_interfaceID,_requestMap.get("NETWORK_CODE")+"_"+ "COMV_INIT_ID");
                ...
                /*String pwd=FileCache.getValue(_interfaceID, _requestMap.get("NETWORK_CODE")+"_"+"COMV_INIT_PASSWORD");
                ...
                String url=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+ "COMV_SOAP_URL");
                ...
                String keepAlive=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "KEEP_ALIVE");
                ...
                String valEndPoint=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "COMV_ONE_SOAP_VAL_ENDPOINT_URL");
                if(InterfaceUtil.isNullString(valEndPoint))
                {
                	EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "ComverseINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "COMV_ONE_SOAP_VAL_ENDPOINT_URL is not defined in IN File ");
                	throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("COMV_ONE_SOAP_VAL_ENDPOINT_URL",valEndPoint.trim());
                
                
                String topEndPoint=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "COMV_ONE_SOAP_TOP_ENDPOINT_URL");
                if(InterfaceUtil.isNullString(topEndPoint))
                {
                	EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "ComverseINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "COMV_ONE_SOAP_TOP_ENDPOINT_URL is not defined in IN File ");
                	throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("COMV_ONE_SOAP_TOP_ENDPOINT_URL",topEndPoint.trim());
                
                
                
                String adjEndPoint=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "COMV_ONE_SOAP_ADJ_ENDPOINT_URL");
                if(InterfaceUtil.isNullString(adjEndPoint))
                {
                	EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "ComverseINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "COMV_ONE_SOAP_ADJ_ENDPOINT_URL is not defined in IN File ");
                	throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("COMV_ONE_SOAP_ADJ_ENDPOINT_URL",adjEndPoint.trim());
                ...
                String connectTimeout=FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "CONNECT_TIME_OUT");
             ...
             /*   String readTimeout=FileCache.getValue(_interfaceID, "READ_TIME_OUT");
                ...
                {
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO, "ComverseINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "READ_TIME_OUT is not defined in IN File ");
                    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("READ_TIME_OUT",readTimeout.trim());*/
                
                /** VFI: Birendra: 24April15 */
                String NonVoucherSuccessAlwd = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "Comv_Soap_NonVoucher_Success_Res");
                if(InterfaceUtil.isNullString(NonVoucherSuccessAlwd))
                {
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ComverseINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "Comv_Soap_NonVoucher_Success_Res is not defined in IN File ");
                    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("Comv_Soap_NonVoucher_Success_Res",NonVoucherSuccessAlwd.trim());
                /** VFI: Birendra: 24April15 */
                
                String balancename = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),_requestMap.get("NETWORK_CODE")+"_"+ "BALANCE_NAME");
                if(InterfaceUtil.isNullString(balancename))
                {
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "ComverseINHandler[setInterfaceParameters]","REFERENCE ID = "+_referenceID+"MSISDN = "+_msisdn," INTERFACE ID = "+_interfaceID, "Network code "+(String) _requestMap.get("NETWORK_CODE") , "balance name is not defined in IN File ");
                    throw new BTSLBaseException(this,methodName,InterfaceErrorCodesI.ERROR_CONFIG_PROBLEM);
                }
                _requestMap.put("BALANCE_NAME",balancename.trim());
        ...
        private void setLanguageFromMapping() throws Exception
        {
            ...
            try
            {
                ...
                String mappingString = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"), "LANGUAGE_MAPPING");
        ...
        private void handleCancelTransaction() throws BTSLBaseException
        {
            ...
            try
            {
                _requestMap.put("REMARK1",FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),"REMARK1"));
                _requestMap.put("REMARK2",FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),"REMARK2"));
                //get reconciliation log object associated with interface
                reconLog = ReconcialiationLog.getLogObject((String)_requestMap.get("INTERFACE_ID"));
                ...
                if("N".equals(cancelTxnAllowed))
                {
                    ...
                    if(!InterfaceErrorCodesI.SUCCESS.equals(cancelTxnStatus))
                    	throw new BTSLBaseException(this,"handleCancelTransaction",cancelTxnStatus);  ////Based on the value of SYSTEM_STATUS mark the transaction as FAIL or AMBIGUOUS to the system.
                    _requestMap.put("TRANSACTION_STATUS",InterfaceErrorCodesI.SUCCESS);
    	...
    	{
    	    if(_log.isDebugEnabled()) _log.debug("getInterfaceAmount","Entered p_map = "+p_map);
    	    ...
    	    {
    	        if(_log.isDebugEnabled())_log.debug("getInterfaceAmount","Exiting interfaceAmountStr = "+interfaceAmountStr);
    	    }
        ...
        {
            if (_log.isDebugEnabled())_log.debug("balanceEnquiryForAmbigousTransaction", "Entered.");
            try
            {
            ...
            {
            	if (_log.isDebugEnabled())_log.debug("balanceEnquiryForAmbigousTransaction", "Exited");
            }
        }
        ...
        {
        	final String methodName = "postCreditEnquiry";
            if(_log.isDebugEnabled()){
            	_log.debug(methodName,"Entered in "+ methodName+ " p_requestMap:= "+p_requestMap);
            }
            ...
            try
            {
                   ...
                   for(int i=0; i < removePrefixList.length; i++){
                           String removePrefix = removePrefixList[i];
                                   if(_msisdn.startsWith(removePrefix)){
                                           _msisdn = _msisdn.substring(_msisdn.indexOf(removePrefix) + removePrefix.length());
               ...
               _requestMap.put("IN_TXN_ID",_inTXNID);
               _referenceID=(String)_requestMap.get("TRANSACTION_ID");
               multFactor = FileCache.getValue((String)_requestMap.get("INTERFACE_ID"),"MULT_FACTOR");
   			...
   			if(PretupsI.YES.equals(_requestMap.get("ENQ_POSTBAL_IN")))
			{
               ...
               try{
            	   ...
            	   _requestMap.put("NEW_GRACE_DATE", InterfaceUtil.getInterfaceDateFromDateString(((String) _responseMap.get("Expire_Date")).trim(), "yyyy-MM-dd"));
               }
               catch(Exception e){
            	   if(_log.isDebugEnabled()) {
                   	_log.debug(methodName,"Handled Expire_Date value.");
                   }
               }
               ...
               _requestMap.put("LMB_ALLOWED_VALUE","1");
               // Added By Birendra
               _requestMap.put("POST_BALANCE_ENQ_SUCCESS",PretupsI.YES);
            //   _requestMap.put("INTERFACE_POST_BALANCE", PretupsBL.getSystemAmount(_responseMap.get("availableBalance").toString()));
               _requestMap.put("INTERFACE_POST_BALANCE", String.valueOf(PretupsBL.getSystemAmount(_responseMap.get("Balance").toString())));
			}
   			else
   			{
...
//				    e.printStackTrace();
        ...
        public void mapError (String errorCodeStr,int p_action) throws BTSLBaseException
    	{
    		String methodName="mapError"; 
        	if(errorCodeStr.equalsIgnoreCase("CANNT_FIND_EXTERNALID")){
    			/*topUpVO.setStatusCode("404");
    			topUpVO.setStatusComments("Subscriber Not Found");*/
    			
    			_log.error(methodName,"Subscriber not found with MSISDN := "+_msisdn);
                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,(String)_requestMap.get("INTERFACE_ID"), EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Subscriber is not found at IN");
                throw new BTSLBaseException(InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND);
    		}
        	//Abhilasha Check it for success
    		else if(errorCodeStr.startsWith("VOUCHER_RECHARGE_SUCCESS_WITH_ERROR_") || errorCodeStr.equals("0")){
    			return;
    		}
    		else if(errorCodeStr.equalsIgnoreCase("VOUCHER_RECHARGE_ERROR_75")){
    			/*topUpVO.setStatusCode("471");
    	    		topUpVO.setStatusComments("InValid Amount");	*/
    			_log.error(methodName,"Invalid Amount");
                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,(String)_requestMap.get("INTERFACE_ID"), EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Invalid Amount");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
    		}
    		else if(errorCodeStr.startsWith("VOUCHER_RECHARGE_ERROR_")){
    			/*topUpVO.setStatusCode("453");
    			topUpVO.setStatusComments("Voucher Recharge Error");*/
    			_log.error(methodName,"Voucher Recharge Error");
                EventHandler.handle(EventIDI.INTERFACE_INVALID_RESPONSE,(String)_requestMap.get("INTERFACE_ID"), EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.INFO,"ComverseINHandler[sendRequestToIN]",_referenceID,_msisdn, (String) _requestMap.get("NETWORK_CODE")+" INTERFACE ID = "+_interfaceID+" Stage = "+p_action,"Voucher Recharge Error");
                throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_RESPONSE);
    		}
    		else{
    			/*topUpVO.setStatusCode("412");
    			topUpVO.setStatusComments("Other Internal Error");*/
    			
    			_log.error(methodName,"Other Internal Error");
