...
public class TibcoVMClient {
    private static Log _log = LogFactory.getLog(TibcoVMClient.class.getName());
    ...
    public static void main(String args[]) {
        ...
        try {
            Constants.load("C:\\Abhay\\PROJECTS\\pretups_kenya\\src\\configfiles\\Constants.props");

            // System.out.println("INTERFACE_DIRECTORY===="+Constants.getProperty("INTERFACE_DIRECTORY"));
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("TibcoVMTestClient.main() while loading Constants.props Exception e=" + e.getMessage());
        }
        try {
            ...
            e.printStackTrace();
            System.out.println("TibcoVMTestClient.main() while loading IN File Exception e=" + e.getMessage());
        }
        // System.out.println("TibcoVMTestClient.main() before IN requestMap="+requestMap);

        sendRequesttoIN(requestMap);

        System.out.println("TibcoVMTestClient.main() after IN requestMap=" + requestMap);

        System.out.println("TibcoVMTestClient.main() End...########################");
    }

    public static void sendRequesttoIN(HashMap p_requestMap) {
        if (_log.isDebugEnabled())
            _log.debug("TibcoVMTestClient.sendRequesttoIN()", "Entered...");
        // System.out.println("TibcoVMTestClient.sendRequesttoIN() Entered...");

        TibcoVMINHandler inHandler = new TibcoVMINHandler();
        try {
            ...
            } else if (p_requestMap.get("ACTION").toString().equalsIgnoreCase("3")) {
                inHandler.debitAdjust(p_requestMap);
            } else if (p_requestMap.get("ACTION").toString().equalsIgnoreCase("4")) {
                inHandler.creditAdjust(p_requestMap);
            }
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("TibcoVMTestClient.sendRequesttoIN() Exception e=" + e.getMessage());
        }
        if (_log.isDebugEnabled())
            _log.debug("TibcoVMTestClient.sendRequesttoIN()", "Exited...");
        // System.out.println("TibcoVMTestClient.sendRequesttoIN() Exited...");
    }
