 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class TibcoVMUrlConnection extends BTSLConnection {
    private HttpURLConnection _urlConnection = null;

    public TibcoVMUrlConnection(String p_url, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength, String p_soapAction) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("TibcoVMUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive + " p_soapAction=" + p_soapAction);
        try {
            ...
            e.printStackTrace();
            _log.error("TibcoVMUrlConnection", "Exception " + e.getMessage());
            if (_log.isDebugEnabled())
                _log.debug("TibcoVMUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("TibcoVMUrlConnection", "Exiting");
    }
