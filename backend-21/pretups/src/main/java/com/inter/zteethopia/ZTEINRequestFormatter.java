 ...
 *                           --------------------------------------------------
 *                           ---------------------------------------------
 *                           This class is responsible to generate the request
 *                           and parse the response for the ZTE interface.
 */
public class ZTEINRequestFormatter {
    ...
    public String generateRequest(String fileCacheId, int p_action, HashMap p_map) throws BTSLBaseException, Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("generateRequest", " Entered p_action=" + p_action + " map: " + p_map);
        String str = null;
        ...
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateRequest", " Exited Request String: str=" + str);
        }// end of finally
    ...
    public HashMap<String, String> parseResponse(int p_action, String p_responseStr) throws BTSLBaseException, Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("parseResponse", " Entered p_action=" + p_action + " p_responseStr=" + p_responseStr);
        ...
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("parseRespons", " Exiting map: " + map);
        }// end of finally
        return map;
    ...
    private String generateGetAccountInfoRequest(String fileCacheId, HashMap p_map) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("generateGetAccountInfoRequest", " Entered p_map= " + p_map);
        ...
        try {
            ...
            headerInfoBuffer.append(spacePad(acntInfoService, 8, " ", 'r'));// Account
                                                                            // Info
                                                                            // service
            ...
            headerInfoBuffer.append(spacePad((String) p_map.get("TRANSACTIONHEADERID"), 8, "0", 'l'));// txn
                                                                                                      // id
                                                                                                      // in
                                                                                                      // header
                                                                                                      // of
            ...
            requestDataBuffer.append("DN=" + msisdn + ",");
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateGetAccountInfoRequest", " Exiting  requestStr:" + requestStr);
        }
        return requestStr;
    }
    ...
    private HashMap<String, String> parseGetAccountInfoResponse(String p_responseStr) throws Exception {
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("parseGetAccountInfoResponse", " Entered p_responseStr: " + p_responseStr);
        HashMap<String, String> map = null;
        int index = 0;
        String status = null;
        String attr = null;
        ...
        try {
            ...
            transactionId = getResponseTransactionId(p_responseStr);// get
                                                                    // transactionId
            map.put("IN_TRANSACTIONHEADERID", transactionId);// put
                                                             // transactionId in
                                                             // map

            if (map.get("RETN").equals("0000")) {
                if (!ZTEINI.RESULT_OK.equals(status))// if status is not OK then
                                                     // return else proceed with
                                                     // other values
                    return map;

                index = p_responseStr.indexOf("ACNTSTAT=");
                attr = p_responseStr.substring(index + 9, p_responseStr.indexOf(",", index));// get
                                                                                             // parameters
                                                                                             // queried
                                                                                             // in
                                                                                             // validate
                ...
                map.put("ACNTSTAT", attr);
                index = p_responseStr.indexOf("BLACKLIST=");
                attr = p_responseStr.substring(index + 10, p_responseStr.indexOf(",", index));// get
                                                                                              // parameters
                                                                                              // queried
                                                                                              // in
                                                                                              // validate
                                                                                              ...
                                                                                              // '&'.
                                                                                              // (SUBCOSID&SERVICESTOP&ACTIVESTOP&BALANCE&LANGUAGETYPE&FRAUDLOCK&SUSPENDSTOP)
                map.put("BLACKLIST", attr);
                // result=p_responseStr.substring(index+8,p_responseStr.indexOf("\"",index+8));//get
                // values of parameters queried in validate separated by
                // '|'(1|1|1|1|1|1|1|1|)
                index = p_responseStr.indexOf("BRANDINDEX=");
                attr = p_responseStr.substring(index + 11, p_responseStr.length() - 8);// get
                                                                                       // parameters
                                                                                       // queried
                                                                                       // in
                                                                                       // validate
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("parseGetAccountInfoResponse", " Exit  map:" + map);
        }
        return map;
    }
    ...
    private String generateRechargeCreditRequest(String fileCacheId, HashMap<String, String> p_map) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("generateRechargeCreditRequest", " Entered p_map= " + p_map);
        ...
        try {
            ...
            if (InterfaceUtil.isNullString(rptflag)) {
                _logger.error("generateAccountInfoRequest", "Value of RPTFLAG is not defined in the INFile");
                // EventHandler.handle(EventIDI.SYSTEM_ERROR,
                // EventComponentI.INTERFACES, EventStatusI.RAISED,
                // EventLevelI.FATAL,
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "DLGCON is not defined in the INFile.");
                throw new BTSLBaseException(this, "setInterfaceParameters", InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            ...
            processingfee = (String) p_map.get("REQUESTED_AMOUNT");
            adddays = (String) p_map.get("VALIDITY_DAYS");

            StringBuffer headerInfoBuffer = new StringBuffer(versionNumber);// Version-number

            headerInfoBuffer.append(spacePad(term, 8, " ", 'l'));// TERMINAL
            headerInfoBuffer.append(spacePad(serviceName, 8, " ", 'r'));// Account
                                                                        // Info
                                                                        // service
            headerInfoBuffer.append(spacePad(sessionID, 8, "0", 'l'));// session
                                                                      // id
            headerInfoBuffer.append(dlgCon);// represents session continued
            headerInfoBuffer.append(spacePad(rsv, 4, "0", 'l'));// reserve
            headerInfoBuffer.append(spacePad(transactionID, 8, "0", 'l'));// txn
                                                                          // id
                                                                          // in
                                                                          // header
                                                                          // of
            ...
            rechargeDataBuffer.append("DN=" + msisdn + ",");
            rechargeDataBuffer.append("FEE=" + processingfee + ",");
            rechargeDataBuffer.append("DAYS=" + adddays + ",");
            rechargeDataBuffer.append("RPTFLAG=" + rptflag);
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateRechargeCreditRequest ", "Exiting  requestStr:" + requestStr);
        }
        return requestStr;
    }
    ...
    private HashMap<String, String> parseRechargeCreditResponse(String p_responseStr) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("parseRechargeCreditResponse ", "Entered p_responseStr: " + p_responseStr);
        HashMap<String, String> map = null;
        int index = 0;
        String status = null;
        String transactionId = null;
        ...
        try {
            ...
            if (map.get("RETN").equals("0000")) {
                index = p_responseStr.indexOf("BEFOROPER=");
                if (index < 0)
                    throw new BTSLBaseException(this, "parseRechargeCreditResponse", InterfaceErrorCodesI.ERROR_RESPONSE);

                map.put("accountPrevBalance", (p_responseStr.substring(index + 10, p_responseStr.indexOf(",", index))).trim());
                index = p_responseStr.indexOf("FEE=");
                if (index < 0)
                    throw new BTSLBaseException(this, "parseRechargeCreditResponse", InterfaceErrorCodesI.ERROR_RESPONSE);
                index = p_responseStr.indexOf("AFTEROPER=");
                if (index < 0)
                    throw new BTSLBaseException(this, "parseRechargeCreditResponse", InterfaceErrorCodesI.ERROR_RESPONSE);
                map.put("accountPostBalance", (p_responseStr.substring(index + 10, p_responseStr.length() - 8)));
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("parseRechargeCreditResponse", " Exit  map:" + map);
    ...
    private String generateLoginRequest(String fileCacheId, HashMap<String, String> p_map) throws BTSLBaseException, Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("generateLoginRequest ", "Entered p_map: " + p_map);
        ...
        try {
            ...
            if (BTSLUtil.isNullString(transactionID)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "ACCOUNT_INFO_SERIVICE is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            ...
            if (BTSLUtil.isNullString(dlgCtrl)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "DLGCTRL is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            ...
            if (BTSLUtil.isNullString(tsrv)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "TSRV is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateLoginRequest ", "login command after putting password and user name loginCommand:" + loginCommand);
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateLoginRequest", " Exiting  requestStr:" + requestStr);
        }
        return requestStr;
    }
    ...
    private HashMap<String, String> parseLoginResponse(String p_responseStr) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("parseLoginResponse", " Entered p_responseStr: " + p_responseStr);
        HashMap<String, String> map = null;
        int index = 0;
        String status = null;
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("parseLoginResponse ", "status:" + status);
        } catch (Exception e) {
            _logger.error("parseLoginResponse ", "Exception e:" + e.getMessage());
            throw e;
        } finally {
            if (_logger.isDebugEnabled())
                _logger.debug("parseLoginResponse", " Exiting map:" + map);
        }
        return map;
    }
    ...
    private String generateLogoutRequest(String fileCacheId, HashMap<String, String> p_map) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("generateLogoutRequest", " Entered p_map: " + p_map);
        ...
        try {
            ...
            if (BTSLUtil.isNullString(startFlag)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "START_FLAG is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            p_map.put("START_FLAG", startFlag.trim());
            ...
            if (BTSLUtil.isNullString(versionNumber)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "VERSION_NUMBER is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            p_map.put("VERSION_NUMBER", versionNumber.trim());
            ...
            if (BTSLUtil.isNullString(term)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "TERM is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            p_map.put("TERM", term.trim());
            ...
            if (BTSLUtil.isNullString(rsv)) {
                ...
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
                // , (String) _requestMap.get("NETWORK_CODE"),
                // "RSV is not defined in the INFile.");
                throw new BTSLBaseException("setInterfaceParameters" + InterfaceErrorCodesI.ERROR_BAD_REQUEST);
            }
            p_map.put("RSV", rsv.trim());
            ...
            dlgCon = FileCache.getValue(fileCacheId, "DLGCON");

            if (BTSLUtil.isNullString(dlgCon)) {
                _logger.error("setInterfaceParameters ", "Value of DLGCON is not defined in the INFile");
                // EventHandler.handle(EventIDI.SYSTEM_ERROR,
                // EventComponentI.INTERFACES, EventStatusI.RAISED,
                // EventLevelI.FATAL,
                // "HuaweiINHandler[setInterfaceParameters]",_referenceID,"INTERFACE ID"+_interfaceID+" MSISDN "+_msisdn
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateLogoutRequest ", "logout command after putting password and user name logoutCommand:" + logoutCommand);
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateLogoutRequest ", "Exiting  requestStr:" + requestStr);
        }
        return requestStr;
    }
    ...
    private HashMap<String, String> parseLogoutResponse(String p_responseStr) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("parseLogoutResponse ", "Entered p_responseStr: " + p_responseStr);
        HashMap<String, String> map = null;
        int index = 0;
        ...
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("parseLogoutResponse", " response_status:" + response_status);
        } catch (Exception e) {
            _logger.error("parseLogoutResponse ", "Exception e:" + e.getMessage());
            throw e;
        } finally {
            if (_logger.isDebugEnabled())
                _logger.debug("parseLogoutResponse", " Exiting map:" + map);
        }
        return map;
    }
    ...
    private String generateHeartBeatRequest(HashMap<String, String> p_map) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("generateHeartBeatRequest", " Entered p_map= " + p_map);
        String requestStr = null;
        String requestData = null;
        String startFlag = null;
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("generateHeartBeatRequest ", "Exiting  requestStr:" + requestStr);
        }
        return requestStr;
    }
    ...
    private String getCheckSum(String p_headerAndDataString) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("getCheckSum ", "Entered p_headerAndDataString:" + p_headerAndDataString);
        int[] c = null;
        int index = 0;
        ...
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("getCheckSum ", "Exited");
        }
    ...
    private String getResponseTransactionId(String p_responseStr) throws Exception {
        if (_logger.isDebugEnabled())
            _logger.debug("getResponseTransactionId ", "Entered p_responseStr:" + p_responseStr);
        int index = 0;
        ...
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("getResponseTransactionId ", "Exited transIdStr:" + transIdStr);
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            String t_responseStr = "`SC`00801.00CCB00000PPS     00000000DLGCONFFFF00000001TXEND FFFFACK:RECHARGE PPS:RETN=0006, DESC=\"The dialog has not been connected.\"   BE86FA91";
            // String
            // t_responseStradjust="`SC`00E41.00CCB00000PPS     00000000DLGCONFFFF10000001TXEND FFFFACK:RECHARGE PPS:RETN=0000, DESC=\"Success\", LOYALTY=0, SERIALNO=PB0000000032, ACCOUNT=5000, TopupAmount=4333, SOURCE=VTOPUPKA1062217580136, ProcessingFee=0, Servicetax=0F3F6C3F";
