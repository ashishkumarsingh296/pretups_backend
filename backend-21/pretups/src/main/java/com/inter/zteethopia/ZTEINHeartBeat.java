...
public class ZTEINHeartBeat implements Runnable {
            ...
            try {
                ...
                try {
                    if (!noFreeConnection && socketConnected) {
                        try {
                            ZTEINHBLogger.logMessage("HB Request For Interface Id" + _interfaceID + ",[Request]" + _requestStr);
                            sentHeartBeat(_socketConnection);
                            String hbresponse = getHeartBeatResponse(_socketConnection);
                            ZTEINHBLogger.logMessage("HB Response For Interface Id" + _interfaceID + ",[Response]" + hbresponse);
                            ...
                            try {
                                busyList.remove(_socket);
                                if (_socketConnection != null) {
                                    ZTEINHBLogger.logMessage("HB Request For Interface Id" + _interfaceID + ",[Error] destroy the close object");
                            ...
                            try {
                                ...
                                busyList.add(_socket);
                                if (logsEnable)
                                    ZTEINHBLogger.logMessage("CHECK 4 For Interface Id" + _interfaceID + ",[busyList]=" + busyList.toString());
                            }
    ...
    private void sentHeartBeat(ZTEINSocketWrapper p_socketConnection) throws BTSLBaseException {
        try {
            _out = p_socketConnection.getPrintWriter();// getPrintWriter returns
                                                       // the Object of
                                                       // OutPutStream.
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("run", " Heartbeat message : " + _requestStr + "sent to IN at " + getCurrentTime() + " ,_heartBeatSleepTime :" + _heartBeatSleepTime);
    ...
    private String getHeartBeatResponse(ZTEINSocketWrapper p_socketConnection) throws BTSLBaseException {
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("sendRequestToIN ", " reading message");
            int c = 0;
            int cnt = 0;
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("run ", " response received for heartbeat in  " + timeDiff + " ms");
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("run ", " Heartbeat message : " + _requestStr + "sent to IN at " + getCurrentTime() + " ,_heartBeatSleepTime :" + _heartBeatSleepTime + "RESPONSE of the heart beat from the IN" + _responseStr);
