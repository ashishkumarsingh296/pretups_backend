...
public class ZTEINHeartBeatController {
    ...
    public ZTEINHeartBeatController(String p_poolIDs) {
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("ZTEINHeartBeatController[constructor]", " Entered p_poolIDs:" + p_poolIDs);
        try {
            int nodes = 0;
            ...
            _heartBeatThreadMap = new HashMap<String, ZTEINHeartBeat>(inStrArray.length);// initialize
                                                                                         // the
                                                                                         // Map
                                                                                         // size
                                                                                         // equal
            ...
            for (int i = 0, size = inStrArray.length; i < size; i++) {
                ...
                for (int node = 1; node <= nodes; node++) {
                    ...
                    interfaceID = inStrArray[i] + "_" + node;
                    _heartBeatThreadMap.put(interfaceID.trim(), new ZTEINHeartBeat(fileCacheId, interfaceID));
        ...
        try {
            ...
            for (int i = 0, size = inStrArray.length; i < size; i++) {
                ...
                for (int node = 1; node <= nodes; node++) {
                    ...
                    interfaceID = inStrArray[i] + "_" + node;
                    ZTEINHBLogger.logMessage("inside the Loop" + node + " interfaceID=" + interfaceID + " _heartBeatThreadMap" + _heartBeatThreadMap.toString());
                    if (_heartBeatThreadMap != null && _heartBeatThreadMap.containsKey(interfaceID)) {

                        zteHearBeat = (ZTEINHeartBeat) _heartBeatThreadMap.remove(interfaceID);
