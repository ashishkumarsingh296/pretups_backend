 ...
 *                        ------------------------------------------------------
 *                        ------------------------------------------
 *                        This class would be responsible to provide the Wrapper
 *                        of SocketConnection class.
 */
public class SocketWrapper extends Socket {
    ...
    public SocketWrapper(String p_interfaceId) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("SocketWrapper[constructor]", "Entered p_interfaceId::" + p_interfaceId);
        ...
        try {
            ...
            if (_log.isDebugEnabled())
                _log.debug("SocketWrapper[constructor]", "_sessionId :: " + _sessionId);
            // Fetch the IP address from the INFile.
            String ip = FileCache.getValue(p_interfaceId, "SOCKET_IP");
            if (_log.isDebugEnabled())
                _log.debug("SocketWrapper[constructor]", "ip :: " + ip);
            if (InterfaceUtil.isNullString(ip)) {
                _log.error("SocketWrapper[constructor]", "SOCKET_IP is not defined in the INFile");
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "SocketWrapper[constructor]", "", "_interfaceID:" + _interfaceID, "", "SOCKET_IP is not defined in the INFile");
                throw new BTSLBaseException(this, "SocketWrapper[constructor]", InterfaceErrorCodesI.ERROR_INIT_SOCKET_CONNECTION);
            }
            // Fetch the Port from the INFile.
            String portStr = FileCache.getValue(p_interfaceId, "SOCKET_PORT");
            if (_log.isDebugEnabled())
                _log.debug("SocketWrapper[constructor]", "portStr :: " + portStr);
            if (InterfaceUtil.isNullString(portStr) || !InterfaceUtil.isNumeric(portStr.trim())) {
                _log.error("SocketWrapper[constructor]", "SOCKET_PORT is either not defined in the INFile or its value is not numeric");
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "SocketWrapper[constructor]", "", "_interfaceID:" + _interfaceID, "", "SOCKET_PORT is either not defined in the INFile or its value is not numeric");
                throw new BTSLBaseException(this, "SocketWrapper[constructor]", InterfaceErrorCodesI.ERROR_INIT_SOCKET_CONNECTION);
            }
            ...
            if (InterfaceUtil.isNullString(portStr) || !InterfaceUtil.isNumeric(conTimeOutStr.trim())) {
                _log.error("SocketWrapper[constructor]", "SOCKET_TIMEOUT is either not defined in the INFile or its value is not numeric");
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "SocketWrapper[constructor]", "", "_interfaceID:" + _interfaceID, "", "SOCKET_PORT is not defined in the INFile");
                throw new BTSLBaseException(this, "SocketWrapper[constructor]", InterfaceErrorCodesI.ERROR_INIT_SOCKET_CONNECTION);
            }

            // String conTimeOutStr = "30000";
            ...
            String loginReqStr = _formatter.generateRequest(HuaweiI.ACTION_LOGIN, _requestMap);
            ...
            // String loginReqStr =
            // _formatter.generateRequest(HuaweiI.ACTION_LOGIN,_requestMap);
            // String loginReqStr = "LOGIN:PSWD=TELLIN,USER=vpnadm;";
            ...
            _responseMap = _formatter.parseResponse(HuaweiI.ACTION_LOGIN, loginRespStr, null);
            // If the login is successfull then the response map would contain
            ...
            if (!HuaweiI.RESULT_OK.equals(status)) {
                _log.error("SocketWrapper[constructor]", "Login response from IN is not OK, status:" + status);
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "SocketWrapper[constructor]", "", "_interfaceID:" + _interfaceID, "", "Login response from IN is NOT OK, status:" + status);
                throw new BTSLBaseException(this, "SocketWrapper[constructor]", InterfaceErrorCodesI.ERROR_INIT_SOCKET_CONNECTION);
            }
        } catch (BTSLBaseException be) {
            _log.error("SocketWrapper[constructor]", "BTSLBaseException be:" + be.getMessage());
            ...
            e.printStackTrace();
            _log.error("SocketWrapper[constructor]", "Exception e:" + e.getMessage());
            if (_socketConnection != null) {
                try {
                    _socketConnection.close();
                } catch (Exception ex) {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "SocketWrapper[constructor]", "", "_interfaceID:" + _interfaceID, "", "While intializing Socket connection get Exception e:" + e.getMessage());
            throw new BTSLBaseException(this, "SocketWrapper[constructor]", InterfaceErrorCodesI.ERROR_INIT_SOCKET_CONNECTION);
        }
    }
    ...
    private String generateSessionID() throws BTSLBaseException {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "SocketWrapper[generateSessionID]", "", "_interfaceID:" + _interfaceID, "", "Exception:" + e.getMessage());
    ...
    public void destroy() {
        ...
        try {
            ...
            String logoutReqStr = _formatter.generateRequest(HuaweiI.ACTION_LOGOUT, _requestMap);
            sendLogoutRequest(logoutReqStr);
            String logoutRespStr = getLogoutResponse();
            // Parse the MML response string
            if (!InterfaceUtil.isNullString(logoutRespStr))
                _responseMap = _formatter.parseResponse(HuaweiI.ACTION_LOGOUT, logoutRespStr, null);
            /*
            ...
            _log.error("SocketWrapper[destroy]", "Error while destroying socket connection");
        } finally {
            try {
                _out.close();
            } catch (Exception e) {
    ...
    /**
     * @return Returns the isDestroyed.
     */
