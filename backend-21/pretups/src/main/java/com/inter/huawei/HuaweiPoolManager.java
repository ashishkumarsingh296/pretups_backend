...
public class HuaweiPoolManager {
            ...
            for (int i = 0, size = inStrArray.length; i < size; i++) {
                ...
                for (int loop = 0; loop < poolSize; loop++) {
                    try {
                        ...
                        e.printStackTrace();
                        _log.error("initialize", "Exception e:" + e.getMessage());
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiPoolManager[initialize]", "", "", "", "While instantiation of the Client objects got the Exception " + e.getMessage());
                        throw new BTSLBaseException(InterfaceErrorCodesI.ERROR_OBJECT_POOL_INITIALIZATION);
                    }
                }// end of for loop
     ...
     * @throws Exception
     */
    public static synchronized SocketWrapper getClientObject(String p_interfaceID, boolean p_module) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("getClientObject", "Entered p_interfaceID::" + p_interfaceID + " p_module:" + p_module);
        SocketWrapper clientObject = null;
        ...
        try {
            ...
            clientObject = (SocketWrapper) freeList.remove(0);
     ...
     * @throws BTSLBaseException
     */
    public static synchronized SocketWrapper getNewClientObject(String p_interfaceID) throws BTSLBaseException {
        if (_log.isDebugEnabled())
            _log.debug("getNewClientObject", "p_interfaceID::" + p_interfaceID);
        SocketWrapper socketWrapper = null;
        try {
            socketWrapper = new SocketWrapper(p_interfaceID);
        } catch (BTSLBaseException be) {
            throw be;
        } catch (Exception e) {
            e.printStackTrace();
            _log.error("getNewClientObject", "Exception e::" + e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.INTERFACES, EventStatusI.RAISED, EventLevelI.FATAL, "HuaweiPoolManager[getNewClientObject]", "INTERFACEID:" + p_interfaceID, "", "", "While getting the instance of Client objects got the Exception " + e.getMessage());
                            ...
                            try {
                                ...
                                if (objectList != null) {
                                    ...
                                    for (int i = 0; i < size; i++) {
                                        SocketWrapper socketWrapper = (SocketWrapper) objectList.get(i);
