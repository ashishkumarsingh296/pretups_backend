...
public class HuaweiHeartBeat implements Runnable {
    ...
    public HuaweiHeartBeat(String p_interfaceID) throws Exception {
        ...
        if (InterfaceUtil.isNullString(heartBeatCommand)) {
            _log.error("HuaweiHeartBeat[Constructor]", "HEART_BEAT_COMMAND is not defined in the INFile");
            throw new BTSLBaseException(this, "HuaweiHeartBeat[Constructor]", InterfaceErrorCodesI.ERROR_BAD_REQUEST);
        }
        ...
        if (InterfaceUtil.isNullString(_startFlag)) {
            _log.error("HuaweiHeartBeat[Constructor]", "START_FLAG is not defined in the INFile");
            throw new BTSLBaseException(this, "HuaweiHeartBeat[Constructor]", InterfaceErrorCodesI.ERROR_BAD_REQUEST);
        }
        _requestMap.put("START_FLAG", _startFlag.trim());

        String heartBeatSleepTimeStr = FileCache.getValue(_interfaceID, "HEART_BEAT_SLEEP_TIME");
        if (InterfaceUtil.isNullString(heartBeatSleepTimeStr)) {
            _log.error("HuaweiHeartBeat[Constructor]", "HEART_BEAT_SLEEP_TIME is not defined in the INFile");
        ...
        try {
            _requestStr = _formatter.generateRequest(HuaweiI.ACTION_HEART_BEAT, _requestMap);
        } catch (Exception e) {
            e.printStackTrace();// for testing
        }
        _freeList = (Vector) HuaweiPoolManager._freeBucket.get(_interfaceID);
        _busyList = (Vector) HuaweiPoolManager._busyBucket.get(_interfaceID);
    ...
    public void run() {
        ...
        while (_startHeartBeat) {
            try {
                ...
                _out.flush();
                HeartBeatSentLog.log("Heartbeat message : " + _requestStr + " for session id = " + _socketConnection.getSessionID() + "sent to IN at " + getCurrentTime());
                if (_log.isDebugEnabled())
                    _log.debug("run", " Heartbeat message : " + _requestStr + " for session id = " + _socketConnection.getSessionID() + "sent to IN at " + getCurrentTime());
                /*
                ...
                try {
                    ...
                    _busyList.add(_socketConnection);
                } catch (Exception ex) {
