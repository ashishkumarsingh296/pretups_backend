/*
 * ChennaiCDRGeneration.java
 * Name Date History
 * ------------------------------------------------------------------------
 * Amit Ruwali 24/05/2006 Initial Creation
 * Manish K. Singh 01/12/2006 For PreTUPS CDR
 * ------------------------------------------------------------------------
...
public class ChennaiCDRGeneration implements CDRRecordGeneratorI {
    private static Log _log = LogFactory.getLog(ChennaiCDRGeneration.class.getName());
    ...
    public String[] generateCDRRecords(ArrayList p_queueVOList, String p_interfaceID) throws BTSLBaseException {
        ...
        try {
            ...
            if (BTSLUtil.isNullString(fileSizeRecord))
                throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecords", "FILE_SIZE_RECORD should be Y or N");
            try {
                recordSize = Integer.parseInt(FileCache.getValue(p_interfaceID, "RECORD_SIZE"));
            } catch (Exception e) {
                throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecords", "RECORD_SIZE is invalid in IN file");
            }
            try {
                ...
                if (fileSize < 1) // the size of the file should be at least 1
                                  // MB
                {
                    throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecords", "FILE_SIZE should be greater than or equal to 1");
                }
            } catch (Exception e) {
                throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecords", "Invalid FILE_SIZE in IN File");
            }
            ...
            e.printStackTrace();
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecords", "Exception e:" + e.getMessage());
        } finally {
            if (_log.isDebugEnabled())
    ...
    private String padCharactersWithValue(char p_paddingChar, String p_strValue, int p_strLength, boolean p_leftDir) throws BTSLBaseException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new BTSLBaseException("ChennaiCDRGeneration", "padCharactersWithValue", "Exception e:" + e.getMessage());
        } finally {
            if (_log.isDebugEnabled())
    ...
    private String generateFileName(String p_interfaceID) throws BTSLBaseException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateFileName", "Exception e:" + e.getMessage());
        } finally {
            if (_log.isDebugEnabled())
    ...
    private String generateCDRRecordString(QueueTableVO p_queueVO) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(_cdrFieldSeperator))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Cannot find constant CDR_FIELD_SEPARATOR");

        // null checks for mandatory fields
        if (BTSLUtil.isNullString(p_queueVO.getMsisdn()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field Msisdn is missing");
        if (BTSLUtil.isNullString(p_queueVO.getImsi()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field getImsi is missing");
        if (BTSLUtil.isNullString(p_queueVO.getTransferID()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field getTransferID is missing");
        if (BTSLUtil.isNullString(p_queueVO.getProductCode()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field ProductCode is missing");
        if (BTSLUtil.isNullString(p_queueVO.getEntryType()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field EntryType is missing");
        if (BTSLUtil.isNullString(p_queueVO.getEntryFor()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field EntryFor is missing");
        if (BTSLUtil.isNullString(p_queueVO.getServiceType()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field ServiceType is missing");
        if (BTSLUtil.isNullString(p_queueVO.getAccountID()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field AccountID is missing");
        if (BTSLUtil.isNullString(p_queueVO.getServiceClass()))
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field ServiceClass is missing");
        // amount <0 checks for mandatory fields
        if (p_queueVO.getInterfaceAmount() < 0)
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field getInterfaceAmount() < 0 ");
        if (p_queueVO.getAccessFee() < 0)
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field getAccessFee() < 0 ");
        if (p_queueVO.getAmount() < 0)
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Mandatory field getAmount() < 0 ");

        StringBuffer cdrBuff = null;
        try {
            cdrBuff = new StringBuffer(200);
            cdrBuff.append(_sequenceNumber);
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getMsisdn());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getImsi());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getTransferID());
            cdrBuff.append(_cdrFieldSeperator);
            // Date date = p_queueVO.getEntryOn();
            Date date = Calendar.getInstance().getTime();
            cdrBuff.append(_callDateStr.format(date)); // Transaction Date
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(_callTimeStr.format(date)); // Transaction Time
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getProductCode());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getAmount());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getInterfaceAmount());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getAccessFee());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getEntryType());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getEntryFor());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getServiceType());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getAccountID());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(p_queueVO.getServiceClass());
            cdrBuff.append(_cdrFieldSeperator);
            cdrBuff.append(BTSLUtil.isNullString(p_queueVO.getOtherInfo()) ? "" : p_queueVO.getOtherInfo());
            cdrBuff.append(_cdrFieldSeperator);// for OtherInfo2
            // cdrBuff.append(BTSLUtil.isNullString(p_queueVO.getOtherInfo())?"":p_queueVO.getOtherInfo());
            // //for OtherInfo2
            _sequenceNumber = _sequenceNumber + 1;
        } catch (Exception e) {
            e.printStackTrace();
            throw new BTSLBaseException("ChennaiCDRGeneration", "generateCDRRecordString", "Exception e:" + e.getMessage());
        } finally {
            if (_log.isDebugEnabled())
    ...
    private String writeCDRData(List p_queueVOList, String p_fileName, String p_interfaceID, int p_recordSize) throws BTSLBaseException {
        ...
        try {
            ...
            throw new BTSLBaseException("ChennaiCDRGeneration", "writeCDRData", "Exception e:" + e.getMessage());
        } finally {
            file = null;
            if (pw != null) {
                try {
