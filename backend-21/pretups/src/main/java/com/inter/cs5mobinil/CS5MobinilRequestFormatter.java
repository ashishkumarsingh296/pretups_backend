...
public class CS5MobinilRequestFormatter{
    */
/**
    ...
    protected String generateRequest(int p_action, HashMap<String, String> p_map) throws Exception {
        ...
        try {
            switch (p_action) {
            case CS5MobinilI.ACTION_ACCOUNT_INFO: {
                str = generateGetAccountInfoRequest(p_map);
                break;
            }
            case CS5MobinilI.ACTION_ACCOUNT_DETAILS: {
                str = generateGetAccountDetailRequest(p_map);
                break;
            }
            case CS5MobinilI.ACTION_RECHARGE_CREDIT: {
                str = generateRechargeCreditRequest(p_map);
                break;
            }
            case CS5MobinilI.ACTION_IMMEDIATE_DEBIT: {
                str = generateImmediateDebitRequest(p_map);
                break;
            }
            case CS5MobinilI.ACTION_DEDICATED_ACCOUNT_CD: {
	...
	public CS5MobinilRequestFormatter() throws Exception
	{
		if(_log.isDebugEnabled()) _log.debug("CS5MobinilRequestFormatter[constructor]","Entered");
		try
		{
		...
		{
			_log.errorTrace("CS5MobinilRequestFormatter[constructor]", e);
			_log.error("CS5MobinilRequestFormatter[constructor]","Exception e::"+e.getMessage());
			throw e;
		}
		finally
		{
			if(_log.isDebugEnabled()) _log.debug("CS5MobinilRequestFormatter[constructor]","Exited");
		}
	}
	*/
/**
    ...
    private HashMap<String, String> parseImmediateDebitResponse(String p_responseStr) throws Exception {
        ...
        try {
            ...
            if (tempIndex > 0) {
                ...
                responseMap.put("responseCode", responseCode.trim());
                Object[] successList = CS5MobinilI.RESULT_OK.split(",");
                // if(!CS5MobinilI.RESULT_OK.equals(responseCode))
                if (!Arrays.asList(successList).contains(responseCode))
                    return responseMap;
            }
            indexStart = p_responseStr.indexOf("<member><name>originTransactionID", indexEnd);
...
/**
    ...
    private String getCS5TransDateTime() throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("getCS5TransDateTime", "Entered");
        ...
        try {
            ...
            if (_log.isDebugEnabled())
                _log.debug("getCS5TransDateTime", "Exited transDateTime: " + transDateTime);
        }
...
/**
    ...
    private String generateGetAccountDetailRequest(HashMap p_requestMap) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("generateGetAccountDetailRequest", "Entered p_requestMap::" + p_requestMap);
        String requestStr = null;
        StringBuffer stringBuffer = null;
        try {
            stringBuffer = new StringBuffer(1028);
            ...
            if (_log.isDebugEnabled())
                _log.debug("generateGetAccountDetailRequest", "Exiting Request String:requestStr::" + requestStr);
        }
        return requestStr;
    }
...
/**
    ...
    private HashMap<String, String> parseGetAccountDetailResponse(String p_responseStr) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("parseGetAccountDetailResponse", "Entered p_responseStr::" + p_responseStr);
        HashMap<String, String> responseMap = null;
        int indexStart = 0;
        int indexEnd = 0;
        int tempIndex = 0;
        try {
            ...
            if (_log.isDebugEnabled())
                _log.debug("parseGetAccountDetailResponse", "Exited responseMap::" + responseMap);
        }
        return responseMap;
    }
...
/**
	...
	{
		if(_log.isDebugEnabled())
			_log.debug("generateDedicatedAccountCDRequest","Entered p_requestMap::"+p_requestMap);
		String requestStr= null;
		StringBuffer stringBuffer = null;
		...
		try
		{
			...
			interfaceAction=(String)p_requestMap.get("INTERFACE_ACTION");
			{
			...
			String dedicatedAccountID=(String)p_requestMap.get("CARD_GROUP_SELECTOR");
		...
		{
			_log.error("generateDedicatedAccountCDRequest","Exception e: "+e.getMessage());
			throw e;
		}
		finally
		{
			if(_log.isDebugEnabled())
				_log.debug("generateDedicatedAccountCDRequest","Exiting  requestStr="+requestStr);
		}
		return requestStr;
	}
...
/**
	...
	{
		if(_log.isDebugEnabled()) 
			_log.debug("parseDedicatedAccountInfo","Entered");
		String dedicatedID="";
		...
		try
		{
			...
			if(indexStart>0)
			{
				...
				String dedicatedAccString = p_responseString.substring(p_responseString.indexOf("<value><array><data><value><struct>")+tempIndex,p_responseString.indexOf("</struct></value></data></array></value></member>"));
				...
				for(int i=0; i<dedicatedAccInfo.length; i++)
				{
					...
					if(tempIndex>0)
					{
						...
						if(InterfaceUtil.isNullString(dedicatedID)|| i==0)
					...
					if(tempIndex>0)
					{
						...
						indexEnd = dedicatedAccInfo[i].indexOf("</member>",indexStart);
						if(InterfaceUtil.isNullString(dedicatedValue)|| i==0)
					...
					if(tempIndex>0)
					{
						...
						indexEnd = dedicatedAccInfo[i].indexOf("</member>",indexStart);
						if(InterfaceUtil.isNullString(dedicatedExpiry)|| i==0)
		...
		{
			_log.errorTrace("parseDedicatedAccountInfo", e);
			throw e;
		}
		finally
		{
			if(_log.isDebugEnabled()) 
				_log.debug("parseDedicatedAccountInfo","Exited with dedicatedID="+dedicatedID+" dedicatedValue="+dedicatedValue+" dedicatedExpiry="+dedicatedExpiry);
		}
	}
...
/**
	...
	{
		if(_log.isDebugEnabled()) _log.debug("getDedicatedAccExpiryDate","Entered");
		String dedicatedExpiry="";
		String cardGrpSelector;
		String split="%7C";
		String replacePipeEncodeString="%FE%FF%00";
		try
		{
			...
			{
				String dedicatedAccId = ((String)p_requestMap.get("DEDICATED_ACC_ID"));
				String dedicatedAccExpiry = ((String)p_requestMap.get("DEDICATED_ACC_EXPIRY"));
				if(!dedicatedAccId.contains(split) && !dedicatedAccExpiry.contains(split)){
					if(dedicatedAccId.contains(replacePipeEncodeString) && dedicatedAccExpiry.contains(replacePipeEncodeString)){
						split = replacePipeEncodeString;
					}
				}
				if(_log.isDebugEnabled()) {
					_log.debug("getDedicatedAccExpiryDate","split="+split+", dedicatedAccId="+dedicatedAccId+", dedicatedAccExpiry="+dedicatedAccExpiry);
				}
				String[] dedAccIDArray = dedicatedAccId.split(split);
				String[] dedAccExpiryArray = dedicatedAccExpiry.split(split);
				for(int i=0; i<dedAccIDArray.length; i++)
				{
					if(_log.isDebugEnabled()) {
						_log.debug("getDedicatedAccExpiryDate","dedAccIDArray[i]="+dedAccIDArray[i]+" , After Replace="+dedAccIDArray[i].replace(replacePipeEncodeString, ""));
					}
					if(!cardGrpSelector.equals(dedAccIDArray[i].replace(replacePipeEncodeString, ""))) 
						continue;
					else
					{
						dedicatedExpiry=dedAccExpiryArray[i].replace(replacePipeEncodeString, "");
						dedicatedExpiry=dedicatedExpiry.replace("%3A",":");
						dedicatedExpiry=dedicatedExpiry.replace("%2B","+");
						dedicatedExpiry=dedicatedExpiry.replace(replacePipeEncodeString,"");
		...
		{
			_log.errorTrace("getDedicatedAccExpiryDate", e);
			throw e;
		}
		finally
		{
			if(_log.isDebugEnabled()) 
				_log.debug("getDedicatedAccExpiryDate","Exited with dedicatedExpiry="+dedicatedExpiry);
		}
		return dedicatedExpiry;
	}
	
	
	public String generateMatrixRequest(int p_action,HashMap<String, String> p_map) throws Exception {
...
/**
    ...
    private String getDedicatedAccExpiryDate(HashMap<String, String> p_requestMap) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("getDedicatedAccExpiryDate", "Entered");
	...
	public HashMap<String, String> parseMatrixResponse(int p_action,String responseStr) throws Exception {
		String methodName="parseMatrixResponse";
       if(_log.isDebugEnabled())_log.debug(methodName,"Entered p_action::"+p_action);
		...
		try
		{
				...
				if( CS5MobinilI.ACTION_ACCOUNT_INFO==p_action)
				{
					
					map=parseMatrixValidationRequest(responseStr);
					map.put("action",String.valueOf(p_action));	
				}
				else
				{
					map=parseMatrixCreditRequest(responseStr);
					map.put("action",String.valueOf(p_action));
		...
		{
			if(_log.isDebugEnabled())_log.debug(methodName,"Exited Request String: map::"+map);
		}//End of finally
	...
	private HashMap<String,String> parseMatrixValidationRequest(String responseStr) throws Exception
	{
		_log.debug("parseMatrixValidationRequest","Entered responseStr="+responseStr);
		...
		{  
			...
			String returnCode=responseStr.substring(index+"<Result>".length(),responseStr.indexOf("</Result>",index));
			responseMap.put("resp_returnCode",returnCode);
			if(CS5MobinilI.MATRIX_RESULT_OK.contains(returnCode))
			{
			
				index=responseStr.indexOf("<Status>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<Status>".length(),responseStr.indexOf("</Status>",index));
					responseMap.put("Status",resp_status);
				}	
				index=responseStr.indexOf("<AccountBrand>");
				if(index!=-1)
				{
					String resp_serviceClass=responseStr.substring(index+"<AccountBrand>".length(),responseStr.indexOf("</AccountBrand>",index));
			...
			{
				responseMap.put("faultCode",returnCode);
				index=responseStr.indexOf("<Status>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<Status>".length(),responseStr.indexOf("</Status>",index));
					responseMap.put("faultString",resp_status);
				}
				else{
					responseMap.put("faultString","Status Not Found");
				}
				index=responseStr.indexOf("<KYCBlock>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<KYCBlock>".length(),responseStr.indexOf("</KYCBlock>",index));
					responseMap.put("KYCBlock",resp_status);
				}
				index=responseStr.indexOf("<FraudBlock>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<FraudBlock>".length(),
							responseStr.indexOf("</FraudBlock>",index));
					responseMap.put("FraudBlock",resp_status);
				}
				index=responseStr.indexOf("<LegalBlock>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<LegalBlock>".length(),
							responseStr.indexOf("</LegalBlock>",index));
					responseMap.put("LegalBlock",resp_status);
				}
				index=responseStr.indexOf("<CMBlock>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<CMBlock>".length(),
							responseStr.indexOf("</CMBlock>",index));
					responseMap.put("CMBlock",resp_status);
				}
				index=responseStr.indexOf("<SIMLost>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<SIMLost>".length(),
							responseStr.indexOf("</SIMLost>",index));
					responseMap.put("SIMLost",resp_status);
				}
		...
		{
			_log.error("parseMatrixValidationRequest","Exception e: "+e);
			_log.errorTrace("parseMatrixValidationRequest", e);
			throw e;
		}
		finally
		{
			_log.debug("parseMatrixValidationRequest","Exiting with responseMap="+responseMap);
		}
	}

	private HashMap<String,String> parseMatrixCreditRequest(String responseStr) throws Exception
	{
		_log.debug("parseMatrixCreditRequest","Entered responseStr="+responseStr);
		...
		{  
			...
			if(CS5MobinilI.MATRIX_RESULT_OK.contains(returnCode))
			{
				...
				index=responseStr.indexOf("<BalanceTemplateId>21124</BalanceTemplateId>");
				
				String newresponseStr=responseStr.substring(index);
				_log.debug("parseMatrixCreditRequest","Entered NewresponseStrForMainBalance="+newresponseStr);
				
				index=newresponseStr.indexOf("<CurrentBalanceAmount>");
				if(index!=-1)
				{
					String currentBalanceAmount=newresponseStr.substring(index+"<CurrentBalanceAmount>".length(),newresponseStr.indexOf("</CurrentBalanceAmount>",index));
					responseMap.put("CurrentBalanceAmount",currentBalanceAmount);
				}	
				
				
				index=newresponseStr.indexOf("<ImpactAmount>");
				if(index!=-1)
				{
					String impactAmount=newresponseStr.substring(index+"<ImpactAmount>".length(),newresponseStr.indexOf("</ImpactAmount>",index));
					responseMap.put("ImpactAmount",impactAmount);
				}
				
				}
				catch(Exception e){
					responseMap.put("CurrentBalanceAmount","0.00000");
					responseMap.put("ImpactAmount","0.00000");
				}
			...
			{
				responseMap.put("faultCode",returnCode);
				index=responseStr.indexOf("<ResultText>");
				if(index!=-1)
				{
					String resp_status=responseStr.substring(index+"<ResultText>".length(),responseStr.indexOf("</ResultText>",index));
					responseMap.put("faultString",resp_status);
				}
				else{
					responseMap.put("ResultText","Status Not Found");
				}
		...
		{
			_log.error("parseMatrixCreditRequest","Exception e: "+e);
			_log.errorTrace("parseMatrixCreditRequest", e);
			throw e;
		}
		finally
		{
			_log.debug("parseMatrixCreditRequest","Exiting with responseMap="+responseMap);
		}
	}
