...
public class VOMSUrlConnection extends BTSLConnection {

    private HttpURLConnection _urlConnection = null;

    public VOMSUrlConnection(String p_url, int p_connectTimeout, int p_readTimeout, String p_keepAlive, long p_contentLength, String p_soapAction) throws Exception {
        if (_log.isDebugEnabled())
            _log.debug("VOMSUrlConnection", "Entered p_url: " + p_url + "p_connecttimeout=" + p_connectTimeout + " p_readtimeout=" + p_readTimeout + " p_keepAlive=" + p_keepAlive + " p_soapAction=" + p_soapAction);
        try {
            ...
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.HTTPS_ENABLE)).booleanValue())
                _urlConnection = BTSLUtil.getConnection(url);
            else
                _urlConnection = (HttpURLConnection) url.openConnection();
            ...
            if (_log.isDebugEnabled())
                _log.debug("VOMSUrlConnection", "Exception " + e.getMessage());
            throw e;
        }
        if (_log.isDebugEnabled())
            _log.debug("VOMSUrlConnection", "Exiting");
    }
