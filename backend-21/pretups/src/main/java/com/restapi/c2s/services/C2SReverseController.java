...
/*@Path("")*/
@io.swagger.v3.oas.annotations.tags.Tag(name = "${C2SReverseController.name}", description = "${C2SReverseController.desc}")//@Api(tags ="C2S Services", value="C2S Services")
@RestController
@RequestMapping(value = "/v1/c2sServices")
public class C2SReverseController {
	 ...
	 /*@Context
	    ...
	    private static long requestIdChannel = 0;
	    	    ...
	    	    @PostMapping(value = "/rcrev", consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
		@ResponseBody
	    /*@ApiOperation(value = "Recharge Reversal", response = PretupsResponse.class ,
	    		authorizations = {
	    ...
	    */
				...
				@io.swagger.v3.oas.annotations.Operation(summary = "${rcrev.summary}", description="${rcrev.description}",
				...
				public BaseResponseMultiple processRechargeReversalRequest(@Parameter(hidden = true) @RequestHeader MultiValueMap<String, String> headers,
	            @Parameter(description = SwaggerAPIDescriptionI.RECHARGE_REVERSAL, required = true)
	            @RequestBody C2SRechargeReversalRequestVO requestVO,HttpServletResponse response1) throws Exception {
	    	    	BaseResponseMultiple response;
	        ++requestIdChannel;
	       
	        final String requestIDStr = String.valueOf(requestIdChannel);
	        response = processRequestChannel(requestVO, "rcrev".toUpperCase(),requestIDStr,headers, response1);
	 ...
	 private BaseResponseMultiple processRequestChannel(C2SRechargeReversalRequestVO requestVO1, String serviceKeyword, String requestIdChannel ,MultiValueMap<String, String> headers,HttpServletResponse response1)  {
			...
			if ((PretupsI.BCU_USER.equalsIgnoreCase(userVO.getCategoryCode())
	 	       ...
	 	       if(!BTSLUtil.isNullOrEmptyList(rowErrorMsgListsFinal))
				{
					...
					response1.setStatus(HttpStatus.SC_BAD_REQUEST);
					response.setService("RCREVRESP");
					response.setMessage(PretupsErrorCodesI.ERROR_REVERSE);
