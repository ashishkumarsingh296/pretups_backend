...
@Tag(name = "${AddCardGroup.name}", description = "${AddCardGroup.desc}")
public class AddCardGroup {

	public static final Log log = LogFactory.getLog(AddCardGroup.class.getName());
	
	@PostMapping(value = "/addCardgroup", consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
	//@ApiOperation(value = "Add Voucher card group", response = PretupsResponse.class)
	public PretupsResponse<JsonNode> addCardGroup(
			@Parameter(description = SwaggerAPIDescriptionI.ADD_CARD_GROUP, required = true)
	...
	@PostMapping(value = "/modifyCardgroup", consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
	//@ApiOperation(value = "Modify Voucher card group", response = PretupsResponse.class)
	public PretupsResponse<JsonNode> modifyCardGroup(
			@Parameter(description = SwaggerAPIDescriptionI.MODIFY_CARD_GROUP, required = true)
			@RequestBody  String requestData)
			throws Exception {
...
//					  if (cardGroupSetVO.getCardGroupSetID().equals(setVersionVO.getCardGroupSetID()) && cardGroupSetVO.getVersion().equals(setVersionVO.getVersion())) {       //PRETUPS-26072
				if (cardGroupSetVO.getCardGroupSetID().equals(setVersionVO.getCardGroupSetID())) {
	...
	public ActionErrors validation(CardGroupSetVO cardGroupSetVO,JsonNode cardGroupSetListNode,ObjectMapper objectMapper,Connection con) throws BTSLBaseException{
        ...
        Boolean isINPromoRequired = (Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.IN_PROMO_REQUIRED);
        Boolean isDuplicateCardGroupCodeAllow = (Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.DUPLICATE_CARDGROUP_CODE_ALLOW);
                	...
                	try{
               		...
               		for (int i = 1, j = sortedlist.size(); i < j; i++) {
                		 ...
                		 if (currCrdVO.getCardGroupCode().equalsIgnoreCase(nextCardVO.getCardGroupCode())) {
     						if(!isDuplicateCardGroupCodeAllow){
