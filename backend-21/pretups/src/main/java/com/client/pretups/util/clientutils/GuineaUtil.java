...
public class GuineaUtil extends OperatorUtil {
    ...
    public boolean validateTransactionPassword(ChannelUserVO p_channelUserVO, String p_password) throws BTSLBaseException {
        ...
        try {
            if (p_channelUserVO != null) {
                /*
                 * changes done by ashishT for hashing implementation.
                 * comparing the hashvalue of password in channeluservo to the
                 * hashvalue of p_password.
                 */
                ...
                if ("SHA".equalsIgnoreCase((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.PINPAS_EN_DE_CRYPTION_TYPE))) {
                    boolean checkpassowrd;
                    if (p_password.length() > ((Integer) (PreferenceCache.getSystemPreferenceValue(PreferenceI.MAX_LOGIN_PWD_LENGTH))).intValue()) {
                        checkpassowrd = BTSLUtil.decryptText(p_channelUserVO.getPassword()).equals(p_password);
                    } else {
                        checkpassowrd = (!PretupsI.FALSE.equalsIgnoreCase(BTSLUtil.compareHash2String(p_channelUserVO.getPassword(), p_password)));
                    }

                    if (!BTSLUtil.isNullString(p_channelUserVO.getPassword()) && (!checkpassowrd)) {
                ...
                if (!decryptedPin.equalsIgnoreCase(p_requestPin)) {
                    ...
                    if (p_senderVO.getPinBlockCount() > 0) {
                        ...
                        if (updateStatus < 0) {
                            ...
                            throw new BTSLBaseException("OperatorUtil", "validatePIN", PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
                        }
                    }
                }
                if (updatePinCount) {
                    updateStatus = subscriberDAO.updatePinStatus(p_con, p_senderVO, false);
                    if (updateStatus > 0 && !isUserBarred) {
                        throw new BTSLBaseException("OperatorUtil", "validatePIN", PretupsErrorCodesI.ERROR_INVALID_PIN);
                    } else if (updateStatus > 0 && isUserBarred) {
                        p_senderVO.setBarUserForInvalidPin(true);
                        throw new BTSLBaseException("OperatorUtil", "validatePIN", PretupsErrorCodesI.ERROR_SNDR_PINBLOCK, 0, new String[] { String
    ...
    public void setCalculatedCardGroupValues(String p_subService, CardGroupDetailsVO p_cardGroupDetailVO, TransferVO p_transferVO) throws Exception {
        ...
        try {
            /**
             * In case of Selector value 1 all values are set as calculated.
             * In case of Selector value other than 1 , validity and grace will
             * be set to 0.
             * 
             */
            ...
            if ((String.valueOf(PretupsI.CHNL_SELECTOR_C_VALUE)).equals(p_subService) || (String.valueOf(PretupsI.CHNL_SELECTOR_VG_VALUE)).equals(p_subService))// C
            {
    ...
    private void validateIfNotBuddy(Connection p_con, RequestVO p_requestVO, TransferVO p_transferVO) throws BTSLBaseException, Exception {
        ...
        if (_log.isDebugEnabled()) {
            _log.debug("validateIfNotBuddy", " requestMessageArray length:" + requestMessageArray.length);
        }
        if (requestMessageArray.length < 5 || requestMessageArray.length > 7) {
            ...
            case 5:
                {
                    ...
                    // whether PIN validation is required or not.
                    if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.CP2P_PIN_VALIDATION_REQUIRED)).booleanValue()) {
                        if (senderVO.getStatus().equalsIgnoreCase(PretupsI.STATUS_ACTIVE)) {
                            throw new BTSLBaseException(this, "validateIfNotBuddy", PretupsErrorCodesI.P2P_INVALID_MESSAGEFORMAT, 0, new String[] { p_requestVO
                                .getActualMessageFormat() }, null);
                        }
                    }
            ...
            case 6:
                {
                    ...
                    if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.CP2P_PIN_VALIDATION_REQUIRED)).booleanValue()) {
                        if (actualPin.equalsIgnoreCase((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_DEFAULT_SMSPIN))) {
                            ...
                            /*
                             * else
                             * if(senderVO.getStatus().equalsIgnoreCase(PretupsI.
                             * STATUS_ACTIVE))
                             * {
                             * senderVO.setPin(BTSLUtil.encryptText(pin));
                             * senderVO.setPinUpdateReqd(true);
