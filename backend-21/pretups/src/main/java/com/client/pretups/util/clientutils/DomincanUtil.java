...
public class DomincanUtil extends OperatorUtil {
    ...
    public void validatePIN(Connection p_con, ChannelUserVO p_channelUserVO, String p_requestPin) throws BTSLBaseException {
        ...
        try {
            ...
            // (userPhoneVO.getPinModifiedOn().getTime())==(userPhoneVO.getCreatedOn().getTime()))
            ...
            if (userPhoneVO.isForcePinCheckReqd() && userPhoneVO.getPinModifiedOn() != null && ((userPhoneVO.getModifiedOn().getTime() - userPhoneVO.getPinModifiedOn()
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "DomincanUtil[validatePIN]", "", userPhoneVO
                ...
                throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.CHNL_ERROR_SNDR_FORCE_CHANGEPIN, 0, strArr, null);
            } else {
                ...
                // if(userPhoneVO.isForcePinCheckReqd() &&
                // ((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.C2S_DEFAULT_SMSPIN)).equals(decryptedPin))
                // throw new BTSLBaseException("DomincanUtil", "validatePIN",
                ...
                if (!decryptedPin.equals(p_requestPin)) {
                    ...
                    if (userPhoneVO.getInvalidPinCount() > 0) {
                        ...
                        if (updateStatus < 0) {
                            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "DomincanUtil[validatePIN]", "",
                                userPhoneVO.getMsisdn(), "", "Not able to update invalid PIN count for users");
                            throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.ERROR_EXCEPTION);
                        }
                    }
                }
                if (increaseInvalidPinCount) {
                    ...
                    if (updateStatus > 0 && !isUserBarred) {
                        throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.CHNL_ERROR_SNDR_INVALID_PIN);
                    } else if (updateStatus > 0 && isUserBarred) {
                        throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.CHNL_ERROR_SNDR_PINBLOCK);
                    } else {
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "DomincanUtil[validatePIN]", "",
                            userPhoneVO.getMsisdn(), "", "Not able to update invalid PIN count for users");
                        throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.ERROR_EXCEPTION);
        ...
        try {
            ...
            if (p_senderVO.isForcePinCheckReqd() && p_senderVO.getPinModifiedOn() != null && ((p_senderVO.getModifiedOn().getTime() - p_senderVO.getPinModifiedOn().getTime()) / (24 * 60 * 60 * 1000)) > ((Integer) (PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_DAYS_AFTER_CHANGE_PIN))).intValue()) {
                ...
                if (p_senderVO.isForcePinCheckReqd() && (String) PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_DEFAULT_SMSPIN).equals(decryptedPin)) {
                    throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.CHNLUSR_CHANGE_DEFAULT_PIN);
                }
                ...
                if (!decryptedPin.equalsIgnoreCase(p_requestPin)) {
                    ...
                    if (p_senderVO.getPinBlockCount() > 0) {
                        ...
                        if (updateStatus < 0) {
                            ...
                            throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
                        }
                    }
                }
                if (updatePinCount) {
                    updateStatus = subscriberDAO.updatePinStatus(p_con, p_senderVO, false);
                    if (updateStatus > 0 && !isUserBarred) {
                        throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.ERROR_INVALID_PIN);
                    } else if (updateStatus > 0 && isUserBarred) {
                        p_senderVO.setBarUserForInvalidPin(true);
                        throw new BTSLBaseException("DomincanUtil", "validatePIN", PretupsErrorCodesI.ERROR_SNDR_PINBLOCK, 0, new String[] { String
                        ...
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "DomincanUtil[validatePIN]", "", p_senderVO
    ...
    public void validatePIN(String p_pin) throws BTSLBaseException {
        if (BTSLUtil.isNullString(p_pin)) {
            throw new BTSLBaseException(this, "validatePIN", PretupsErrorCodesI.PIN_INVALID);
        } else if (!BTSLUtil.isNumeric(p_pin)) {
            throw new BTSLBaseException(this, "validatePIN", PretupsErrorCodesI.NEWPIN_NOTNUMERIC);
            ...
            throw new BTSLBaseException(this, "validatePIN", PretupsErrorCodesI.PIN_LENGTHINVALID, 0, msg, null);
        } else {
            final int result = BTSLUtil.isSMSPinValid(p_pin);
            if (result == -1) {
                throw new BTSLBaseException(this, "validatePIN", PretupsErrorCodesI.PIN_SAMEDIGIT);
            } else if (result == 1) {
                throw new BTSLBaseException(this, "validatePIN", PretupsErrorCodesI.PIN_CONSECUTIVE);
            }
        }
    }
