/**
 * @(#)CollectionEnquiryController.java
 *                                      Name Date History
 *                                      ----------------------------------------
 *                                      --------------------------------
 *                                      Akanksha Gupta 18/12/2013 Initial
 *                                      Creation
 *                                      ----------------------------------------
 *                                      --------------------------------
 *                                      Copyright (c) 2013 Mahindra Comviva
 *                                      Technologies Ltd.
 *                                      Controller class for handling the
 *                                      Collection Billpayment
 */
...
import com.inter.claroCollectionEnqWS.ClaroCollEnqWSI;

public class CollectionEnquiryController implements ServiceKeywordControllerI, Runnable {
    private static Log _log = LogFactory.getLog(CollectionEnquiryController.class.getName());
    ...
    static {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[initialize]", "", "", "",
                "Exception while loading the class at the call:" + e.getMessage());
        }
    }

    public CollectionEnquiryController() {
        _c2sTransferVO = new C2STransferVO();
        _currentDate = new Date();
        /*
         * The process below is used to control the messages that are send to
         * receiver.
         * If COLLECTION_REC_GEN_FAIL_MSG_REQD_V is Y then all validation fail
         * message are send to receiver
         * If COLLECTION_REC_GEN_FAIL_MSG_REQD_T is Y then all topup fail
         * message are send to receiver.
         */
        if ("Y".equals(BTSLUtil.NullToString(Constants.getProperty("COLLECTION_REC_GEN_FAIL_MSG_REQD_V")))) {
            _recValidationFailMessageRequired = true;
        }
        if ("Y".equals(BTSLUtil.NullToString(Constants.getProperty("COLLECTION_REC_GEN_FAIL_MSG_REQD_T")))) {
            _recTopupFailMessageRequired = true;
        }
        _notAllowedSendMessGatw = BTSLUtil.NullToString(Constants.getProperty("COLLECTION_SEN_MSG_NOT_REQD_GW"));
        _notAllowedRecSendMessGatw = BTSLUtil.NullToString(Constants.getProperty("COLLECTION_REC_MSG_NOT_REQD_GW"));
    }
    ...
    public void process(RequestVO p_requestVO) {
        ...
        // 14. Spawn the thread and do the actual bill payment
        ...
        try {
            ...
            _operatorUtil.validateC2SRequestWithoutAmount(con, _c2sTransferVO, p_requestVO);
            ...
            if (!_receiverVO.getSubscriberType().equals(_type)) {
                ...
                if (_networkPrefixVO != null) {
                    ...
                    EventHandler
                        .handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "CollectionEnquiryController[process]", "", "", "",
            ...
            if (_senderMSISDN.equals(_receiverMSISDN) && !((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.C2S_ALLOW_SELF_BILLPAY))).booleanValue()) {
            ...
            if (PretupsI.SUSPEND.equals(_channelUserVO.getCommissionProfileStatus())) {
                ...
                throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.CHNL_ERROR_SENDER_OUT_SUSPEND_COLLECTION);
            }
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            }
            ...
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SKEY_REQUIRED)).booleanValue()) {
				...
				if (mcomCon != null) {
					mcomCon.close("CollectionEnquiryController#process");
					mcomCon = null;
				}
                con = null;
                ...
                if ((p_requestVO.getRequestGatewayType()).equalsIgnoreCase(PretupsI.GATEWAY_TYPE_WEB)) {

                    if (p_requestVO.isSuccessTxn()) {
                        EnquiryVO enqVO = null;
                        String messageNew = p_requestVO.getSenderReturnMessage() + "&INVOICE_SIZE=" + p_requestVO.getInvoiceSize() + "&TOTAL_PENDING_BALANCE=" + p_requestVO
                            .getXMontoDeudaTotal() + "&SERVICE_NAME=" + p_requestVO.getXOpcionRecaudacion() + "&SERVICE_CODE=" + p_requestVO.getXCodTipoServicio();
                       int requestInvoiceSize = p_requestVO.getInvoiceSize();
                        for (int i = 0; i < requestInvoiceSize; i++) {
                            enqVO = (EnquiryVO) p_requestVO.getEnquiryItemList().get(i);
                            messageNew = messageNew + "&SERVICE_NAME_" + i + "=" + enqVO.getxDescripServ() + "&SERVICE_CODE_" + i + "=" + enqVO.getxTipoServicio() + "&PERIOD_PENDING_BALANCE_" + i + "=" + enqVO
                                .getxMontoDebe() + "&MIN_PENDING_BALANCE_" + i + "=" + enqVO.getxMontoFact() + "&INVOICED_PENDING_BALANCE_" + i + "=" + enqVO
                                .getxImportePagoMin() + "&BILL_PERIOD_START_" + i + "=" + enqVO.getxFechaEmision() + "&BILL_PERIOD_END_" + i + "=" + enqVO.getxFechaVenc() + "&INVOICE_NUM_" + i + "=" + enqVO
                                .getxNumeroDoc();
                        }
                        p_requestVO.setSenderReturnMessage(messageNew);
                    } else {
                        final String messageNew = p_requestVO.getSenderReturnMessage();
                        p_requestVO.setSenderReturnMessage(messageNew);
                    }
            ...
            if (_recValidationFailMessageRequired) {
                ...
                if (_c2sTransferVO.getReceiverReturnMsg() == null || !((BTSLMessages) _c2sTransferVO.getReceiverReturnMsg()).isKey()) {
                    if (_transferID != null) {
                        _c2sTransferVO.setReceiverReturnMsg(new BTSLMessages(PretupsErrorCodesI.C2S_RECEIVER_FAIL_COLLECTION, new String[] { PretupsBL
            ...
            // raising alarm
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[process]", _transferID,
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "CollectionEnquiryController[process]", _transferID,
                    _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            }
            if (BTSLUtil.isNullString(p_requestVO.getMessageCode())) {
    ...
    private void processSKeyGen(Connection p_con) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[processSKeyGen]",
    ...
    public void processTransfer(Connection p_con) throws BTSLBaseException {
        ...
        try {
            ...
            _c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[processTransfer]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    public void run() {
        ...
        try {
            ...
            try {
                ...
                if (((_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS) && ((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.C2S_SNDR_CREDIT_BK_AMB_STATUS_BILLPAY))).booleanValue())) || _c2sTransferVO
                ...
                // Validating the receiver Limits and updating it
                PretupsBL.validateRecieverLimits(con, _c2sTransferVO, PretupsI.TRANS_STAGE_AFTER_INTOP, PretupsI.C2S_MODULE);
                throw new BTSLBaseException("CollectionEnquiryController", methodName, "");
            ...
            if (_log.isDebugEnabled()) {
                _log.debug("CollectionEnquiryController",
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "{CollectionEnquiryController[run]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            ...
            if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_FAIL) && (_c2sTransferVO.getReceiverReturnMsg() == null || !((BTSLMessages) _c2sTransferVO
                ...
                _c2sTransferVO.setReceiverReturnMsg(new BTSLMessages(PretupsErrorCodesI.C2S_RECEIVER_FAIL_COLLECTION, new String[] { PretupsBL.getDisplayAmount(_c2sTransferVO
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "CollectionEnquiryController[run]", _transferID,
            ...
            if (con != null) {
				...
				if (mcomCon != null) {
					mcomCon.close("CollectionEnquiryController#run");
					mcomCon = null;
				}
                con = null;
            }
            ...
            if (_receiverMessageSendReq && !BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(), _notAllowedRecSendMessGatw) && !"ALL"
                ...
                if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                    if (_c2sTransferVO.getReceiverReturnMsg() == null) {
                        getReceiverSuccessMessage();
                        // (new
                        // PushMessage(_receiverMSISDN,getReceiverSuccessMessage(),_transferID,_c2sTransferVO.getRequestGatewayCode(),_receiverLocale)).push();
            ...
            if(_ussdReceiverMessageSendReq&&!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(),_notAllowedRecSendMessGatw)&&!"ALL".equals(_notAllowedRecSendMessGatw))
			{
				...
				{
					if(_c2sTransferVO.getReceiverReturnMsg()==null)
						(new USSDPushMessage(_receiverMSISDN,getReceiverFailMessage(),_transferID,_c2sTransferVO.getRequestGatewayCode(),_receiverLocale)).push();
					else if(_c2sTransferVO.getReceiverReturnMsg()!=null&&((BTSLMessages)_c2sTransferVO.getReceiverReturnMsg()).isKey())
					{
						BTSLMessages btslRecMessages=(BTSLMessages)_c2sTransferVO.getReceiverReturnMsg();
						(new USSDPushMessage(_receiverMSISDN,BTSLUtil.getMessage(_receiverLocale,btslRecMessages.getMessageKey(),btslRecMessages.getArgs()),_transferID,_c2sTransferVO.getRequestGatewayCode(),_receiverLocale)).push();
            ...
            if (!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(), _notAllowedSendMessGatw)) {
                ...
                if (btslMessages != null) {
                    ...
                    if (!BTSLUtil.isNullString(_c2sTransferVO.getSenderReturnMessage())) {
                        ...
                        // pushMessages=new
                        // PushMessage(_senderPushMessageMsisdn,getSenderSuccessMessage(),_transferID,_c2sTransferVO.getRequestGatewayCode(),_senderLocale);
    ...
    public void updateForReceiverValidateResponse(String str) throws BTSLBaseException {
        ...
        final HashMap map = BTSLUtil.getStringToHashNew(str, "&", "=");
        ...
        if (!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) || isRequired) {
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                    "CollectionEnquiryController[updateForReceiverValidateResponse]", _transferID, _senderMSISDN, _senderNetworkCode,
            ...
            if (!InterfaceErrorCodesI.SUCCESS.equals(status)) {
                ...
                throw new BTSLBaseException("CollectionEnquiryController", methodName, _c2sTransferVO.getErrorCode(), 0, strArr, null);
            }
            if (status.equals(InterfaceErrorCodesI.SUCCESS)) {
                final ArrayList _enquiryList = new ArrayList();

                if (!BTSLUtil.isStringContain(ClaroCollEnqWSI.RESPONSE_NOBILLPAY_SUCCESS, (String) map.get("INTERFACE_STATUS"))) {
                    EnquiryVO _enquiryVO = null;
                    _requestVO.setInvoiceSize(Integer.parseInt((String) map.get("INVOICE_SIZE")));
                    _requestVO.setXMontoDeudaTotal((String) map.get("TOTAL_PENDING_BALANCE"));
                    _requestVO.setXOpcionRecaudacion((String) map.get("SERVICE_NAME"));
                    _requestVO.setXCodTipoServicio((String) map.get("SERVICE_CODE"));
                    int  requestInvoiceSize = _requestVO.getInvoiceSize();
                    for (int i = 0; i < requestInvoiceSize; i++) {
                        ...
                        _enquiryVO.setxFechaEmision((String) map.get("BILL_PERIOD_START_" + i));
                        _enquiryVO.setxFechaVenc((String) map.get("BILL_PERIOD_END_" + i));
                        _enquiryList.add(i, _enquiryVO);
                    }
                } else {
                    _requestVO.setMessageCode(PretupsErrorCodesI.C2S_RECEIVER_BILLPAY_NOTFOUND);
                    _c2sTransferVO.setMessageCode(PretupsErrorCodesI.C2S_RECEIVER_BILLPAY_NOTFOUND);
                    _requestVO.setSuccessTxn(false);
                }
                _requestVO.setEnquiryItemList(_enquiryList);
            }
    ...
    public void updateForReceiverCreditResponse(String str) throws BTSLBaseException {
        ...
        if (!BTSLUtil.isNullString((String) map.get("IN_TXN_ID"))) {
            try {
                ...
                _log.errorTrace(methodName, e);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                    "CollectionEnquiryController[updateForReceiverCreditResponse]", _transferID, _senderMSISDN, _senderNetworkCode,
                    "Exception while parsing for interface txn ID , Exception:" + e.getMessage());
            }
        }
    ...
    public void populateServiceInterfaceDetails(Connection p_con, String action) throws BTSLBaseException {
        ...
        if (!isReceiverFound) {
            throw new BTSLBaseException("CollectionEnquiryController", "populateServiceInterfaceDetails", PretupsErrorCodesI.C2S_ERROR_NOTFOUND_SERVICEINTERFACEMAPPING);
        }
    }
    ...
    private void checkTransactionLoad() throws BTSLBaseException {
        ...
        try {
            ...
            if (recieverLoadStatus == 0) {
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug("CollectionEnquiryController[checkTransactionLoad]", "_transferID=" + _transferID + " Successfully through load");
                }
            }
            // Request in Queue
                ...
                throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.REQUEST_IN_QUEUE, 0, strArr, null);
            }
            // Refuse the request
            else {
                throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
            }
        } catch (BTSLBaseException be) {
            _log.error("CollectionEnquiryController[checkTransactionLoad]", "Refusing request getting Exception:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.errorTrace(methodName, e);
            throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        ...
        try {
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getValExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                    "CollectionEnquiryController[processValidationRequest]", _transferID, _senderMSISDN, _senderNetworkCode,
                    "Exception: System is taking more time till validation");
                throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_VAL);
            }
			...
			if (mcomCon != null) {
				mcomCon.close("CollectionEnquiryController#processValidationRequest");
				mcomCon = null;
			}
            con = null;
            ...
            // was debitted
            if (_transferDetailAdded && _userBalancesVO != null) {
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getTopUpExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "CollectionEnquiryController[run]", _transferID,
                    _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till topup");
                throw new BTSLBaseException("CollectionEnquiryController", "run", PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO = null;
            ...
            _log.error("CollectionEnquiryController[processValidationRequest]", "Getting BTSL Base Exception:" + be.getMessage());
    ...
    public String getReceiverValidateStr() {
        ...
        strBuff.append("&SERVICE_TYPE=" + _c2sTransferVO.getServiceType());
    ...
    private String getReceiverCommonString() {
        ...
        strBuff.append("&SENDER_MSISDN=" + _senderMSISDN);
        // To make origin Node Type configurable based on channel user's
        // category code : added by harsh
        ...
        if (!BTSLUtil.isNullString(_requestVO.getInvoiceno())) {
            strBuff.append("&INVOICE_NUMBER=" + _requestVO.getInvoiceno());
        }
    ...
    private String getReceiverSuccessMessage() {
        final String[] messageArgArray = null;
        KeyArgumentVO keyArgumentVO = null;
        final String key = null;
        EnquiryVO enqVO = null;
        final ArrayList arrList = new ArrayList();
        String arr[] = new String[2];
        String receiverSucsessMessage = null;
        keyArgumentVO = new KeyArgumentVO();
        keyArgumentVO.setKey(PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_LIST_COLLECTIONENQUIRY);
        final ArrayList _enqList = _requestVO.getEnquiryItemList();
        if (_enqList.size() > 0) {
            for (int i = 0; i < (_requestVO.getInvoiceSize() > Integer.parseInt(Constants.getProperty("NO_OF_LIST_COLLECTIONENQUIRY_RECMSG")) ? Integer.parseInt(Constants
                .getProperty("NO_OF_LIST_COLLECTIONENQUIRY_RECMSG")) : _requestVO.getInvoiceSize()); i++) {
                enqVO = (EnquiryVO) _enqList.get(i);

                final String arr1[] = { enqVO.getxNumeroDoc(), enqVO.getxMontoDebe(), URLDecoder.decode(enqVO.getxFechaEmision()), _receiverMSISDN, URLDecoder.decode(enqVO
                    .getxFechaVenc()), enqVO.getxTipoServicio(), enqVO.getxDescripServ(), enqVO.getxMontoDebe(), enqVO.getxMontoFact() };
                keyArgumentVO.setArguments(arr1);
                arrList.add(keyArgumentVO);
                final String array = BTSLUtil.getMessage(_receiverLocale, keyArgumentVO.getKey(), arr1, _requestVO.getRequestGatewayType());

                (new PushMessage(_receiverMSISDN, array, _transferID, _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
            }
        } else {
            arr = new String[] { _requestVO.getXMontoDeudaTotal(), _requestVO.getXOpcionRecaudacion(), _transferID };
            receiverSucsessMessage = BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_COLLECTIONENQUIRY, arr, _requestVO.getRequestGatewayType());
            new PushMessage(_receiverMSISDN, receiverSucsessMessage, _transferID, _c2sTransferVO.getRequestGatewayCode(), _receiverLocale).push();
        }
        return receiverSucsessMessage;
    }
	 ...
	 * @return
	 */
	private String getReceiverUSSDSuccessMessage()
	{
		...
		keyArgumentVO=new KeyArgumentVO();
		keyArgumentVO.setKey(PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_LIST_COLLECTIONENQUIRY);
		ArrayList _enqList = _requestVO.getEnquiryItemList();
		if(_enqList.size()>0){	
			for(int i=0;i<(_requestVO.getInvoiceSize()>Integer.parseInt(Constants.getProperty("NO_OF_LIST_COLLECTIONENQUIRY_RECMSG"))?Integer.parseInt(Constants.getProperty("NO_OF_LIST_COLLECTIONENQUIRY_RECMSG")):_requestVO.getInvoiceSize());i++)
			{
				enqVO = (EnquiryVO)_enqList.get(i);
				
				String arr1[]={enqVO.getxNumeroDoc(),enqVO.getxMontoDebe(),URLDecoder.decode(enqVO.getxFechaEmision()),_receiverMSISDN,URLDecoder.decode(enqVO.getxFechaVenc()),enqVO.getxTipoServicio(),enqVO.getxDescripServ(),enqVO.getxMontoDebe(),enqVO.getxMontoFact()};
				keyArgumentVO.setArguments(arr1);
				arrList.add(keyArgumentVO);
				String array = BTSLUtil.getMessage(_receiverLocale,keyArgumentVO.getKey(),arr1,_requestVO.getRequestGatewayType());

				(new USSDPushMessage(_receiverMSISDN,array,_transferID,_c2sTransferVO.getRequestGatewayCode(),_receiverLocale)).push();
			}
		}
		else{
			arr=new String[]{_requestVO.getXMontoDeudaTotal(),_requestVO.getXOpcionRecaudacion(),_transferID,_receiverMSISDN};
			receiverSucsessMessage=BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_COLLECTIONENQUIRY,arr,_requestVO.getRequestGatewayType());
			new USSDPushMessage(_receiverMSISDN,receiverSucsessMessage,_transferID,_c2sTransferVO.getRequestGatewayCode(),_receiverLocale).push();
		}
		return  receiverSucsessMessage;
	}
    ...
    private String getSenderSuccessMessage() {
        ...
        if (_enqList.size() > 0) {
            for (int i = 0; i < (_requestVO.getInvoiceSize() > Integer.parseInt(Constants.getProperty("NO_OF_LIST_COLLECTIONENQUIRY_RECMSG")) ? Integer.parseInt(Constants
                ...
                keyArgumentVO.setArguments(arr1);
                arrList.add(keyArgumentVO);
                final String array = BTSLUtil.getMessage(_senderLocale, keyArgumentVO.getKey(), arr1, _requestVO.getRequestGatewayType());
                (new PushMessage(_senderMSISDN, array, _transferID, _c2sTransferVO.getRequestGatewayCode(), _senderLocale)).push();
            }
        } else {
            arr = new String[] { _requestVO.getXMontoDeudaTotal(), _requestVO.getXOpcionRecaudacion(), _transferID };
            receiverSucsessMessage = BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_COLLECTIONENQUIRY, arr, _requestVO.getRequestGatewayType());
            new PushMessage(_senderMSISDN, receiverSucsessMessage, _transferID, _c2sTransferVO.getRequestGatewayCode(), _senderLocale).push();
        }
        return receiverSucsessMessage;
    ...
    private String getReceiverUnderProcessMessage() {
        final String[] messageArgArray = { _transferID, _receiverMSISDN, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderPushMessageMsisdn, _transferID, _channelUserVO
            .getUserName(), _requestVO.getPosUserMSISDN() };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.C2S_RECEIVER_UNDERPROCESS_COLLECTIONENQUIRY, messageArgArray, _requestVO.getRequestGatewayType());
    ...
    private String getSndrUPMsgBeforeValidation() {
        final String[] messageArgArray = { _transferID, _receiverMSISDN, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderPushMessageMsisdn, _transferID, _channelUserVO
            .getUserName(), _requestVO.getPosUserMSISDN() };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.C2S_SENDER_UNDERPROCESS_B4VAL_COLLECTIONENQUIRY, messageArgArray, _requestVO.getRequestGatewayType());

    }

    private String getReceiverAmbigousMessage() {
        final String[] messageArgArray = { _transferID, _senderPushMessageMsisdn, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderPushMessageMsisdn, _transferID, _channelUserVO
            .getUserName(), _requestVO.getPosUserMSISDN() };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.C2S_RECEIVER_AMBIGOUS_KEY_COLLECTIONENQUIRY, messageArgArray, _requestVO.getRequestGatewayType());
    ...
    private String getReceiverFailMessage() {
        final String[] messageArgArray = { _transferID, _senderPushMessageMsisdn, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderPushMessageMsisdn, _transferID, _channelUserVO
            .getUserName(), _requestVO.getPosUserMSISDN() };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.C2S_RECEIVER_FAIL_KEY_COLLECTIONENQUIRY, messageArgArray, _requestVO.getRequestGatewayType());
    ...
    private void updateSenderForFailedTransaction(Connection p_con) throws BTSLBaseException {
        ...
        if (_requestVO.getMessageGatewayVO().getFlowType().equals(PretupsI.MSG_GATEWAY_FLOW_TYPE_REQUEST)) {
            ...
            _requestVO.setMessageCode(PretupsErrorCodesI.C2S_SENDER_CREDIT_SUCCESS_ENQUIRY);
        }
        TransactionLog.log(_transferID, _requestIDStr, _senderMSISDN, _senderNetworkCode, PretupsI.TXN_LOG_REQTYPE_INT, PretupsI.TXN_LOG_TXNSTAGE_PROCESS, "Credit Back Done",
    ...
    private void processValidationRequestInThread() throws BTSLBaseException, Exception {
        ...
        try {
            ...
            _log.error("CollectionEnquiryController[processValidationRequestInThread]", "Getting BTSL Base Exception:" + be.getMessage());
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "CollectionEnquiryController[processValidationRequestInThread]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            TransactionLog.log(_transferID, _requestIDStr, _receiverMSISDN, _receiverVO.getNetworkCode(), PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
            ...
            if (_transferID != null) {
                ...
                try {
					...
					if (mcomCon != null) {
						mcomCon.close("CollectionEnquiryController#processValidationRequestInThread");
						mcomCon = null;
					}
                    con=null;
                    _log.errorTrace(methodName, be);
    ...
    private void addEntryInTransfers(Connection p_con) {
        ...
        try {
            ...
            if (!_transferDetailAdded && _transferEntryReqd) {
                ...
                // in
                // database
                _transferDetailAdded = true;// added by akanksha
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[addEntryInTransfers]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            if (!_isCounterDecreased && _decreaseTransactionCounts) {
                LoadController.decreaseTransactionLoad(_transferID, _senderNetworkCode, LoadControllerI.DEC_LAST_TRANS_COUNT);
                _isCounterDecreased = true;
    ...
    private boolean getInterfaceRoutingDetails(Connection p_con, String p_msisdn, long p_prefixID, String p_subscriberType, String p_networkCode, String p_serviceType, String p_interfaceCategory, String p_userType, String p_action) throws BTSLBaseException {
        ...
        try {
            ...
            if (subscriberRoutingControlVO != null) {
                                ...
                                try {
                                    ...
                                    EventHandler
                                        .handle(
                                            ...
                                            EventStatusI.RAISED,
                                            EventLevelI.MAJOR,
                                            "CollectionEnquiryController[getInterfaceRoutingDetails]",
                                            _transferID,
                                            _receiverMSISDN,
                                            "",
                ...
                // based routing
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                    "CollectionEnquiryController[getInterfaceRoutingDetails]", _transferID, _senderMSISDN, _senderNetworkCode,
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "CollectionEnquiryController[getInterfaceRoutingDetails]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            isSuccess = false;
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        }
        if (_log.isDebugEnabled()) {
    ...
    private void performAlternateRouting(ArrayList altList) throws BTSLBaseException {
        ...
        try {
            if (altList != null && altList.size() > 0) {
                ...
                case 1: {
                    ...
                    try {
                        ...
                        // commented for time being. At this time this is not
                    ...
                    try {
                        ...
                        if (be.isKey() && InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(be.getMessageKey())) {
                            ...
                            try {
                                 ...
                                 * PretupsBL.updateSubscriberInterfaceRouting(
                                 * _receiverTransferItemVO
                                 * .getInterfaceID(),_externalID
                                 * ,_receiverMSISDN,
                                 * _type,_channelUserVO.getUserID
                                 * (),_currentDate);
                                 * else
                                 * {
                                 * SubscriberRoutingControlVO
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[performAlternateRouting]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        }
    }
    ...
    public void receiverValidateResponse(String str, int p_attempt, int p_altSize) throws BTSLBaseException {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "CollectionEnquiryController[receiverValidateResponse]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception while parsing for interface txn ID , Exception:" + e.getMessage());
        }
        // }
        ...
        if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
            ...
            throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.C2S_RECEIVER_FAIL_COLLECTION, 0, strArr, null);
        }
        try {
            _receiverTransferItemVO.setPreviousBalance(Long.parseLong((String) map.get("INTERFACE_PREV_BALANCE")));
        } catch (Exception e) {
    ...
    public void processFromQueue(TransferVO p_transferVO) {
        ...
        try {
			...
			if (mcomCon != null) {
				mcomCon.close("CollectionEnquiryController#processFromQueue");
				mcomCon = null;
			}
            con = null;

            if (_log.isDebugEnabled()) {
                _log.debug("CollectionEnquiryController[processFromQueue]", "_transferID=" + _transferID + " Successfully through load");
            }
            ...
            // raising alarm
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "CollectionEnquiryController[processFromQueue]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            // logging in the transaction log
            TransactionLog.log(_transferID, _requestVO.getRequestIDStr(), _requestVO.getFilteredMSISDN(), "", PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
                _c2sTransferVO.getSenderReturnMessage(), PretupsI.TXN_LOG_STATUS_FAIL, "Getting Code=" + _requestVO.getMessageCode());
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "CollectionEnquiryController[processFromQueue]",
                    _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            }
        ...
        if (!BTSLUtil.isNullString(p_languageCode)) {
            try {
                if (LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode) == null) {
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                        "CollectionEnquiryController[updateReceiverLocale]", _transferID, _receiverMSISDN, "",
     ...
     * generatePostPaidBillPmtTransferID(TransferVO p_transferVO)
     * {
     ...
     * throw new BTSLBaseException("CollectionEnquiryController",
     * "generatePostPaidBillPmtTransferID"
    ...
    private static synchronized void generatePostPaidBillPmtTransferID(TransferVO p_transferVO) {
        ...
        try {
            ...
            if (_transactionIDCounter == 0) {
                throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
            transferID = _operatorUtil.formatTransferID(p_transferVO, _transactionIDCounter, "C");
            if (transferID == null) {
                throw new BTSLBaseException("CollectionEnquiryController", methodName, PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
            p_transferVO.setTransferID(transferID);
