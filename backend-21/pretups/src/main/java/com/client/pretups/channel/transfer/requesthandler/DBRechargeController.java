...
public class DBRechargeController implements ServiceKeywordControllerI,Runnable 
{
	private Log _log = LogFactory.getLog(DBRechargeController.class.getName());
...
//    private double _receiverConversionFactor=1;
	...
	static
    {
		...
		{
			e.printStackTrace();
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[initialize]","","","","Exception while loading the class at the call:"+e.getMessage());
		}
    }	
    public  DBRechargeController()
	{
	...
	public void process(RequestVO p_requestVO) 
	{
		...
		try
		{
			...
			_operatorUtil.validateProductRechargeRequest(con,_c2sTransferVO,p_requestVO);
			//Handling of Private Recharge 
			if(!BTSLUtil.isNullString(_requestVO.getSid())){
				_c2sTransferVO.setSID(_requestVO.getSid());
			}
			if(_log.isDebugEnabled()) {
				_log.debug(methodName,"_c2sTransferVO.getSID()="+_c2sTransferVO.getSID()+",_requestVO.getSid()="+_requestVO.getSid());
			}
			...
			if(!_receiverVO.getSubscriberType().equals(_type))
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"DBRechargeController[process]","","","","Series ="+_receiverVO.getMsisdnPrefix()+" Not Defined for Series type="+_type+" But request initiated for the same" );
			...
			try {con.commit();} catch(Exception e){ throw new BTSLBaseException("DBRechargeController","process",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
			...
			else
			{
				...
				try {con.commit();} catch(Exception e){ throw new BTSLBaseException("DBRechargeController","process",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
				try {con.close();} catch(Exception e) { throw new BTSLBaseException("DBRechargeController","process",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
				con=null;
				...
				else if(p_requestVO.getMessageGatewayVO().getFlowType().equals(PretupsI.MSG_GATEWAY_FLOW_TYPE_REQUEST))
				{
					...
					if(!BTSLUtil.isNullString(_requestVO.getSid())) {
						messageArgArray = new String[]{_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPreviousBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),_c2sTransferVO.getProductName()};
					} else {
						messageArgArray = new String[]{_receiverMSISDN,_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPreviousBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),_c2sTransferVO.getProductName()};
					}
		...
		{	
			...
			catch(BTSLBaseException bex)
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[process]",_transferID,_senderMSISDN,_senderNetworkCode,"Leaving Reciever Unmarked Base Exception:"+bex.getMessage());
			}
			catch(Exception e)
			{
				e.printStackTrace();
				_c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[process]",_transferID,_senderMSISDN,_senderNetworkCode,"Leaving Reciever Unmarked Base Exception:"+e.getMessage());
			}
			...
			if(_recValidationFailMessageRequired)
			{
				...
				if(_c2sTransferVO.getReceiverReturnMsg()==null || !((BTSLMessages)_c2sTransferVO.getReceiverReturnMsg()).isKey())
				{
					...
					if(_transferID!=null)
						_c2sTransferVO.setReceiverReturnMsg(new BTSLMessages(PretupsErrorCodesI.DBRC_RECEIVER_FAIL,new String[]{String.valueOf(_transferID),PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount())}));
					else
...
//			Populate the ChannelRequestDailyLogVo and log
		...
		catch(Exception e)
		{
			...
			catch(Exception ex)
			{
				...
				_c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[process]",_transferID,_senderMSISDN,_senderNetworkCode,"Leaving Reciever Unmarked Base Exception:"+ex.getMessage());
			}
			...
			//raising alarm
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[process]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
...
//			Populate the ChannelRequestDailyLogVo and log
		...
		finally
		{
			...
			{
				_log.errorTrace("processFromQueue", be);
	...
	private void processSKeyGen(Connection p_con) throws BTSLBaseException,Exception
	{
		...
		catch(Exception e)
		{
			...
			e.printStackTrace();
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[processSKeyGen]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			throw new BTSLBaseException("DBRechargeController","processSKeyGen",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
		}
		finally
		{
			if(_log.isDebugEnabled())
	...
	public void processTransfer(Connection p_con) throws BTSLBaseException
	{
		...
		try
		{
			...
			generateDBRCTransferID(_c2sTransferVO);
		...
		catch(Exception e)
		{
			...
			_c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
			e.printStackTrace();
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[processTransfer]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			throw new BTSLBaseException("DBRechargeController","processTransfer",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
		}
	}
	...
	public void run() 
	{
		...
		if(_log.isDebugEnabled())_log.debug(methodName,_transferID,"Entered");
		...
		try
		{
			...
			if(_log.isDebugEnabled())_log.debug(methodName,_transferID,"Got the response from IN Module receiverCreditResponse="+receiverCreditResponse);
			...
			if(PretupsI.YES.equals(_c2sTransferVO.getDifferentialAllowedForService()))
			{
				...
				catch(BTSLBaseException be)
				{
					...
					if(_log.isDebugEnabled()) _log.debug("DBRechargeController","For _transferID="+_transferID+" Diff applicable="+_c2sTransferVO.getDifferentialApplicable()+" Diff Given="+_c2sTransferVO.getDifferentialGiven() +" Not able to give Diff commission getting BTSL Base Exception="+be.getMessage()+" Leaving transaction status as Under process");
					EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"DBRechargeController[run]",_c2sTransferVO.getTransferID(),_c2sTransferVO.getSenderMsisdn(),_c2sTransferVO.getNetworkCode(),"Exception:"+be.getMessage());
				}
				catch(Exception e)
				{
					_finalTransferStatusUpdate=false;
					if(_log.isDebugEnabled()) _log.debug("DBRechargeController","For _transferID="+_transferID+" Diff applicable="+_c2sTransferVO.getDifferentialApplicable()+" Diff Given="+_c2sTransferVO.getDifferentialGiven() +" Not able to give Diff commission getting Exception="+e.getMessage()+" Leaving transaction status as Under process");
				}
			}//end if
			...
			if(_log.isDebugEnabled()) _log.debug("DBRechargeController","For _transferID="+_transferID+" Diff applicable="+_c2sTransferVO.getDifferentialApplicable()+" Diff Given="+_c2sTransferVO.getDifferentialGiven());
			...
			if(((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.LMS_APPL)).booleanValue() && _c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS))
			{
				try{
					if(_log.isDebugEnabled()) {
						_log.debug("process","getControlGroup="+_channelUserVO.getControlGroup()+",getOptInOutStatus="+_channelUserVO.getOptInOutStatus()+",OPT_IN_OUT_ALLOW="+((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.OPT_IN_OUT_ALLOW)).booleanValue());
					}
					if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.OPT_IN_OUT_ALLOW)).booleanValue()) {
						if(!BTSLUtil.isNullString(_channelUserVO.getLmsProfile()) && !BTSLUtil.isNullString(_channelUserVO.getOptInOutStatus()) && !BTSLUtil.isNullString(_channelUserVO.getControlGroup())  && PretupsI.NO.equalsIgnoreCase(_channelUserVO.getControlGroup()) && ( PretupsI.OPT_IN.equalsIgnoreCase(_channelUserVO.getOptInOutStatus()) || PretupsI.NORMAL.equalsIgnoreCase(_channelUserVO.getOptInOutStatus())))
						{
							...
							//Handling of OPT-IN/OPT-OUT incase of OPT type profile associated and no response received from end user
							try {
								LoyaltyController _loyaltyController= new LoyaltyController();
								con=OracleUtil.getConnection();
								PromotionDetailsVO promotionDetailsVO=_loyaltyController.loadProfile(con, loyaltyVO.getSetId());
								if(promotionDetailsVO!=null){
									if(PretupsI.YES.equalsIgnoreCase(promotionDetailsVO.getOptInOutEnabled())){
										if(PretupsI.NORMAL.equalsIgnoreCase(_channelUserVO.getOptInOutStatus())){
											if(_log.isDebugEnabled()) _log.debug("process"," No Bonus will be provided to user because of No response received from user end.");
								...
								if(con!=null) 
								{
									try{con.commit();}catch(Exception e){_log.errorTrace("process", e);}
									try{con.close();}catch(Exception e){_log.errorTrace("process", e);}
									con=null;
								}
		...
		{	
			...
			if(_log.isDebugEnabled())_log.debug(methodName,_transferID,"Error Code:"+_c2sTransferVO.getErrorCode());
		...
		catch(Exception e)
		{
			...
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
		...
		finally
		{
			...
			catch(Exception e)
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception while updating Receiver last request status in database , Exception:"+e.getMessage());
			}//end catch
			...
			catch(Exception e)
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception while updating transfer details in database , Exception:"+e.getMessage());
			}
			...
			if(!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(),_notAllowedSendMessGatw))
			{
				...
				{
					if(_log.isDebugEnabled()) {
						_log.debug(methodName,"((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ADD_COMM_SEPARATE_MSG))).booleanValue()="+((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ADD_COMM_SEPARATE_MSG))).booleanValue()+",is additional comminion flag ="+!BTSLUtil.isNullString(_c2sTransferVO.getSenderReturnMessage()));
					}
...
//			Populate the ChannelRequestDailyLogVo and log
			...
			_userBalancesVO=null;
			commonClient=null;
			if(_log.isDebugEnabled())_log.debug(methodName,_transferID,"Exiting");
		}//end of finally
	}
	...
	public void updateForReceiverValidateResponse(String str) throws BTSLBaseException
	{
		...
		if(!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) || isRequired)
		{
			...
			_receiverTransferItemVO.setReferenceID((String)map.get("REQUEST_ID"));
			...
			if(BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS))
			{
				...
				//throw new BTSLBaseException("DBRechargeController","updateForReceiverValidateResponse",PretupsErrorCodesI.C2S_RECEIVER_FAIL,0,strArr,null);
				//throw new BTSLBaseException("DBRechargeController","updateForReceiverValidateResponse",_c2sTransferVO.getErrorCode(),0,strArr,null);
				if(InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED.equals(_receiverTransferItemVO.getValidationStatus()))
					throw new BTSLBaseException("DBRechargeController","updateForReceiverValidateResponse",InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED+"_S",0,strArr,null);
				else
					throw new BTSLBaseException("DBRechargeController","updateForReceiverValidateResponse",_c2sTransferVO.getErrorCode(),0,strArr,null);
			}
			_receiverTransferItemVO.setTransferStatus(status);
...
//		End 07/02/2008
		...
		{
			try{_receiverTransferItemVO.setInterfaceReferenceID((String)map.get("TRANSACTION_ID"));}
			catch(Exception e){e.printStackTrace();}
		}
		...
		else if (status.equals(InterfaceErrorCodesI.AMBIGOUS))
		{
			...
			//change for SID in case of private recharge gets failed
			if(!BTSLUtil.isNullString(_requestVO.getSid())){
				strArr=new String[]{_transferID,_requestVO.getSid(),PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),_c2sTransferVO.getProductName()};
			} else {
				strArr=new String[]{_transferID,_receiverTransferItemVO.getMsisdn(),PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),_c2sTransferVO.getProductName()};
			}
	...
	public void populateServiceInterfaceDetails(Connection p_con,String action) throws BTSLBaseException
	{
		...
		if(!isReceiverFound)
			throw new BTSLBaseException("DBRechargeController","populateServiceInterfaceDetails",PretupsErrorCodesI.C2S_ERROR_NOTFOUND_SERVICEINTERFACEMAPPING);
	...
	private void checkTransactionLoad() throws BTSLBaseException
	{
		...
		try
		{
			...
			if(recieverLoadStatus==0)
			{
				...
				if(_log.isDebugEnabled()) _log.debug("DBRechargeController[checkTransactionLoad]","_transferID="+_transferID+" Successfully through load");
			}
			//Request in Queue
			else if(recieverLoadStatus==1)
			{
				...
				throw new BTSLBaseException("DBRechargeController","checkTransactionLoad",PretupsErrorCodesI.REQUEST_IN_QUEUE,0,strArr,null);
			}
			//Refuse the request
			else
				throw new BTSLBaseException("DBRechargeController","checkTransactionLoad",PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
		}
		catch(BTSLBaseException be)
		{
			_log.error("DBRechargeController[checkTransactionLoad]","Refusing request getting Exception:"+be.getMessage());
			throw be;
		}
		catch(Exception e)
		{
			e.printStackTrace();
			_log.error("DBRechargeController[checkTransactionLoad]","Refusing request getting Exception:"+e.getMessage());
			throw new BTSLBaseException("DBRechargeController","checkTransactionLoad",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
		}
	}
	...
	private void processValidationRequest() throws BTSLBaseException,Exception
	{
		...
		try
		{
            ...
            {
            	EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"DBRechargeController[processValidationRequest]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception: System is taking more time till validation");
            	throw new BTSLBaseException("DBRechargeController","processValidationRequest",PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_VAL);
            }
            ...
            {
            	EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"DBRechargeController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception: System is taking more time till topup");
            	throw new BTSLBaseException("DBRechargeController","processValidationRequest",PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO=null;
		...
		catch(BTSLBaseException be)
		{
			...
			_log.error("DBRechargeController[processValidationRequest]","Getting BTSL Base Exception:"+be.getMessage());
			if(_transferDetailAdded)
			{
				if(con==null)
					con=OracleUtil.getConnection();
		...
		catch(Exception e)
		{
			...
			throw new BTSLBaseException("DBRechargeController","processValidationRequest",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
 		}
		finally
		{
	...
	private String getReceiverCommonString()
	{
		...
		strBuff.append("&SELECTOR_BUNDLE_ID="+_receiverBundleID);
        //Set Domain Code Value for CP_ID(Mali)
	...
	private String getReceiverSuccessMessage()
	{
		...
		if(!("N".equals(_c2sTransferVO.getBoth()) && "N".equals(_c2sTransferVO.getOnline())))
		{
			...
			key=PretupsErrorCodesI.DBRC_IMPLICIT_MSG;
		}
		else
		{
			//For Get NUMBER BACK Service
			if(_c2sTransferVO.getReceiverTransferItemVO().isNumberBackAllowed())
			{
				...
				if(_c2sTransferVO.getBonusTalkTimeValue()==0)
					key=PretupsErrorCodesI.DBRC_RECEIVER_GET_NUMBER_BACK_SUCCESS;
				else
					key=PretupsErrorCodesI.DBRC_RECEIVER_GET_NUMBER_BACK_SUCCESS_WITH_BONUS;
			}
			...
			if(!"N".equals(_receiverPostBalanceAvailable))
			{
				...
				if(_c2sTransferVO.getBonusTalkTimeValue()==0)
					key=PretupsErrorCodesI.DBRC_RECEIVER_SUCCESS;//return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS,messageArgArray);
				else
					key=PretupsErrorCodesI.DBRC_RECEIVER_SUCCESS_WITH_BONUS;
			}
			else
			{
				if(((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.IS_SEPARATE_BONUS_REQUIRED))).booleanValue())
		...
		if(!BTSLUtil.isNullString(_receiverTransferItemVO.getChangedBundleCodes()) && _receiverTransferItemVO.getChangedBundleCodes().length()>0)
		{
		    ...
		    key=PretupsErrorCodesI.DBRC_RECEIVER_SUCCESS_ALL_BALANCES;
		}
	...
	private String getSenderSuccessMessage()
	{
		...
		String[] messageArgArray=null;
		if(!BTSLUtil.isNullString(_requestVO.getSid())) {
			messageArgArray = new String[]{_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),_c2sTransferVO.getProductName()};
		} else {
		...
		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.DBRC_SENDER_SUCCESS,messageArgArray);
	}
	/**
	...
	private String getReceiverUnderProcessMessage()
	{
		...
		return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.DBRC_RECEIVER_UNDERPROCESS,messageArgArray,_requestVO.getRequestGatewayType());
	}
	...
	private String getSenderUnderProcessMessage()
	{
		...
		String[] messageArgArray=null;
		if(!BTSLUtil.isNullString(_requestVO.getSid())){
			messageArgArray=new String[]{_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),_c2sTransferVO.getProductName()};
		} else {
		...
		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.DBRC_SENDER_UNDERPROCESS,messageArgArray);
	}
	...
	private String getSndrUPMsgBeforeValidation()
	{
		...
		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.DBRC_SENDER_UNDERPROCESS_B4VAL,messageArgArray);
	}
	...
	private String getReceiverAmbigousMessage()
	{
		...
		return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.DBRC_RECEIVER_AMBIGOUS_KEY,messageArgArray,_requestVO.getRequestGatewayType());
	}	
	private String getReceiverFailMessage()
	{
		String[] messageArgArray={_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),_senderPushMessageMsisdn,_channelUserVO.getUserName(),_requestVO.getPosUserMSISDN()};
		return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.DBRC_RECEIVER_FAIL_KEY,messageArgArray,_requestVO.getRequestGatewayType());
	}	
	/**
	...
	private void updateSenderForFailedTransaction(Connection p_con) throws BTSLBaseException
	{
		try
		{
			...
			if(_requestVO.getMessageGatewayVO().getFlowType().equals(PretupsI.MSG_GATEWAY_FLOW_TYPE_REQUEST))
			{
				...
				String[] messageArgArray=null;
				if(!BTSLUtil.isNullString(_requestVO.getSid())) {
					messageArgArray=new String[]{_c2sTransferVO.getReceiverMsisdn(),PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),_c2sTransferVO.getTransferID(),PretupsBL.getDisplayAmount(_userBalancesVO.getBalance()),_c2sTransferVO.getProductName()};
				} else {
					messageArgArray=new String[]{_c2sTransferVO.getReceiverMsisdn(),PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),_c2sTransferVO.getTransferID(),PretupsBL.getDisplayAmount(_userBalancesVO.getBalance()),_c2sTransferVO.getProductName()};
				}
				_requestVO.setMessageArguments(messageArgArray);
				_requestVO.setMessageCode(PretupsErrorCodesI.DBRC_SENDER_CREDIT_SUCCESS);
			}
	...
	private void processValidationRequestInThread() throws BTSLBaseException,Exception
	{
		...
		{
			_log.error("DBRechargeController[processValidationRequestInThread]","Getting BTSL Base Exception:"+be.getMessage());
		...
		catch(Exception e)
		{
			...
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[processValidationRequestInThread]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
		...
		finally
		{
			...
			{	
				...
				catch(Exception e)
				{
					...
					e.printStackTrace();
					_log.error("process","Exception:"+e.getMessage());
					EventHandler.handle(EventIDI.DATABASE_CONECTION_PROBLEM,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[processValidationRequestInThread]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
				}
				finally
				{
					if(con!=null)try{con.close();}catch(Exception e){e.printStackTrace();}
	...
	private boolean getInterfaceRoutingDetails(Connection p_con,String p_msisdn,long p_prefixID,String p_subscriberType,String p_networkCode,String p_serviceType,String p_interfaceCategory,String p_userType,String p_action)  throws BTSLBaseException
	{
		...
		if(_log.isDebugEnabled()) _log.debug(methodName," Entered with MSISDN="+p_msisdn+" Prefix ID="+p_prefixID+" p_subscriberType="+p_subscriberType+" p_networkCode="+p_networkCode+" p_serviceType="+p_serviceType+" p_interfaceCategory="+p_interfaceCategory+" p_userType="+p_userType+" p_action="+p_action);
		...
		try
		{
			if(_log.isDebugEnabled()) _log.debug(methodName," subscriberRoutingControlVO="+subscriberRoutingControlVO);
					...
					if(p_interfaceCategory.equalsIgnoreCase(PretupsI.INTERFACE_CATEGORY_PRE))
					{
						...
						else if(subscriberRoutingControlVO.isSeriesCheckBool())
						{
							...
							if(((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SELECTOR_INTERFACE_MAPPING)).booleanValue())
							{
								...
								if(interfaceMappingVO1==null) {
									MSISDNPrefixInterfaceMappingVO interfaceMappingVO = (MSISDNPrefixInterfaceMappingVO) MSISDNPrefixInterfaceMappingCache.getObject(p_prefixID,interfaceType,p_action); 
			...
			else
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"DBRechargeController[getInterfaceRoutingDetails]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:Routing control information not defined so performing series based routing");
				ServiceSelectorInterfaceMappingVO interfaceMappingVO1=null;
				if(((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SELECTOR_INTERFACE_MAPPING)).booleanValue())
				{
					interfaceMappingVO1=(ServiceSelectorInterfaceMappingVO)ServiceSelectorInterfaceMappingCache.getObject(_serviceType+"_"+_c2sTransferVO.getSubService()+"_"+p_action+"_"+p_networkCode+"_"+p_prefixID);
		...
		{
			e.printStackTrace();
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[getInterfaceRoutingDetails]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			isSuccess=false;
			throw new BTSLBaseException(this,methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
		}
		if(_log.isDebugEnabled()) _log.debug(methodName," Exiting with isSuccess="+isSuccess);
		return isSuccess;
	}
	...
	private void performAlternateRouting(ArrayList altList) throws BTSLBaseException
	{
		...
		{
			e.printStackTrace();
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[performAlternateRouting]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
	...
	public void processFromQueue(TransferVO p_transferVO)
	{
		...
		try
		{
			...
			_receiverVO.setUnmarkRequestStatus(true);
			try {con.commit();} catch(Exception e){ throw new BTSLBaseException("DBRechargeController","processFromQueue",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
			try {con.close();} catch(Exception e) { throw new BTSLBaseException("DBRechargeController","processFromQueue",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
			con=null;
						
			
			if(_log.isDebugEnabled()) _log.debug("DBRechargeController[processFromQueue]","_transferID="+_transferID+" Successfully through load");
		...
		catch(BTSLBaseException be)
		{
			...
			{
				_log.errorTrace("processFromQueue", bex);
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,"Leaving Reciever Unmarked Base Exception:"+bex.getMessage());
			}
			catch(Exception e)
			{
				e.printStackTrace();
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,"Leaving Reciever Unmarked Base Exception:"+e.getMessage());
				_c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
			}
		...
		{
			if(con!=null)
				try {con.close();} catch(Exception ex) {_log.errorTrace("processFromQueue", ex);}
			con=null;
			...
			{
				_log.errorTrace("processFromQueue", ex);
				_c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,"Leaving Reciever Unmarked Exception:"+ex.getMessage());
			}
			//checking condition whether channel receiver required the general failure message
			if(_recValidationFailMessageRequired)
			{
			...
			//raising alarm
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"DBRechargeController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			//logging in the transaction log
			TransactionLog.log(_transferID,_requestVO.getRequestIDStr(),_requestVO.getFilteredMSISDN(),"",PretupsI.TXN_LOG_REQTYPE_RES,PretupsI.TXN_LOG_TXNSTAGE_PROCESS,_c2sTransferVO.getSenderReturnMessage(),PretupsI.TXN_LOG_STATUS_FAIL,"Getting Code="+_requestVO.getMessageCode());
		}
		finally
		...
		if(!BTSLUtil.isNullString(p_languageCode))
		{
			try
			{
				if(LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode)==null)
					EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"DBRechargeController[updateReceiverLocale]",_transferID,_receiverMSISDN,"","Exception: Notification language returned from IN is not defined in system p_languageCode: "+p_languageCode);
				else
	...
	/*private static synchronized void generateC2STransferID(TransferVO p_transferVO)
	{
		...
		try
		{
			...
			if(_transactionIDCounter==0)
				throw new BTSLBaseException("DBRechargeController","generateC2STransferID",PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
	...
	private static synchronized void generateDBRCTransferID(TransferVO p_transferVO)
	{
		...
		try
		{
			...
			p_transferVO.setCreatedOn(mydate);
	  		...
	  		if(_transactionIDCounter==0)
				throw new BTSLBaseException("DBRechargeController","generateDBRCTransferID",PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
			transferID=_operatorUtil.formatDBRCTransferID(p_transferVO,_transactionIDCounter);
			if(transferID==null)
				throw new BTSLBaseException("DBRechargeController","generateDBRCTransferID",PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
			p_transferVO.setTransferID(transferID);
		}
...
/*	This code was commented on 01/04/08 to eliminate fetch conversion rate step.
	...
	private String getSenderParentSuccessMessage()
	{
		String[] messageArgArray=null;
		if(!BTSLUtil.isNullString(_requestVO.getSid())) {
			messageArgArray = new String[]{_receiverMSISDN,_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),_requestVO.getSenderLoginID(),_c2sTransferVO.getProductName()};
		} else {
			messageArgArray = new String[]{_receiverMSISDN,_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),_requestVO.getSenderLoginID(),_c2sTransferVO.getProductName()};
		}
