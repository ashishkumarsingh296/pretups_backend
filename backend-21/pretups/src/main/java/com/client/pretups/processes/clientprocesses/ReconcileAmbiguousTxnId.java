...
import com.inter.righttel.zte.ZTEINPoolLoaderServlet;
import com.web.pretups.channel.transfer.businesslogic.ChannelTransferWebDAO;

/**
 * @(#)ReconcileAmbiguousTxnId.java
 ...
 *                               ----------------------------------------------
 *                               ---
 *                               June 26, 2018 Initial Creation
 * 
 */

public class ReconcileAmbiguousTxnId {
   
    private static Log _log = LogFactory.getLog(ReconcileAmbiguousTxnId.class.getName());
    ...
    private static PreparedStatement _pstmtSOSSelect = null;
    private static PreparedStatement _pstmtSOSReconcilationStatusUpdate = null;
    ...
    public static void main(String args[]) {
        ...
        if (args.length != 3) {
            _log.info(methodName, "Usage : ReconcileAmbiguousTxnId [Constants file] [LogConfig file] [File Name]");
            return;
        }
        final File constantsFile = new File(args[0]);
        if (!constantsFile.exists()) {
            _log.debug(methodName, "ReconcileAmbiguousTxnId main() Constants file not found on location:: " + constantsFile.toString());
            return;
        }
        final File logconfigFile = new File(args[1]);
        if (!logconfigFile.exists()) {
            _log.debug(methodName, "ReconcileAmbiguousTxnId main() Logconfig file not found on location:: " + logconfigFile.toString());
            return;
        }
        String module =args[2];
        if (module.equalsIgnoreCase("")) {
            _log.debug(methodName, "ReconcileAmbiguousTxnId main() module not defined:: " + module);
            return;
        }
        try {
            ...
            String outfilepath="";
        	if(BTSLUtil.isNullString(Constants.getProperty("RECONN_MODULE_OUTPUT_PATH"))){
        		outfilepath="/pretupsvar/";
           	}else{
           		outfilepath=Constants.getProperty("RECONN_MODULE_OUTPUT_PATH");
           	}
           
        	
            failOutPutFile = failOutPutFile + "_" + BTSLUtil.getFileNameStringFromDate(currentDate) + ".log";
     
            _fileObjectFail = new File(outfilepath+"/" + failOutPutFile);
            if (_fileObjectFail == null) {
                throw new BTSLBaseException("ReconcileAmbiguousTxnId", "main[]", "Error creating the failure transaction file ouput file");
            }
            ...
            _log.info(methodName, "ReconcileAmbiguousTxnId main() Not able to load Process Cache");
        ...
        try {
            ...
            final ReconcileAmbiguousTxnId ReconcileAmbiguousTxnId = new ReconcileAmbiguousTxnId();
            con = OracleUtil.getSingleConnection();
            if (module.startsWith("RP2P")) {
                ReconcileAmbiguousTxnId.handleChannelAmbigousCasesRP2P(con,PretupsI.C2S_MODULE);
            }
            if (module.startsWith("CP2P")) {
            	new ZTEINPoolLoaderServlet().initialize();
                ReconcileAmbiguousTxnId.handleChannelAmbigousCasesCP2P(con, PretupsI.P2P_MODULE);
            }
            if (module.startsWith("SOS")) {
                ReconcileAmbiguousTxnId.handleChannelAmbigousCasesSOS(con, PretupsI.SOS_REQUEST_TYPE);
            }
            ...
            final String senderMessage = "Ambigous module: " + module + " is processed successfully Total: " + _totalRecord + ", records, Processed :" + _totalProcessedRecords + " records, Not processed:" + _notProcessedRecords + " records";
     ...
     * @param p_successList
     */
    public void handleChannelAmbigousCasesRP2P(Connection p_con,String p_moduleCode) {
        final String methodName = "handleChannelAmbigousCases";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Entered with p_moduleCode="+p_moduleCode);
        }
        try {
            final HandleUnsettledCombinedCasesQueries handleUnsettledCombinedCasesQueries = new HandleUnsettledCombinedCasesQueries();
            _pstmtSelect = p_con.prepareStatement(handleUnsettledCombinedCasesQueries.loadC2STransferListQuery());
            ...
            ArrayList c2sReconnlist = loadC2STransferList( _pstmtSelect);
            _totalRecord=c2sReconnlist.size();
            C2STransferVO c2sTransferVO=null;
            for (int i=0, j=c2sReconnlist.size();i< j;i++)
            {	c2sTransferVO=(C2STransferVO) c2sReconnlist.get(i);
            String status=processValidationRequestC2S(c2sTransferVO);
            if(BTSLUtil.isNullString(status)){
            	
            	 writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListRP2P]", c2sTransferVO.getTransferID(),
                         "Not able Settled as status=" + status);
            	 
            	if (_log.isDebugEnabled()) {
            		_log.debug("handleChannelAmbigousCasesRP2P", "No Status Found   ");
            	}
            }else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_SUCCESS)){

            	reconcileTransactionVORP2P(p_con, PretupsErrorCodesI.TXN_STATUS_SUCCESS, "Success", c2sTransferVO, p_moduleCode);
            }else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_FAIL)){
            	reconcileTransactionVORP2P(p_con, PretupsErrorCodesI.TXN_STATUS_FAIL, "Fail", c2sTransferVO, p_moduleCode);

            }else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS)){
            	writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListRP2P]", c2sTransferVO.getTransferID(),
                        "Not able Settled as status=" + status);
            	
            	if (_log.isDebugEnabled()) {
            		_log.debug("handleChannelAmbigousCasesRP2P", "Ambiguous Status Found   ");
            	}
            	...
            	if (_log.isDebugEnabled()) {
            		_log.debug("handleChannelAmbigousCasesRP2P", "Invalid Status Found   ");
            	}
            }
            }
     ...
     * @param p_successList
     */
    public void handleChannelAmbigousCasesCP2P(Connection p_con, String p_moduleCode) {
        final String methodName = "handleChannelAmbigousCases";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Entered with p_moduleCode =" + p_moduleCode);
        }
        try {
            final HandleUnsettledCombinedCasesQueries handleUnsettledCombinedCasesQueries = new HandleUnsettledCombinedCasesQueries();
            _pstmtP2PSelect = p_con.prepareStatement(handleUnsettledCombinedCasesQueries.loadP2PReconciliationList());
            ...
            ArrayList p2pReconnList = loadP2PReconciliationList( _pstmtP2PSelect, _pstmtP2PTransferItemsSelect);
            
            _totalRecord=p2pReconnList.size();
            
            for (int i=0, j=p2pReconnList.size();i< j;i++)
		    {	p2pTransferVO=(P2PTransferVO) p2pReconnList.get(i);
		    
		    String status="";
		    if(p2pTransferVO.getServiceType().equalsIgnoreCase(PretupsI.SERVICE_TYPE_P2PRECHARGE))
		    {
		    	status=processValidationRequestP2PPRC(p2pTransferVO);
		    }else{
		    	status=processValidationRequestP2P(p2pTransferVO);

		    }

            	if(BTSLUtil.isNullString(status)){
                	writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListCP2P]", p2pTransferVO.getTransferID(),
                            "Not able Settled as status=" + status);
            		...
            		reconcileTransactionCP2PVO(p_con, PretupsErrorCodesI.TXN_STATUS_SUCCESS, "Success", p2pTransferVO, p_moduleCode);
            	}else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_FAIL)){
            		reconcileTransactionCP2PVO(p_con, PretupsErrorCodesI.TXN_STATUS_FAIL, "Fail", p2pTransferVO, p_moduleCode);
            	}else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS)){
     ...
     * @param p_successList
     */
    public void handleChannelAmbigousCasesSOS(Connection p_con, String p_moduleCode) {
        final String methodName = "_pstmtSOSReconcilationStatusUpdate";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Entered with p_moduleCode =" + p_moduleCode);
        }
        try {
            final HandleUnsettledCombinedCasesQueries handleUnsettledCombinedCasesQueries = new HandleUnsettledCombinedCasesQueries();
            _pstmtSOSSelect = p_con.prepareStatement(handleUnsettledCombinedCasesQueries.loadSOSReconciliationList());
            _pstmtSOSReconcilationStatusUpdate = p_con.prepareStatement(handleUnsettledCombinedCasesQueries.updateSOSReconcilationStatus());
    
            SOSVO p2pTransferVO=null;
            ArrayList p2pReconnList = loadSOSReconciliationList( _pstmtSOSSelect );
            _totalRecord=p2pReconnList.size();
            for (int i=0, j=p2pReconnList.size();i< j;i++)
		    {	p2pTransferVO=(SOSVO) p2pReconnList.get(i);
            	String status=processValidationRequestSOS(p2pTransferVO);
            	
            	if(BTSLUtil.isNullString(status)){
            		writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListSOS]", p2pTransferVO.getTransactionID(),
                            "Not able Settled as status=" + status);

            		if (_log.isDebugEnabled()) {
            			_log.debug("_pstmtSOSReconcilationStatusUpdate", "No Status Found   ");
            		}
            	}else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_SUCCESS)){
            		reconcileTransactionSOSVO(p_con, PretupsErrorCodesI.TXN_STATUS_SUCCESS, "Success", p2pTransferVO, p_moduleCode);
            	}else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_FAIL)){
            		reconcileTransactionSOSVO(p_con, PretupsErrorCodesI.TXN_STATUS_FAIL, "Fail", p2pTransferVO, p_moduleCode);
            	}else if(status.equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS)){
            		writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListSOS]", p2pTransferVO.getTransactionID(),
                            "Not able Settled as status=" + status);
            		if (_log.isDebugEnabled()) {
            			_log.debug("_pstmtSOSReconcilationStatusUpdate", "Ambiguous Status Found   ");
            		}
            	}else{
            		writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListSOS]", p2pTransferVO.getTransactionID(),
                            "Not able Settled as status=" + status);
            		if (_log.isDebugEnabled()) {
            			_log.debug("_pstmtSOSReconcilationStatusUpdate", "Invalid Status Found   ");
            		}
            	}
     	    }
            ...
            try {
                if (_pstmtSOSSelect != null) {
                	_pstmtSOSSelect.close();
                }
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
            }
            ...
            try {
                if (_pstmtSOSReconcilationStatusUpdate != null) {
                	_pstmtSOSReconcilationStatusUpdate.close();
                }
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
            }
            if (_log.isDebugEnabled()) {
                _log.debug("reconcileTransactionListSOS", "Exiting ");
            }
        }// end of finally
    }
    ...
    public void reconcileTransactionCP2PVO(Connection p_con, String p_status, String p_statusTxt, P2PTransferVO p2pTransferVO, String p_moduleCode) {
        final String methodName = "reconcileTransactionListRP2P";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Entered with p_status=" + p_status + " p_statusTxt=" + p_statusTxt + " p2pTransferVO=" + p2pTransferVO.getTransferID());
        }
        try {
            ...
            long p = 0;
                ...
                transactionID = p2pTransferVO.getTransferID();
                if (_log.isDebugEnabled()) {
                ...
                try {
                    ...
                    if (updateCount > 0) {
                        ...
                        writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListCP2P]", transactionID,
                                "Settled to status=" + p_status);
                    } else {
                        p_con.rollback();
                        if (_log.isDebugEnabled()) {
                    ...
                    p_con.rollback();
                    writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListCP2P]", transactionID, "getting Exception=" + be.getMessage());
                    ...
                    writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListCP2P]", transactionID, "getting Exception=" + e.getMessage());
                }
     ...
     * @return
     */
    public ArrayList loadC2STransferList( PreparedStatement pstmtSelect) throws BTSLBaseException {
        final String methodName = "loadC2STransferList";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Entered" );
        }
        ArrayList c2sReconnList=null;
        ResultSet rs = null;
        C2STransferVO c2sTransferVO = null;
        ...
        try {
            ...
            if (pstmtSelect != null) {
            	...
            	pstmtSelect.setDate(i++,getSQLDate(new String(Constants.getProperty("C2SReconn_ConfigureDate_From"))));
            	pstmtSelect.setTimestamp(i++,getSQLTODate(new String(Constants.getProperty("C2SReconn_ConfigureDate_TIME"))));
                rs = pstmtSelect.executeQuery();
            }
            if (rs != null && rs.next()) {
            	
            	if(c2sReconnList==null)
            	{
            		c2sReconnList=new ArrayList();
            	}
                ...
                c2sReconnList.add(c2sTransferVO);
            }
        }// end of try
        catch (SQLException sqle) {
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail,methodName, "", " SQL Exception:" + sqle.getMessage());
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, methodName, "", " Exception:" + e.getMessage());
        }// end of catch
        finally {
            try {
                if (rs != null) {
        ...
        return c2sReconnList;
    }

    

    
    public ArrayList loadP2PReconciliationList( PreparedStatement p_pstmtSelect, PreparedStatement p_pstmtP2PTransferItemsSelect) throws BTSLBaseException {
        final String methodName = "loadP2PReconciliationList";
     
        ArrayList p2pReconnList=null;
        ResultSet rs = null;
        P2PTransferVO p2pTransferVO = null;
        try {
            int i = 1;
            if (p_pstmtSelect != null) {
                ...
                p_pstmtSelect.setDate(i++,getSQLDate(new String(Constants.getProperty("P2PReconn_ConfigureDate_From"))));
                p_pstmtSelect.setTimestamp(i++,getSQLTODate(new String(Constants.getProperty("P2PReconn_ConfigureDate_TIME"))));
                rs = p_pstmtSelect.executeQuery();
            }
            while (rs != null && rs.next()) {
            	if(p2pReconnList==null){
            		p2pReconnList=new ArrayList();
            	}
                ...
                p2pTransferVO.setServiceType(rs.getString("SERVICE_TYPE"));
                p2pTransferVO.setTransferItemList(loadP2PReconciliationItemsList(p2pTransferVO.getTransferID(), p_pstmtP2PTransferItemsSelect));
                p2pReconnList.add(p2pTransferVO);
            }

        }// end of try
        catch (SQLException sqle) {
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[loadP2PReconciliationVO]", "", "SQLException: " + sqle.getMessage());
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[loadP2PReconciliationVO]", "", "Exception: " + e.getMessage());
        }// end of catch
        finally {
            try {
                if (rs != null) {
    ...
    public int markC2SReceiverAmbiguous(String p_transferID, PreparedStatement p_markC2SReceiverAmbiguous) throws BTSLBaseException {
        ...
        try {
            ...
            if (updateCount <= 0) {
                writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[markC2SReceiverAmbiguous]", p_transferID, "SQL Processing Error");
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[markC2SReceiverAmbiguous]", p_transferID, "SQLException " + sqle.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[markC2SReceiverAmbiguous]", p_transferID, "Exception " + e.getMessage());
            updateCount = 0;
    ...
    // writeOutPutFile(_fwriterFail,"ReconcileAmbiguousTxnId[loadC2STransferItemsVOList]",p_transferID," SQL Exception:"+sqle.getMessage());
    ...
    // writeOutPutFile(_fwriterFail,"ReconcileAmbiguousTxnId[loadC2STransferItemsVOList]",p_transferID," Exception:"+e.getMessage());
    // e.printStackTrace();
    // throw new BTSLBaseException(this, "loadC2STransferItemsVOList",
    ...
    public int updateReconcilationStatus(C2STransferVO p_c2sTransferVO, PreparedStatement p_pstmtReconcilationStatusUpdate) throws BTSLBaseException {
        ...
        try {
            ...
            if (updateCount <= 0) {
                writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateReconcilationStatus]", p_c2sTransferVO.getTransferID(), " Record is already modified");
            }
            ...
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateReconcilationStatus]", p_c2sTransferVO.getTransferID(), "SQLException: " + sqe.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        } catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateReconcilationStatus]", p_c2sTransferVO.getTransferID(), "Exception: " + e.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        } finally {
        ...
        try {
            ...
            if (transferItemsList != null && transferItemsList.size() > 0) {
                for (int j = 0, k = transferItemsList.size(); j < k; j++) {
                    ...
                    if (addCount < 0) {
                        writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[addC2STransferItemDetails]", p_transferID, " SQL Exception General processing error");
                        throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
                    }
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[addC2STransferItemDetails]", p_transferID, " SQLException " + sqle.getMessage());
            addCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[addC2STransferItemDetails]", p_transferID, " Exception " + e.getMessage());
            addCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
    ...
    public ArrayList loadP2PReconciliationItemsList(String p_transferID, PreparedStatement pstmtP2PTransferItemsSelect) throws BTSLBaseException {
        ...
        try {
            ...
            while (rs != null && rs.next()) {
                ...
                if(rs.getString("user_type").equalsIgnoreCase("SENDER")){
                p2pTransferItemVO.setTransferStatus("200");
                }else{
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[loadP2PReconciliationItemsList]", p_transferID, " SQLException " + sqle.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[loadP2PReconciliationItemsList]", p_transferID, " Exception " + e.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
            try {
    ...
    public int markP2PReceiverAmbiguous(String p_transferID, PreparedStatement p_markC2SReceiverAmbiguous) throws BTSLBaseException {
        ...
        try {
            ...
            if (updateCount <= 0) {
                writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[markP2PReceiverAmbiguous]", p_transferID, "QSL processing error");
                throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
            }
            return updateCount;
        }// end of try
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[markP2PReceiverAmbiguous]", p_transferID, "SQLException " + sqle.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[markP2PReceiverAmbiguous]", p_transferID, "Exception " + e.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
    ...
    public int updateP2PReconcilationStatus(P2PTransferVO p_p2pTransferVO, PreparedStatement pstmtReconcilationStatusUpdate, PreparedStatement pstmtInsertTransferItems) throws BTSLBaseException {
        ...
        try {
            ...
            if (updateCount <= 0) {
                writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateP2PReconcilationStatus]", p_p2pTransferVO.getTransferID(), "Record is already modified");
            } else // if(updateCount>=1)
            {
        ...
        try {
            ...
            if (p_transferItemsList != null && p_transferItemsList.size() > 0) {
                for (int j = 0, k = p_transferItemsList.size(); j < k; j++) {
                    ...
                    itemCount = itemCount + 1;
                    if (addCount <= 0) {
                        writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[addP2PTransferItemDetails]", p_transferID, "SQL general processing error");
                        throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
                    }
                }
            }
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[addP2PTransferItemDetails]", p_transferID, "SQLException " + sqle.getMessage());
            addCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            addCount = 0;
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[addP2PTransferItemDetails]", p_transferID, "Exception " + e.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
            if (_log.isDebugEnabled()) {
    ...
    public VomsVoucherVO loadVomsVoucherVO(TransferVO p_c2sTransferVO, PreparedStatement p_pstmtC2SVOMSSelect) throws BTSLBaseException {
        ...
        try {
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[loadVomsVoucherVO]", p_c2sTransferVO.getTransferID(), "SQLException " + sqle.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[loadVomsVoucherVO]", p_c2sTransferVO.getTransferID(), "Exception " + e.getMessage());
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
            try {
        ...
        try {
            ...
            if ("Success".equals(p_Operation)) {
                if (VOMSI.VOUCHER_UNPROCESS.equals(p_vomsVoucherVO.getCurrentStatus()) || VOMSI.VOUCHER_REP_ENABLE.equals(p_vomsVoucherVO.getCurrentStatus())) {
                    ...
                    writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateVoucherStatus]", p_transferVO.getTransferID(), "Invalid Voucher Status");
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateVoucherStatus]", p_transferVO.getTransferID(), "SQLException " + sqle.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateVoucherStatus]", p_transferVO.getTransferID(), "Exception " + e.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
        ...
        try {
            ...
            if ("Success".equals(p_Operation)) {
                if (VOMSI.VOUCHER_UNPROCESS.equals(p_vomsVoucherVO.getCurrentStatus()) || VOMSI.VOUCHER_REP_ENABLE.equals(p_vomsVoucherVO.getCurrentStatus())) {
                    ...
                    writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateVoucherAuditStatus]", p_transferVO.getTransferID(), "Invalid Voucher Status");
            ...
            _log.errorTrace(methodName, sqle);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateVoucherAuditStatus]", p_transferVO.getTransferID(), "SQLException " + sqle.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
        }// end of catch
        catch (Exception e) {
            _log.errorTrace(methodName, e);
            writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[updateVoucherAuditStatus]", p_transferVO.getTransferID(), "Exception " + e.getMessage());
            updateCount = 0;
            throw new BTSLBaseException(this, methodName, "error.general.processing");
        }// end of catch
        finally {
    ...
    public String getReceiverValidateP2PStr(TransferItemVO transferItemVO ,P2PTransferVO p2pTransferVO) throws BTSLBaseException,SQLException
	{
		StringBuffer strBuff=null;
		final String methodName = "getReceiverValidateStr";
		try
		{
		strBuff=new StringBuffer();
		strBuff.append("TRANSACTION_ID="+p2pTransferVO.getVoucherSerialNumber());
		strBuff.append("&INTERFACE_ID="+transferItemVO.getInterfaceID());
		strBuff.append("&MSISDN="+ p2pTransferVO.getReceiverMsisdn());
		strBuff.append("&NETWORK_CODE="+p2pTransferVO.getNetworkCode());
		strBuff.append("&INTERFACE_ID="+transferItemVO.getInterfaceID());
		...
		strBuff.append("&MODULE="+p2ptype);
		strBuff.append("&INTERFACE_AMOUNT="+Long.toString(p2pTransferVO.getTransferValue()));
        ...
        catch (Exception e)
        {
            ...
            _log.errorTrace(methodName,e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"ChannelTransferAction[getReceiverValidateStr]","","","","SQL Exception:"+e.getMessage());
              throw new BTSLBaseException(this, methodName, e.getMessage());
        }
       finally
        {
          if (_log.isDebugEnabled()) 	_log.debug(methodName, "Exiting strbuff" + strBuff.toString());
        }// end of finally
  
	}
    
    
    public String getReceiverValidateP2PPRCStr(TransferItemVO transferItemVO ,P2PTransferVO p2pTransferVO) throws BTSLBaseException,SQLException
   	{
   		StringBuffer strBuff=null;
   		final String methodName = "getReceiverValidateStr";
   		try
   		{
   		strBuff=new StringBuffer();
   		strBuff.append("TRANSACTION_ID="+p2pTransferVO.getTransferID());
   		strBuff.append("&INTERFACE_ID="+transferItemVO.getInterfaceID());
   		strBuff.append("&MSISDN="+ p2pTransferVO.getReceiverMsisdn());
   		strBuff.append("&MSISDN="+ p2pTransferVO.getReceiverMsisdn());
   		strBuff.append("&SENDER_MSISDN="+ p2pTransferVO.getSenderMsisdn());
   		strBuff.append("&RECEIVER_MSISDN="+ p2pTransferVO.getReceiverMsisdn());	
    ...
    private String processValidationRequestP2PPRC(P2PTransferVO p2pTransferVO) throws BTSLBaseException,Exception
    {
    	Connection con = null;
    	ChannelTransferWebDAO channelTransferWebDAO =null;
    	TransferItemVO transferItemVO =null;
    	if(BTSLUtil.isNullString(Constants.getProperty("RECONN_MODULE_TYPE_P2PPRC"))){
    		p2ptype="RC";
    	}else{
    		p2ptype=Constants.getProperty("RECONN_MODULE_TYPE_P2PPRC");
    	}
    
    	String status="";

    	final String methodName = "processValidationRequest";
    	if(_log.isDebugEnabled()) _log.debug(methodName,"Entered and performing validations for transfer ID="+p2pTransferVO.getTransferID()+ " "+p2pTransferVO.getNetworkCode()+" "+p2ptype);  
    	try
    	{	
    		con=OracleUtil.getConnection();
    		channelTransferWebDAO = new ChannelTransferWebDAO();
    		
    		NetworkInterfaceModuleVO networkInterfaceModuleVOS=(NetworkInterfaceModuleVO)NetworkInterfaceModuleCache.getObject(PretupsI.C2S_MODULE,p2pTransferVO.getNetworkCode(),PretupsI.INTERFACE_CATEGORY_PRE);
    		//		LoadController.incrementTransactionInterCounts(channeltransferVO.getTransferID(),LoadControllerI.SENDER_UNDER_VAL);
    		transferItemVO = channelTransferWebDAO.loadUserInterfaceDetails(con,p2ptype);
    		...
    		_transferID=p2pTransferVO.getTransferID();
    		
    		String requestStr=getReceiverValidateP2PPRCStr(transferItemVO,p2pTransferVO);	
    		CommonClient commonClient=new CommonClient();
    		String receiverValResponse=commonClient.process(requestStr,p2pTransferVO.getTransferID(),networkInterfaceModuleVOS.getCommunicationType(),networkInterfaceModuleVOS.getIP(),networkInterfaceModuleVOS.getPort(),networkInterfaceModuleVOS.getClassName());
    		if(_log.isDebugEnabled())_log.debug(methodName,p2pTransferVO.getTransferID(),"Got the validation response from IN Module receiverValResponse="+receiverValResponse);

    		HashMap map=BTSLUtil.getStringToHash(receiverValResponse,"&","=");
    	...
    	catch (SQLException sqle)
    	{
    		...
    		EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,methodName,"","","","SQL Exception:"+sqle.getMessage());
    		throw new BTSLBaseException(this, methodName, "error.general.sql.processing");
    	}// end of catch
    	catch (Exception e)
    	{
    		...
    		_log.errorTrace(methodName,e);
    		if(con!=null) con.rollback();
    		EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,methodName,"","","","SQL Exception:"+e.getMessage());
    		throw new BTSLBaseException(this, methodName, e.getMessage());
    	}
    	finally
    	{
    		if(con!=null) try {con.close(); } catch(Exception e) {_log.errorTrace(methodName,e);}
    		con=null;
    		if (_log.isDebugEnabled())
    			_log.debug(methodName, "status=" + status);

    	}
    }

    private String processValidationRequestP2P(P2PTransferVO p2pTransferVO) throws BTSLBaseException,Exception
    {
    	Connection con = null;
    	ChannelTransferWebDAO channelTransferWebDAO =null;
    	TransferItemVO transferItemVO =null;
    	...
    	if(BTSLUtil.isNullString(Constants.getProperty("RECONN_MODULE_TYPE_P2P"))){
    		p2ptype="RC";
    	}else{
    		p2ptype=Constants.getProperty("RECONN_MODULE_TYPE_P2P");
    	}
    	...
    	{	
    		...
    		_intModClassNameS=networkInterfaceModuleVOS.getClassName();
    		_transferID=p2pTransferVO.getTransferID();
    		String requestStr=getReceiverValidateP2PStr(transferItemVO,p2pTransferVO);	
    		CommonClient commonClient=new CommonClient();
    		String receiverValResponse=commonClient.process(requestStr,p2pTransferVO.getTransferID(),networkInterfaceModuleVOS.getCommunicationType(),networkInterfaceModuleVOS.getIP(),networkInterfaceModuleVOS.getPort(),networkInterfaceModuleVOS.getClassName());
    		if(_log.isDebugEnabled())_log.debug(methodName,p2pTransferVO.getTransferID(),"Got the validation response from IN Module receiverValResponse="+receiverValResponse);
    		...
    		status=(String)map.get("INTERFACE_STATUS");
    ...
    public String getReceiverValidateC2SStr(TransferItemVO transferItemVO ,C2STransferVO p2pTransferVO) throws BTSLBaseException,SQLException
	{
		StringBuffer strBuff=null;
		final String methodName = "getReceiverValidateStr";
		try
		...
		strBuff.append("&INT_MOD_PORT="+_intModPortS);
		strBuff.append("&INT_MOD_CLASSNAME="+_intModClassNameS);
		strBuff.append("&MODULE="+c2stype);
		strBuff.append("&INTERFACE_AMOUNT="+p2pTransferVO.getTransferValue());
		strBuff.append("&USER_TYPE=S");
		strBuff.append("&ADJUST=Y");
		strBuff.append("&INTERFACE_ACTION="+PretupsI.INTERFACE_CREDIT_ACTION);
    ...
    private String processValidationRequestC2S(C2STransferVO p2pTransferVO) throws BTSLBaseException,Exception
    {
    	Connection con = null;
    	ChannelTransferWebDAO channelTransferWebDAO =null;
    	TransferItemVO transferItemVO =null;
    	if(BTSLUtil.isNullString(Constants.getProperty("RECONN_MODULE_TYPE_C2S"))){
    		c2stype="RC";
    	}else{
    		c2stype=Constants.getProperty("RECONN_MODULE_TYPE_C2S");
    	}
    	String status="";

    	final String methodName = "processValidationRequest";
    	if(_log.isDebugEnabled()) _log.debug(methodName,"Entered and performing validations for transfer ID="+p2pTransferVO.getTransferID()+ " "+p2pTransferVO.getNetworkCode()+" "+c2stype);  
    	try
    	{	
    		con=OracleUtil.getConnection();
    		channelTransferWebDAO = new ChannelTransferWebDAO();
    		NetworkInterfaceModuleVO networkInterfaceModuleVOS=(NetworkInterfaceModuleVO)NetworkInterfaceModuleCache.getObject(PretupsI.C2S_MODULE,p2pTransferVO.getNetworkCode(),PretupsI.INTERFACE_CATEGORY_PRE);
    		//		LoadController.incrementTransactionInterCounts(channeltransferVO.getTransferID(),LoadControllerI.SENDER_UNDER_VAL);
    		transferItemVO = channelTransferWebDAO.loadUserInterfaceDetails(con,c2stype);
    		_intModCommunicationTypeS=networkInterfaceModuleVOS.getCommunicationType();
    		_intModIPS=networkInterfaceModuleVOS.getIP();
    		_intModPortS=networkInterfaceModuleVOS.getPort();
    		_intModClassNameS=networkInterfaceModuleVOS.getClassName();
    		_transferID=p2pTransferVO.getTransferID();
    		String requestStr=getReceiverValidateC2SStr(transferItemVO,p2pTransferVO);	
    		CommonClient commonClient=new CommonClient();
     ...
     * @param p_list
     */
    public void reconcileTransactionVORP2P(Connection p_con, String p_status, String p_statusTxt, C2STransferVO c2sTransferVO, String p_moduleCode) {
        final String methodName = "reconcileTransactionVORP2P";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Entered with p_status=" + p_status + " p_statusTxt=" + p_statusTxt );
        }
        try {
            String transactionID = null;
            ...
            long p = 0;
                ...
                transactionID = c2sTransferVO.getTransferID();
                if (_log.isDebugEnabled()) {
                    _log.debug(methodName, "Got transactionID=" + transactionID);
                }
                updateCount = 0;
                ...
                try {
                    ...
                    if (updateCount > 0) {
                        ...
                        writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListRP2P]", transactionID,
                                "Settled to status=" + p_status);
                        ...
                        writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionVORP2P]", transactionID,
                            "Not able Settled to status=" + p_status);
                    }
                    p_con.rollback();
                    p = System.currentTimeMillis();
    ...
    private java.sql.Date getSQLDate(String dateNoParam) {
...
private java.sql.Timestamp getSQLTODate(String dateNoParam) {
		...
		DateFormat dateFormat = new SimpleDateFormat("dd-MMM-yy HH:mm:ss");
		cal.add(Calendar.DATE, - 0);
		cal.add(Calendar.MINUTE, -Integer.parseInt(dateNoParam));
		
		return new java.sql.Timestamp(cal.getTimeInMillis());
	}

public ArrayList loadSOSReconciliationList( PreparedStatement p_pstmtSelect) throws BTSLBaseException {
	final String methodName = "loadSOSReconciliationList";

	ArrayList sosReconnList=null;
	ResultSet rs = null;
	SOSVO sosVO = null;
	try {
		int i = 1;
		if (p_pstmtSelect != null) {
			...
			p_pstmtSelect.setDate(i++,getSQLDate(new String(Constants.getProperty("SOSReconn_ConfigureDate_From"))));
			p_pstmtSelect.setTimestamp(i++,getSQLTODate(new String(Constants.getProperty("SOSReconn_ConfigureDate_TIME"))));
			p_pstmtSelect.setString(i++, PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS);
			p_pstmtSelect.setString(i++, PretupsErrorCodesI.TXN_STATUS_UNDER_PROCESS);
			p_pstmtSelect.setString(i++, PretupsI.C2S_ERRCODE_VALUS);
			p_pstmtSelect.setString(i, PretupsI.C2S_ERRCODE_VALUS);

			rs = p_pstmtSelect.executeQuery();
		}
		while (rs != null && rs.next()) {
			if(sosReconnList==null){
				sosReconnList=new ArrayList();
			}
			sosVO = new SOSVO();
...
public String getReceiverValidateSOSStr(TransferItemVO transferItemVO ,SOSVO p2pTransferVO) throws BTSLBaseException,SQLException
	{
		StringBuffer strBuff=null;
		final String methodName = "getReceiverValidateStr";
		try
		{
		strBuff=new StringBuffer();
		strBuff.append("TRANSACTION_ID="+p2pTransferVO.getTransactionID());
		strBuff.append("&INTERFACE_ID="+transferItemVO.getInterfaceID());
		strBuff.append("&MSISDN="+ p2pTransferVO.getMsisdn());
		strBuff.append("&NETWORK_CODE="+p2pTransferVO.getNetworkCode());
		strBuff.append("&INTERFACE_ID="+transferItemVO.getInterfaceID());
		strBuff.append("&INTERFACE_HANDLER="+transferItemVO.getInterfaceHandlerClass());
		strBuff.append("&INT_MOD_COMM_TYPE="+_intModCommunicationTypeS);
		...
		strBuff.append("&INT_MOD_PORT="+_intModPortS);
		strBuff.append("&INT_MOD_CLASSNAME="+_intModClassNameS);
		strBuff.append("&MODULE="+sostype);
		strBuff.append("&INTERFACE_AMOUNT="+Long.toString(p2pTransferVO.getDebitAmount()));
		strBuff.append("&USER_TYPE=S");
		strBuff.append("&ADJUST=Y");
		strBuff.append("&INTERFACE_ACTION="+PretupsI.INTERFACE_CREDIT_ACTION);
		return strBuff.toString();
   ...
   private String processValidationRequestSOS(SOSVO p2pTransferVO) throws BTSLBaseException,Exception
   {
   	Connection con = null;
   	ChannelTransferWebDAO channelTransferWebDAO =null;
   	TransferItemVO transferItemVO =null;
   	...
   	if(BTSLUtil.isNullString(Constants.getProperty("RECONN_MODULE_TYPE_SOS"))){
   		sostype="RC";
   	}else{
   		sostype=Constants.getProperty("RECONN_MODULE_TYPE_SOS");
   	}
   
   	String status="";

   	final String methodName = "processValidationRequestSOS";
   	if(_log.isDebugEnabled()) _log.debug(methodName,"Entered and performing validations for transfer ID="+p2pTransferVO.getTransactionID()+ " "+p2pTransferVO.getNetworkCode()+" "+sostype);  
   	try
   	{	
   		con=OracleUtil.getConnection();
   		channelTransferWebDAO = new ChannelTransferWebDAO();
   		...
   		NetworkInterfaceModuleVO networkInterfaceModuleVOS=(NetworkInterfaceModuleVO)NetworkInterfaceModuleCache.getObject(PretupsI.C2S_MODULE,p2pTransferVO.getNetworkCode(),PretupsI.INTERFACE_CATEGORY_PRE);
   		//		LoadController.incrementTransactionInterCounts(channeltransferVO.getTransferID(),LoadControllerI.SENDER_UNDER_VAL);
   		transferItemVO = channelTransferWebDAO.loadUserInterfaceDetails(con,sostype);
   		_intModCommunicationTypeS=networkInterfaceModuleVOS.getCommunicationType();
   		_intModIPS=networkInterfaceModuleVOS.getIP();
   		_intModPortS=networkInterfaceModuleVOS.getPort();
   		_intModClassNameS=networkInterfaceModuleVOS.getClassName();
   		_transferID=p2pTransferVO.getTransactionID();
   		String requestStr=getReceiverValidateSOSStr(transferItemVO,p2pTransferVO);	
   		CommonClient commonClient=new CommonClient();
   		//	TransactionLog.log(_transferID,_requestIDStr,_receiverMSISDN,_receiverVO.getNetworkCode(),PretupsI.TXN_LOG_REQTYPE_REQ,PretupsI.TXN_LOG_TXNSTAGE_INVAL,requestStr,PretupsI.TXN_LOG_STATUS_SUCCESS,"");
   		String receiverValResponse=commonClient.process(requestStr,p2pTransferVO.getTransactionID(),networkInterfaceModuleVOS.getCommunicationType(),networkInterfaceModuleVOS.getIP(),networkInterfaceModuleVOS.getPort(),networkInterfaceModuleVOS.getClassName());

   		//		TransactionLog.log(_transferID,_requestIDStr,_receiverMSISDN,_receiverVO.getNetworkCode(),PretupsI.TXN_LOG_REQTYPE_RES,PretupsI.TXN_LOG_TXNSTAGE_INVAL,receiverValResponse,PretupsI.TXN_LOG_STATUS_SUCCESS,"");
   		if(_log.isDebugEnabled())_log.debug(methodName,p2pTransferVO.getTransactionID(),"Got the validation response from IN Module receiverValResponse="+receiverValResponse);
   ...
   public int updateReconcilationStatus(Connection p_con, SOSVO p_sosVO) throws BTSLBaseException {
       ...
       try {
    	   ...
    	   int i = 1;
    	   _pstmtSOSReconcilationStatusUpdate.clearParameters();
           _pstmtSOSReconcilationStatusUpdate.setString(i++, p_sosVO.getTransactionStatus());
           _pstmtSOSReconcilationStatusUpdate.setString(i++, p_sosVO.getModifiedBy());
           _pstmtSOSReconcilationStatusUpdate.setTimestamp(i++, BTSLUtil.getTimestampFromUtilDate(p_sosVO.getModifiedOn()));
           _pstmtSOSReconcilationStatusUpdate.setString(i++, p_sosVO.getModifiedBy());
           _pstmtSOSReconcilationStatusUpdate.setTimestamp(i++, BTSLUtil.getTimestampFromUtilDate(p_sosVO.getModifiedOn()));
           _pstmtSOSReconcilationStatusUpdate.setString(i++, p_sosVO.getTransactionID());
           _pstmtSOSReconcilationStatusUpdate.setString(i++, PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS);
           _pstmtSOSReconcilationStatusUpdate.setString(i++, PretupsErrorCodesI.TXN_STATUS_UNDER_PROCESS);

           updateCount = _pstmtSOSReconcilationStatusUpdate.executeUpdate();
           if (updateCount <= 0) {
   ...
   public void reconcileTransactionSOSVO(Connection p_con, String p_status, String p_statusTxt, SOSVO sosTransferVO, String p_moduleCode) {
       final String methodName = "reconcileTransactionSOSVO";
       if (_log.isDebugEnabled()) {
           _log.debug(methodName, "Entered with p_status=" + p_status + " p_statusTxt=" + p_statusTxt + " p2pTransferVO=" + sosTransferVO.getTransactionID());
       }
       try {
           String transactionID = null;
           ...
           long p = 0;
               ...
               transactionID = sosTransferVO.getTransactionID();
               if (_log.isDebugEnabled()) {
                   _log.debug(methodName, "Got transactionID=" + transactionID);
               }
               updateCount = 0;
               ...
               try {
                   ...
                   sosTransferVO.setTransactionStatus(p_status);
                   sosTransferVO.setModifiedBy(PretupsI.SYSTEM_USER);
                   sosTransferVO.setModifiedOn(currentDate);
                   
                   updateCount = updateReconcilationStatus(p_con,sosTransferVO);
                   if (updateCount > 0) {
                       p_con.commit();
                       _totalProcessedRecords++;
                       _log.info(methodName, "TransactionID=" + transactionID + " Succesfully Settled to status=" + p_status);
                       writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionListSOS]", transactionID,
                               "Settled to status=" + p_status);
                   } else {
                       p_con.rollback();
                       if (_log.isDebugEnabled()) {
                   ...
                   _log.errorTrace(methodName, be);
                   p_con.rollback();
                   writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionSOSVO]", transactionID, "getting Exception=" + be.getMessage());
                   _log.info(methodName, "TransactionID=" + transactionID + " Not able Settled to status=" + p_status + " getting BTSL Exception=" + be.getMessage());
               } catch (Exception e) {
                   _log.errorTrace(methodName, e);
                   p_con.rollback();
                   _log.info(methodName, "TransactionID=" + transactionID + " Not able Settled to status=" + p_status + " getting Exception=" + e.getMessage());
                   writeOutPutFile(_fwriterFail, "ReconcileAmbiguousTxnId[reconcileTransactionSOSVO]", transactionID, "getting Exception=" + e.getMessage());
               }
