...
/**
 * @(#)WarrAdvCautController.java
 *                                Name Date History
 *                                ----------------------------------------------
 *                                --------------------------
 *                                Narendra Kumar 05/12/2015 Initial Creation
 *                                ----------------------------------------------
 *                                --------------------------
 *                                Copyright (c) 2005 Bharti Telesoft Ltd.
 *                                Controller class for handling the Channel to
 *                                subscriber transfers
 *                                This class is used to handel the Transaction
 *                                of Warranty recharge,Caution recharge,Advance
 *                                Recharge as per Meditel.
 */
...
public class WarrAdvCautController implements ServiceKeywordControllerI, Runnable {
    private static Log _log = LogFactory.getLog(WarrAdvCautController.class.getName());
    ...
    private static int WAC_TRANSFER_ID_PAD_LENGTH = 4;

    // Loads operator specific class
    static {
        final String methodName = "WarrAdvCautController";
    ...
    public WarrAdvCautController() {
        _c2sTransferVO = new C2STransferVO();
        _currentDate = new Date();
        if ("WAC_REC_GEN_FAIL_MSG_REQD_V".equals(BTSLUtil.NullToString(Constants.getProperty("Y")))) {
            _recValidationFailMessageRequired = true;
        }
        if ("WAC_REC_GEN_FAIL_MSG_REQD_T".equals(BTSLUtil.NullToString(Constants.getProperty("Y")))) {
            _recTopupFailMessageRequired = true;
        }
        _notAllowedSendMessGatw = BTSLUtil.NullToString(Constants.getProperty("WAC_SEN_MSG_NOT_REQD_GW"));
        _notAllowedRecSendMessGatw = BTSLUtil.NullToString(Constants.getProperty("WAC_REC_MSG_NOT_REQD_GW"));
        _RecAlternetGatewaySMS = BTSLUtil.NullToString(Constants.getProperty("WAC_REC_MSG_REQD_BY_ALT_GW"));
    }
    ...
    public void process(RequestVO p_requestVO) {
        ...
        try {
            ...
            TransactionLog.log(methodName, p_requestVO.getRequestIDStr(), p_requestVO.getFilteredMSISDN(), _channelUserVO.getNetworkID(), PretupsI.TXN_LOG_REQTYPE_REQ,
            ...
            if (!BTSLUtil.isStringIn(_serviceType, (String) PreferenceCache.getSystemPreferenceValue(PreferenceI.DECIMAL_ALLOW_SERVICES))) {
                try {
                    ...
                    _log.errorTrace(methodName, e);
                    throw new BTSLBaseException(methodName, PretupsErrorCodesI.ERROR_INVALID_AMOUNT);
                }
            }
            ...
            if (!_receiverVO.getSubscriberType().equals(_type)) {
                // Refuse the Request
                _log.error(this, methodName, "Series =" + _receiverVO.getMsisdnPrefix() + " Not Defined for Series type=" + _type);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, methodName, "", "", "", "Series =" + _receiverVO
                    .getMsisdnPrefix() + " Not Defined for Series type=" + _type + " But request initiated for the same");
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.ERROR_NOTFOUND_SERIES_TYPE);
            }
            ...
            if (PretupsI.SUSPEND.equals(_channelUserVO.getTransferProfileStatus())) {
                throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.CHNL_ERROR_SNDR_TRANPROFILE_SUSPEND);
            }
            ...
            if (PretupsI.SUSPEND.equals(_channelUserVO.getCommissionProfileStatus())) {
                throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.CHNL_ERROR_SNDR_COMMPROFILE_SUSPEND);
            } else if (PretupsI.YES.equalsIgnoreCase(_channelUserVO.getOutSuspened())) {
                throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.CHNL_ERROR_SENDER_OUT_SUSPEND);
            }
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            }
            ...
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SKEY_REQUIRED)).booleanValue()) {
				...
				if (mcomCon != null) {
					mcomCon.close("WarrAdvCautController#process");
					mcomCon = null;
				}
                con = null;
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, methodName, _transferID, _senderMSISDN,
                    _senderNetworkCode, "Leaving Reciever Unmarked Base Exception:" + bex.getMessage());
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
                _c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, methodName, _transferID, _senderMSISDN,
                    _senderNetworkCode, "Leaving Reciever Unmarked Base Exception:" + e.getMessage());
            }
            ...
            try {
                ...
                _c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, methodName, _transferID, _senderMSISDN,
                    _senderNetworkCode, "Leaving Reciever Unmarked Base Exception:" + ex.getMessage());
            }
            ...
            // raising alarm
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, methodName, _transferID, _senderMSISDN,
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                _log.error(methodName, "Exception:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "WarrAdvCautController[process]", _transferID,
    ...
    public void processTransfer(Connection p_con) throws BTSLBaseException {
        ...
        try {
            ...
            generateWRCADVCAUTTransferID(_c2sTransferVO);
    ...
    public void processThread() {
        ...
        try {
            ...
            if ((!_senderNetworkCode.equals(_receiverNetworkCode)) && (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ALLOW_ROAM_ADDCOMM))).booleanValue())) {
				...
				if (mcomCon != null) {
					mcomCon.close("WarrAdvCautController#processThread");
					mcomCon = null;
				}
                con = null;
                ...
                try {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "WarrAdvCautController[processThread]",
                ...
                if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                    ...
                    try {
                        ...
                        if (sosvo != null) {
                            ...
                            _log.error(this, methodName, "processThread" + " No record found in database for this number :" + _receiverTransferItemVO.getMsisdn());
                            // throw new
                            // BTSLBaseException(methodName,"processThread",PretupsErrorCodesI.SOS_ERROR_EXCEPTION);
                        }
                    } catch (BTSLBaseException be) {
                        _log.errorTrace(methodName, be);
                        _log.error(this, methodName, "processThread BTSLBaseException:" + "Transaction ID: " + sosvo.getTransactionID() + "Msisdn" + _receiverTransferItemVO
                            .getMsisdn() + "Getting Exception while processing LMB request :" + be.getMessage());
                    } catch (Exception e) {
                        _log.errorTrace(methodName, e);
                        _log.error(this, methodName, "processThread Exception:" + "Transaction ID: " + sosvo.getTransactionID() + "Msisdn" + _receiverTransferItemVO
            ...
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.LMS_APPL)).booleanValue() && _c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                try {
                    if (!BTSLUtil.isNullString(_channelUserVO.getLmsProfile())) {
                        ...
                        _log.error(methodName, "Exception during LMS Module.SetId not found");
                    }
                } catch (Exception ex) {
                    _log.errorTrace(methodName, ex);
                    _log.error(methodName, "Exception durign LMS Module " + ex.getMessage());

                }
            } // / ends here

            if (_log.isDebugEnabled()) {
                _log.debug(methodName, "For _transferID=" + _transferID + " Diff applicable=" + _c2sTransferVO.getDifferentialApplicable() + " Diff Given=" + _c2sTransferVO
                    .getDifferentialGiven());
            }
        }// end try
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "WarrAdvCautController[processThread]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "WarrAdvCautController[processThread]", _transferID,
            ...
            try {
                ...
                _log.errorTrace(methodName, be);
                _log.error("processThread", _transferID, "BTSLBaseException while updating transfer details in database:" + be.getMessage());
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
                _log.error("processThread", _transferID, "Exception while updating transfer details in database:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "WarrAdvCautController[processThread]", _transferID,
            ...
            if (!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(), _notAllowedSendMessGatw)) {
                ...
                if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS) && ((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.GRPT_CHRG_ALLOWED)) != null && ((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.GRPT_CHRG_ALLOWED))
                    ...
                    try {
                        ...
                        // load the user processThreadning and profile counters
                        // Check the counters
                        // update the counters
        ...
        if (!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) || isRequired) {
            ...
            if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
                ...
                if (InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED.equals(_receiverTransferItemVO.getValidationStatus())) {
                    throw new BTSLBaseException(methodName, "updateForReceiverValidateResponse", InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED + "_S", 0, strArr,
                        null);
                } else {
                    throw new BTSLBaseException(methodName, "updateForReceiverValidateResponse", _c2sTransferVO.getErrorCode(), 0, strArr, null);
                }
            }
            _receiverTransferItemVO.setTransferStatus(status);
    ...
    public void updateForReceiverCreditResponse(String str) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(status) || (!status.equals(InterfaceErrorCodesI.SUCCESS) && !status.equals(InterfaceErrorCodesI.AMBIGOUS))) {
            ...
            if (SystemPreferences.ENQ_POSTBAL_ALLOW) {
                ...
                // e){_log.errorTrace(methodName, e);};
                try {
    ...
    public void populateServiceInterfaceDetails(Connection p_con, String action) throws BTSLBaseException {
        ...
        if (!isReceiverFound) {
            throw new BTSLBaseException(methodName, PretupsErrorCodesI.C2S_ERROR_NOTFOUND_SERVICEINTERFACEMAPPING);
        }
    ...
    private void checkTransactionLoad() throws BTSLBaseException {
        ...
        try {
            ...
            if (recieverLoadStatus == 0) {
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug("WarrAdvCautController[checkTransactionLoad]", "_transferID=" + _transferID + " Successfully through load");
                }
            }
            // Request in Queue
                ...
                throw new BTSLBaseException(methodName, "checkTransactionLoad", PretupsErrorCodesI.REQUEST_IN_QUEUE, 0, strArr, null);
            }
            // Refuse the request
            else {
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
            }
        } catch (BTSLBaseException be) {
            _log.error(methodName, "Refusing request getting Exception:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.errorTrace(methodName, e);
            _log.error(methodName, "Refusing request getting Exception:" + e.getMessage());
            throw new BTSLBaseException(methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        }
    }
    ...
    private void processValidationRequest() throws BTSLBaseException, Exception {
        ...
        try {
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getValExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "WarrAdvCautController[processValidationRequest]",
                    _transferID, _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till validation");
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_VAL);
            }
			...
			if (mcomCon != null) {
				mcomCon.close("WarrAdvCautController#processValidationRequest");
				mcomCon = null;
			}
            con = null;
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getTopUpExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "WarrAdvCautController[run]", _transferID,
                    _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till topup");
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO = null;
            ...
            _log.error(methodName, "Getting BTSL Base Exception:" + be.getMessage());
            if (_transferDetailAdded) {
                if (mcomCon == null) {
                    mcomCon = new MComConnection();con=mcomCon.getConnection();
                }
    ...
    private String getReceiverCommonString() {
        ...
        strBuff.append("&SSN=" + _channelUserVO.getSsn());
        strBuff.append("&USER_NAME=" + _channelUserVO.getUserName());
        return strBuff.toString();
    }
        ...
        if (!("N".equals(_c2sTransferVO.getBoth()) && "N".equals(_c2sTransferVO.getOnline()))) {
            ...
            if (_c2sTransferVO.getReceiverTransferItemVO().isNumberBackAllowed()) {
                ...
                if (!_receiverTransferItemVO.getPostValidationStatus().equals(InterfaceErrorCodesI.SUCCESS)) {
                    ...
                    if ((_c2sTransferVO.getBonusTalkTimeValue() == 0)) {
                        key = PretupsErrorCodesI.C2S_RECEIVER_SUCCESS;
                    } else {
    ...
    private String getReceiverUnderProcessMessage() {
        final String[] messageArgArray = { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverTransferValue()), String.valueOf(_receiverTransferItemVO
            ...
            .getReceiverAccessFee()), _channelUserVO.getUserName(), _requestVO.getPosUserMSISDN() };
    ...
    private void processValidationRequestInThread() throws BTSLBaseException, Exception {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "WarrAdvCautController[processValidationRequestInThread]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            TransactionLog.log(_transferID, _requestIDStr, _receiverMSISDN, _receiverVO.getNetworkCode(), PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
            ...
            if (_transferID != null && !_transferDetailAdded) {
                ...
                try {
					...
					if (mcomCon != null) {
						mcomCon.close("WarrAdvCautController#processValidationRequestInThread");
						mcomCon = null;
					}
                    _log.errorTrace(methodName, be);
                    _log.error(methodName, "BTSLBaseException:" + be.getMessage());
    ...
    private boolean getInterfaceRoutingDetails(Connection p_con, String p_msisdn, long p_prefixID, String p_subscriberType, String p_networkCode, String p_serviceType, String p_interfaceCategory, String p_userType, String p_action) throws BTSLBaseException {
        ...
        try {
            ...
            if (subscriberRoutingControlVO != null) {
                    ...
                    if (p_interfaceCategory.equalsIgnoreCase(PretupsI.INTERFACE_CATEGORY_PRE)) {
                        ...
                        if (listValueVO != null) {
                            ...
                            if (!PretupsI.YES.equals(listValueVO.getStatus()) && PretupsI.INTERFACE_STATUS_TYPE_MANUAL.equals(listValueVO.getStatusType())) {
                                ...
                                throw new BTSLBaseException(methodName, PretupsErrorCodesI.INTERFACE_NOT_ACTIVE);
                            }
                ...
                // based routing
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "WarrAdvCautController[getInterfaceRoutingDetails]",
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "WarrAdvCautController[getInterfaceRoutingDetails]",
    ...
    private void performAlternateRouting(ArrayList altList) throws BTSLBaseException {
        try {
                    ...
                    try {
                        ...
                        if (InterfaceErrorCodesI.SUCCESS.equals(_receiverTransferItemVO.getValidationStatus())) {
                            ...
                            if (_receiverInterfaceInfoInDBFound) {
                                PretupsBL.updateSubscriberInterfaceRouting(_receiverTransferItemVO.getInterfaceID(), _externalID, _receiverMSISDN, _type, _channelUserVO
                                ...
                                if (!_receiverInterfaceInfoInDBFound && subscriberRoutingControlVO != null && subscriberRoutingControlVO.isDatabaseCheckBool()) {
                                    PretupsBL.insertSubscriberInterfaceRouting(_receiverTransferItemVO.getInterfaceID(), _externalID, _receiverMSISDN, _type, _channelUserVO
                                        .getUserID(), _currentDate);
                                    _receiverInterfaceInfoInDBFound = true;
                                }
                            }
            ...
            _log.errorTrace("updateSubscriberInterfaceRouting", e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "WarrAdvCautController[performAlternateRouting]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
    ...
    public void receiverValidateResponse(String str, int p_attempt, int p_altSize) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
            ...
            _senderTransferItemVO.setTransferStatus(PretupsErrorCodesI.REQUEST_NOT_APPLICABLE);
            strArr = new String[] { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()) };
            throw new BTSLBaseException(methodName, PretupsErrorCodesI.C2S_RECEIVER_FAIL);
        }
    ...
    public void processFromQueue(TransferVO p_transferVO) {
        ...
        try {
			...
			if (mcomCon != null) {
				mcomCon.close("WarrAdvCautController#processFromQueue");
				mcomCon = null;
			}
            con = null;

            if (_log.isDebugEnabled()) {
                _log.debug("WarrAdvCautController[processFromQueue]", "_transferID=" + _transferID + " Successfully through load");
            }
            ...
            try {
                ...
                _log.errorTrace(methodName, bex);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "WarrAdvCautController[processFromQueue]",
            ...
            // raising alarm
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "WarrAdvCautController[processFromQueue]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            // logging in the transaction log
            TransactionLog.log(_transferID, _requestVO.getRequestIDStr(), _requestVO.getFilteredMSISDN(), "", PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
                _c2sTransferVO.getSenderReturnMessage(), PretupsI.TXN_LOG_STATUS_FAIL, "Getting Code=" + _requestVO.getMessageCode());
            ...
            try {
                ...
                // try{if(con!=null) con.rollback() ;}catch(Exception
                // ex){_log.errorTrace(methodName, e);};
                _log.errorTrace(methodName, be);
                _log.error(methodName, "BTSLBaseException:" + be.getMessage());
            } catch (Exception e) {
                // try{if(con!=null) con.rollback() ;}catch(Exception
                ...
                _log.errorTrace(methodName, e);
                _log.error(methodName, "Exception:" + e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "WarrAdvCautController[processFromQueue]",
                    _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            }
        ...
        if (!BTSLUtil.isNullString(p_languageCode)) {
            try {
                if (LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode) == null) {
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "WarrAdvCautController[updateReceiverLocale]",
    ...
    private synchronized void generateWRCADVCAUTTransferID(TransferVO p_transferVO) {
        final String methodName = "generateWRCADVCAUTTransferID";
        ...
        try {
            ...
            if (_transactionIDCounter == 0) {
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
            // transferID=_operatorUtil.formatWRCADVCAUTTransferID(p_transferVO,_transactionIDCounter);
            transferID = formatWRCADVCAUTTransferID(p_transferVO, _transactionIDCounter);
            if (transferID == null) {
                throw new BTSLBaseException(methodName, PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
     ...
     *      long)
     *      Specific for Meditel
     */
    public String formatWRCADVCAUTTransferID(TransferVO p_transferVO, long p_tempTransferID) {
        String returnStr = null;
        final String methodName = "formatWRCADVCAUTTransferID";
        String ServiceType = p_transferVO.getServiceType();

        try {
            ServiceType = ServiceType.trim();
            final String paddedTransferIDStr = BTSLUtil.padZeroesToLeft(Long.toHexString(p_tempTransferID), WAC_TRANSFER_ID_PAD_LENGTH);
            if (ServiceType.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_WARRANTY_RECHARGE)) {
                returnStr = "W" + currentDateTimeFormatString(p_transferVO.getCreatedOn()) + "." + currentTimeFormatString(p_transferVO.getCreatedOn()) + "." + Constants
                    .getProperty("INSTANCE_ID") + paddedTransferIDStr;
            } else if (ServiceType.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_ADVANCE_RECHARGE)) {
                returnStr = "A" + currentDateTimeFormatString(p_transferVO.getCreatedOn()) + "." + currentTimeFormatString(p_transferVO.getCreatedOn()) + "." + Constants
                    .getProperty("INSTANCE_ID") + paddedTransferIDStr;
            } else if (ServiceType.equalsIgnoreCase(PretupsI.SERVICE_TYPE_CHNL_CAUTION_RECHARGE)) {
                returnStr = "U" + currentDateTimeFormatString(p_transferVO.getCreatedOn()) + "." + currentTimeFormatString(p_transferVO.getCreatedOn()) + "." + Constants
                    .getProperty("INSTANCE_ID") + paddedTransferIDStr;
            }
