...
/**@(#)PromoSAPVASController.java
 * Name                                 Date            History
 *------------------------------------------------------------------------
 * Harsh  Dixit					09-FEB-2016		Dedicated Controller for SAPVAS Transactions
 *------------------------------------------------------------------------
 * Copyright (c) 2016 MComviva Ltd.
 * Controller class for handling the SAP VAS transfers
 */
...
import com.btsl.pretups.channel.transfer.requesthandler.C2SPrepaidRoamHelper;
...
public class PromoSAPVASController implements ServiceKeywordControllerI,Runnable 
{
	private static Log _log = LogFactory.getLog(PromoSAPVASController.class.getName());
	...
	private static final String _vasPrefix="CP";
	...
	static
    {
		...
		catch(Exception e)
		{
			...
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[initialize]","","","","Exception while loading the class at the call:"+e.getMessage());
		}
    }	
    public  PromoSAPVASController()
	{
		_c2sTransferVO=new C2STransferVO();
		_currentDate=new Date();
		if(BTSLUtil.NullToString(Constants.getProperty("PVAS_REC_GEN_FAIL_MSG_REQD_V")).equals("Y"))
		{
			_recValidationFailMessageRequired=true;
		}
		if(BTSLUtil.NullToString(Constants.getProperty("PVAS_REC_GEN_FAIL_MSG_REQD_T")).equals("Y"))
		{
			_recTopupFailMessageRequired=true;
		}
		_notAllowedSendMessGatw=BTSLUtil.NullToString(Constants.getProperty("PVAS_SEN_MSG_NOT_REQD_GW"));
		_notAllowedRecSendMessGatw=BTSLUtil.NullToString(Constants.getProperty("PVAS_REC_MSG_NOT_REQD_GW"));
		_RecAlternetGatewaySMS=BTSLUtil.NullToString(Constants.getProperty("PVAS_REC_MSG_REQD_BY_ALT_GW"));
	...
	public void process(RequestVO p_requestVO) 
	{
		...
		try
		{
			NetworkServiceVO networkServiceVO=(NetworkServiceVO)NetworkServicesCache.getObject(PretupsI.C2S_MODULE, _senderNetworkCode, _receiverNetworkCode, _c2sTransferVO.getServiceType());
			...
			//PretupsBL.checkGeoFencing(_requestVO, _channelUserVO);
			...
			_c2sTransferVO.setCommission(_requestVO.getCommission());
			_c2sTransferVO.setCommissionApplicable(_requestVO.getCommissionApplicable());
			_c2sTransferVO.setCommissionType(_requestVO.getCommissionType());
			...
			//commented and added by Ashish to allow DeCenter Roaming
			//if(!_senderNetworkCode.equals(_receiverNetworkCode) && ((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ALLOW_ROAM_RECHARGE))).booleanValue() &&  networkServiceVO!=null && "Y".equalsIgnoreCase(networkServiceVO.getStatus()))
			if(!_senderNetworkCode.equals(_receiverNetworkCode) && ((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ALLOW_ROAM_RECHARGE))).booleanValue() &&  networkServiceVO!=null && "Y".equalsIgnoreCase(networkServiceVO.getStatus()) && !BTSLUtil.isNullString(Constants.getProperty(_receiverNetworkCode+"_DECENTER_ROAM")) && Constants.getProperty(_receiverNetworkCode+"_DECENTER_ROAM").equalsIgnoreCase("YES"))
			{
				C2SPrepaidRoamHelper c2sPrepaidRoamHelper = new C2SPrepaidRoamHelper();
				c2sPrepaidRoamHelper.process(p_requestVO,_c2sTransferVO);
				return;
			}
			
			//added by Karan for checking whether the subcriber and retailer belong to same n/w or different
			if(_senderNetworkCode.equals(_receiverNetworkCode))
				_requestedtype="Home";
			else
				_requestedtype="Roam";
			...
			if(!_receiverVO.getSubscriberType().equals(_type))
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[process]","","","",loggerValue.toString());
			...
			try {con.commit();} catch(Exception e){ 
				_log.errorTrace(methodName, e);
				throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
			...
			else
			{
				...
				try {con.commit();} catch(Exception e){ 
					_log.errorTrace(methodName, e);
					throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
				if(mcomCon != null){mcomCon.close("PromoSAPVASController#process");mcomCon=null;}
				con=null;
		...
		{	
			...
			catch(BTSLBaseException bex)
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"PromoSAPVASController[process]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
			}
			catch(Exception e)
			{
				_log.errorTrace(methodName, e);
...
//			Populate the ChannelRequestDailyLogVo and log
		...
		catch(Exception e)
		{
			...
			loggerValue.append(e.getMessage());
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[process]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
...
//			Populate the ChannelRequestDailyLogVo and log
		...
		finally
		{
			try
			{
				...
				{	
					...
					if(_log.isInfoEnabled()) {
						...
						_log.info(methodName,p_requestVO.getRequestIDStr(),loggerValue);
					}
				}
			}
			catch(BTSLBaseException be)
			...
			if(con!=null)
			{
				...
				try {con.commit();} catch(Exception e){_log.errorTrace(methodName, e);};
				if(mcomCon != null){mcomCon.close("PromoSAPVASController#process");mcomCon=null;}
				con=null;
			}//end if
			...
			if(_receiverMessageSendReq&&_recValidationFailMessageRequired&&!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(),_notAllowedRecSendMessGatw)&&!"ALL".equals(_notAllowedRecSendMessGatw))
			{
                ...
                if(!BTSLUtil.isNullString(_RecAlternetGatewaySMS)&& (_RecAlternetGatewaySMS.split(":")).length>=2)
                {
                    if(requestGW.equalsIgnoreCase(_RecAlternetGatewaySMS.split(":")[0]))
                    {
                        ...
                        if(_log.isDebugEnabled()) {
							_log.debug("process: Reciver Message push through alternate GW",requestGW,"Requested GW was:"+_c2sTransferVO.getRequestGatewayCode());
						}
                    }
                }
			...
			if (PretupsErrorCodesI.TXN_STATUS_FAIL.equals(_c2sTransferVO.getTransferStatus())) {
				String strarray[] = {};
				strarray = p_requestVO.getMessageArguments();
				BTSLMessages btslRecMessages=new BTSLMessages(_c2sTransferVO.getErrorCode(),strarray);
				(new PushMessage(_c2sTransferVO.getSenderMsisdn(),btslRecMessages,_transferID,"",_senderLocale, _c2sTransferVO.getNetworkCode())).push();
			}
			//making entry in the transaction log	
            if(_oneLog) {
	...
	private void processSKeyGen(Connection p_con) throws BTSLBaseException,Exception
	{
		...
		{
			_log.errorTrace(methodName, e);
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[processSKeyGen]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
		}
		finally
		{
	...
	public void processTransfer(Connection p_con) throws BTSLBaseException
	{
		...
		try
		{
			...
			//common Transfer ID For All C2S Module
		...
		catch(Exception e)
		{
			...
			_c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
			_log.errorTrace(methodName, e);
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[processTransfer]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
		}
	}
	...
	public void run() 
	{
		...
		try
		{
			...
			if((!_senderNetworkCode.equals(_receiverNetworkCode))&& (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ALLOW_ROAM_ADDCOMM))).booleanValue()))
			{
				try {con.commit();} catch(Exception e){ 
					_log.errorTrace(methodName, e);
					throw new BTSLBaseException("PromoSAPVASController","process",PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
				if(mcomCon != null){mcomCon.close("PromoSAPVASController#methodmethodname");mcomCon=null;}
				con=null;
	           ...
	           // calculate roam commission
	            ...
	            catch(BTSLBaseException be)
				{
					...
					if(_log.isDebugEnabled()) {
						...
						_log.debug("PromoSAPVASController",loggerValue);
					}
					loggerValue.setLength(0);
					loggerValue.append("Exception:");
					loggerValue.append(be.getMessage());
					EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[run]",_c2sTransferVO.getTransferID(),_c2sTransferVO.getSenderMsisdn(),_c2sTransferVO.getNetworkCode(),loggerValue.toString());
					_log.errorTrace(methodName, be);
				}
	            catch(Exception e)
				{
			...
			if(PretupsI.YES.equals(_c2sTransferVO.getDifferentialAllowedForService()))
			{
				...
				if(giveBonus)
				{
					...
					catch(BTSLBaseException be)
					{
						_finalTransferStatusUpdate=false;
						if(_log.isDebugEnabled()) {
							_log.debug("PromoSAPVASController","For _transferID="+_transferID+" Diff applicable="+_c2sTransferVO.getDifferentialApplicable()+" Diff Given="+_c2sTransferVO.getDifferentialGiven() +" Not able to give Diff commission getting BTSL Base Exception="+be.getMessage()+" Leaving transaction status as Under process");
						}
						EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[run]",_c2sTransferVO.getTransferID(),_c2sTransferVO.getSenderMsisdn(),_c2sTransferVO.getNetworkCode(),"Exception:"+be.getMessage());
						_log.errorTrace(methodName, be);
					}
					catch(Exception e)
					{
						_finalTransferStatusUpdate=false;
						if(_log.isDebugEnabled()) {
							_log.debug("PromoSAPVASController","For _transferID="+_transferID+" Diff applicable="+_c2sTransferVO.getDifferentialApplicable()+" Diff Given="+_c2sTransferVO.getDifferentialGiven() +" Not able to give Diff commission getting Exception="+e.getMessage()+" Leaving transaction status as Under process");
						}
						_log.errorTrace(methodName, e);
					}
				 }
			...
			if((_c2sTransferVO.getCommissionApplicable().equalsIgnoreCase(PretupsI.YES)))
			{
				if(mcomCon==null) {
							mcomCon = new MComConnection();con=mcomCon.getConnection();
				}
		        try
	            {
	            	...
	            	//new DiffCalBL().differentialCalculationsBonus(_c2sTransferVO, PretupsI.C2S_MODULE);
	            ...
	            catch(BTSLBaseException be)
				{
					...
					if(_log.isDebugEnabled()) {
						_log.debug("PromoSAPVASController","For _transferID="+_transferID+" roam commission applicable="+_c2sTransferVO.getDifferentialApplicable()+" roam commission Given="+_c2sTransferVO.getDifferentialGiven() +" Not able to give roam commission getting BTSL Base Exception="+be.getMessage()+" Leaving transaction status as Under process");
					}
					EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[run]",_c2sTransferVO.getTransferID(),_c2sTransferVO.getSenderMsisdn(),_c2sTransferVO.getNetworkCode(),"Exception:"+be.getMessage());
					_log.errorTrace(methodName, be);
				}
	            catch(Exception e)
				{
					mcomCon.partialRollback();
					if(_log.isDebugEnabled()) {
						_log.debug("PromoSAPVASController","For _transferID="+_transferID+" roam commission applicable="+_c2sTransferVO.getDifferentialApplicable()+" roam commission Given="+_c2sTransferVO.getDifferentialGiven() +" Not able to give roam commission getting Exception="+e.getMessage()+" Leaving transaction status as Under process");
					}
					_log.errorTrace(methodName, e);
				}
			...
			if(con!=null)
			{
				...
				if(mcomCon != null){mcomCon.close("PromoSAPVASController#run");mcomCon=null;}
				con=null;
			...
			if(SystemPreferences.SOS_ONLINE_ALLOW)
			{
				if( _c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS))
				{
					...
					try
					{
						...
						{
							_log.error(this, "PromoSAPVASController", methodName +" No record found in database for this number :"+_receiverTransferItemVO.getMsisdn());
							//	throw new BTSLBaseException("PromoSAPVASController","run",PretupsErrorCodesI.SOS_ERROR_EXCEPTION);
						}
					}
					catch(BTSLBaseException be)
					{
						_log.errorTrace(methodName, be);
						_log.error(this, "PromoSAPVASController", "run BTSLBaseException:" +"Transaction ID: "+sosvo.getTransactionID()+"Msisdn"+_receiverTransferItemVO.getMsisdn()+"Getting Exception while processing LMB request :"+be.getMessage());
					}
					catch(Exception e)
					{
						_log.errorTrace(methodName, e);
						_log.error(this, "PromoSAPVASController", "run Exception:" +"Transaction ID: "+sosvo.getTransactionID()+"Msisdn"+_receiverTransferItemVO.getMsisdn()+"Getting Exception while processing LMB request :"+e.getMessage());
					}	
					finally
					{
						if(con!=null) 
			...
			if(_log.isDebugEnabled()) {
				_log.debug("PromoSAPVASController","For _transferID="+_transferID+" Diff applicable="+_c2sTransferVO.getDifferentialApplicable()+" Diff Given="+_c2sTransferVO.getDifferentialGiven());
			}
		}//end try
		...
		catch(Exception e)
		{
			...
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
		...
		finally
		{
			...
			catch(Exception e)
			{
				try{if(con!=null) {
					con.rollback() ;
				}}catch(Exception ex){_log.errorTrace(methodName, ex);}
				_log.errorTrace(methodName, e);
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"PromoSAPVASController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception while updating Receiver last request status in database , Exception:"+e.getMessage());
			}//end catch
			...
			{
				_log.errorTrace(methodName, e);
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"PromoSAPVASController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception while updating transfer details in database , Exception:"+e.getMessage());
			}
			...
			{
				if(_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)&& !(_requestVO.getRecmsg().equals("N")))
				{
					if(!BTSLUtil.isStringIn(_receiverVO.getNetworkCode(),SystemPreferences.SUBSRBR_MSG_STOP))
					{
            ...
            List<String> serviceList = Arrays.asList(SystemPreferences.C2S_REQUEST_COMMON_MESSAGE.split(","));
            ...
            if(!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(),_notAllowedSendMessGatw))
            {
            	...
            	else if(serviceList.contains(_c2sTransferVO.getServiceType()))
            	{
            		if(PretupsI.YES.equalsIgnoreCase(_c2sTransferVO.getCommissionGiven()) && Long.parseLong(_c2sTransferVO.getCommission())>0){
            			pushMessages=(new PushMessage(_senderPushMessageMsisdn,getSenderCommonBonusMessage(),_transferID,_c2sTransferVO.getRequestGatewayCode(),_senderLocale));
            		}
            		else{
            			pushMessages=(new PushMessage(_senderPushMessageMsisdn,getSenderCommonMessage(),_transferID,_c2sTransferVO.getRequestGatewayCode(),_senderLocale));
            		}	//end if
            		...
            		if(((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ADD_COMM_SEPARATE_MSG))).booleanValue()) {
            			if(!BTSLUtil.isNullString(_c2sTransferVO.getSenderReturnMessage())) {
                			(new PushMessage(_senderPushMessageMsisdn,_c2sTransferVO.getSenderReturnMessage(),_transferID,_c2sTransferVO.getRequestGatewayCode(),_senderLocale)).push();
                		}
            			if(_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
...
// Narendra start
	...
	public void updateForReceiverValidateResponse(String str) throws BTSLBaseException
	{
		...
		if(!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) || isRequired)
		{
			...
			try{_receiverTransferItemVO.setAccountStatus(URLDecoder.decode((String)map.get("ACCOUNT_STATUS")));}catch(Exception e){_log.errorTrace(methodName, e);};
			...
			if(BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS))
			{
				...
				//throw new BTSLBaseException("PromoSAPVASController","updateForReceiverValidateResponse",PretupsErrorCodesI.C2S_RECEIVER_FAIL,0,strArr,null);
				//throw new BTSLBaseException("PromoSAPVASController","updateForReceiverValidateResponse",_c2sTransferVO.getErrorCode(),0,strArr,null);
				if(InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED.equals(_receiverTransferItemVO.getValidationStatus())) {
					throw new BTSLBaseException("PromoSAPVASController",methodName,InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED+"_S",0,strArr,null);
				} else {
					throw new BTSLBaseException("PromoSAPVASController",methodName,_c2sTransferVO.getErrorCode(),0,strArr,null);
				}
			}
			_receiverTransferItemVO.setTransferStatus(status);
			...
			try{_receiverTransferItemVO.setPreviousExpiry(BTSLUtil.getDateFromDateString((String)map.get("OLD_EXPIRY_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
			try{
			...
			try{_receiverTransferItemVO.setPreviousBalance(Long.parseLong((String)map.get("INTERFACE_PREV_BALANCE")));}catch(Exception e){_log.errorTrace(methodName, e);};
			...
			try{
				String lmbAllowedValue = (String) map.get("LMB_ALLOWED_VALUE");
			...
			if(!BTSLUtil.isNullString(_requestVO.getInfo4()))
				_c2sTransferVO.setSubscriberType(_requestVO.getInfo4());
			else
				_c2sTransferVO.setSubscriberType(PretupsI.SERIES_TYPE_PREPAID);
		}
	}
...
//		End 07/02/2008
        ...
        }catch(Exception e){_log.errorTrace(methodName, e);};//@@ ends 
		...
		else
		{
			...
			{
				try{_receiverTransferItemVO.setNewExpiry(BTSLUtil.getDateFromDateString((String)map.get("NEW_EXPIRY_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
				//try{_receiverTransferItemVO.setNewGraceDate(BTSLUtil.getDateFromDateString((String)map.get("NEW_GRACE_DATE"),"ddMMyyyy"));}catch(Exception e){};
				try{_receiverTransferItemVO.setPostBalance(Long.parseLong((String)map.get("INTERFACE_POST_BALANCE")));}catch(Exception e){_log.errorTrace(methodName, e);};
				try{_receiverTransferItemVO.setPreviousBalance(Long.parseLong((String)map.get("INTERFACE_PRE_BALANCE")));}catch(Exception e){_log.errorTrace(methodName, e);};
				try{_receiverTransferItemVO.setPostValidationStatus((String)map.get("POST_BALANCE_ENQ_SUCCESS"));}catch(Exception e){_log.errorTrace(methodName, e);};
			}
			try
			{
				if(!BTSLUtil.isNullString((String)map.get("IN_POSTCREDIT_VAL_TIME"))) {
		...
		try{
			String newExpDateStr = (String)map.get("NEW_EXPIRY_DATE");
			if(!BTSLUtil.isNullString(newExpDateStr)){
				_c2sTransferVO.setNewExpiry(BTSLUtil.getDateFromDateString(newExpDateStr,"ddMMyyyy"));
			}
		}
		catch(Exception e){_log.errorTrace(methodName, e);}
		try{
			String newGraceDateStr = (String)map.get("NEW_GRACE_DATE");
			if(!BTSLUtil.isNullString(newGraceDateStr)){
				_c2sTransferVO.setNewGraceDate(BTSLUtil.getDateFromDateString(newGraceDateStr,"ddMMyyyy"));
			}
		}catch(Exception e){_log.errorTrace(methodName, e);};
		try{
			String interfacePostBalanceStr = (String)map.get("INTERFACE_POST_BALANCE");
			if(!BTSLUtil.isNullString(interfacePostBalanceStr)){
				_c2sTransferVO.setPostBalance(Long.parseLong(interfacePostBalanceStr));
				_receiverTransferItemVO.setPostBalance(Long.parseLong(interfacePostBalanceStr));
			}
		}catch(Exception e){_log.errorTrace(methodName, e);};
		...
		try{_c2sTransferVO.setPostCreditCoreValidity(BTSLUtil.getDateFromDateString((String)map.get("POSTCRE_NEW_EXPIRY_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
		try{_c2sTransferVO.setNewGraceDate(BTSLUtil.getDateFromDateString((String)map.get("NEW_GRACE_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
		try{_c2sTransferVO.setPostCreditCoreBalance(Long.parseLong((String)map.get("POSTCRE_INTERFACE_POST_BALANCE")));}catch(Exception e){_log.errorTrace(methodName, e);};
		try{_c2sTransferVO.setPostCreditPromoBalance(Long.parseLong((String)map.get("POSTCRE_INTERFACE_PROMO_POST_BALANCE")));}catch(Exception e){_log.errorTrace(methodName, e);};
		try{_c2sTransferVO.setPostCreditPromoValidity(BTSLUtil.getDateFromDateString((String)map.get("POSTCRE_NEW_PROMO_EXPIRY_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
		try{_c2sTransferVO.setPostValidationStatus((String)map.get("POSTCRE_TRANSACTION_STATUS"));}catch(Exception e){_log.errorTrace(methodName, e);};
		if(!BTSLUtil.isNullString(_requestVO.getInfo4()))
		{
			_c2sTransferVO.setReceiverSubscriberType(_requestVO.getInfo4());
			_receiverTransferItemVO.setSubscriberType(_requestVO.getInfo4());
			_c2sTransferVO.setSubscriberType(_requestVO.getInfo4());
		}
		else
			_c2sTransferVO.setSubscriberType(PretupsI.SERIES_TYPE_PREPAID);
	...
	public void populateServiceInterfaceDetails(Connection p_con,String action) throws BTSLBaseException
	{
		...
		if((!_receiverInterfaceInfoInDBFound && action.equals(PretupsI.INTERFACE_NETWORK_PREFIX_UPDATE_ACTION)) || action.equals(PretupsI.INTERFACE_NETWORK_PREFIX_VALIDATION_ACTION)) {
			...
			{
				//if(_receiverVO.getListValueVO()!=null)
				...
				//System.out.println("((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.MNP_ALLOWED))).booleanValue()"+((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.MNP_ALLOWED))).booleanValue()+isReceiverFound);
			}
			if(!isReceiverFound)
		...
		if(!isReceiverFound) {
			throw new BTSLBaseException("PromoSAPVASController","populateServiceInterfaceDetails",PretupsErrorCodesI.C2S_ERROR_NOTFOUND_SERVICEINTERFACEMAPPING);
		}
	...
	private void checkTransactionLoad() throws BTSLBaseException
	{
		...
		try
		{
			...
			if(recieverLoadStatus==0)
			{
				...
				if(_log.isDebugEnabled()) {
					_log.debug("PromoSAPVASController[checkTransactionLoad]","_transferID="+_transferID+" Successfully through load");
				}
			}
			//Request in Queue
			else if(recieverLoadStatus==1)
			{
				...
				throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.REQUEST_IN_QUEUE,0,strArr,null);
			}
			//Refuse the request
 else {
				throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
			}
		}
		catch(BTSLBaseException be)
		{
			_log.error("PromoSAPVASController[checkTransactionLoad]","Refusing request getting Exception:"+be.getMessage());
			throw be;
		}
		catch(Exception e)
		{
	...
	protected void processValidationRequest() throws BTSLBaseException,SQLException
	{
		...
		try
		{
			...
			// added for lite recharge
			if(_requestVO.getRequestMap()!=null && _requestVO.getRequestMap().get("SERVICECLASS") != null && !BTSLUtil.isNullString((String)_requestVO.getRequestMap().get("SERVICECLASS"))){
            ...
            if((System.currentTimeMillis()-_c2sTransferVO.getRequestStartTime())>interfaceVO.getValExpiryTime())
            {
            	...
            	EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[processValidationRequest]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception: System is taking more time till validation");
            	throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_VAL);
            }
			...
			try { con.commit(); } catch(Exception be) {_log.errorTrace(methodName, be);}
			if(mcomCon != null){mcomCon.close("PromoSAPVASController#processValidationRequest");mcomCon=null;}
			con=null;
            ...
            {
            	EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[run]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception: System is taking more time till topup");
            	throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO=null;
		...
		catch(BTSLBaseException be)
		{
        	...
        	loggerValue.append(be.getMessage());
			_log.error("PromoSAPVASController[processValidationRequest]",loggerValue);
			if(_transferDetailAdded)
			{
				if(mcomCon==null) {
					mcomCon = new MComConnection();con=mcomCon.getConnection();
	...
	public String getReceiverValidateStr()
	{
		...
		if(!_subValRequired)//added for lite recharge
	...
	public String getReceiverCreditStr()
	{
		...
		//try{strBuff.append("&OLD_EXPIRY_DATE="+BTSLUtil.getDateStringFromDate(_receiverTransferItemVO.getPreviousExpiry(),"ddMMyyyy"));}catch(Exception e){	_log.errorTrace(methodName, e);}
		//try{strBuff.append("&OLD_GRACE_DATE="+BTSLUtil.getDateStringFromDate(_receiverTransferItemVO.getPreviousGraceDate(),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);}
		...
		// Birendra NULLPOINTERFIX START
		try{
			String prevPromoBalStr = _c2sTransferVO.getPreviousPromoBalance();
		...
		try{
			String prevPromoExpDateStr = _c2sTransferVO.getPreviousPromoExpiry();
			if(!BTSLUtil.isNullString(prevPromoExpDateStr))
				_previousPromoExpiry=(BTSLUtil.getDateFromDateString(prevPromoExpDateStr,"ddMMyyyy"));
			}
		catch(Exception e){	
			_log.errorTrace(methodName, e);
		}
		// Birendra NULLPOINTERFIX END
		...
		strBuff.append("&REC_ACCESS_FEES="+_c2sTransferVO.getReceiverAccessFee());
		strBuff.append("&GEO_DOMAIN_CODE="+_c2sTransferVO.getGrphDomainCode());//added for gp by karan 
        if(_c2sTransferVO.getInfo10()!=null)   strBuff.append("&GET_INFO_10="+_c2sTransferVO.getInfo10());//added for gp by karan
	...
	private String getReceiverCommonString()
	{
		...
		//to check whether the retailer and subscriber(reciever) belong to same network added by karan
		strBuff.append("&REQUEST_TYPE="+_requestedtype);
		//Added for sending store id in request
		
		if(!BTSLUtil.isNullString(_c2sTransferVO.getInfo1()))
			strBuff.append("&STORE_ID="+_c2sTransferVO.getInfo1());
		return strBuff.toString();
	}
		...
		else
		{
			...
			if(_c2sTransferVO.getReceiverTransferItemVO().isNumberBackAllowed())
			{
				...
				*/
				...
				{
					if((_c2sTransferVO.getBonusTalkTimeValue()==0)) {
						key=PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITHOUT_POSTBAL;//return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITHOUT_POSTBAL,messageArgArray);
					} else {
				...
				{
					if((_c2sTransferVO.getBonusTalkTimeValue()==0)) {
						key=PretupsErrorCodesI.C2S_RECEIVER_SUCCESS;//return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITHOUT_POSTBAL,messageArgArray);
					} else {
			...
			if(!PretupsI.NO.equals(_receiverPostBalanceAvailable))
			{
				...
				if(_c2sTransferVO.getBonusTalkTimeValue()==0) {
					key=PretupsErrorCodesI.C2S_RECEIVER_SUCCESS;//return BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS,messageArgArray);
				} else {
					key=PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITH_BONUS;
				}
			}
	...
	private String getSenderSuccessMessage()
	{
		...
		if (BTSLUtil.isNullString(_c2sTransferVO.getInfo1()))
			return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.PROMO_VAS_SENDER_SUCCESS,messageArgArray,_requestVO.getRequestGatewayType());
	   	else
    		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.PROMO_VAS_SENDER_SUCCESS_OFFER,messageArgArray,_requestVO.getRequestGatewayType());
	...
	{
//		 change for SID
		String[] messageArgArray={_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance())};
	...
	private void processValidationRequestInThread() throws BTSLBaseException,Exception
	{
		...
		{     loggerValue.setLength(0);
		      loggerValue.append("Getting BTSL Base Exception:");
		      loggerValue.append(be.getMessage());
			_log.error("PromoSAPVASController[processValidationRequestInThread]",loggerValue);
		...
		catch(Exception e)
		{
			...
			_c2sTransferVO.setTransferStatus(PretupsErrorCodesI.TXN_STATUS_FAIL);
			 ...
			 loggerValue.append("Exception:");
			 loggerValue.append(e.getMessage());
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[processValidationRequestInThread]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
		...
		finally
		{
			...
			{	
				...
				{
					if(con!=null) {
						try{con.rollback ();}catch(Exception ex){_log.errorTrace(methodName, ex);}
					}
					if(mcomCon != null){mcomCon.close("PromoSAPVASController#processValidationRequestInThread");mcomCon=null;}
					con=null;
					_log.errorTrace(methodName, be);
				}
				catch(Exception e)
				{
					...
					loggerValue.append("Exception:");
					loggerValue.append(e.getMessage());
					EventHandler.handle(EventIDI.DATABASE_CONECTION_PROBLEM,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[processValidationRequestInThread]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
				}
				finally
				{
					if(mcomCon != null){mcomCon.close("PromoSAPVASController#processValidationRequestInThread");mcomCon=null;}
	...
	protected void addEntryInTransfers(Connection p_con)
	{
		...
		{
			_log.errorTrace(methodName, e);
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[process]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			if(!_isCounterDecreased && _decreaseTransactionCounts)
			{
				LoadController.decreaseTransactionLoad(_transferID,_senderNetworkCode,LoadControllerI.DEC_LAST_TRANS_COUNT);
				_isCounterDecreased=true;
		...
		private boolean getInterfaceRoutingDetailsForMNP(Connection p_con,String p_msisdn,long p_prefixID,String p_subscriberType,String p_networkCode,String p_serviceType,String p_interfaceCategory,String p_userType,String p_action)  throws BTSLBaseException
	{
		...
		try
		{
			...
			if(_receiverVO.getListValueVO()!=null && "".equals(_receiverVO.getListValueVO().getValue()))
			{
				//System.out.println("value is "+_receiverVO.getListValueVO().getValue());
	...
	{
		_log.errorTrace(methodName, e);
		EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[getInterfaceRoutingDetails]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
		isSuccess=false;
		throw new BTSLBaseException(this,methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
	}
	if(_log.isDebugEnabled()) {
	...
	private boolean getInterfaceRoutingDetails(Connection p_con,String p_msisdn,long p_prefixID,String p_subscriberType,String p_networkCode,String p_serviceType,String p_interfaceCategory,String p_userType,String p_action)  throws BTSLBaseException
	{
		...
		try
		{
			...
			else
			{
				...
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.INFO,"PromoSAPVASController[getInterfaceRoutingDetails]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:Routing control information not defined so performing series based routing");
				ServiceSelectorInterfaceMappingVO interfaceMappingVO1=null;
				if(((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SELECTOR_INTERFACE_MAPPING)).booleanValue())
				{
					interfaceMappingVO1=(ServiceSelectorInterfaceMappingVO)ServiceSelectorInterfaceMappingCache.getObject(_serviceType+"_"+_c2sTransferVO.getSubService()+"_"+p_action+"_"+p_networkCode+"_"+p_prefixID);
	...
	{     
		...
		catch(Exception e)
		{
			...
			loggerValue.append("Exception:");
			loggerValue.append(e.getMessage());
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[performAlternateRouting]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
	...
	public void receiverValidateResponse(String str,int p_attempt,int p_altSize) throws BTSLBaseException
	{
		...
		try{_receiverTransferItemVO.setPreviousExpiry(BTSLUtil.getDateFromDateString((String)map.get("OLD_EXPIRY_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
		try{_receiverTransferItemVO.setPreviousGraceDate(BTSLUtil.getDateFromDateString((String)map.get("OLD_GRACE_DATE"),"ddMMyyyy"));}catch(Exception e){_log.errorTrace(methodName, e);};
		_receiverTransferItemVO.setServiceClassCode((String)map.get("SERVICE_CLASS"));
	...
	public void processFromQueue(TransferVO p_transferVO)
	{
		...
		try
		{
			...
			try {con.commit();} catch(Exception e){ 
				_log.errorTrace(methodName, e);
				throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.C2S_ERROR_EXCEPTION);}
			if(mcomCon != null){mcomCon.close("PromoSAPVASController#processFromQueue");mcomCon=null;}
			con=null;
						
			
			if(_log.isDebugEnabled()) {
				_log.debug("PromoSAPVASController[processFromQueue]","_transferID="+_transferID+" Successfully through load");
			}
		...
		catch(BTSLBaseException be)
		{
			...
			catch(BTSLBaseException bex)
			{
				...
				loggerValue.append("Leaving Reciever Unmarked Base Exception:");
				loggerValue.append(bex.getMessage());
				EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"PromoSAPVASController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
			}
			catch(Exception e)
			{
				_log.errorTrace(methodName, e);
		...
		catch(Exception e)
		{
			...
			//raising alarm
			EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,"Exception:"+e.getMessage());
			//logging in the transaction log
			TransactionLog.log(_transferID,_requestVO.getRequestIDStr(),_requestVO.getFilteredMSISDN(),"",PretupsI.TXN_LOG_REQTYPE_RES,PretupsI.TXN_LOG_TXNSTAGE_PROCESS,_c2sTransferVO.getSenderReturnMessage(),PretupsI.TXN_LOG_STATUS_FAIL,"Getting Code="+_requestVO.getMessageCode());
		}
		finally
		{
			...
			catch(Exception e)
			{
				...
				loggerValue.append("Exception:");
				loggerValue.append(e.getMessage());
				EventHandler.handle(EventIDI.SYSTEM_ERROR,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.FATAL,"PromoSAPVASController[processFromQueue]",_transferID,_senderMSISDN,_senderNetworkCode,loggerValue.toString());
			}
		...
		if(!BTSLUtil.isNullString(p_languageCode))
		{
			try
			{
				if(LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode)==null) {
					EventHandler.handle(EventIDI.SYSTEM_INFO,EventComponentI.SYSTEM,EventStatusI.RAISED,EventLevelI.MAJOR,"PromoSAPVASController[updateReceiverLocale]",_transferID,_receiverMSISDN,"","Exception: Notification language returned from IN is not defined in system p_languageCode: "+p_languageCode);
				} else {
					_receiverLocale=(LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode));
				}
			}
			catch(Exception e){_log.errorTrace("updateReceiverLocale", e);}
		}
		if(_log.isDebugEnabled()) {
	...
	/*private static synchronized void generateC2STransferID(TransferVO p_transferVO)
	{
		...
		try
		{
			...
			if(_transactionIDCounter==0)
				throw new BTSLBaseException("PromoSAPVASController","generateC2STransferID",PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
	...
	static synchronized void generateC2STransferID(TransferVO p_transferVO)
	{
		...
		try
		{
			...
			p_transferVO.setCreatedOn(mydate);
	  		...
	  		if(_transactionIDCounter==0) {
				throw new BTSLBaseException("PromoSAPVASController",methodName,PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
			}
...
/*	This code was commented on 01/04/08 to eliminate fetch conversion rate step.
	...
	private String getSenderParentSuccessMessage()
	{
		...
		String[] messageArgArray={_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),_requestVO.getSenderLoginID()};
		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.C2S_PARENT_SUCCESS,messageArgArray);
	}
	
	private String getSenderCommonBonusMessage()
	{
		// change for SID
	    SimpleDateFormat sdf = new SimpleDateFormat ("dd/mm/yyyy;HH:mm");
	    String curDate=sdf.format(_currentDate);
	    String customerMSISDN=_c2sTransferVO.getReceiverMsisdn();
	    if(!BTSLUtil.isNullString(_requestVO.getSid()))
	    {
	    	customerMSISDN=_requestVO.getSid();
	    }
	    String[] messageArgArray={_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()+Long.parseLong(_c2sTransferVO.getCommission())),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),customerMSISDN,PretupsBL.getDisplayAmount(_c2sTransferVO.getRetAdjAmt()),PretupsBL.getDisplayAmount(Long.parseLong(_c2sTransferVO.getCommission())),curDate};
		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.C2S_COMMON_BONUS_SUCCESS,messageArgArray);
	}
	
	private String getSenderCommonMessage()
	{
		// change for SID
	    SimpleDateFormat sdf = new SimpleDateFormat ("dd/mm/yyyy;HH:mm");
	    String curDate=sdf.format(_currentDate);
	    ...
	    String[] messageArgArray={_requestVO.getSid(),_transferID,PretupsBL.getDisplayAmount(_c2sTransferVO.getTransferValue()),PretupsBL.getDisplayAmount(_senderTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getValidity()),PretupsBL.getDisplayAmount(_receiverTransferItemVO.getPostBalance()),String.valueOf(_receiverTransferItemVO.getNewGraceDate()),PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverAccessFee()),_c2sTransferVO.getSubService(),customerMSISDN,PretupsBL.getDisplayAmount(_c2sTransferVO.getRetAdjAmt()),curDate};
		return BTSLUtil.getMessage(_senderLocale,PretupsErrorCodesI.C2S_COMMON_SUCCESS,messageArgArray);
	}
}
