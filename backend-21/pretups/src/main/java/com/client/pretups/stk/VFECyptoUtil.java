...
public class VFECyptoUtil extends CryptoUtil {

    private static Log _log = LogFactory.getLog(VFECyptoUtil.class.getName());
    ...
    public String decrypt348Data(String cipherText, String myKeyString, SimProfileVO simProfile) throws Exception348, GeneralSecurityException {
        ...
        // System.out.println("Decrypted hex"+bytesToBinHex(plainText));
     ...
     */
    public String encrypt(String cipherText, String myKeyString, SimProfileVO simProfile) throws Exception348, GeneralSecurityException {
        if (_log.isDebugEnabled()) {
    ...
    public static void main(String args[]){
    	try{
        String iccid = "284966CEA2A6F48EEBD35FCE6C7DE607";
        String masterkey = "89918860900000000500000000000000";
        ...
        VFECyptoUtil abc = new VFECyptoUtil();
        // Code for Encrypting
        // String test = "00000000000089912901071270000020";
        // String ciphertext ="89912901071270000020000000000000";
        // String ciphertext = abc.encrypt(test,keyString.toLowerCase());
        // if(_log.isDebugEnabled())
        // _log.debug("encrypt","12121:::"+ciphertext);
        if (_log.isDebugEnabled()) {
            _log.debug("main: ","PlainText: " + abc.decrypt348Data(iccid.toLowerCase(), masterkey.toLowerCase(), simProfile));
            }
     ...
     */
    /*
     * private String modifyMessage(String decodedData,String strProfile) throws
     * Exception348
     * {
     * if(!"0".equalsIgnoreCase(strProfile))
     * {
     * int fixedIndex = decodedData.indexOf(" F");
     * if(_log.isDebugEnabled()) _log.debug("modifyMessage","fixedIndex "
     * +fixedIndex );
     * 
     * if(fixedIndex != -1)
     * {
     ...
     * if(_log.isDebugEnabled()) _log.debug("encrypt","admInd " +admInd );
     * if(admInd == -1)
     * {
     ...
     * }
     * int spaceInd = -1;
     * if(_log.isDebugEnabled()) _log.debug("encrypt","S1=" +s1 + ": s2=" + s2 +
     * ": s3=" + s3 );
     * do
     * {
     * spaceInd = s2.indexOf(' ');
     ...
     * if(_log.isDebugEnabled()) _log.debug("modifyMessage","Decoded Data"
     * +decodedData );
     * }
     * }///end of indx
     * else//This block has to be reviewed in future
     * {
     * throw new Exception348(
     * "Message cannot be properly decrypted because fixed info is not found for decoded message decodedData"
     * );
     * }
     * }
     * int ind;
