...
public class AdditionalCommissionDetailsReportPostgresQry  implements AdditionalCommissionDetailsReportQry{
	...
	public PreparedStatement loadAdditionalCommisionDetailsOperatorQry(Connection con, UsersReportModel usersReportModel){
		...
		if(log.isDebugEnabled()){ 
			...
			loggerValue.append(" ParentCatCode: ");
        	loggerValue.append(usersReportModel.getParentCategoryCode());
        	loggerValue.append(" FromDate: ");
        	loggerValue.append(usersReportModel.getRptfromDate());
			loggerValue.append(" ToDate: ");
        	loggerValue.append(usersReportModel.getRpttoDate());
		...
		selectQueryBuff.append("TO_CHAR(ADJ.margin_rate").append(",").append(FORMAT).append(" ) as margin_rate, ADJ.otf_type,ADJ.otf_rate as otf_rate, to_number(ADJ.otf_amount,'9999999999999999999999D99') as otf_amount, ");
		...
		selectQueryBuff.append("WHERE DATE_TRUNC('day',CST.TRANSFER_DATE::timestamp)  >= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND DATE_TRUNC('day',CST.TRANSFER_DATE::timestamp)  <= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) ");
		selectQueryBuff.append("AND DATE_TRUNC('day',ADJ.ADJUSTMENT_DATE::timestamp)  >= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND DATE_TRUNC('day',ADJ.ADJUSTMENT_DATE::timestamp)  <= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) ");
		selectQueryBuff.append("AND DATE_TRUNC('minute',ADJ.created_on::timestamp) >= DATE_TRUNC('minute',TO_TIMESTAMP(?,'dd/mm/yy HH24:MI:SS')) ");
		selectQueryBuff.append("AND DATE_TRUNC('minute',ADJ.created_on::timestamp) <= DATE_TRUNC('minute',TO_TIMESTAMP(?,'dd/mm/yy HH24:MI:SS')) AND ADJ.network_code=? AND ADJ.user_id =  CASE ?  WHEN 'ALL' THEN ADJ.user_id ELSE  ? END ");
		...
		selectQueryBuff.append("AND UG.grph_domain_code IN ( with recursive q as( SELECT grph_domain_code, status FROM GEOGRAPHICAL_DOMAINS where grph_domain_code IN (SELECT grph_domain_code FROM USER_GEOGRAPHIES ug1 WHERE UG1.grph_domain_code =CASE ?  WHEN 'ALL' THEN UG1.grph_domain_code ELSE ? END AND UG1.user_id=?) ");		
		selectQueryBuff.append("union all SELECT gd.grph_domain_code, gd.status FROM GEOGRAPHICAL_DOMAINS gd join q on q.grph_domain_code = gd.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y', 'S')) ");
	...
	public PreparedStatement loadAdditionalCommisionDetailsOperatorOldQry(
		...
		selectQueryBuff = BTSLUtil.appendZero(selectQueryBuff, CST_TRANSFER_VALUE, "transfer_value").append(" , ");
		selectQueryBuff.append("GU.user_id,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN N'' ELSE GU.user_name END) grand_name,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN '' ELSE GU.msisdn END) grand_msisdn,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN N'' ELSE GGD.grph_domain_name END) grand_geo_domain,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN N'' ELSE GC.category_name END) grand_category, ");
		selectQueryBuff.append("TO_CHAR(ADJ.created_on, ?) as time,(CASE WHEN ADJ.commission_type='DIFF' THEN 'DIFFERENTIAL' ELSE ADJ.commission_type END) AS Commission_type ");
		...
		selectQueryBuff.append("WHERE ADJ.ADJUSTMENT_DATE  >= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND ADJ.ADJUSTMENT_DATE  <= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS'))  ");
		selectQueryBuff.append("AND ADJ.created_on >= TO_DATE(?,'dd/mm/yy HH24:MI:SS') AND ADJ.created_on <= TO_DATE('?,'dd/mm/yy HH24:MI:SS') AND ADJ.network_code=?  ");
		selectQueryBuff.append("AND ADJ.user_id =  CASE ?  WHEN 'ALL' THEN ADJ.user_id ELSE  ? END AND C.category_code = CASE ?  WHEN 'ALL' THEN C.category_code ELSE  ? END AND ADJ.user_category = C.category_code ");
		selectQueryBuff.append("AND C.domain_code IN(?) AND ADJ.user_id=U.user_id AND CST.transfer_id=ADJ.reference_id AND UG.grph_domain_code = GD.grph_domain_code AND UG.user_id=U.user_id AND PUG.grph_domain_code = PGD.grph_domain_code ");
		selectQueryBuff.append("AND PUG.user_id=PU.user_id AND PU.user_id=CASE U.parent_id WHEN 'ROOT' THEN U.user_id ELSE U.parent_id END AND PU.category_code=PC.category_code AND OUG.grph_domain_code = OGD.grph_domain_code AND OUG.user_id=OU.user_id AND OU.user_id=U.owner_id ");
		selectQueryBuff.append("AND OU.category_code=OC.category_code AND GUG.grph_domain_code = GGD.grph_domain_code AND GUG.user_id=GU.user_id AND GU.user_id=CASE PU.parent_id WHEN 'ROOT' THEN PU.user_id ELSE PU.parent_id END AND GU.category_code=GC.category_code AND ADJ.SERVICE_TYPE=ST.SERVICE_TYPE AND UG.grph_domain_code IN ( ");
		selectQueryBuff.append("with recursive q as( ");
		selectQueryBuff.append("SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN  (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) ");
	...
	public PreparedStatement loadAdditionalCommisionDetailsChannelQry(
		...
		StringBuilder selectQueryBuff = new StringBuilder();
		selectQueryBuff.append("SELECT ADJ.adjustment_id, U.user_id,U.user_name, U.msisdn,C.category_name,GD.grph_domain_name, PU.user_id,(CASE U.parent_id WHEN 'ROOT' THEN N'' ELSE PU.user_name END) parent_name,(CASE  WHEN U.parent_id = 'ROOT' THEN '' ELSE PU.msisdn END) parent_msisdn,(CASE U.parent_id WHEN 'ROOT' THEN N'' ELSE PC.category_name END) parent_cat, (CASE U.parent_id WHEN 'ROOT' THEN N'' ELSE PGD.grph_domain_name END) parent_geo_name, ");
		selectQueryBuff.append("(CASE  WHEN U.parent_id = 'ROOT' THEN N'' WHEN U.user_id = OU.user_id THEN N'' ELSE OU.user_name END) owner_user,(CASE  WHEN U.parent_id = 'ROOT' THEN ''  WHEN U.user_id = OU.user_id THEN '' ELSE OU.msisdn END) owner_msisdn,(CASE  WHEN U.parent_id = 'ROOT' THEN N'' WHEN U.user_id = OU.user_id THEN N'' ELSE OC.category_name END) owner_cat,(CASE  WHEN U.parent_id = 'ROOT' THEN N'' WHEN U.user_id = OU.user_id THEN N'' ELSE OGD.grph_domain_name END)  owner_geo, ");
		selectQueryBuff = BTSLUtil.appendZero(selectQueryBuff, ADJ_MARGIN_AMOUNT, "margin_amount").append(" , ");
		selectQueryBuff.append("TO_CHAR(ADJ.margin_rate").append(",").append(FORMAT).append(" ) as margin_rate,  CST.differential_applicable App_Add_Comm,ST.NAME,CST.receiver_msisdn, ");
		selectQueryBuff = BTSLUtil.appendZero(selectQueryBuff, CST_TRANSFER_VALUE, "transfer_value").append(" , ");
		...
		selectQueryBuff.append("grand_category, TO_CHAR(ADJ.created_on, ?) as time FROM ADJUSTMENTS ADJ, USERS U, C2S_TRANSFERS CST, USER_GEOGRAPHIES UG, GEOGRAPHICAL_DOMAINS GD,CATEGORIES C, USERS PU, USERS OU, CATEGORIES PC, CATEGORIES OC, USER_GEOGRAPHIES PUG, GEOGRAPHICAL_DOMAINS PGD, USER_GEOGRAPHIES OUG, GEOGRAPHICAL_DOMAINS OGD,SERVICE_TYPE ST,USERS GU, USER_GEOGRAPHIES GUG, CATEGORIES GC,GEOGRAPHICAL_DOMAINS GGD ");
		selectQueryBuff.append("WHERE CST.TRANSFER_DATE  >= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND CST.TRANSFER_DATE  <= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS'))");
		selectQueryBuff.append("AND ADJ.ADJUSTMENT_DATE  >= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND ADJ.ADJUSTMENT_DATE  <= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND date_trunc('day',ADJ.created_on::timestamp) >= TO_DATE(?,'dd/mm/yy HH24:MI:SS') AND date_trunc('day',ADJ.created_on::timestamp) <= TO_DATE(?,'dd/mm/yy HH24:MI:SS') AND ADJ.network_code=? ");
		selectQueryBuff.append("AND ADJ.user_id =  CASE ?  WHEN 'ALL' THEN ADJ.user_id ELSE  ? END AND C.category_code = CASE ?  WHEN 'ALL' THEN C.category_code ELSE  ? END AND ADJ.user_category = C.category_code AND C.domain_code IN(?) AND ADJ.user_id=U.user_id AND CST.transfer_id=SUBSTR(ADJ.adjustment_id, 1,LENGTH(ADJ.adjustment_id)-1) AND UG.grph_domain_code = GD.grph_domain_code ");
		selectQueryBuff.append("AND UG.user_id=U.user_id AND PUG.grph_domain_code = PGD.grph_domain_code AND PUG.user_id=PU.user_id AND PU.user_id=CASE U.parent_id WHEN 'ROOT' THEN U.user_id ELSE U.parent_id END AND PU.category_code=PC.category_code AND OUG.grph_domain_code = OGD.grph_domain_code AND OUG.user_id=OU.user_id AND OU.user_id=U.owner_id AND OU.category_code=OC.category_code AND GUG.grph_domain_code = GGD.grph_domain_code AND GUG.user_id=GU.user_id ");
		selectQueryBuff.append("AND GU.user_id=CASE PU.parent_id WHEN 'ROOT' THEN PU.user_id ELSE PU.parent_id END AND GU.category_code=GC.category_code AND ADJ.SERVICE_TYPE=ST.SERVICE_TYPE AND CST.REVERSAL_ID is null AND TXN_TYPE='T' AND UG.grph_domain_code IN ( with recursive q as( ");
		selectQueryBuff.append("SELECT grph_domain_code, status FROM GEOGRAPHICAL_DOMAINS where grph_domain_code IN (SELECT grph_domain_code FROM USER_GEOGRAPHIES ug1 WHERE UG1.grph_domain_code =CASE ?  WHEN 'ALL' THEN UG1.grph_domain_code ELSE  ? END  AND UG1.user_id=?) ");
		selectQueryBuff.append("union all ");
		selectQueryBuff.append("SELECT gd.grph_domain_code, gd.status FROM GEOGRAPHICAL_DOMAINS gd join q on q.grph_domain_code = gd.parent_grph_domain_code) SELECT grph_domain_code FROM q WHERE status IN('Y', 'S')) ");
		selectQueryBuff.append(" GROUP BY CST.transfer_id, ADJ.adjustment_id,U.user_name, U.msisdn, C.category_name, GD.grph_domain_name, PU.user_id, U.parent_id, PU.user_name, PU.msisdn, PC.category_name, PGD.grph_domain_name, U.user_id, OU.user_name, OU.user_id, OU.msisdn, OC.category_name, OGD.grph_domain_name, ADJ.margin_rate, ADJ.otf_type, ADJ.otf_rate, ADJ.otf_amount, CST.differential_applicable,  ST.NAME, CST.receiver_msisdn, GU.user_id, PU.parent_id,GU.user_name, GU.msisdn, GGD.grph_domain_name, GC.category_name, ADJ.created_on, ADJ.commission_type ");
	...
	public PreparedStatement loadAdditionalCommisionDetailsChannelOldQry(
		...
		selectQueryBuff = BTSLUtil.appendZero(selectQueryBuff, CST_TRANSFER_VALUE, "transfer_value").append(" , ");
		selectQueryBuff.append("GU.user_id,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN N'' ELSE GU.user_name END) grand_name,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN '' ELSE GU.msisdn END) grand_msisdn,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN N'' ELSE GGD.grph_domain_name END) grand_geo_domain,(CASE WHEN PU.parent_id = 'ROOT' or U.parent_id = 'ROOT' THEN N'' ELSE GC.category_name END) ");
		selectQueryBuff.append("grand_category, TO_CHAR(ADJ.created_on, ?) as time ");
		selectQueryBuff.append("FROM ADJUSTMENTS ADJ, USERS U, C2S_TRANSFERS_OLD CST, USER_GEOGRAPHIES UG, GEOGRAPHICAL_DOMAINS GD,CATEGORIES C, USERS PU, USERS OU, CATEGORIES PC, CATEGORIES OC, USER_GEOGRAPHIES PUG, GEOGRAPHICAL_DOMAINS PGD, USER_GEOGRAPHIES OUG, GEOGRAPHICAL_DOMAINS OGD,SERVICE_TYPE ST,USERS GU, USER_GEOGRAPHIES GUG, CATEGORIES GC,GEOGRAPHICAL_DOMAINS GGD ");
		selectQueryBuff.append("WHERE  ADJ.ADJUSTMENT_DATE  >= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND ADJ.ADJUSTMENT_DATE  <= DATE_TRUNC('day',TO_DATE(?,'dd/mm/yy HH24:MI:SS')) AND ADJ.created_on >= TO_DATE(?,'dd/mm/yy HH24:MI:SS') AND ADJ.created_on <= TO_DATE(?,'dd/mm/yy HH24:MI:SS') ");
		selectQueryBuff.append("AND ADJ.network_code=? AND ADJ.user_id =  CASE ?  WHEN 'ALL' THEN ADJ.user_id ELSE  ? END AND C.category_code = CASE ?  WHEN 'ALL' THEN C.category_code ELSE  ? END AND ADJ.user_category = C.category_code AND C.domain_code IN(?) ");
		selectQueryBuff.append("AND ADJ.user_id=U.user_id AND CST.transfer_id=SUBSTR(ADJ.adjustment_id, 1,LENGTH(ADJ.adjustment_id)-1) AND UG.grph_domain_code = GD.grph_domain_code AND UG.user_id=U.user_id AND PUG.grph_domain_code = PGD.grph_domain_code AND PUG.user_id=PU.user_id AND PU.user_id=CASE U.parent_id WHEN 'ROOT' THEN U.user_id ELSE U.parent_id END ");
		selectQueryBuff.append("AND PU.category_code=PC.category_code AND OUG.grph_domain_code = OGD.grph_domain_code AND OUG.user_id=OU.user_id AND OU.user_id=U.owner_id AND OU.category_code=OC.category_code AND GUG.grph_domain_code = GGD.grph_domain_code AND GUG.user_id=GU.user_id AND GU.user_id=CASE PU.parent_id WHEN 'ROOT' THEN PU.user_id ELSE PU.parent_id END AND GU.category_code=GC.category_code ");
		selectQueryBuff.append("AND ADJ.SERVICE_TYPE=ST.SERVICE_TYPE AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN  (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) ");
		selectQueryBuff.append("AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ");
		selectQueryBuff.append(" GROUP BY CST.transfer_id, ADJ.adjustment_id,U.user_name, U.msisdn, C.category_name, GD.grph_domain_name, PU.user_id, U.parent_id, PU.user_name, PU.msisdn, PC.category_name, PGD.grph_domain_name, U.user_id, OU.user_name, OU.user_id, OU.msisdn, OC.category_name, OGD.grph_domain_name, ADJ.margin_rate, ADJ.otf_type, ADJ.otf_rate, ADJ.otf_amount, CST.differential_applicable,  ST.NAME, CST.receiver_msisdn, GU.user_id, PU.parent_id,GU.user_name, GU.msisdn, GGD.grph_domain_name, GC.category_name, ADJ.created_on, ADJ.commission_type ");
		selectQueryBuff.append("ORDER BY 27 Desc");
