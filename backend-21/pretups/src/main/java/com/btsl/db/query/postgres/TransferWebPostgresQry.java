...
public class TransferWebPostgresQry implements TransferWebQry{
	@Override
	public String loadTransferRuleListQry(){
        ...
        strBuff.append("FROM transfer_rules tr left join categories ct on  ct.category_code=tr.category_code left join channel_grades gr on  gr.grade_code=tr.grade_code ");
	...
	public String loadP2PReconciliationListQry(String pnetworkCodeType){
        ...
        selectQueryBuff.append("FROM subscriber_transfers STRF left join p2p_subscribers U on STRF.sender_id=U.user_id ");
        selectQueryBuff.append("left join key_values KV on (STRF.error_code=KV.key and KV.type=?) ");
        selectQueryBuff.append("left join key_values KV1 on (STRF.transfer_status = KV1.key and KV1.type=?) ,products PROD,service_type ST ");
        selectQueryBuff.append("WHERE ");
        selectQueryBuff.append("STRF.transfer_date >=? AND STRF.transfer_date < ? ");
	...
	public String loadUserListQry(){
        ...
        strBuff.append(") AND U.user_type= ? AND u.status IN ('Y','S','SR') ");
	...
	public String addPromotionalTransferRuleFileQry(){
        ...
        selectMSISDN.append("AND UG.grph_domain_code IN (with recursive q as (select grph_domain_code, status from geographical_domains WHERE GRPH_DOMAIN_TYPE =? ");
        selectMSISDN.append("union all select m.grph_domain_code, m.status from geographical_domains m join q on q.grph_domain_code=m.parent_grph_domain_code ");
        selectMSISDN.append(")select grph_domain_code from q where status IN ('Y','S')) AND u.status IN ('Y','S','SR') ");
