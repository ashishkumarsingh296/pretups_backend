...
public class LMSProgressiveMessagesOracleQry implements LMSProgressiveMessagesQry{
	...
	public String getUsersforMessageQry() {
		...
		selectQueryBuff.append(" AND trunc(psv.applicable_from) <= ? ");
		selectQueryBuff.append(" and Pd.VERSION=(SELECT psv.VERSION FROM PROFILE_SET_VERSION psv ");
		selectQueryBuff.append(" WHERE (applicable_from=(SELECT MAX(applicable_from) FROM PROFILE_SET_VERSION WHERE TRUNC(applicable_from)<=? and status='Y' ");
		...
		// version here
		// ");
		selectQueryBuff.append(" AND psv.status=? and trunc (psv.applicable_to + 1)>=? ");// Closing
		// bracket
		// for
		// version
		// here
	...
	public String sendAlertsGetSumSenderTransferAmountQry() {
        ...
        selectQueryBuff.append(" where user_id =? and service_type=? and TRANS_DATE >= ? and TRANS_DATE <=sysdate-1");
		return selectQueryBuff.toString();
	}
	...
	public String sendAlertsGetSumC2CTransferOutAmountQry() {
        ...
        selectQueryBuff2.append(" where user_id =?  and TRANS_DATE >= ? and TRANS_DATE <=sysdate-1");
	...
	public String sendAlertsGetSumO2CTransferInAmountQry() {
        ...
        selectQueryBuff3.append(" where user_id =?  and TRANS_DATE >= ? and TRANS_DATE <=sysdate-1");
