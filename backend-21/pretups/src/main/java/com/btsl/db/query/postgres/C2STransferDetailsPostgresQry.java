 ...
 *
 */
public class C2STransferDetailsPostgresQry implements C2STransferRptQry{
	...
	private static final String FROMDATETIMESTAMP = "FromDateTimestamp";
	private static final String TODATETIMESTAMP = "ToDateTimestamp";
	...
	public PreparedStatement loadC2sTransferChannelUserStaffReport(	Connection con, UsersReportModel usersReportModel) {
		...
		final String methodName ="loadC2sTransferChannelUserStaffReport";
		 final StringBuilder strBuff = new StringBuilder(); 		
		 strBuff.append("SELECT CT.transfer_status status, CT.transfer_id, TO_CHAR(CT.transfer_date, ?) transfer_date, TO_CHAR(CT.transfer_date_time, ?)transfer_date_time, N.network_name, U.user_name, UC.user_name initiator_user, CT.transfer_value, KV.value transfer_status, ST.name service_name, CT.receiver_msisdn, CT.sender_msisdn, L.selector_name subService_name, CT.receiver_transfer_value, CT.receiver_access_fee, CT.receiver_bonus_value, CT.request_gateway_type,CT.error_code,CTI.service_class_id,SC.service_class_name FROM c2s_transfers_old CT, networks N, key_values KV, service_type ST, service_type_selector_mapping L, users U, users UC, categories CAT, user_geographies UG, geographical_domains GD ,service_classes SC left join c2s_transfer_items CTI on SC.service_class_id=CTI.service_class_id WHERE CTI.transfer_id=CT.transfer_id AND CTI.sno=2 AND CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.key AND KV.type = 'C2S_STATUS' AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.service_type = ST.service_type AND CT.service_type = case ? when 'ALL' then CT.service_type else ? end AND CT.sub_service = L.selector_code AND CT.service_type = L.service_type AND CT.network_code = ? AND CT.transfer_date >= ? AND CT.transfer_date <= ? AND CT.sender_id IN( with recursive q as( SELECT U11.user_id FROM users U11 where U11.user_id=? union all SELECT U11.user_id FROM users U11 join q on q.user_id = U11.parent_id ) SELECT user_id FROM q where user_id=CASE ? WHEN 'ALL' THEN user_id ELSE ? END ) AND CT.sender_id = U.user_id AND CAT.category_code = case ? when 'ALL' then CAT.category_code else ? end AND CAT.category_code = U.category_code -- AND CAT.sequence_no >= ? AND CAT.domain_code = ? AND U.user_id = UG.user_id AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ORDER BY CT.transfer_date_time desc");
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
	...
	public PreparedStatement loadC2sTransferChannelUserNewReport(Connection con, UsersReportModel usersReportModel) {
		 ...
		 final StringBuilder strBuff = new StringBuilder(); 				
		 strBuff.append("SELECT CT.transfer_status status, CT.transfer_id, TO_CHAR(CT.transfer_date, ?) transfer_date, ");
		 strBuff.append("TO_CHAR(CT.transfer_date_time, ?) transfer_date_time, N.network_name, U.user_name, CT.transfer_value, KV.VALUE transfer_status, ST.NAME service_name,");
		 strBuff.append("(Case when CT.subs_sid IS NULL then CT.receiver_msisdn else CT.subs_sid end) Reciever_MSISDN, CT.sender_msisdn, L.selector_name subService_name, ");
		 strBuff.append("CT.receiver_transfer_value, CT.receiver_access_fee, CT.receiver_bonus_value, CT.request_gateway_type,CT.error_code,CT.service_class_id,");
		 strBuff.append("SC.service_class_name,UP.PRIMARY_NUMBER MSISDN_TYPE, CT.REVERSAL_ID FROM NETWORKS N, KEY_VALUES KV, SERVICE_TYPE ST, SERVICE_TYPE_SELECTOR_MAPPING L, ");
		 strBuff.append("USERS U, CATEGORIES CAT, USER_GEOGRAPHIES UG, GEOGRAPHICAL_DOMAINS GD ,SERVICE_CLASSES SC left join C2S_TRANSFERS CT on SC.service_class_id=CT.service_class_id ,USER_PHONES UP ");
		 strBuff.append("WHERE CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.KEY AND KV.TYPE = 'C2S_STATUS' ");
		 strBuff.append("AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.SERVICE_TYPE = ST.SERVICE_TYPE ");
		 strBuff.append("AND CT.SERVICE_TYPE = CASE ? WHEN 'ALL' THEN CT.SERVICE_TYPE ELSE ? END AND CT.sub_service = L.selector_code AND CT.SERVICE_TYPE = L.SERVICE_TYPE ");
		 strBuff.append("AND CT.network_code =?");
		 strBuff.append("AND CT.TRANSFER_DATE >= DATE_TRUNC('day',TO_DATE(?,?)) AND CT.TRANSFER_DATE <= DATE_TRUNC('day',TO_DATE(?,?))");
		 strBuff.append(" AND DATE_TRUNC('minute',CT.TRANSFER_DATE_TIME::timestamp) >= DATE_TRUNC('minute',TO_TIMESTAMP(?,?)) AND DATE_TRUNC('minute',CT.TRANSFER_DATE_TIME::timestamp) <= DATE_TRUNC('minute',TO_TIMESTAMP(?,?))");
		 strBuff.append(" AND CT.sender_id = CASE ? WHEN 'ALL' THEN CT.sender_id ELSE ? END AND CT.sender_id = U.user_id");
		 strBuff.append(" AND CAT.category_code = CASE ? WHEN 'ALL' THEN CAT.category_code ELSE ? END AND CAT.category_code = U.category_code AND CAT.domain_code = ? AND U.user_id = UG.user_id AND U.user_id=UP.USER_ID AND CT.SENDER_MSISDN=UP.MSISDN AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ORDER BY CT.transfer_date_time Desc");		
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
		 try {
		      if (log.isDebugEnabled()) {
	...
	public PreparedStatement loadC2sTransferChannelUserStaffNewReport(Connection con, UsersReportModel usersReportModel) {		
		 ...
		 final StringBuilder strBuff = new StringBuilder(); 		
		 strBuff.append("SELECT CT.transfer_status status, CT.transfer_id, TO_CHAR(CT.transfer_date, ?)transfer_date, TO_CHAR(CT.transfer_date_time, ?) transfer_date_time, N.network_name, U.user_name, UC.user_name initiator_user, CT.transfer_value, KV.value transfer_status, ST.name service_name, CT.receiver_msisdn, CT.sender_msisdn, L.selector_name subService_name, CT.receiver_transfer_value, CT.receiver_access_fee, CT.receiver_bonus_value, CT.request_gateway_type,CT.error_code,CT.service_class_id,SC.service_class_name ");
		 strBuff.append("FROM networks N, key_values KV, service_type ST, service_type_selector_mapping L, users U, users UC, categories CAT, user_geographies UG, geographical_domains GD,service_classes SC right join c2s_transfers CT on SC.service_class_id=CT.service_class_id ");
		 strBuff.append("WHERE CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.key AND KV.type = 'C2S_STATUS' AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.service_type = ST.service_type AND CT.service_type = case ? when 'ALL' then CT.service_type else ? end AND CT.sub_service = L.selector_code AND CT.service_type = L.service_type AND CT.network_code = ? ");
		 strBuff.append("AND CT.transfer_date >= ? AND CT.transfer_date <= ? AND CT.sender_id IN( with recursive q as( SELECT U11.user_id FROM users U11 where U11.user_id=? union all SELECT U11.user_id FROM users U11 join q on q.user_id = U11.parent_id ) SELECT user_id FROM q where user_id=CASE ? WHEN 'ALL' THEN user_id ELSE ? END ) AND CT.sender_id = U.user_id AND CT.active_user_id=UC.user_id AND CAT.category_code = case ? when 'ALL' then CAT.category_code else ? end AND CAT.category_code = U.category_code AND CAT.sequence_no >= ? AND CAT.domain_code = ? AND U.user_id = UG.user_id AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ORDER BY CT.transfer_date_time desc");
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
	...
	public PreparedStatement loadC2sTransferNewReport(Connection con,UsersReportModel usersReportModel) {
		...
		final String methodName ="loadC2sTransferNewReport";
		 final StringBuilder strBuff = new StringBuilder(); 		
		 strBuff.append("SELECT CT.transfer_status status, CT.transfer_id, TO_CHAR(CT.transfer_date, ?)transfer_date,TO_CHAR(CT.transfer_date_time, ?) transfer_date_time, N.network_name, U.user_name, CT.transfer_value, KV.VALUE transfer_status, ST.NAME service_name, (Case when CT.subs_sid IS NULL then CT.receiver_msisdn else CT.subs_sid end) Reciever_MSISDN, CT.sender_msisdn, L.selector_name subService_name, CT.receiver_transfer_value, CT.receiver_access_fee, CT.receiver_bonus_value, CT.request_gateway_type,CT.error_code,CT.service_class_id,SC.service_class_name,UP.PRIMARY_NUMBER MSISDN_TYPE, CT.REVERSAL_ID, CT.MULTICURRENCY_DETAIL,U.external_code ");
		 strBuff.append("FROM NETWORKS N, KEY_VALUES KV, SERVICE_TYPE ST, SERVICE_TYPE_SELECTOR_MAPPING L, USERS U, CATEGORIES CAT, USER_GEOGRAPHIES UG, GEOGRAPHICAL_DOMAINS GD ,SERVICE_CLASSES SC right join C2S_TRANSFERS CT on SC.service_class_id=CT.service_class_id ,USER_PHONES UP ");
		 strBuff.append("WHERE CT.transfer_date >= ? AND CT.transfer_date <= ? AND CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.KEY AND KV.TYPE = 'C2S_STATUS' AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.SERVICE_TYPE = ST.SERVICE_TYPE AND CT.SERVICE_TYPE = CASE ? WHEN 'ALL' THEN CT.SERVICE_TYPE ELSE ? END AND CT.sub_service = L.selector_code AND CT.SERVICE_TYPE = L.SERVICE_TYPE AND CT.network_code = ? AND DATE_TRUNC('day', CT.TRANSFER_DATE_TIME ::timestamp ) >= TO_DATE(?,?) AND DATE_TRUNC('day', CT.TRANSFER_DATE_TIME ::timestamp ) <= TO_DATE(?,?) AND DATE_TRUNC('minute',CT.TRANSFER_DATE_TIME::timestamp) >= DATE_TRUNC('minute',TO_TIMESTAMP(?,?)) AND DATE_TRUNC('minute',CT.TRANSFER_DATE_TIME::timestamp) <= DATE_TRUNC('minute',TO_TIMESTAMP(?,?)) AND CT.sender_id = CASE ? WHEN 'ALL' THEN CT.sender_id ELSE ? END AND CT.sender_id = U.user_id AND CAT.category_code = CASE ? WHEN 'ALL' THEN CAT.category_code ELSE ? END AND CAT.category_code = U.category_code AND CAT.domain_code = ? AND U.user_id = UG.user_id AND U.user_id=UP.USER_ID AND CT.SENDER_MSISDN=UP.MSISDN AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ORDER BY CT.transfer_date_time Desc");
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
	...
	public PreparedStatement loadC2sTransferReport(Connection con,UsersReportModel usersReportModel) {
		 ...
		 final StringBuilder strBuff = new StringBuilder(); 		
		 strBuff.append("SELECT CT.transfer_status status, CT.transfer_id, TO_CHAR(CT.transfer_date, ?) transfer_date, TO_CHAR(CT.transfer_date_time, ?)transfer_date_time, N.network_name, U.user_name, CT.transfer_value, KV.VALUE transfer_status, ST.NAME service_name, CT.receiver_msisdn, CT.sender_msisdn, L.selector_name subService_name, CT.receiver_transfer_value, CT.receiver_access_fee, CT.receiver_bonus_value, CT.request_gateway_type,CT.error_code,CTI.service_class_id,SC.service_class_name,UP.PRIMARY_NUMBER MSISDN_TYPE ");
		 strBuff.append("FROM NETWORKS N, KEY_VALUES KV, SERVICE_TYPE ST, SERVICE_TYPE_SELECTOR_MAPPING L, USERS U, CATEGORIES CAT, USER_GEOGRAPHIES UG, GEOGRAPHICAL_DOMAINS GD ,C2S_TRANSFER_ITEMS CTI left join C2S_TRANSFERS CT_OLD on CTI.transfer_id=CT.transfer_id AND CTI.sno=2 left join SERVICE_CLASSES SC on SC.service_class_id=CTI.service_class_id ,USER_PHONES UP WHERE CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.KEY AND KV.TYPE = 'C2S_STATUS' AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.SERVICE_TYPE = ST.SERVICE_TYPE AND CT.SERVICE_TYPE = CASE ? WHEN 'ALL' THEN CT.SERVICE_TYPE ELSE ? END AND CT.sub_service = L.selector_code AND CT.SERVICE_TYPE = L.SERVICE_TYPE AND CT.network_code = ? ");
		 strBuff.append("AND CT.TRANSFER_DATE >= DATE_TRUNC('day',TO_DATE(?,?)) AND CT.TRANSFER_DATE <= DATE_TRUNC('day',TO_DATE(?,?)) AND CT.TRANSFER_DATE_TIME >= TO_DATE(?,?) AND CT.TRANSFER_DATE_TIME <= TO_DATE(?,?) AND CT.sender_id = CASE ? WHEN 'ALL' THEN CT.sender_id ELSE ? END AND CT.sender_id = U.user_id AND CAT.category_code = CASE ? WHEN 'ALL' THEN CAT.category_code ELSE ? END AND CAT.category_code = U.category_code AND CAT.domain_code = ? AND U.user_id = UG.user_id AND U.user_id=UP.USER_ID AND CT.SENDER_MSISDN=UP.MSISDN AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) order by CT.transfer_date_time desc");
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
	...
	public PreparedStatement loadC2sTransferStaffNewReport(Connection con,UsersReportModel usersReportModel) {
		...
		final String methodName ="loadC2sTransferStaffNewReport";
		 ...
		 strBuff.append("FROM networks N, key_values KV, service_type ST, service_type_selector_mapping L, users U, users UC, categories CAT, user_geographies UG, geographical_domains GD,service_classes SC right join c2s_transfers CT on SC.service_class_id=CT.service_class_id ");
		 strBuff.append("WHERE CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.key AND KV.type = 'C2S_STATUS' AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.service_type = ST.service_type AND CT.service_type = case ? when 'ALL' then CT.service_type else ? end AND CT.sub_service = L.selector_code AND CT.service_type = L.service_type AND CT.network_code = ? ");
		 strBuff.append("AND CT.transfer_date >= ? AND CT.transfer_date <= ? AND CT.sender_id = case ? when 'ALL' then CT.sender_id else ? end AND CT.active_user_id=UC.user_id AND CT.sender_id = U.user_id AND CAT.category_code = case ? when 'ALL' then CAT.category_code else ? end AND CAT.category_code = U.category_code AND CAT.domain_code = ? AND U.user_id = UG.user_id AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ORDER BY CT.transfer_date_time Desc");
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
	...
	public PreparedStatement loadC2sTransferStaffReport(Connection con,	UsersReportModel usersReportModel) {		
		...
		final String methodName ="loadC2sTransferStaffReport";
		 final StringBuilder strBuff = new StringBuilder(); 		
		 strBuff.append("SELECT CT.transfer_status status, CT.transfer_id, TO_CHAR(CT.transfer_date, ?)transfer_date, TO_CHAR(CT.transfer_date_time, ?) transfer_date_time, N.network_name, U.user_name, UC.user_name initiator_user, CT.transfer_value, KV.value transfer_status, ST.name service_name, CT.receiver_msisdn, CT.sender_msisdn, L.selector_name subService_name, CT.receiver_transfer_value, CT.receiver_access_fee, CT.receiver_bonus_value, CT.request_gateway_type,CT.error_code,CTI.service_class_id,SC.service_class_name FROM c2s_transfers_old CT, networks N, key_values KV, service_type ST, service_type_selector_mapping L, users U, users UC, categories CAT, user_geographies UG, geographical_domains GD ,service_classes SC right join c2s_transfer_items CTI on SC.service_class_id=CTI.service_class_id WHERE CTI.transfer_id=CT.transfer_id AND CTI.sno=2 AND CT.receiver_network_code = N.network_code AND CT.transfer_status = KV.key AND KV.type = 'C2S_STATUS' AND CT.transfer_status= CASE ? WHEN 'ALL' THEN CT.transfer_status ELSE ? END AND CT.service_type = ST.service_type AND CT.service_type = case ? when 'ALL' then CT.service_type else ? end AND CT.sub_service = L.selector_code AND CT.service_type = L.service_type AND CT.network_code = ? AND CT.transfer_date >= ? AND CT.transfer_date <= ? AND CT.sender_id = case ? when 'ALL' then CT.sender_id else ? end AND CT.sender_id = U.user_id AND CAT.category_code = case ? when 'ALL' then CAT.category_code else ? end AND CAT.category_code = U.category_code AND CAT.domain_code = ? AND U.user_id = UG.user_id AND UG.grph_domain_code = GD.grph_domain_code AND UG.grph_domain_code IN ( with recursive q as( SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 where grph_domain_code IN (SELECT grph_domain_code FROM user_geographies UG1 WHERE UG1.grph_domain_code = (case ? when 'ALL' then UG1.grph_domain_code else ? end) AND UG1.user_id=?) union all SELECT gd1.grph_domain_code, gd1.status FROM geographical_domains GD1 join q on q.grph_domain_code = gd1.parent_grph_domain_code ) SELECT grph_domain_code FROM q WHERE status IN('Y','S') ) ORDER BY CT.transfer_date_time Desc");
		 String sqlSelect=strBuff.toString();		
		 PreparedStatement pstmt = null;
...
@Override
	public PreparedStatement getTotalSalesQuery(Connection con,UsersReportForm thisForm) {
		...
		int amountMultFactor = ((Integer) (PreferenceCache.getSystemPreferenceValue(PreferenceI.AMOUNT_MULT_FACTOR))).intValue();			
			...
			selectQueryBuff.append("sum(d.C2C_TRANSFER_OUT_AMOUNT+d.C2C_WITHDRAW_IN_AMOUNT+d.C2S_TRANSFER_IN_AMOUNT+d.C2C_REVERSE_IN_AMOUNT)/")
