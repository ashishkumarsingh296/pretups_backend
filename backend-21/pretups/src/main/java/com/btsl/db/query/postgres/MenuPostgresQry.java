...
public class MenuPostgresQry implements MenuQry{
	...
	public StringBuilder loadAssignedMenuItemListQry() {
		...
		if(PretupsI.YES.equalsIgnoreCase(Constants.getProperty("IS_SPRING_ENABLE"))){
         ...
         queryBuff.append(" PAGES.menu_item,PAGES.sequence_no,MODULE.module_name,PAGES.menu_level,coalesce(NULLIF(ROLES.from_hour,''),'0') FROMHOUR,coalesce(NULLIF(ROLES.to_hour,''),'24') TOHOUR,MODULE.sequence_no MSEQ,ROLES.role_code");
         ...
         queryBuff.append(" AND (ROLES.role_type IS NULL OR ROLES.role_type=(case ? when 'ALL' then ROLES.role_type else ? end))");
	...
	public StringBuilder loadAssignedMenuItemListGroupRoleQry() {
		...
		groupRoleQueryBuff.append(" PAGES.menu_item,PAGES.sequence_no,MODULE.module_name,PAGES.menu_level,coalesce(NULLIF(ROLES.from_hour,''),'0') FROMHOUR,coalesce(NULLIF(ROLES.to_hour,''),'24') TOHOUR,MODULE.sequence_no MSEQ,GR.role_code");
         ...
         groupRoleQueryBuff.append(" AND (ROLES.status IS NULL OR ROLES.status='Y')   AND PAGES.module_code=MODULE.module_code AND (ROLES.role_type IS NULL OR ROLES.role_type=case ? when 'ALL' then ROLES.role_type else ? end)  ");
         // Start Zebra and Tango
	...
	public StringBuilder loadFixedMenuItemListQry() {
        ...
        queryBuff.append(" PAGES.menu_item,PAGES.sequence_no,MODULE.module_name,PAGES.menu_level,coalesce(CAST(ROLES.from_hour as integer),0) FROMHOUR, coalesce(CAST(ROLES.to_hour as integer),24) TOHOUR,MODULE.sequence_no MSEQ,ROLES.role_code");
        queryBuff.append(" ,ROLES.ACCESS_TYPE ");
