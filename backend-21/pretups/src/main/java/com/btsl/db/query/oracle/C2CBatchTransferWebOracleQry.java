...
public class C2CBatchTransferWebOracleQry implements C2CBatchTransferWebQry {
	...
	public String loadBatchC2CMasterDetailsForTxrQry(String p_itemStatus,
		...
		strBuff.append(" , SUM(DECODE(cbi.status,?,1,0)) NEW,SUM(DECODE(cbi.status,?,1,0)) cncl, SUM(DECODE(cbi.status,?,1,0)) CLOSE, ");
		...
		strBuff.append(" c2cb.sms_default_lang,c2cb.sms_second_lang,cbi.transfer_type,cbi.transfer_sub_type ,c2cb.domain_code,c2cb.batch_date,c2cb.created_by , cbi.category_code, p.PRODUCT_NAME ");
	...
	public String loadBatchC2CMasterDetailsForWdrQry(String p_itemStatus,
		...
		StringBuilder strBuff = new StringBuilder();
		strBuff.append("SELECT * FROM ( SELECT DISTINCT c2cb.batch_id,c2cb.batch_name,c2cb.batch_total_record, c2cb.user_id,  ");
		strBuff.append(" SUM(DECODE(cbi.status,?,1,0)) NEW,SUM(DECODE(cbi.status, ?,1,0)) cncl, ");
		strBuff.append(" SUM(DECODE(cbi.status, ?,1,0)) CLOSE,  c2cb.network_code,c2cb.network_code_for,c2cb.product_code, c2cb.modified_by, c2cb.modified_on , ");
		strBuff.append("   c2cb.domain_code,c2cb.batch_date,c2cb.sms_default_lang,c2cb.sms_second_lang,cbi.transfer_type,cbi.transfer_sub_type,c2cb.created_by,cbi.category_code, p.PRODUCT_NAME   ");
		strBuff.append(" FROM C2C_BATCHES c2cb,C2C_BATCH_ITEMS cbi, Products p");
		strBuff.append(" WHERE c2cb.user_id=? ");
		strBuff.append(" AND c2cb.status= ? AND  c2cb.batch_id=cbi.batch_id AND cbi.PRODUCT_CODE = p.PRODUCT_CODE  ");
	...
	public String processOrderByBatchQry() {
		...
		sqlBuffer
				.append("SELECT c2cb.batch_total_record,SUM(DECODE(cbi.status,?,1,0)) new,  ");
		sqlBuffer
				.append(" SUM(DECODE(cbi.status,?,1,0)) appr,SUM(DECODE(cbi.status,?,1,0)) cncl, ");
		sqlBuffer.append(" SUM(DECODE(cbi.status,?,1,0)) close ");
	...
	public String loadBatchC2CMasterDetailsForTxrAndWdrQry(String p_itemStatus,
		...
		strBuff.append(" c2cb.user_id,c2cb.network_code,c2cb.network_code_for, c2cb.modified_by, c2cb.modified_on, ");
	...
	public String loadBatchDetailsByAdvancedQry() {
		...
		strBuff.append(" \r\n"
				...
				+ "  SUM(DECODE(cbi.status, ? ,1,0)) NEW, \r\n"
				+ "  SUM(DECODE(cbi.status, ? ,1,0)) CNCL, \r\n"
				+ "  SUM(DECODE(cbi.status, ? ,1,0)) CLOSE,\r\n"
				+ "  cb.created_on \r\n"
				+ " from c2c_batches cb,c2c_batch_items cbi,domains d ,products p ,categories c,users u\r\n"
				+ "  where cb.batch_id =cbi.batch_id   \r\n"
				+ "  AND TRUNC(cbi.transfer_date) >= ? AND TRUNC(cbi.transfer_date) <= ? \r\n"
				+ "  and d.domain_code = cb.domain_code \r\n"
				+ "  AND d.DOMAIN_CODE = ?\r\n"
				+ "  AND cb.CREATED_BY = ?\r\n"
				+ "  and p.product_code =cbi.product_code \r\n"
	...
	public String loadBatchDetailsByBatchIdQry() {
		StringBuilder strBuff = new StringBuilder();
		strBuff.append("\r\n"
				+ "      SELECT  SUM(DECODE(cbi.status, ? ,1,0)) NEW,  \r\n"
				+ "	 	 SUM(DECODE(cbi.status, ? ,1,0)) CNCL,    \r\n"
				+ "	     SUM(DECODE(cbi.status, ? ,1,0)) CLOSE,\r\n"
				+ "		 ccb.BATCH_ID ,ccb.BATCH_NAME,\r\n"
				+ "		 p.PRODUCT_NAME ,p.PRODUCT_CODE,\r\n"
				+ "		 ccb.BATCH_DATE ,ccb.BATCH_TOTAL_RECORD	  \r\n"
				+ "		 FROM C2C_BATCHES ccb,C2C_BATCH_ITEMS cbi,PRODUCTS p WHERE \r\n"
				+ "		 ccb.BATCH_ID =?\r\n"
