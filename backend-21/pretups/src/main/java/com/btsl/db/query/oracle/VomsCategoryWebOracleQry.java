...
public class VomsCategoryWebOracleQry implements VomsCategoryWebQry {
	...
	public String loadSubCategoryListForView(boolean isSubCategory, String segment) {
		...
		if (isSubCategory) {
			strBuff.append("  VC.voucher_type=decode(?,?,VC.voucher_type,?) AND ");
		}

		strBuff.append(
				" VC.network_code = ? AND category_type = ? AND (VC.status = ? OR VC.status IS NULL) AND L.lookup_code=VC.status "
	...
	public String loadCategoryListForUser(String p_status, String p_userId) {
		StringBuilder strBuff = new StringBuilder(
				" SELECT vc.category_id,vc.description,vc.category_type,vc.category_short_name,vc.mrp,vc.status,");
		strBuff.append(
				" vc.category_name,vc.payable_amount,vvsm.sub_service typ,vc.voucher_type,vvsm.service_type,vvsm.sub_service, vc.voucher_segment, vt.TYPE voucher_type_code FROM voms_categories vc,voms_vtype_service_mapping vvsm , user_vouchertypes uv, users u, voms_types vt  ");
		strBuff.append(
				" WHERE vc.service_id=vvsm.service_id AND (vc.status = ? OR vc.status IS NULL)   and uv.USER_ID= u.USER_ID and u.USER_ID= ? and uv.VOUCHER_TYPE = vvsm.VOUCHER_TYPE  and vc.VOUCHER_TYPE=vt.VOUCHER_TYPE and vc.network_code=? ORDER BY mrp");
		return strBuff.toString();
	}
