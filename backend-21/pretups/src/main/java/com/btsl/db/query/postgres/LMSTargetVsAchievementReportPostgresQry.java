...
public class LMSTargetVsAchievementReportPostgresQry  implements LMSTargetVsAchievementReportQry{
	...
	public String loadRefTargetProfileQry() {
		...
		selectQueryBuffer.append(" FROM profile_details pd, profile_set ps, profile_set_version psv, channel_users cu,  user_phones up right join  users u on up.user_id=u.parent_id, products p, user_oth_profiles uop,user_geographies geo,categories cat,domains dom,geographical_domains gdomains");
		...
		selectQueryBuffer.append(" and DATE_TRUNC('day',(psv.applicable_to + interval '1 day')::timestamp) >=? "); // Plus 1 added to provide the bonus if weekly/monthly profile created
	...
	public String loadNonRefTargetProfilesQry() {
		...
		selectQueryBuffer.append(" ps.OPT_IN_OUT_ENABLED ,OPT_IN_OUT_STATUS, ");
		selectQueryBuffer.append(" dom.DOMAIN_NAME,gdomains.GRPH_DOMAIN_NAME,u.user_name ");
		selectQueryBuffer.append(" FROM profile_details pd, profile_set ps, profile_set_version psv, channel_users cu, user_phones up right join  users u on  up.user_id=u.parent_id   , products p,user_geographies geo,categories cat,domains dom,geographical_domains gdomains ");
		...
		selectQueryBuffer.append(" AND cu.CONTROL_GROUP=? ");
		selectQueryBuffer.append(" AND ps.PROMOTION_TYPE=? ");
		selectQueryBuffer.append(" AND u.user_id=cu.user_id AND psv.set_id=pd.set_id AND date_trunc('day',psv.applicable_from::timestamp) < ? AND u.status in ('Y','S') AND p.product_code=pd.product_code ");
