...
public class DailyDetailsUserBalancePostgresQry implements DailyDetailsUserBalanceQry{
	@Override
	public String fetchUBSDataQry() {
         ...
         selectQueryBuffcatwise.append(" COALESCE(count(u.user_id),0)  persons , ((COALESCE(sum(udb.BALANCE),0))::integer/sp.default_value::integer)  BALANCE	");
	...
	public String fetchUBSDataSelectUserBalQry() {
		 final StringBuffer selectQueryBuffUserWise = new StringBuffer(
	                "SELECT u.category_code,u.user_id||','||u.user_name||','||(udb.balance)::integer/sp.default_value::integer  closingBal   ");
	            selectQueryBuffUserWise.append(" FROM users u,user_daily_balances udb , categories cat , system_preferences sp where u.USER_TYPE NOT IN(?,?) and udb.balance_date=?");
