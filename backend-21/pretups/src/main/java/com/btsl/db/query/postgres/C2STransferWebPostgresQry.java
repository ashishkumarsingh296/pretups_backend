...
public class C2STransferWebPostgresQry implements C2STransferWebQry{
	
	private Log LOG = LogFactory.getLog(C2STransferWebPostgresQry.class.getName());
	...
	public PreparedStatement loadC2STransferVOListQry(Connection con, String networkCode, Date fromDate, Date toDate, ArrayList userList, String receiverMsisdn, String transferID, String serviceType, String senderCat,
		...
		String ownerID=null;
           ...
           selectQueryBuff.append("FROM " + tbl_name + " CTRF left outer join key_values KV on (KV.key=CTRF.error_code AND KV.type= ?) left outer join key_values KV2 on (KV2.key=CTRF.transfer_status AND KV2.type=? ) , products PROD,service_type ST, users U ");
	...
	public PreparedStatement getReversalTransactionsQry(String msisdn,Connection con, String senderMsisdn,String txID,Date date,String time) throws SQLException{
        ...
        if (BTSLUtil.isNullString(msisdn)) {
            selectQueryBuff.append("WHERE transfer_date >= ? and sender_msisdn = ? and  transfer_id like ? and (transfer_date_time between (?::timestamp without time zone - interval '");
            selectQueryBuff.append(time);
            selectQueryBuff.append("' hour) and ?::timestamp without time zone) and service_type =? and reversal_id is null and transfer_status=? ");
            final String selectQuery = selectQueryBuff.toString();
