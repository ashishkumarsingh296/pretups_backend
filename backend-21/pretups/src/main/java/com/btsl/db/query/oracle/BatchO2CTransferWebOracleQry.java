...
public class BatchO2CTransferWebOracleQry implements BatchO2CTransferWebQry{
	...
	public String validateUsersForBatchO2CQry(String receiverStatusAllowed,
		...
		strBuff.append("WHERE status = 'Y' CONNECT BY PRIOR grph_domain_code = parent_grph_domain_code ");
		strBuff.append("START WITH grph_domain_code IN(" + geographicalDomainCode + ")) ");
		strBuff.append("AND CPSV.applicable_from =nvl ( (SELECT MAX(applicable_from) FROM ");
		strBuff.append("commission_profile_set_version WHERE applicable_from <= ? AND ");
		strBuff.append("comm_profile_set_id=CU.comm_profile_set_id),CPSV.applicable_from) ");
		strBuff.append("ORDER BY C.sequence_no,CU.user_grade,U.login_id");
	...
	public String closeOrderByBatchSelectBalanceQry() {
		StringBuilder sqlBuffer = new StringBuilder(" SELECT balance,balance_type FROM user_balances");
		sqlBuffer.append(" WHERE user_id = ? AND product_code = ? AND network_code = ? AND network_code_for = ? FOR UPDATE OF balance");
	    LogFactory.printLog("closeOrderByBatchSelectBalanceQry", sqlBuffer.toString(), log);
		return sqlBuffer.toString();
	}
	...
	public String closeOrderByBatchItemsDetailsQry() {
		StringBuilder sqlBuffer = new StringBuilder("SELECT ob.batch_total_record,SUM(DECODE(obi.status,?,1,0)) new,");
	        sqlBuffer.append(" SUM(DECODE(obi.status,?,1,0)) appr1,SUM(DECODE(obi.status,?,1,0)) cncl, ");
	        sqlBuffer.append(" SUM(DECODE(obi.status,?,1,0)) appr2,SUM(DECODE(obi.status,?,1,0)) closed ");
