...
/**
 * HandleUnsettledCasesPostgresQry
 * @author sadhan.k
 *
 */
public class HandleUnsettledCasePostgresQry implements HandleUnsettledCasesQry{
	...
	public String loadC2STransferVO() {
        ...
        selectQueryBuff.append("FROM c2s_transfers CTRF left join key_values KV on (KV.key=CTRF.error_code AND KV.type=?) ");
        selectQueryBuff.append("left join key_values KV1 on (KV1.key=CTRF.transfer_status AND KV1.type=?), ");
        selectQueryBuff.append("products PROD,service_type ST,users U,user_phones UP  ");
        selectQueryBuff.append("WHERE CTRF.transfer_id=? AND CTRF.transfer_date=? AND U.user_id = UP.user_id AND UP.primary_number='Y' AND U.user_id = CTRF.sender_id ");        
