...
/**
 * This class provides postgres compatible query
 * @author lalit.chattar
 *
 */
public class LoyaltyDAOPostgresQry implements LoyaltyDAOQry {
	...
	public String loadLMSProfileAndVersion(String lmsProfileServiceType) {
        ...
        selectQueryBuild.append(" WHERE (applicable_from=(SELECT MAX(applicable_from) FROM PROFILE_SET_VERSION WHERE applicable_from<=current_timestamp and psv.applicable_to>=current_timestamp ");
	...
	public String checkUserAlreadyExist(String actionType) {
         ...
         if (BTSLUtil.isNullString(actionType)) {
             qryBuilder.append(" AND product_code=? AND points_date=? AND bucket_code=? FOR UPDATE");
             ...
             qryBuilder.append(" AND product_code=? AND points_date=? FOR UPDATE ");
         }
