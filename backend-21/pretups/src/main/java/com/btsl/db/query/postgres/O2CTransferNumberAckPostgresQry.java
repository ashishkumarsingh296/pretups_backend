 ...
 *
 */
public class O2CTransferNumberAckPostgresQry implements O2CTransfernumberAckRptQry {
	...
	public PreparedStatement loado2cTransfeAskChannelUserReportQry(
		...
		strBuffer.append(" FROM channel_transfers CT ");
		strBuffer.append(" left join transfer_profile TP on CT.receiver_txn_profile = TP.profile_id ");
		strBuffer.append(" left join  transfer_profile TP2 on CT.sender_txn_profile = TP2.profile_id ");
		strBuffer.append(" left join  users APPU1 on CT.first_approved_by = APPU1.user_id ");
		strBuffer.append(" left join  users APPU2 on  CT.second_approved_by = APPU2.user_id ");
		strBuffer.append(" left join  users APPU3 on CT.third_approved_by = APPU3.user_id ");
		strBuffer.append(" left join  users APPU5  on CT.transfer_initiated_by = APPU5.user_id ");
		strBuffer.append(" left join  users APPU6 on CT.modified_by = APPU6.user_id ");
		strBuffer.append(" left join lookups L on CT.transfer_category = L.lookup_code and L.lookup_type = 'TRFTY' ");
		strBuffer.append(" left join lookups L1 on CT.status = L1.lookup_code  AND L1.lookup_type = 'TSTAT' ");
		strBuffer.append(" left join lookups L2 on CT.transfer_sub_type =  L2.lookup_code AND L2.lookup_type = 'TRFT' ");
		strBuffer.append(" left join  lookups L3 on CT.pmt_inst_type =  L3.lookup_code   AND L3.lookup_type = 'PMTYP' ");
		strBuffer.append(" , geographical_domains G, categories C ,  categories C1 ,users U , commission_profile_set CPS, domains D ");
		strBuffer.append(" , channel_transfers_items CTI, products P ");
		...
		strBuffer.append(" AND (case CT.from_user_id when 'OPT' then  CT.to_user_id end = U.user_id  OR ");
		strBuffer.append(" case CT.to_user_id when 'OPT' then  CT.from_user_id end = U.user_id) ");
