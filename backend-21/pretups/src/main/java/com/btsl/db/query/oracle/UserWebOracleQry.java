...
public class UserWebOracleQry implements UserWebQry{
	public static  final Log _log = LogFactory.getLog(UserWebOracleQry.class);
	@Override
	public String loadUsersListQry( String p_networkCode, String pcategoryCode, String puserName, String puserID, String pownerID, String psessionUserID, String pstatusUsed, String pstatus) {
        ...
        strBuff.append("USR.owner_id,USR.allowed_ip,USR.allowed_days,USR.payment_type,USR.contact_person,");
        ...
        strBuff.append("categories ONR_CAT, categories  PRNT_CAT,lookups l ");
        if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.IS_MSISDN_ASSOCIATION_REQ))).booleanValue()) {
            strBuff.append(" ,channel_users chnl_usr ");
        }
        strBuff.append("WHERE USR.network_code = ? ");

        if (pstatusUsed.equals(PretupsI.STATUS_IN)) {
            strBuff.append("AND USR.status IN (" + pstatus + ")");
        } else if (pstatusUsed.equals(PretupsI.STATUS_NOTIN)) {
            strBuff.append("AND USR.status NOT IN (" + pstatus + ")");
        } else if (pstatusUsed.equals(PretupsI.STATUS_EQUAL)) {
            strBuff.append("AND USR.status =? ");
        } else if (pstatusUsed.equals(PretupsI.STATUS_NOTEQUAL)) {
            strBuff.append("AND USR.status <> ? ");
        }

        if (!BTSLUtil.isNullString(pcategoryCode)) {
            strBuff.append(" AND USR.category_code =?");
        }
        if (!BTSLUtil.isNullString(puserID)) {
            strBuff.append(" AND USR.user_id =?");
        }
        if (!BTSLUtil.isNullString(pownerID)) {
	...
	public PreparedStatement loadReportOwnerUserListQry(Connection p_con, String p_parentGraphDomainCode, String p_userID, String p_username, String p_domainCode, String p_networkCode) throws SQLException{
	        ...
	        if (_log.isDebugEnabled()) {
	            ...
	            pstmt.setString(3, PretupsI.GEO_DOMAIN_STATUS);
	            pstmt.setString(4, p_parentGraphDomainCode);
	            pstmt.setString(5, p_parentGraphDomainCode);
	            pstmt.setString(6, p_userID);
	            // commented for DB2pstmt.setFormOfUse(6,
	...
	public PreparedStatement loadApprovalUsersListQry(Connection p_con,
        ...
        strBuff.append("FROM users u,users u1,users u2,categories c,lookups l,users OUSR,users PUSR,user_phones UP WHERE ");// UP
        // added
        // by
        // deepika
        strBuff.append(" u.category_code = ? AND u.category_code = c.category_code ");
        strBuff.append(" AND OUSR.user_id = u.owner_id ");
        strBuff.append(" AND PUSR.user_id(+) = u.parent_id ");
        ...
        strBuff.append("FROM geographical_domains WHERE grph_domain_type= ? ");
        ...
        pstmt.setString(5, p_grphDomainType);
        pstmt.setString(6, p_networkCode);
        pstmt.setString(7, p_parentGrphDomainCode);
        return pstmt;
	}


	public PreparedStatement loadApprovalUsersListQry1(Connection p_con,
			String p_categoryCode, String p_lookupType, int p_sequenceNo,
			String p_grphDomainType, String p_networkCode,
			String p_parentGrphDomainCode, String p_status, String p_userType)
			throws SQLException {
	...
	public PreparedStatement isUserInSameGRPHDomainQry(Connection p_con, String p_userId, String p_sessionUserId,String p_userGrphDomainType, String p_sessionUserGrphDomainType) throws SQLException{
		 ...
		 PreparedStatement pstmt = null;
	        ...
	        if (p_userGrphDomainType.equals(p_sessionUserGrphDomainType)) {
	            strBuff.append(" SELECT 1 from geographical_domains ");
	            strBuff.append("WHERE status <> 'N' AND grph_domain_code in (SELECT grph_domain_code from user_geographies where user_id = ?) ");
	            strBuff.append("start with  grph_domain_code  in (SELECT grph_domain_code from user_geographies where user_id = ?) ");
	            strBuff.append("connect by  prior grph_domain_code = parent_grph_domain_code");
	        } else {
	            strBuff.append(" SELECT 1 from geographical_domains ");
	            strBuff.append("WHERE status <> 'N' AND grph_domain_code in (SELECT grph_domain_code from user_geographies where user_id = ?) ");
	            strBuff.append("start with  parent_grph_domain_code  in (SELECT grph_domain_code from user_geographies where user_id = ?) ");
	            strBuff.append("connect by  prior grph_domain_code = parent_grph_domain_code");
	        }
            ...
            pstmt.setString(2, p_sessionUserId);
		return pstmt;
	}
	...
	public String loadUsersListForUserTypeQry( String p_statusUsed,String p_status, boolean p_isChannelUser)  {
        ...
        if (p_isChannelUser) {
            strBuff.append(" ,(SELECT user_id FROM USERS CONNECT BY PRIOR user_id = parent_id START WITH user_id= ? ) X ");
        }
	...
	public PreparedStatement loadUserListOnZoneCategoryHierarchyQry(Connection p_con, String p_userCategory, String p_zoneCode, String p_userName, String p_loginuserID, String domainCode) throws SQLException{
		PreparedStatement pstmtSelect = null;
		  final StringBuilder strBuff = new StringBuilder(" SELECT  U.user_id, U.user_name,U.login_id ");
	...
	public PreparedStatement loadSTKApprovalUsersListQry(Connection p_con,
        ...
        strBuff.append("u.level2_approved_by,u.level2_approved_on,u.user_code,u.appointment_date,");
        strBuff.append("u.previous_status,l.lookup_name,u1.user_name parent_user_name,u2.user_name request_user_name ");
        strBuff.append("FROM users u,users u1,users u2,categories c,lookups l,users OUSR,users PUSR WHERE ");
        strBuff.append(" u.category_code = ? AND u.category_code = c.category_code ");
        strBuff.append(" AND OUSR.user_id = u.owner_id ");
        strBuff.append(" AND PUSR.user_id(+) = u.parent_id ");
        strBuff.append(" AND u1.user_id = u.created_by ");
	...
	public String checkBarLimitQry(){
		final StringBuilder strBuff = new StringBuilder();
        strBuff.append("SELECT COUNT(*) LIMIT FROM user_event_remarks WHERE trunc(created_on) = ? AND event_type = ?");
        return strBuff.toString();
	}
