 ...
 *
 */
public class StaffC2CTransferDetailsPostgresQry implements
		StaffC2CTransferdetailsRptQry {

	public static final Log log = LogFactory
			.getLog(StaffC2CTransferDetailsPostgresQry.class.getName());
	...
	public PreparedStatement loadstaffc2cTransferDetailsChannelUserReportQry(
		...
		strBuff.append(" SELECT CTRF.ACTIVE_USER_ID, CTRF.to_user_id,  ");
		strBuff.append("(U.user_name ) from_user,(U2.user_name || '(' ||CTRF.to_msisdn||')') to_user,CTRF.transfer_id,");
		strBuff.append(" L.lookup_name transfer_sub_type,  ");
		strBuff.append(" CTRF.TYPE, TO_CHAR(CTRF.close_date, ? ) close_date,");
		strBuff.append(" P.product_name, SUM(CTI.required_quantity) transfer_mrp, ");
		strBuff.append(" SUM(CTI.payable_amount) payable_amount, ");
		strBuff.append(" SUM(CTI.net_payable_amount) net_payable_amount,  ");
		strBuff.append(" L1.lookup_name status, ");
		strBuff.append(" SUM(CTI.mrp) mrp,  ");
		strBuff.append(" SUM(CTI.commission_value) commision, ");
		strBuff.append(" SUM(CTI.commision_quantity) commision_quantity,SUM(CTI.receiver_credit_quantity) receiver_credit_quantity, ");
		strBuff.append(" SUM(CTI.sender_debit_quantity) sender_debit_quantity, ");
		strBuff.append(" SUM(CTI.tax3_value)tax3_value,SUM(CTI.tax1_value)tax1_value, ");
		strBuff.append(" SUM(CTI.tax2_value)tax2_value, CTRF.sender_category_code, CTRF.receiver_category_code, ");
		strBuff.append(" SEND_CAT.category_name sender_category_name,  ");
		strBuff.append(" REC_CAT.category_name receiver_category_name,CTRF.SOURCE ");
		strBuff.append(" FROM ");
		strBuff.append(" CHANNEL_TRANSFERS CTRF, CHANNEL_TRANSFERS_ITEMS CTI,USERS U, ");
		strBuff.append(" USERS U2,PRODUCTS P,LOOKUPS L, LOOKUPS L1, CATEGORIES SEND_CAT, ");
		...
		strBuff.append(" AND CTRF.control_transfer<>'A' AND CTRF.sender_category_code");
		strBuff.append(" = SEND_CAT.category_code ");
		...
		strBuff.append(" AND CTRF.ACTIVE_USER_ID=CASE ?   ");
		strBuff.append(" WHEN 'ALL' THEN ACTIVE_USER_ID ELSE ? END ");
		...
		strBuff.append(" AND CTRF.transfer_id = CTI.transfer_id AND CTI.product_code = P.product_code  AND L.lookup_type ='TRFT' ");
		strBuff.append(" AND L.lookup_code = CTRF.transfer_sub_type  AND CTRF.status = 'CLOSE'  AND L1.lookup_code = CTRF.status ");
		strBuff.append(" AND L1.lookup_type = 'CTSTA' GROUP BY CTRF.ACTIVE_USER_ID, CTRF.to_user_id, ");
		strBuff.append(" U.user_name, CTRF.msisdn,U2.user_name, CTRF.to_msisdn ,CTRF.transfer_id,  L.lookup_name , CTRF.TYPE,CTRF.close_date,P.product_name, L1.lookup_name,");
		strBuff.append(" CTRF.sender_category_code, CTRF.receiver_category_code,SEND_CAT.category_name , ");
		strBuff.append("  REC_CAT.category_name,CTRF.SOURCE ");
		strBuff.append(" UNION ");
		strBuff.append(" SELECT CTRF.ACTIVE_USER_ID, CTRF.to_user_id, ");
		strBuff.append(" (U.user_name ) from_user,(U2.user_name || '(' ||CTRF.to_msisdn||')') to_user,CTRF.transfer_id, ");
		strBuff.append(" L.lookup_name transfer_sub_type, CTRF.TYPE, TO_CHAR(CTRF.close_date, ? ) close_date, P.product_name, ");
		strBuff.append("  SUM(CTI.required_quantity) transfer_mrp, SUM(CTI.payable_amount) payable_amount, ");
		strBuff.append(" SUM(CTI.net_payable_amount) net_payable_amount, L1.lookup_name status, SUM(CTI.mrp) mrp,  ");
		strBuff.append(" SUM(CTI.commission_value) commision, ");
		...
		strBuff.append(" SUM(CTI.tax1_value)tax1_value,SUM(CTI.tax2_value)tax2_value, CTRF.sender_category_code, CTRF.receiver_category_code, ");
		strBuff.append(" SEND_CAT.category_name sender_category_name, REC_CAT.category_name receiver_category_name,CTRF.SOURCE FROM  ");
		strBuff.append(" CHANNEL_TRANSFERS CTRF, CHANNEL_TRANSFERS_ITEMS CTI,USERS U,USERS U2,PRODUCTS P,LOOKUPS L, LOOKUPS L1, CATEGORIES SEND_CAT, CATEGORIES REC_CAT  WHERE CTRF.TYPE = 'C2C' ");
		...
		strBuff.append(" AND CTRF.ACTIVE_USER_ID=CASE ?  WHEN 'ALL' THEN ACTIVE_USER_ID ELSE ? END  AND CTRF.transfer_sub_type = CASE ? WHEN 'ALL' THEN CTRF.transfer_sub_type ELSE ?  END");
		...
		strBuff.append("  CTRF.TYPE,CTRF.close_date,P.product_name, L1.lookup_name, CTRF.sender_category_code,");
		...
		try {
			...
			pstmt.setString(i++, usersReportModel.getTransferUserCategoryCode());
			...
			pstmt.setString(i++, usersReportModel.getDomainCode());
			pstmt.setString(i++, usersReportModel.getTransferUserCategoryCode());
			pstmt.setString(i++, usersReportModel.getParentUserID());
			pstmt.setString(i++, usersReportModel.getUserID());
			pstmt.setString(i++, usersReportModel.getUserID());
			pstmt.setString(i++, usersReportModel.getTxnSubType());
			pstmt.setString(i, usersReportModel.getTxnSubType());
