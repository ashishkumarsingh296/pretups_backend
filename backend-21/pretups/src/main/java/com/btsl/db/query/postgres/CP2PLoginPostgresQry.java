...
public class CP2PLoginPostgresQry  implements CP2PLoginQry{
	private String calssName = "CP2PLoginPostgresQry";

	@Override
	public String loadUserDetailsByMsisdnOrLoginIdQry(String msisdn, String loginId)
			 {
		String methodName = calssName+"#loadUserDetailsByMsisdnOrLoginIdQry";
		  StringBuilder sqlBuffer = new StringBuilder(" SELECT uowner.user_name owner_name, uparent.user_name parent_name, u.user_id, u.user_name, u.network_code,l.network_name,l.report_header_name, u.login_id, u.password, ");
	        ...
	        sqlBuffer.append(" FROM users u left outer join users uparent on u.parent_id=uparent.user_id left outer join networks l on U.network_code=L.network_code ");
	        sqlBuffer.append(" left outer join channel_users cusers on u.user_id=cusers.user_id left outer join user_phones up on ( u.msisdn = up.msisdn AND u.user_id=up.user_id )  " );
	...
	public String loadCP2PSubscriberDetailsQry(String servicetype) {
		String methodName = calssName+"#loadCP2PSubscriberDetailsQry";
        ...
        sqlBuffer.append(" FROM  P2P_SUBSCRIBERS p2ps");
        if(servicetype!=null){
        	sqlBuffer.append(" left outer join  P2P_SUBSCRIBERS_COUNTERS p2psc on p2ps.msisdn=p2psc.msisdn ");
        }
        sqlBuffer.append("  left outer join NETWORKS l on p2ps.network_code=L.network_code ");
        sqlBuffer.append(" WHERE  UPPER(login_id)=UPPER(?) ");
        if(servicetype!=null){
        	sqlBuffer.append("  AND p2psc.service_type=?");
        }
        sqlBuffer.append(" AND p2ps.status <> ? AND p2ps.status <> ?  ");
        LogFactory.printLog(methodName, sqlBuffer.toString(), LOG);
        return sqlBuffer.toString();
	}
