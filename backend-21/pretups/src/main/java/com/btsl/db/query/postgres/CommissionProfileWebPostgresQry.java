...
import com.btsl.db.query.oracle.CommissionProfileWebOracleQry;
...
public class CommissionProfileWebPostgresQry implements CommissionProfileWebQry{
	...
	public String loadCommissionProfileListQry() {
        ...
        strBuff.append(" AND cps.category_code = (CASE WHEN ?='ALL'then cps.category_code ELSE ? END)");
        strBuff.append(" AND cps.category_code = c.category_code");
        strBuff.append(" AND cps.comm_profile_set_id = cpsv.comm_profile_set_id");
        strBuff.append(" AND date_trunc('day',applicable_from::TIMESTAMP) between ? AND ?");
	...
	public String loadCommissionProfileSetVersionQry1(String geoCode, String gradeCode ,String status) {
	    ...
	    final StringBuilder strBuff = new StringBuilder();
        strBuff.append(" WITH temp AS (SELECT cv.comm_profile_set_id,cv.comm_profile_set_version,cv.dual_comm_type,cv.applicable_from,cs.geography_code,cs.comm_profile_set_name,cs.status,cs.created_on,cs.is_default,cs.status,c.services_allowed,c.category_name ");
		...
		strBuff.append(
				" JOIN (SELECT comm_profile_set_id,comm_profile_set_version,ROW_NUMBER() OVER (PARTITION BY comm_profile_set_id ORDER BY CASE WHEN MAX(applicable_from) <= ? THEN -extract(epoch from applicable_from) ELSE extract(epoch from applicable_from) END) AS rank ");
		strBuff.append(
