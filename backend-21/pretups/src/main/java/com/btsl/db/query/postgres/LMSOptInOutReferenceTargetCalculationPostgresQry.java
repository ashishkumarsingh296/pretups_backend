...
public class LMSOptInOutReferenceTargetCalculationPostgresQry implements LMSOptInOutReferenceTargetCalculationQry{
	...
	public String loadTargetProfileQry() {
		   final StringBuilder selectQueryBuffer = new StringBuilder();
	        ...
	        selectQueryBuffer.append(" AND u.user_id=cu.user_id AND psv.set_id=pd.set_id AND psv.applicable_from >=DATE_TRUNC('day',CURRENT_TIMESTAMP::TIMESTAMP) AND u.status=?  AND p.product_code=pd.product_code AND "); 
	        selectQueryBuffer.append(" psv.VERSION IN (SELECT psv.VERSION FROM PROFILE_SET_VERSION psv ");
	        selectQueryBuffer.append(" WHERE (applicable_from<=(SELECT MAX(applicable_from) FROM PROFILE_SET_VERSION WHERE applicable_from>=DATE_TRUNC('day',CURRENT_TIMESTAMP::TIMESTAMP)  AND DATE_TRUNC('day',applicable_to::timestamp)>=DATE_TRUNC('day',CURRENT_TIMESTAMP::TIMESTAMP) AND set_id=cu.lms_profile) ) ");
