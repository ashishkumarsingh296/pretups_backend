...
public class CryptoUtil {
    ...
    public static final Log _log = LogFactory.getLog(CryptoUtil.class.getName());
    ...
    protected int binHexToBytes(String sBinHex, byte[] data, int nSrcPos, int nNumOfBytes) {
        ...
        final int nOutputCapacity = data.length;
    ...
    public String decrypt(String cipherText, String myKeyString) throws Exception348, GeneralSecurityException {
        ...
        final byte[] cipherBytes = new byte[cipherText.length() / 2];
        ...
        final SecretKey desEdeKey = new SecretKeySpec(keyBytes, Algorithm);
        Cipher desEdeCipher;
        // Create the cipher
        desEdeCipher = Cipher.getInstance(CipherParameters);
        final IvParameterSpec ivSpec1 = new IvParameterSpec(iv);
    ...
    public ArrayList decryptList(ArrayList cipherList, String myKeyString) throws GeneralSecurityException {
        String keyString = myKeyString.toLowerCase();
        int   cipherListSize = cipherList.size(); 
        final byte[] keyBytes = new byte[DESedeKeyLength];

        // if keyString is 16 bytes make it 24
        if ((keyString.length() / 2 != 16) && (keyString.length() / 2 != 24)) {
        	_log.error("decryptList", "Wrong key Size (" + keyString.length() + "): " + "Should be a 32 or 48 characters string");
        } else if (keyString.length() / 2 == 16) {
            keyString = keyString.concat(keyString.substring(0, 16));
        }

        if (binHexToBytes(keyString, keyBytes, 0, DESedeKeyLength) != (DESedeKeyLength)) {
        	_log.error("decryptList", "Error while converting keyStrings to byte[] ");
        }
        ...
        final ArrayList plainList = new ArrayList();
        final byte[] cipherBytes = new byte[PlainTextLength];
        byte[] plainBytes = new byte[cipherBytes.length];

        for (int j = 0; j < cipherListSize; j++) {
            cipherText = (String) cipherList.get(j);
            if (binHexToBytes(cipherText.toLowerCase(), cipherBytes, 0, cipherText.length() / 2) != (cipherText.length() / 2)) {
            	_log.error("decryptList", "Error while converting cipherText to byte[] ");
            }
    ...
    public ArrayList encryptList(ArrayList plainList, String myKeyString) throws GeneralSecurityException {
        String keyString = myKeyString.toLowerCase();
        ...
        // if keyString is 16 bytes make it 24
        if ((keyString.length() / 2 != 16) && (keyString.length() / 2 != 24)) {
        	_log.error("encryptList", "Wrong key Size (" + keyString.length() + "): " + "Should be a 32 or 48 characters string");
        } else if (keyString.length() / 2 == 16) {
            keyString = keyString.concat(keyString.substring(0, 16));
        }
        if (binHexToBytes(keyString, keyBytes, 0, DESedeKeyLength) != (DESedeKeyLength)) {
        	_log.error("encryptList", "Error while converting keyStrings to byte[] ");
        }

        String plainText = null;
        String cipherText = null;
        final ArrayList cipherList = new ArrayList();
   ...
   /* public static void main(String args[]){
    	try{
        final String keyString = "202122232425262728292A2B2C2D2E2F";
        ...
        final ArrayList plainList = crypto.decryptList(list2, keyString);

        for (int i = 0; i < 100; i++) {
            _log.debug("main", " " + plainList.get(i));
        }
