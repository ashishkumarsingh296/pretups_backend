...
/**
 * Based on OWASP CryptoUtil.java and CSRFGuard.java written by Eric Sheriden
 * for OWASP
 * 
...
public final class CSRFTokenUtil {
    private final static String DEFAULT_PRNG = "SHA1PRNG"; // algorithm to
                                                           // generate key
     ...
     * 
     * @param session
     *            HTTP Session for user - must be valid
     * @return token for the session, a new one is created if it doesn't already
     *         exist
     * @throws ServletException
     *             if session is null
     * @throws NoSuchAlgorithmException
     *             if random number generator algorithm doesn't exist
     */
     ...
     * 
     * @param request
     *            Inbound HttpRequest that you wish to check if it has a valid
     *            anti-CSRF token
     * @return true if the parameter value matches the token in the session,
     *         false otherwise
     * @throws ServletException
