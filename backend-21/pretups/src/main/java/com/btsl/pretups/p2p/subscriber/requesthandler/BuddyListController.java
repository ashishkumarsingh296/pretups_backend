        ...
        try {
            ...
            case (PretupsI.MESSAGE_LENGTH_BUDDYLIST - 1): {
                if ((((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.CP2P_PIN_VALIDATION_REQUIRED)).booleanValue()) && !(BTSLUtil.encryptText((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_DEFAULT_SMSPIN)).equals(actualPin))) {
                    throw new BTSLBaseException(this, "process", PretupsErrorCodesI.P2P_ERROR_BUDDYLIST_INVALIDMESSAGEFORMAT, 0, new String[] { p_requestVO
            ...
            final ArrayList list = p2PBuddiesDAO.loadBuddyList(con, senderVO.getUserID());
            if (_log.isDebugEnabled()) {
            ...
            if (list != null && !list.isEmpty()) {
                ...
                final String argsArr[] = { SubscriberBL.loadBuddyListForSMS(list) };
                p_requestVO.setMessageArguments(argsArr);
                p_requestVO.setMessageCode(PretupsErrorCodesI.BUDDY_LIST_SUCCESS);
            } else {
                p_requestVO.setMessageCode(PretupsErrorCodesI.BUDDY_LIST_NOTFOUND);
            }
        } catch (BTSLBaseException be) {
            ...
            if (be.isKey()) {
                ...
                p_requestVO.setMessageCode(PretupsErrorCodesI.BUDDY_LIST_ERROR);
            }
        } catch (Exception e) {
            p_requestVO.setSuccessTxn(false);
            try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "BuddyListController[process]", ((SenderVO) p_requestVO
                .getSenderVO()).getMsisdn(), "", "", "Exception while getting buddy List" + e.getMessage());
            p_requestVO.setMessageCode(PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
			if (mcomCon != null) {
				mcomCon.close("BuddyListController#process");
				mcomCon = null;
			}
            if (_log.isDebugEnabled()) {
