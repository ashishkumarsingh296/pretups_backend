...
public class SMCDListViewController implements ServiceKeywordControllerI {
    ...
    public void process(RequestVO p_requestVO) {
        ...
        try {
            ...
            if (BTSLUtil.isNullString(_listName) && BTSLUtil.isNullString(_sctype)) {
                throw new BTSLBaseException(this, "process", PretupsErrorCodesI.P2P_ERROR_SMCD_LIST_VIEW_SCTYPE_LISTNAME_REQUIRED);
            }
            ...
            try {
                ...
                mcdtxnDAO.checkScheduleListStatus(con, p_requestVO, _senderVO.getUserID(), _listName, _sctype);
                _buddyList = mcdtxnDAO.loadScheduleBuddyList(con, _senderVO.getUserID(), p_requestVO.getMcdListName(), _sctype);
            ...
            if (p_requestVO.isSuccessTxn()) {
                ...
                if (!BTSLUtil.isNullString(_finalMessage)) {
                    ...
                    // (new
                    // PushMessage(_senderMsisdn,getSenderSuccessMessage(_senderLocale,_listName),"",_gatewayCode,_senderLocale)).push();
                }
            } else if (key != null) {
                // (new
                // PushMessage(_senderMsisdn,getSenderMessage(_senderLocale,_listName,key),"",_gatewayCode,_senderLocale)).push();
            } else {
                // (new
                // PushMessage(_senderMsisdn,getSenderFailMessage(_senderLocale,_listName),"",_gatewayCode,_senderLocale)).push();
            }
            ...
            if (con != null) {
				...
				if (mcomCon != null) {
					mcomCon.close("SMCDListViewController#process");
					mcomCon = null;
				}
                con = null;
            }
    ...
    public void prepareFinalMessage(ArrayList p_arrayList) {
        ...
        for (int i = 0; i < arrayListSizes; i++) {
            ...
            mcdMap.put(mcdVO.getMsisdn(), mcdVO.getMsisdn() + ":" + mcdVO.getAmount1());
    ...
    public void validateSenderPin(Connection p_con, RequestVO p_requestVO) throws BTSLBaseException {
        ...
        if (!BTSLUtil.isNullString(pin)) {
            ...
            throw new BTSLBaseException("SMCDListViewController", "validateSenderPin", PretupsErrorCodesI.P2P_MCDL_PIN_REQUIRED);
        }
