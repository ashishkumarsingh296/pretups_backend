...
public class C2STransferOracleQry implements C2STransferQry {
	...
	public PreparedStatement loadC2STransferVOListQry(Connection pCon,
		...
		selectQueryBuff
				.append("FROM "
						+ tbl_name
						+ " CTRF, products PROD,service_type ST,users U,key_values KV,key_values KV2 ,networks NTWK ");
		selectQueryBuff
				.append("WHERE CTRF.transfer_date >=? AND CTRF.transfer_date < ? AND CTRF.network_code=? AND U.user_id = CTRF.sender_id AND KV.key(+)=CTRF.error_code AND KV.type(+)=? ");
		selectQueryBuff
				.append("AND KV2.key(+)=CTRF.transfer_status AND KV2.type(+)=? ");
		selectQueryBuff
				.append("AND CTRF.service_type=decode(?,'ALL',CTRF.service_type,?) ");
		selectQueryBuff
	...
	public String updateReconcilationStatusQry(C2STransferVO p_c2sTransferVO) {
		...
		updateQuery.append(", error_code = nvl(error_code,"
				+ PretupsErrorCodesI.TXN_STATUS_UNDER_PROCESS + ")");
	...
	public PreparedStatement loadC2STransferItemsVOList_oldQry(
		...
		selectQueryBuff
				.append("FROM c2s_transfer_items,key_values KV,key_values KV2,key_values KV3  ");
		selectQueryBuff
				.append("WHERE transfer_id=? AND KV.key(+)=transfer_status AND KV.type(+)=?  ");
		selectQueryBuff
				.append("AND KV2.key(+)=transfer_type AND KV2.type(+)=?  ");
		selectQueryBuff
				.append("AND KV3.key(+)=interface_response_code AND KV3.type(+)=?  ");
	...
	public PreparedStatement loadC2STransferItemsVOListQry(Connection p_con,
		...
		selectQueryBuff
				.append( " where ct.transfer_id = adj.reference_id and adj.ADJUSTMENT_ID like '%U' ) differntial "); //trasha");
		selectQueryBuff
				.append(" FROM c2s_transfers ct ,key_values KV,key_values KV2,key_values KV3  ");
		selectQueryBuff
				.append("WHERE transfer_date=? AND transfer_id=? AND KV.key(+)=transfer_status AND KV.type(+)=?  ");
		selectQueryBuff
				.append("AND KV2.key(+)=transfer_type AND KV2.type(+)=?  ");
		selectQueryBuff
				.append("AND KV3.key(+)=interface_response_code AND KV3.type(+)=?  ");
	...
	public PreparedStatement loadC2STransferVOListQry(Connection p_con, String p_networkCode, Date p_fromDate, Date p_toDate, ArrayList userList, String p_receiverMsisdn, String p_transferID, String p_serviceType, String senderCat,ListValueVO user ) throws SQLException {
        ...
        selectQueryBuff.append("FROM key_values KV,key_values KV2, products PROD,service_type ST,users U, " + tbl_name + " CTRF ");
	...
	public String getChanneltransAmtDatewiseQry(Date p_fromDate, Date p_toDate,String p_amount) {
        ...
        selectQueryBuff.append("FROM " + tbl_name + " CTRF,SERVICE_TYPE ST,KEY_VALUES KV1 ");
	...
	public PreparedStatement getReversalTransactionsQry(String msisdn,Connection con, String senderMsisdn,String txID,Date date,String time) throws SQLException{
        ...
        if (BTSLUtil.isNullString(msisdn)) {
            selectQueryBuff
                .append("WHERE sender_msisdn = ? and  transfer_id like ? and (transfer_date_time between (?-(?/24)) and ?) and service_type =? and reversal_id is null and transfer_status=? ");
            final String selectQuery = selectQueryBuff.toString();
            pstmtSelect = con.prepareStatement(selectQuery);
            pstmtSelect.setString(1, senderMsisdn);
            pstmtSelect.setString(2, "%" + txID);
            pstmtSelect.setTimestamp(3, BTSLUtil.getTimestampFromUtilDate(date));
            pstmtSelect.setString(4, time);
            ...
            selectQueryBuff
                .append("WHERE sender_msisdn = ? and  receiver_msisdn= ? and (transfer_date_time between (?-(?/24)) and ?) and service_type=? and reversal_id is null and transfer_status=?");
            final String selectQuery = selectQueryBuff.toString();
            if (LOG.isDebugEnabled()) {
                LOG.debug(methodName, "select query:" + selectQuery);
            }
            pstmtSelect = con.prepareStatement(selectQuery);
            pstmtSelect.setString(1, senderMsisdn);
            pstmtSelect.setString(2, msisdn);
            pstmtSelect.setTimestamp(3, BTSLUtil.getTimestampFromUtilDate(date));
            pstmtSelect.setString(4, time);
            pstmtSelect.setTimestamp(5, BTSLUtil.getTimestampFromUtilDate(date));
            pstmtSelect.setString(6, PretupsI.SERVICE_TYPE_CHNL_RECHARGE);
	...
	public PreparedStatement loadOldTxnIDForReversalQry(Connection con,C2STransferVO c2sTransferVO, RequestVO requestVO, String serviceType,Date date,String time) throws SQLException, ParseException{
         ...
         if (PretupsI.SERVICE_TYPE_C2S_PREPAID_REVERSAL.equals(requestVO.getServiceType())) {
             selectQueryBuff.append(" and (transfer_date_time between (?-(?/24)) and ?) ");
         } else {
	...
	public String insertTPSDetailsQry(Map<Date,Integer> pTpsMap) {
		...
		StringBuilder strBuff = new StringBuilder("INSERT ALL ");
			for(Map.Entry<Date,Integer> tpsPair : pTpsMap.entrySet())
			{
				
				strBuff.append(" INTO TPS_DETAILS(TPS_DATE_TIME , INSTANCE_CODE , TPS ,TPS_DATE) VALUES (?,?,?,?) ");
			}
            	strBuff.append(" SELECT * FROM dual  ");
