 ...
 *
 */
public class DownloadCSVReportsStaff {

	public static final Log log = LogFactory.getLog(DownloadCSVReportsStaff.class.getName());
	private static String path = Constants.getProperty("REPORTS_DOWNLOAD_PATH");
	private static String c2cTransferDetails = Constants.getProperty("O2C_TRANSFER_DETAILS_REPORTS_DOWNLOAD_FILE_NAME");
	private StaffC2CTransferdetailsRptQry staffSelfC2CQuery;
	/**
	 * DownloadCSVReportsNew() constructor
	 */
	public DownloadCSVReportsStaff() {
		
		staffSelfC2CQuery=(StaffC2CTransferdetailsRptQry) ObjectProducer.getObject(QueryConstants.STAFFC2C_TRANSFER_DETAILS_REPORT_QRY,QueryConstants.QUERY_PRODUCER);
	...
	public String prepareDatastaffSelfC2CReport(
		...
		try {
			...
			/*if (TypesI.CHANNEL_USER_TYPE.equalsIgnoreCase(usersReportModel
					.getUserType())) {*/
				pstmt = staffSelfC2CQuery.loadstaffc2cTransferDetailsDailyReportQry(usersReportModel, con);
				...
				sb.setLength(0);
				final String columnHeader = sb.append(PretupsRestUtil
						.getMessageString("pretups.c2s.query.staff.label.activeuserid")).append(",")
