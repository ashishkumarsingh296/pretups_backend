...
public class O2CTransferApprovalController implements ServiceKeywordControllerI {
    private static Log _log = LogFactory.getLog(O2CTransferApprovalController.class.getName());
    ...
    static {
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "O2CTransferApprovalController", "", "", "",
	...
	public void process(RequestVO p_requestVO) {
        ...
        try 
        {
            ...
            //channelTrfDAO.loadChannelTransfersVO(con, channelTransferVO); // new method need to write with transfer_id, transfer_date & PENDING status
            channelTrfDAO.loadChannelTransferDetail(con, channelTransferVO,PretupsI.CHANNEL_TRANSFER_ORDER_PENDING);
            channelTransferVO.setChannelTransferitemsVOList(channelTrfDAO.loadChannelTransferItems(con, channelTransferVO.getTransferID()));
            ...
            if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.PAYMENT_VERIFICATION_ALLOWED))).booleanValue()  && pgStatus.equalsIgnoreCase(PretupsI.SUCCESS)) {
            
            	Map validateRefmap=_operatorUtilI.validatePaymentRefId(channelTransferVO);
            	pgStatus=validateRefmap.get("STATUS").toString();
            	try
            	{
        	...
        	if("SUCCESS".equals(pgStatus)){
            	...
            	if(PretupsI.CHANNEL_TRANSFER_SUB_TYPE_TRANSFER.equals(channelTransferVO.getTransferSubType())){
            		...
            		ChannelTransferBL.prepareNetworkStockListAndCreditDebitStock(con, channelTransferVO, p_userId, currentDate, true);
            		ChannelTransferBL.updateNetworkStockTransactionDetails(con, channelTransferVO, p_userId, currentDate);
            		if (PretupsI.COMM_TYPE_POSITIVE.equals(channelTransferVO.getDualCommissionType())) {
            			ChannelTransferBL.prepareNetworkStockListAndCreditDebitStockForCommision(con, channelTransferVO, p_userId, currentDate, true);
            			ChannelTransferBL.updateNetworkStockTransactionDetailsForCommision(con, channelTransferVO, p_userId, currentDate);
            		}
            		final UserBalancesDAO userBalancesDAO = new UserBalancesDAO();
            		userBalancesDAO.updateUserDailyBalances(con, currentDate, constructBalanceVOFromTxnVO(channelTransferVO));
                    ...
                    channelTransferVO.setChannelVoucherItemsVoList(channelTrfDAO.loadChannelVoucherItemsList(con, channelTransferVO.getTransferID(),channelTransferVO.getTransferDate()));
            ...
            updateCount = channelTrfDAO.updateChannelTransferApproval(con, channelTransferVO, true, isFromPaymentGateway); 
            if (updateCount < 0) {
            	try {
    				mcomCon.finalRollback();
    			} catch (SQLException sqle) {
    				_log.errorTrace("O2CTransferApprovalController#process", sqle);
    			}
            ...
            mcomCon.finalCommit();
            if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.O2C_EMAIL_NOTIFICATION))).booleanValue()) {
            	if(PretupsI.CHANNEL_TRANSFER_ORDER_NEW.equals(channelTransferVO.getStatus()))
						sendEmailNotification(con, channelTransferVO, channelUserVO, channelTrfDAO, "APV1O2CTRF", "o2c.email.notification.content");
				/*else if(PretupsI.CHANNEL_TRANSFER_ORDER_CLOSE.equals(channelTransferVO.getStatus()))
						sendEmailNotification(con, channelTransferVO, channelUserVO, channelTrfDAO, "", "o2c.email.notification.content.transfer.completed");*/
			}
            ...
            p_requestVO.setMessageArguments(new String[]{ channelTransferVO.getTransferID(), p_requestVO.getExternalReferenceNum(), PretupsBL.getDisplayAmount(channelTransferVO.getRequestedQuantity()) });
            ...
            if ("SUCCESS".equals(pgStatus) && ((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.LMS_APPL)).booleanValue()) {
                try {
            ...
            if(p_requestVO.getRequestGatewayType().equalsIgnoreCase(PretupsI.MOBILE_APP_GATEWAY)){
            	...
            	p_requestVO.setMessageArguments(new String[]{ channelTransferVO.getTransferID(), PretupsBL.getDisplayAmount(channelTransferVO.getRequestedQuantity()) });
				...
				responseStr.append(" \"message\": \"" +  RestAPIStringParser.getMessage(p_requestVO.getLocale(), p_requestVO.getMessageCode(), p_requestVO.getMessageArguments()) +  "\"}");
        	...
        	loggerValue.append(ex.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "O2CTransferApprovalController[process]", "", "", "",
            		loggerValue.toString());
            p_requestVO.setMessageCode(PretupsErrorCodesI.ERROR_USER_TRANSFER);
            return;
        } finally {
        	PGTransactionLog.log(p_requestVO.getTransactionID(), channelTransferVO.getPayInstrumentNum(), channelTransferVO.getReceiverMsisdn(), 
            		channelTransferVO.getNetworkCode(), PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_DEBIT,
            		p_requestVO.getRequestMessage(), PretupsI.EMPTY, p_requestVO.isSuccessTxn() ? PretupsI.TXN_LOG_STATUS_SUCCESS : PretupsI.TXN_LOG_STATUS_FAIL, "");
			if (mcomCon != null) {
				mcomCon.close("O2CTransferApprovalController#process");
				mcomCon = null;
			}
    ...
    private void sendEmailNotification(Connection p_con, ChannelTransferVO p_channelTransferVO, ChannelUserVO userVO, ChannelTransferDAO p_channelTransferDAO, String p_roleCode, String p_content) {
		...
		try {
			...
			String message = BTSLUtil.getMessage(locale,p_content) + 
				"<br>" + BTSLUtil.getMessage(locale,"o2c.email.channeluser.details") + 
					"<br>" + BTSLUtil.getMessage(locale,"o2c.email.transferid") + " " + p_channelTransferVO.getTransferID() + 
					...
					"<br>" + BTSLUtil.getMessage(locale,"o2c.email.notification.content.net.payable.amount")+ " " + PretupsBL.getDisplayAmount(p_channelTransferVO.getNetPayableAmount())+
					"<br>" + BTSLUtil.getMessage(locale,"o2c.email.transfer.type") + " " + p_channelTransferVO.getType() + 
					"<br>" + BTSLUtil.getMessage(locale,"o2c.email.initiator.name") + " " + arrayList.get(0) + 
					"<br>" + BTSLUtil.getMessage(locale,"o2c.email.initiator.msisdn") + " " + arrayList.get(1);
			if(p_channelTransferVO.getChannelVoucherItemsVoList() != null && !p_channelTransferVO.getChannelVoucherItemsVoList().isEmpty())
            {
                  message1 = "<table><tr>"
                              ...
                              + "<td style='width: 10%;'>"+ BTSLUtil.getMessage(locale, "o2c.email.notification.voucherType") + "</td>"
                              + "</tr>";
                  for(int i=1 ;i <= p_channelTransferVO.getChannelVoucherItemsVoList().size();i++)
                  {     
                              message1 = message1 + "<tr><td>" + i + "</td>" + 
                                       "<td>" + ((ChannelVoucherItemsVO)p_channelTransferVO.getChannelVoucherItemsVoList().get(i-1)).getTransferMrp() + "</td>" +  
                                       "<td>" + ((ChannelVoucherItemsVO)p_channelTransferVO.getChannelVoucherItemsVoList().get(i-1)).getRequiredQuantity() + "</td>" +
                                       "<td>" + new VomsProductDAO().getNameFromVoucherType(p_con,((ChannelVoucherItemsVO)p_channelTransferVO.getChannelVoucherItemsVoList().get(i-1)).getVoucherType()) + "</td>" +
                                       "</tr>";
                  }
