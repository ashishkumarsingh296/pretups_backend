 ...
 *    --------------------------------------------------------------------------
 *    ------
 *    ranjana.chouhan 19/02/09 Initial creation
 *    --------------------------------------------------------------------------
 *    ------
 ...
 */
public class InternateRechargeBatchFileParser implements BatchFileParserI {

    private static Log _log = LogFactory.getLog(InternateRechargeBatchFileParser.class.getName());
    ...
    public void downloadFile(Connection p_con, Writer p_fileWriter, String p_fileType, HashMap p_scheduleMap) throws BTSLBaseException {
        ...
        try {
            if (PretupsI.BATCH_TYPE_CORPORATE.equals(p_fileType)) {
                downloadCorpInternateFile(p_con, p_fileWriter, p_scheduleMap);
            } else {
                downloadNormalInternateFile(p_fileWriter, p_scheduleMap);
            }
            ...
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadFile", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    public void downloadFileForReshedule(Connection p_con, Writer p_fileWriter, String p_fileType, HashMap p_scheduleMap) throws BTSLBaseException {
        ...
        try {
            if (PretupsI.BATCH_TYPE_CORPORATE.equalsIgnoreCase(p_fileType)) {
                downloadCorporateInternateFileForReschedule(p_con, p_fileWriter, p_scheduleMap);
            } else {
                downloadNormalInternateFileForReschedule(p_con, p_fileWriter, p_scheduleMap);
            }
        } catch (BTSLBaseException be) {
            ...
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadFileForReshedule", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    public void uploadFile(Connection p_con, String p_fileType, HashMap p_scheduleInfoMap, boolean p_isErrorFound) throws BTSLBaseException {
        ...
        try {
            if (PretupsI.BATCH_TYPE_CORPORATE.equalsIgnoreCase(p_fileType)) {
                ...
                uploadNormalInternateFile(p_con, p_scheduleInfoMap, p_isErrorFound);
            }
        } catch (BTSLBaseException be) {
            ...
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "uploadFile", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
     ...
     * @throws BTSLBaseException
     */
    private void downloadCorpInternateFile(Connection p_con, Writer p_fileWriter, HashMap p_scheduleMap) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("downloadCorpInternateFile", "Entered");
        }
        final String METHOD_NAME = "downloadCorpInternateFile";
        try {
            ...
            String key = null;
            boolean isInternateMsisdn = false;
            while (iterator.hasNext()) {
                key = (String) iterator.next();
                restrictedSubscriberVO = (RestrictedSubscriberVO) hashMap.get(key);
                isInternateMsisdn = isInternateMsisdn(restrictedSubscriberVO.getMsisdn());
                if (isInternateMsisdn) {
            ...
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadCorpInternateFile", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "downloadCorpInternateFile");
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    /**
     * Method downloadNormalInternateFile.
     ...
     * @throws BTSLBaseException
     */
    private void downloadNormalInternateFile(Writer p_fileWriter, HashMap p_scheduleMap) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("downloadNormalInternateFile", "Entered");
        }
        final String METHOD_NAME = "downloadNormalInternateFile";
        try {
            String heading = (String) ((ArrayList) p_scheduleMap.get(HEADER_KEY)).get(0);
            ArrayList columnHeaderList = (ArrayList) p_scheduleMap.get(COLUMN_HEADER_KEY);
            ...
            _log.error("downloadNormalInternateFile", "Unable to write data " + ex.getMessage());
            _log.errorTrace(METHOD_NAME, ex);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadNormalInternateFile", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "downloadNormalInternateFile");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("downloadNormalInternateFile", "Exiting");
            }
        }
    }

    /**
     * Method downloadCorporateInternateFileForReschedule.
     * This method is called to download corporate file at the time of
     ...
     * @throws BTSLBaseException
     */
    private void downloadCorporateInternateFileForReschedule(Connection p_con, Writer p_fileWriter, HashMap p_scheduleMap) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("downloadCorporateInternateFileForReschedule", "Entered p_scheduleMap.size()= " + p_scheduleMap.size());
        }
        final String METHOD_NAME = "downloadCorporateInternateFileForReschedule";
        try {
            ...
            _log.error("downloadCorporateInternateFileForReschedule", "Unable to write data  " + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.error("downloadCorporateInternateFileForReschedule", "Unable to write data " + e.getMessage());
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadCorporateInternateFileForReschedule", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("downloadCorporateInternateFileForReschedule", "Exiting");
            }
        }
    }
    ...
    /**
     * Method downloadCorporateInternateFileForRescheduleRest.
     * This method is called to download corporate file at the time of
     * re-scheduling the file
     * 
     * @param p_con
     ...
     * @throws BTSLBaseException
     */
    private void downloadCorporateInternateFileForRescheduleRest(Connection p_con, Writer p_fileWriter, HashMap p_scheduleMap) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("downloadCorporateInternateFileForRescheduleRest", "Entered p_scheduleMap.size()= " + p_scheduleMap.size());
        }
        final String METHOD_NAME = "downloadCorporateInternateFileForRescheduleRest";
        try {
            RestrictedSubscriberDAO restrictedSubscriberDAO = new RestrictedSubscriberDAO();
            LinkedHashMap hashMap = restrictedSubscriberDAO.loadScheduleBatchDetailsList(p_con, (String) (p_scheduleMap.get(BATCH_ID)), PretupsI.STATUS_EQUAL, PretupsI.SCHEDULE_STATUS_SCHEDULED);
            p_scheduleMap.put("DATA_MAP", hashMap);
            ...
            _log.error("downloadCorporateInternateFileForRescheduleRest", "Unable to write data  " + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.error("downloadCorporateInternateFileForRescheduleRest", "Unable to write data " + e.getMessage());
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadCorporateInternateFileForRescheduleRest", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("downloadCorporateInternateFileForRescheduleRest", "Exiting");
            }
        }
    }
    ...
    /**
     * Method downloadNormalInternateFileForReschedule.
     ...
     * @throws BTSLBaseException
     */
    private void downloadNormalInternateFileForReschedule(Connection p_con, Writer p_fileWriter, HashMap p_scheduleInfoMap) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("downloadNormalInternateFileForReschedule", "Entered p_scheduleInfoMap.size()=" + p_scheduleInfoMap.size());
        }
        final String METHOD_NAME = "downloadNormalInternateFileForReschedule";
        try {
            RestrictedSubscriberDAO restrictedSubscriberDAO = new RestrictedSubscriberDAO();
            ...
            _log.error("downloadNormalInternateFileForReschedule", "Unable to write data  " + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.error("downloadNormalInternateFileForReschedule", "Unable to write data " + e.getMessage());
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadNormalInternateFileForReschedule", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("downloadNormalInternateFileForReschedule", "Exiting");
            }
        }
    }
    ...
    /**
     * Method downloadNormalInternateFileForRescheduleRest.
     * This method is called to download normal file at the time of
     * re-scheduling the file
     * 
     * @param p_con
     ...
     * @throws BTSLBaseException
     */
    private void downloadNormalInternateFileForRescheduleRest(Connection p_con, Writer p_fileWriter, HashMap p_scheduleInfoMap) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("downloadNormalInternateFileForRescheduleRest", "Entered p_scheduleInfoMap.size()=" + p_scheduleInfoMap.size());
        }
        final String METHOD_NAME = "downloadNormalInternateFileForRescheduleRest";
        try {
            RestrictedSubscriberDAO restrictedSubscriberDAO = new RestrictedSubscriberDAO();
            ArrayList list = restrictedSubscriberDAO.loadBatchDetailVOList(p_con, (String) p_scheduleInfoMap.get(BATCH_ID), PretupsI.STATUS_EQUAL, PretupsI.SCHEDULE_STATUS_SCHEDULED);
            String heading = (String) ((ArrayList) p_scheduleInfoMap.get(HEADER_KEY)).get(0);
            ...
            _log.error("downloadNormalInternateFileForRescheduleRest", "Unable to write data  " + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.error("downloadNormalInternateFileForRescheduleRest", "Unable to write data " + e.getMessage());
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "downloadNormalInternateFileForRescheduleRest", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("downloadNormalInternateFileForRescheduleRest", "Exiting");
            }
        }
    }
    ...
    public void uploadCorporateInternateFile(Connection p_con, HashMap p_scheduleInfoMap, boolean isErrorFound) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("uploadCorporateInternateFile", "Entered isErrorFound= " + isErrorFound + " p_scheduleInfoMap.size()= " + p_scheduleInfoMap.size());
        }
        ...
        try {
            ...
            while (finalListSize != countData) {
                ...
                if (errorVO.getErrorCode() == null) {
                    ...
                    if (networkPrefixVO == null) {
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadCorporateInternateFile", "Not Network prefix found " + arr[0]);
                        }
                    ...
                    if (!networkCode.equals(userVO.getNetworkID())) {
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadCorporateInternateFile", "Not supporting Network" + arr[0]);
                        }
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadCorporateInternateFile", "Not a valid Receiver Msisdn= " + filteredMsisdn);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                    ...
                    try {
                        if (BTSLUtil.isNullString(arr[9])) {
                            if (_log.isDebugEnabled()) {
                                _log.debug("uploadCorporateInternateFile", "MSISDN is Null " + arr[0]);
                            }
                            ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Receiver msisdn not found", "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadCorporateInternateFile", "Not a valid notification MSISDN " + arr[0]);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                    ...
                    if (reqAmt < restrictedSubscriberVO.getMinTxnAmount() || reqAmt > restrictedSubscriberVO.getMaxTxnAmount()) {
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadCorporateInternateFile", "Requested amount is not in between minimum amount and maximum amount= " + arr[8]);
                        }
            ...
            if (addCount == validDataList.size()) {
                ...
                if (addCount > 0) {
                    ...
                    throw new BTSLBaseException(this, "uploadCorporateInternateFile", errorUploadKeys[14], "scheduleTopUpAuthorise");
                }
            } else {
                p_con.rollback();
                _log.error("uploadCorporateInternateFile", "Scheduling file for the service recharge is unsuccessful");
            ...
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "uploadCorporateInternateFile", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    /**
     * Method uploadNormalInternateFile.
     ...
     * @throws BTSLBaseException
     */
    private void uploadNormalInternateFile(Connection p_con, HashMap p_scheduleInfoMap, boolean isErrorFound) throws BTSLBaseException {
        if (_log.isDebugEnabled()) {
            _log.debug("uploadNormalInternateFile", "Entered isErrorFound= " + isErrorFound + " p_scheduleInfoMap.size()= " + p_scheduleInfoMap.size());
        }
        final String METHOD_NAME = "uploadNormalInternateFile";
        String filePathAndFileName = null;
        try {
            String dataStr = null;
            int countData = 0;
            ...
            while (finalListSize != countData) {
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug("uploadNormalInternateFile", "Processing starts for  " + dataStr);
                }
                countData++;
                if (errorVO.getErrorCode() == null) {
                    arr = dataStr.split(",");
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not a valid MSISDN " + arr[0]);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                        ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not a valid MSISDN", "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                        errorVO.setErrorCode(invalidMSISDN);
                    ...
                    if (networkPrefixVO == null) {
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not Network prefix found " + arr[0]);
                        }
                        ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not Network prefix found", "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                        errorVO.setErrorCode(prefixNotFound);
                        errorVO.setMsisdn(arr[0]);
                    ...
                    if (!networkCode.equals(userVO.getNetworkID())) {
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not supporting Network" + arr[0]);
                        }
                        ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not Supporting Network", "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                        errorVO.setErrorCode(networkNotSupported);
                        errorVO.setMsisdn(arr[0]);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not a valid Receiver Msisdn= " + filteredMsisdn);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                        ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not a internate series number entered", "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                        errorVO.setErrorCode(errorUploadKeys[16]);
                        errorVO.setErrorCodeArgs(new String[] { filteredMsisdn });
                        errorVO.setMsisdn(arr[0]);
                        isErrorFound = true;
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not a valid sub-service = " + tmpSubService);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                        if (!BTSLUtil.isNullString(tmpSubService)) {
                            ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not a valid sub-service value = " + tmpSubService, "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                    ...
                    try {
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not a valid requested amount tmpAmt: " + tmpAmt);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                        if (!BTSLUtil.isNullString(tmpAmt)) {
                            ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not a valid requested amount = " + tmpAmt, "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                    ...
                    try {
                        if (BTSLUtil.isNullString(arr[3])) {
                            if (_log.isDebugEnabled()) {
                                _log.debug("uploadNormalInternateFile", "Notification MSISDN is Null " + arr[0]);
                            }
                        ...
                        if (_log.isDebugEnabled()) {
                            _log.debug("uploadNormalInternateFile", "Not a valid notification MSISDN " + arr[0]);
                        }
                        _log.errorTrace(METHOD_NAME, e);
                        ScheduleFileProcessLog.log("Validating Data", createdBy, arr[0], batchID, "Not a valid notification MSISDN", "FAIL", "UPLOADED FILE =" + filePathAndFileName + ", TYPE=" + requestFor);
                        errorVO.setErrorCode(invalidNotificationMsisdn);
            ...
            if (validDataList.isEmpty()) {
                ...
                scheduleBatchMasterVO.setUploadFailedCount(countData);
                if (_log.isDebugEnabled()) {
                    _log.debug("uploadNormalInternateFile", "No valid Data in the list, size :" + validDataList.size());
                }
                throw new BTSLBaseException(this, "uploadNormalInternateFile", errorUploadKeys[10], "showMsg");
            }
            ...
            if (addCount > 0) {
                p_con.commit();
                if (_log.isDebugEnabled()) {
                    _log.debug("uploadNormalInternateFile", "commit connection");
                }
            }
            if (addCount == validDataList.size()) {
                scheduleBatchMasterVO.setTotalCount(countData);
                ...
                if (addCount > 0) {
                    ...
                    p_con.rollback();
                    _log.error("uploadNormalInternateFile", "Scheduling file for the service recharge is unsuccessful");
                    throw new BTSLBaseException(this, "uploadNormalInternateFile", errorUploadKeys[7], "scheduleTopUpAuthorise");
                }
            } else {
                p_con.rollback();
                _log.error("uploadNormalInternateFile", "Scheduling file for the service recharge is unsuccessful");
            ...
            _log.error("uploadNormalInternateFile", "Unable to write data  " + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.error("uploadNormalInternateFile", "Scheduling file for the service recharge is unsuccessful " + e.getMessage());
            _log.errorTrace(METHOD_NAME, e);
            throw new BTSLBaseException("InternateRechargeBatchFileParser", "uploadNormalInternateFile", PretupsErrorCodesI.GENERAL_PROCESSING_ERROR, "scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("uploadNormalInternateFile", "Exiting");
            }
        }
    }
    ...
    public String getHeaderKey(String p_fileType) {
        ...
        String heading = FILE_HEADER_KEYS[3];
     ...
     * @throws BTSLBaseException
     */
    public boolean isInternateMsisdn(String p_msisdn) {
        if (_log.isDebugEnabled()) {
            _log.debug("isInternateMsisdn", "Entered p_msisdn=" + p_msisdn);
        }
        boolean isInternateMsisdn = false;
        final String METHOD_NAME = "isInternateMsisdn";
        String msisdnPrefix = null;
        NetworkPrefixVO networkPrefixVO = null;
        ...
        try {
            ...
            if (BTSLUtil.isValidIdentificationNumber(filteredMsisdn)) {
                ...
                if (networkPrefixVO != null) {
                    ...
                    if (msisdnPrefixInterfaceMappingVO != null) {
                        String _str = PretupsI.SERVICE_TYPE_CHNL_RECHARGE_INTR + "_" + msisdnPrefixInterfaceMappingVO.getInterfaceTypeID();
                        itr = arrList.iterator();
                        while (itr.hasNext()) {
                            String _serviceTypeIntfId = (String) itr.next();
                            if (_str.equalsIgnoreCase(_serviceTypeIntfId)) {
                                isInternateMsisdn = true;
                            }
                        }
            ...
            _log.error("isInternateMsisdn", "isInternateMsisdn " + e.getMessage());
            _log.errorTrace(METHOD_NAME, e);
            // throw new
            // BTSLBaseException("isPstnMsisdn","isPstnMsisdn",PretupsErrorCodesI.GENERAL_PROCESSING_ERROR,"scheduleTopUpAuthorise");
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug("isInternateMsisdn", "Exiting");
            }
        }
        return isInternateMsisdn;
    }
	...
	public void downloadFileForResheduleRest(Connection p_con, Writer p_fileWriter, String p_fileType,
        ...
        if (_log.isDebugEnabled()) {
            _log.debug("downloadFileForReshedule", "Entered p_fileType = " + p_fileType + ",p_scheduleMap.size()=" + p_scheduleInfoMap.size());
        }
        final String METHOD_NAME = "downloadFileForResheduleRest";
        try {
            if (PretupsI.BATCH_TYPE_CORPORATE.equalsIgnoreCase(p_fileType)) {
                downloadCorporateInternateFileForRescheduleRest(p_con, p_fileWriter, p_scheduleInfoMap);
            } else {
                downloadNormalInternateFileForRescheduleRest(p_con, p_fileWriter, p_scheduleInfoMap);
            }
        } catch (BTSLBaseException be) {
            _log.error("downloadFileForReshedule", "Unable to write data " + be.getMessage());
            throw be;
