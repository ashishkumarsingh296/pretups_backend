...
public class ChannelUserTransferPostgresQry implements ChannelUserTransferQry {
	...
	public String createStatementForLloadInitiatedUserListForUserTransferQry(String status,String statusUsed){
        ...
        strBuff.append(" SELECT grph_domain_code FROM geographical_domains gd WHERE ");
        strBuff.append("  grph_domain_code = ? union  all select m.grph_domain_code from geographical_domains m join q on q.grph_domain_code=m.parent_grph_domain_code ");
        strBuff.append("  ) select grph_domain_code from q   ");
        strBuff.append(" WHERE grph_domain_type=c.grph_domain_type ) ");
	...
	public PreparedStatement loadGeogphicalHierarchyListByToParentIdQry(Connection con,String toParentID ) throws SQLException{
    	...
    	strBuff.append(" grph_domain_code IN (SELECT grph_domain_code FROM USER_GEOGRAPHIES WHERE user_id=?) ");
    	strBuff.append("union  all select m.grph_domain_code, m.network_code, m.grph_domain_name, m.parent_grph_domain_code, m.status, m.grph_domain_type from geographical_domains m join q on q.grph_domain_code=m.parent_grph_domain_code ");
    	strBuff.append(" ");
    	strBuff.append(")select grph_domain_code, network_code, grph_domain_name, parent_grph_domain_code, status, grph_domain_type  from q  WHERE status=? ");
