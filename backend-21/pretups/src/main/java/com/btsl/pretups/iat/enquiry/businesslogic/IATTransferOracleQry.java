...
public class IATTransferOracleQry implements IATTransferQry{
	private String className = "IATTransferOracleQry";
	...
	public PreparedStatement loadIATTransferVOListNewQry(Connection pCon, String pNetworkCode, Date pFromdate, Date p_toDate, String pSenderMsisdn, String pTransferID, String pServiceType,  UserPhoneVO phoneVo) throws SQLException {
		...
		String methodname = "#loadIATTransferVOList_newQry";
		...
		selectQueryBuff.append("FROM PRODUCTS PROD, KEY_VALUES KV,KEY_VALUES KV2,KEY_VALUES KV3, SERVICE_TYPE ST,IAT_COUNTRY_MASTER ICM ,USERS U, C2S_TRANSFERS CTRF, C2S_IAT_TRANSFER_ITEMS ITI ");
		...
		selectQueryBuff.append("AND ITI.SERVICE_TYPE=DECODE(?,'ALL',CTRF.SERVICE_TYPE,?)");
		selectQueryBuff.append("AND ITI.SERVICE_TYPE =ST.SERVICE_TYPE AND CTRF.product_code=PROD.product_code ");
		selectQueryBuff.append("AND CTRF.network_code=? AND KV.KEY(+)=CTRF.error_code AND KV.TYPE(+)=? ");
		selectQueryBuff.append("AND KV2.KEY(+)=CTRF.transfer_status AND KV2.TYPE(+)=? ");
		selectQueryBuff.append("AND KV3.KEY(+)=ITI.transfer_status AND KV3.TYPE(+)=? ");
	...
	public PreparedStatement loadIATTransferVOListOldQry(Connection pCon,
		...
		String methodName = "#loadIATTransferVOList_oldQry";
		String isSenderPrimary = phoneVo.getPrimaryNumber();
		StringBuilder selectQueryBuff = new StringBuilder();
		...
		selectQueryBuff.append("ITI.credit_resp_code, ITI.credit_msg, ITI.rec_nw_error_code, ITI.rec_nw_message, ITI.chk_status_resp_code, ITI.sent_amt,  ");
		selectQueryBuff.append("ITI.fees, ITI.prov_ratio, ITI.exchange_rate, ITI.failed_at, ITI.rec_bonus,ITI.rcvd_amt, ITI.sent_amt_iattorec ");
		selectQueryBuff.append("FROM PRODUCTS PROD, KEY_VALUES KV,KEY_VALUES KV2,KEY_VALUES KV3, SERVICE_TYPE ST,IAT_COUNTRY_MASTER ICM ,USERS U, C2S_TRANSFERS_OLD CTRF, C2S_TRANSFER_ITEMS CTRIS, C2S_IAT_TRANSFER_ITEMS ITI ");
		selectQueryBuff.append("WHERE CTRF.transfer_date >=? AND CTRF.transfer_date < ? ");
	...
	public PreparedStatement loadIATTransferItemsVOListQry(Connection pCon,
		...
		//local_index_implemented
		String methodName = "#loadIATTransferItemsVOListQry";
		StringBuilder selectQueryBuff = new StringBuilder();
        ...
        selectQueryBuff.append("FROM c2s_transfers,key_values KV,key_values KV2,key_values KV3 ");
        selectQueryBuff.append("WHERE transfer_date=? AND transfer_id=? AND KV.key(+)=transfer_status AND KV.type(+)=? ");
        selectQueryBuff.append("AND KV2.key(+)=transfer_type AND KV2.type(+)=? ");
        selectQueryBuff.append("AND KV3.key(+)=interface_response_code AND KV3.type(+)=? ");
	...
	public PreparedStatement loadIATTransferItemsVOListOldQry(Connection pCon,
        ...
        selectQueryBuff.append("FROM c2s_transfer_items,key_values KV,key_values KV2,key_values KV3   ");
