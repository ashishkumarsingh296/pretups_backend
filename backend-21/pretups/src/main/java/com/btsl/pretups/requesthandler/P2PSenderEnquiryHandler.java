 ...
 * 
 * This program processes the request for Customer P2P Sender enquiry for
 * MSISDN, FromDate, ToDate, TransactionID.
 * It loads the Customer P2P Sender transaction Summary(ies) (alTransferVOs
 * object)
 * or transaction Details (alTransferDetailsVOs object) and sets it in Hashmap
 ...
 * Dates FromDate and ToDate are optional
 */
public class P2PSenderEnquiryHandler implements ServiceKeywordControllerI {

    private Log log = LogFactory.getLog(this.getClass().getName());
    private HashMap _requestMap = null;
    private static final String MSISDN_STR = "SENDERMSISDN";
    private static final String TRANSACTION_ID = "TRANSACTIONID";

    /**
     * This method is the entry point in the class. The method is declared in
     * the Interface ServiceKeywordControllerI
     * This method processes the request for the SENDERMSISDN, TRANSACTIONID,
     * FROMDATE and TODATE
     * calls the validate() for validating MSISDN, LOGINID, FROMDATE and TODATE
     * and calls loadTransferData() that sets the Customer P2P Sender
    ...
    public void process(RequestVO p_requestVO) {
        ...
        try {
            ...
            TransferVO requestTransferVO = validate(p_requestVO);
			...
			if (mcomCon != null) {
				mcomCon.close("P2PSenderEnquiryHandler#process");
				mcomCon = null;
			}
    ...
    /**
     * This method checks for the mandatory value MSISDN OR TRANSACTIONID is not
     * null,
     ...
     */
    private TransferVO validate(RequestVO p_requestVO) throws BTSLBaseException {
        if (log.isDebugEnabled()) {
        ...
        try {
            ...
            if (networkCode != null && !networkCode.equals(((UserVO) p_requestVO.getSenderVO()).getNetworkID())) {
                if (log.isDebugEnabled()) {
                    log.debug("validate", "No supporting Network for msisdn=" + msisdn);
                }
    ...
    private void loadTransferData(Connection p_con, TransferVO p_requestTransferVO, RequestVO p_requestVO) throws BTSLBaseException {
        ...
        try {
            ...
            alTransferVOs = subscriberTransferDAO.loadSubscriberDetails(p_con, p_requestTransferVO);
            ...
            if (alTransferVOs == null || alTransferVOs.isEmpty()) {
                ...
                int transferDetailsSize = alTransferDetails.size();
                for (int i = 0; i < transferDetailsSize; i++) {
