...
public class ChannelTransferReportOracleQry implements ChannelTransferReportQry {
	...
	public String loadVoucherO2CTransferDetailsQry(UsersReportForm thisForm)
	 {
		...
		selectQueryBuff.append(" AND CTRF.grph_domain_code IN( SELECT GD1.grph_domain_code FROM	GEOGRAPHICAL_DOMAINS GD1 WHERE ");
		selectQueryBuff.append(" GD1.status IN(	'Y','S'	) CONNECT BY PRIOR GD1.grph_domain_code = GD1.parent_grph_domain_code ");
		selectQueryBuff.append(" START WITH GD1.grph_domain_code IN( SELECT	UG1.grph_domain_code FROM USER_GEOGRAPHIES UG1 ");
	...
	public String queryC2CTransferVoucherDetailsForChannelUser(UsersReportForm thisForm)
	 {
		...
		// From 	
		 selectQueryBuff.append("( SELECT USR.user_id FROM ");
		 selectQueryBuff.append(" users USR CONNECT BY PRIOR USR.user_id = USR.parent_id START WITH USR.user_id = ?  ) X,");
		 selectQueryBuff.append(
		...
		// WHERE 
		 ...
		 selectQueryBuff.append(
			...
			selectQueryBuff.append("AND CTRF.to_user_id = UG.user_id AND UG.grph_domain_code IN ( ");
			selectQueryBuff.append(" SELECT grph_domain_code FROM GEOGRAPHICAL_DOMAINS GD1 ");
			selectQueryBuff.append(" WHERE status IN( 'Y','S') CONNECT BY PRIOR grph_domain_code = parent_grph_domain_code START WITH grph_domain_code IN( ");
		    ...
		    //FROM
			 ...
			 selectQueryBuff.append(
				...
				selectQueryBuff.append("AND CTRF.TRANSFER_TYPE = 'TRANSFER' AND CTRF.transfer_id = CVI.transfer_id AND CTRF.FROM_USER_ID = FU.USER_ID AND CTRF.TO_USER_ID = TU.USER_ID AND CVI.VOUCHER_SEGMENT = LKP.LOOKUP_CODE AND CVI.VOUCHER_TYPE = VTYPE.VOUCHER_TYPE AND CVI.PRODUCT_ID = VP.PRODUCT_ID ");
				selectQueryBuff.append("AND CTRF.from_user_id = UG.user_id AND UG.grph_domain_code IN ( ");
				selectQueryBuff.append(" SELECT grph_domain_code FROM GEOGRAPHICAL_DOMAINS GD1 ");
				selectQueryBuff.append(" WHERE status IN( 'Y','S') CONNECT BY PRIOR grph_domain_code = parent_grph_domain_code START WITH grph_domain_code IN( ");
				selectQueryBuff.append(" SELECT grph_domain_code FROM USER_GEOGRAPHIES UG1 ");
				selectQueryBuff.append(" WHERE UG1.grph_domain_code =(CASE ?	WHEN 'ALL' THEN UG1.grph_domain_code	ELSE ?	END	) AND UG1.user_id = ?) ) ");
	...
	public String queryC2CTransferVoucherDetailsForOperatorUser(UsersReportForm thisForm)
	 {
		...
		selectQueryBuff.append(
				" SELECT GD1.grph_domain_code FROM GEOGRAPHICAL_DOMAINS GD1 WHERE GD1.status IN('Y','S') CONNECT BY PRIOR GD1.grph_domain_code = GD1.parent_grph_domain_code");
		selectQueryBuff
				.append(" START WITH GD1.grph_domain_code IN (SELECT UG1.grph_domain_code FROM USER_GEOGRAPHIES UG1");
		selectQueryBuff.append(
				" WHERE UG1.grph_domain_code = CASE ? WHEN 'ALL' THEN UG1.grph_domain_code ELSE ? END AND UG1.user_id=?))");
		selectQueryBuff.append(
	...
	public String queryVoucherAvailabilityDetailsQry(UsersReportForm thisForm)
	{
		...
		selectQueryBuff.append(" ,lk.LOOKUP_NAME as voucher_segment,vv.VOUCHER_TYPE as voucher_type,vv.sold_status as sold_status,uu.user_type as user_type, ");
		selectQueryBuff.append(" vp.PRODUCT_NAME as product_name, COUNT( 1 ) AS no_of_vouchers ");
		selectQueryBuff.append(" FROM VOMS_VOUCHERS vv, LOOKUPS lk, VOMS_PRODUCTS vp, users uu ");
		selectQueryBuff.append(" WHERE uu.USER_ID = vv.USER_ID AND vv.VOUCHER_SEGMENT = lk.LOOKUP_CODE AND vv.STATUS IN(?,?,?,?,?,?) AND vv.PRODUCT_ID = vp.PRODUCT_ID AND vv.USER_ID IN( ");
		selectQueryBuff.append(" SELECT user_id FROM users u ");
		selectQueryBuff.append(" CONNECT BY PRIOR u.user_id = u.parent_id START WITH u.user_id = ? ) GROUP BY ");
		selectQueryBuff.append(" uu.NETWORK_CODE, uu.CATEGORY_CODE, uu.PARENT_ID, uu.MSISDN, uu.user_name, vv.status, vv.mrp, lk.LOOKUP_NAME, vv.VOUCHER_TYPE,vp.product_name,vv.sold_status,uu.USER_TYPE ");
		return selectQueryBuff.toString();
	}
	...
	public String queryVoucherConsumptionDetailsQry(UsersReportForm thisForm)
	{
		...
		StringBuilder selectQueryBuff = new StringBuilder(" SELECT uu.user_name as user_id,uu.NETWORK_CODE,uu.CATEGORY_CODE,uu.PARENT_ID,       ");
		
		selectQueryBuff.append(" ( CASE uu.parent_id WHEN   'ROOT' THEN 'NA' ELSE TO_CHAR(parentu.user_name) END ) as \"PARENT_USER_NAME\", ");
		selectQueryBuff.append(" ( CASE uu.parent_id WHEN   'ROOT' THEN 'NA' ELSE TO_CHAR(parentu.msisdn) END ) as \"PARENT_MSISDN\", ");

		
		selectQueryBuff.append("  uu.MSISDN AS Channel_USErs_MSISDN,vv.STATUS,TO_NUMBER(vv.MRP/").append(((Integer) (PreferenceCache.getSystemPreferenceValue(PreferenceI.AMOUNT_MULT_FACTOR))).intValue()).append(",'9999999999999999999999D99') as mrp,lk.LOOKUP_NAME as voucher_segment,vv.VOUCHER_TYPE,vv.sold_status,uu.user_type, ");
		selectQueryBuff.append(" vp.PRODUCT_NAME, COUNT( 1 ) AS no_of_vouchers ");
		selectQueryBuff.append(" FROM VOMS_VOUCHERS vv, LOOKUPS lk, VOMS_PRODUCTS vp, users uu,   users parentu ");
		selectQueryBuff.append(" WHERE uu.USER_ID = vv.USER_ID   and ( CASE uu.parent_id WHEN 'ROOT' THEN uu.owner_id ELSE uu.parent_id END ) =  parentu.user_id    AND vv.VOUCHER_SEGMENT = lk.LOOKUP_CODE AND vv.STATUS = 'CU' AND vv.LAST_CONSUMED_ON BETWEEN ? AND ? AND vv.PRODUCT_ID = vp.PRODUCT_ID AND vv.USER_ID IN( ");
		selectQueryBuff.append(" SELECT user_id FROM users u ");
		selectQueryBuff.append(" CONNECT BY PRIOR u.user_id = u.parent_id START WITH u.user_id = ? ) GROUP BY ");
		selectQueryBuff.append(" uu.NETWORK_CODE, uu.CATEGORY_CODE, uu.PARENT_ID,   parentu.msisdn,  parentu.user_name, uu.MSISDN, uu.user_name, vv.status, vv.mrp, lk.LOOKUP_NAME, vv.VOUCHER_TYPE,vp.product_name,vv.sold_status,uu.USER_TYPE ");
		return selectQueryBuff.toString();
	}
	...
	public String queryC2CNLevelTrackingDetailReport() {
		...
		selectQueryBuff.append("  ( CASE currentUsers.parent_id WHEN   'ROOT' THEN N'NA' ELSE parentUsr.user_name END )  AS \"Parent Channel User Name\" ,  ");
		
		
		
		//selectQueryBuff.append(" parentUsr.MSISDN AS \"Parent Channel User MSISDN\",   ");
		selectQueryBuff.append("  ( CASE currentUsers.parent_id WHEN   'ROOT' THEN 'NA' ELSE parentUsr.MSISDN END )  AS \"Parent Channel User MSISDN\" ,  ");
