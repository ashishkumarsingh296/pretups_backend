 ...
 * 
 * This class handles the request for the resumption of P2P services for a
 * subscriber.
 * The subscriber will be resumed for the services based on the mobile number
...
public class ResumeP2PServiceHandler implements ServiceKeywordControllerI {

    private Log log = LogFactory.getLog(ResumeP2PServiceHandler.class.getName());
     ...
     * resumption for a subscriber are called from this method.
     * 
     * @param p_requestVO
     */
    public void process(RequestVO p_requestVO) {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ResumeP2PServiceHandler[process]", "", "", "", "Exception:" + ex.getMessage());
            _requestVO.setMessageCode(PretupsErrorCodesI.REQ_NOT_PROCESS);
            return;
        }// end of Exception
        finally {
			...
			if (mcomCon != null) {
				mcomCon.close("ResumeP2PServiceHandler#process");
				mcomCon = null;
			}
            if (log.isDebugEnabled()) {
    ...
    private void validate() throws BTSLBaseException {
        ...
        try {
            ...
            if (BTSLUtil.isNullString(msisdn)) {
                ...
                throw new BTSLBaseException("ResumeP2PServiceHandler", "validate", PretupsErrorCodesI.CCE_XML_ERROR_MISSING_MANDATORY_VALUE);
            }
            ...
            if (networkPrefixVO == null) {
                ...
                throw new BTSLBaseException("ResumeP2PServiceHandler", "validate", PretupsErrorCodesI.CCE_XML_ERROR_UNSUPPORTED_NETWORK, arr);
            }
            ...
            if (networkCode != null && !networkCode.equals(((UserVO) _requestVO.getSenderVO()).getNetworkID())) {
                throw new BTSLBaseException("ResumeP2PServiceHandler", "validate", PretupsErrorCodesI.CCE_XML_ERROR_NETWORK_NOT_MATCHING_REQUEST);
            }
        } catch (BTSLBaseException be) {
            log.errorTrace(METHOD_NAME, be);
            ...
            log.errorTrace(METHOD_NAME, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ResumeP2PServiceHandler[validate]", "", "", "", "Exception:" + e.getMessage());
            throw new BTSLBaseException("ResumeP2PServiceHandler", "validate", PretupsErrorCodesI.REQ_NOT_PROCESS);
        }
        if (log.isDebugEnabled()) {
    ...
    private void loadUserDetails(Connection p_con) throws BTSLBaseException {
        ...
        try {
            ...
            if (subscriberList == null || subscriberList.size() <= 0) {
                throw new BTSLBaseException("ResumeP2PServiceHandler", "loadUserDetails", PretupsErrorCodesI.CCE_ERROR_SUBSCRIBER_DETAIL_NOT_FOUND);
            }
            ...
            log.errorTrace(METHOD_NAME, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ResumeP2PServiceHandler[loadUserDetails]", "", "", "", "Exception:" + e.getMessage());
            throw new BTSLBaseException("ResumeP2PServiceHandler", "loadUserDetails", PretupsErrorCodesI.REQ_NOT_PROCESS);
        }
        if (log.isDebugEnabled()) {
    ...
    /**
     * This method is the method which actually resumes the P2P services of the
     * subscriber.
     ...
     * @throws BTSLBaseException
     */
    private void resumeService(Connection p_con) throws BTSLBaseException {
        final String METHOD_NAME = "resumeService";
        if (log.isDebugEnabled()) {
            log.debug("resumeService", "Entering ....: ");
        }
        ...
        try {
            ...
            if (_senderVO.getStatus().equals(PretupsI.USER_STATUS_ACTIVE)) {
                throw new BTSLBaseException("ResumeP2PServiceHandler", "resumeService", PretupsErrorCodesI.CCE_ERROR_USER_ALREADY_ACTIVE);
            } else if (_senderVO.getStatus().equals(PretupsI.USER_STATUS_SUSPEND)) {
                btslMessage = new BTSLMessages(PretupsErrorCodesI.P2PSUBSCRIBER_SERVICE_RESUME);
                _senderVO.setStatus(PretupsI.USER_STATUS_ACTIVE);
            } else {
                throw new BTSLBaseException("ResumeP2PServiceHandler", "resumeService", PretupsErrorCodesI.CCE_ERROR_USER_STATUS_INVALID);
            }
            ...
            if (p_con != null) {
                if (updateCount > 0) {
                    ...
                    throw new BTSLBaseException("ResumeP2PServiceHandler", "resumeService", PretupsErrorCodesI.REQ_NOT_PROCESS);
                }
            }
        } catch (BTSLBaseException be) {
            log.errorTrace(METHOD_NAME, be);
            log.error("resumeService", "BTSLBaseException " + be.getMessage());
            throw be;
        } catch (Exception e) {
            log.error("resumeService", "Exception " + e.getMessage());
            log.errorTrace(METHOD_NAME, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "ResumeP2PServiceHandler[resumeService]", "", "", "", "Exception:" + e.getMessage());
            throw new BTSLBaseException("ResumeP2PServiceHandler", "resumeService", PretupsErrorCodesI.REQ_NOT_PROCESS);
        }
        if (log.isDebugEnabled()) {
            log.debug("resumeService", "Exiting....");
        }
    }
}
