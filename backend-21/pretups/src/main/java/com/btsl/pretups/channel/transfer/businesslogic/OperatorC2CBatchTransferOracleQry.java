...
public class OperatorC2CBatchTransferOracleQry implements OperatorC2CBatchTransferQry {

	@Override
	public String loadBatchC2CMasterDetailsForTxrQry(String p_itemStatus, String p_currentLevel) {
		StringBuffer strBuff = new StringBuffer();
		strBuff.append(" SELECT * FROM ( SELECT DISTINCT c2cb.opt_batch_id,c2cb.batch_id,c2cb.batch_name,c2cb.batch_total_record,c2cb.user_id, p.product_name,p.short_name, ");
	...
	public String loadBatchC2CMasterDetailsQry(String p_batchid) {
		...
		strBuff.append(" SUM(DECODE(cbi.status,?,1,0)) CLOSE,c2cb.created_on ");
		strBuff.append(" FROM C2C_BATCHES c2cb,C2C_BATCH_ITEMS cbi,PRODUCTS p");
	...
	public String loadBatchItemsMapQry(String p_itemStatus) {
		...
		strBuff.append(" FROM C2C_BATCH_ITEMS cbi,C2C_BATCHES c2cb,CATEGORIES c,CHANNEL_GRADES cg, USERS u, USERS intu, USERS fapp, USERS usender ");
		...
		strBuff.append(" AND cbi.rcrd_status=? AND  c2cb.created_by = intu.user_id(+) ");
	...
	public String processOrderByBatchQry() {
		StringBuffer sqlBuffer = new StringBuffer("SELECT c2cb.batch_total_record,SUM(DECODE(cbi.status,?,1,0)) new,");
        sqlBuffer.append(" SUM(DECODE(cbi.status,?,1,0)) appr,SUM(DECODE(cbi.status,?,1,0)) cncl, ");
	...
	public String closeOrderByBatchQry() {
		StringBuffer sqlBuffer = new StringBuffer("SELECT fb.batch_total_record,SUM(DECODE(cbi.status,?,1,0)) new,");
        sqlBuffer.append(" SUM(DECODE(cbi.status,?,1,0)) appr,SUM(DECODE(cbi.status,?,1,0)) cncl, ");
        sqlBuffer.append(" SUM(DECODE(cbi.status,?,1,0)) close ");
