...
/**
 * @(#)C2SFixLineController.java
 *                               Name Date History
 *                               ----------------------------------------------
 *                               --------------------------
 *                               Ranjana Chouhan 09/01/2009 Initial Creation
 *                               ----------------------------------------------
 *                               --------------------------
 *                               Copyright (c) 2005 Bharti Telesoft Ltd.
 *                               Controller class for handling the Channel to
 *                               subscriber transfers for fix line customer
 *                               only.
 */
...
public class C2SFixLineController extends C2SBaseController implements ServiceKeywordControllerI, Runnable {

    private static Log _log = LogFactory.getLog(C2SFixLineController.class.getName());
    ...
    private String _notificationMSISDN;
    ...
    static {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[initialize]", "", "", "",
                "Exception while loading the class at the call:" + e.getMessage());
        }
    }

    public C2SFixLineController() {
        _c2sTransferVO = new C2STransferVO();
        _currentDate = new Date();
        if ("Y".equals(BTSLUtil.NullToString(Constants.getProperty("C2S_FIXLINE_RC_REC_GEN_FAIL_MSG_REQD_V")))) {
            _recValidationFailMessageRequired = true;
        }
        if ("Y".equals(BTSLUtil.NullToString(Constants.getProperty("C2S_FIXLINE_RC_REC_GEN_FAIL_MSG_REQD_T")))) {
            _recTopupFailMessageRequired = true;
        }
        _notAllowedSendMessGatw = BTSLUtil.NullToString(Constants.getProperty("C2S_FIXLINE_SEN_MSG_NOT_REQD_GW"));
        _notAllowedRecSendMessGatw = BTSLUtil.NullToString(Constants.getProperty("C2S_FIXLINE_REC_MSG_NOT_REQD_GW"));
    }
    ...
    public void process(RequestVO p_requestVO) {
        ...
        try {
            ...
            _operatorUtil.validateC2SFixLineRechargeRequest(con, _c2sTransferVO, p_requestVO);
            ...
            if (!_receiverVO.getSubscriberType().equals(_type)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "C2SFixLineController[process]", "", "", "",
            ...
            // notification number to send message
            _notificationMSISDN = p_requestVO.getNotificationMSISDN();
            // to check that the subscriber is exist in restricted list or not.
            ...
            if (_senderMSISDN.equals(_receiverMSISDN) && !((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.FIXLINE_RC_ALLOW_SELF_TOPUP))).booleanValue()) {
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                throw new BTSLBaseException("C2SFixLineController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            }

            // forwarding request to process the transfer request and to
            ...
            // transfer VO
            // This will be used in validate ReceiverLimit method of PretupsBL
            // when receiverTransferItemVO is null
            ...
            // commiting transaction and closing the transaction as it is not
            // requred
            try {
            	mcomCon.finalCommit();
            } catch (Exception e) {
            ...
            if(mcomCon != null){mcomCon.close("C2SFixLineController#process");mcomCon=null;}
            con = null;
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "C2SFixLineController[process]", _transferID,
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[process]", _transferID,
                _senderMSISDN, _senderNetworkCode,  loggerValue.toString());
            ...
            try {
                ...
                // ||
                // p_requestVO.getMessageGatewayVO().getFlowType().equals(PretupsI.MSG_GATEWAY_FLOW_TYPE_THREAD)
                // ||
                // p_requestVO.getMessageGatewayVO().getFlowType().equals(PretupsI.MSG_GATEWAY_FLOW_TYPE_REQUEST)
                // &&
                // !p_requestVO.getMessageCode().equals(PretupsI.TXN_STATUS_UNDER_PROCESS))))
     ...
     * and generating the transfer ID .
     * 
     * @param p_con
    ...
    public void processTransfer(Connection p_con) throws BTSLBaseException {
        ...
        try {
            ...
            // Generating the transfer ID for FixLine Recharge
            generateFixLineTransferID(_c2sTransferVO);
            _transferID = _c2sTransferVO.getTransferID();
            _receiverVO.setLastTransferID(_transferID);

            // Set sender transfer item and receiver item details
            ...
            _c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[processTransfer]", _transferID,
    ...
    public void run() {
        ...
        try {
            ...
            // send topup request for receiver
            ...
            if (PretupsI.YES.equals(_c2sTransferVO.getDifferentialAllowedForService())) {
                try {
                    ...
                    _log.errorTrace(methodName, e);
                    throw new BTSLBaseException("C2SFixLineController", "process", PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
                }
                if(mcomCon != null){mcomCon.close("C2SFixLineController#run");mcomCon=null;}
                con = null;
                ...
                try {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "C2SFixLineController[run]", _c2sTransferVO
            ...
            if (_log.isDebugEnabled()) {
                _log.debug("C2SFixLineController",
                    "For _transferID=" + _transferID + " Diff applicable=" + _c2sTransferVO.getDifferentialApplicable() + " Diff Given=" + _c2sTransferVO
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[run]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "C2SFixLineController[run]", _transferID,
            ...
            if (_receiverMessageSendReq && !BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(), _notAllowedRecSendMessGatw) && !"ALL"
                ...
                if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                    if (_c2sTransferVO.getReceiverReturnMsg() == null) {
                        (new PushMessage(_notificationMSISDN, getReceiverSuccessMessage(), _transferID, _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                    } else if (_c2sTransferVO.getReceiverReturnMsg() != null && ((BTSLMessages) _c2sTransferVO.getReceiverReturnMsg()).isKey()) {
                        final BTSLMessages btslRecMessages = (BTSLMessages) _c2sTransferVO.getReceiverReturnMsg();
                        (new PushMessage(_notificationMSISDN, BTSLUtil.getMessage(_receiverLocale, btslRecMessages.getMessageKey(), btslRecMessages.getArgs()), _transferID,
                            _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                    } else {
                        (new PushMessage(_notificationMSISDN, (String) _c2sTransferVO.getReceiverReturnMsg(), _transferID, _c2sTransferVO.getRequestGatewayCode(),
                    ...
                    if (_c2sTransferVO.getReceiverReturnMsg() == null) {
                        (new PushMessage(_notificationMSISDN, getReceiverAmbigousMessage(), _transferID, _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                    } else if (_c2sTransferVO.getReceiverReturnMsg() != null && ((BTSLMessages) _c2sTransferVO.getReceiverReturnMsg()).isKey()) {
                        final BTSLMessages btslRecMessages = (BTSLMessages) _c2sTransferVO.getReceiverReturnMsg();
                        (new PushMessage(_notificationMSISDN, BTSLUtil.getMessage(_receiverLocale, btslRecMessages.getMessageKey(), btslRecMessages.getArgs()), _transferID,
                            _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                    ...
                    if (_c2sTransferVO.getReceiverReturnMsg() == null) {
                    	...
                    	final String message=getReceiverFailMessage( _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderPushMessageMsisdn, _channelUserVO
                                .getUserName(), _receiverMSISDN,_receiverLocale, PretupsErrorCodesI.FIXLINE_RECEIVER_FAIL_KEY,_requestVO.getRequestGatewayType());
                        (new PushMessage(_notificationMSISDN,message , _transferID, _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                    } else if (_c2sTransferVO.getReceiverReturnMsg() != null && ((BTSLMessages) _c2sTransferVO.getReceiverReturnMsg()).isKey()) {
                        final BTSLMessages btslRecMessages = (BTSLMessages) _c2sTransferVO.getReceiverReturnMsg();
                        (new PushMessage(_notificationMSISDN, BTSLUtil.getMessage(_receiverLocale, btslRecMessages.getMessageKey(), btslRecMessages.getArgs()), _transferID,
                            _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
            ...
            if (!BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(), _notAllowedSendMessGatw)) {
                ...
                if (btslMessages != null) {
                    ...
                    if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.ADD_COMM_SEPARATE_MSG))).booleanValue()) {
                        if (!BTSLUtil.isNullString(_c2sTransferVO.getSenderReturnMessage())) {
                            if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                                pushMessages2 = (new PushMessage(_senderPushMessageMsisdn,
                                        _c2sTransferVO.getSenderReturnMessage(), _transferID,
                                        _c2sTransferVO.getRequestGatewayCode(), _senderLocale));
                                pushMessages = (new PushMessage(_senderPushMessageMsisdn, getSenderSuccessMessage(),
                                        _transferID, _c2sTransferVO.getRequestGatewayCode(), _senderLocale));
                            }
                        ...
                        if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                            ...
                            pushMessages = (new PushMessage(_senderPushMessageMsisdn,
                                    _c2sTransferVO.getSenderReturnMessage(), _transferID,
                                    _c2sTransferVO.getRequestGatewayCode(), _senderLocale));
                        }
                    }
    ...
    public void updateForReceiverValidateResponse(String str) throws BTSLBaseException {
        ...
        if (!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) || isRequired) {
            ...
            if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
                ...
                // throw new
                // BTSLBaseException("C2SFixLineController","updateForReceiverValidateResponse",PretupsErrorCodesI.C2S_RECEIVER_FAIL,0,strArr,null);
                // throw new
                // BTSLBaseException("C2SFixLineController","updateForReceiverValidateResponse",_c2sTransferVO.getErrorCode(),0,strArr,null);
                if (InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED.equals(_receiverTransferItemVO.getValidationStatus())) {
                    throw new BTSLBaseException("C2SFixLineController", methodName, InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED + "_S", 0, strArr, null);
                } else {
                    throw new BTSLBaseException("C2SFixLineController", methodName, _c2sTransferVO.getErrorCode(), 0, strArr, null);
                }
            }
            _receiverTransferItemVO.setTransferStatus(status);
            ...
            // to set the notification msisdn,
            final String notificationMsisdn = (String) map.get("RCVR_NOTIFY_MSISDN");
            if (notificationMsisdn != null) {
                _requestVO.setNotificationMSISDN(notificationMsisdn);
            }
    ...
    public void updateForReceiverCreditResponse(String str) throws BTSLBaseException {
        ...
        try{_requestVO.setNewExpiryDate(BTSLUtil.formatDateFromOnetoAnother((String)map.get("NEW_EXPIRY_DATE"),"ddMMyyyyHHmmss", "dd/MM/yyyy HH:mm:ss"));}
        catch(Exception e){
    ...
    public void populateServiceInterfaceDetails(Connection p_con, String action) throws BTSLBaseException {
        ...
        if (!isReceiverFound) {
            throw new BTSLBaseException("C2SFixLineController", "populateServiceInterfaceDetails", PretupsErrorCodesI.C2S_ERROR_NOTFOUND_SERVICEINTERFACEMAPPING);
        }
    ...
    private void checkTransactionLoad() throws BTSLBaseException {
        ...
        try {
            ...
            if (recieverLoadStatus == 0) {
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug("C2SFixLineController[checkTransactionLoad]", "_transferID=" + _transferID + " Successfully through load");
                }
            }
            // Request in Queue
                ...
                throw new BTSLBaseException("C2SFixLineController", methodName, PretupsErrorCodesI.REQUEST_IN_QUEUE, 0, strArr, null);
            }
            // Refuse the request
            else {
                throw new BTSLBaseException("C2SFixLineController", methodName, PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
            }
        } catch (BTSLBaseException be) {
            _log.error("C2SFixLineController[checkTransactionLoad]", "Refusing request getting Exception:" + be.getMessage());
    ...
    private void processValidationRequest() throws BTSLBaseException,SQLException{
        ...
        try {
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getValExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "C2SFixLineController[processValidationRequest]",
                    _transferID, _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till validation");
                throw new BTSLBaseException("C2SFixLineController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_VAL);
            }
            ...
            if(mcomCon != null){mcomCon.close("C2SFixLineController#processValidationRequest");mcomCon=null;}
            con = null;
            ...
            if (_c2sTransferVO.isUnderProcessMsgReq() && _receiverMessageSendReq && !BTSLUtil.isStringIn(_c2sTransferVO.getRequestGatewayCode(), _notAllowedRecSendMessGatw) && !"ALL"
                .equals(_notAllowedRecSendMessGatw)) {
                (new PushMessage(_notificationMSISDN, getReceiverUnderProcessMessage(), _transferID, _c2sTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
            }
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getTopUpExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "C2SFixLineController[run]", _transferID,
                    _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till topup");
                throw new BTSLBaseException("C2SFixLineController", "run", PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO = null;
            ...
            _log.error("C2SFixLineController[processValidationRequest]", loggerValue);
            if (_transferDetailAdded) {
                if (mcomCon == null) {
                	mcomCon = new MComConnection();con=mcomCon.getConnection();
                }
    ...
    public String getReceiverCreditStr() {
        ...
        strBuff.append("&SELECTOR_BUNDLE_ID=" + _c2sTransferVO.getSelectorBundleId());
    ...
    private String getReceiverCommonString() {
        ...
        // added by ranjana
        strBuff.append("&NOTIFICATION_MSISDN=" + _notificationMSISDN);
        return strBuff.toString();
    }
    ...
    private String getReceiverSuccessMessage() {
        ...
        if (_c2sTransferVO.getReceiverTransferItemVO().isNumberBackAllowed()) {
            ...
            messageArgArray = new String[] { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverTransferValue()), String.valueOf(_receiverTransferItemVO
                ...
                .valueOf(_c2sTransferVO.getReceiverCreditBonusValidity()), _receiverMSISDN };
            if (_c2sTransferVO.getBonusTalkTimeValue() == 0) {
                key = PretupsErrorCodesI.FIXLINE_RECEIVER_GET_NUMBER_BACK_SUCCESS;
            } else {
                key = PretupsErrorCodesI.FIXLINE_RECEIVER_GET_NUMBER_BACK_SUCCESS_WITH_BONUS;
            }
        }
        ...
        if (!"N".equals(_receiverPostBalanceAvailable)) {
            ...
            messageArgArray = new String[] { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverTransferValue()), String.valueOf(_receiverTransferItemVO
                ...
                .getReceiverBonus1Validity()), String.valueOf(_c2sTransferVO.getReceiverBonus2Validity()), String.valueOf(_c2sTransferVO.getReceiverCreditBonusValidity()), _receiverMSISDN };
            if (_c2sTransferVO.getBonusTalkTimeValue() == 0) {
                key = PretupsErrorCodesI.FIXLINE_RECEIVER_SUCCESS;// return
                // BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS,messageArgArray);
            } else {
                key = PretupsErrorCodesI.FIXLINE_RECEIVER_SUCCESS_WITH_BONUS;
            }
        } else {
            if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.IS_SEPARATE_BONUS_REQUIRED))).booleanValue()) {
                _c2sTransferVO.setCalminusBonusvalue(_c2sTransferVO.getReceiverTransferValue() - _c2sTransferVO.getBonusTalkTimeValue());
            ...
            messageArgArray = new String[] { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverTransferValue()), _senderPushMessageMsisdn, PretupsBL
                ...
                .getReceiverBonus1Validity()), String.valueOf(_c2sTransferVO.getReceiverBonus2Validity()), String.valueOf(_c2sTransferVO.getReceiverCreditBonusValidity()), _receiverMSISDN };
            if (_c2sTransferVO.getBonusTalkTimeValue() == 0) {
                key = PretupsErrorCodesI.FIXLINE_RECEIVER_SUCCESS_WITHOUT_POSTBAL;// return
                // BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITHOUT_POSTBAL,messageArgArray);
            } else {
                key = PretupsErrorCodesI.FIXLINE_RECEIVER_SUCCESS_WITHOUT_POSTBAL_WITH_BONUS;
            }
        }
    ...
    private String getSenderSuccessMessage() {
        ...
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.FIXLINE_SENDER_SUCCESS, messageArgArray);
    }

    /**
     * Method to get the under process message to be sent to receiver on
     * notification number
     * 
     * @return
     */
    private String getReceiverUnderProcessMessage() {
        final String[] messageArgArray = { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverTransferValue()), String.valueOf(_receiverTransferItemVO
            ...
            .getReceiverAccessFee()), _channelUserVO.getUserName(), _receiverMSISDN };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.FIXLINE_RECEIVER_UNDERPROCESS, messageArgArray, _requestVO.getRequestGatewayType());
    }
    ...
    private String getSenderUnderProcessMessage() {
        ...
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.FIXLINE_SENDER_UNDERPROCESS, messageArgArray);
    }
    ...
    private String getSndrUPMsgBeforeValidation() {
        ...
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.FIXLINE_SENDER_UNDERPROCESS_B4VAL, messageArgArray);
    }
    ...
    private String getReceiverAmbigousMessage() {
        final String[] messageArgArray = { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderPushMessageMsisdn, _channelUserVO
            .getUserName(), _receiverMSISDN };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.FIXLINE_RECEIVER_AMBIGOUS_KEY, messageArgArray, _requestVO.getRequestGatewayType());
    }
    ...
    private void processValidationRequestInThread() throws BTSLBaseException, Exception {
        ...
        try {
        	...
        	loggerValue.append("Getting BTSL Base Exception:");
        	loggerValue.append(be.getMessage());
            _log.error("C2SFixLineController[processValidationRequestInThread]", loggerValue );
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "C2SFixLineController[processValidationRequestInThread]", _transferID, _senderMSISDN, _senderNetworkCode, loggerValue.toString());
            TransactionLog.log(_transferID, _requestIDStr, _receiverMSISDN, _receiverVO.getNetworkCode(), PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
            ...
            if (_transferID != null && !_transferDetailAdded) {
                ...
                try {
                    ...
                    if(mcomCon != null){mcomCon.close("C2SFixLineController#processValidationRequestInThread");mcomCon=null;}
                    _log.errorTrace(methodName, be);
                } catch (Exception e) {
                    if (con != null) {
                        try {
                    ...
                    EventHandler.handle(EventIDI.DATABASE_CONECTION_PROBLEM, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                        "C2SFixLineController[processValidationRequestInThread]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
                } finally {
                	if(mcomCon != null){mcomCon.close("C2SFixLineController#processValidationRequestInThread");mcomCon=null;}
                    con = null;
                }
    ...
    private boolean getInterfaceRoutingDetails(Connection p_con, String p_msisdn, long p_prefixID, String p_subscriberType, String p_networkCode, String p_serviceType, String p_interfaceCategory, String p_userType, String p_action) throws BTSLBaseException {
        ...
        try {
            ...
            if (subscriberRoutingControlVO != null) {
                        ...
                        if (listValueVO != null) {
                            ...
                            if (interfaceMappingVO1 == null) {
                                final MSISDNPrefixInterfaceMappingVO interfaceMappingVO = (MSISDNPrefixInterfaceMappingVO) MSISDNPrefixInterfaceMappingCache.getObject(
                                    p_prefixID, p_serviceType, p_action);
                ...
                // based routing
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "C2SFixLineController[getInterfaceRoutingDetails]",
        	...
        	loggerValue.append("Exception:");
        	loggerValue.append(e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[getInterfaceRoutingDetails]",
    ...
    private void performAlternateRouting(ArrayList altList) throws BTSLBaseException {
        ...
        try {
            ...
            loggerValue.append("Exception:");
            loggerValue.append(e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[performAlternateRouting]",
                _transferID, _senderMSISDN, _senderNetworkCode,  loggerValue.toString() );
    ...
    public void receiverValidateResponse(String str, int p_attempt, int p_altSize) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
            ...
            _senderTransferItemVO.setTransferStatus(PretupsErrorCodesI.REQUEST_NOT_APPLICABLE);
            strArr = new String[] { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()) };
            throw new BTSLBaseException("C2SFixLineController", "updateForReceiverValidateResponse", PretupsErrorCodesI.C2S_RECEIVER_FAIL, 0, strArr, null);
        }
    ...
    public void processFromQueue(TransferVO p_transferVO) {
        ...
        try {
            ...
            if(mcomCon != null){mcomCon.close("C2SFixLineController#processFromQueue");mcomCon=null;}
            con = null;
            ...
            if (_log.isDebugEnabled()) {
                ...
                _log.debug("C2SFixLineController[processFromQueue]",  loggerValue );
            }
            ...
            try {
            	...
            	loggerValue.append("Leaving Reciever Unmarked Base Exception:");
            	loggerValue.append(bex.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "C2SFixLineController[processFromQueue]",
                    _transferID, _senderMSISDN, _senderNetworkCode,  loggerValue.toString() );
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
                loggerValue.setLength(0);
            ...
            loggerValue.append("Exception:");
            loggerValue.append(e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[processFromQueue]", _transferID,
                _senderMSISDN, _senderNetworkCode,  loggerValue.toString() );
            // logging in the transaction log
            TransactionLog.log(_transferID, _requestVO.getRequestIDStr(), _requestVO.getFilteredMSISDN(), "", PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
                _c2sTransferVO.getSenderReturnMessage(), PretupsI.TXN_LOG_STATUS_FAIL, "Getting Code=" + _requestVO.getMessageCode());
            ...
            try {
                ...
                loggerValue.append("Exception:" );
                loggerValue.append(e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "C2SFixLineController[processFromQueue]",
                    _transferID, _senderMSISDN, _senderNetworkCode, loggerValue.toString());
            }
     ...
     * @return
     */
    private static synchronized void generateFixLineTransferID(TransferVO p_transferVO) {
        final String methodName = "generateFixLineTransferID";
        ...
        try {
            ...
            if (_transactionIDCounter == 0) {
                throw new BTSLBaseException("C2SFixLineController", "generateC2STransferID", PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
            transferID = _operatorUtil.formatFixLineRCTransferID(p_transferVO, _transactionIDCounter);
            if (transferID == null) {
                throw new BTSLBaseException("C2SFixLineController", "generateC2STransferID", PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
        ...
        if (!BTSLUtil.isNullString(p_languageCode)) {
            try {
                if (LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode) == null) {
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "C2SFixLineController[updateReceiverLocale]",
