...
/**
 * @(#)PVasPrepaidController.java
 *                                Name Date History
 *                                ----------------------------------------------
 *                                --------------------------
 *                                Rahul Dutt 11/3/2011 Initial creation
 *                                ----------------------------------------------
 *                                --------------------------
 *                                Copyright (c) 2011 Comviva Technologies Ltd.
 *                                Controller class for handling the Promo Vas
 *                                Recharge request
 */
...
public class PVasPrepaidController implements ServiceKeywordControllerI, Runnable {
    private static Log _log = LogFactory.getLog(PVasPrepaidController.class.getName());
    ...
    // private String _vasServiceList=null; //not required as suggested by gopal
    ...
    static {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[initialize]", "", "", "",
                "Exception while loading the class at the call:" + e.getMessage());
        }
    }

    public PVasPrepaidController() {
        ...
        // with gopal
        // _vasServiceList=BTSLUtil.NullToString(Constants.getProperty("VAS_SERVICE_LIST"));
    }
    ...
    public void process(RequestVO p_requestVO) {
        ...
        try {
            ...
            _operatorUtil.validatePVasRechargeRequest(con, _c2sTransferVO, p_requestVO);
            ...
            if (!_receiverVO.getSubscriberType().equals(_type)) {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PVasPrepaidController[process]", "", "", "",
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                throw new BTSLBaseException("PVasPrepaidController", "process", PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            }
            ...
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SKEY_REQUIRED)).booleanValue()) {
                ...
                if(mcomCon != null){mcomCon.close("PVasPrepaidController#process");mcomCon=null;}
                con = null;
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PVasPrepaidController[process]", _transferID,
            ...
            // raising alarm
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[process]", _transferID,
            ...
            if (con != null) {
				...
				if (mcomCon != null) {
					mcomCon.close("PVasPrepaidController#process");
					mcomCon = null;
				}
                con = null;
            }// end if
    ...
    private void processSKeyGen(Connection p_con) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[processSKeyGen]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException("PVasPrepaidController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    public void processTransfer(Connection p_con) throws BTSLBaseException {
        ...
        try {
            ...
            // generatePVASTransferID(_c2sTransferVO);//as suggested by bobba
            // sir to use same transfer id logic for VAS services
            VasPrepaidController.generateVASTransferId(_c2sTransferVO);
            ...
            _c2sTransferVO.setErrorCode(PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[processTransfer]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException("PVasPrepaidController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (PretupsI.YES.equals(_c2sTransferVO.getDifferentialAllowedForService())) {
				...
				if (mcomCon != null) {
					mcomCon.close("PVasPrepaidController#run");
					mcomCon = null;
				}
				con = null;
                ...
                try {
                    ...
                    if (_log.isDebugEnabled()) {
                        ...
                        _log.debug("PVasPrepaidController",loggerValue );
                    }
                    _log.errorTrace(methodName, be);
                    loggerValue.setLength(0);
                    loggerValue.append("Exception:" );
                    loggerValue.append( be.getMessage());
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PVasPrepaidController[run]", _c2sTransferVO
                    ...
                    if (_log.isDebugEnabled()) {
                    	...
                    	loggerValue.append(" Leaving transaction status as Under process");
                        _log.debug(
                            "PVasPrepaidController",loggerValue );
                    }
                    _log.errorTrace(methodName, e);
                }
            }// end if
                ...
                if (_c2sTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                    ...
                    try {
                        ...
                        if (sosvo != null) {
                        	...
                        	loggerValue.setLength(0);
                        	loggerValue.append("process");
                        	loggerValue.append(" No record found in database for this number :");
                        	loggerValue.append(_receiverTransferItemVO.getMsisdn());
                            _log.error(this, "PVasPrepaidController",  loggerValue);
            ...
            loggerValue.append("Exception:");
            loggerValue.append(e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[run]", _transferID,
                _senderMSISDN, _senderNetworkCode,  loggerValue.toString());
            ...
            try {
                ...
                loggerValue.append(e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PVasPrepaidController[run]", _transferID,
                    _senderMSISDN, _senderNetworkCode,  loggerValue.toString() );
            }// end catch
            ...
            if (con != null) {
                ...
                if(mcomCon != null){mcomCon.close("PVasPrepaidController#run");mcomCon=null;}
                con = null;
            }
    ...
    private void populateVOFromRequest(RequestVO p_requestVO) {
        ...
        _c2sTransferVO.setActiveUserId(_channelUserVO.getActiveUserID());// added
        // by
        // ankuj
    ...
    public void updateForReceiverValidateResponse(String str) throws BTSLBaseException {
        ...
        if (!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) || isRequired) {
            ...
            if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
                ...
                // throw new
                // BTSLBaseException("PVasPrepaidController","updateForReceiverValidateResponse",PretupsErrorCodesI.C2S_RECEIVER_FAIL,0,strArr,null);
                // throw new
                // BTSLBaseException("PVasPrepaidController","updateForReceiverValidateResponse",_c2sTransferVO.getErrorCode(),0,strArr,null);
                if (InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED.equals(_receiverTransferItemVO.getValidationStatus())) {
                    throw new BTSLBaseException("PVasPrepaidController", methodName, InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED + "_S", 0, strArr, null);
                } else {
                    throw new BTSLBaseException("PVasPrepaidController", methodName, _c2sTransferVO.getErrorCode(), 0, strArr, null);
                }
            }
            _receiverTransferItemVO.setTransferStatus(status);
    ...
    public void updateForReceiverCreditResponse(String str) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(status) || (!status.equals(InterfaceErrorCodesI.SUCCESS) && !status.equals(InterfaceErrorCodesI.AMBIGOUS))) {
            if (vasErrorMsgReqd.equals(PretupsI.YES)) {
    ...
    public void populateServiceInterfaceDetails(Connection p_con, String action) throws BTSLBaseException {
        ...
        if (!isReceiverFound) {
            throw new BTSLBaseException("PVasPrepaidController", "populateServiceInterfaceDetails", PretupsErrorCodesI.C2S_ERROR_NOTFOUND_SERVICEINTERFACEMAPPING);
        }
    ...
    private void checkTransactionLoad() throws BTSLBaseException {
        ...
        try {
            ...
            if (recieverLoadStatus == 0) {
                ...
                if (_log.isDebugEnabled()) {
                    _log.debug("PVasPrepaidController[checkTransactionLoad]", "_transferID=" + _transferID + " Successfully through load");
                }
            }
            // Request in Queue
                ...
                throw new BTSLBaseException("PVasPrepaidController", methodName, PretupsErrorCodesI.REQUEST_IN_QUEUE, 0, strArr, null);
            }
            // Refuse the request
            else {
                throw new BTSLBaseException("PVasPrepaidController", methodName, PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
            }
        } catch (BTSLBaseException be) {
            _log.error("PVasPrepaidController[checkTransactionLoad]", "Refusing request getting Exception:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.errorTrace(methodName, e);
            throw new BTSLBaseException("PVasPrepaidController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION);
    ...
    private void processValidationRequest() throws BTSLBaseException, SQLException {
        ...
        try {
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getValExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PVasPrepaidController[processValidationRequest]",
                    _transferID, _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till validation");
                throw new BTSLBaseException("PVasPrepaidController", methodName, PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_VAL);
            }
            ...
            if (_c2sTransferVO.getRequestedAmount() == 0) {
                ...
                if (cardGrpDetails.size() > 1) {
                	int cardsGrpDetails=cardGrpDetails.size();
                    for (int i = 0; i < cardsGrpDetails; i++) {
                ...
                _c2sTransferVO.setRequestedAmount(cardGroupDetailsVO.getStartRange());
                _c2sTransferVO.setTransferValue(cardGroupDetailsVO.getStartRange());
            }
            ...
            if(mcomCon != null){mcomCon.close("PVasPrepaidController#processValidationRequest");mcomCon=null;}
            con = null;
            ...
            if ((System.currentTimeMillis() - _c2sTransferVO.getRequestStartTime()) > interfaceVO.getTopUpExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PVasPrepaidController[run]", _transferID,
                    _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till topup");
                throw new BTSLBaseException("PVasPrepaidController", "run", PretupsErrorCodesI.C2S_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO = null;
            ...
            loggerValue.append(be.getMessage());
            _log.error("PVasPrepaidController[processValidationRequest]", loggerValue );
            if (_transferDetailAdded) {
                if (mcomCon == null) {
                	mcomCon = new MComConnection();con=mcomCon.getConnection();
                }
        ...
        if (!("N".equals(_c2sTransferVO.getBoth()) && "N".equals(_c2sTransferVO.getOnline()))) {
            ...
            if (_c2sTransferVO.getReceiverTransferItemVO().isNumberBackAllowed()) {
                ...
                if (_c2sTransferVO.getBonusTalkTimeValue() == 0) {
                    key = PretupsErrorCodesI.PVAS_RECEIVER_GET_NUMBER_BACK_SUCCESS;
                } else {
                    key = PretupsErrorCodesI.PVAS_RECEIVER_GET_NUMBER_BACK_SUCCESS_WITH_BONUS;
                }
            }
            ...
            if (!"N".equals(_receiverPostBalanceAvailable)) {
                ...
                if (!_receiverTransferItemVO.getPostValidationStatus().equals(InterfaceErrorCodesI.SUCCESS)) {
                    if ((_c2sTransferVO.getBonusTalkTimeValue() == 0)) {
                        key = PretupsErrorCodesI.PVAS_RECEIVER_SUCCESS_WITHOUT_POSTBAL;// return
                        // BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITHOUT_POSTBAL,messageArgArray);
                    } else {
                        key = PretupsErrorCodesI.PVAS_RECEIVER_SUCCESS_WITHOUT_POSTBAL_WITH_BONUS;
                    }
                } else {
                    if ((_c2sTransferVO.getBonusTalkTimeValue() == 0)) {
                        key = PretupsErrorCodesI.PVAS_RECEIVER_SUCCESS;// return
                        // BTSLUtil.getMessage(_receiverLocale,PretupsErrorCodesI.C2S_RECEIVER_SUCCESS_WITHOUT_POSTBAL,messageArgArray);
                    } else {
                        key = PretupsErrorCodesI.PVAS_RECEIVER_SUCCESS_WITH_BONUS;
                    }
                }
            } else {
                if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.IS_SEPARATE_BONUS_REQUIRED))).booleanValue()) {
    ...
    private String getSenderSuccessMessage() {
        ...
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.PVAS_SENDER_SUCCESS, messageArgArray);
    }
    ...
    private String getReceiverUnderProcessMessage() {
        final String[] messageArgArray = { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getReceiverTransferValue()), String.valueOf(_receiverTransferItemVO
            ...
            .getUserName() };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.PVAS_RECEIVER_UNDERPROCESS, messageArgArray, _requestVO.getRequestGatewayType());
    }
    ...
    private String getSenderUnderProcessMessage() {
        ...
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.PVAS_SENDER_UNDERPROCESS, messageArgArray);
    }
    ...
    private String getSndrUPMsgBeforeValidation() {
        ...
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.PVAS_SENDER_UNDERPROCESS_B4VAL, messageArgArray);
    }
    ...
    private String getReceiverAmbigousMessage() {
        ...
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.PVAS_RECEIVER_AMBIGOUS_KEY, messageArgArray, _requestVO.getRequestGatewayType());
    }

    private String getReceiverFailMessage() {
        final String[] messageArgArray = { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()), _senderMSISDN, _channelUserVO.getUserName() };
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.PVAS_RECEIVER_FAIL_KEY, messageArgArray, _requestVO.getRequestGatewayType());
    }
    ...
    private void processValidationRequestInThread() throws BTSLBaseException, Exception {
        ...
        try {
        	...
        	loggerValue.append("Getting BTSL Base Exception:");
        	loggerValue.append( be.getMessage());
            _log.error("PVasPrepaidController[processValidationRequestInThread]",  loggerValue);
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PVasPrepaidController[processValidationRequestInThread]", _transferID, _senderMSISDN, _senderNetworkCode,  loggerValue.toString() );
            TransactionLog.log(_transferID, _requestIDStr, _receiverMSISDN, _receiverVO.getNetworkCode(), PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
            ...
            if (_transferID != null && !_transferDetailAdded) {
                ...
                try {
                    ...
                    if(mcomCon != null){mcomCon.close("PVasPrepaidController#processValidationRequestInThread");mcomCon=null;}
                    con=null;
                    _log.errorTrace(methodName, be);
                } catch (Exception e) {
                    if (con != null) {
    ...
    private boolean getInterfaceRoutingDetails(Connection p_con, String p_msisdn, long p_prefixID, String p_subscriberType, String p_networkCode, String p_serviceType, String p_interfaceCategory, String p_userType, String p_action) throws BTSLBaseException {
        ...
        try {
            ...
            loggerValue.append("Exception:");
            loggerValue.append(e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[getInterfaceRoutingDetails]",
    ...
    private void performAlternateRouting(ArrayList altList) throws BTSLBaseException {
        ...
        try {
            ...
            loggerValue.append("Exception:");
            loggerValue.append( e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[performAlternateRouting]",
                _transferID, _senderMSISDN, _senderNetworkCode,  loggerValue.toString());
    ...
    public void receiverValidateResponse(String str, int p_attempt, int p_altSize) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
            ...
            _senderTransferItemVO.setTransferStatus(PretupsErrorCodesI.REQUEST_NOT_APPLICABLE);
            strArr = new String[] { _transferID, PretupsBL.getDisplayAmount(_c2sTransferVO.getRequestedAmount()) };
            throw new BTSLBaseException("PVasPrepaidController", "updateForReceiverValidateResponse", PretupsErrorCodesI.C2S_RECEIVER_FAIL, 0, strArr, null);
        }
    ...
    public void processFromQueue(TransferVO p_transferVO) {
        ...
        try {
			...
			if (mcomCon != null) {
				mcomCon.close("PVasPrepaidController#processFromQueue");
				mcomCon = null;
			}
			con = null;
            ...
            if (_log.isDebugEnabled()) {
                ...
                _log.debug("PVasPrepaidController[processFromQueue]", loggerValue );
            }
        	...
        	if(mcomCon != null){mcomCon.close("PVasPrepaidController#processFromQueue");mcomCon=null;}
            con = null;
            try {
                if (_receiverVO != null && _receiverVO.isUnmarkRequestStatus()) { // getting
                    // database
                ...
                loggerValue.append("Leaving Reciever Unmarked Base Exception:");
                loggerValue.append(bex.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PVasPrepaidController[processFromQueue]",
                    _transferID, _senderMSISDN, _senderNetworkCode,  loggerValue.toString());
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
                loggerValue.setLength(0);
            ...
            loggerValue.append("Exception:");
            loggerValue.append(e.getMessage());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[processFromQueue]", _transferID,
                _senderMSISDN, _senderNetworkCode,  loggerValue.toString() );
            // logging in the transaction log
            TransactionLog.log(_transferID, _requestVO.getRequestIDStr(), _requestVO.getFilteredMSISDN(), "", PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
                _c2sTransferVO.getSenderReturnMessage(), PretupsI.TXN_LOG_STATUS_FAIL, "Getting Code=" + _requestVO.getMessageCode());
            ...
            try {
                ...
                loggerValue.append("Exception:");
                loggerValue.append( e.getMessage());
                EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PVasPrepaidController[processFromQueue]",
                    _transferID, _senderMSISDN, _senderNetworkCode,  loggerValue.toString());
            }
        ...
        if (!BTSLUtil.isNullString(p_languageCode)) {
            try {
                if (LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode) == null) {
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PVasPrepaidController[updateReceiverLocale]",
     ...
     * throw new
     * BTSLBaseException("PVasPrepaidController","generateC2STransferID"
     ...
     */
    /*
     * private static synchronized void generatePVASTransferID(TransferVO
     * p_transferVO)
     * {
     * 
     * String transferID=null;
     ...
     * if(((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SELECTOR_INTERFACE_MAPPING)).booleanValue())
     * {
     * transferID=_operatorUtil.formatVASPVASTransferID(p_transferVO,
     * _transactionIDCounter,_pvasPrefix);
     * }
     * if(transferID==null)
     * throw new
     * BTSLBaseException("PVasPrepaidController","generateC2STransferID"
