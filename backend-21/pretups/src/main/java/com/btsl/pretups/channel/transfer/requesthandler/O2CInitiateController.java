 ...
 * 
 * @author pankaj.rawat
 * This controller initiates an O2C stock request(multi-product) by a channel user.
 * Also lists major errors(if any) in the response
 */

@io.swagger.v3.oas.annotations.tags.Tag(name = "${O2CInitiateController.name}", description = "${O2CInitiateController.desc}")//@Api(tags = "O2C Services", defaultValue = "Initiate O2C Request for Stock")
@RestController
@RequestMapping(value = "/v1/o2c")

public class O2CInitiateController {
	private static Log _log = LogFactory.getLog(O2CInitiateController.class.getName());
	public static OperatorUtilI _operatorUtil = null;
	@SuppressWarnings("unchecked")
	@PostMapping(value = "/stockInitiate", consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
	@ResponseBody
	...
	*/
	...
	@io.swagger.v3.oas.annotations.Operation(summary = "${stockInitiate.summary}", description="${stockInitiate.description}",
	 ...
	 * @throws IOException
	 */
	public BaseResponseMultiple initiateStockO2CRequest(@Parameter(hidden = true) @RequestHeader MultiValueMap<String, String> headers,
			HttpServletResponse response,
			@RequestBody O2CInitiateRequestVO o2CInitiateRequestVO) 
			throws BTSLBaseException, SQLException, JsonParseException,JsonMappingException, IOException
	{
		final String METHOD_NAME = "initiateStockO2CRequest";
		...
		try {
			...
			o2CInitiateRequestVO.setData(new OAuthUserData());
			OAuthenticationUtil.validateTokenApi(o2CInitiateRequestVO, headers, apiResponse); 
			
			final String loginID = o2CInitiateRequestVO.getData().getLoginid();
			final String msisdn = o2CInitiateRequestVO.getData().getMsisdn();
			final String type = new UserDAO().loadUserDetailsCompletelyByMsisdn(con, msisdn).getUserType();
			ChannelUserVO receiverUserVO = null;
			if(type.equals(PretupsI.STAFF_USER_TYPE)) {
				 receiverUserVO = (ChannelUserVO) ((new ChannelUserDAO()).loadStaffUserDetailsByLoginId(con, loginID));	
			}
			else {
				receiverUserVO = (ChannelUserVO) ((new ChannelUserDAO()).loadChannelUserDetails(con, msisdn));	
			}

			final ChannelTransferRuleVO channelTransferRuleVO = new ChannelTransferRuleDAO().loadTransferRule(con, receiverUserVO.getNetworkID(), receiverUserVO.getDomainID(),
            ...
            String errorArguements= receiverUserVO.getUserName();
            if (receiverStatusVO == null) {
				MasterErrorList masterErrorList = new MasterErrorList();
			...
			if (receiverUserVO.getCommissionProfileApplicableFrom().after(curDate)) {
				MasterErrorList masterErrorList = new MasterErrorList();
			...
			//profile validations
				...
				errorArgs = new String[] { errorArguements, receiverUserVO.getCommissionProfileLang2Msg() };
                ...
                if (PretupsI.LANG1_MESSAGE.equals(localeVO.getMessage())) {
                	errorArgs = new String[] { errorArguements, receiverUserVO.getCommissionProfileLang1Msg() };
                }
                MasterErrorList masterErrorList = new MasterErrorList();
				String msg = RestAPIStringParser.getMessage(locale, "commissionprofile.notactive.msg", errorArgs);
            	...
            	errorArgs = new String[] { errorArguements };
                MasterErrorList masterErrorList = new MasterErrorList();
				String msg = RestAPIStringParser.getMessage(locale, "transferprofile.notactive.msg", errorArgs);
				masterErrorList.setErrorCode(PretupsErrorCodesI.CHNL_ERROR_RECEIVER_NOTALLOWED);
				masterErrorList.setErrorMsg(msg);
				masterErrorListMain.add(masterErrorList);
			}
			...
			if (receiverUserVO.getInSuspend() != null && PretupsI.USER_TRANSFER_IN_STATUS_SUSPEND.equals(receiverUserVO.getInSuspend())) {
				MasterErrorList masterErrorList = new MasterErrorList();
				String msg = RestAPIStringParser.getMessage(locale, "message.channeltransfer.transfernotallowed.userinsuspend", errorArgs);
				masterErrorList.setErrorCode(PretupsErrorCodesI.CHNL_ERROR_RECEIVER_NOTALLOWED);
				masterErrorList.setErrorMsg(msg);
				masterErrorListMain.add(masterErrorList);
			}
			...
			if (barredUserDAO.isExists(con, PretupsI.C2S_MODULE, networkPrefixVO.getNetworkCode(), msisdn,
				...
				throw new BTSLBaseException(this, METHOD_NAME,PretupsErrorCodesI.ERROR_USER_TRANSFER_CHANNEL_RECEIVER_BAR , new String[] {msisdn});
			}
			...
			ArrayList<O2cInitiateResponseVO> baseResponseFinalSucess = new ArrayList<>();
			p_requestVO.setServiceType(_serviceType);
			p_requestVO.setRequestGatewayType(o2CInitiateRequestVO.getReqGatewayType());
			p_requestVO.setLocale(locale);
			
			List<O2CInitiateReqData> dataSets = o2CInitiateRequestVO.getO2CInitiateReqData();
			...
			try
			{
				...
				final ArrayList prodTypListtemp = receiverUserVO.getAssociatedProductTypeList();
...
//					BaseResponse baseResponse = new BaseResponse();
					...
					O2CInitiateReqData dataSet = dataSets.get(i);
					ChannelTransferVO channelTransferVO = new ChannelTransferVO();
					List<PaymentDetailsO2C> paymentDetails = (BTSLUtil.isNullorEmpty(dataSet.getPaymentdetails())) ? null: dataSet.getPaymentdetails();
					...
					if (userEventRemarksEnabled != null && userEventRemarksEnabled) {
						if (remarks == null) {
							MasterErrorList masterErrorList = new MasterErrorList();
							...
							masterErrorLists.add(masterErrorList);
							isContinue = true;
						} else if (remarks.length() > 100) { // db won't allow to store remarks length more than 100
							MasterErrorList masterErrorList = new MasterErrorList();
					...
					if (userPhoneVO.getPinRequired().equals(PretupsI.YES)) {
						try {
							ChannelUserBL.validatePIN(con, receiverUserVO, pin);
					...
					if(!isContinue)
					{
						try
						{
							...
							if(!PretupsI.PAYMENT_INSTRUMENT_TYPE_ONLINE.equalsIgnoreCase(paymentDetails.get(0).getPaymenttype())) {
							...
							prepareChannelTransferVO(con,p_requestVO, channelTransferVO, new Date(), receiverUserVO, channelTransferVO.getChannelTransferitemsVOList(), receiverUserVO);
						}
						catch(BTSLBaseException be)
						{
		          ...
		          //For RazorPay Gateway
		            if(PretupsI.PAYMENT_INSTRUMENT_TYPE_ONLINE.equals(channelTransferVO.getPayInstrumentType())) {
		            	String gatewayOrderId = RazorPay.createNewOrder(con, channelTransferVO);
		            ...
		            channelTransferVO.setSource(PretupsI.GATEWAY_TYPE_WEB);
		            ...
		            o2cInitiateResponseVO.setTransactionId(channelTransferVO.getTransferID());
				...
				return apiResponse;
					...
					EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
							"O2CInitiateController " + METHOD_NAME, "", "", "", "Exception:" + e.getMessage());
					...
					if (mcomCon != null) {
						mcomCon.close("O2CInitiateController#" + METHOD_NAME);
						mcomCon = null;
					}
					if (_log.isDebugEnabled()) {
	 ...
	 * @throws BTSLBaseException
	 */
	private void prepareChannelTransferVO(Connection con,RequestVO p_requestVO, ChannelTransferVO p_channelTransferVO, Date p_curDate, ChannelUserVO p_channelUserVO, ArrayList p_prdList, UserVO p_userVO) throws BTSLBaseException {
		...
		if(PretupsI.USER_TYPE_STAFF.equalsIgnoreCase(p_channelUserVO.getUserType())){
			ChannelUserVO parentUserVO = (ChannelUserVO) ((new ChannelUserDAO()).loadChannelUserByUserID(con, p_channelUserVO.getParentID()));
			p_channelTransferVO.setToUserID(parentUserVO.getUserID());
			p_channelTransferVO.setTransferInitatedBy(parentUserVO.getUserID());
		}
	 ...
	 * @return
	 */
	private boolean validatePaymentDetails(List<PaymentDetailsO2C> paymentDetails, RowErrorMsgLists rowErrorMsgLists)
	{
		StringBuilder loggerValue= new StringBuilder(); 
    	if (_log.isDebugEnabled()) {
    		loggerValue.setLength(0);
		...
		for(int i=0;i<paymentDetails.size();i++)	
		{
			...
			if(!BTSLUtil.isNullString(paymentDetails.get(0).getPaymenttype()) 
					&& PretupsI.PAYMENT_INSTRUMENT_TYPE_ONLINE.equalsIgnoreCase(paymentDetails.get(0).getPaymenttype()))
			{
				if(BTSLUtil.isNullString(paymentDetails.get(0).getPaymentgatewayType()))
				{
					error=true;
					MasterErrorList masterErrorListssPayment = new MasterErrorList();
					masterErrorListssPayment.setErrorCode(PretupsErrorCodesI.EXTSYS_BLANK);
					masterErrorListssPayment.setErrorMsg(RestAPIStringParser.getMessage(locale, PretupsErrorCodesI.EXTSYS_BLANK, new String[] {XmlTagValueConstant.TAG_PAYMENTGATEWAY}));
					masterErrorLists1Payment.add(masterErrorListssPayment);
				}
				boolean isPaymentGatewayValid = false; 
				for(ListValueVO lvo : paymentGatewayList)
				{	if(lvo.getLabel().equalsIgnoreCase(paymentDetails.get(0).getPaymentgatewayType()))
					{	
				...
				{
					MasterErrorList masterErrorListssPayment = new MasterErrorList();
					masterErrorListssPayment.setErrorCode(PretupsErrorCodesI.INVALID_PAYMENT_GATEWAY);
					masterErrorListssPayment.setErrorMsg(RestAPIStringParser.getMessage(locale, PretupsErrorCodesI.INVALID_PAYMENT_GATEWAY, new String[] {XmlTagValueConstant.TAG_PAYMENTGATEWAY}));
					masterErrorLists1Payment.add(masterErrorListssPayment);
				}
			}
			//payment date will be autopopulated in case of online payment
			if(!PretupsI.PAYMENT_INSTRUMENT_TYPE_ONLINE.equalsIgnoreCase(paymentDetails.get(0).getPaymenttype()) && BTSLUtil.isNullString(paymentDetails.get(0).getPaymentdate()))
			{
				error=true;
				MasterErrorList masterErrorListssPayment = new MasterErrorList();
			...
			{
				if(!PretupsI.PAYMENT_INSTRUMENT_TYPE_ONLINE.equalsIgnoreCase(paymentDetails.get(0).getPaymenttype()) && !BTSLUtil.isValidDatePattern(paymentDetails.get(0).getPaymentdate()))
				{
					error=true;
					MasterErrorList masterErrorListssPayment = new MasterErrorList();
