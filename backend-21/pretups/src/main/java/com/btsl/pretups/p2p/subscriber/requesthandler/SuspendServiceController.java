        ...
        try {
            ...
            case (PretupsI.MESSAGE_LENGTH_SUSPEND - 1): {
                if ((((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.CP2P_PIN_VALIDATION_REQUIRED)).booleanValue()) && !(BTSLUtil.encryptText((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_DEFAULT_SMSPIN)).equals(actualPin))) {
                    throw new BTSLBaseException(this, "process", PretupsErrorCodesI.P2P_ERROR_SUSPEND_SERVCE_INVALIDMESSAGEFORMAT, 0, new String[] { p_requestVO
            ...
            if (count > 0) {
            	mcomCon.finalCommit();
                p_requestVO.setMessageCode(PretupsErrorCodesI.USER_STATUS_SUSPEND_SUCCESS);
            } else {
            	mcomCon.finalRollback();
                throw new BTSLBaseException(this, "process", PretupsErrorCodesI.USER_STATUS_SUSPEND_FAILED);
            }
        } catch (BTSLBaseException be) {
            try {
                if (con != null) {
            ...
            if (be.isKey()) {
                ...
                p_requestVO.setMessageCode(PretupsErrorCodesI.USER_STATUS_SUSPEND_FAILED);
            }
        } catch (Exception e) {
            try {
                if (con != null) {
            ...
            _log.errorTrace(METHOD_NAME, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "SuspendServiceController[process]", p_requestVO
                .getFilteredMSISDN(), "", "", "Exception while suspending services:" + e.getMessage());
            p_requestVO.setMessageCode(PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
			if (mcomCon != null) {
				mcomCon.close("SuspendServiceController#process");
				mcomCon = null;
			}
            if (_log.isDebugEnabled()) {
