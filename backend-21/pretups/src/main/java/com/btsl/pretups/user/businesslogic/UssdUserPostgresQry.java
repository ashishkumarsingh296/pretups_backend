...
public class UssdUserPostgresQry implements UssdUserQry{
	...
	public String loadUsersDetailsQry() {
		...
		strBuff.append(" FROM users PRNT_USR right join users USR on USR.parent_id=PRNT_USR.user_id left join categories ");
		strBuff.append(" PRNT_CAT on PRNT_USR.category_code=PRNT_CAT.category_code ,users ONR_USR,categories USR_CAT, ");
	...
	public PreparedStatement loadDefaultGeographyQry(Connection con, String parentMsisdn, String childCatgCode) throws SQLException {
		final StringBuilder strBuff = new StringBuilder(" SELECT X.grph_domain_code,X.grph_domain_type FROM (" );

		strBuff.append(" WITH RECURSIVE q AS (");
		strBuff.append(" SELECT GD1.grph_domain_code,GD1.grph_domain_type ,GD1.status,GD1.IS_DEFAULT FROM GEOGRAPHICAL_DOMAINS GD1 WHERE  ");
		strBuff.append(" GD1.grph_domain_code =(SELECT grph_domain_code FROM USER_GEOGRAPHIES ug1");
		strBuff.append(" WHERE UG1.user_id=(SELECT user_id FROM USERS WHERE msisdn=? AND status=? AND user_type='CHANNEL'))");
		strBuff.append(" UNION ALL");
		strBuff.append(" SELECT GD2.grph_domain_code,GD2.grph_domain_type,GD2.status,GD2.IS_DEFAULT FROM GEOGRAPHICAL_DOMAINS GD2");
		strBuff.append(" join q on q.grph_domain_code = GD2.parent_grph_domain_code");
		strBuff.append(" )");
		strBuff.append(" SELECT grph_domain_code,grph_domain_type FROM  q  WHERE  status =? AND IS_DEFAULT=? ");
		...
		pstmtSelect.setString(1, parentMsisdn);
