...
public class LoyaltyPointsRedemptionOracleQry implements LoyaltyPointsRedemptionQry{
	...
	public String loaduserProfileRelatedDetailsQry(){
		 ...
		 selectQueryBuff.append(" SELECT psv.OPT_CONTRIBUTION ,psv.PRT_CONTRIBUTION,u.PARENT_ID,psv.set_id,psv.version, up.country, up.phone_language lang ");
			...
			selectQueryBuff.append("WHERE (psv.applicable_from=(SELECT MAX(psv.applicable_from) FROM PROFILE_SET_VERSION psv, profile_details pd WHERE psv.applicable_from<=SYSDATE AND psv.set_id=ch.lms_profile and pd.set_id=psv.set_id AND pd.PRODUCT_CODE = ?) ) AND psv.status='Y' and psv.SET_ID=ch.lms_profile)");
