...
public class TransferEnquiryOracleQry implements TransferEnquiryQry{

	private Log log = LogFactory.getLog(TransferEnquiryOracleQry.class.getName());
	...
	public PreparedStatement loadLastXTransfersOldQryIfC2S(Connection con,
		...
		strBuff
		.append("  select CS.transfer_id, CS.transfer_date_time,CS.SUBS_SID, CS.transfer_status, CS.transfer_value net_payable_amount, CTI.post_balance, CS.receiver_msisdn, ");
		strBuff.append("  CS.transfer_date_time created_on, CS.service_type service, ST.name, 'C2S' type, KV.value statusname,CTI.sno,p.SHORT_NAME ");
		strBuff.append("  FROM C2S_TRANSFERS_OLD CS, C2S_TRANSFER_ITEMS CTI,key_values KV,SERVICE_TYPE ST,products p  where ");
		...
		strBuff.append("  AND CTI.sno = (select max(cti1.sno) from C2S_TRANSFER_ITEMS cti1 where cti1.user_type= ? and CS.transfer_id=CTI1.TRANSFER_ID) ");
		strBuff.append("  AND CS.transfer_status=KV.key(+) AND KV.type(+)=? AND CS.service_type=ST.service_type AND cs.PRODUCT_CODE=p.PRODUCT_CODE ");
	...
	@Override
	public PreparedStatement loadLastXTransfersOldQryIfC2C(Connection p_con,
			String recMsisdn, String p_user_id, int p_noLastTxn, int noDays, String service, java.sql.Date txnDate )
					throws SQLException {
		...
		strBuff.append(" CT.type, CT.transfer_sub_type, CTI.sender_previous_stock, CTI.receiver_previous_stock , CTI.approved_quantity, KV2.value statusname FROM LOOKUPS LK,KEY_VALUES KV2 ,CHANNEL_TRANSFERS CT, CHANNEL_TRANSFERS_ITEMS CTI WHERE CT.ACTIVE_USER_ID=? ");
		if (isRecentDate(noDays,txnDate)) {
		...
		int i = 0;
		PreparedStatement pstmt1 = p_con.prepareStatement(sqlSelect1);
	...
	public PreparedStatement loadLastXTransfersOldQryIfO2C(Connection con,
		...
		strBuff
		.append(" SELECT CT.transfer_id, CT.CLOSE_DATE, CT.status, CT.transfer_sub_type,CTI.approved_quantity, CTI.sender_previous_stock, CTI.receiver_previous_stock, CT.msisdn, CT.to_msisdn, CT.created_on, CT.transfer_type service, LK.lookup_name name, ");
		strBuff
		.append(" CT.type, KV2.value statusname FROM LOOKUPS LK,KEY_VALUES KV2 ,CHANNEL_TRANSFERS CT, CHANNEL_TRANSFERS_ITEMS CTI WHERE (CT.to_user_id = ? OR CT.from_user_id = ? ) ");
		if (isRecentDate(noDays,txnDate)) {
			strBuff.append(" AND CT.transfer_date >= ? ");
		}
		if (aa.length == 2) {
		...
		if (log.isDebugEnabled()) {
			log.debug(methodName, "QUERY sqlSelect=" + sqlSelect2);
		}
		int  i = 0;
	...
	public PreparedStatement loadLastXTransfersNewQryIfC2S(Connection p_con,
		...
		String methodName = "loadLastXTransfersNewQryIfC2S";
		...
		strBuff.append("  CS.transfer_date_time created_on, CS.service_type service, ST.name, 'C2S' type, KV.value statusname,p.SHORT_NAME ");
		strBuff.append("  from C2S_TRANSFERS CS,key_values KV,SERVICE_TYPE ST,products p  where ");
		if (isRecentDate(noDays,txnDate)) {
			strBuff.append(" CS.transfer_date >= ? AND ");
		}
	...
	public PreparedStatement loadLastXTransfersNewQryIfC2C(Connection con,
			String recMsisdn, String userId, int noLastTxn, int noDays,
			String service, java.sql.Date txnDate) throws SQLException {
		String methodName = "loadLastXTransfersNewQryIfC2C";
		final Date differenceDate = BTSLUtil.getDifferenceDate(new Date(), -noDays); 
		final String aa[] = service.split(":");
		...
		strBuff.append( "FROM LOOKUPS LK,KEY_VALUES KV2 ,CHANNEL_TRANSFERS CT, CHANNEL_TRANSFERS_ITEMS CTI WHERE (CT.ACTIVE_USER_ID=? OR CT.TO_USER_ID=?) ");
		if (isRecentDate(noDays,txnDate)) {
			strBuff.append(" AND CT.transfer_date >= ? ");
		}
		if (aa.length == 2) {
	...
	public PreparedStatement loadLastXTransfersNewQryIfO2C(Connection con,
		...
		String lastxChnlTxnstatusAllowed = (String)PreferenceCache.getSystemPreferenceValue(PreferenceI.LAST_X_CHNL_TXNSTATUS_ALLOWED);
         ...
         strBuff.append(" CT.type, KV2.value statusname FROM LOOKUPS LK,KEY_VALUES KV2 ,CHANNEL_TRANSFERS CT, CHANNEL_TRANSFERS_ITEMS CTI WHERE (CT.to_user_id = ? OR CT.from_user_id = ? ) ");
         if (isRecentDate(noDays,txnDate)) {
             strBuff.append(" AND CT.transfer_date >= ? ");
         }
         if (aa.length == 2) {
