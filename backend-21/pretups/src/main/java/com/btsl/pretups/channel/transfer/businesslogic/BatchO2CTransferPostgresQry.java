...
public class BatchO2CTransferPostgresQry  implements BatchO2CTransferQry{
	...
	public String loadBatchO2CMasterDetailsQry(String[] itemStatus, String currentLevel) {
		...
		strBuff
		.append(" GROUP BY fb.batch_id,fb.batch_name,fb.batch_total_record, p.product_name,p.unit_value,fb.network_code,fb.network_code_for,fb.product_code, fb.modified_by, fb.modified_on,fb.sms_default_lang,fb.sms_second_lang,p.product_type,fbg.geography_code,p.short_name ,fb.domain_code,fb.batch_date ORDER BY fb.batch_date DESC ) X ");
	...
	public String loadBatchO2CMasterDetailsQry(String batchid, String msisdn,
		...
		strBuff.append(" FROM user_geographies ug , foc_batch_geographies fbg,foc_batches fb,foc_batch_items fbi,products p,user_domains ud,  user_product_types upt, geographical_domains GRPD ");
		...
		strBuff.append(" AND upt.product_type=p.product_type AND fb.transfer_type=? AND fb.transfer_sub_type=? AND fbg.batch_id=fb.batch_id AND fb.product_code=p.product_code AND fb.batch_id=fbi.batch_id ");
	...
	public String loadO2CTransferApprovalListQry(String currentLevel, String itemStatus, String categoryCode, String geoDomain, String domainCode) {
		...
		strBuff
		.append(" SELECT * FROM (SELECT DISTINCT fb.batch_id,fb.batch_name,fb.batch_total_record, p.product_name,p.short_name,p.unit_value,sum(case fbi.status when ? then 1 else 0 end ) as new, ");
		strBuff.append(" sum(case fbi.status when ? then 1 else 0 end ) appr1,sum(case fbi.status when ? then 1 else 0 end ) cncl,  ");
		strBuff.append(" sum(case fbi.status when ? then 1 else 0 end ) appr2,sum(case fbi.status when ? then 1 else 0 end ) closed,  ");
		strBuff
	...
	public String loadBatchTransferApprovalDetails(String itemStatus,String approvalLevel) {
		StringBuilder strBuff = new  StringBuilder();
		strBuff.append(" SELECT * FROM ( SELECT DISTINCT fb.batch_id, fb.batch_name,p.product_code, fb.batch_total_record, fb.BATCH_FILE_NAME , p.product_name,p.product_code,fb.batch_date,");
		strBuff.append("sum(case fbi.status when ? then 1 else 0 end ) as new, ");
		strBuff.append(" sum(case fbi.status when ? then 1 else 0 end ) appr1,sum(case fbi.status when ? then 1 else 0 end ) cncl,  ");
		strBuff.append(" sum(case fbi.status when ? then 1 else 0 end ) appr2,sum(case fbi.status when ? then 1 else 0 end ) closed,  ");
		...
		.append(" AND (SELECT count(fbg1.geography_code) FROM foc_batch_geographies fbg1 WHERE fbg1.batch_id=fbg.batch_id) <= (SELECT count(ug1.user_id) FROM user_geographies ug1 ,geographical_domains gd WHERE ug1.user_id=ug.user_id AND ug1.grph_domain_code=gd.grph_domain_code AND gd.status='Y') ");
		strBuff
		.append(" GROUP BY p.product_code,fb.batch_id,fb.batch_name,fb.batch_total_record, fbi.first_approver_remarks ,fb.sms_default_lang , fb.sms_second_lang ,fbi.second_approver_remarks,p.product_name,p.unit_value,fb.network_code,fb.network_code_for,fb.product_code, fb.modified_by, fb.modified_on,fb.sms_default_lang,fb.sms_second_lang,p.product_type,fbg.geography_code,p.short_name ,fb.domain_code,fb.batch_date ,fbi.initiator_remarks, fb.batch_file_name ORDER BY fb.batch_date DESC ) qry");
		if (PretupsI.CHANNEL_TRANSFER_ORDER_NEW.equals(approvalLevel)) {
			strBuff.append(" WHERE  new>0 ");
		} else if(PretupsI.CHANNEL_TRANSFER_ORDER_APPROVE1.equals(approvalLevel)){
			strBuff.append(" WHERE  appr1>0 ");
	...
	public String loadBatchWithdrawalorFOCApprovalDetails(String itemStatus,String approvalLevel) {
		StringBuilder strBuff = new  StringBuilder();
		strBuff.append(" SELECT * FROM ( SELECT DISTINCT fb.batch_id, fb.batch_name,p.product_code, fb.batch_date,fb.batch_total_record, fb.BATCH_FILE_NAME ,fb.batch_date, p.product_name,fbi.bonus_type,");
		strBuff.append("sum(case fbi.status when ? then 1 else 0 end ) as new, ");
		strBuff.append(" sum(case fbi.status when ? then 1 else 0 end ) appr1,sum(case fbi.status when ? then 1 else 0 end ) cncl,  ");
		strBuff.append(" sum(case fbi.status when ? then 1 else 0 end ) appr2,sum(case fbi.status when ? then 1 else 0 end ) closed,  ");
		strBuff.append(" fbi.initiator_remarks,fbi.first_approver_remarks ,fbi.second_approver_remarks ,fb.sms_default_lang , fb.sms_second_lang ");
	...
	public String loadBatchO2CItemsMapQryFOCorWithdrawal(String itemStatus) {
		 final StringBuilder strBuff = new StringBuilder();
	        ...
	        strBuff.append(" fbi.commission_type, fbi.commission_rate, fbi.commission_value, fbi.tax1_type, ");
	        strBuff.append(" fbi.tax1_rate, fbi.tax1_value, fbi.tax2_type, fbi.tax2_rate, fbi.tax2_value, ");
	        strBuff.append(" fbi.tax3_type, fbi.tax3_rate, fbi.tax3_value ,fbi.dual_comm_type,fbi.bonus_type ");
	        strBuff.append(" FROM foc_batch_items fbi  left join  users fapp on fbi.first_approved_by = fapp.user_id left join users sapp on fbi.second_approved_by = sapp.user_id ,foc_batches fb left join users intu on fb.created_by = intu.user_id,categories c,channel_grades cg, users u");
