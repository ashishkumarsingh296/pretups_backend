...
package com.btsl.pretups.inter.cache;
...
public class FileCache  implements Runnable{
    ...
    public void run() {
 		try {
 			...
 			_log.error("loadAtStartUp init() Exception ", e);
 		}
 	}
    /**
	...
	private static HashMap loadInterfaceDetail() {
        ...
        // String directoryPathPG =
        // Constants.getProperty("INTERFACE_DIRECTORY_PG");

        File directory = new File(directoryPath);
        // File directoryPG = new File(directoryPathPG);
		String instanceID=Constants.getProperty("INSTANCE_ID");
        ...
        try {
            ...
            if (directory.isDirectory()) {
                ...
                String fileCacheType = Constants.getProperty("FILE_CACHE_TYPE");
                if (interfaceFiles != null && !BTSLUtil.NullToString(fileCacheType).equalsIgnoreCase("FILECACHE")) {
					...
					ArrayList<InterfaceVO> list=null;
					HashMap<String,ArrayList<InterfaceVO>> nodesData=null;
					nodesData =new InterfaceDAO().loadNodeDetailsInCache();
					StringBuilder sb = new StringBuilder(1024);
					//String fileCacheType = Constants.getProperty("FILE_CACHE_TYPE");
					if(BTSLUtil.NullToString(fileCacheType).equalsIgnoreCase("DATABASE")){
					
							for (String key : nodesData.keySet()) {
								interfaceId = key;
								properties = new InterfaceDAO().getInterfaceIdProperties(interfaceId,instanceID);
								list=nodesData.get(interfaceId);
								if(list!=null && !list.isEmpty()){
									maxActiveNodes=list.size();
									for (int j=0;j<list.size();j++){
										properties.setProperty("IP_"+(j+1),list.get(j).getIp());
										properties.setProperty("PORT_"+(j+1), list.get(j).getPort());										
										if(list.get(j).getUri()!=null)
										properties.setProperty("URI_"+(j+1), list.get(j).getUri());
										String urlKey="URL_"+(j+1);
										if(properties.contains(urlKey))
											  properties.remove(urlKey);
										sb.setLength(0);
										properties.setProperty(urlKey,sb.append("http://").append(list.get(j).getIp()).append(":").append(list.get(j).getPort()).append("/").append(list.get(j).getUri()).toString());										
									}
								}
								properties.setProperty("MAX_ACTIVE_NODES", String.valueOf(maxActiveNodes));	
								properties.setProperty("MAX_ALWD_NO", String.valueOf(maxActiveNodes));	
								properties.setProperty("MAX_NODE", String.valueOf(maxActiveNodes));	
								
								map.put(interfaceId, properties.clone());
								properties.clear();
							}
					}else{
							for (int i = 0, k = interfaceFiles.length; i < k; i++)
							{
								...
								if(index!=-1)
								{
									...
									if (interfaceFiles[i].canRead() && ext.equals(fileExtension) && (filePrefix.trim().length() <= 0 || interfaceId.startsWith(filePrefix)) )
									{
										properties.clear();
										try {							        		
										fileInputStream = new FileInputStream(interfaceFiles[i]);
                ...
                throw new BTSLBaseException(FileCache.class, methodName, "error.directory.notexist");
            }
            /*
             ...
             * Constants.getProperty("INTERFACE_FILE_EXTENSION_PG");
             * String filePrefix =
             * Constants.getProperty("INTERFACE_FILE_PREFIX_PG");
             * String ext = null;
             * String interfaceId = null;
             ...
             * interfaceId = fileName.substring(0, index);
             * 
             * if (interfaceFilesPG[i].canRead() && ext.equals(fileExtension) &&
             * (filePrefix.trim().length() <= 0 ||
             * interfaceId.startsWith(filePrefix)) )
             * {
             ...
             * } else
             * {
             * throw new BTSLBaseException(FileCache.class,
             * methodName, "error.directory.notexist");
             * }
             */
