...
public class FOCBatchTransferPostgresQry implements FOCBatchTransferQry {
	...
	@Override
	public String loadBatchDPMasterDetailsQry(String itemStatus,String currentLevel) {
		...
		strBuff .append(" GROUP BY fb.batch_id,fb.batch_name,fb.batch_total_record, p.product_name,p.unit_value,fb.network_code,fb.network_code_for,fb.created_by,fb.product_code, fb.modified_by, fb.modified_on,fb.sms_default_lang,fb.sms_second_lang,p.product_type,fbg.geography_code,p.short_name ,fb.domain_code,fb.batch_date ORDER BY fb.batch_date DESC ) X");
		...
		LogFactory.printLog("loadBatchDPMasterDetails", strBuff.toString(), log);
		return strBuff.toString();
	}
	...
	public String getMultipleOffQry() {
		...
		strBuffSelectCProfileProd.append("(select user_id from users where msisdn=? and login_id=? and status <> ? )) order by COMM_PROFILE_SET_VERSION::integer desc");
