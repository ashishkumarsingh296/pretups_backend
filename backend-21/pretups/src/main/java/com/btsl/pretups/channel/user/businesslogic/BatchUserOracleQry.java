...
public class BatchUserOracleQry implements BatchUserQry{
	...
	public String loadBatchListForApprovalQry(String[] geographyCode,
		...
		// end
		strBuff.append(" SUM(DECODE(U.status,?,1,0)) new,SUM(DECODE(U.status,?,1,0)) active ");
		strBuff.append(" FROM batches B, users U, users USR_MOD, users USR_CR,batch_geographies BG, categories C ");
		...
		strBuff.append(")");
		strBuff.append(" AND B.created_by=USR_CR.user_id(+) AND B.modified_by= USR_MOD.user_id(+) ");
	...
	public String loadBatchDetailsListQry() {
		...
		strBuff.append(" FROM users U,channel_users CU,users USR_PRT,user_geographies UG,categories C ");
		strBuff.append(" WHERE U.batch_id=? AND U.status=? AND CU.user_id=U.user_id ");
		strBuff.append(" AND U.parent_id = USR_PRT.user_id(+) AND U.user_id=UG.user_id AND C.category_code = U.category_code ");
	...
	public String loadBatchListForEnquiryQry() {
		...
		strBuff.append(" SUM(DECODE(U.status,?,1,0)) new, SUM(DECODE(U.status,?,1,0)) active, SUM(DECODE(U.status,?,1,0)) reject ");
		strBuff.append(" FROM batches B, users U, users USR_CR, lookups LK WHERE B.batch_id=U.batch_id AND B.batch_id=?");
		strBuff.append(" AND U.creation_type=? AND B.network_code=? AND B.created_by=USR_CR.user_id(+)");
	...
	public String loadBatchListForEnquiryQry(String[] geographyCode, String userType) {
		...
		strBuff.append(" SUM(DECODE(U.status,?,1,0)) new,SUM(DECODE(U.status,?,1,0)) active, SUM(DECODE(U.status,?,1,0)) reject ");
		strBuff.append(" FROM batches B, users U, users USR_MOD, users USR_CR,batch_geographies BG, categories C,lookups LK  ");
		// added by shashank
		strBuff.append(" WHERE  U.creation_type=? AND BG.geography_code IN (");
		// Changes made to incorporate batch user creation by channel users
		strBuff.append(" select grph_domain_code from geographical_domains ");
		strBuff.append(" connect by prior grph_domain_code=parent_grph_domain_code ");
		strBuff.append(" start with grph_domain_code in (");
		...
		strBuff.append(" AND B.network_code=? ");
		// end
		strBuff.append(" AND BG.batch_id=B.batch_id AND B.batch_id=U.batch_id AND TRUNC(B.created_on)>=? AND TRUNC(B.created_on)<=?");
	...
	public String loadBatchDetailsListForEnqQry() {
		...
		strBuff.append(" U.contact_no, U.designation, U.division, U.department, U.msisdn, U.user_type, U.created_by, U.created_on, U.modified_by, U.modified_on,");
		strBuff.append(" U.address1, U.address2, U.city, U.state, U.country, U.ssn, U.user_name_prefix, U.external_code, U.user_code, U.short_name, U.reference_id, U.firstname, U.lastname, U.company, U.fax,");// U.firstname,U.lastname,U.company,U.fax
		strBuff.append(" U.invalid_password_count, U.level1_approved_by, U.level1_approved_on, U.level2_approved_by, U.level2_approved_on,");
		...
		strBuff.append(" CU.user_grade, CU.transfer_profile_id, TP.profile_name, CU.suboutlet_code, decode(CU.suboutlet_code,'','',SLK.sub_lookup_name) sub_lookup_name, CU.outlet_code, decode(CU.outlet_code,'','',LK2.lookup_name) OutletName, CU.out_suspend, CU.in_suspend, CU.contact_person,");
		...
		strBuff.append(" FROM ");
		strBuff.append(" users U,channel_users CU, users USR_PRT, user_geographies UG,");				
		strBuff.append(" categories C, lookups LK1, lookups LK2, geographical_domains GD, sub_lookups SLK, commission_profile_set CPS, transfer_profile TP");
		strBuff.append(" WHERE U.batch_id=?  AND   CU.user_id=U.user_id AND U.network_code=CPS.network_code AND CU.comm_profile_set_id=CPS.comm_profile_set_id AND C.category_code=U.category_code(+) AND GD.grph_domain_code=UG.grph_domain_code");
		strBuff.append(" AND U.parent_id = USR_PRT.user_id(+) AND U.user_id=UG.user_id AND C.category_code = U.category_code");
		strBuff.append(" AND SLK.sub_lookup_code=DECODE(CU.suboutlet_code,'',(select sub_lookup_code from sub_lookups where lookup_type=? and rownum=1),SLK.sub_lookup_code,CU.suboutlet_code)");
		strBuff.append(" AND LK1.lookup_code=U.status AND LK2.lookup_code=DECODE(CU.outlet_code,'',(select lookup_code from lookups where lookup_type=? and rownum=1),LK2.lookup_code,CU.outlet_code) AND LK1.lookup_type=?");
	...
	public String loadBatchDetailsListForEnqTcpQry() {
		...
		strBuff.append(" U.invalid_password_count, U.level1_approved_by, U.level1_approved_on, U.level2_approved_by, U.level2_approved_on,");
		strBuff.append(" U.appointment_date, U.password_count_updated_on, U.previous_status, U.batch_id, U.creation_type, U.remarks user_remarks,");
		strBuff.append(" CU.user_grade, CU.transfer_profile_id,  CU.suboutlet_code, decode(CU.suboutlet_code,'','',SLK.sub_lookup_name) sub_lookup_name, CU.outlet_code, decode(CU.outlet_code,'','',LK2.lookup_name) OutletName, CU.out_suspend, CU.in_suspend, CU.contact_person,");
		strBuff.append(" CU.comm_profile_set_id, CPS.comm_profile_set_name, CU.activated_on, ");
		strBuff.append(" USR_PRT.login_id parent_login_id, USR_PRT.msisdn parent_msisdn, UG.grph_domain_code, GD.grph_domain_name, C.category_name, C.fixed_roles, C.services_allowed,");
		strBuff.append(" cu.application_id, cu.mpay_profile_id, cu.user_profile_id, cu.is_primary, cu.mcommerce_service_allow, cu.low_bal_alert_allow");
		strBuff.append(", u.rsaflag, u.authentication_allowed");
		strBuff.append(" FROM users U,channel_users CU, users USR_PRT, user_geographies UG,");
		strBuff.append(" categories C, lookups LK1, lookups LK2, geographical_domains GD, sub_lookups SLK, commission_profile_set CPS ");
		strBuff.append(" WHERE U.batch_id=? AND CU.user_id=U.user_id AND U.network_code=CPS.network_code AND CU.comm_profile_set_id=CPS.comm_profile_set_id AND C.category_code=U.category_code(+) AND GD.grph_domain_code=UG.grph_domain_code");
		strBuff.append(" AND U.parent_id = USR_PRT.user_id(+) AND U.user_id=UG.user_id AND C.category_code = U.category_code");
		strBuff.append(" AND SLK.sub_lookup_code=DECODE(CU.suboutlet_code,'',(select sub_lookup_code from sub_lookups where lookup_type=? and rownum=1),SLK.sub_lookup_code,CU.suboutlet_code)");
		strBuff.append(" AND LK1.lookup_code=U.status AND LK2.lookup_code=DECODE(CU.outlet_code,'',(select lookup_code from lookups where lookup_type=? and rownum=1),LK2.lookup_code,CU.outlet_code) AND LK1.lookup_type=?");
	...
	public String loadBatchDetailsListForEnqSelectServicesQry() {
		...
		selectServices.append(" FROM user_services US, service_type ST,users U,category_service_type CST");
		selectServices.append(" WHERE US.user_id=? AND US.status='Y' AND US.service_type(+)=ST.service_type");
	...
	public String loadBatchDetailsListForEnqSelectRolesQry() {
		final StringBuilder selectRoles = new StringBuilder(
				"SELECT DISTINCT UR.role_code, R.role_name FROM user_roles UR, roles R WHERE UR.user_id=? AND UR.role_code(+)=R.role_code ORDER BY UR.role_code");
	...
	public PreparedStatement loadBatchUserListForModifyQry(Connection con, String categoryCode, String geographyCode, String userID ) throws SQLException {
		...
		strBuff.append(" U.ssn, U.designation, U.address1, U.address2, U.city,U.state,U.country,U.company,U.fax,U.firstname,U.lastname, U.email, U.rsaflag,U.authentication_allowed,");// company,fax,firstname,lastname
		strBuff.append("C.category_name");
		strBuff.append(",U.longitude,U.latitude,U.document_type, U.document_no, U.payment_type,CU.COMM_PROFILE_SET_ID,CU.TRANSFER_PROFILE_ID,CU.USER_GRADE");
		if (SystemPreferences.USERWISE_LOAN_ENABLE) 
			strBuff.append(",culoan.profile_id");
		strBuff.append(" FROM users U, user_geographies UG, user_phones UP, channel_users CU, categories C, domains D");
		if (SystemPreferences.USERWISE_LOAN_ENABLE) 
			  strBuff.append(",channel_user_loan_info culoan");
		...
		if (SystemPreferences.USERWISE_LOAN_ENABLE) 
			strBuff.append(" AND culoan.user_id(+)=U.user_id");
		strBuff.append(" AND UP.primary_number(+)=? AND UG.grph_domain_code IN   (SELECT grph_domain_code FROM ");
		...
		LogFactory.printLog("loadBatchUserListForModifyQry", strBuff.toString(), log);
	...
	public PreparedStatement loadMasterGeographyForCategoryListQry(Connection con, String category, String geographicsCode, String loginUserID) throws SQLException {
		final StringBuilder strBuff = new StringBuilder(
				" SELECT distinct GD.grph_domain_code geography_code,GD.grph_domain_name geography_name, GD.grph_domain_type grph_domain_type, ");
		...
		strBuff.append(" AND C.category_code=? AND C.grph_domain_type=GDT.grph_domain_type AND C.status='Y' ");
	...
	public String loadGeographyAndDomainDetailsQry() {
		StringBuilder strbuff = new StringBuilder("select gd.GRPH_DOMAIN_CODE ");
		strbuff.append(" from GEOGRAPHICAL_DOMAINS gd, GEOGRAPHICAL_DOMAIN_TYPES gdt ");
		strbuff.append(" where gdt.GRPH_DOMAIN_TYPE = gd.GRPH_DOMAIN_TYPE and gdt.SEQUENCE_NO=2 ");
		strbuff.append(" connect by prior gd.PARENT_GRPH_DOMAIN_CODE=gd.GRPH_DOMAIN_CODE");
		strbuff.append(" start with gd.GRPH_DOMAIN_CODE in  ");
		strbuff.append(" (select GEOGRAPHY_CODE from batch_geographies where BATCH_ID = ?) ");
		LogFactory.printLog("loadGeographyAndDomainDetailsQry", strbuff.toString(), log);
		return strbuff.toString();
	}
	...
	public PreparedStatement loadBatchUserListForProfileAssociateQry(
		...
		StringBuilder strBuff = new StringBuilder(" SELECT DISTINCT U.user_id, U.user_name_prefix, U.user_name,");
    	...
    	strBuff.append(" FROM users U, user_geographies UG, user_phones UP, channel_users CU, categories C, domains D, channel_grades CG, commission_profile_set CP, transfer_profile TP");
    	strBuff.append(" WHERE  U.category_code=C.category_code AND C.domain_code=D.domain_code" );
    	strBuff.append(" AND U.user_id=UG.user_id AND U.user_id=CU.user_id AND U.status NOT IN(?,?) ");
    	strBuff.append(" AND U.user_type =? AND U.category_code=? AND UP.user_id(+)=U.user_id" );
	...
	public PreparedStatement loadBatchUserListForProfileAssociateTcpQry(
    	...
    	strBuff.append(" C.category_name,");
    	strBuff.append(" CU.user_grade,CU.transfer_profile_id,CU.comm_profile_set_id");
    	strBuff.append(" FROM users U, user_geographies UG, user_phones UP, channel_users CU, categories C, domains D, channel_grades CG, commission_profile_set CP ");
    	strBuff.append(" WHERE  U.category_code=C.category_code AND C.domain_code=D.domain_code" );
    	strBuff.append(" AND U.user_id=UG.user_id AND U.user_id=CU.user_id AND U.status NOT IN(?,?) ");
    	strBuff.append(" AND U.user_type =? AND U.category_code=? AND UP.user_id(+)=U.user_id" );
	...
	public String loadCommProfileListQry() {
		...
		strBuff.append(" WHERE CAT.domain_code=? AND CAT.category_code=CPS.category_code AND CPS.GRADE_CODE=CG.GRADE_CODE(+) AND GD.GRPH_DOMAIN_CODE(+)=CPS.GEOGRAPHY_CODE ");
	...
	public PreparedStatement loadBatchUserListForModifyPOIQry(Connection con,
	        ...
	        strBuff.append(" CU.outlet_code,CU.suboutlet_code, CU.contact_person, U.contact_no,");
	        strBuff
	            .append(" U.ssn, U.designation, U.address1, U.address2, U.city,U.state,U.country,U.company,U.fax,U.firstname,U.lastname, U.email, U.rsaflag,U.authentication_allowed,");
	        strBuff.append("C.category_name");
	        strBuff.append(",U.longitude,U.latitude,U.document_type, U.document_no, U.payment_type");
	        if (SystemPreferences.USERWISE_LOAN_ENABLE) 
	        	strBuff.append(",culoan.profile_id");
	...
	public String loadBatchDetailsListForUsrEnqQry() {
		...
		strBuff.append(" cu.application_id, cu.mpay_profile_id, cu.user_profile_id, cu.is_primary, cu.mcommerce_service_allow, cu.low_bal_alert_allow");
		strBuff.append(", u.rsaflag, u.authentication_allowed");
		strBuff.append(" FROM ( SELECT user_id,parent_id,owner_id    FROM USERS CONNECT BY PRIOR user_id = parent_id START WITH user_id=? ) X, ");
		strBuff.append(" users U,channel_users CU, users USR_PRT, user_geographies UG,");				
		strBuff.append(" categories C, lookups LK1, lookups LK2, geographical_domains GD, sub_lookups SLK, commission_profile_set CPS, transfer_profile TP");
		strBuff.append(" WHERE U.batch_id=? AND U.USER_ID = X.user_id AND   CU.user_id=U.user_id AND U.network_code=CPS.network_code AND CU.comm_profile_set_id=CPS.comm_profile_set_id AND C.category_code=U.category_code(+) AND GD.grph_domain_code=UG.grph_domain_code");
		strBuff.append(" AND U.parent_id = USR_PRT.user_id(+) AND U.user_id=UG.user_id AND C.category_code = U.category_code");
		strBuff.append(" AND SLK.sub_lookup_code=DECODE(CU.suboutlet_code,'',(select sub_lookup_code from sub_lookups where lookup_type=? and rownum=1),SLK.sub_lookup_code,CU.suboutlet_code)");
		strBuff.append(" AND LK1.lookup_code=U.status AND LK2.lookup_code=DECODE(CU.outlet_code,'',(select lookup_code from lookups where lookup_type=? and rownum=1),LK2.lookup_code,CU.outlet_code) AND LK1.lookup_type=?");
		strBuff.append(" AND CU.transfer_profile_id=TP.profile_id AND U.network_code=TP.network_code ");
	...
	public String loadBatchListForUsrEnquiryQry(String[] geographyCode, String userType,String loggedInUserID) {
		...
		strBuff.append(" SUM(DECODE(U.status,?,1,0)) new,SUM(DECODE(U.status,?,1,0)) active, SUM(DECODE(U.status,?,1,0)) reject ");
		strBuff.append(" FROM ( SELECT user_id,parent_id,owner_id    FROM USERS CONNECT BY PRIOR user_id = parent_id START WITH user_id=? ) X, ");
		strBuff.append("  batches B, users U, users USR_MOD, users USR_CR,batch_geographies BG, categories C,lookups LK  ");
		// added by shashank
	...
	public String loadBatchListForEnquiryQryUsr() {
		...
		strBuff.append(" SUM(DECODE(U.status,?,1,0)) new, SUM(DECODE(U.status,?,1,0)) active, SUM(DECODE(U.status,?,1,0)) reject ");
		strBuff.append(" FROM ( SELECT user_id,parent_id,owner_id    FROM USERS CONNECT BY PRIOR user_id = parent_id START WITH user_id=? ) X, ");
		strBuff.append(" batches B, users U, users USR_CR, lookups LK WHERE B.batch_id=U.batch_id AND B.batch_id=?  AND U.USER_ID = X.USER_ID ");
		strBuff.append(" AND U.creation_type=? AND B.network_code=? AND B.created_by=USR_CR.user_id(+)");
		strBuff.append(" AND LK.lookup_code=B.status AND LK.lookup_type=?");
		strBuff.append(" GROUP BY B.batch_id, B.batch_name, B.file_name,B.status, B.modified_on,");
		strBuff.append(" B.created_on, B.batch_type, B.batch_size, USR_CR.user_name,LK.lookup_name ORDER BY B.created_on DESC");
