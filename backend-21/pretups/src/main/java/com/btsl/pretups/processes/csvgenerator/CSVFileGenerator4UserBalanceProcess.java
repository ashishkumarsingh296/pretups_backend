...
/**
 * @(#)CSVFileGenerator4UserBalanceProcess
 *                             Copyright(c) 2021, Comviva Technologies Ltd.
 *                             All Rights Reserved
 * 
 *                             ------------------------------------------------
 *                             -------------------------------------------------
 ...
 *                             ------------------------------------------------
 *                             -------------------------------------------------
 *                             Diwakar 04/06/2021 Initial Creation
 *                             ------------------------------------------------
 *                             -------------------------------------------------
 */
...
public class CSVFileGenerator4UserBalanceProcess {
    ...
    private static Log logger = LogFactory.getLog(CSVFileGenerator4UserBalanceProcess.class.getName());
            ...
            if (statusOk) {
                ...
                if (processedUpto != null) {
                    ...
                    if (processedUpto.compareTo(currentDate) == 0) {
                        throw new BTSLBaseException(processName, methodName, PretupsErrorCodesI.DWH_PROCESS_ALREADY_EXECUTED_TILL_TODAY);
                    }
                    ...
                    for (dateCount = BTSLUtil.getSQLDateFromUtilDate(processedUpto); dateCount.before(BTSLUtil.addDaysInUtilDate(currentDate, -beforeInterval)); dateCount = BTSLUtil
	                    ...
	                    try {
	                        while (itr.hasNext()) {
	                            ...
	                            fetchQuery(con, dateCount, csvfilevo.getDirName(), csvfilevo.getPrefixName(), csvfilevo.getHeaderName(), csvfilevo.getExtName(), csvfilevo.getQueryName(), csvfilevo.getTempTable(), Long.parseLong(csvproperties.getProperty("MAX_ROWS")));
	                            processStatusVO.setExecutedUpto(BTSLUtil.getSQLDateFromUtilDate(dateCount));
	                            Thread.sleep(500);
	                        }
	                    } catch (Exception e) {
	                        logger.errorTrace(methodName, e);
		...
		try{
	        ...
	        int n = Integer.parseInt(csvproperties.getProperty(processId+"_DYN_ARG"));
               ...
               else
               {
					...
					int m=1;
					if(!BTSLUtil.isNullString(lastnmonths)) {
						for(int i=1;i<=n;i++){							
               			...
               			//Go for dynamic date
               			for(int i=1;i<=n;i++){							
               				c2sInsertPstmt.setDate(i, BTSLUtil.getSQLDateFromUtilDate(pbeingProcessedDate));
						}
               		}
               }
		    ...
		    if(insertCount>0) {
			    ...
			    logger.debug(methodName,"There is no record found for date:"+pbeingProcessedDate+" due to having ambiguous transaction or MIS has not executed successfully.");
		    	throw new BTSLBaseException(processName, methodName, PretupsErrorCodesI.DWH_ERROR_EXCEPTION);
		    }
        } catch (BTSLBaseException be) {
            logger.error(methodName, "BTSLBaseException : " + be.getMessage());
    ...
    private static void writeDataInFile(Connection pcon, String pdirPath, String pfileName, String pfileLabel, Date pbeingProcessedDate, String pfileEXT, long pmaxFileLength, String ptempTbl, int pmaxCount) throws BTSLBaseException {
        ...
        try {
            ...
            fileNumber = 1;
            Date date = new Date();
            //Get Yesterday Date as process is being executed for previous date.
            //date = BTSLUtil.addDaysInUtilDate(date, -1);
            date = pbeingProcessedDate;
