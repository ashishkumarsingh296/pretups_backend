/**
 * @(#)LMSOptInOutReferenceTargetCalculation.java
 *                                                Copyright(c) 2005, Bharti
 *                                                Telesoft Ltd.
 *                                                All Rights Reserved
 * 
 *                                                <Process to calculate target
 *                                                for channel users of reference
 *                                                based profile >
 *                                                ------------------------------
 *                                                ------------------------------
 *                                                ------------------------------
...
public class LMSOptInOutReferenceTargetCalculation {

    private static Log _logger = LogFactory.getLog(LMSOptInOutReferenceTargetCalculation.class.getName());
    ...
    public static void main(String[] args) {
        ...
        try {
            if (args.length != 2) {
                System.out.println("Usage : LMSOptInOutReferenceTargetCalculation [Constants file] [LogConfig file] [Upload File Path]");
                return;
            }
        ...
        try {
            final LMSOptInOutReferenceTargetCalculation lmsOptInOutReferenceTargetCalculation = new LMSOptInOutReferenceTargetCalculation();
            lmsOptInOutReferenceTargetCalculation.process();
    ...
    public void process() throws BTSLBaseException, SQLException {
        ...
        try {
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.OPT_IN_OUT_ALLOW)).booleanValue()) {
                ...
                String lmsOptInOutRefTarCalMisCkReq = Constants.getProperty("LMSOPTINOUTREFTGT_MIS_CHECK_REQUIRED");
                if (BTSLUtil.isNullString(lmsOptInOutRefTarCalMisCkReq)) {
                    lmsOptInOutRefTarCalMisCkReq = PretupsI.YES;
                } else if ("null".equalsIgnoreCase(lmsOptInOutRefTarCalMisCkReq) || !PretupsI.NO.equalsIgnoreCase(lmsOptInOutRefTarCalMisCkReq)) {
                    lmsOptInOutRefTarCalMisCkReq = PretupsI.YES;
                }
                if (PretupsI.YES.equals(lmsOptInOutRefTarCalMisCkReq)) {
                    ...
                    if (processedUptoMIS != null) {
                        ...
                        if (cal24CurrentDate.compareTo(cal34MisExecutedUpTo) != 0) {
                            ...
                            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                                "LMSOptInOutReferenceTargetCalculation[process]", "", "", "", "The MIS has not been executed for the previous day.");
                ...
                _processStatusVO = _processBL.checkProcessUnderProcess(con, ProcessI.LMS_OPTINOUT_REF_CAL);
                ...
                if (statusOk) {
                    ...
                    if (processedUpto != null) {
                        ...
                        try {
                            ...
                            prevDateStr = sdf.format(processedUpto);// Last MIS
                            // Done Date
                            // +1
                        } catch (Exception e) {
                            reportTo = "";
                        ...
                        // -1
                        if (_logger.isDebugEnabled()) {
                            _logger.debug("LMSOptInOutReferenceTargetCalculation[process]",
                        ...
                        if (processedUpto.compareTo(currentDate) >= 0) {
                            EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                                "LMSOptInOutReferenceTargetCalculation[main]", "", "", "", "LMS Target Credit Controller has already been executed for the date=" + String
                                    .valueOf(currentDate));
                            return;
                        }
                ...
                if (BTSLUtil.isNullString(instanceID)) {
                    if (_logger.isDebugEnabled()) {
                        _logger.debug(METHOD_NAME, " Not able to get the instance ID for the network=");
                    }
                    throw new BTSLBaseException(METHOD_NAME, "LMSOptInOutReferenceTargetCalculation", "lms.point.redemption.form.error.unableto.initiateo2c", "");
                }
                ...
                for (dateCount = BTSLUtil.getSQLDateFromUtilDate(processedUpto); dateCount.before(limitDate); dateCount = BTSLUtil.addDaysInUtilDate(dateCount, 1)) {
                    ...
                    if (userProfileList.isEmpty()) {
                        ...
                        _logger.info(METHOD_NAME, "process", "No Target profile active for the date = " + dateCount);
                    } else {
                ...
                _logger.info(METHOD_NAME,
                    "Opt IN/OUT Reference Target calculation Process will execute only if the preference<" + ProcessI.LMS_OPTINOUT_REF_CAL + "> is enabled into the system.");
            }
            ...
            try {
                ...
                if (statusOk) {
                    if (markProcessStatusAsComplete(con, ProcessI.LMS_OPTINOUT_REF_CAL) == 1) {
                        try {
                            con.commit();
                        } catch (Exception e) {
                            _logger.errorTrace(METHOD_NAME, e);
				...
				if (mcomCon != null) {
					mcomCon.close("LMSOptInOutReferenceTargetCalculation#process");
					mcomCon = null;
				}
				 try {
	                    if (psmtInsert != null) {
                ...
                _logger.errorTrace(METHOD_NAME, ex);
                System.out.println("Exception while closing statement in LMSOptInOutReferenceTargetCalculation method ");
            } finally {
                if (_logger.isDebugEnabled()) {
    ...
    ArrayList loadTargetProfile(Connection p_con, Date sumTxnDate) {
        ...
        if (_logger.isDebugEnabled()) {
            _logger.debug(METHOD_NAME, " Entered : sumTxnDate = " + sumTxnDate);
        }
        ...
        LMSOptInOutReferenceTargetCalculationQry calculationQry = (LMSOptInOutReferenceTargetCalculationQry) 
        		ObjectProducer.getObject(QueryConstants.LMS_OPT_IN_OUT_REF_TARGET_CAL_QRY, QueryConstants.QUERY_PRODUCER);

        final String selectQuery = calculationQry.loadTargetProfileQry();
        if (_logger.isDebugEnabled()) {
        ...
        try {
			...
			pstmtSelect.setString(index++,PretupsI.OPT_OUT);
    ...
    ArrayList loadProfileMapping(Connection p_con) {
        ...
        selectQueryBuffer.append(" select SET_ID,ps.MODIFIED_ON from PROFILE_SET ps where status=? ");
