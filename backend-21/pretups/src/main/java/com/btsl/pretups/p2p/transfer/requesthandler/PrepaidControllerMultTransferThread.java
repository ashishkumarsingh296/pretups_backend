...
/**
 * @(#)PrepaidControllerMultTransferThread.java
...
public class PrepaidControllerMultTransferThread implements Runnable {
    private static Log _log = LogFactory.getLog(PrepaidControllerMultTransferThread.class.getName());
    ...
    // Reciever
    // MSISDN
    private static  final int SRC_BEFORE_INRESP_CAT_ROUTING = 1; // To denote the
    // process from where
    // interface routing
    // has been called,
    // Before IN Validation
    // of Receiver
    private static  final int SRC_AFTER_INRESP_CAT_ROUTING = 2; // To denote the process
    ...
    private boolean _isSenderSuccess = false;

    public PrepaidControllerMultTransferThread() {
     ...
     * transfer.
     * If more than one receiver is there in the request, then it will create
     * the same number of object of this class to
     * handle the request.
     */
    public void process(RequestVO p_requestVO) {
        ...
        PrepaidControllerMultTransferThread newContr = null;
        try {
            seperator = ((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.MULT_CRE_TRA_DED_ACC_SEP));
            decrMsg = p_requestVO.getDecryptedMessage();
            if (_log.isDebugEnabled()) {
                _log.debug("PROCESS", "decrMsg=" + decrMsg + " seperator=" + seperator);
            }
            if (!BTSLUtil.isNullString(decrMsg) && decrMsg.contains(seperator)) {
                if (!BTSLUtil.isNullString(decrMsg)) {
                    reqMsgArr = decrMsg.split(seperator);
                    for (int i = 0; i < reqMsgArr.length; i++) {
                        final String[] messageArray = PretupsBL.parsePlainMessage(reqMsgArr[i]);
                        p_requestVO.setRequestMessageArray(messageArray);
                        newContr = new PrepaidControllerMultTransferThread();
                        newContr.processReq(p_requestVO);
                        Thread.sleep(1500);
                        if (newContr._isSenderSuccess) {
                            successReq++;
                            if (BTSLUtil.isNullString(succMSISDN)) {
                                succMSISDN = newContr._receiverMSISDN;
                            } else {
                                succMSISDN += "," + newContr._receiverMSISDN;
                            }
                        } else {
                            failRequest++;
                            if (BTSLUtil.isNullString(failMSISDN)) {
                                failMSISDN = newContr._receiverMSISDN;
                            } else {
                                failMSISDN += "," + newContr._receiverMSISDN;
                            }
                        }
                        ...
                        if (senderLocale == null) {
                            senderLocale = newContr._senderLocale;
                        }
                        if (senderMsisdn == null) {
                            senderMsisdn = newContr._senderMSISDN;
                        }
                    }
                }
            } else {
                totalRequest++;
                processReq(p_requestVO);
                Thread.sleep(1000);
                if (_isSenderSuccess) {
                    successReq++;
                    succMSISDN += _receiverMSISDN;
                } else {
                    failRequest++;
                    failMSISDN += _receiverMSISDN;
                }
                senderLocale = _senderLocale;
            ...
            try {
                ...
                String gwCode = null;
                if (totalRequest > 1) {
                    txnID = newContr._transferID;
                    gwCode = newContr._p2pTransferVO.getRequestGatewayCode();
                } else {
                    txnID = _transferID;
                    gwCode = _p2pTransferVO.getRequestGatewayCode();
                }

                if (_log.isDebugEnabled()) {
                    _log.debug("PROCESS", _requestIDStr, "txnID=" + txnID + " gwCode=" + gwCode + " senderLocale=" + senderLocale);
                }

                if (successReq > 0) {
                    (new PushMessage(senderMsisdn, getSenderFinalMessage(senderLocale, succMSISDN, successReq, totalRequest, true), txnID, gwCode, senderLocale)).push();
                }

                if (failRequest > 0) {
                    (new PushMessage(senderMsisdn, getSenderFinalMessage(senderLocale, failMSISDN, failRequest, totalRequest, false), txnID, gwCode, senderLocale)).push();
                }
            } catch (Exception e) {
                _log.errorTrace(methodName, e);
            }
            if (_log.isDebugEnabled()) {
                _log.debug("PROCESS", "totalRequest=" + totalRequest + " successReq=" + successReq + " failRequest=" + failRequest);
                _log.debug("PROCESS", "EXITED with succMSISDN=" + succMSISDN + " and failMSISDN=" + failMSISDN);
            }
        }
    }
     ...
     * 1. If any authorized user is sending the transfer request then register
     ...
     * Thread along with top-up or before that
     ...
     * initially not found in DB then insert the
     ...
     * controlling limits.
     * 25. If ambiguous then check whether credit back request for sender needs
     * to be send or not.
     * 26. If Success make the transaction status as success and send message
     * accordingly.
     */
    public void processReq(RequestVO p_requestVO) {
        final String methodName = "processReq";
		Connection con = null;
        ...
        try {
            ...
            if (_type.equals(PretupsI.INTERFACE_CATEGORY_BOTH)) {
                ...
                if (_serviceInterfaceRoutingVO != null) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PrepaidControllerMultTransfer[process]", "",
                ...
                if (_serviceInterfaceRoutingVO != null) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PrepaidControllerMultTransfer[process]", "",
            ...
            if (!_receiverVO.getSubscriberType().equals(_type)) {
                ...
                if (_networkPrefixVO != null) {
                    ...
                    EventHandler
                        .handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PrepaidControllerMultTransfer[process]", "", "", "",
            ...
            try {
                ...
                _log.errorTrace(methodName, e);
                throw new BTSLBaseException("PrepaidControllerMultTransfer", "process", PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
            }
            ...
            if (((Boolean) PreferenceCache.getSystemPreferenceValue(PreferenceI.SKEY_REQUIRED)).booleanValue() && _senderVO.getSkeyRequired().equals(PretupsI.YES)) {
				...
				if (mcomCon != null) {
					mcomCon.close("PrepaidControllerMultTransferThread#processReq");
					mcomCon = null;
				}
                con = null;
            ...
            if (_senderVO != null) {
                try {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PrepaidControllerMultTransfer[process]",
                        _transferID, _senderMSISDN, _senderNetworkCode, "Base Exception while updating Subscriber Last Details:" + bex.getMessage());
                } catch (Exception e) {
                    _log.errorTrace(methodName, e);
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PrepaidControllerMultTransfer[process]",
                        _transferID, _senderMSISDN, _senderNetworkCode, "Exception while updating Subscriber Last Details:" + e.getMessage());
            ...
            try {
                ...
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PrepaidControllerMultTransfer[process]",
                    _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + ex1.getMessage());
                _p2pTransferVO.setErrorCode(PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
            }
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[process]", _transferID,
    ...
    private void processSKeyGen(Connection p_con) throws BTSLBaseException, Exception {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[processSKeyGen]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled()) {
        ...
        try {
            ...
            try {
                ...
                if (!_senderVO.isDefUserRegistration() && _senderTransferItemVO.getValidationStatus().equals(InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND)) {
                    throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.P2P_SENDER_ALREADY_REG_NOT_FOUND_IN_VAL, 0,
            ...
            if ((System.currentTimeMillis() - _p2pTransferVO.getRequestStartTime()) > interfaceVO.getValExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                    "PrepaidControllerMultTransfer[processValidationRequest]", _transferID, _senderMSISDN, _senderNetworkCode,
                    "Exception: System is taking more time till validation of sender");
                throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION_TKING_TIME_TILL_VAL_S);
            }
            interfaceVO = null;
            ...
            if (con != null) {
				...
				if (mcomCon != null) {
					mcomCon.close("PrepaidControllerMultTransferThread#processValidationRequest");
					mcomCon = null;
				}
                con = null;
            }
            ...
            if ((System.currentTimeMillis() - _p2pTransferVO.getRequestStartTime()) > interfaceVO.getValExpiryTime()) {
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO, "PrepaidControllerMultTransfer[run]", _transferID,
                    _senderMSISDN, _senderNetworkCode, "Exception: System is taking more time till validation of reciever");
                throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION_TKING_TIME_TILL_VAL_R);
            }
            interfaceVO = null;
            ...
            // Push Under Process Message to Receiver , this might have to be
            // implemented on flag basis whether to send message or not
            if (_p2pTransferVO.isUnderProcessMsgReq()) {
                // In case of Self TopUp,Under process request message will be
            ...
            if ((System.currentTimeMillis() - _p2pTransferVO.getRequestStartTime()) > interfaceVO.getTopUpExpiryTime()) {
                ...
                throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION_TKING_TIME_TILL_TOPUP);
            }
            interfaceVO = null;
            ...
            _log.error("PrepaidControllerMultTransfer[processValidationRequest]", "Getting BTSL Base Exception:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.errorTrace(methodName, e);
            if (con != null) {
            ...
            _log.error("PrepaidControllerMultTransfer[processValidationRequest]", "Getting Exception:" + e.getMessage());
            if (BTSLUtil.isNullString(_receiverTransferItemVO.getTransferStatus()) || _receiverTransferItemVO.getTransferStatus().equals(InterfaceErrorCodesI.SUCCESS)) {
                _receiverTransferItemVO.setTransferStatus(PretupsErrorCodesI.REQUEST_NOT_APPLICABLE);
                _receiverTransferItemVO.setUpdateStatus(PretupsErrorCodesI.REQUEST_NOT_APPLICABLE);
                _senderTransferItemVO.setTransferStatus(PretupsErrorCodesI.REQUEST_NOT_APPLICABLE);
            ...
            throw new BTSLBaseException(this, methodName, "Exception in performing validation request.");
        } finally {
			if (mcomCon != null) {
				mcomCon.close("PrepaidControllerMultTransferThread#processValidationRequest");
				mcomCon = null;
    ...
    /**
     * Process Transfer Request , Generates the Transfer ID and populates the
     * Transfer Items VO
     * 
     * @param p_con
     * @throws BTSLBaseException
    ...
    public void processTransfer(Connection p_con) throws BTSLBaseException {
        ...
        try {
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[processTransfer]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        }
    }
    ...
    /**
     * This method will perform either top-up in thread or both validation and
     * top-up on thread based on Flow Type
     */
    public void run() {
        ...
        try {
            ...
            try {
                ...
                // If transaction is Ambiguous and Preference flag is Set to
                // true (Whether credit back is true in ambigous case)
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[run]", _transferID,
                _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            ...
            try {
                ...
                try {
                    ...
                    _log.errorTrace(methodName, e);
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[run]",
                        _transferID, _senderMSISDN, _senderNetworkCode, "Not able to update Subscriber Last Details Exception:" + e.getMessage());
                }
                ...
                try {
                    ...
                    _log.errorTrace(methodName, e);
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[run]",
                        _transferID, _senderMSISDN, _senderNetworkCode, "Not able to unmark Receiver Last Request, Exception:" + e.getMessage());
                }
            ...
            if (con != null) {
				...
				if (mcomCon != null) {
					mcomCon.close("PrepaidControllerMultTransferThread#run");
					mcomCon = null;
				}
                con = null;
            }
            ...
            if (_p2pTransferVO.getTransferStatus().equals(PretupsErrorCodesI.TXN_STATUS_SUCCESS)) {
                ...
                if (_p2pTransferVO.getReceiverReturnMsg() == null) {
                    (new PushMessage(_receiverMSISDN, getReceiverAmbigousMessage(), _transferID, _p2pTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                } else if (_p2pTransferVO.getReceiverReturnMsg() != null && ((BTSLMessages) _p2pTransferVO.getReceiverReturnMsg()).isKey()) {
                    final BTSLMessages btslRecMessages = (BTSLMessages) _p2pTransferVO.getReceiverReturnMsg();
                    (new PushMessage(_receiverMSISDN, BTSLUtil.getMessage(_receiverLocale, btslRecMessages.getMessageKey(), btslRecMessages.getArgs()), _transferID,
                        _p2pTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                ...
                if (_p2pTransferVO.getReceiverReturnMsg() == null) {
                    (new PushMessage(_receiverMSISDN, getReceiverFailMessage(), _transferID, _p2pTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
                } else if (_p2pTransferVO.getReceiverReturnMsg() != null && ((BTSLMessages) _p2pTransferVO.getReceiverReturnMsg()).isKey()) {
                    final BTSLMessages btslRecMessages = (BTSLMessages) _p2pTransferVO.getReceiverReturnMsg();
                    (new PushMessage(_receiverMSISDN, BTSLUtil.getMessage(_receiverLocale, btslRecMessages.getMessageKey(), btslRecMessages.getArgs()), _transferID,
                        _p2pTransferVO.getRequestGatewayCode(), _receiverLocale)).push();
            ...
            final PushMessage pushMessages = null;
    ...
    /**
     * Method to get the sender registration message
     * 
     * @return
     */
    ...
    /**
     * Method to get the Receiver Ambiguous Message
     * 
     * @return
     */
    ...
    private void setSenderTransferItemVO() {
        _senderTransferItemVO = TransferItemVO.getInstance();
        // set sender transfer item details
    ...
    private void setReceiverTransferItemVO() {
        _receiverTransferItemVO = TransferItemVO.getInstance();
    ...
    public void populateServicePaymentInterfaceDetails(Connection p_con, String action) throws BTSLBaseException {
        ...
        if (!isSenderFound) {
            if (!_senderVO.isDefUserRegistration()) {
                throw new BTSLBaseException("PrepaidControllerMultTransfer", "populateServicePaymentInterfaceDetails",
                    PretupsErrorCodesI.P2P_SENDER_ALREADY_REG_NOT_FOUND_IN_VAL, 0, new String[] { ((LookupsVO) LookupsCache.getObject(PretupsI.SUBSRICBER_TYPE, _senderVO
                        .getSubscriberType())).getLookupName() }, null);
            }
            throw new BTSLBaseException("PrepaidControllerMultTransfer", "populateServicePaymentInterfaceDetails", PretupsErrorCodesI.P2P_NOTFOUND_PAYMENTINTERFACEMAPPING);
        }
        ...
        if ((!_receiverInterfaceInfoInDBFound && action.equals(PretupsI.INTERFACE_NETWORK_PREFIX_UPDATE_ACTION)) || action
            ...
            if (action.equals(PretupsI.INTERFACE_NETWORK_PREFIX_VALIDATION_ACTION) && !isReceiverFound && _performIntfceCatRoutingBeforeVal && _useAlternateCategory && !_interfaceCatRoutingDone) {
                ...
                if (_networkPrefixVO != null) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                        "PrepaidControllerMultTransfer[populateServicePaymentInterfaceDetails]", "", "", "",
                        "Series =" + _receiverVO.getMsisdnPrefix() + " Not Defined for Series type=" + _type + " But alternate Category Routing was required on interface");
        ...
        if (!isReceiverFound) {
            throw new BTSLBaseException("PrepaidControllerMultTransfer", "populateServicePaymentInterfaceDetails", PretupsErrorCodesI.P2P_NOTFOUND_SERVICEINTERFACEMAPPING);
        }
    }
    ...
    public String getSenderDebitAdjustStr() {
        ...
        strBuff.append("&DEDICATED_ACC_ID=" + _senderTransferItemVO.getDedicatedAccountID());
        strBuff.append("&DEDICATED_ACC_VALUE=" + _senderTransferItemVO.getDedicatedAccountValues());
        strBuff.append("&DEDICATED_ACC_EXPIRY=" + _senderTransferItemVO.getDedicatedAccountExpiry());
    ...
    public String getReceiverCreditStr() {
        ...
        // added by amit for card group off-line field
        strBuff.append("&COMBINED_RECHARGE=" + _p2pTransferVO.getBoth());
        strBuff.append("&EXPLICIT_RECHARGE=" + _p2pTransferVO.getOnline());
        // Dedicated account information
        strBuff.append("&DEDICATED_ACC_ID=" + _receiverTransferItemVO.getDedicatedAccountID());
        strBuff.append("&DEDICATED_ACC_VALUE=" + _receiverTransferItemVO.getDedicatedAccountValues());
        strBuff.append("&DEDICATED_ACC_EXPIRY=" + _receiverTransferItemVO.getDedicatedAccountExpiry());
        strBuff.append("&DEDI_EXPIRY_DATE_S=" + _receiverTransferItemVO.getSenderDediAccountExpiry());
    ...
    public void updateForSenderValidateResponse(String str) throws BTSLBaseException {
        ...
        if ((!InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) && isLDCCTest) || isRequired) {
            ...
            if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
               ...
               throw new BTSLBaseException("PrepaidControllerMultTransfer", "updateForSenderValidateResponse", _p2pTransferVO.getErrorCode(), 0, strArr, null);
            }
            _senderTransferItemVO.setTransferStatus(status);
            ...
            // Dedicated account information
            _senderTransferItemVO.setDedicatedAccountID((String) map.get("DEDICATED_ACC_ID"));
            _senderTransferItemVO.setDedicatedAccountValues((String) map.get("DEDICATED_ACC_VALUE"));
            _senderTransferItemVO.setDedicatedAccountExpiry((String) map.get("DEDICATED_ACC_EXPIRY"));
        }
    }
    ...
    public void updateForSenderDebitResponse(String str) throws BTSLBaseException {
        ...
        ;
        _receiverTransferItemVO.setSenderDediAccountExpiry((String) map.get("DEDI_EXPIRY_DATE_S"));
    }
    ...
    public void updateForReceiverCreditResponse(String str) throws BTSLBaseException {
        ...
        if (BTSLUtil.isNullString(status) || (!status.equals(InterfaceErrorCodesI.SUCCESS) && !status.equals(InterfaceErrorCodesI.AMBIGOUS))) {
            ...
            _receiverTransferItemVO.setUpdateStatus(status);
            strArr = new String[] { _transferID, _receiverTransferItemVO.getMsisdn(), PretupsBL.getDisplayAmount(_p2pTransferVO.getRequestedAmount()) };
            throw new BTSLBaseException(this, "updateForReceiverCreditResponse", PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS, 0, strArr, null);
        } else {
            _receiverTransferItemVO.setTransferStatus(status);
    ...
    public void updateForSenderCreditBackResponse(String str) throws BTSLBaseException {
        ...
        final HashMap map = BTSLUtil.getStringToHash(str, "&", "=");
        _senderCreditBackStatusVO = TransferItemVO.getInstance();
        ...
        try {
            ...
            if (senderLoadStatus == 0) {
                ...
                if (recieverLoadStatus == 0) {
                    ...
                    if (_log.isDebugEnabled()) {
                        _log.debug("PrepaidControllerMultTransfer[checkTransactionLoad()]", "_transferID=" + _transferID + " Successfully through load");
                    }
                }
                // Request in Queue
                    ...
                    throw new BTSLBaseException("PrepaidControllerMultTransfer", "checkTransactionLoad()", PretupsErrorCodesI.REQUEST_IN_QUEUE, 0, strArr, null);
                }
                // Refuse the request
                else {
                    throw new BTSLBaseException("PrepaidControllerMultTransfer", "checkTransactionLoad()", PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
                }
            }
            // Request in Queue
            ...
            _log.errorTrace(methodName, be);
            _log.error("PrepaidControllerMultTransfer[checkTransactionLoad()]", "Refusing request getting Exception:" + be.getMessage());
            throw be;
        } catch (Exception e) {
            _log.errorTrace(methodName, e);
            throw new BTSLBaseException("PrepaidControllerMultTransfer", "checkTransactionLoad()", PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        }
    }
    ...
    private void checkTransactionLoad(String p_userType, String p_interfaceID) throws BTSLBaseException {
        final String methodName = "checkTransactionLoad";
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, "Checking load for transfer ID=" + _transferID + " on interface=" + p_interfaceID + " for p_userType=" + p_userType);
        }
        int recieverLoadStatus = 0;
            ...
            if (PretupsI.USER_TYPE_SENDER.equals(p_userType)) {
                ...
                if (senderLoadStatus == 0) {
                    ...
                    // Refuse the request
                        ...
                        throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
                    }
                }
                // Request in Queue
                else if (senderLoadStatus == 1) {
                    final String strArr[] = { _receiverMSISDN, String.valueOf(_p2pTransferVO.getRequestedAmount()) };
                    throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.REQUEST_IN_QUEUE, 0, strArr, null);
                }
                // Refuse the request
                else {
                    throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.REQUEST_REFUSE_FROM_INTLOAD);
    ...
    public void processFromQueue(TransferVO p_transferVO) {
        ...
        try {
            ...
            if (_type.equals(PretupsI.INTERFACE_CATEGORY_BOTH)) {
                ...
                if (_serviceInterfaceRoutingVO != null) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                        "PrepaidControllerMultTransfer[processFromQueue]", "", _senderMSISDN, _senderNetworkCode,
                        "Service Interface Routing control Not defined, thus using default type=" + ((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.TRSFR_DEF_SRVCTYPE)));
                    _type = ((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.TRSFR_DEF_SRVCTYPE));
                    // Changed on 27/05/07 for Service Type selector Mapping
                ...
                if (_serviceInterfaceRoutingVO != null) {
                    ...
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                        "PrepaidControllerMultTransfer[processFromQueue]", "", _senderMSISDN, _senderNetworkCode,
                        "Service Interface Routing control Not defined, thus using default selector=" + _oldDefaultSelector);
                }
            }
			...
			mcomCon = new MComConnection();
			con = mcomCon.getConnection();
            // Loading receiver's control parameters
			...
			if (mcomCon != null) {
				mcomCon.close("PrepaidControllerMultTransferThread#processFromQueue");
				mcomCon = null;
			}
            con = null;
            ...
            if (_log.isDebugEnabled()) {
                _log.debug("PrepaidControllerMultTransfer[processFromQueue]", "_transferID=" + _transferID + " Successfully through load");
            }
            processValidationRequest();
            // Set under process message for the sender and receiver
            ...
            try {
                ...
                _log.errorTrace(methodName, bex);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR, "PrepaidControllerMultTransfer[processFromQueue]",
            ...
            TransactionLog.log(_transferID, _requestIDStr, _requestVO.getFilteredMSISDN(), "", PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS, _p2pTransferVO
                .getSenderReturnMessage(), PretupsI.TXN_LOG_STATUS_FAIL, "Getting Code=" + _requestVO.getMessageCode());
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[processFromQueue]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
        } finally {
            try {
                if (mcomCon == null) {
    ...
    private void creditBackSenderForFailedTrans(CommonClient p_commonClient, boolean p_onlyDecreaseOnly) throws BTSLBaseException {
        ...
        try {
                ...
                if (((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_SNDR_CREDIT_BACK_ALLOWED))).booleanValue()) {
                    if ((((Boolean) (PreferenceCache.getSystemPreferenceValue(PreferenceI.P2P_SNDR_CREDIT_BK_AMB_STATUS))).booleanValue() && _p2pTransferVO.getTransferStatus().equalsIgnoreCase(PretupsErrorCodesI.TXN_STATUS_AMBIGUOUS)) || _p2pTransferVO
                        ...
                        if (isCounterToBeDecreased) {
                            SubscriberBL.decreaseTransferOutCounts(con, _p2pTransferVO,PretupsI.MULT_CRE_TRA_DED_ACC);
                        }
                        ...
                        EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                            "PrepaidControllerMultTransfer[processFromQueue]", _transferID, _senderMSISDN, _senderNetworkCode,
                            "Credit back not required in case of Ambigous cases");
                    }
                } else {
                    EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                        "PrepaidControllerMultTransfer[creditBackSenderForFailedTrans]", _transferID, _senderMSISDN, _senderNetworkCode,
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[creditBackSenderForFailedTrans]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            TransactionLog.log(_transferID, _requestIDStr, _senderMSISDN, "", PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
			...
			if (mcomCon != null) {
				mcomCon.close("PrepaidControllerMultTransferThread#creditBackSenderForFailedTrans");
				mcomCon = null;
			}
        }
    ...
    private void processValidationRequestInThread() throws BTSLBaseException, Exception {
        ...
        try {
            ...
            _log.errorTrace(methodName, be);
            _log.error("PrepaidControllerMultTransfer[processValidationRequestInThread]", "Getting BTSL Base Exception:" + be.getMessage());
            TransactionLog.log(_transferID, _requestIDStr, _senderMSISDN, _senderNetworkCode, PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
            ...
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[processValidationRequestInThread]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            TransactionLog.log(_transferID, _requestIDStr, _senderMSISDN, _senderNetworkCode, PretupsI.TXN_LOG_REQTYPE_RES, PretupsI.TXN_LOG_TXNSTAGE_PROCESS,
            ...
            if (_transferID != null && !_transferDetailAdded) {
                ...
                try {
                    ...
                    if (_p2pTransferVO.getTransferStatus() == null) {
                        _p2pTransferVO.setTransferStatus(PretupsErrorCodesI.TXN_STATUS_FAIL);
                    }
					...
					if (mcomCon != null) {
						mcomCon.close("PrepaidControllerMultTransferThread#processValidationRequestInThread");
						mcomCon = null;
					}
                    con = null;
                }
    ...
    private String getReceiverUnderProcessMessage() {
        ...
        return BTSLUtil.getMessage(_receiverLocale, PretupsErrorCodesI.P2P_RECEIVER_UNDERPROCESS, messageArgArray);
    }
    ...
    private String getSenderCreditBackMessage() {
        if (BTSLUtil.isNullString(_senderCreditPostBalanceAvailable) || "Y".equals(_senderCreditPostBalanceAvailable)) {
            ...
            return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.P2P_SENDER_CREDIT_BACK, messageArgArray);
        }
        final String[] messageArgArray = { _receiverMSISDN, _transferID, PretupsBL.getDisplayAmount(_p2pTransferVO.getRequestedAmount()) };
        return BTSLUtil.getMessage(_senderLocale, PretupsErrorCodesI.P2P_SENDER_CREDIT_BACK_WITHOUT_POSTBAL, messageArgArray);
    }
    ...
    private boolean getInterfaceRoutingDetails(Connection p_con, String p_msisdn, long p_prefixID, String p_subscriberType, String p_networkCode, String p_serviceType, String p_interfaceCategory, String p_userType, String p_action) throws BTSLBaseException {
        ...
        try {
            if (subscriberRoutingControlVO != null) {
                ...
                // based routing
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                    "PrepaidControllerMultTransfer[getInterfaceRoutingDetails]", _transferID, _senderMSISDN, _senderNetworkCode,
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[getInterfaceRoutingDetails]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            isSuccess = false;
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        }
        if (_log.isDebugEnabled()) {
        ...
        try {
            if (altList != null && !altList.isEmpty()) {
                ...
                case 1: {
					...
					if (mcomCon != null) {
						mcomCon.close("PrepaidControllerMultTransferThread#performSenderAlternateRouting");
						mcomCon = null;
					}
                    con=null;
                    ...
                    try {
                        ...
                        if (be.isKey() && InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(be.getMessageKey())) {
                            ...
                            try {
                                ...
                                _log.errorTrace(methodName, e);
                                throw new BTSLBaseException(this, methodName, "Exception in performing the sender alternate intreface routing controls.");
                            }
                        } else {
                            throw be;
                        }
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[performSenderAlternateRouting]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
			if (mcomCon != null) {
				mcomCon.close("PrepaidControllerMultTransferThread#performSenderAlternateRouting");
        ...
        if (!BTSLUtil.isNullString(p_languageCode)) {
            try {
                if (LocaleMasterCache.getLocaleFromCodeDetails(p_languageCode) == null) {
                    EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                        "PrepaidControllerMultTransfer[updateReceiverLocale]", _transferID, _receiverMSISDN, "",
    ...
    public void performAlternateCategoryRouting() throws BTSLBaseException {
        ...
        try {
            ...
            if (con != null) {
				...
				if (mcomCon != null) {
					mcomCon.close("PrepaidControllerMultTransferThread#performAlternateCategoryRouting");
					mcomCon = null;
				}
                con = null;
            }
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[performAlternateCategoryRouting]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
            if (con != null) {
                try {
    ...
    public void populateAlternateInterfaceDetails(Connection p_con) throws BTSLBaseException {
        ...
        if (!_interfaceCatRoutingDone) {
            ...
            if (_networkPrefixVO != null) {
                ...
                _log.error(this, "Series Not Defined for Alternate Interface =" + _type + " For Series=" + _receiverVO.getMsisdnPrefix());
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.INFO,
                    "PrepaidControllerMultTransfer[populateAlternateInterfaceDetails]", "", "", "",
    ...
    private void performReceiverAlternateRouting(ArrayList altList, int p_source) throws BTSLBaseException {
        ...
        try {
            if (altList != null && !altList.isEmpty()) {
                ...
                case 1: {
                    ...
                    try {
                        ...
                        _log.errorTrace(methodName, e);
                        throw new BTSLBaseException(this, methodName, "Exception in perform the Interface routing for the subscriber MSISDN when alt size is 1");
                    }
                    ...
                    try {
                        ...
                        if (be.isKey() && InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(be.getMessageKey())) {
                            ...
                            try {
                                ...
                                _log.errorTrace(methodName, e);
                                throw new BTSLBaseException(this, methodName, "Exception in perform the Interface routing for the subscriber MSISDN.");
                            }
                        } else {
                            throw be;
                        }
                        ...
                        _log.errorTrace(methodName, e);
                        throw new BTSLBaseException(this, methodName, "Exception in perform the Interface routing for the subscriber MSISDN when alt size is 2");
                    }
                    break;
                }
                }
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[performAlternateRouting]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        }
        if (_log.isDebugEnabled()) {
            _log.debug(methodName, _requestIDStr, " Exiting ");
    ...
    public void receiverValidateResponse(String str, int p_attempt, int p_altSize, int p_source) throws BTSLBaseException {
        ...
        if (InterfaceErrorCodesI.INTERFACE_MSISDN_NOT_FOUND.equals(status) && p_attempt == 1 && p_attempt < p_altSize) {
            ...
            if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
                ...
                strArr = new String[] { _receiverMSISDN, PretupsBL.getDisplayAmount(_p2pTransferVO.getRequestedAmount()), _transferID };
                throw new BTSLBaseException("PrepaidControllerMultTransfer", "receiverValidateResponse", PretupsErrorCodesI.P2P_SENDER_FAIL, 0, strArr, null);
            }
            _receiverTransferItemVO.setTransferStatus(status);
            _receiverVO.setTransactionStatus(PretupsErrorCodesI.TXN_STATUS_SUCCESS);
            _receiverTransferItemVO.setSubscriberType(_type);
    ...
    public void populateReceiverItemsDetails(HashMap p_map) throws BTSLBaseException {
        ...
        if (!BTSLUtil.isNullString((String) p_map.get("IN_TXN_ID"))) {
            try {
                ...
                _log.errorTrace(METHOD_NAME, e);
                EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
                    "PrepaidControllerMultTransfer[updateForReceiverValidateResponse]", _transferID, _senderMSISDN, _senderNetworkCode,
        ...
        if (BTSLUtil.isNullString(status) || !status.equals(InterfaceErrorCodesI.SUCCESS)) {
            ...
            strArr = new String[] { _receiverMSISDN, PretupsBL.getDisplayAmount(_p2pTransferVO.getRequestedAmount()), _transferID };
            // throw new
            // BTSLBaseException("PrepaidControllerMultTransfer","updateForReceiverValidateResponse",PretupsErrorCodesI.P2P_SENDER_FAIL,0,strArr,null);
            // throw new
            // BTSLBaseException("PrepaidControllerMultTransfer","populateReceiverItemsDetails",_p2pTransferVO.getErrorCode(),0,strArr,null);
            if (InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED_P2P.equals(_receiverTransferItemVO.getValidationStatus())) {
                throw new BTSLBaseException("PrepaidControllerMultTransfer", "populateReceiverItemsDetails",
                    InterfaceErrorCodesI.INTERFACE_CUSTOMER_RECHARGENOTALLOWED_P2P + "_S", 0, strArr, null);
            } else {
                throw new BTSLBaseException("PrepaidControllerMultTransfer", "populateReceiverItemsDetails", _p2pTransferVO.getErrorCode(), 0, strArr, null);
            }
        }
        _receiverTransferItemVO.setTransferStatus(status);
        _receiverTransferItemVO.setSubscriberType(_type);
        ...
        // Dedicated account information
        _receiverTransferItemVO.setDedicatedAccountID((String) p_map.get("DEDICATED_ACC_ID"));
        _receiverTransferItemVO.setDedicatedAccountValues((String) p_map.get("DEDICATED_ACC_VALUE"));
        _receiverTransferItemVO.setDedicatedAccountExpiry((String) p_map.get("DEDICATED_ACC_EXPIRY"));
    }
    ...
    private void setInterfaceDetails(long p_prefixID, String p_userType, ListValueVO p_listValueVO, boolean p_useInterfacePrefixVO, MSISDNPrefixInterfaceMappingVO p_MSISDNPrefixInterfaceMappingVO, ServiceSelectorInterfaceMappingVO p_serviceSelectorInterfaceMappingVO) throws BTSLBaseException {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL, "PrepaidControllerMultTransfer[setInterfaceDetails]",
                _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled()) {
    ...
    private void updateSubscriberRoutingDetails(String p_userType, String p_networkCode, String p_interfaceID, String p_externalID, String p_msisdn, String p_interfaceCategory, String p_userID, Date p_currentDate) throws BTSLBaseException {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[updateSubscriberRoutingDetails]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug(methodName, _requestIDStr, " Exiting ");
    ...
    public static synchronized void generateTransferID(TransferVO p_transferVO) throws BTSLBaseException {
        ...
        try {
            ...
            if (_transactionIDCounter == 0) {
                throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
            transferID = _operatorUtil.formatMultP2PTransferID(p_transferVO, _transactionIDCounter);
            if (transferID == null) {
                throw new BTSLBaseException("PrepaidControllerMultTransfer", methodName, PretupsErrorCodesI.NOT_GENERATE_TRASNFERID);
            }
    ...
    public void handleLDCCRequest() throws BTSLBaseException {
        ...
        try {
			...
			if (mcomCon != null) {
				mcomCon.close("PrepaidControllerMultTransferThread#handleLDCCRequest");
				mcomCon = null;
			}
            con=null;
            requestStr = getSenderValidateStr();
    ...
    private void updateSubscriberAilternateRouting(String p_userType, String p_networkCode, String p_interfaceID, String p_externalID, String p_msisdn, String p_interfaceCategory, String p_userID, Date p_currentDate) throws BTSLBaseException {
        ...
        try {
            ...
            _log.errorTrace(methodName, e);
            EventHandler.handle(EventIDI.SYSTEM_ERROR, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.FATAL,
                "PrepaidControllerMultTransfer[updateSubscriberAilternateRouting]", _transferID, _senderMSISDN, _senderNetworkCode, "Exception:" + e.getMessage());
            throw new BTSLBaseException(this, methodName, PretupsErrorCodesI.P2P_ERROR_EXCEPTION);
        } finally {
            if (_log.isDebugEnabled()) {
                _log.debug(methodName, _requestIDStr, " Exiting ");
     ...
     * Method updated for notification message using service class date 15/05/06
     */
    private String getSenderFinalMessage(Locale p_senderLocale, String p_Msisdn, int p_succReq, int p_totalReq, boolean p_isSuccess) {
        if (_log.isDebugEnabled()) {
            _log.debug("getSenderFinalMessage", " p_senderLocale=" + p_senderLocale.getDisplayName() + " p_Msisdn=" + p_Msisdn + " p_isSuccess=" + p_isSuccess);
        }
        String key = null;
        String[] messageArgArray = null;

        if (p_isSuccess) {
            key = PretupsErrorCodesI.SUCCESS_DEDICATED_ACC;
            messageArgArray = new String[] { String.valueOf(p_succReq), String.valueOf(p_totalReq) };
        } else {
            key = PretupsErrorCodesI.FAIL_DEDICATED_ACC;
            messageArgArray = new String[] { p_Msisdn };
        }
    ...
    /***
     * 
     * Method updated for notification message
     */
    private String getReceiverSuccessMessage() {
        String[] messageArgArray = null;
        String key = null;
        if (_log.isDebugEnabled()) {
            _log.debug("getSenderFinalMessage", " _newDefaultSelector=" + _newDefaultSelector + " _oldDefaultSelector=" + _oldDefaultSelector);
        }

        if ("5".equals(_newDefaultSelector)) {
            key = PretupsErrorCodesI.P2P_RECEIVER_SUCCESS_DEDICATED_ACC_MINUTES;
        } else {
            key = PretupsErrorCodesI.P2P_RECEIVER_SUCCESS_DEDICATED_ACC_EGP;
        }
        // Transaction {0}: Dear plus customer you have received {1} {2}.
        messageArgArray = new String[] { _transferID, PretupsBL.getDisplayAmount(_receiverTransferItemVO.getTransferValue()) };
