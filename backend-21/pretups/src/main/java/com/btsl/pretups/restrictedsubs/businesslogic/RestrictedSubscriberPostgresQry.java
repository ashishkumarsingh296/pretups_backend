...
public class RestrictedSubscriberPostgresQry implements RestrictedSubscriberQry{

	private String className = "RestrictedSubscriberPostgresQry";
	@Override
	public PreparedStatement loadScheduleBatchDetailsListQry(Connection p_con,String p_batch_id,String p_statusUsed, String p_status) throws SQLException {
		...
		selectSQL.append(" FROM scheduled_batch_detail SBD left outer join key_values KV on ( SBD.transfer_status = KV.key AND KV.type = ? ), scheduled_batch_master SBM, restricted_msisdns RM, lookups L ");
	...
	public PreparedStatement loadBatchDetailVOListQry(Connection p_con,
			String p_batchID, String p_statusUsed, String p_status,String batchType)
			throws SQLException {
		String mathodName = className+"#loadBatchDetailVOListQry";
		 StringBuilder strBuff = new StringBuilder("SELECT SBD.batch_id, SBD.subscriber_id, SBD.msisdn, SBD.amount, SBD.processed_on, SBD.status,");
	        ...
	        strBuff.append(" FROM scheduled_batch_detail SBD left outer join  key_values KV on ( SBD.transfer_status = KV.key and  KV.type = ? ) ,scheduled_batch_master SBM,lookups L, ");
	        if(PretupsI.BATCH_TYPE_CORPORATE.equalsIgnoreCase(batchType)) {
