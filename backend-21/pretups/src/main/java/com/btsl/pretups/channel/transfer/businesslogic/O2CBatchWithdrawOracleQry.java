...
public class O2CBatchWithdrawOracleQry implements O2CBatchWithdrawQry{

	private Log log = LogFactory.getLog(this.getClass());
	
	private String databasetypeCheckForDB2 = "databasetype";
	...
	public String loadBatchO2CMasterDetailsQry(String currentLevel, String itemStatus) {
        ...
        strBuff.append(" GROUP BY fb.batch_id,fb.batch_name,fb.batch_total_record, p.product_name,p.unit_value,fb.network_code,fb.network_code_for,fb.created_by,fb.product_code, fb.modified_by, fb.modified_on,fb.sms_default_lang,fb.sms_second_lang, fb.txn_wallet,p.product_type,fbg.geography_code,p.short_name ,fb.domain_code,fb.batch_date ORDER BY fb.batch_date DESC ) ");
	...
	public String closeOrderByBatchQry() {
		...
		if (multipleWalletApply) {
            ...
            if (PretupsI.DATABASE_TYPE_DB2.equalsIgnoreCase(Constants.getProperty(databasetypeCheckForDB2))) {
                sqlBuffer.append("TRUNC(daily_stock_updated_on) <> TRUNC(?) FOR UPDATE OF foc_stock WITH RS");
            } else {
                sqlBuffer.append("TRUNC(daily_stock_updated_on) <> TRUNC(?) FOR UPDATE OF foc_stock");
            }
            ...
            sqlBuffer.append("WHERE network_code = ? AND network_code_for = ? AND ");
            if (PretupsI.DATABASE_TYPE_DB2.equalsIgnoreCase(Constants.getProperty(databasetypeCheckForDB2))) {
                sqlBuffer.append("TRUNC(daily_stock_updated_on) <> TRUNC(?) FOR UPDATE OF stock WITH RS");
            } else {
                sqlBuffer.append("TRUNC(daily_stock_updated_on) <> TRUNC(?) FOR UPDATE OF stock");
            }
        }
	...
	public String closeOrderByBatchSelectNetworkStockQry() {
		...
		if (multipleWalletApply) {
	            ...
	            sqlBuffer.append(" FROM network_stocks ");
	            if (PretupsI.DATABASE_TYPE_DB2.equalsIgnoreCase(Constants.getProperty(databasetypeCheckForDB2))) {
	                sqlBuffer.append(" WHERE network_code = ? AND product_code = ? AND network_code_for = ? FOR UPDATE OF foc_stock WITH RS");
	            } else {
	                sqlBuffer.append(" WHERE network_code = ? AND product_code = ? AND network_code_for = ? FOR UPDATE OF foc_stock ");
	            }
	        } else {
	            sqlBuffer = new StringBuilder(" SELECT ");
	            ...
	            sqlBuffer.append(" FROM network_stocks ");
	            if (PretupsI.DATABASE_TYPE_DB2.equalsIgnoreCase(Constants.getProperty(databasetypeCheckForDB2))) {
	                sqlBuffer.append(" WHERE network_code = ? AND product_code = ? AND network_code_for = ? FOR UPDATE OF stock WITH RS ");
	            } else {
	...
	public String closeOrderByBatchSelectUserBalancesQry() {
		StringBuilder sqlBuffer = new StringBuilder(" SELECT user_id, network_code, network_code_for, product_code, balance, prev_balance, ");
	        ...
	        LogFactory.printLog("closeOrderByBatchSelectUserBalances", sqlBuffer.toString(), log);
		return sqlBuffer.toString();
	}
