...
public class UserReportPostgresQry  implements UserReportQry{

	@Override
	public PreparedStatement loadUserClosingBalanceQry(Connection con, String networkCode, String zone, String domainCode, String categoryCode, String userId, String loginUserId, Date formDate, Date toDate, String fromAmt, String toAmt, String userType) throws SQLException, BTSLBaseException {
		...
		if (PretupsI.USER_TYPE_CHANNEL.equals(userType)) {
			...
			strUserBal.append(" ,(WITH RECURSIVE q AS ( ");
			strUserBal.append(" SELECT user_id FROM USERS WHERE ");
			strUserBal.append("  user_id=?  ");
			strUserBal.append(" UNION ALL");
			strUserBal.append(" SELECT u1.user_id FROM USERS u1 ");
			strUserBal.append(" join q on q.user_id = u1.parent_id ");
			strUserBal.append(" ) ");
			strUserBal.append(" SELECT user_id FROM q");
			strUserBal.append(" WHERE user_id=CASE ? WHEN 'ALL' THEN user_id ELSE ? END ");
			strUserBal.append( ")X ");
		...
		strUserBal.append("AND UG.grph_domain_code IN (");

		strUserBal.append("WITH RECURSIVE q AS ( ");
		strUserBal.append("SELECT grph_domain_code, status FROM GEOGRAPHICAL_DOMAINS WHERE  ");
		strUserBal.append("grph_domain_code IN  ");
		strUserBal.append("(SELECT grph_domain_code  FROM USER_GEOGRAPHIES  ");
		strUserBal.append("UG1 WHERE ug1.GRPH_DOMAIN_CODE=CASE ? WHEN 'ALL' THEN UG1.grph_domain_code ELSE ? END  AND UG1.user_id=? ) "); 
		strUserBal.append("UNION ALL ");
		strUserBal.append("SELECT gd.grph_domain_code, gd.status FROM GEOGRAPHICAL_DOMAINS gd  ");
		strUserBal.append(" join q on q.grph_domain_code = gd.parent_grph_domain_code  ");
		strUserBal.append(") ");
		strUserBal.append("SELECT grph_domain_code FROM q ");
		strUserBal.append(" WHERE status IN ('Y','S') ");
