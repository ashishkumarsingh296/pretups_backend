...
public class ServiceKeywordCacheVO implements Serializable {
    ...
    private transient ArrayList _subKeywordList = null;
    private String _requestParam = null;
    private String _responseParam = null;
    private boolean _servUderPrcsChkReqrd = false;
    ...
    public ServiceKeywordCacheVO(ServiceKeywordCacheVO cache) {
        ...
        _responseParam = cache.getResponseParam();
        _servUderPrcsChkReqrd = cache.getServUderPrcsChkReqrd();
    }
    ...
    public String getServiceTypeKeyword() {
        ...
        temp.append(this.getServiceType());
        temp.append("_");
        temp.append(this.getKeyword());
...
@Override
    public String toString() {
        ...
        strBuild.append(" keyword  ").append(_keyword);
        strBuild.append(" module  ").append(_module);
        strBuild.append(" requestInterfaceType  ").append(_requestInterfaceType);
        strBuild.append(" serverPort  ").append(_serverPort);
        strBuild.append(" serviceType  ").append(_serviceType);
        strBuild.append(" requestHandlerClass  ").append(_requestHandlerClass);
        strBuild.append(" errorKey  ").append(_errorKey);
        strBuild.append(" allowedVersion  ").append(_allowedVersion);
        strBuild.append(" status  ").append(_status);
        strBuild.append(" type  ").append(_type);
        strBuild.append(" _externalInterface  ").append(_externalInterface);
        strBuild.append(" _flexible  ").append(_flexible);
        strBuild.append(" _name  ").append(_name);
        strBuild.append(" _messageFormat  ").append(_messageFormat);
        strBuild.append(" modifiedServiceType  ").append(_modifiedServiceType);
        strBuild.append(" modifiedOnServiceKeyword  ").append(_modifiedOnServiceKeyword);
        strBuild.append(" _useInterfaceLanguage  ").append(_useInterfaceLanguage);
        strBuild.append(" _groupType  ").append(_groupType);
        strBuild.append(" _subKeywordApplicable  ").append(_subKeywordApplicable);
        strBuild.append(" _subKeyword  ").append(_subKeyword);
        strBuild.append(" _requestParam  ").append(_requestParam);
        strBuild.append(" _responseParam  ").append(_responseParam);
        strBuild.append(" _servUderPrcsChkReqrd  ").append(_servUderPrcsChkReqrd);
        return strBuild.toString();
    }

    public boolean equalsServiceKeywordCacheVO(ServiceKeywordCacheVO p_serviceCacheVO) {
        boolean flag = false;

        if (this.getModifiedOnServiceKeywordTimestamp().equals(p_serviceCacheVO.getModifiedOnServiceKeywordTimestamp())&&this.getModifiedServiceTypeTimestamp().equals(p_serviceCacheVO.getModifiedServiceTypeTimestamp())) {
...
public native boolean equals(Object obj);
    ...
    public String differences(ServiceKeywordCacheVO p_serviceCacheVO) {
        ...
        if (!BTSLUtil.isNullString(this.getServiceType()) && !this.getServiceType().equals(p_serviceCacheVO.getServiceType())) {
            ...
            strBuild.append(p_serviceCacheVO.getServiceType());
            strBuild.append(middleSeperator);
        ...
        if (!BTSLUtil.isNullString(this.getRequestHandlerClass()) && !this.getRequestHandlerClass().equals(p_serviceCacheVO.getRequestHandlerClass())) {
            strBuild.append(startSeperator);
            strBuild.append("Request Handler Class");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getRequestHandlerClass());
            strBuild.append(middleSeperator);
            strBuild.append(this.getRequestHandlerClass());
        }

        if (!BTSLUtil.isNullString(this.getErrorKey()) && !this.getErrorKey().equals(p_serviceCacheVO.getErrorKey())) {
            strBuild.append(startSeperator);
            strBuild.append("Error Key");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getErrorKey());
            strBuild.append(middleSeperator);
            strBuild.append(this.getErrorKey());
        }

        if (!BTSLUtil.isNullString(this.getAllowedVersion()) && !this.getAllowedVersion().equals(p_serviceCacheVO.getAllowedVersion())) {
            strBuild.append(startSeperator);
            strBuild.append("Allowed Version");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getAllowedVersion());
            strBuild.append(middleSeperator);
            strBuild.append(this.getAllowedVersion());
        }
        ...
        if (!BTSLUtil.isNullString(this.getName()) && !this.getName().equals(p_serviceCacheVO.getName())) {
            ...
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getName());
            strBuild.append(middleSeperator);
        ...
        if (!BTSLUtil.isNullString(this.getMessageFormat()) && !this.getMessageFormat().equals(p_serviceCacheVO.getMessageFormat())) {
            strBuild.append(startSeperator);
            strBuild.append("MessageFormat");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getMessageFormat());
            strBuild.append(middleSeperator);
            strBuild.append(this.getMessageFormat());
        }
        ...
        if (!BTSLUtil.isNullString(this.getType()) && !this.getType().equals(p_serviceCacheVO.getType())) {
            ...
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getType());
            strBuild.append(middleSeperator);
            strBuild.append(this.getType());
        }
        if (!BTSLUtil.isNullString(this.getExternalInterface()) && !this.getExternalInterface().equals(p_serviceCacheVO.getExternalInterface())) {
            strBuild.append(startSeperator);
            strBuild.append("External Interface");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getExternalInterface());
            strBuild.append(middleSeperator);
            strBuild.append(this.getExternalInterface());
        }

        if (!BTSLUtil.isNullString(this.getFlexible()) && !this.getFlexible().equals(p_serviceCacheVO.getFlexible())) {
            strBuild.append(startSeperator);
            strBuild.append("Flexible");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getFlexible());
            strBuild.append(middleSeperator);
            strBuild.append(this.getFlexible());
        }
        if (!BTSLUtil.isNullString(this.getStatus()) && !this.getStatus().equals(p_serviceCacheVO.getStatus())) {
            strBuild.append(startSeperator);
            strBuild.append("Status");
            strBuild.append(middleSeperator);
            strBuild.append(p_serviceCacheVO.getStatus());
            strBuild.append(middleSeperator);
    ...
    public void setResponseParam(String param) {
        _responseParam = param;
    }

    public boolean getServUderPrcsChkReqrd() {
        return _servUderPrcsChkReqrd;
    }

    public void setServUderPrcsChkReqrd(boolean uderPrcsChkReqrd) {
        _servUderPrcsChkReqrd = uderPrcsChkReqrd;
    }
}
