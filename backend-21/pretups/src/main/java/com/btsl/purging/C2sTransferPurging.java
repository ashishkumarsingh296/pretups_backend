...
public class C2sTransferPurging {
    ...
    private static Log _logger = LogFactory.getLog(C2sTransferPurging.class.getName());

    /**
	 * to ensure no class instantiation 
	 */
    private C2sTransferPurging() {
    ...
    public static void main(String[] arg) {
        ...
        try {
            ...
            if (!constantsFile.exists()) {
                System.out.println(" Consts File Not Found ");
                return;
            }
        ...
        try {
            ...
            System.out.print(" INFO: Have you verified the files before deleting the data:  ");
            ...
            if ((BTSLUtil.NullToString(data).trim().toUpperCase().indexOf("Y") != -1) && (BTSLUtil.NullToString(data).length() == 1)) {
                System.out.println(" QUES.: Are you sure you want to perform this operation? ");
            ...
            archiveDoneNetworkList = ArchivePretupsUtil.getArchiveDoneNetworks(con, PretupsI.CHNL_TRA, PretupsI.CHNL_TRA_ITEMS);
            ...
            for (i = 0; i < networkListSize; i++) {
                ...
                for (int j = 0; j < archiveDoneNetworkListSize; j++) {
                    if (_networkCode.equalsIgnoreCase(archiveDoneNetworkList.get(j).toString())) {
                        ...
                        DatabasePurgingLog.purgeLog("C2sTransferPurging", "NETWORK CHECK FAIL");
                        _logger.error(methodName, "Network mismatch occured.");
                    }
                }
            }
            DatabasePurgingLog.purgeLog("C2sTransferPurging", "NETWORK CHECK SUCCESFUL");
            ...
            //String retentionStr = new String("SELECT retention_period from TABLES_PURGING WHERE table_name=?");
            retentionPstmt = con.prepareStatement("SELECT retention_period from TABLES_PURGING WHERE table_name=?");
           
            retentionPstmt.setString(1, PretupsI.C2S_TRA);
            ...
            if (dateTillKeepData == null) {
                ...
                if (dateTillKeepDataSql.compareTo(_endDateSql) <= 0) {
                    ...
                    // Check for the c2s transfer data that is available
                    status = checkC2sTransferItemsRecords(con, _startDateSql, _endDateSql);
                    if (status == 0) {
                        if (_logger.isDebugEnabled()) {
    ...
    private static int checkC2sTransferItemsRecords(Connection p_con, java.sql.Date p_fromDate, java.sql.Date p_toDate) {
    	//local_index_missing
        final String methodName = "checkC2sTransferItemsRecords";
        if (_logger.isDebugEnabled()) {
        ...
        StringBuffer strBuffCheck = new StringBuffer("SELECT min(trunc( CT.transfer_date)) MINDATE");
        strBuffCheck.append(" FROM C2s_transfers CT ");
        try {
            lastDayUptoArchived = getLastArchivedDate(p_con, PretupsI.C2S_TRA, PretupsI.C2S_TRA_ITEMS);
            if (_logger.isDebugEnabled()) {
            ...
            if (lastDayUptoArchived == null) {
                ...
                _messageToWrite = "No C2s transfer Data to purge as none is written in file, Please make the file of data for backUp ";
                returnStatus = 1;
                return returnStatus;
            }
            ...
            if (rs.next()) {
                ...
                if (checkDate != null && checkDate.before(_startDateSql)) {
                    ...
                    returnStatus = 2;
                    DatabasePurgingLog.purgeLog("C2sTransferPurging", "DATA FILE HAS NOT BEEN WRITTEN");
                    return returnStatus;
                }
            }
            ...
            if (dateCompare == 0) {
                ...
                deleteUpto = deleteC2sTransferItemsEntries(p_con, _startDateSql, lastDayUptoArchived);
                ...
                deleteUpto = deleteC2sTransferItemsEntries(p_con, _startDateSql, _endDateSql);
                if (deleteUpto == null) {
                    p_con.rollback();
                    if (_logger.isDebugEnabled()) {
						_logger.info(methodName, " No Transfer data deleted");
    ...
    /**
     * This table will delete records from C2s transfer
     * 
     * @param p_con
     *            Date
     * @param p_fromDate
     ...
     * @throws Exception
     */
    private static java.sql.Date deleteC2sTransferItemsEntries(Connection p_con, Date p_fromDate, Date p_toDate) {
        final String methodName = "deleteC2sTransferItemsEntries";
        if (_logger.isDebugEnabled()) {
        ...
        try {
            ...
            DatabasePurgingLog.purgeLog("C2sTransferPurging", "DATA SUCCESSFULLY DELETED");
            if (delCount < 0) {
                if (_logger.isDebugEnabled()) {
            ...
            if (p_con != null) {
                retUpdate = updateLastArchivedOp(p_con, PretupsI.C2S_TRA, PretupsI.C2S_TRA_ITEMS, BTSLUtil.getSQLDateFromUtilDate(p_toDate));
                if (retUpdate > 0) {
                    p_con.commit();
                    if (_logger.isDebugEnabled()) {
						_logger.info(methodName, " Successfully Deleted Transfer Records till Date=" + p_fromDate);
					}
                } else {
                    p_con.rollback();
                    p_fromDate = null;
            ...
            if (_logger.isDebugEnabled()) {
				_logger.info(methodName, " Records deleted=" + noOfRecords + ", Transfer date=" + p_fromDate);
			}
