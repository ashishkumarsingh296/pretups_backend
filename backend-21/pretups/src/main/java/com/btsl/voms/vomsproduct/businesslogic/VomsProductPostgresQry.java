...
public class VomsProductPostgresQry implements VomsProductQry{
	@Override
	public String loadProductDetailsListQry(String pStatusStr, boolean pUseALL,String pType){
		  StringBuilder strBuff = new StringBuilder(" SELECT VP.mrp,VP.product_id,VP.description,VP.talktime,VP.validity,VP.modified_on,VP.category_id,");
	        ...
	        if (pUseALL) {
	            strBuff.append(" WHERE VP.status = case " + pStatusStr + " when 'ALL' then  VP.status else  " + pStatusStr + " end ");
	        } else {
	...
	public String loadProductDetailsListQuery(String pStatusStr,boolean pUseALL, String pType, String networkCode,String voucher_segment){
        ...
        strBuff.append(" AND (VP.CREATED_ON + VP.EXPIRY_PERIOD * (interval '1 day') >= ? ");
        strBuff.append(" OR VP.EXPIRY_DATE >= ?) ");
	...
	public String loadActiveProductDetailsListQry(){
        ...
        if (!(((String) PreferenceCache.getSystemPreferenceValue(PreferenceI.VOMS_DATE_FORMAT))).trim().contains(" ")) {
            strBuff.append(" AND date_trunc('day',vap.applicable_from::TIMESTAMP) > date_trunc('day',?::TIMESTAMP) ");
	...
	public String loadProductsByProductionLocationQry(String psubCategory){
        ...
        strBuff.append(" coalesce(P.individual_entity,'Y') entity, P.attribute1, P.created_by, P.created_on,P.modified_on, P.modified_by ,CAT.type  ");
	...
	public String getProductIDQry(){
        ...
        strBuff.append(" AND vap.applicable_from=(SELECT MAX(applicable_from) FROM voms_active_products WHERE applicable_from<=CURRENT_TIMESTAMP)");
	...
	public String getProductIDQuery(String profileName){
        ...
        // 12-09-2013
        strBuff.append(" AND vap.applicable_from in (SELECT applicable_from FROM voms_active_products WHERE applicable_from<=CURRENT_TIMESTAMP)");
