...
/**
 * VoucherChangeUserId - Update VOMS_BATCH , VOMS_VOUCHERS Table - C2C Voucher
 * Approval Process
 * 
 *
 */
public class VoucherChangeUserId extends Thread{
	...
	public VoucherChangeUserId(ArrayList list) throws BTSLBaseException {
		final String methodName = "VoucherChangeUserId";
		batch_list = list;
		...
		try {
			...
			if (batch_list.size() > 0) {
				...
				for (int i = 0; i < batchlistSize; i++) {
					...
					updateCount = channelTransferDAO.updateVomsVoucherUserId(con, batchVO);
					...
					if (updateCount > 0) {
						...
						batchVO.setMessage("Updated successfully");
						updateCount1 = channelTransferDAO.updateVomsBatchC2C(con, batchVO);
						if (updateCount1 > 0) {
							con.commit();
						} else {
							con.rollback();
