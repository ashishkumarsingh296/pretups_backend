...
public class VoucherConsumptionReportGenerator {
	private static Log _logger = LogFactory.getLog(VoucherConsumptionReportGenerator.class.getName());
    private long seed;
	...
	public static void process() throws BTSLBaseException {
		...
		try {
			...
			ChannelTransferReportDAO channelTransferReportDao =  new ChannelTransferReportDAO();
			System.out.println("Calling VoucherConsumptionReportGenerator");
			channelTransferReportDao.generateVoucherConsumptionReport(con);
			...
			EventHandler.handle(EventIDI.SYSTEM_INFO, EventComponentI.SYSTEM, EventStatusI.RAISED, EventLevelI.MAJOR,
					"VoucherConsumptionReportGenerator[process]", "", "", "",
					" VoucherConsumptionReportGenerator process could not be executed successfully.");
			throw new BTSLBaseException("VoucherConsumptionReportGenerator", METHOD_NAME,
