...
public class OAuthPostgresQry implements OAuthQry {
	...
	public String updateOAuthAccessTokenQry() {
		...
		return "UPDATE oauth_access_token SET TOKEN  = ?::bytea, MODIFIED_ON = ?, TOKEN_ID = ? WHERE TOKEN_ID = ? ";
	}

	public String updateOAuthRefreshTokenQry() {
		return "UPDATE oauth_refresh_token SET TOKEN  = ?::bytea, MODIFIED_ON = ? , TOKEN_ID = ? WHERE TOKEN_ID = ? ";
	}
	...
	public String persistAccessToken() {
		return "INSERT INTO oauth_access_token ( token , scope, created_on, modified_on, expires, client_id, token_id )  VALUES (?::bytea,?,?,?,?,?,?)";
	}

	public String persistRefreshToken() {
		return "INSERT INTO oauth_refresh_token ( token , created_on, modified_on,expires, token_id ) VALUES (?::bytea,?,?,?,?)";
	}
	...
	public String getAccessTokenId() {
		...
		strBuff.append("SELECT OAT.TOKEN_ID  FROM \r\n" +
				...
				"   AND OULI.LOGIN_ID=?  ");// +
				//"   AND (OAT.modified_on + (OAT.expires * interval '1 second')  )  > systimestamp )  ORDER BY OAT.modified_on ASC  ");
